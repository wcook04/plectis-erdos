...
-- `allBaseTotientKernelSeq` is already declared publicly in
-- `Erdos249257.TotientKernelConditional` (same definition: the `(j, r)` base-`k`
-- kernel channel of Euler's totient over `ℚ`, which for `k = 2` is
-- `totientKernelSeq`); this file builds on that rather than redeclaring it.
