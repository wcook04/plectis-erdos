...
def truncatedMersenneWeight (s d : ℕ) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.truncatedMersenneWeight s d

def seamSubsetTarget (s : ℕ) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.seamSubsetTarget s

def seamWeightsFrom (s : ℕ) (d : ℕ) : List ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.seamWeightsFrom s d
...
def weightedBoolSum (weights : List ℕ) (bits : List Bool) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.weightedBoolSum weights bits

def integerGreedyBits (weights : List ℕ) (C : ℕ) : List Bool :=
  Erdos249257.HalfCylinderIntegerGreedy.integerGreedyBits weights C

def integerGreedyRemainder (weights : List ℕ) (C : ℕ) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.integerGreedyRemainder weights C

def seamIntegerGreedyRemainder (s : ℕ) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.seamIntegerGreedyRemainder s

def rowPulse (s d : ℕ) : ℕ :=
  Erdos249257.HalfCylinderIntegerGreedy.rowPulse s d
...
noncomputable def seamAdjacentCut (s : ℕ) (hs : 5 ≤ s) : SeamAdjacentCutView where
  successorCarries :=
    (Erdos249257.HalfCylinderIntegerGreedy.seamAdjacentCut s hs).successorCarries
  belowPulse :=
    (Erdos249257.HalfCylinderIntegerGreedy.seamAdjacentCut s hs).belowPulse

def affineRightRunCharge (pulse : ℕ → ℕ) (k : ℕ) : ℕ :=
  Erdos249257.affineRightRunCharge pulse k
