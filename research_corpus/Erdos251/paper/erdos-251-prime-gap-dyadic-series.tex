...
of the prime and gap terms.

Two statements here are unconditional and evaluable on their own.  The first
is an exclusion: an explicit quadratic countermodel $g_n=2(n^2+4n+2)$ with
tails $T_n=2(n+4)^2$ shows that positivity, parity, polynomial growth,
unboundedness, and nonperiodicity of the digit sequence, taken together, do
not force the smallness that the integral-shift mechanism needs.  Every
argument for Problem~\#251 that uses only those five properties of the prime
gaps therefore fails, and this closes a class rather than an instance.  The
construction is an elementary telescope and carries no priority claim; it is
the same species as the telescoping counterexample by which Kova\v{c}
refuted the adjacent variable-denominator conjecture~\cite{kovac2026}, and its
role here is internal, to kill one of our own producers.  The
second is a size bound: any rational representation of the prime series has
denominator at least $2^{39998}$, which exceeds $10^{12041}$.  That figure
comes from $23369$ certified continued-fraction partial quotients over an
$80000$-bit exact rational bracket whose omitted tail is controlled by the
Lean-checked estimate $p_i\le1250(i+1)^4$, and it transfers to the gap series
by the summation-by-parts identity below.  It is an exact computation, not a
Lean theorem, and it is a function of the bit budget.

No irrationality result is proved.  The principal partial result keeps the actual
prime-gap tail.  If the dyadic gap series were rational, every rescaled real
tail would be represented by one rational recurrence state; its denominator
dynamics then force one positive shift to be integral eventually.  Actual
prime-gap nonperiodicity supplies the mismatch side of the checked adjacent-
shift obstruction; a cofinal smallness estimate would then contradict that
eventual integrality and finish the argument, but the estimate is not proved.
The quadratic polynomial-shift countermodel
shows why positivity, parity, polynomial growth, unboundedness, and
nonperiodicity alone cannot supply it.

For orientation, summation by parts, with the endpoint retained,
yields
\[
...
\]
This exact identity transfers Problem~\#251 to the dyadic gap series; it is a
useful coordinate, not the principal advance.
...
\textbf{Actual-tail reduction.} Hypothetical rationality forces an eventual
positive integral shift of the concrete prime-gap tail; cofinal adjacent small
mismatches would contradict it.  \textbf{Strategy boundary.} The quadratic
polynomial-shift countermodel shows that positivity, parity, polynomial
growth, unboundedness, and nonperiodicity do not produce the needed smallness.
\textbf{Exact coordinate.} Summation by parts identifies the prime series with
two plus the dyadic prime-gap series, and tail rationality admits exact shift
and LCM-diagonal classifiers.  \textbf{Open boundary.}
