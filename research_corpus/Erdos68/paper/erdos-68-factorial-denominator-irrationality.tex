...
\subtitle{A smoothness bound, a size bound, an adjacent-unit no-go, and exact
rationality coordinates for Erd\H{o}s Problem~\#68}
\author{Will Cook}
\date{31 August 2026}
...
\]
is irrational.  This problem remains open.  What is proved here are two
exclusions on any displayed rational representation $\Ser=a/q$ with $q>0$, and
they are incomparable.  The first is a smoothness exclusion,
\[
  q\nmid299999! ,
\]
so no $299999$-smooth integer is the reduced denominator, however large.  The
second is a size exclusion,
\[
  q\ge2^{39991}>10^{12039},
\]
which says nothing about smoothness.  Neither implies the other, and together
they require any displayed denominator to exceed $10^{12039}$ and to carry a
prime factor above $299999$, or a prime power beyond its multiplicity in
$299999!$.  The first rests on a Lean-checked consumer together with one exact
...
rational bracket.  Both are exact computations, not floating-point estimates,
and neither is a step toward irrationality.

A third unconditional result is a no-go.  Two consecutive unit carries reduce,
after an exact denominator telescope, to the universal inequality
$0<U_{m+2}\le V_{m+2}$.  A divisibility obstruction derived only after assuming
those two carries therefore cannot rule them out, and the adjacent-unit
cancellation route is closed.
...
\textbf{Finite denominator exclusions} & Two incomparable statements: $q\nmid299999!$ from the carry certificate and $q\ge2^{39991}>10^{12039}$ from certified continued fractions. \\
...
\textbf{Moving-factor criterion} & One moving prefix-private prime plus two explicit scale bounds implies irrationality; arbitrary split factors give a stronger normalized-collision criterion, while every fixed owner pair is eventually absorbed.  The cofinal scale and disagreement producers remain open. \\
...
Explicit denominator exclusions & Checked implications; exact finite certificates & Exact reduction gives $60\nmid Z_{60}$, $64\nmid Z_{64}$, and $67\nmid Z_{67}$.  The GMP carry computation gives $q\nmid299999!$ (hence $q\ge300000$); an independent bracket-certified continued-fraction computation gives $q\ge2^{39991}>10^{12039}$.  Neither changes a quantifier. \\
...
$m=500$.  A cofinal floor-stability theorem is still needed.
...
\noindent The proof holds uniformly for every prime.  It supplies, at zero
...
any cofinal family.  Even strict nonvanishing would only
exclude denominators dividing the associated determinant; without a bound
below the rational lattice spacing, it only excludes divisors of the associated
determinant.  Section~\ref{sec:open} states the stronger scalar nonintegrality
condition.
...
excludes every $299999$-smooth denominator, including those much larger than
$300000$.
...
the final convergent's separation from $\Ser$, yielding
$q\ge2^{39991}>10^{12039}$ by best approximation of the second kind.  This
is also a finite exclusion.  It bounds the size of $q$, whereas the carry
calculation excludes $299999$-smooth values of $q$; neither proves the required
...
Consequently, any prime, prime-power, or quotient-gcd obstruction extracted
only after assuming the two carries is just the universal reduced-fraction
bound rewritten through a common factor.  To exclude adjacent unit carries,
one must instead constrain the predecessor state before making that
assumption, for example by Archimedean non-concentration or independent prefix
arithmetic.
...
$\det A$.  To prove irrationality by this method, one would need such
nonvanishing cofinally, together with either a bound below the rational lattice
spacing or an independent contradiction for every possible denominator.
...
factorial gaps and the reduced prefix.
...
raises the finite size exclusion to $q\ge2^{39991}>10^{12039}$; its script and
...
The adjacent-unit window of \S\ref{sec:adjacent-unit-no-go} cannot supply this
after the pair assumption; an argument must constrain the predecessor state
before that assumption.
...
$q\ge2^{39991}>10^{12039}$ by the bracket-certified continued-fraction
...
$q\ge2^{39991}>10^{12039}$.  This exact bracket computation is independent of
