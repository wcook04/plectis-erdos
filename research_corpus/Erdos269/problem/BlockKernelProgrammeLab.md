...
**What this fences.** The forcing phase does not become independent of `S` at large `L` — it
shadows `Q_a S` with error `≍ W/P ≍ poly/8^L`. So any attempt to establish the `q = 1` slice by
asserting that `Ê′` equidistributes mod `P/2` is **circular**: at `q = 1` that assertion *is*
"`Q_a S` is not near an integer", which is the irrationality-flavoured content being sought.
This is why §10's two SPREAD corners are "strictly incomparable" — and it says precisely where
the incomparability comes from. A20 is unaffected: its content is the `q ≠ 1` mass, and the
one-launch bound must be proved for the multipliers `q ≥ 2` where no such shadowing identity
exists. Any future proof that treats `q = 1` on the same footing as generic `q` is wrong.

### 14.9 Where the frontier now stands

```
B > 1  :  SPREAD(m, ½N^{−1/m}, [1,N)) cofinally.
          Now equivalent (A20) to a one-launch half-circle anti-concentration bound,
