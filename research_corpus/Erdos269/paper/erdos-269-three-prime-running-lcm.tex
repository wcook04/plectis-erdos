...
\subtitle{Nonsingular minors of every order, an exact torus--Fourier value, a
denominator floor, and the open boundary for Erd\H{o}s Problem~\#269}
\author{Will Cook}
\date{July 2026}
...
On the size of a displayed denominator for \(\{2,3,5\}\), a certified
continued-fraction prefix excludes every rational representation with
denominator below \(2^{22482}\), which exceeds \(10^{6768}\).  The expansion
is computed from an exact rational bracket, so the exclusion is an exact
computation rather than a Lean theorem, and it is a bit-budget figure rather
than a measure of progress.
...
integral-branch rigidity, and the finite carry contradiction.  Any displayed
denominator exceeds $10^{6768}$.  A
...
Both conclusions are checked:
\lword{Erdos269/RationalLatticeReduction.lean}{196}{qsmul_normalizedTailState_eq_int_of_value_eq_rat}{integrality
of $qX_a$} and
\lword{Erdos269/RationalLatticeReduction.lean}{296}{exists_normalizedTailState_collision_of_value_eq_rat}{the
collision}.
...
window}), and the extinction theorem above consumes them.  No declaration
packages the three into one statement, and none of this proves that the actual
series supplies the factorisation, the carry recurrence, or the cofinal escape
hypothesis.  In particular there is no irrationality theorem here for the
three-prime running-LCM series, whose endpoint remains open.
