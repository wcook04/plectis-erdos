...
\renewcommand{\commit}{ca0e13f8acf5ccf48506e4bdb870953d3a0856fa}
\renewcommand{\commitshort}{ca0e13f8acf5}
\hypersetup{
...
\subtitle{A rational base outside the Bundschuh--V\"a\"an\"anen region,
...
substitution of a rational base directly into its polynomial identity, and the
resulting exponent budget gives the threshold
\(\log b/\log a<C_0/C_1=0.4056830213840605\ldots\) for coprime \(a>b\ge1\).
Since \(\log 4/\log 31<81/200\) lies below it, \(F\bigl((31/4)^r\bigr)\) is
irrational for every \(r\ge1\).  Zudilin announced in 2016 that his results
extend to a rational base under an assumption of exactly this shape, for some
computable constant he did not name; what is supplied here is that constant,
and with it the strip \((0.398678,0.405683]\) above the
Bundschuh--V\"a\"an\"anen region, which contains \(31/4\) and no integer base.
The proof consumes Lemmas~1, 2 and~7 of Zudilin~2004 and nothing else external.
It is not kernel-checked; Lean checks the finite part, namely the trigamma
constants, the bounds \(81/200<\theta^*<1/2\), and the membership and exclusion
of the bases involved.  A companion Archimedean cap shows that every Pad\'e-type
family of this shape is confined to \(b^2<a\), so the same mechanism reaches no
base with \(b^2\ge a\), \(3/2\) included.

Our strongest unconditional theorem sharpens the normalized Hankel estimate in Zudilin's
Pad\'e construction.  At \(x=z=1\), if \(V_N^*\) is his rank-\(N\)
normalized determinant, then for every \(N\ge1\)
\[
 ...
 [q^{\operatorname{ord}_q V_N^*}]V_N^*
   =\frac{(N!)^2(N+1)!}{2^N}.
\]
The published argument proves the displayed order only as a lower bound.  An
associated-graded recurrence identifies every transformed-row leading
coefficient, and a Vandermonde determinant proves equality.  Thus additional
normalized-Hankel \(q\)-order cannot supply the missing analytic decay in this
specific construction.  This is a route closure, not an irrationality result:
denominator-factor extraction and alternative integral models remain open.

Separately, at a rational target $a/q$, prime support in the denominator
coordinate of an integral approximation row forces the sharp gap
$|Ba/q-A|\ge1/q$.  A first-power determinant condition forces that gap in at
least one of two rows, and two noncollinear integral rows cannot both have
remainders tending to zero.  These are exact arithmetic endgames, but their
prime-support and analytic-decay hypotheses are not constructed for the
actual $q$-Ap\'ery family.

At \(3/2\), a second exact input is
\[
 2^{64}<3^{41}<2^{65}.
\]
It yields a uniform gap greater than \(3/13\) between
\(\log 2/\log 3\) and every threshold in the rectangular
Hermite--Pad\'e exponent model considered here.  It also shows that the scalar
factor, and the scalar factor together with the universally forced
first-order border, fall short of the required \(39/41\) charge.  These
inequalities exclude the stated height mechanisms only; they do not exclude
other Pad\'e or determinant constructions.

The same bracket sharpens a finite four-jet count.  At bottom depth
\(R=41T\), any \(M\ge130T+2S\) integral polynomial pairs have two binary
selectors with the same two endpoint jets in both coordinates.  For \(T=1\)
this counting threshold is exact.  A bounded-fibre lemma shows how a uniform
bound on analytic-remainder multiplicities would force one collision outside
the remainder nullspace.  No such bound is proved for an actual
\(q\)-Ap\'ery or Zudilin family.  Under the additional hypothesis that adjacent
...
Two further obstructions explain why direct integer-base arguments do not
transfer automatically to \(3/2\).  Integer scalar content scales local
divisibility and Archimedean determinant height by the same factor, while unit
endpoint coefficients keep both \(2\) and \(3\) out of a common divisor after
specialisation.  A six-condition coordinatewise clearing scheme forces
\(s^{N+K+1}<r(N+K)\); at \(r/s=3/2\) this is impossible for every
\(N,K\ge1\).  The exact cleared-tail recurrence contains the forcing term
\(Bc(N+1)s^{N+1}\), which is at least \(2^{N+1}\) when \(s\ge2\) and the
integer data are positive.

For comparison, Bundschuh and V\"a\"an\"anen's published criterion proves
irrationality for a restricted rational-base family containing \(7/2\); only
its elementary height check is formalised here.  The Pad\'e exponent bounds and
the \(81/200\) logarithmic-region calculations below likewise supply no
analytic remainder estimate at \(3/2\).  The unresolved task is to construct a
primitive, noncollapsed approximation family whose endpoint divisibility
survives normalisation, whose selected polynomial pair and remainder are
nonzero, and whose decay beats its height.
\end{abstract}

\begin{paperresult}{Sharp formula and exact remaining boundary}
\textbf{All-rank formula.} Zudilin's normalized Hankel lower bound is an
equality at $x=z=1$, with first coefficient $(N!)^2(N+1)!/2^N$; hidden
normalized-Hankel order is therefore unavailable as extra analytic decay.
\textbf{Rational gap.} Prime-supported integral rows have a sharp $1/q$
gap, and noncollinear pairs cannot both decay to zero; constructing the
required rows remains open.
...
$c>0$~\cite{zudilin2016}.  No value of $c$ is computed there.
Section~\ref{sec:rational-base-irrationality} computes a threshold of exactly
that shape, $\log s/\log r<C_0/C_1$, from the earlier
direction~\cite{zudilin2004} rather than from the generalized $q$-logarithm.
The remark is therefore both the closest antecedent and the sharpest reason to
withhold any priority claim: the rational-base extension was contemplated in
print, only its constant was left uncomputed.

Section~\ref{sec:hankel-order} returns to the normalized determinant in
Zudilin's Section~4 and sharpens its $q$-order estimate at $x=z=1$.  The
closest recent determinant comparison we found is Chern--Jiu--Li--Wang's
theory of $q$-binomial transforms~\cite{chern2026}.  Their theorems determine
the highest degree and leading coefficient in an independent transform
parameter for two finite $q$-binomial transforms of an arbitrary sequence.
The result below instead determines the lowest $q$-exponent and first
coefficient of Zudilin's particular infinite $q$-hypergeometric moment
sequence after fixing $x=z=1$.  Their paper neither treats this sequence nor
the generalized $q$-logarithm, so its leading-coefficient theorems do not imply
Theorem~\ref{res:zudilin-sharp-qorder}.  This is an exact comparison of theorem
statements, not a priority claim.
No claim of priority is made for the analysis below.  It records the formal
status and limits of the argument; it does not add a theorem about $F(t)$.
...
models.  The normalized-Hankel theorem is an ordinary all-rank proof with a
Lean-checked source-tail, recurrence, and determinant chain plus finite replay,
not yet an end-to-end Lean theorem.  Bundschuh and V\"a\"an\"anen's theorem at \(7/2\),
...
\paragraph{Structure.}
Section~\ref{sec:hankel-order} proves the all-rank normalized-Hankel order and
leading coefficient and states exactly which analytic route it closes.
Section~\ref{sec:sharp} derives the height and charge exclusions from the sharp
$41/65$ power certificate.  Section~\ref{sec:prime-support} proves the sharp
one- and two-row rational gaps, the no-double-decay theorem, the real
unit-determinant height--decay tradeoff, and the reduced selector collision.
Section~\ref{sec:primitive} proves that integer scalar content is neutral for the
...
Hankel determinant; $q$-logarithm; Pad\'e approximation; Lean~4.
\qquad
...
\emph{This section records the state of the argument on 2 September 2026.  It
is an ordinary complete proof consuming three published lemmas of
Zudilin~2004, with the finite part Lean-checked and the whole not
kernel-checked.}

Write $F(t)=\sum_{m\ge1}(t^m-1)^{-1}$ as in the problem statement, and for
coprime integers $a>b\ge1$ put $t=a/b$.  Two published thresholds bound the
rational bases at which irrationality is currently reachable by a height
argument.  Duverney's Th\'eor\`eme~2 \cite[Th.~2, p.~174]{duverney1996} covers
$\log b/\log a<\tfrac13(1-3/\pi^2)=0.23207\ldots$, and Bundschuh and
V\"a\"an\"anen's Theorem~2 in the case $\alpha=-1$ \cite[Thm.~2,
p.~177]{bv1994} covers the larger region
\[
  \frac{\log b}{\log a}<\frac12-\frac1{\pi^2}=0.398678\ldots .
\]
Zudilin's Theorem~1 \cite[Thm.~1]{zudilin2004} gives the irrationality
exponent $\mu=2.46497868\ldots$ for the integer-base $q$-harmonic series in the
direction $(\alpha_0,\alpha_1,\alpha_2;\beta)=(14,12,14;27)$ with $m=15$, and
the closing paragraph of \cite[\S2]{zudilin2016} announces that the results
extend to non-integer $p=r/s$ with $|p|>1$ under an assumption
$\log|r|>c\log|s|$ for some computable $c>0$, without naming $c$ and without
printing a proof.  The theorem below supplies that constant, namely
$c=C_1/C_0=\mu$, from the 2004 direction.  It is an explicit-constant extension
of an announced remark rather than a statement of a new kind, and what it buys
is the strip $(0.398678,0.405683]$, which contains $\log4/\log31=0.403699$.
...
\[
  ...
  \frac{\log b}{\log a}<\theta^*=\frac{C_0}{C_1}=0.4056830213840605\ldots,
  \qquad \mu=\frac{C_1}{C_0}=2.4649786835749750\ldots
\]
...
\noindent In the normalisation of \cite[\S2]{zudilin2016}, where the announced
hypothesis is $\log|r|>c\log|s|$ for a computable $c>0$, the constant is
$c=\mu$, the irrationality exponent of Zudilin's own Theorem~1.  Negative bases
are not treated.

The two constants are the exponent budgets of Zudilin's direction.  Explicitly
\[
  C_1=(\alpha_0+\alpha_1+\alpha_2)\beta-\tfrac12(\alpha_1^2+\alpha_2^2+\beta^2)
     =\frac{1091}2,
  \qquad
  C_0=266-\frac3{\pi^2}\bigl(225-J\bigr)=221.30008816500502\ldots,
\]
where
\[
  J=\int_0^1\omega(x)\,d\bigl(-\psi'(x)\bigr)
   =77.94318447500909\ldots,
  \quad
  \omega(x)=\max\bigl\{0,\ \lfloor14x\rfloor+\lfloor13x\rfloor-\lfloor12x\rfloor-\lfloor15x\rfloor,\
  2\lfloor14x\rfloor-\lfloor13x\rfloor-\lfloor15x\rfloor\bigr\},
\]
so that $C_1/C_0=\mu=2.46497868\ldots$ and $\theta^*=1/\mu$.  Both published
thresholds have this shape: Bundschuh--V\"a\"an\"anen's is $1/\mu_{BV}$ with
$\mu_{BV}=2\pi^2/(\pi^2-2)=2.5082847619\ldots$, and $4^{\mu}=30.4835\ldots<31<4^{\mu_{BV}}=32.3696\ldots$,
which is exactly why $31/4$ sits between the two regions.  The function
$\omega$ takes the value $1$ on exactly thirteen intervals, the first being
$[1/14,1/12)$ and the last $[13/14,14/15)$; the list agrees with the one
printed in \cite{zudilin2004}.

\begin{corollary}\label{res:thirtyone-four}
$F\bigl((31/4)^r\bigr)$ is irrational for every integer $r\ge1$.
\end{corollary}

\noindent The strip $(1/\mu_{BV},\theta^*]$ gained over
...
The problem's own base $3/2$ sits at $\theta=0.630930\ldots$, far outside.

\begin{theorem}[Archimedean cap on base-uniform rank-two families]
\label{res:archimedean-cap}
Let $(U_n,V_n)\in\mathbb Z[x]^2$ satisfy $\Lambda_n(x)=U_n(x)F(x)-V_n(x)\ne0$,
$\deg U_n,\deg V_n\le\delta n^2(1+o(1))$, and
$\log|\Lambda_n(x)|=-\sigma n^2\log x\,(1+o(1))$ for every real $x>1$, with
$\sigma,\delta>0$ independent of $x$.  Then the homogenised forms
$b^{\deg U_n}\Lambda_n(a/b)$ tend to zero exactly on
...
$\mathbb Z[x]$, Zudilin's included, and $3/2$ is out of reach for this
...
\noindent The height ratio is invariant under a common positive power, and
\[
  \frac25<\frac{\log4}{\log31}<\frac{81}{200}<\theta^* .
\]
The two rational comparisons are exact integer inequalities, $31^2<4^5$ and
$4^{200}<31^{81}$, and both are Lean-checked
(\lword{Erdos1049/ZudilinHeightRegion.lean}{65}{twoFifths\_lt\_thirtyoneFour\_log\_ratio}{lower
bound},
\lword{Erdos1049/ZudilinHeightRegion.lean}{30}{thirtyoneFour\_log\_ratio\_lt\_eightyOne\_twoHundredths}{upper
bound}), as is the invariance of the region under common powers
(\lword{Erdos1049/ZudilinHeightRegion.lean}{59}{thirtyoneFour\_power\_mem\_zudilinHeightRegion}{power
family}).  The last comparison $81/200<\theta^*$ is a finite rational
computation: truncating each $\psi_1(u)-\psi_1(v)$ after nine terms gives
$J>389699/5000$, and $\pi>157/50$ then gives
$C_0>88371/400=(81/200)\cdot(1091/2)$.  The cutoff $81/200$ is an elementary
rational sub-boundary of the contour computed here.  It is not a threshold
stated by Zudilin, and it should never be cited as one.

The base $31/4$ lies strictly outside the Bundschuh--V\"a\"an\"anen region,
since $\log4/\log31>2/5>1/2-1/\pi^2$; that exclusion is Lean-checked
(\lword{Erdos1049/ZudilinHeightRegion.lean}{79}{bundschuhVaananenMargin\_lt\_twoFifths}{margin},
\lword{Erdos1049/ZudilinHeightRegion.lean}{91}{thirtyoneFour\_outside\_bundschuhVaananenHeightRegion}{exclusion}).
...
\subsection*{What the published lemmas supply}

Zudilin's Lemma~7 is a statement in $\mathbb Z[p]$.  It gives
\[
  \Lambda_n(p)=p^{-M_n}D_N(p)\,\Omega_n(p)^{-1}F_n(p)
    =U_n(p)\,h_p(1)-V_n(p),
  \qquad U_n,V_n\in\mathbb Z[p],
\]
where $h_p(1)=\sum_{m\ge1}(p^m-1)^{-1}$, by Gaussian binomial integrality,
cyclotomic clearing through the least common multiple, stability of the Heine
transform, and Gauss's lemma for division by the monic $\Omega_n$.  The pair
$(U_n,V_n)$ is unique because $h_p(1)\notin\mathbb Q(p)$.  The standing
hypothesis $p\in\mathbb Z$ of \cite{zudilin2004} is used only later, at its
equation~(24), to pass from $\mathbb Z[p]$ to $\mathbb Z$; the argument here
substitutes the rational base into the polynomial identity instead, and never
enters that step.  Lemma~2 is the equidistribution input.  It gives
\[
  \frac{K_n-W_n}{n^2}\longrightarrow 266-\frac3{\pi^2}(225-J)=C_0 ,
\]
with weight $\psi_1(u)-\psi_1(v)$ over the intervals on which $\omega=1$; only
intervals with $u\ge1/14$ occur, so no level $l>15n$ contributes.  Zudilin's
Archimedean estimates (17) and (25) are not consumed, and no rational-base
asymptotic theorem is needed or invoked.

\subsection*{Evidence boundary}

Theorem~\ref{res:rational-base-threshold} is an ordinary complete proof whose
only external inputs are Lemmas~1, 2 and~7 of \cite{zudilin2004}.  It is not
kernel-checked.  Lean checks the finite part: the trigamma definition of $J$,
the bounds $81/200<\theta^*$ from the single $k=0$ term of each trigamma
difference together with $\pi>3.14$, and $\theta^*<1/2$ from
$J<\psi_1(1/14)<198$; the membership of $31/4$ and of every power
$(31/4)^r$ in the contour region; the exclusion of $3/2$ from it; and the exact
integer bookkeeping of the forms.  Those are in
\path{ErdosProblems/Erdos1049/RationalBaseContour.lean} and
\path{ErdosProblems/Erdos1049/ZudilinHeightRegion.lean}.  The written proof is
\path{ErdosProblems/Erdos1049/RationalBaseIrrationality_31_4_Proof.md}, and the
contour derivation is
\path{ErdosProblems/Erdos1049/ZudilinRationalHomogenisationDerivation.md}.  The
forms $U_n,V_n$ were reconstructed exactly for $n\le3$ by an independent
implementation, which verified the polynomial divisibility that Lemma~7
asserts, matched the closed-form degree bookkeeping to the computed degrees,
confirmed nonvanishing of $\Lambda_n$ and the Archimedean bracket on
$F_n$, and reproduced the identity to relative error below $10^{-25}$;
those receipts are the probes
\path{formal_math/probes/erdos1049_zudilin_c0_c1_check.py} and
\path{formal_math/probes/erdos1049_zudilin_exact_forms.py} and the logs under
\path{state/formal_math/erdos257_period_noncollapse/erdos1049_contour_adversary/}.
The convergence $(K_n-W_n)/n^2\to C_0$ is exact at $n=1,2,3$ and numerical at
$n=10,100,400$; the limit itself is Zudilin's Lemma~2, not a computation.

Priority is bounded rather than settled.  Three rational-base regions are in
print: Duverney 1996 at $0.23207\ldots$, Bundschuh--V\"a\"an\"anen 1994 at
$0.398678\ldots$, and Zudilin 2016's unquantified announcement, whose constant
is not named there.  Zudilin 2004, Tachiya 2004, B\'ezivin 1988, Borwein 1991
and 1992, Van Assche 2001, Amdeberhan--Zeilberger 1998, and
Postelmans--Van Assche 2006 all work at integer base.  So does
Matala-aho, V\"a\"an\"anen and Zudilin~\cite{mvz2006}, whose standing
hypothesis is $p=1/q\in\mathbb Z\setminus\{0,\pm1\}$ and which states on p.~880
that its methods do not sharpen Zudilin~2004.  What is claimed here is
the explicit constant and the resulting decidable membership test, not a new
kind of theorem, and the theorem should be presented as proved rather than as
first.
...
\[
 ...
 \frac{N(N-1)(2N-1)}6
\]
for every $N\ge1$~\cite[Section~4]{zudilin2016}.  Tracking the first
associated-graded coefficient shows that this estimate is always sharp.
...
\begin{proof}
We sharpen the source row operation before taking the determinant.  For a
formal series $H(X)=1+\sum_{r\ge1}a_rX^r$, let $c_{j,t}$ be the coefficient of
tail state zero after $j$ applications of the associated-graded backward
shift, starting in tail state $t$.  A positive state moves down one place with
coefficient $-1$, while state zero emits state $s-1$ with coefficient $a_s$.
Thus
\[
...
\]
If $h_r=[X^r]H(X)^{-1}$, the reciprocal identity
$h_{r+1}=-\sum_{s=1}^{r+1}a_sh_{r+1-s}$ gives, by induction,
\[
 c_{j,t}=(-1)^jh_{j-t}\quad(0\le t\le j),
 \qquad c_{j,t}=0\quad(t>j).
\]

For the actual normalized summand, the ratio of consecutive moment indices
has associated-grade factor
\[
 H_t(X)=\frac{(1-X)^3(1-q^tX)^2}
 {(1-q^tX^2)(1-q^{t+1}X^2)}.
\]
At $q=0$ this is $(1-X)^3$ for $t\ge1$, while
$H_0(X)=(1-X)^4/(1+X)$.  Consequently the $t\ge1$ states contribute
$(-1)^j\binom{j-t+2}{2}$, and state zero contributes
\[
...
After extracting the row factors, the first coefficient matrix is
$(q^{j\ell})_{0\le j,\ell<N}$.  Its Vandermonde determinant has order
$N(N-1)(N-2)/6$; the row factors contribute
$N(N-1)(N+1)/6$.  Their sum is the asserted
$N(N-1)(2N-1)/6$.  The product of the row coefficients has absolute value
\[
 \prod_{j=0}^{N-1}\frac{(j+1)^2(j+2)}2
 =\frac{(N!)^2(N+1)!}{2^N},
\]
and the row signs cancel the leading Vandermonde sign.  Hence the first
coefficient is positive and has the displayed value.
\end{proof}

\paragraph{Consequence and evidence boundary.}
The theorem closes one named possibility: the normalized determinant cannot
decay faster through an unrecorded increase in $q$-order.  It does not extract
a denominator factor, improve the remaining height balance, or prove
irrationality at $3/2$.  Lean checks the generic recurrence
\lword{Erdos1049/AdelicHeightBridge.lean}{185}{hankelAssociatedCoeff_eq_reciprocal}{at all depths} and its actual positive/zero tail specializations
\lword{Erdos1049/AdelicHeightBridge.lean}{388}{hankelAssociatedCoeff_zudilinPositiveTail_closed}{and} \lword{Erdos1049/AdelicHeightBridge.lean}{419}{hankelAssociatedCoeff_zudilinZeroTail_zero}{respectively}.
The exact tail fan-in \lword{Erdos1049/AdelicHeightBridge.lean}{1454}{zudilinAssociatedTailRowCoeff_eq}{gives the row scalar, while the literal source recurrence} \lword{Erdos1049/AdelicHeightBridge.lean}{1206}{zudilinNormalizedTail_succ}{holds for every index}.
The step is $1$ below degree $n+1$ \lword{Erdos1049/AdelicHeightBridge.lean}{1005}{coeff_zudilinNormalizedTailStepUnit_eq_zero_of_pos_lt}{with first coefficient $-5$ or $-3$} \lword{Erdos1049/AdelicHeightBridge.lean}{930}{coeff_zudilinNormalizedTailStepUnit_first}{and the exact operator recurrence} \lword{Erdos1049/AdelicHeightBridge.lean}{591}{zudilinBackwardShiftApply_succ}{holds on arbitrary sequences}.
The first transformed row is complete in every column: \lword{Erdos1049/AdelicHeightBridge.lean}{1407}{zudilinTransformedNormalizedMoment_one_initialMonomial}{packages exact order $\ell+1$ and leading coefficient $-6$}.
The exact source $D_j$ row matrix preserves every Hankel determinant
\lword{Erdos1049/AdelicHeightBridge.lean}{700}{det_zudilinBackwardShiftedMomentMatrix}{in power series}; exact tails have unit first monomial
\lword{Erdos1049/AdelicHeightBridge.lean}{1294}{coeff_zudilinNormalizedTail_leading}{and moments isolate the zero tail below degree $n+1$} \lword{Erdos1049/AdelicHeightBridge.lean}{1349}{coeff_zudilinNormalizedMoment_of_lt}{in every row}.
Lean checks determinant noncancellation \lword{Erdos1049/AdelicHeightBridge.lean}{1567}{det_zudilinAssociatedLeadingMatrix}{at every rank}, exact order \lword{Erdos1049/AdelicHeightBridge.lean}{1727}{six_mul_order_det_zudilinAssociatedLeadingMatrix}{and the}
factorial assembly \lword{Erdos1049/AdelicHeightBridge.lean}{1738}{zudilinSharpHankelOrderAndCoeff_algebraicAssembly}{plus the generic unique-minimum bridge} \lword{Erdos1049/AdelicHeightBridge.lean}{771}{order_det_eq_of_unique_minimizing_permutation}{for filtered determinants}.
The serialized Lean replay and exact ranks $1$ through $7$ pass; only the filtered initial-monomial iteration for rows $j\ge2$ and all columns remains formal.
...
\noindent The minor propagation is
...
\[
 ...
 \qquad(a>b>0).
\]
No bibliographic source for an analytic irrationality theorem at this cutoff is
asserted here.  The Lean module instead treats the displayed inequality as a
definition and proves elementary memberships and exclusions.  In particular,
\[
 \frac25
 <\frac{\log4}{\log31}
 <\frac{81}{200}
...
earlier recorded logarithmic region.  Membership alone supplies no
irrationality theorem; the theorem for $31/4$ comes from
Section~\ref{sec:rational-base-irrationality}, which places $81/200$ below the
homogenisation threshold $\theta^*$ and imports two lemmas of Zudilin.
...
\bibitem{duverney1996} D.~Duverney,
  ...
  s\'erie $\sum_{n=1}^{+\infty}x^n/(q^n-1)$}},
  J. Th\'eor. Nombres Bordeaux \textbf{8} (1996), no.~1, 173--181.
  Th\'eor\`eme~2, printed p.~174, gives irrationality of
  $\sum_{n\ge1}(q^n-1)^{-1}$ for $q=r/s$ under
  $\log|s|/\log|r|<\tfrac13(1-3/\pi^2)$.
...
\bibitem{mvz2006} T.~Matala-aho, K.~V\"a\"an\"anen and W.~Zudilin,
  \emph{New irrationality measures for $q$-logarithms},
  Math. Comp. \textbf{75} (2006), no.~254, 879--889.  Integer base throughout;
  p.~880 records that the methods there do not sharpen the exponent of
  \cite{zudilin2004}.
\bibitem{chern2026} S.~Chern, L.~Jiu, S.~Li, and L.~Wang,
  \href{https://arxiv.org/abs/2401.17220}{\emph{Leading coefficient in the
  Hankel determinants related to binomial and $q$-binomial transforms}},
  Adv. Appl. Math. \textbf{176} (2026), 103051,
  doi:\href{https://doi.org/10.1016/j.aam.2026.103051}{10.1016/j.aam.2026.103051}.
