...
(`state/formal_math/erdos257_period_noncollapse/erdos1049_region_adversary/REPORT.md`, commit `b7a2783559`): Theorems A and B survived all eight attacks, with an exact
...
**Theorem C (Archimedean cap, proof-backed no-go for `3/2`; corrected 2026-09-02).**
...
3. `log max(H(U_n), H(V_n)) ≤ h n² (1+o(1))`, where `H(P)` is the largest absolute value of a
   coefficient of `P` (coefficient height);
4. `log|Λ_n(x)| = −σ n² log x (1+o(1))` for every real `x > 1`.

Then the homogenised forms `b^{deg U_n} Λ_n(a/b)` tend to `0` exactly on
`log b / log a < σ/(σ+δ)`, and `σ ≤ δ`, i.e. `σ/(σ+δ) ≤ 1/2`.  Hence no such family proves
irrationality of `F(a/b)` unless `b² < a`; in particular none reaches `3/2`.
...
Zudilin's family satisfies 1–4: `log H(U_n) = 18.43, 46.73, 72.02` and
`log H(V_n) = 21.54, 50.70, 76.49` for `n = 1, 2, 3`, i.e. `O(n)`, and
`log|U_n(p)|/(W_n log p) = 1.0064, 1.0035, 1.0013` at `p = 2`
(`…/erdos1049_region_adversary/adv_theorem_c_heights.log`); independently, the paper's (25) is
the value bound `|A_n(p)| ≤ |p|^{C₁n² + O(n)}` at integer `p`, which suffices.  "Padé-type
family" below means a family satisfying 1–4; that Zudilin's does is verified, that every
Padé-type construction does is a definition of the class considered, not a theorem.
...
relative `1e−97` at `31/4`): `state/formal_math/erdos257_period_noncollapse/erdos1049_region_adversary/REPORT.md`, attack 2.
...
Borwein 1991 in general), so `μ(F(p)) ≥ 2` by Dirichlet, hence `(σ − δ) log p ≤ h` for every
integer `p ≥ 2`.  Since `σ, δ, h` do not depend on `p` (hypotheses 3–4), letting `p → ∞` gives
`σ ≤ δ`, i.e. `σ/(σ+δ) ≤ 1/2`.  For `3/2`, `θ = log 2/log 3 = 0.6309 > 1/2`. ∎

(When `h = o(n²)`, as for Zudilin's family where `log H(U_n) = O(n)`, a single integer `p`
already gives `σ ≤ δ`; the limit `p → ∞` is what makes the theorem hold at `h > 0`.)

For Zudilin's family `σ = C₀`, `δ = C₁ − C₀`, so `σ/(σ+δ) = θ*` and `(σ+δ)/σ = μ`: the
rational-base threshold of a base-uniform family with bounded coefficient height is the
reciprocal of its integer-base irrationality-exponent bound, which is why `θ* = 1/μ` and
`1/2 − 1/π² = 1/μ_BV`.  The packet's
...
| Theorem C (corrected form) | exponent lemma with coefficient-height hypothesis + Dirichlet + `p → ∞` | (c) new, complete after the 2026-09-02 correction (adversary attack 7); hypothesis 3 verified for Zudilin's family at `n ≤ 3` and by the paper's (25) |
...
* Independent adversary (commit `b7a2783559`): `state/formal_math/erdos257_period_noncollapse/erdos1049_region_adversary/REPORT.md` with `adv_constants_region.py`, `adv_exact_forms.py` (exact `n ≤ 4`), `adv_theorem_c_heights.py` (coefficient heights); Theorems A, B survived all eight attacks, Theorem C corrected as above
