...
an integer-height conclusion, and `F(3/2)` stays open. The lattice result
measures rescaling followed by integer combination; a rational combination does
leave `Z x bZ`, at the cost of a coefficient denominator it does not measure. The manuscripts in
...
retained. Rescaling the tails and recombining them over `Z` does not produce
one: `long1049:res:tail-lattice` shows that the primitive rows and the endpoint
