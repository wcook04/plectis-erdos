#!/usr/bin/env python3
"""Exact five-sheet replay for the recentered positive row-20 carrier cell."""
...
SPEC = importlib.util.spec_from_file_location(
    "erdos1041_positive_row19_source",
    HERE / "check_erdos1041_quintic_double_tight_positive_row19_recentered.py",
)
ROW19 = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = ROW19
assert SPEC.loader is not None
SPEC.loader.exec_module(ROW19)

ROW18 = ROW19.ROW18
ENGINE = ROW18.ENGINE
report = ROW18.report

CENTER = ROW19.CENTER * Fr(207, 200)
RADIUS = Fr(1, 2850)
LEFT20 = Fr(
    118019477858920695849180116434155238431624156093757,
...
report(
    "PR20 exact recentered endpoints and R19 overlap",
    new_row[0].left == LEFT20
    and new_row[0].right == RIGHT20
    and old_row[0].right == R19
    and LEFT20 < R19 < RIGHT20,
    f"[{LEFT20},{RIGHT20}], overlap={R19-LEFT20}",
)
report(
    "PR20 five Krawczyk self-maps",
    all(
...
report(
    "PR20 regular oriented five-sheet row",
...
report(
    "PR20 upper disjoint exhaustive row",
...
for sheet in range(1, 5):
    ...
    report(
        f"PR20 robust-sheet exact sign {sheet}",
        upper < 0,
...
for sheet, (old_cell, new_cell) in enumerate(zip(old_row, new_row)):
    ...
    report(
        f"PR20 exact branch overlap sheet {sheet}",
...
print("ALL ROW-20 CHECKS PASSED")
