#!/usr/bin/env python3
"""Rigorous first dyadic stationary boundary slab."""
...
def main() -> int:
    ...
    expected_digest = "b7e5bbae6ec896953e677aa5da3deffcfc68d431b772090a266ba12e6edeceb4"
    passed = all((ieee, not red,
                  minimum.hex() == "0x1.6ad821bab65bbp+2",
                  digest.hexdigest() == expected_digest))
    result = {
        "schema": "erdos1041_three_exterior_stationary_dyadic_boundary_slab_one_receipt_v1",
        "interval_engine_sha256": ENGINE_SHA256,
        "certified_box": {"t": ["0", "1"], "r": ["0", "1/8"],
                          "s": ["1/4", "3/8"], "w": ["7/8", "15/16"]},
        ...
        "dyadic_boundary_slab_one_positive": passed,
        "claim_boundary": ("Proves S_stat>0 for all t, r in [0,1/8], s in "
                           "[1/4,3/8], and w in [7/8,15/16]. With adjoining "
                           "blocks this covers w in [1/4,15/16] on that r,s "
