#!/usr/bin/env python3
"""Exact five-sheet replay on the first macroscopic positive t-cell."""
...
SPEC = importlib.util.spec_from_file_location(
    "erdos1041_complete_large_affine_engine",
...
for index, initial_mu in enumerate(SUPPORT.FIBRE_SCOUTS[Fr(1, 1000)]):
    # First isolate the exact centre fibre.  Its rational-ball midpoint is a
    # scout for the ordered carrier; acceptance is redone by affine Krawczyk.
    ...
    report(
        f"CLC{1 + 3*index} sheet {index} carrier regular",
        cell.krawczyk_a < cell.correction_a
        and cell.krawczyk_mu < cell.correction_mu
        and cell.det_lower > 0
        and cell.cross_margin > 0,
        f"Ka={float(cell.krawczyk_a):.4g}, Kmu={float(cell.krawczyk_mu):.4g}, "
        f"det>{float(cell.det_lower):.4g}",
    )
    report(
        f"CLC{2 + 3*index} sheet {index} exact centre sign",
        point["hi"] < -Fr(1, 50),
        f"E0<{point['hi_f']:.9f}",
    )
    report(
        f"CLC{3 + 3*index} sheet {index} whole-cell sign",
        upper < -Fr(1, 100),
        f"L<{float(L):.6g}, E<{float(upper):.9f}",
    )

report(
    "CLC16 complete upper-sheet exhaustion",
...
report(
    "CLC17 common macroscopic interval",
    all(cell.left == Fr(3, 3125) and cell.right == Fr(13, 12500)
        for cell, *_ in rows)
    and all(row[7] < -Fr(1, 100) for row in rows),
    "all five sheets negative on [3/3125,13/12500]",
)
