#!/usr/bin/env python3
"""Exact five-sheet replay for the recentered positive row-21 carrier cell."""
...
SPEC = importlib.util.spec_from_file_location(
    "erdos1041_positive_row18_source_for_row21",
...
# Rebuild rows 19 and 20 cheaply from their exact source parameters.  Their
# expensive sign certificates live in their own landed checkers; here they are
# used only to supply independently certified carrier scouts for row 21.
...
CENTER = ROW18.NEW_CENTER * Fr(207, 200) ** 3
RADIUS = Fr(1, 2750)
LEFT21 = Fr(
    4714457186469516218396195177553885577347512340797977,
...
R20 = Fr(
    122213781858920695849180116434155238431624156093757,
...
report(
    "PR21 exact recentered endpoints and R20 overlap",
    new_row[0].left == LEFT21
    and new_row[0].right == RIGHT21
    and old_row[0].right == R20
    and LEFT21 < R20 < RIGHT21,
    f"[{LEFT21},{RIGHT21}], overlap={R20-LEFT21}",
)
report(
    "PR21 five Krawczyk self-maps",
    all(
...
report(
    "PR21 regular oriented five-sheet row",
...
report(
    "PR21 upper disjoint exhaustive row",
...
for sheet in range(1, 5):
    ...
    report(
        f"PR21 robust-sheet exact sign {sheet}",
        upper < 0,
...
for sheet, (old_cell, new_cell) in enumerate(zip(old_row, new_row)):
    ...
    report(
        f"PR21 exact branch overlap sheet {sheet}",
...
print("ALL ROW-21 CHECKS PASSED")
