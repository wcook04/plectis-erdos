#!/usr/bin/env python3
"""Exact five-sheet replay for the recentered positive row-19 carrier cell."""
...
SPEC = importlib.util.spec_from_file_location(
    "erdos1041_positive_row18_source",
...
CENTER = ROW18.NEW_CENTER * Fr(207, 200)
RADIUS = Fr(1, 3000)
LEFT19 = Fr(
    30016426431457080053185892813159226654366680929,
    1572864000000000000000000000000000000000000000000,
)
RIGHT19 = Fr(
    31065002431457080053185892813159226654366680929,
    1572864000000000000000000000000000000000000000000,
)
R18 = ROW18.RIGHT18

old_row = ROW18.new_row
...
report(
    "PR19 exact recentered endpoints and R18 overlap",
    new_row[0].left == LEFT19
    and new_row[0].right == RIGHT19
    and old_row[0].right == R18
    and LEFT19 < R18 < RIGHT19,
    f"[{LEFT19},{RIGHT19}], overlap={R18-LEFT19}",
)
report(
    "PR19 five Krawczyk self-maps",
    all(
...
report(
    "PR19 regular oriented five-sheet row",
...
report(
    "PR19 upper disjoint exhaustive row",
...
for sheet in range(1, 5):
    ...
    report(
        f"PR19 robust-sheet exact sign {sheet}",
        upper < 0,
...
for sheet, (old_cell, new_cell) in enumerate(zip(old_row, new_row)):
    ...
    report(
        f"PR19 exact branch overlap sheet {sheet}",
...
print("ALL ROW-19 CHECKS PASSED")
