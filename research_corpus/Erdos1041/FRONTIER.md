...
- target_deciding_but_stronger: `critical_point_selection_with_inverse_ray_length_control`, `one_root_covering_of_the_first_merge_component`, `free_pair_geometric_mean_two_arm_selector`, `fixed_safe_monodromy_block_average_lift_length`
- auxiliary_not_target_deciding: `sextic_second_mode_mixed_neighborhood_transfer`, `concyclic_form_a_cut_objective_below_one`, `sextic_unrestricted_chord_covering_band`
- out_of_scope_for_target: none

The declared target is the frontier. Open producers are candidate routes and must not replace it unless an exact equivalence is recorded.
...
Prove the componentwise combined-charge lemma. For each nontrivial connected component C of the admissible critical forest, show sum_{e in C}(D_e+K_e)>=0, where D_e=(R+<a,U_a>)+(R+<b,U_b>) uses the global root circumradius R and K_e=integral_e kappa<z,N> ds. The exact identity 2R-L(e)=D_e+K_e then yields sum_{e in C}L(e)<=2R|E(C)|; summing and averaging gives min_{|f(c)|<1}L(c)<=2R, and R<1 solves the problem. On each Newton half-edge d arg T=-Im((f''/f')dz), so attack the component boundary using cyclic critical-ray order and the argument principle while retaining external-root factors in f'/f. Do not require K alone to be positive, allocate charge root by root, replace R by the component-root circumradius, take absolute curvature, drop admissibility, or sample raw angular coarea: each is now falsified or structurally insufficient. UPDATE 2026-09-05 (W2-B, conductor-audited): the minimum two-arm statement min_{|f(c)|<1} L(c) <= 2R was reported refuted at degree five and the refutation was WITHDRAWN as a saddle-connection wall specimen (NearFeketeInnerModelWidening.md section 5.4, boxed audit); the statement stands unrefuted. What that note proves for this producer: in the near-Fekete cusp every critical point is admissible (no component may be discarded), the two-arm excess at a simple hub equals minus its total curvature charge (A_j = -K_j), so the obligation is exactly max_j K_j >= 0 in the cusp, and the corpus's kappa_n margin governs only a one-parameter slice of a (2n-5)-parameter model (the widened guarded margin at n = 4 is at most -0.1033, about ten times smaller than kappa_4). Any climb on this model must carry the wall guard.
