#!/usr/bin/env python3
"""Exact replay and lineage checks for the #1041 successor nonet revision 6."""
...
PACKET_DIR = ROOT / "docs/formal_math/type_b_packets/erdos1041_frontier_coevolution_nonet_r6_2026_08_26"
OUT_ROOT = Path("/Users/willcook/Downloads/erdos1041_frontier_coevolution_nonet_r6_20260826")
PREDECESSOR_DIR = ROOT / "docs/formal_math/type_b_packets/erdos1041_frontier_coevolution_nonet_r5_2026_08_26"
RETURN_INTAKE = ROOT / "state/formal_math/type_b_return_batches/erdos1041_20260826_return_stream_01/intake.json"
PREDECESSOR_PORTFOLIO_SHA256 = "4774d83c42c12b39359584d4e4072889e2a721a4a0eb28d270eb8bcc7b478773"
...
def check_successor_frontier() -> dict[str, object]:
    ...
    for label, checker in {
        ...
        "entrance_energy_ratio_bridge": ENTRANCE_ENERGY_CHECKER,
        "attachment_age_lifetime_orlicz": AGE_LIFETIME_CHECKER,
...
def check_manifests(require_built: bool) -> dict[str, object]:
    ...
    for path in manifests:
        ...
        if packet_id == "03_forest_combined_charge":
            assert any(path.endswith("EntranceEnergyRatioBridge.md") for path in embedded_paths)
        if packet_id in {
            "01_unrestricted_fanin",
            "02_forest_metric_consumer",
            "09_sparse_defect_pair_allocation",
        }:
            assert any(path.endswith("AttachmentAgeLifetimeOrlicz.md") for path in embedded_paths)
            ask_text = data["ask_type_b"]["decision_or_question"]
            assert "two-young" in ask_text.lower() or "two roots" in ask_text.lower()
            assert "energy" in ask_text.lower()
            assert "linear" in ask_text.lower()
...
def main() -> int:
    ...
    result = {
        "schema": "erdos1041_frontier_coevolution_nonet_r6_check_v1",
        "status": "pass",
