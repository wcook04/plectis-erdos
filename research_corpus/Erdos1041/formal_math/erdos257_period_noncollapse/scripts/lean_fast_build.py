...
def lake_stale_modules_batched(
    project_root: Path,
    lake: str,
    modules: Iterable[str],
    *,
...
def materialization_mode_conflicts(args: argparse.Namespace) -> list[str]:
    """Return build/plan selectors that materialization would otherwise ignore."""

    conflicts: list[str] = []
    if args.targets:
        conflicts.append("positional_targets")
    if args.changed_from is not None:
        conflicts.append("--changed-from")
    if args.verbose_plan:
        conflicts.append("--verbose-plan")
    elif args.plan:
        conflicts.append("--plan")
    if args.lake_staleness:
        conflicts.append("--lake-staleness")
    if args.no_final_build:
        conflicts.append("--no-final-build")
    return conflicts


def emit_materialization_mode_mix_rejection(
    args: argparse.Namespace,
    conflicting_selectors: list[str],
) -> None:
    print(
        json.dumps(
            {
                "schema": "lean_fast_build_invalid_materialization_mode_mix_v0",
                "status": "rejected_materialization_build_mode_mix",
                "proof_scope": "not_proof_evidence",
                "conflicting_selectors": conflicting_selectors,
                "materialize_packages": list(args.materialize_dependencies or []),
                "exit_code": 64,
                "next_action": (
                    "run_dependency_materialization_and_plan_or_build_as_separate_commands"
                ),
            },
            sort_keys=True,
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.materialize_dependencies is not None:
        materialization_conflicts = materialization_mode_conflicts(args)
        if materialization_conflicts:
            emit_materialization_mode_mix_rejection(args, materialization_conflicts)
            return 64
    if args.verbose_plan:
        args.plan = True
...
def pinned_package_git_preflight(project_root: Path) -> dict[str, object]:
    """Check existing manifest Git packages before Lake can materialize them.

    Lake treats failed optional Git captures as identity mismatches and may
    remove a package. This check never fetches, checks out, or repairs packages.
    Missing package directories remain Lake's normal materialization concern.
    """
    receipt: dict[str, object] = {
        "schema": "lean_pinned_package_git_preflight_v1",
        "status": "clear", "project_root": str(project_root),
        "git_executable": shutil.which("git"), "path": os.environ.get("PATH", ""),
        "git_environment": {name: os.environ[name] for name in (
            "GIT_DIR", "GIT_WORK_TREE", "GIT_COMMON_DIR", "GIT_NAMESPACE",
            "GIT_CONFIG_GLOBAL", "GIT_CONFIG_SYSTEM", "GIT_CONFIG_NOSYSTEM",
            "GIT_CONFIG_COUNT", "GIT_NO_REPLACE_OBJECTS", "GIT_REPLACE_REF_BASE",
    ...
    try:
        ...
        package_root = project_root / manifest.get("packagesDir", ".lake/packages")
        for package in packages:
            if package.get("type") != "git":
                continue
            directory = package_root / package["name"]
            if not directory.is_dir():
                continue
            row = {"name": package["name"], "directory": str(directory),
                   "expected_revision": package["rev"], "expected_url": package["url"],
                   "commands": []}
            rows.append(row)
            for arguments in (["rev-parse", "--verify", "--end-of-options", "HEAD"],
                              ["remote", "get-url", "origin"]):
                command = ["git", *arguments]
                try:
                    result = subprocess.run(command, cwd=directory, capture_output=True,
                                            text=True, check=False, timeout=15)
                    probe = {"argv": command, "exit_code": result.returncode,
                             "stdout": result.stdout.strip(), "stderr": result.stderr.strip()}
                except (OSError, subprocess.TimeoutExpired) as exc:
                    probe = {"argv": command, "exit_code": None, "stdout": "",
                             "stderr": str(exc)}
                row["commands"].append(probe)
            head, remote = row["commands"]
            # Match Lake's exact URL comparison, or equivalent existing local paths.
            url_equal = remote["stdout"] == package["url"]
            if not url_equal and remote["exit_code"] == 0:
                try:
                    url_equal = (directory / remote["stdout"]).resolve(strict=True) == (
                        directory / package["url"]).resolve(strict=True)
                except (OSError, ValueError):
                    pass
            if head["exit_code"] != 0 or head["stdout"] != package["rev"]:
                errors.append(package["name"] + ": pinned HEAD lookup failed or differs")
            if remote["exit_code"] != 0 or not url_equal:
                errors.append(package["name"] + ": remote URL lookup failed or differs")
    except (OSError, ValueError, KeyError, TypeError) as exc:
        errors.append("manifest identity check failed: " + str(exc))
    if errors:
        receipt["status"] = "blocked_before_lake"
    return receipt


def compact_pinned_package_git_preflight(
    receipt: dict[str, object],
) -> dict[str, object]:
    """Keep successful plan output focused on the dependency-wave decision."""
    packages = receipt.get("packages")
    errors = receipt.get("errors")
    return {
        "schema": "lean_pinned_package_git_preflight_summary_v0",
        "status": receipt.get("status"),
        "checked_package_count": (
            len(packages) if isinstance(packages, list) else 0
        ),
        "error_count": len(errors) if isinstance(errors, list) else 0,
        "detail": "rerun the plan with --verbose-plan for the full identity receipt",
    }
...
def _main_admitted(args: argparse.Namespace, project_root: Path) -> int:
    ...
    successful_compact_plan = (
        args is not None
        and args.plan
        and not args.verbose_plan
        and identity["status"] == "clear"
    )
    print(
        json.dumps(
            compact_pinned_package_git_preflight(identity)
            if successful_compact_plan
            else identity,
            sort_keys=True,
        )
    )
    ...
    target_resolutions = [
        (target, target_module(target, project_root, modules)) for target in targets
    ]
    roots = [module for _target, module in target_resolutions if module is not None]
    unresolved_targets = [
        target for target, module in target_resolutions if module is None
    ]
    if args.plan and args.targets and unresolved_targets:
        print(
            json.dumps(
                {
                    "schema": "lean_fast_build_unresolved_plan_target_v0",
                    "status": "rejected_unresolved_plan_target",
                    "proof_scope": "not_proof_evidence",
                    "requested_targets": list(args.targets),
                    "resolved_local_modules": roots,
                    "unresolved_targets": unresolved_targets,
                    "exit_code": 64,
                    "next_action": (
                        "select_existing_local_lean_modules_or_run_without_plan_for_"
                        "raw_lake_targets"
                    ),
                },
                sort_keys=True,
    ...
    # Missing roots do not invalidate restored dependency traces. Honor the
    # explicit planner choice and let Lake identify the actual missing cone.
    ...
    if use_lake_staleness:
        ...
        stale_waves = (
            ...
            # Execution must not traverse every import cone before starting
            # the first build and then repeat that traversal below. Inspect
            # each wave just in time, after its dependencies have been built.
            # --plan still computes the complete current stale inventory.
            else ([
                lake_stale_modules(
                    project_root,
                    args.lake,
        ...
        for wave in waves:
            for module in wave:
                # Passing a populated olean cache and config=None intentionally
                # classifies source/dependency/missing staleness without the
                # coarse project-config timestamp fallback.
                if module_is_stale(
                    project_root,
                    module,
                    modules,
                    ...
                    definitely_stale.add(module)
                    continue
                output_mtime = olean_mtime_ns(project_root, module, output_mtimes)
                if (
                    config_mtime is not None
                    and output_mtime is not None
                    and config_mtime > output_mtime
                ):
                    config_only_candidates.append(module)
        # A config timestamp is a conservative hint, not proof that an olean's
        # content trace is stale. Lake may replay a current module without
        # rewriting the olean, leaving the timestamp relation true forever.
        config_stale = set(
            lake_stale_modules_batched(
                project_root,
                args.lake,
                config_only_candidates,
                facet=prebuild_facet,
            )
        )
        stale_waves = [
            [module for module in wave if module in definitely_stale or module in config_stale]
            for wave in waves
        ]
    stale_waves = [wave for wave in stale_waves if wave]
    print(
        f"lean-fast-build: partition={len(selected)}/{len(modules)} local modules for "
        f"{len(roots)} root(s); {sum(map(len, stale_waves))} "
        f"{'candidates (trace checked per wave)' if use_lake_staleness and not args.plan else 'stale/missing'} across "
