...
\lloc{Erdos1041/PaperTrinomial.lean}{38}.
\end{proof}
...
\lloc{Erdos1041/PaperTrinomial.lean}{79}.
...
\lword{Erdos1041/PaperMetricScaling.lean}{40}{connectedBelow_affine}{the affine
...
boxed inequality is
\lword{Erdos1041/PaperFiniteDual.lean}{68}{arity_floor_of_sSup}{the finite dual
...
\lloc{Erdos1041/PaperStraightObstructions.lean}{178}.  The first defeats a
...
The hypothesis here is genuinely a separation condition in the value plane.
It does not follow from uniform separation of the roots and critical points.
Indeed, the quartic
\[
 z^4-\frac{z^3}{262144}-2z^2+\frac{3z}{262144}+\frac12
\]
has four roots and three critical points mutually more than $1/4$ apart, but
two distinct critical values differ by $1/65536$, below
$3((1/4)/4)^4=3/65536$.  The complete exact argument, including exhaustion
of the four roots, is recorded in
\path{ErdosProblems/Erdos1041/PointSeparationCriticalValueNoGoR13.md}.
This obstruction leaves the theorem above unchanged because its
critical-value gap is an explicit hypothesis.
...
than a supplied minimum, is \lloc{Erdos1041/PaperSeparationCounterexample.lean}{189}.
...
The root-spoke inequality used in Theorem~\ref{res:trinomial-all-degree} is
\lword{Erdos1041/CyclicTrinomialFiberCase.lean}{103}
{trinomialRoot_spoke_norm_lt_one_of_norm_lt_one}{the all-root trinomial spoke theorem}.
The ordinary proof supplies Vieta's hypothesis and concatenates the segments.
The finite convex identity is in
\path{ErdosProblems/Erdos1041/AbelControlPolygon.lean}.

The proximity selector, cubic and quintic selectors, and Chebyshev alternation
kernels are in
\path{CriticalTwoRootProximity.lean},
\path{CubicQuotientFiberCase.lean},
\path{PrimitiveQuinticBoundaryTail.lean},
\path{PrimitiveQuinticInteriorTail.lean} and
\path{SharpCollinearChebyshev.lean}, all under
\path{ErdosProblems/Erdos1041/}.
Their finite statements and the ordinary geometric assemblies are distinguished
in the corresponding proofs and the long record.

The complete ordinary analyses are located in
\path{AngularBudgetLowCriticalClosure.md},
\path{UnconditionalConstantFactorBound.md},
\path{DiskFamilyCriticalValueSeparation.md},
\path{FreePointQuadraticAllDegrees.md},
\path{AttachmentAgeLifetimeOrlicz.md},
\path{AttachmentAwareReeb.md} and
\path{GenericSufficiencyClosure.md} in the same directory.
The first includes the full-mode rational checker; the other files identify
any finite formal kernels separately from their analytic arguments.
The complete declaration and source-generation inventories remain in the
long record.  No new formal verification is asserted by these pointers.
