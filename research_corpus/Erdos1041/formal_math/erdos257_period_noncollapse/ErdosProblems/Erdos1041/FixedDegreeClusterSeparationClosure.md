...
```sh
python3 research_corpus/Erdos1041/scripts/check_erdos1041_fixed_degree_cluster_closure.py
python3 research_corpus/Erdos1041/scripts/check_erdos1041_fixed_degree_cluster_closure.py --audit
```
...
**Scope, not arithmetic.** Every certified constant in the table above stands.
`mu_4 = 61/100`, `mu_5 = 139/250`, `mu_6 = 253/500`, `mu_7 = 23/50`,
`mu_8 = 209/500`, `mu_9 = 19/50` are unchanged, and so is Theorem A.

What is stale is the comparison prose under the table. It was written against
the all-degree constant `9/25`, and the all-degree constant is now `13/25`
(`AngularBudgetLowCriticalClosure.md`, Theorem A''). Reading the same table
against `13/25 = 0.52`:

- `n = 4`: `61/100 = 0.61 > 0.52`. This row is still the better statement.
- `n = 5`: `139/250 = 0.556 > 0.52`. Still the better statement.
- `n = 6`: `253/500 = 0.506 < 0.52`. The all-degree theorem is better here.
- `n = 7`: `23/50 = 0.46 < 0.52`. All-degree is better.
- `n = 8`: `209/500 = 0.418 < 0.52`. All-degree is better.
- `n = 9`: `19/50 = 0.38 < 0.52`. All-degree is better.
- `n >= 10`: the inherited rows are superseded outright by `13/25`.

So the sentence "at degrees `8` and `9` they are the first fixed-degree
constants to beat the all-degree `9/25`" is true of `9/25` and is no longer
the live comparison, and the sentence "At degree `10` and above the all-degree
theorem is still the better statement" now begins at degree `6`. The correct
live statement is the one already written in
`AngularBudgetLowCriticalClosure.md` §5: this fixed-degree arm is authoritative
where it is stronger, which is now only `n = 4` and `n = 5`.

The static mechanism here remains independent of the comparison ODE, so this
note is not superseded as a derivation. Re-running the fixed-degree lane
against the circle-slice floor `(CS)` is the obvious next landing and has not
been attempted.

**Downstream.** The claim-frontier row
`erdos1041.fixed_degree_angular_cluster_closure` carries the same stale
comparison in its `band_basis` and in its `statement`; the correction is
emitted as
`docs/strategy/staging/rapid_breakout/audit/patches/frontier_1041.json`.
