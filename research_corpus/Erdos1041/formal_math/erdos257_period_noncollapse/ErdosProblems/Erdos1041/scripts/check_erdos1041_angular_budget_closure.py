#!/usr/bin/env python3
"""Exact rational certificate for the angular-budget (circle-slice) closure.

Same comparison ODE as ClusterSeparationLowCriticalClosure.md and
HyperbolicPackingArityFloor.md; the only thing that moves is the arity floor.

Under failure, at a regular level t, the ancestor component is uniformised by
the disc with the COVER point h at 0; its roots b_1..b_k satisfy

    (i)   pairwise hyperbolic distance >= D := d_min(a),  cosh(D/2) = e^(2/a),
    (ii)  d_j := d(0,b_j) >= d_low(a) = lam(delta(a)/2),
    (iii) sum_j lam(d_j) >= x,      lam(d) = -log tanh(d/2).

CIRCLE-SLICE LEMMA.  The balls B_j = B_hyp(b_j, D/2) are pairwise disjoint, so
for EVERY hyperbolic circle C_r about 0 the arcs B_j cap C_r are disjoint, and
their half-widths sum to at most pi:

    sum_j w(d_j, r) <= pi,
    w(d, r) = arccos( (cosh d cosh r - cosh(D/2)) / (sinh d sinh r) )   [clamped]
            = arccos(1 - om),  om = (cosh(D/2) - cosh(d-r)) / (sinh d sinh r).

DUAL CERTIFICATE.  For any radii r_1..r_p > 0 and weights sigma_i >= 0, put
S = sum sigma_i and U = sup_{d >= d_low} [ lam(d) - sum_i sigma_i w(d,r_i) ].
Then  x <= sum_j lam(d_j) <= k U + pi S,  so  k >= (x - pi S)/U  when U > 0.
Only S and U are certified; the sigma themselves are proposed by a floating LP
(or a stdlib fallback) and their optimality is irrelevant to validity.

MONOTONICITY IN a.  d_low(a) and D(a) both decrease in a, so the feasible set
grows with a and every certified pair (S,U) computed at a' >= a is valid at a.
The table is therefore built on a coarse a-grid and a is rounded UP into it.

k is an integer, and every floor below is a real lower bound for it, so ceil()
is applied to each floor before the maximum is taken.

Usage:
    check_erdos1041_angular_budget_closure.py [--quick] [--audit] [--geometry-audit] [--comparison-audit]
