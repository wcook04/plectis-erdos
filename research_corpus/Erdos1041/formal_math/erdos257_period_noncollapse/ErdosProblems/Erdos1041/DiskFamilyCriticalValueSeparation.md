...
**Extracted all-centre profile (2026-09-07).**  Write `A = coth 1` and
`p = a(1-a)`.  The logarithmic factor in (T) equals `2` precisely when
`S^2 - A S + p = 0`.  The larger root

```text
S_∞(a) = (A + sqrt(A^2 - 4a(1-a))) / 2
```

is admissible (`S_∞(a) > max(a, 1-a)`) and satisfies `S_∞(a) ≤ A < 2 ≤ n-1`
for every `n ≥ 3`, so the prefactor `(S/(n-1))^{2/n}` is strictly below one
and `C(n, S_∞(a), p) < 2` in every finite degree.  This is the smallest
degree-uniform radius this coefficient formula can certify.  The endpoint
values `S_∞(0) = S_∞(1) = coth 1` were already recorded; the centre-dependent
formula is the extracted statement.  The obstructing critical value in a
failed test may depend on the proposed centre: failure means

```text
∀ a ∈ [0,1],   D(a) ≤ max(a, 1-a)  or  C(n, D(a), a) ≥ 2,
```

with `D(a) = min_j |v_j - a|`, not that one fixed `v_j` lies in the
intersection of all exclusion discs.  With a single other critical value
(degree three) the two readings coincide.
...
* **Dubinin, "Some inequalities for polynomials and rational functions
  associated with a lemniscate", Zap. Nauchn. Sem. POMI 404 (2012) 83–99,
  Theorem 1 on printed page 85; English translation J. Math. Sci. 193 (2013)
  45–54, doi 10.1007/s10958-013-1432-4.**  The 2026-09-02 prior-art pass (E1)
  flagged this as a possibly uncited input to the separation argument, on the
  strength of Tao's 25 March 2026 comment ("By Theorem 1 of this paper of
  Dubinin (a Pólya type inequality), this area is at most `s^{2/m}` times the
  area of `U`").  The bibliographic disposition is now closed.  Theorem 1
  assumes a holomorphic function giving a full `n`-fold covering of an annulus
  `t_1 < |w| < t_2` and, with `E` the complementary set defined there, states
  `(t_2/t_1)^(2/n) <= m(E union D)/m(E)`.  The full covering and the explicit
  complementary set are hypotheses of the theorem.  Under the reading in which
  `E` and `E union D` are two nested sublevel sets, that inequality is the
  relative Pólya statement Tao quoted.  It enters **neither** the 2026-08-30
  separation argument **nor** the present proof: both use only the absolute
  inequality `Area(K) <= pi cap(K)^2` on a lemniscate component.  The one row
  in this directory of the same relative shape is
  `actual_component_merge_tree_area_lower_bound`
  (`ActualComponentMergeTreeAreaLowerBound`, the branch monotonicity
  `A(R)/R^(2/k)` nondecreasing), which the first-merge capacity closures
  consume; that row carries its own proof by boundary Cauchy–Schwarz and
  planar isoperimetry, and Dubinin's theorem is the published neighbouring
  form it should cite.  Nothing here is a missing proof step, and no priority
  is adjudicated.
...
That intersection is the residual for a *single* other critical value, as in
degree three: the same `w` must serve every centre.  With several other
critical values the obstructing `v_j` may depend on `w0`.  Failure of the
normalised test is the quantified statement in §1, not membership of one
fixed value in `E_n`.  In degree three the crescent `1 <= |w| < 1.109`,
`|w - 1| < 1.109`, `|w - 1/2| < 0.631` is exactly `E_3`; in every degree
`E_n` sits inside `{1 <= |w| < coth(1)} ∩ {|w - 1| < coth(1)}`.  Any
counterexample to Erdős 1041 with a simple minimising hub and only one other
critical value has that value in this lens.  The random sweep in the receipt
