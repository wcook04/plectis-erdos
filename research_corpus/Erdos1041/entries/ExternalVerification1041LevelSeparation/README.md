# Erdős #1041: finite-family level and argument separation
...
common translation after which all values are nonzero, no two lie on the same
positive ray, and no two have the same modulus.
```
...
`ErdosProblems.Erdos1041.exists_small_translation_separating_arguments_and_norms`
from the source module.  The negative configuration adds an irrelevant `True`
hypothesis, producing a deliberate declaration-type mismatch for replay.

This is a finite-family generic-position result.  It does not prove a
critical-point path-length bound, establish the global topological gluing, or
...
systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  ...
  bash -c 'lake env comparator ExternalVerification1041LevelSeparation/comparator.json'

systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  -E PATH="$PATH" --working-directory "$PWD" -- \
  bash -c 'lake env comparator ExternalVerification1041LevelSeparation/comparator-negative-mismatch.json'
```
