# Erdős #1041: reciprocal power-sum bound

This directory isolates the finite coefficient estimate that bounds the
`m`-th reciprocal power sum when every root has modulus at least `r > 0`.
It is the quantitative input used by the reciprocal expansion's geometric
tail estimates.
...
This is a finite norm inequality, not a proof of the reciprocal logarithmic
series, the #1041 short-path theorem, unrestricted Erdős #1041, Comparator
...
systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  ...
  bash -c 'lake env comparator ExternalVerification1041NormRecipPowerSum/comparator.json'

systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  -E PATH="$PATH" --working-directory "$PWD" -- \
  bash -c 'lake env comparator ExternalVerification1041NormRecipPowerSum/comparator-negative-mismatch.json'
```
