# Erdős #1041: weighted-chain consumer

This directory isolates the ordered-algebra step that uses positivity of the
total size and monotonicity of the Orlicz values to transfer a weighted
lifetime lower bound to the floor value.
...
systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  ...
  bash -c 'lake env comparator ExternalVerification1041WeightedChainConsumer/comparator.json'

systemd-run --property=RestrictAddressFamilies=~AF_UNIX --user --pty \
  -E PATH="$PATH" --working-directory "$PWD" -- \
  bash -c 'lake env comparator ExternalVerification1041WeightedChainConsumer/comparator-negative-mismatch.json'
```
