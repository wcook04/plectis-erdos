...
$q\ge2^{39990}>10^{12039}$, from $23438$ certified continued-fraction partial
quotients computed from an $80000$-bit exact rational bracket.  That is an
exact computation and not a Lean theorem.  The Lean-checked Farey exclusion
...
{\footnotesize\emph{Evidence:} an elementary counting argument given in
Section~\ref{sec:rank}.  It is not formalised, and no declaration in the
pinned source states it.}
...
denominator below $10^{12039}$ without restriction.  The certified quotients
...
and conditional-rank layers with a paper-level independence argument.  The
zero-residue relation, the division-free composite-base reduction, and the
unique quotient/nonzero-digit coordinates for each admissible fixed-level
residue are proved in this paper and are not formalised; the module that would
carry them has not landed.  The conditional layer proves that the canonical family spans
...
$q\ge2^{39990}>10^{12039}$ of Section~\ref{sec:denominator} is larger and is
