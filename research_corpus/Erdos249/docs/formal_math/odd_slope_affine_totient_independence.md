...
| **B. Totient k-kernel rank `k^e + 1` for every integer `k ≥ 2`** | **Lean-checked and unconditional as of 2026-09-02**, in `Erdos257PeriodNoncollapse/AllBaseTotientKernel.lean`: `linearIndependent_totientAffineForms` removes the odd-slope restriction by an arbitrary-auxiliary-prime row, so `finrank_allBaseTotientKernel_eq (k e) (hk : 2 ≤ k)` carries no `LinearIndependent` hypothesis, and `allBaseTotientKernelBasis` is the `Basis`. The relation module's *dimension* is checked (`finrank_allBaseRelationModule_eq = ∑_{1≤j<e} k^j`); the named syzygies are proved to lie in the kernel but **not** proved to span it, so the complete relation normal form is still not Lean-checked. | Rank formula, explicit basis and complete relation normal form: no source located. Novelty unassessed pending specialist review. |
| **C. Erdős #249** | — | Untouched. Open. Nothing here bears on it. |

---

## Status change, 2026-09-02

Row **B** above previously read "the unconditional all-base independence theorem,
`Basis`, and complete relation normal form are not in Lean." Two thirds of that
is now false and the row has been rewritten rather than left to rot.

What closed: the all-slope auxiliary-prime row. The 2-adic parity determinant
that `TotientMahlerDefect.lean` uses needs odd slopes; replacing the fixed prime
2 by an arbitrary odd auxiliary prime `ℓ`, chosen above every slope and content
and paired with primes `q ≡ 1 (mod ℓ)` on the off-diagonal, makes the evaluation
matrix diagonal and nonsingular mod `ℓ` for **arbitrary positive slopes**. So
`finrank_allBaseTotientKernel_eq (k e) (hk : 2 ≤ k) : … = k^e + 1` is
unconditional, and `allBaseTotientKernelBasis` is the `Basis`.

What did not close: the syzygies are proved to lie in `ker T` and `ker T`'s
dimension is proved, but they are not proved to **span** it. The complete
relation normal form therefore remains un-Lean-checked, and row B says so.

Three notes for anyone continuing this, learned the expensive way:

* `exists_large_distinct_primes_modEq_one` in `TotientMahlerDefect.lean` is
  hard-wired to modulus `2^K` and does **not** transfer to arbitrary `ℓ`; it had
  to be restated as `..._of_modulus`. `exists_affine_root_mod_prime` and
  `affine_target_coprime_of_cross` did transfer verbatim.
* The oddness of `ℓ` is needed in exactly one place, and not the obvious one.
  `ℓ ∤ (p−1)` follows from `p ≡ 2 (mod ℓ)` for *any* prime `ℓ`; oddness is needed
  for `gcd(A_i, B_i) = 1`, i.e. `ℓ ∤ B_i` given `B_i ≡ 2`.
* In this Mathlib the Dirichlet entry points are
  `Nat.infinite_setOf_prime_modEq_one` and `Nat.forall_exists_prime_gt_and_modEq`,
  and `Nat.dvd_sub'` no longer exists — it is `Nat.dvd_sub` with the same
  signature, which will bite anyone porting the corpus's older proofs.

The provenance correction below is unchanged and still governs: this is a Lean
formalisation of a corollary of Martin's Theorem 1, not an original theorem.
