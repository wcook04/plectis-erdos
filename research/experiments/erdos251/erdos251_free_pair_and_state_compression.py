...
"""Replay finite free-pair scans and a finite denominator exclusion for Erdős 251.

The free-pair and averaging measurements use floating-point tails. The
continued-fraction denominator check uses integers and the analytic bound
explained in the paper. A passed finite-window test proves no cofinality or
irrationality statement and does not run Lean.
Use --help for explicit range and precision controls; the README gives the
settings of the archived 20-million-prime-limit run.
"""
...
def main() -> int:
    ...
    args = ap.parse_args()
    if args.limit < 1000 or args.max_t < 1 or args.precision_bits < 32:
        ap.error("use --limit >= 1000, --max-t >= 1, and --precision-bits >= 32")
    if not 0 < args.late_frac < 1 or args.witness_slack < 0:
        ap.error("--late-frac must lie between 0 and 1; --witness-slack must be nonnegative")

    p, g, T = build(args.limit)
    m = len(g)
    if m <= args.precision_bits + 300:
        ap.error("the prime range is too small for the requested denominator precision")
    K = 64
    lo, hi = int(m * args.late_frac), m - K - 2
    if hi - lo < args.max_t:
        ap.error("the late window is too small for the requested --max-t")
    ...
    falsifiers = [
        {"name": "finite_window_witnesses_all_residues",
        ...
        {"name": "finite_window_averaging_bound_with_endpoint", "detail": avg,
