...
# Raised to 52_900 for the three naturalistic boundaries the companion
# mathematics papers exposed: a kernel-checked theorem no record entry owns,
# a status defeated by literature outside the repository, and an obstruction
# that was a property of a normalisation. Those are failure classes the
# perimeter does not reach, so the material is architecture rather than
# experiment, which is what this budget exists to protect. It was funded
# first by deleting the paragraph of stronger design variants the project did
# not build and by compressing the evaluation-stage roadmap, which together
# returned about a thousand bytes; the raise covers the remainder.
MAX_SYSTEMS_PAPER_BYTES = 52_900
