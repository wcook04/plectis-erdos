...
and `M_n ∣ c_n` (checked in the probe, identity I2). The LCM-prefactor form
`limsup Q_n/M_n < ∞` (equivalently `limsup Z_n < ∞` for
`Z_n = (A_{n-1}/a_n)(a_n^2/a_{n+1}-1)`) is the bounded-record-excess case
`V_n ≥ -B` of the later weighted-record theorem, transferred by the
quadratic-limit dictionary `V_n + t_n Z_n = o(1)`. It is not a parent
solution: on cancellation-free surviving blocks the LCM defect is trivial and
the quantity reduces to the product form. UPDATE 2026-09-07 (r3): this
composition is recorded as `res:lcmbounded` in the live note.
...
## Correction block, 2026-09-06 (corrective-integration desk, #243)

Appended, not rewritten. Every line above stands as authored; the corrections
below take precedence where they conflict.

1. **The canonicalization line is stale.** The `canonicalization_state` field
   above says "Theorem 4 subsumes `boundedNegativePart_*` as the strongest
   longitudinal claim". That is no longer the corpus position. The frontier row
   `erdos243.slow_negative_part_rigidity` carries
   `superseded_by: [erdos243.record_excess_dichotomy_inclusive_loglog]` on disk,
   and the reciprocal `supersedes` is populated on the other row. The strongest
   longitudinal conditional statement in the corpus is now
   `EightReturnRigidityCriteria.md` Theorem 13 with Corollary 14, which
   supersedes Theorem 4 here on two independent axes: the observable is the
   drawdown-corrected record increment `r_n = (-E_n - B_n)_+ <= (-E_n)_+`
   rather than every negative error, and the boundary is inclusive.

2. **Proposition 7's cap is not sharp as printed.** Proposition 7 gives an
   admissible rise of `O(log u)` for the whole-modulus-avoiding walk.
   `EightReturnRigidityCriteria.md` Proposition 16 sharpens the cap for the
   class `l(m_j) = j + O(1)` to `limsup (u_{n+1} - u_n)/l(u_n) = sigma^{-1}`
   exactly, with `sigma = prod_j (1 - 1/m_j)`, so the true cap on that class is
   `sigma^{-1} l(u) = Theta(log log u)` and `sigma^{-1}` is family-dependent.
   Witness 17 gives the Fermat family `m_j = 2^{2^j} + 1` with
   `prod_{j<J} m_j = 2^{2^J} - 1` exactly, `sigma_J -> 1/2`, and gap constant
   exactly `2`. The `O(log u)` statement of Proposition 7 remains correct as an
   upper bound for the general class; it is not the sharp constant, and the
   sentence "no argument using only pairwise coprimality and whole-modulus
   avoidance can exclude negative parts of order `log C_n`" is unaffected.

3. **Corollary 5's constant is now strict.** Corollary 5 states
   `limsup (-E_n)_+ / log2 log2 C_n >= 1`. Corollary 14 of the first 2026-09-05
   batch upgrades this to `> 1` strictly on every counterexample, through the
   Euler amplification step, where pre-sieving the CRT block with
   `x = 0 (mod v_N)` gives `K_u >= v_N/phi(v_N)` with the ratio strictly
   increasing along the tail, so the supremum is never attained. The exact
   remaining obstacle displayed at the end of section 5 should therefore be read
   with the strict lower boundary.

4. **Negative-knowledge label.** Proposition 7 with Proposition 16 is a
   `proved_class_obstruction`: it bounds what a whole class of arguments can
   reach, and it is not evidence about the orbit. The `1/n` threshold of
   Theorem 8 is weaker than Theorem 4 and is retained as the one
   number-anchored statement in the language of the original problem; its own
   text already says so.

5. **Public placement, for the paper_assimilation block above.** Theorem 4 is
   now stated in the public note as Theorem `res:slownegative`, Theorem 6 as
   Theorem `res:strausbounded`, and Theorem 8 as Theorem `res:onethreshold`, in
   `R/paper/erdos-243-reciprocal-tail-rigidity.tex` section `sec:records` and in
   the reasoning surface. The paper_assimilation line "not edited here" is
   correct for this note; the edit was made by the corrective-integration desk.

Evidence class for this block: every quoted statement was read this pass from
the file it is attributed to. No build was run.
