...
/-- Statement interface for short `thm:variable-fractional-cover`.
The proof is supplied by `PaperCompleteR8.PositiveCoverReturn`; this definition
alone is not evidence for the theorem. -/
...
/-- Statement interface for long `thm:257-weighted`, including the all-base
hereditary consequence of a finite binary weighted mass. The proof is supplied
by `PaperCompleteR8.WeightedReturn`. -/
...
/-- Statement interface for short `res:mixed-supports`. The proof is supplied by
`PaperCompleteR8.WeightedReturn`; no separate-return premise is substituted for
the common observation-scale estimates used there. -/
