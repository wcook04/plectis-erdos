...
from query_corpus import json_transport_path, decode_json_transport, encode_json_transport
...
def safe_read_text(path: Path) -> str:
    ...
    return decode_json_transport(safe_read_bytes(json_transport_path(path)))
...
def safe_write_text(path: Path, text: str) -> None:
    ...
    try:
        ...
        with os.fdopen(descriptor, "wb") as stream:
            descriptor = -1
            stream.write(encode_json_transport(text) if path.suffix == ".gz" else text.encode("utf-8"))
    finally:
        if descriptor >= 0:
            os.close(descriptor)
...
def file_digest(path: Path) -> str:
    """Hash a regular generated file without materialising it in memory."""
    candidate = _safe_semantic_path(path)
    flags = os.O_RDONLY
    flags |= getattr(os, "O_CLOEXEC", 0)
    flags |= getattr(os, "O_NONBLOCK", 0)
...
def check_receipt(
    ...
    return {
        ...
        "output_transport": "gzip",
        "output_transport_digest": "sha256:" + hashlib.sha256(encode_json_transport(text)).hexdigest(),
        "surface_digests": {
            path.relative_to(ROOT).as_posix(): (
                f"sha256:{hashlib.sha256(expected.encode('utf-8')).hexdigest()}"
            )
            for path, expected in sorted(
                surfaces.items(), key=lambda item: item[0].relative_to(ROOT).as_posix()
            )
        },
        "summary": {
            "statement_nodes": payload["summary"]["statement_nodes"],
            "declarations": payload["summary"]["declarations"],
            "orphan_count": payload["summary"]["coverage"]["orphan_count"],
            "public_semantic_census": payload["summary"]["public_semantic_census"],
        },
        "verification_posture": (
            "content_addressed_generated_corpus_and_public_census_surfaces_"
...
def write_check_receipt(path: Path, receipt: dict) -> None:
    ...
    safe_write_text(path, json.dumps(receipt, indent=2, sort_keys=True) + "\n")
...
def receipt_matches(
    ...
    *,
    ...
    return (
        ...
        and receipt.get("builder_schema") == "erdos249257-semantic-corpus/1"
        and receipt.get("input_fingerprint") == input_fingerprint
        and receipt.get("output_digest") == output_digest
        and receipt.get("surface_digests") == surface_digests
    )


def load_cached_check() -> dict | None:
    """Accept a receipt only when all inputs and generated bytes still match."""
    if not json_transport_path(OUTPUT).is_file():
        return None
    input_fingerprint = semantic_input_fingerprint()
    try:
        output_digest = "sha256:" + hashlib.sha256(safe_read_text(OUTPUT).encode("utf-8")).hexdigest()
        surface_digests = {
        ...
        return None
    for path in (LOCAL_CHECK_RECEIPT, TRACKED_CHECK_RECEIPT):
        if not path.is_file():
            continue
        try:
            receipt = load(path)
        except (json.JSONDecodeError, OSError, UnsafeSemanticCorpusInput):
            continue
        if receipt.get("output_transport") == "gzip" and receipt.get("output_transport_digest") != file_digest(json_transport_path(OUTPUT)):
            continue
        if receipt_matches(
            receipt,
...
def main() -> int:
    ...
    parser.add_argument(
        "--full-check",
        action="store_true",
        help="ignore exact receipts and rebuild the corpus in memory",
    )
    ...
    if args.check and not args.full_check:
        ...
        if cached is not None:
            ...
            print(
                "semantic corpus current: cached "
                f"{summary['statement_nodes']} statement nodes over "
                f"{summary['declarations']} declarations, "
    ...
    if args.check:
        ...
        if stale_surfaces:
            ...
            return 1
        write_check_receipt(LOCAL_CHECK_RECEIPT, receipt)
