...
# A problem row is "open" until a resolution lands with its own evidence. The only
# non-open status admitted today is the external negative answer to #1041, whose
# counterexample is kernel-checked in ErdosProblems.Erdos1041.Counterexample; a
# resolved row must still record its residual open_obligations.
CANONICAL_PROBLEM_STATUSES = frozenset({"open", "negative_answer_external_lean_formalised"})
...
def load_owner() -> tuple[dict, dict, dict, dict]:
    ...
    for row in projection["problems"]:
        status = row.get("status")
        if status not in CANONICAL_PROBLEM_STATUSES:
            raise ValueError(
                f"canonical problem-index row {row.get('problem_id')!r} has status {status!r}; "
                f"allowed: {sorted(CANONICAL_PROBLEM_STATUSES)}"
            )
        if status != "open" and not row.get("open_obligations"):
            raise ValueError(
                f"canonical problem-index row {row.get('problem_id')!r} is not open but records no "
                "residual open_obligations; a resolved programme must still name what remains"
            )
...
def render_formalization(
    ...
    lines = [
        ...
        "  description: \"Lean 4 formalization of subsidiary mathematics for eight Erdős problems (seven open; #1041 answered in the negative by ani and formalised here), including unconditional irrationality theorems for structured Mersenne supports, exact conditional endpoint reductions, arithmetic classifications, and method barriers; none of the eight open problems is claimed solved.\"",
        "  authors:",
        "    - \"Will Cook\"",
        "  license: \"Apache-2.0\"",
...
def render_human(
    ...
    human = "\n".join(
        [
            ...
            "# Plectis verification: eight Erdős programmes",
            "",
            "> [!IMPORTANT]",
            ...
            (
                ...
                "surface shows one checked frontier for each of eight Erdős problems, seven of them open. "
