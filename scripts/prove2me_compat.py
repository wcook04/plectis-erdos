#!/usr/bin/env python3
"""Offline, source-bound Prove2Me handoff for one selected public theorem.
...
UNITS = {
    "erdos257_reciprocal_summable_support": {
        ...
        "paper_label": "res:reciprocal-support",
        "remaining_open": "remaining_open.universal_257_all_infinite_supports",
        "title": "Reciprocal-summable support irrationality at every integer base",
        "statement": (
            "Let $A$ be an infinite set of positive integers with "
            "$\\sum_{a\\in A}1/a<\\infty$. For every integer base $b\\ge 2$, "
            "$\\sum_{a\\in A}1/(b^a-1)$ is irrational. The reciprocal-summability "
            "hypothesis is part of this theorem; it does not settle "
            "the question for every infinite support."
        ),
        "tags": ["number-theory", "irrationality", "erdos-257"],
        "attachment": {
    ...
    "erdos249_all_base_totient_kernel_paper_theorem": {
        ...
        "dependency_source": "lean/Erdos249257/AllBaseTotientKernel.lean",
        "dependency_needle": "theorem finrank_allBaseThroughLevelFamily_eq ",
        "paper_source": "paper/249/erdos-249-binary-totient-series.tex",
        "paper_label": "thm:kkernelrank",
        "remaining_open": "remaining_open.erdos_249_irrationality",
        "title": "All-base finite-level totient-kernel basis and rank",
        "statement": (
...
def prepare(unit: str = DEFAULT_UNIT) -> dict:
    if unit not in UNITS:
        raise ValueError(f"unknown theorem unit {unit!r}")
    config = UNITS[unit]
    ...
    if len(matches) != 1:
        raise ValueError(f"expected one claim {config['claim_id']}")
    claim = matches[0]
    if (claim.get("paper_label") != config["paper_label"]
            or config["remaining_open"] not in claim.get("remaining_open_proposition_ids", [])
            or claim.get("status") != config["claim_status"]
            or (config.get("registered_declaration") and not any(
                row.get("name") == config["theorem"].split(".")[-1]
                and row.get("module") == lean_source.removeprefix("lean/")
                for row in claim.get("declarations", [])))):
        raise ValueError("the claim/declaration/paper/open-boundary mapping changed")
    line = source_line(lean_source, f"theorem {config['theorem'].split('.')[-1]}")
    if config.get("registered_declaration") and not any(
            row.get("name") == config["theorem"].split(".")[-1] and row.get("line") == line
            for row in claim["declarations"]):
        raise ValueError("the claim declaration line changed")
    paper_line = source_line(paper_source, f"\\label{{{config['paper_label']}}}")
    ...
    packet = {
        ...
        "native_draft": {
            ...
            "theorem_title": config["title"],
            "natural_language_statement": config["statement"],
            "source": (f"https://github.com/wcook04/plectis-erdos/blob/{public_commit}/"
                       f"{lean_source}#L{line}" if public_source_matches else None),
            "paper_source": (f"https://github.com/wcook04/plectis-erdos/blob/{public_commit}/"
        ...
        "mapping": {
            ...
            "local_attachment": ["source hashes", "paper passage", "prior credit", "open boundary", "type and replay receipts"],
            "omitted": ["private parent state", "service credentials", "mission for an already-proved theorem", "unsupported acceptance claims"],
        },
    }
    if "dependency_source" in config:
...
def validate(packet: dict, environment: dict | None = None, staged: dict | None = None) -> dict:
    ...
    try:
        current = prepare(packet.get("unit"))
        ...
        for field in ("unit", "public_source_commit", "claim_record_sha256", "claim_id", "claim_status", "paper_label", "remaining_open_proposition_ids",
    ...
    if staged is None:
        ...
        config = UNITS.get(packet.get("unit"), {})
        if staged.get("source_sha256") != packet.get("sources_sha256", {}).get(config.get("lean_source")):
        ...
        if not formal.startswith(f"theorem {config.get('theorem')} ") or not formal.rstrip().endswith(":= by sorry"):
...
def main() -> int:
    ...
    prep.add_argument("--unit", choices=tuple(UNITS), default=DEFAULT_UNIT)
