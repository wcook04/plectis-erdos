...
class Portfolio:
    ...
    unavailable_required_packages: tuple[str, ...] = ()
...
def _family_disposition_class(disposition: str) -> str:
    ...
    if disposition.startswith("existing_isolated_headline_retained"):
        return "represented_by_selected_interface"
...
def validate_legacy_package_record_assimilation(
    ...
    for row in rows:
        ...
        if package_id not in required:
            raise PortfolioError(
                f"legacy Palomar package is absent from historical membership: {package_id}"
            )
        historical_names.extend(names)
        if package_id not in current_by_package:
            continue
...
def lean_module_path(root: Path, module: str) -> Path:
    """Resolve a Lean module named by a Comparator config."""
    if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_']*(?:\.[A-Za-z_][A-Za-z0-9_']*)*", module):
        raise PortfolioError(f"invalid Lean module in Comparator config: {module!r}")
    return root / (module.replace(".", "/") + ".lean")


def validate_challenge_imports(challenge: Path, package_id: str) -> None:
    ...
    own_statements = f"{package_id}.Statements"
    for module in imports:
        ...
        statements = challenge.parent / "Statements.lean"
...
def package_directories(
    ...
    """Discover config-owned current packages, with the legacy root excluded.

    Historical membership can name packages absent at the current checkout.
    Those rows remain visible in generated metadata; they do not require
    restoring old package source trees merely to regenerate current projections.
    """
    packages: list[Path] = []
...
def build_portfolio(root: Path = ROOT) -> Portfolio:
    ...
    for directory in package_directories(root, targets, required_packages):
        config_path = directory / "comparator.json"
        try:
            config = json.loads(config_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise PortfolioError(f"cannot read Comparator config {config_path}: {exc}") from exc
        challenge_module = config.get("challenge_module")
        solution_module = config.get("solution_module")
        configured_names = config.get("theorem_names")
        if (
            not isinstance(challenge_module, str)
            or not isinstance(solution_module, str)
            or not isinstance(configured_names, list)
            or not configured_names
            or not all(isinstance(name, str) and name for name in configured_names)
            or configured_names != list(dict.fromkeys(configured_names))
        ):
            raise PortfolioError(f"malformed Comparator config: {config_path}")
        challenge = lean_module_path(root, challenge_module)
        solution = lean_module_path(root, solution_module)
        if not challenge.is_file() or not solution.is_file():
            raise PortfolioError(
                "Comparator config names a missing challenge or solution module: "
                f"{config_path}"
            )
        validate_challenge_imports(challenge, directory.name)
        validate_solution(solution)
        challenge_names = declarations_in(challenge, reject_private=True)
        if not challenge_names:
            raise PortfolioError(f"per-entry Challenge has no theorem: {challenge}")
        if set(configured_names) != set(challenge_names):
            raise PortfolioError(
                f"Comparator config theorem roster disagrees with Challenge: {config_path}"
            )
        theorem_names = tuple(configured_names)
    ...
    unavailable_required_packages = tuple(
        sorted(set(required_packages) - {package.name for package in package_tuple})
    )
    current_by_package = {
        package.name: set(package.theorem_names) for package in package_tuple
    }
    for package_id, names in required_interfaces.items():
        if package_id in unavailable_required_packages:
            continue
...
def palomar_challenge_disposition(package: Package, root: Path) -> dict[str, Any]:
    ...
    challenge = lean_module_path(root, package.challenge_module)
    imports = imports_in(challenge)
    disallowed = sorted(
        module
        for module in imports
...
def source_bound_package_matches(
    ...
    for package in portfolio.packages:
        solution = lean_module_path(root, package.solution_module)
...
def palomar_entry_outputs(
    ...
    for package in portfolio.packages:
        ...
        challenge_path = lean_module_path(root, package.challenge_module)
        solution_path = lean_module_path(root, package.solution_module)
...
def render_membership(portfolio: Portfolio) -> str:
    ...
    payload = {
        ...
        "current_package_ids": sorted(package.name for package in portfolio.packages),
        "unavailable_required_package_ids": list(portfolio.unavailable_required_packages),
        "current_package_count": len(portfolio.packages),
        "unavailable_required_package_count": len(portfolio.unavailable_required_packages),
        "growth_policy": (
            "New eligible config-owned packages and theorem interfaces are included "
            "automatically. Historical required rows absent at the current checkout are "
            "retained explicitly as unavailable rather than silently counted as current "
            "or forcing restoration of unrelated legacy source trees."
        ),
        "boundary": (
