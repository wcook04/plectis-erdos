...
def claim_registration(programme_claim_id: str, modules: list[dict], claims: dict | None) -> dict:
    """Join exact registry coordinates without promoting unlisted declarations."""
    if claims is None:
        return {"state": "unknown", "source": "docs/claims.json",
                "programme_claim_id": None, "registered_claim_ids": []}
    paths = {library_identity_path(row["path"]) for row in modules}
    rows = claims.get("claims", [])
    programme = next((row for row in rows if row.get("id") == programme_claim_id), None)
    registered = sorted({
        row["id"] for row in rows
        if any(library_identity_path(declaration.get("module", "")) in paths
               for declaration in row.get("declarations", []))
    })
    result = {
        "state": ("registered_declarations" if registered else
                  "programme_only" if programme else "not_registered"),
        "source": "docs/claims.json",
        "programme_claim_id": programme_claim_id if programme else None,
        "registered_claim_ids": registered,
        "boundary": "Only the listed claim rows carry their recorded public status; other declarations are not promoted by module membership or a kernel build.",
    }
    if programme:
        result["programme_status"] = programme["status"]
        result["programme_statement"] = programme["statement"]
    return result


def render_programme_card(payload: dict, claims: dict) -> str:
    """Publish the same programme boundaries as the claim owner, without new judgement."""
    lines = ["_Generated by `scripts/build_problem_index.py`; edit the claim or problem owner._",
             "", claims["external_verification_packet"]["boundary"], ""]
    for row in payload["problems"]:
        registration = row["claim_registration"]
        if not registration.get("programme_claim_id"):
            raise ValueError(f"{row['problem_id']}: no registered programme boundary")
        lines.extend([
            f"**#{row['erdos_number']} — {row['question']}**", "",
            registration["programme_statement"], "",
            f"One recorded result: {row['what_is_checked'][0]}", "",
            f"[Short paper](../../{row['note']['rendered_path']}) · "
            f"[Source](../../{row['note']['source_path']})", "",
        ])
    lines.extend(["The examples are not an exhaustive result list. `docs/problems.json` carries",
...
def render_problem_questions(payload: dict, *, prose: bool = False) -> str:
    ...
    lines = ["_Questions generated from `docs/problem_index_source.json`._", "",
             "| Problem | Question |", "|---|---|"]
    for row in payload["problems"]:
        lines.append(f"| [#{row['erdos_number']}](../{row['note']['rendered_path']}) | "
                     f"{row['question'].replace('|', '&#124;')} |")
    return "\n".join(lines)


def render_contribution_guide(payload: dict, corpus: dict) -> str:
    """Join existing mathematics and source routes; never invent starter tasks."""
    lines = ["# Work on a paper", "",
    ...
    for row in payload["problems"]:
        ...
        lines += [f"## Problem {n}", "", row["question"], "",
                  row["what_is_checked"][0], "", row["claim_registration"]["programme_statement"], ""]
        papers = [paper for paper in corpus.get("papers", [])
...
def build(
    ...
    for row in source["problems"]:
        ...
        registration = claim_registration(
            row.get("programme_claim_id", row["problem_id"]), modules, claims
        )
        problem = {
            "problem_id": row["problem_id"],
            "erdos_number": row["erdos_number"],
            "short_title": row["short_title"],
            "status": registration.get("programme_status", row["status"]),
            "question": row["question"],
            "library_root": "ErdosProblems.lean",
            "claim_registry_status": registration["state"],
...
def main() -> int:
    ...
    if args.check:
        if not CONTRIBUTION_GUIDE.is_file() or CONTRIBUTION_GUIDE.read_text(encoding="utf-8") != contribution_projection:
            print("paper contribution guide is stale; run python3 scripts/build_problem_index.py")
            return 1
    ...
    CONTRIBUTION_GUIDE.write_text(contribution_projection, encoding="utf-8")
