...
theorem twentyOne_qRow_cocycle
  ...
  unfold TwentyOneQRowExact twentyOneSuffixReservoir
    twentyOneBorrowContribution at hrow ⊢
...
theorem twentyOne_failedBorrow_prefix_interval
  ...
  have hgapQ :
    ...
    exact_mod_cast (show 1 ≤ localPrefixQuotient D (M + 1) -
      twentyOneQuotientTarget (M + 1) by omega)
  have hgapUpperQ :
      (localPrefixQuotient D (M + 1) : ℚ) -
          twentyOneQuotientTarget (M + 1) ≤ D.card := by
  ...
  have hlower : (1 / 21 : ℚ) < localMersennePrefixValue D := by
    have hdiff : 0 < localMersennePrefixValue D - (1 / 21 : ℚ) :=
      pos_of_mul_pos_left hscaledPos hpowPos.le
    linarith
  have hscaledUpper :
      (2 : ℚ) ^ (M + 1) *
          (localMersennePrefixValue D - (1 / 21 : ℚ)) ≤
