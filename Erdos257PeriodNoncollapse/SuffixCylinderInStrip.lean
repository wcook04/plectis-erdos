import Erdos257PeriodNoncollapse.SuffixCylinderFeedbackFanIn
...
namespace Erdos257PeriodNoncollapse.SuffixCylinderInStrip
...
open Erdos257PeriodNoncollapse.SuffixCylinderTwoSheet
    ...
    have hchildCoeff :
      ...
      · have hbit : bit q = true := by simp [bit, hb]
        have hsupp : wordSupport (child q hqL hqU) =
          ...
          simp [hbit]
        rw [hsupp]
        rw [Erdos257PeriodNoncollapse.HalfCylinderFiniteShadow.supportCoeff_insert_self
          _ (by omega)]
        · push_cast
          rw [hbaseCoeff]
          simp [hb]
...
noncomputable def CylinderStage.feedbackSplitCrossingInStrip
  ...
  let Wbase : SelectedHalfWindow N δ :=
    Erdos257PeriodNoncollapse.SuffixCylinderTwoSheet.SelectedHalfWindow.take
      S.window δ hδltR.le
...
theorem CylinderStage.feedbackAdvance_or_inStripTwoSheet
  ...
  · left
    exact ⟨
      (Erdos257PeriodNoncollapse.SuffixCylinderGlobalProducer.CylinderStage.promoteLow
  ...
  · by_cases habove :
    ...
    · have hcrossLow : 2 ^ (N - (K + 1)) < S.endpoint := by omega
      ...
      · left
        exact ⟨
          Erdos257PeriodNoncollapse.SuffixCylinderBelow.CylinderStage.feedbackAdvanceCrossingBelow
            S hN hK1N hcrossLow hcrossHigh c
            (by simpa [δ] using hprofile) hholeBelow⟩
      · by_cases hholeAbove : halfStripBound (N + 1) < 2 * δ - c
        · left
          exact ⟨
            Erdos257PeriodNoncollapse.SuffixCylinderTwoSheet.CylinderStage.feedbackAdvanceCrossingAbove
              S hN hK1N hcrossLow hcrossHigh c
              (by simpa [δ] using hprofile) hholeAbove⟩
        · right
          exact ⟨
            Erdos257PeriodNoncollapse.SuffixCylinderInStrip.CylinderStage.feedbackSplitCrossingInStrip
...
end Erdos257PeriodNoncollapse.SuffixCylinderInStrip
