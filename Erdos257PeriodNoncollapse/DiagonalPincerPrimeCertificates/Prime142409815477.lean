...
import Erdos257PeriodNoncollapse.DiagonalPincerPrimeCertificates.Prime11867484623
...
theorem prime_lucas_142409815477 : Nat.Prime 142409815477 := by
  have hfermat : (2 : ZMod 142409815477) ^ (142409815477 - 1) = 1 := by
    rw [← binaryPow_eq_pow, binaryPow_zmod, natCast_zmod_eq_one_iff]
    decide +kernel
  have hfactor_0 : (2 : ZMod 142409815477) ^ ((142409815477 - 1) / 2) ≠ 1 := by
    rw [← binaryPow_eq_pow, binaryPow_zmod, natCast_zmod_ne_one_iff]
    decide +kernel
  have hfactor_1 : (2 : ZMod 142409815477) ^ ((142409815477 - 1) / 3) ≠ 1 := by
    rw [← binaryPow_eq_pow, binaryPow_zmod, natCast_zmod_ne_one_iff]
    decide +kernel
  have hfactor_2 : (2 : ZMod 142409815477) ^ ((142409815477 - 1) / 11867484623) ≠ 1 := by
    rw [← binaryPow_eq_pow, binaryPow_zmod, natCast_zmod_ne_one_iff]
    decide +kernel
