...
/-- The elementary linear budget needed to separate two consecutive middle
transitions. -/
private theorem ten_mul_sub_two_add_four_le_two_pow_succ
    (s : ℕ) (hs10 : 10 ≤ s) :
    10 * (s - 2) + 4 ≤ 2 ^ (s + 1) := by
  induction s, hs10 using Nat.le_induction with
  | base => norm_num
  | succ s hs ih =>
      have hten : 10 ≤ 2 ^ (s + 1) := by
        calc
          10 ≤ 2 ^ 4 := by norm_num
          _ ≤ 2 ^ (s + 1) :=
            Nat.pow_le_pow_right (by norm_num) (by omega)
...
/-- Two consecutive middle transitions are impossible from row ten onward.
The first middle landing leaves the next source remainder within only the
linear pulse budget of `2^s`; the next middle inequality would require the
same remainder to lie below the incompatible half-scale wall. -/
theorem middlePrev_forces_not_middleInequality
    {s : ℕ} (hs10 : 10 ≤ s)
    (hpcarry :
      ¬ (seamAdjacentCut (s - 1) (by omega : 5 ≤ s - 1)).successorCarries)
    (hpmiddle :
      4 * (seamAdjacentCut (s - 1) (by omega : 5 ≤ s - 1)).remainder +
            (seamPerturbedFamily (s - 1) (by omega : 3 ≤ s - 1)).gap -
            (seamAdjacentCut (s - 1) (by omega : 5 ≤ s - 1)).belowPulse <
          (seamAdjacentCut (s - 1) (by omega : 5 ≤ s - 1)).terminalWeight) :
    ¬ (4 * (seamAdjacentCut s (by omega : 5 ≤ s)).remainder +
            (seamPerturbedFamily s (by omega : 3 ≤ s)).gap -
            (seamAdjacentCut s (by omega : 5 ≤ s)).belowPulse <
          (seamAdjacentCut s (by omega : 5 ≤ s)).terminalWeight) := by
  intro hmiddle
  have hlow := seamMiddleReset_sourceRemainder_ge_powShift_of_middlePrev
    (s := s) (by omega) (by omega) hpcarry hpmiddle
  have hpulse := seamAdjacentCut_belowPulse_le
    (s := s) (by omega : 5 ≤ s)
  have hrem := seamAdjacentCut_remainder
    (s := s) (by omega : 5 ≤ s)
  have hgap := seamAdjacentCut_gap_eq
    (s := s) (by omega : 3 ≤ s)
  have hterm := seamAdjacentCut_terminalWeight_eq
    (s := s) (by omega : 5 ≤ s)
  have hlinear := ten_mul_sub_two_add_four_le_two_pow_succ s hs10
  rw [hrem, hgap, hterm] at hmiddle
  omega

/-- The middle reset has positive orientation exactly when its source pulse
is strictly smaller than four times its source remainder. -/
theorem middleResetDeviation_pos_iff_pulse_lt_four_remainder
    {s : ℕ} (hs5 : 5 ≤ s)
    ...
    0 < seamResetDeviation s ↔
      (seamAdjacentCut s hs5).belowPulse <
        4 * (seamAdjacentCut s hs5).remainder := by
  rw [middleResetDeviation_eq hs5 hncarry hmiddle]
  push_cast
  omega

/-- The half-row source bound is a sufficient scalar producer for strict
middle orientation. -/
theorem middleResetDeviation_pos_of_halfRowRemainder
    {s : ℕ} (hs5 : 5 ≤ s)
    (hncarry : ¬ (seamAdjacentCut s hs5).successorCarries)
    (hmiddle :
      4 * (seamAdjacentCut s hs5).remainder +
            ...
            (seamAdjacentCut s hs5).belowPulse <
          (seamAdjacentCut s hs5).terminalWeight)
    (hhalf : s - 1 ≤ 2 * (seamAdjacentCut s hs5).remainder) :
    0 < seamResetDeviation s := by
  apply (middleResetDeviation_pos_iff_pulse_lt_four_remainder
    hs5 hncarry hmiddle).2
  have hpulse := seamAdjacentCut_belowPulse_le hs5
  omega

/-- Every upper reset has strictly negative orientation.  Strictness comes
from the positive old-scale overshoot of the adjacent upper word. -/
theorem upperResetDeviation_neg
    {d : ℕ} (hd5 : 5 ≤ d)
    (hcarry : (seamAdjacentCut d hd5).successorCarries) :
    seamResetDeviation d < 0 := by
  rw [upperResetDeviation_eq hd5 hcarry]
  have hover : 0 < (seamAdjacentCut d hd5).overshoot := by
    unfold HalfCylinderIntegerGreedy.PerturbedFamily.AdjacentCut.overshoot
...
#print axioms middlePrev_forces_not_middleInequality
#print axioms middleResetDeviation_pos_iff_pulse_lt_four_remainder
#print axioms middleResetDeviation_pos_of_halfRowRemainder
#print axioms upperResetDeviation_neg
