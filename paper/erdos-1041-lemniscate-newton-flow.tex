...
Signed-moment cyclic tetranomials & Lean-checked two-root safe-spoke theorem & For a finite root family of $g(w)=w^m+aw^r+bw^s+c$, an exact signed $L^2$ moment budget selects two distinct roots whose complete spokes are safe; $|c|+|b||w|^s<1$ and $|b|+|c|\le1$ remain useful pointwise sufficient conditions. \\
...
There is a stronger, genuinely two-root formal theorem.  Let $S$ be a finite
family of roots, $N=|S|\ge2$, and
\[
  M=\sum_{i\in S}w_i^s.
\]
