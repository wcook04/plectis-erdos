...
\subtitle{Checked dynamical inputs and the remaining metric problem for Erd\H{o}s~\#1041}
\author{Will Cook}
\date{August 2026}
...
Two exact inputs isolate the obstruction. At every critical point, two
nearest-root occurrences have total Euclidean distance at most $2$; Lean
checks the reciprocal critical balance and disk inverse-square estimate used
by the ordinary polynomial-level argument. This does not prove containment.
An exact quintic has a nearest straight spoke that leaves $\{|f|<1\}$, and an
exact cubic has an escaping midpoint on every straight root-pair segment.
...
The remaining metric problem is therefore one of admissible-hub selection.
On the ray-separated dense class, one must find a critical hub $c$ whose two
inverse-ray arms stay in the relevant component and satisfy
\[
