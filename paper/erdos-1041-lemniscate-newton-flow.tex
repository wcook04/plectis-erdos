...
\hypersetup{
  pdftitle={The complete cubic case and explicit paths in polynomial lemniscates for Erdős Problem 1041},
...
\runninghead{Erd\H{o}s \#1041: the complete cubic case}
\title{The Complete Cubic Case and\\Explicit Paths in Polynomial Lemniscates}
\subtitle{Critical-value hubs, solved families, and the general boundary for Erd\H{o}s Problem~\#1041}
\author{Will Cook}
...
\begin{abstract}
Every monic cubic whose roots lie in the open unit disc has two root
occurrences joined inside $\{|f|<1\}$ by a broken line of length less than
$2$; for a squarefree cubic the endpoints are distinct.  A critical point
with the least critical-value modulus is the hub.  A reflected-polynomial
inequality puts this modulus below $1$, and a Schur estimate supplies two
nearby roots after normalization.  The root equation controls both segments.
We also give the origin as an explicit hub for monic trinomials in every
degree, and develop low-critical-value and constant-factor estimates for
general polynomials.  The unrestricted degree problem remains open.
\end{abstract}

\section{The complete cubic case}
\label{sec:cubic-complete}

Erd\H{o}s Problem~\#1041 asks whether two roots of a monic polynomial,
all of whose roots lie in the open unit disc, can be joined inside
$\{|f|<1\}$ by a curve shorter than $2$.  A short Euclidean segment need
not stay in this set.  For cubics, a critical point provides a hub at which
both containment and length can be controlled.

\begin{theorem}[complete cubic case]\label{res:cubic-complete}
Let
\[
 f(z)=\prod_{i=1}^{3}(z-z_i),\qquad |z_i|<1.
\]
There are distinct indices $i,j$ and a point $c\in\mathbb C$ such that the
broken line $z_i\to c\to z_j$ lies in $\{|f|<1\}$ and has length strictly
less than $2$.  It is a continuous curve of bounded variation.  If $f$ is
squarefree, its endpoints are distinct values.  If a root is repeated, two
occurrences of that root may be joined by the constant curve.
\end{theorem}

The proof chooses a critical point with the least critical-value modulus.
A reflected-polynomial estimate makes this modulus smaller than $1$.
After translation and scaling, a cubic root-count estimate supplies two
roots at distance at most $1$ from the hub.  The root equation then controls
the whole of each segment, not just its endpoints.

\begin{proof}
The repeated-root case is immediate, so assume $f$ is squarefree.  Choose a
critical point $c$ for which $|f(c)|$ is least, and write $v=f(c)$.
The critical-value estimate below gives $0<|v|<1$.  Write the other critical
point, with multiplicity, as $c+\delta$.  Integration of
$f'(c+x)=3x(x-\delta)$ gives
\[
...
\]
Minimality of $|v|$ gives
$|1-B^3/2|=|f(c+\delta)/v|\ge1$.  The root-count estimate below supplies
two distinct roots $u,w$ of $P_B$ in the closed unit disc.
For either such root $q$ and $0\le t\le1$, its root equation gives
\[
 P_B(tq)=1-t^2-t^2(1-t)q^3,\qquad
 |P_B(tq)|\le1-t^3\le1.
\]
Thus both segments from $c$ to $c+\alpha u$ and $c+\alpha w$ lie in
$\{|f|<1\}$, since their images have modulus at most $|v|<1$.
Their concatenation has length
\[
 |\alpha|(|u|+|w|)\le2|v|^{1/3}<2.
\]
The two roots are distinct because $\alpha\ne0$.  This is the asserted
piecewise affine curve; in the formal statement its length is the extended
variation on $[0,2]$.
\end{proof}

The \href{https://github.com/wcook04/plectis-erdos/blob/3be82b1a7340284aea72e9a5c8493cb020843921/ErdosProblems/Erdos1041/PaperCubicMonic.lean#L32}{monic-cubic formal endpoint} starts directly from
monicity, degree three and the open-disc root condition.  The fundamental
theorem of algebra supplies the root enumeration, including multiplicities,
so no factorisation is an additional hypothesis.  It produces the continuous
polygonal connector and its variation bound; squarefreeness makes its two
root endpoints distinct.

\paragraph{Why a critical value is smaller than one.}
The reflected-polynomial inequality, for a monic polynomial of degree $n\ge2$
with roots in the closed unit disc and critical points $c_1,\ldots,c_{n-1}$
listed with multiplicity, is
\[
 |f(c_j)|\le\prod_{k=1}^{n-1}|1-\overline{c_k}c_j|.
\]
For a cubic, let $c,d$ be its two critical points, with
$r=|c|\ge|d|$.  Gauss--Lucas gives $r\le1$.  If $r>0$, the displayed
inequality implies
\[
 |f(c)|\le(1-r^2)|1-\overline d c|
 \le(1-r^2)(1+r^2)=1-r^4<1.
\]
If $r=0$, both critical points are zero, so $f(z)=z^3+f(0)$;
Vieta and the strict root-disc hypothesis give $|f(0)|<1$.
Taking the smaller of the two critical-value moduli proves the needed
bound.  Squarefreeness makes that minimum nonzero.

\paragraph{Why the normalized cubic has two small roots.}
...
\]
Here the closed disc is sufficient: the strict factor $|v|<1$ already
provides both strict conclusions in the main proof.
To see the mechanism, write $a=3B/2$ and suppose two roots $r,s$ of
$z^3-az^2+1$ have modulus greater than $1$.  Vieta, with
$x=1/r$, $y=1/s$ and $h=x+y$, gives
\[
...
\]
Indeed its left side is at most
$(1-|y|^2)|x|+(1-|x|^2)|y|
=(|x|+|y|)(1-|xy|)$, and
$|x|+|y|<1+|xy|$.
Putting $t=|h|^2$ and $X=\operatorname{Re}(h^3)$ therefore gives
\[
...
\]
and
$Q(X)-Q(X_0)=2(X-X_0)(7t^3+8X+8X_0+4)>0$.
Using $a^3=(h^3-1)^3/h^3$, the positive expression is
$t^3(27\operatorname{Re}(a^3)-2|a^3|^2)$.
Consequently $|1-4a^3/27|<1$, contradicting $|1-B^3/2|\ge1$.
Finally, a repeated root of $z^3-az^2+1$ forces $1-4a^3/27=0$.
Under the hypothesis all three roots are therefore simple, and at most one
is outside the closed unit disc, as required.

The complete cubic statement is checked in
\href{https://github.com/wcook04/plectis-erdos/blob/bb4e24651b37cd096d3749ef299c3317930b3a3b/ErdosProblems/Erdos1041/PaperCubicCompletion.lean#L297}{Lean},
as are the
\href{https://github.com/wcook04/plectis-erdos/blob/bb4e24651b37cd096d3749ef299c3317930b3a3b/ErdosProblems/Erdos1041/PaperReflectedCompletion.lean#L254}{reflected critical-value inequality}
and the
\href{https://github.com/wcook04/plectis-erdos/blob/bb4e24651b37cd096d3749ef299c3317930b3a3b/ErdosProblems/Erdos1041/PaperCubicSchur.lean#L283}{normalized cubic root count}.
These statements settle the cubic case.  They do not supply a connector of
length less than $2$ for arbitrary degree.  The next section gives a different
explicit hub, the origin, for monic trinomials of every degree.
...
$|\zeta_1|+|\zeta_2|<2$.
Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperTrinomial.lean#L38}{trinomial broken-line theorem with extended variation as length}.
\end{proof}
...
available.  Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperTrinomial.lean#L79}{prescribed-spoke counterexample at a supplied radius}.
...
Pendyala \cite[Thm.~1, p.~1]{june2026} proves the complete degree-four case.
...
\pdecl{allStraightCubic_every_pair_midpoint_escapes}, and Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperStraightObstructions.lean#L178}{polynomial counterexamples with the nearest-root condition quantified over every zero}.  The first defeats a
...
Pendyala's degree-four theorem \cite[Thm.~1, p.~1]{june2026} has no
...
\noindent Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperSeparationCounterexample.lean#L189}{separation counterexample including the least-modulus assertion}.
...
The \href{https://github.com/wcook04/plectis-erdos/blob/3be82b1a7340284aea72e9a5c8493cb020843921/ErdosProblems/Erdos1041/FreePointCentralCompletion.lean#L39}{checked central-region theorem} now has only the point
hypotheses $m\ge1$ and $|c_j|\le\sqrt{1-e^{-2}}$: it gives the equal-weight
geometric row-mean bound, with the logarithmic series inputs discharged.
This is a formalisation of a restricted region of the stronger ordinary
weighted inequality above.
...
free-point conclusion above remains restricted to the central region.
...
The circle comparison, finite Taylor-coefficient identity, Poisson norm-square
majorization and absolute circle-series transport have checked proofs; their
full weighted-energy and maximum-principle assembly remains ordinary.
...
have checked kernels, as do the Poisson majorization and absolute series
transport above; the full weighted-energy and maximum-principle assembly
remains ordinary.  The complete declaration-to-claim map is retained in the
...
Pendyala's degree-four theorem \cite[Thm.~1, p.~1]{june2026} is directly comparable.
...
\bibitem{pendyala2026shortest} V.~S. Pendyala,
  ...
  Theorem~1.2, p.~3, and introductory discussion, p.~2.
