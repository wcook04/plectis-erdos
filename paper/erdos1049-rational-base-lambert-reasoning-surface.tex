...
$q$-logarithm of that paper.  Section~\ref{sec:region} identifies an admissible
...
of its Theorem~1 goes through.  At the direction fixed above the parameter tuple
is $c=(13,14,12,14,15,13)$ with $m(c)=15$ and $s(c)=n$, so $s(c)>0$, and the
source's~(14) holds because $12n+1\le14n+1$ and $26n+2\le27n+2\le28n+2$.  The
lemma therefore applies for every $n\ge1$.  The linear-form identity~(9)--(11) gives
...
gives $\deg U_n=W_n$ and $\deg V_n\le W_n-1$.
...
\texttt{RationalBaseContour}, which is held in the external-verification
package outside this release and is not carried in any cut of it.  The Lean library indexed in
...
Theorem~\ref{res:region} with $b\le3$ is already theirs.  Their printed
...
Theorem~\ref{res:region} is the reciprocal of the smaller integer-base bound
$\mu=C_1/C_0$ printed at \cite[p.~162]{zudilin2004}.  The remark following
Theorem~\ref{res:archcap} records that reciprocal relation for every family
...
Theorem~\ref{res:region}, applied to the forms that carry the smaller exponent.
...
gained are exactly the strip $s^{\mu}<r<s^{\mu_{\mathrm{BV}}}$, which is empty
for $s=2$ and $s=3$ and is first occupied at $s=4$ by the single numerator
$31$.  The kernel-checked declarations of Theorem~\ref{res:31over4} and of
Section~\ref{sec:regionbracket} decide the integer comparisons and the rational
...
module, \texttt{RationalBaseContour}, which is held in the external-verification
package outside this release and is not carried in any cut of it or in the library indexed above.  Its
