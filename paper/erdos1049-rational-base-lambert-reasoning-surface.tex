...
\begin{lemma}[the weight is an indicator]\label{res:omega-indicator}
...
\noindent Writing $\mathbf 1_{[u,v)}$ for indicators, Lemma~\ref{res:omega-indicator}
...
The proof occupies Sections~\ref{sec:source-forms}--\ref{sec:integer-forms}.  It
...
\begin{equation}\label{eq:positive-source-form}
 H_n(x)=\sum_{t\ge0}
...
\begin{equation}\label{eq:c-explicit}
...
Then
\begin{align}
 A_n(X)&=\sum_{k=a_2}^{\beta_n-1}c_{n,k}(X)X^{a_0k},\label{eq:A-explicit}\\
 B_n(X)&=\sum_{k=a_2}^{\beta_n-1}c_{n,k}(X)X^{a_0k}
 \biggl(\sum_{l=1}^{k-a_1}\frac1{X^{l}-1}
 +\sum_{j=1}^{a_0-1}\frac{X^{-j(k-a_1)}}{X^{j}-1}\biggr).\label{eq:B-explicit}
\end{align}
Put
\begin{equation}\label{eq:cyclotomic-products}
...
Lemma~\ref{res:omega-indicator} every $\nu_{n,l}$ is $0$ or $1$, and
$\nu_{n,l}=0$ for $l>15n$ because the intervals all begin at or above $1/14$.

Zudilin's Lemma~7 applies at the parameters above: the tuple is
...
\begin{equation}\label{eq:integer-polynomial-pair}
...
splitting the positive series $F(e^{h})=\sum_m\tau(m)e^{-mh}$ at
$m=\lfloor1/h\rfloor$ bounds the initial part above and below by constant
multiples of $h^{-1}\log(1/h)$ while the remaining terms total $O(h^{-1})$, so
...
over $\Q(X)$ are determined, and~\eqref{eq:integer-polynomial-pair} identifies
them.  This is a statement about $F$ as a function and says nothing about any
individual value.

\subsection{Exact degrees}\label{sec:degrees}
...
$U_n$ is a polynomial by~\eqref{eq:integer-polynomial-pair}, so subtracting
...
in~\eqref{eq:positive-source-form} tends to $1$ and the geometric factor
...
integrality~\eqref{eq:integer-polynomial-pair} gives
\begin{equation}\label{eq:V-degree}
 \deg V_n\le W_n-1 .
\end{equation}
One common homogenising width $W_n$ therefore serves both coefficients.

\subsection{The cyclotomic limit}\label{sec:cyclotomic-limit}
...
Fix one demi-interval $[u,v)$ of Lemma~\ref{res:omega-indicator}.  The condition
...
equation~\eqref{eq:exact-degree} yields
\begin{equation}\label{eq:degree-limits}
 \frac{K_n}{n^{2}}\to C_1,
 \qquad
...
by~\eqref{eq:integer-polynomial-pair}
\[
 \Lambda_n(x):=U_n(x)F(x)-V_n(x)=x^{-M_n}\frac{D_N(x)}{\Omega_n(x)}H_n(x)>0 .
\]
By~\eqref{eq:exact-degree} and~\eqref{eq:V-degree} both polynomial degrees are
at most $W_n$, so
\[
...
\]
the second equality by $\log x=\log a-\log b$ and~\eqref{eq:exact-degree}.
With~\eqref{eq:degree-limits} this proves
\begin{equation}\label{eq:final-limit}
 \lim_{n\to\infty}\frac{\log\widehat\Lambda_n}{n^{2}}=C_1\log b-C_0\log a .
\end{equation}

\begin{proof}[Proof of Theorem~\ref{res:region}]
The hypothesis $\log b/\log a<\theta^{*}=C_0/C_1$ makes the right side
of~\eqref{eq:final-limit} negative, so $\widehat\Lambda_n>0$ and
...
\noindent Above the threshold~\eqref{eq:final-limit} says that these same
...
Theorem~\ref{res:region}.  A finite-precision evaluation of the trigamma series
...
Section~\ref{sec:cyclotomic-limit} replaces them, so they are not part of the
citation set.  Kernel-checked are the finite height comparisons of
Theorem~\ref{res:31over4}, the rational bracket of
Section~\ref{sec:regionbracket}, and the four integer identities of the degree
step in Section~\ref{sec:degrees}.  Checked by exact computation are the
...
the reconstruction is described in Section~\ref{sec:receipts}.  No part of
...
Sections~\ref{sec:source-forms}--\ref{sec:integer-forms}.  Of these only the
...
Theorem~\ref{res:archcap} constrains families satisfying its hypotheses and
does not exclude every possible Pad\'e construction.

For the family of Section~\ref{sec:source-forms} the fourth hypothesis is the
size estimate proved there and the second is~\eqref{eq:exact-degree}.  The third
...
By Lemma~\ref{res:omega-indicator} every $\nu_{n,l}$ is $0$ or $1$, so
...
by~\eqref{eq:c-explicit}, and summing the at most $\beta_n$ terms
of~\eqref{eq:A-explicit} gives $\|A_n\|\le\beta_n2^{2\beta_n}$.  Hence
...
of~\eqref{eq:B-explicit} has modulus at most $1$; also
...
\noindent With Lemma~\ref{res:sourceheight}, Zudilin's family satisfies all four
hypotheses of Theorem~\ref{res:archcap}, with exact degree limit $d=C_1-C_0$
by~\eqref{eq:degree-limits}, and decay exponent $\sigma=C_0$ because
$\log\Lambda_n(x)=-(K_n-W_n)\log x+o(n^{2})$ for each fixed real $x>1$ by the
size estimate of Section~\ref{sec:integer-forms}.  For it $\sigma/(\sigma+d)=\theta^{*}$ and
...
Lemma~\ref{res:sourceheight} is an ordinary proof and is not kernel-checked.
...
\subsection{Finite receipts}\label{sec:receipts}
...
The forms of Section~\ref{sec:source-forms} were reconstructed as explicit
...
inside the bounds of Section~\ref{sec:integer-forms}.

The constant was evaluated independently of the source's printed digits from
$\omega$, the thirteen intervals of Lemma~\ref{res:omega-indicator} and the
...
convergence is $O(1/n)$, from the linear terms of $M_n$ and the $N\log N$ error
in the totient estimate.  Only the limit enters Theorem~\ref{res:region}.
...
\begin{lemma}[all-depth initial monomial]\label{res:allrowinitial}
...
\begin{proof}
Define an operator $\Phi$ on the free module with basis $(e_u)_{u\ge0}$ by
\[
...
\eqref{eq:state-expansion} only the state $u=0$ contributes at degree $E(m,j)$,
...
with $t>j$ contribute $h^{(t)}_{j-t}=0$, so Lemma~\ref{res:allrowinitial} at
$m=j+\ell$, where $E(j+\ell,j)=j(j+1)/2+j\ell$, gives
\begin{equation}\label{eq:row-initial}
...
\begin{proof}[Proof of Theorem~\ref{res:zudilin-sharp-qorder}]
The operators $D_j$ act by lower unitriangular row operations, so they leave
$\det(v_{i+j})_{0\le i,j<N}$ unchanged.  By~\eqref{eq:row-initial} the entry in
...
Theorem~\ref{res:zudilin-sharp-qorder} fixes the first nonzero power of $q$ and
...
cubic exponential factor that neither datum sees.  Closing the analytic question
for $V_N^{*}$ needs a separate estimate on the normalised residual
$R_N$ in $V_N^{*}=C_Nq^{B_N}R_N(q)$, for instance nonvanishing together with
$\log|R_N(2/3)|=o(N^{3})$.  No such estimate is proved here.
...
the leading coefficient are proved here.  The remaining obligation is a formal
one.  Lemma~\ref{res:allrowinitial} and~\eqref{eq:row-initial} give the initial monomial of every row, and the Lean
formalisation of that identity stops at $j=2$, so the Lean statement about
$V_N^{*}$ retains a row hypothesis that the proof above discharges.  Nothing in
...
Theorem~\ref{res:scalar} at the end of the section, whose subject is the scalar
...
Corollary~\ref{res:rankfortyone}; the corollary trades exactness for a
...
$81/200<\theta^{*}<1/2$ of Section~\ref{sec:regionbracket} and the integer
degree bookkeeping of Section~\ref{sec:degrees} lives in a further module,
\texttt{RationalBaseContour}, which the library root imports at the pinned
