...
where \(C_1=1091/2\) and \(C_0=266-(3/\pi^{2})(225-J)\) are the constants of
Zudilin's \((14,12,14;27)\) direction.  The first base this settles beyond the
published region of Bundschuh and V\"a\"an\"anen is \(31/4\), together with
every power \((31/4)^{r}\), \(r\ge1\).

The region theorem is an ordinary proof.  It cites Zudilin's Lemma~7 together
with the inputs of that lemma's own proof, and its remaining steps are proved
here in full.  The finite arithmetic that places \(31/4\) inside the region
and outside the earlier one is checked by the Lean kernel.  The rational
bracket \(81/200<\theta^{*}<1/2\) around the derived constant is checked in
the module \texttt{RationalBaseContour}, which belongs to the
external-verification cut.  This release's Lean library does not carry that
module.
...
inequality alone.  The all-depth recurrence, the initial monomial of the first
transformed row and the closed-form assembly are kernel-checked; the rows
\(j\ge3\) are open, so the Lean identification of the determinant itself stays
conditional on a row hypothesis.
...
(Theorem~\ref{res:region}).  The base $31/4$ and every power $(31/4)^{r}$ lie
in that region and outside the published region of Bundschuh and
V\"a\"an\"anen, whose cutoff is $1/2-1/\pi^{2}=0.398678816\ldots$
(Theorem~\ref{res:31over4}).  \textbf{Evidence.} An ordinary proof citing
...
$0.6309297535714574\ldots$ (Theorem~\ref{res:archcap}).
...
$q$-logarithm of that paper.  Section~\ref{sec:region} computes the constant
for $F$ itself, $c=\mu=2.464978683574975\ldots$, and derives the region and its
first new base from it.
...
treatment of $q$-logarithms keeps the integer hypothesis $p=1/q\in\Z\setminus\{0,\pm1\}$
throughout, and states in print that its methods do not sharpen the
$q$-harmonic case of~\cite{zudilin2004}
\cite[abstract p.~879; introduction p.~880]{matalaaho2006}.  So no printed
region for $F$ reaches $\log4/\log31$.
...
Theorems~\ref{res:region} and~\ref{res:31over4} are ordinary mathematics.  They
consume Zudilin's Lemma~7 together with the inputs of that lemma's own proof,
and every further step is proved in Section~\ref{sec:region}.  They are not
...
\paragraph{Structure.}
Section~\ref{sec:region} states and proves the region theorem, the base $31/4$
...
\section{The region \texorpdfstring{$b^{\mu}<a$}{b\textasciicircum mu < a} and the base \texorpdfstring{$31/4$}{31/4}}\label{sec:region}
...
\]
The constant $\mu$ is the irrationality-exponent bound of
\cite[Thm.~1, p.~154]{zudilin2004} for integer bases.
...
Write $x=a/b$ and keep the notation of \cite[Secs.~2--5]{zudilin2004}, with
$a_j=\alpha_j n+1$ and, for the source's upper parameter, $\beta_n:=\beta n+2=27n+2$
in place of the symbol $b$ that this note reserves for the denominator of the
base.  Further, $m=m(c)=15$, $N=15n$, and $M_n=266n^{2}+34n+1$ is the integer of
that paper's~(16).

\emph{Step 1, the forms.}  Zudilin's Lemma~7 states, for parameters satisfying
his~(14) and $s(c)>0$, that
$p^{-M(c)}D_{m(c)}(p)\,\Omega^{-1}(p)\,H(c)\in\Z[p]\,h_p(1)+\Z[p]$, an inclusion
between polynomial rings; the paper's own display~(24) distinguishes $\Z[p]$
from its evaluation, and that reading is the one under which the source's proof
of its Theorem~1 goes through.  The linear-form identity~(9)--(11) gives
$H_n=A_nF-B_n$ with $A_n,B_n\in\Q(x)$.  Because $F$ is not a rational function
of $x$, the pair $(U_n,V_n)$ supplied by Lemma~7 is forced to be
$U_n=x^{-M_n}D_N A_n/\Omega_n$ and $V_n=x^{-M_n}D_N B_n/\Omega_n$, and Lemma~7
says these two rational functions lie in $\Z[x]$.  The nonrationality of $F$ is
proved from the $\varepsilon^{-1}\log(1/\varepsilon)$ growth of $F(1+\varepsilon)$,
which no nonzero rational function has.

\emph{Step 2, degrees.}  The $x$-degrees of the summands of $A_n$ increase
strictly, with step $\alpha_0+\alpha_1+\alpha_2-k-2=40n+1-k$ for
$k\le\beta_n-2$, so no cancellation occurs at the top and
$K_n:=\deg A_n=(1091n^{2}+81n+2)/2$, whence $K_n/n^{2}\to C_1$.  Setting
$W_n:=K_n-M_n+\sum_{l\le N}\varphi(l)-\sum_{l\le N}\nu_l\varphi(l)$ gives
$\deg U_n=W_n$ and $\deg V_n\le W_n-1$.

\emph{Step 3, homogenisation.}  Since both degrees are at most $W_n$, the
numbers $\widehat U_n=b^{W_n}U_n(a/b)$ and $\widehat V_n=b^{W_n}V_n(a/b)$ are
integers, so
$\widehat\Lambda_n=b^{W_n}\Lambda_n(a/b)=\widehat U_nF(a/b)-\widehat V_n$ lies
in $\Z F(a/b)+\Z$.  This is the only non-Archimedean step, and it is where the
denominator $b$ is paid for.

\emph{Step 4, positivity and size.}  Every factor of the summand
$R(q^{t})$ of $H_n$ is positive for real $x>1$, and $D_N(x)>0$,
$\Omega_n(x)>0$, so $\widehat\Lambda_n>0$ for every $n$.  Each
$q$-Pochhammer factor lies in $[(q;q)_\infty,1]$, which bounds $\log H_n(x)$
uniformly in $n$, and the cyclotomic correction over $l\le N$ costs
$O_x(N\log N)$.  Hence
$\log\widehat\Lambda_n=K_n\log b-(K_n-W_n)\log a+o(n^{2})$.

\emph{Step 5, the limit and the contradiction.}  A Mertens estimate for
$\sum_{l\le y}\varphi(l)$ together with the block decomposition of the
condition $\{n/l\}\in[u,v)$ gives
$(K_n-W_n)/n^{2}\to266-(3/\pi^{2})(225-J)=C_0$.  Therefore
$n^{-2}\log\widehat\Lambda_n\to C_1\log b-C_0\log a$, which is negative exactly
when $\log b/\log a<C_0/C_1=\theta^{*}$, so $\widehat\Lambda_n\to0$.  If
$F(a/b)=P/Q$ with integers $Q\ge1$, then $Q\widehat\Lambda_n=P\widehat U_n-Q\widehat V_n$
is a positive integer for every $n$ and tends to $0$, a contradiction.
\end{proof}
...
Section~\ref{sec:regionbracket}.  Theorem~\ref{res:region} then applies.
\end{proof}

\subsection{The rational bracket around \texorpdfstring{$\theta^{*}$}{theta*}}\label{sec:regionbracket}

The constant $\theta^{*}$ is defined by a trigamma series, so a decidable
membership test needs explicit rational bounds on it.  Two are used.  The
thirteen $k=0$ terms of the trigamma differences give $J\ge776/10$, hence
$C_0>88371/400$ and therefore $81/200<\theta^{*}$.  In the other direction
$J\le\psi_1(1/14)<198$ gives $C_0<266$ and therefore $\theta^{*}<532/1091<1/2$.
So
\[
 \frac{81}{200}<\theta^{*}<\frac12 .
\]
Both bounds, the definitions of $C_0$, $C_1$ and $\theta^{*}$ from the
trigamma series, the membership of $31/4$ and of every power, and the exclusion
of $3/2$ are checked by the Lean kernel in the module
\texttt{RationalBaseContour}, which this release carries in its
external-verification cut.  The Lean library indexed in
Appendix~\ref{app:index} does not carry that module.  The declarations are \texttt{zudilinJ\_ge},
\texttt{zudilinC0\_gt}, \texttt{eightyOne\_twoHundredths\_lt\_zudilinContour},
\texttt{zudilinContour\_lt\_half}, \texttt{thirtyoneFour\_mem\_zudilinContourRegion},
\texttt{thirtyoneFour\_power\_mem\_zudilinContourRegion} and
\texttt{threeHalves\_outside\_zudilinContourRegion}.  A finite-precision
evaluation of the trigamma series with the integral tail bound over its first
thousand terms gives the sharper enclosure
$0.4056830211<\theta^{*}<0.4056830214$; that enclosure is a numerical receipt
and is not kernel-checked.

\subsection{Evidence, attribution and the remaining obligation}

\paragraph{Evidence decomposition.}
Theorems~\ref{res:region} and~\ref{res:31over4} are ordinary proofs.  Their
...
identity~(9)--(11) \cite{zudilin2004}.  Everything else in the outline above is
proved here: the nonrationality of $F$, the transfer of Lemma~7 to explicit
polynomials, the degree computation, positivity, the Archimedean size estimate,
and the limit $(K_n-W_n)/n^{2}\to C_0$.  That last limit is the content of the
source's Lemmas~1 and~2, and the elementary proof given above replaces them, so
they are not part of the citation set.  Kernel-checked are the finite height
comparisons of Theorem~\ref{res:31over4} in this library and the rational
bracket of Section~\ref{sec:regionbracket} in the external-verification cut.
Checked by exact computation are the reconstructions of
$U_n$ and $V_n$ and of every degree and exponent claim for $n\le4$, together
with the numerical values of $J$, $C_0$ and $\theta^{*}$; those are
computational receipts.  No part of
Theorem~\ref{res:region} is a Lean theorem.
...
Theorem~\ref{res:region} with $b\le3$ is already theirs.  Duverney proved an
...
$\mu$ under the standing hypothesis $p=1/q\in\Z\setminus\{0,\pm1\}$, and it
states no rational-base result \cite[Sec.~2, p.~154; Thm.~1]{zudilin2004}.  The
contribution here is the explicit constant $c=\mu$ for $F$ itself, the region
that constant defines, and its first new base.

\paragraph{The exact remaining obligation.}
Theorem~\ref{res:region} settles no base with $\log b/\log a\ge\theta^{*}$, and
...
identity~(9)--(11), the five elementary steps above, and the limit passage in
Step~5.  None of those is formalised here.

\begin{theorem}[degree-budget cap on the mechanism]\label{res:archcap}
...
follows from hypotheses~(2) and~(4), and the degree bound is what makes
$b^{d_n}U_n(a/b)$ and $b^{d_n}V_n(a/b)$ integers.  For $\sigma\le\delta$, fix an
integer $p\ge2$, write $q_n=|U_n(p)|$ and $p_n=\operatorname{sign}(U_n(p))V_n(p)$;
hypotheses~(2) and~(3) give $q_n=e^{(h+\delta\log p)n^{2}(1+o(1))}$, and
hypothesis~(4) makes $|q_nF(p)-p_n|=e^{-\sigma n^{2}\log p\,(1+o(1))}$ two-sided.
A standard minimal-$n$ argument then bounds the irrationality exponent of $F(p)$
by $1+(h+\delta\log p)/(\sigma\log p)$.  Since $F(p)$ is irrational for every
integer $p\ge2$~\cite{erdos1948,borwein1991}, Dirichlet gives
$\mu(F(p))\ge2$, so $(\sigma-\delta)\log p\le h$ for every such $p$; the three
constants do not depend on $p$, so letting $p\to\infty$ gives $\sigma\le\delta$.
The exact-degree statement is the same argument run with $d+\varepsilon$ in
place of $\delta$ for every $\varepsilon>0$.
\end{proof}
...
logarithm is zero and these hypotheses decide neither behaviour.  Zudilin's
family satisfies all four hypotheses with the exact degree limit $d=C_1-C_0$
and decay exponent $\sigma=C_0$, so for it
$\sigma/(\sigma+d)=\theta^{*}$ and $(\sigma+d)/\sigma=\mu$: within this family
the rational-base threshold is the reciprocal of the integer-base
irrationality-exponent bound.  Theorem~\ref{res:archcap} constrains families
satisfying its hypotheses and does not exclude every possible Pad\'e
construction.

\paragraph{The region, explicitly.}
Among coprime $a/b$ with $a\le60$ the region of Theorem~\ref{res:region} has
...
$s^{\mu}<r<s^{\mu_{\mathrm{BV}}}$, which is empty for $s=2$ and $s=3$, is
...
determinant}\label{sec:hankel-order}

The charge comparison of Corollary~\ref{res:sharpgaps} is a statement about how
...
coefficient]\label{res:zudilin-sharp-qorder}
...
\paragraph{Evidence decomposition.}
The proof of Theorem~\ref{res:zudilin-sharp-qorder} is ordinary mathematics: an
all-depth filtered reciprocal-state argument identifies the coefficient of the
minimal monomial in each transformed row, and the row product is then assembled.
Three parts of it are kernel-checked in this library.  The all-depth
...
checked theorem}.  The initial-monomial theorem for rows $j\ge2$ is outside this
library; row $j=2$, with initial monomial $18q^{2l+3}$, is checked in a module
of the external-verification cut, and rows $j\ge3$ are open, so the Lean
identification of $V_N^{*}$ itself with the assembled leading matrix stays
conditional on a row hypothesis.  Independently of the proof, the order and the
...
the leading coefficient are proved here.  The remaining obligation is the
initial-monomial theorem for every row $j\ge3$, which would discharge the row
hypothesis and make the Lean statement unconditional.  Nothing in this
subsection decides the arithmetic nature of $F(3/2)$.
...
$81/200<\theta^{*}<1/2$ of Section~\ref{sec:regionbracket} around the derived
constant of Theorem~\ref{res:region}, and its role is to make membership of the
...
Theorem~\ref{res:31over4}, which is where the irrationality of $F(31/4)$ and of
...
$81/200<\theta^{*}<1/2$ of Section~\ref{sec:regionbracket} lives in a further
module, \texttt{RationalBaseContour}, which this release carries in its
external-verification cut and not in the library indexed above.  Its
declarations are named in Section~\ref{sec:regionbracket} and are not linked
here, because no link coordinate for them exists at the pinned revision.
...
\textbf{Reader position.} 8 of 93; display band: front door.  Formal editorial disposition: hold.  These are separate classifications.
...
\textbf{Reader position.} 14 of 93; display band: major result.  Formal editorial disposition: hold.  These are separate classifications.
...
\textbf{Reader position.} 47 of 93; display band: mechanism.  Formal editorial disposition: hold.  These are separate classifications.
...
\textbf{Reader position.} 68 of 93; display band: frontier.  Formal editorial disposition: split.  These are separate classifications.
...
\textbf{Result-atom population.} 65 of 704 public coordinates. The public atom catalog groups them under this family.
...
\textbf{Reader position.} 73 of 93; display band: frontier.  Formal editorial disposition: hold.  These are separate classifications.
...
\textbf{Reader position.} 76 of 93; display band: frontier.  Formal editorial disposition: hold.  These are separate classifications.
...
\textbf{Reader position.} 86 of 93; display band: technical support.  Formal editorial disposition: hold.  These are separate classifications.
...
Binary selectors collide in the ambient four-jet signature at the stated rank and depth. For a modular tail whose every row is unimodular and whose adjacent minors vanish, all minors vanish and the sharper N<2\textasciicircum{}k collision threshold applies; at modulus 2\textasciicircum{}S 3\textasciicircum{}R with R>0, k>=S+2R suffices. Neither coordinate must be a unit: (2,3) modulo six is an example.

\textbf{Authority and reach.} Lean kernel; locally proved result and no-go result; novelty unassessed.

\textbf{Exact boundary.} The weaker unimodularity hypothesis and compressed selector threshold are formalized. The collision gives a kernel element, not nonvanishing of an analytic remainder or an irrationality conclusion.
...
\begin{itemize}
  \item \nolinkurl{ErdosProblems.Erdos1049.BezoutPluckerJets.adjacent_det_zero_forces_all_det_zero_of_isCoprime}
  \item \nolinkurl{ErdosProblems.Erdos1049.BezoutPluckerJets.zmod_binary_tail_collision_of_adjacent_det_zero_of_isCoprime}
  \item \nolinkurl{ErdosProblems.Erdos1049.BezoutPluckerJets.zmod_binary_tail_collision_of_two_three_depth_of_isCoprime}
\end{itemize}
...
\bibitem{matalaaho2006} T.~Matala-aho, K.~V\"a\"an\"anen and W.~Zudilin,
  ...
  \(p=1/q\in\Z\setminus\{0,\pm1\}\) is carried in the abstract on p.~879 and in
