...
polynomial bound $p_n\le1250(n+1)^4$ proved in Appendix~\ref{app:prime-bound}.
...
Theorem~\ref{res:infinite} below is that reduction in exact form, with the
...
Selberg's sieve.  Sections~\ref{sec:obstructions} draws two consequences from
it.
...
\paragraph{Outline.}
Section~\ref{sec:parts} proves the identity and the irrationality equivalence.
Section~\ref{sec:tail} develops the tail recurrence and states the three exact
criteria: pointwise shift escape, the free-pair form with a free offset, and
the lcm diagonal.  Section~\ref{sec:local-certificate} gives the local
obstruction, its signed normal form, the explicit remainder bound and one
certified actual pair.  Section~\ref{sec:denominator-floor} records the two
denominator floors.  Section~\ref{sec:obstructions} proves the four
obstructions.  Section~\ref{sec:measurements} reports the two finite
measurements.  Section~\ref{sec:open} states the remaining obligation.  All
...
Infinite prime-gap identity & Lean-checked unconditionally & Theorem~\ref{res:infinite}. \\
Prime-series/gap-series irrationality equivalence & Lean-checked unconditionally & Corollary~\ref{res:irr-equivalence}. \\
Elementary polynomial prime bound $p_n\le1250(n+1)^4$ & Lean-checked; proof reprinted here & Appendix~\ref{app:prime-bound}. \\
Actual real prime-gap tail and its recurrence & Lean-checked unconditionally & Section~\ref{sec:tail}. \\
Rationality/integral-shift classification & Lean-checked; an equivalence & Theorem~\ref{res:escape-irrational}. \\
Free-pair irrationality criterion & Lean-checked; an equivalence & Theorem~\ref{res:freepair}. \\
Lcm-diagonal irrationality criterion & Lean-checked; an equivalence & Theorem~\ref{res:lcmdiagonal}. \\
Adjacent small-shift obstruction, rational and real & Lean-checked & Theorem~\ref{res:smallpair}, Corollary~\ref{res:smallpair-real}. \\
Signed two-window normal form & Lean-checked; an equivalence & Theorem~\ref{res:signedwindow}. \\
Explicit remainder bound and the certified pair at $h=1$, $N=2$ & Ordinary proof; exact integer replay in Appendix~\ref{app:certificates} & Propositions~\ref{res:explicit-remainder} and~\ref{res:finite-smallpair}. \\
Denominator floor $b\ge2^{589}$ & Lean-checked by \texttt{decide +kernel}; ordinary proof reprinted here & Theorem~\ref{res:denominatorfloor}. \\
Certified continued-fraction exclusion $q\ge2^{39997}$ & Exact computation over a Lean-checked tail bound & Theorem~\ref{res:cfexclusion}; no Lean declaration. \\
Bounded-perturbation obstruction & Lean-checked & Theorem~\ref{res:boundedperturbation}. \\
Nonconcentration survives the rationalising perturbation & Ordinary proof; external lemma cited & Theorem~\ref{res:nonconcentration} and Corollary~\ref{res:nonconc-primes}. \\
The two-window event has density zero & Ordinary proof; external lemma cited & Theorem~\ref{res:sparse}. \\
Recurring gap values differing by $2$ do not suffice & Ordinary proof & Theorem~\ref{res:polignacfail}. \\
Quadratic polynomial-shift countermodel & Lean-checked & Proposition~\ref{res:polynomialcountermodel}. \\
Rationality alone forces periodic integer coefficients & False & Proposition~\ref{res:telescope}. \\
Affine and fixed-lattice cofinal escape tests & Lean-checked; both equivalences & Theorem~\ref{res:affinecollapse}. \\
Actual prime gaps are unbounded and not eventually periodic & Lean-checked & Proposition~\ref{res:gap-nonperiodic}. \\
Adjacent-mismatch event density below $1.2\times10^{8}$ & Finite measurement, floating-point tails & Section~\ref{sec:measurements}. \\
Free-pair witnesses for every $t\le20$ below $2\times10^{7}$ & Finite measurement & Section~\ref{sec:measurements}. \\
Cofinal adjacent small mismatch & Sufficient condition; not proved & Problem~\ref{prob:smallpair}. \\
\bottomrule
\end{longtable}
\endgroup
...
\begin{proof}
The bound $p_n\le1250(n+1)^4$ of Appendix~\ref{app:prime-bound} makes $(u_n)$
...
\section{The tail recurrence and the exact criteria}\label{sec:tail}
...
\eqref{eq:den-law} at an index where the denominator has become odd, and
...
\]
which converges by Theorem~\ref{res:infinite}.  The scaled tail equals its
...
Theorem~\ref{res:escape-irrational} specialises to the actual series:
...
\subsection{The free-pair criterion and the lcm diagonal}\label{sec:freepair}
...
Theorem~\ref{res:escape-irrational} at shift length $t$.  Conversely, suppose
$S$ is rational.  By \eqref{eq:den-law} the orbit reaches an index $N_0$ beyond
...
\begin{proof}
Theorem~\ref{res:escape-irrational} gives the forward implication.  Conversely,
...
pointwise no larger and is the endpoint used here.  Theorems~\ref{res:freepair}
and~\ref{res:lcmdiagonal} are exact reformulations on the class of
...
\section{A local certificate, and one actual pair}\label{sec:local-certificate}
...
so on that range the shift step identity \eqref{eq:shift-step} turns
...
integers, both would vanish, and \eqref{eq:shift-step} would give
$g_{N+h+1}=g_{N+1}$.  The cofinal statement chooses one such adjacent pair
after the alleged onset of integrality.
\end{proof}

\begin{corollary}[real form and the sufficient condition]\label{res:smallpair-real}
...
Proposition~\ref{res:gap-nonperiodic} it holds for arbitrarily large $N$ at
...
The conjunction in Theorem~\ref{res:smallpair} has an exact normal form.  For
...
Theorem~\ref{res:signedwindow} applies throughout the range where the local
certificate is sought.  The same computation for a real orbit is one line and
is used in Theorem~\ref{res:sparse} below.
...
\begin{proposition}[explicit remainder]\label{res:explicit-remainder}
...
\begin{proposition}[a certified adjacent pair]\label{res:finite-smallpair}
For the actual prime gaps, $h=1$ and $N=2$ satisfy the three hypotheses of
Theorem~\ref{res:smallpair}: both $\sigma_1(2)$ and $\sigma_1(3)$ lie in
...
Appendix~\ref{app:certificates} satisfy
\[
 u'v-uv'=1,\qquad
...
\lword{Erdos251/KernelDenominatorFloor.lean}{309}{cert_10000}{certificate},
the
\lword{Erdos251/KernelDenominatorFloor.lean}{315}{kernel_denominator_floor}{floor
for the prime series}, and the
\lword{Erdos251/KernelDenominatorFloor.lean}{323}{kernel_denominator_floor_primeGap}{floor
...
\section{What cannot supply the missing input}\label{sec:obstructions}
...
\noindent The
\lword{Erdos251/BoundedPerturbationCountermodel.lean}{163}{exists_rational_bounded_perturbation}{construction}
and its
\lword{Erdos251/BoundedPerturbationCountermodel.lean}{179}{not_forall_irrational_of_bounded_perturbation_invariant}{invariant-property
consequence} are kernel checked: a property shared by every admissible
perturbation cannot by itself force irrationality.  The construction is also
checked at the actual consecutive prime gaps for every $M\ge1$ and every $K$,
the
\lword{Erdos251/BoundedPerturbationCountermodel.lean}{200}{exists_rational_bounded_perturbation_primeGap}{rational
perturbed prime-gap series}, with the perturbed gap lying in $[g_n,g_n+M]$ and
congruent to $g_n$ modulo $M$, the
\lword{Erdos251/BoundedPerturbationCountermodel.lean}{220}{perturbed_gap_bounds}{gap
...
Section~\ref{sec:open} and equidistribution of $p_n$ modulo any fixed $q$, on
...
The obvious reply to Theorem~\ref{res:boundedperturbation} is to demand a
finer property of the actual gaps than size and residue.  The finest such
property currently proved is Schlage-Puchta's Lemma~4: for every polynomial
...
\begin{theorem}[nonconcentration is perturbation-stable]\label{res:nonconcentration}
...
\begin{corollary}[nonconcentration does not force irrationality]\label{res:nonconc-primes}
Fix $M\ge1$ and $K\ge0$, and let $b$ be the perturbed sequence supplied by
Theorem~\ref{res:boundedperturbation} at the actual prime gaps.  Then
...
\begin{proof}
Everything except nonconcentration is Theorem~\ref{res:boundedperturbation}
together with $\sum_{i<n}g_i=p_n-2$.  The perturbation takes values in the
fixed two-element set $E=\{0,M\}$, so Theorem~\ref{res:nonconcentration}
...
modulo $M$, positivity, eventual evenness and cumulative growth at the
prime-number-theorem scale, is therefore compatible with a rational dyadic
value.  The boundary is exact: the terms $P_n$ are not asserted to be prime,
...
\begin{theorem}[sparsity of the two-window event]\label{res:sparse}
Fix $h\ge1$.  The set of $N\ge1$ at which the three hypotheses of
Theorem~\ref{res:smallpair} hold for the actual prime gaps has density zero.
...
the cofinal statement Proposition~\ref{res:gap-nonperiodic} supplies.  The full
...
Problem~\ref{prob:smallpair} therefore has to be a cofinality statement about
a sparse set, and no argument that averages over all indices, or that produces
the event on a set of positive density, can succeed.  The measurement reported
in Section~\ref{sec:measurements} is consistent with this: the observed density
...
Theorem~\ref{res:signedwindow} reduces the producer to a $\pm2$ mismatch
...
\begin{theorem}[recurring values are not enough]\label{res:polignacfail}
...
special indices.  Each $U_n$ is an even or integer value by construction, so
...
\lword{Erdos251/PolynomialGapSeriesValue.lean}{89}{tsum_polynomialGapDyadicTerm_eq}{value
$32$ for the shifted series}, whose summand is $c_{n+1}/2^{\,n+1}$; the
rational value is recorded at
\lword{Erdos251/PolynomialGapSeriesValue.lean}{106}{not_irrational_tsum_polynomialGapDyadicTerm}{line~106}.
...
Theorem~\ref{res:signedwindow} fails at every index.  The telescoping mechanism
...
Proposition~\ref{res:gap-nonperiodic}.
...
\section{What is measured}\label{sec:measurements}
...
Theorem~\ref{res:signedwindow} occurs at every offset, with density between
...
Theorem~\ref{res:sparse} permits.  The tails in this scan are double-precision
...
violations.  Proposition~\ref{res:finite-smallpair} is the one pair certified
...
Theorem~\ref{res:freepair} has witnesses running to the last usable index: the
...
\section{The remaining obligation}\label{sec:open}

Problem~\#251 is open.  The exact unresolved condition, equivalent to
irrationality by Theorem~\ref{res:escape-irrational} and the escape
...
\[
 ...
 g_{N+h+1}\ne g_{N+1} .
\tag{8.2}\label{eq:smallpair}
\]
\end{problem}

Corollary~\ref{res:smallpair-real} makes Problem~\ref{prob:smallpair}
sufficient for Problem~\ref{prob:escape}, and Theorem~\ref{res:freepair}
...
What the proved obstructions leave.  By Theorem~\ref{res:boundedperturbation}
no hypothesis stable under bounded residue-preserving perturbation can work;
by Corollary~\ref{res:nonconc-primes} adding fixed-block polynomial
nonconcentration to that list does not repair it; by
Theorem~\ref{res:polignacfail} recurring gap values differing by $2$ in every
residue class do not work at the level of integer recurrences; and by
Theorem~\ref{res:sparse} the event in \eqref{eq:smallpair} has density zero, so
it must be produced cofinally on a sparse set.  Isolated small gaps, isolated
large gaps, average gap estimates and the occurrence of any one fixed finite
pattern are insufficient for a separate reason: both inequalities in
\eqref{eq:smallpair} contain the complete infinite continuation.  Nor does
parity help, since after the first gap every $g_n$ is even.

The finite reduction is available.  Proposition~\ref{res:explicit-remainder}
...
A prescribed positive margin $\eta_h$ therefore converts \eqref{eq:smallpair}
...
Section~\ref{sec:obstructions} say which unconditional substitutes cannot
replace that hypothesis.

\paragraph{What remains to be formalised.}
The two finite certificates of
Propositions~\ref{res:explicit-remainder} and~\ref{res:finite-smallpair}, the
continued-fraction exclusion of Theorem~\ref{res:cfexclusion}, the
nonconcentration transfer of Theorem~\ref{res:nonconcentration} with its
corollary, the sparsity statement of Theorem~\ref{res:sparse}, and the
construction of Theorem~\ref{res:polignacfail} are ordinary mathematical proofs
...
named in Section~\ref{sec:measurements} and the continued-fraction receipt of
Theorem~\ref{res:cfexclusion} record their generating programs with digests
...
The logarithmic-scale construction of Theorem~\ref{res:polignacfail} and the
transfer argument of Theorem~\ref{res:nonconcentration} arose in an external
review of an earlier draft of this note; both proofs printed here were checked
independently.

\appendix
\section{An elementary polynomial bound for the primes}\label{app:prime-bound}
...
\]
contradicting \eqref{eq:binomial-bound}.  Hence $\pi(2m)>n$, so at least $n+1$
...
\section{Integer certificates}\label{app:certificates}

The following calculation uses trial division and integer arithmetic only.  It
reproduces the two rows of Proposition~\ref{res:finite-smallpair} and every
inequality used in Theorem~\ref{res:denominatorfloor}.  The four literals are
...
are prime-specific.  Section~\ref{sec:tail} is stated for arbitrary integer
digits and arbitrary rational or real orbits, while
Section~\ref{sec:local-certificate} records the actual-gap specialisation.  The
...
polynomial countermodel dyadic value & \lrefx{Erdos251/PolynomialGapSeriesValue.lean}{89}{tsum_polynomialGapDyadicTerm_eq}\\
polynomial countermodel rationality & \lrefx{Erdos251/PolynomialGapSeriesValue.lean}{106}{not_irrational_tsum_polynomialGapDyadicTerm}\\
...
kernel-decided certificate & \lrefx{Erdos251/KernelDenominatorFloor.lean}{309}{cert_10000}\\
denominator floor, prime series & \lrefx{Erdos251/KernelDenominatorFloor.lean}{315}{kernel_denominator_floor}\\
denominator floor, gap series & \lrefx{Erdos251/KernelDenominatorFloor.lean}{323}{kernel_denominator_floor_primeGap}\\
bounded-perturbation construction & \lrefx{Erdos251/BoundedPerturbationCountermodel.lean}{163}{exists_rational_bounded_perturbation}\\
invariant-property consequence & \lrefx{Erdos251/BoundedPerturbationCountermodel.lean}{179}{not_forall_irrational_of_bounded_perturbation_invariant}\\
prime-anchored rational perturbation & \lrefx{Erdos251/BoundedPerturbationCountermodel.lean}{200}{exists_rational_bounded_perturbation_primeGap}\\
perturbed prime-gap series is rational & \lrefx{Erdos251/BoundedPerturbationCountermodel.lean}{210}{not_irrational_bounded_perturbation_primeGap}\\
perturbed gap bounds & \lrefx{Erdos251/BoundedPerturbationCountermodel.lean}{220}{perturbed_gap_bounds}\\
