...
Let $E$ be a finite nonempty set of primes, and let $\mathcal U$ be a
nonempty proper upward-closed family of subsets of $E$.  We construct
an infinite support $A_{\mathcal U}$ such that, for every integer $b\ge2$
and every finite prime set $P$ (including $P=\varnothing$, with
$h(a)=1$),
\begin{equation}\label{eq:finite-witness-rule}
 W_{b,P}(A_{\mathcal U})<\infty
 \quad\Longleftrightarrow\quad P\cap E\in\mathcal U.
\end{equation}
Its ordinary reciprocal sum diverges, yet every infinite subset has
irrational $X_B(b)$ at every integer base $b\ge2$.
...
$\{2,3\}$, $\{2,5\}$, $\{3,5\}$.
The construction and its comparison are ordinary proofs; the Lean
result used here is the conditional weighted criterion, not a
formalisation of this particular support.  The novelty of the arithmetic
realisation has not yet been assessed.
