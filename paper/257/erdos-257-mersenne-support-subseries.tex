...
One host realises any monotone rule on a finite set $E$ of primes: if
$\mathcal U$ is an upward-closed family of subsets of $E$ with
$E\in\mathcal U\not\ni\varnothing$, a finite union of prime-cofactor blocks
has divergent reciprocal sum and $W_{b,P}<\infty$ exactly when
$P\cap E\in\mathcal U$, for every $b\ge2$ and finite prime set $P$, so every
infinite subset is irrational at every base.  The companion proves this and
Lean checks it.
