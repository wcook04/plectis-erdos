...
multiple of $\min A$.  Hence conditions~(iii) and~(iv) of their Theorems~1
and~3 fail for every splitting of $c_A$ into two sequences, and the scaled
tail
\[
 ...
 =X_A(b)+\Delta_{b,A}(N)
\]
is at least $X_A(b)$ for every $N\ge1$.  The proofs above average the
...
$a_n\ge n^{1+\tau}$ for some $\tau>0$ and every $n$, together with
$\limsup_na_n^{1/2^n}=\infty$~\cite[Theorem~1]{erdos1975}.  At
$a_n=2^{c_n}-1$ the polynomial hypothesis is automatic, since $c_n\ge n$
gives $a_n\ge2^n-1\ge n^{3/2}$ for every $n\ge1$, and $\log_2(2^{c_n}-1)$
differs from $c_n$ by less than $1$, so the growth hypothesis reduces to
$\limsup_nc_n/2^n=\infty$.  The criterion therefore settles exactly the
supports with $\limsup_nc_n/2^n=\infty$, at every integer base.  That class
and the reciprocal-summable class of Theorem~\ref{res:reciprocal-support} are
incomparable.  The powerful integers are reciprocal summable and their
increasing enumeration satisfies $c_n\asymp n^2$, so $c_n/2^n\to0$.  In the
...
unavailable here, on terms and on exponents alike.  This retires a method;
...
\bibitem{kanekosuzukitachiya} H.~Kaneko, Y.~Suzuki, and Y.~Tachiya,
  \href{https://arxiv.org/abs/2601.20743}{\emph{Refinements of Erd\H{o}s's
  irrationality criterion for certain sparse infinite series}},
  arXiv:2601.20743v1 (2026).  The averaged tail $R_c(q,x,z)$ is~(1.7) and
  Theorem~1 is on p.~3; its rational-integer form, Theorem~3, is on p.~5.
  Lemma~1 (pp.~6--7) derives irrationality from arbitrarily small nonzero
  scaled tails, and Lemma~2 (pp.~7--8) shows that most scaled tails in a range
  are small under a counting condition on the support.  Their criteria assume
  sparse coefficient supports.
...
\bibitem{erdos1975} P.~Erd\H{o}s, \emph{Some problems and results on the
  ...
  1--7.  Theorem 1 proves irrationality of $\sum_n1/a_n$ for increasing
  integers with $a_n\ge n^{1+\tau}$ for some $\tau>0$ and every $n$ and
  $\limsup_na_n^{1/2^n}=\infty$, as restated in Remark~4(4) of
  \cite{bkkkz2026}; the original was not consulted for this note.
\bibitem{bkkkz2026} K.~Barreto, J.~Kang, S.~Kim, V.~Kova\v{c}, and
  S.~Zhang, \href{https://arxiv.org/abs/2601.21442}{\emph{Irrationality of
  rapidly converging series: a problem of Erd\H{o}s and Graham}},
  arXiv:2601.21442v3 (2026).  Theorem~2 (p.~3) settles Erd\H{o}s
  Problem~1051 at the golden-ratio exponent and Theorem~5 (p.~5) shows the
  threshold is sharp; Remark~4(4) (p.~5) identifies the case $d=1$ of its
  Theorem~3 with \cite[Theorem~1]{erdos1975}; Lemma~14 (p.~13) is the
  interval-filling lemma.  An unrefereed preprint with no formalisation; it
  states nothing about \#257.
\end{thebibliography}
