...
% ===========================================================================
% Evidence marks.  paper-evidence.tex places "Lean" and "Comparator" links in
% the margin beside each result from this record's generated file
% paper/evidence/<record>.tex, input below.
% ===========================================================================
\input{paper-evidence}
\input{evidence/erdos257-mersenne-reasoning-surface}
...
short paper \emph{Weighted Support Criteria for Reciprocal Mersenne
Subseries}.  The global notation below is supplemented by definitions
...
A link marked \textsc{Lean source} beside an observation or a proof step
opens the declaration for that step.  Such links are for checking precise
definitions; they are not additional hypotheses.  A finite computation is
described as one where it is used: a result at specified values or below a
bound is not a result at arbitrarily large values.  Neither the universal
problem nor either rational-target question is resolved in this paper.
...
with the omitted proof is possible.  The pairwise-coprime case,
Theorem~\ref{thm:pairwise-coprime}, retains that extra hypothesis. The same
...
has a
\href{https://github.com/wcook04/plectis-erdos/blob/c91562bd574a387cde904481e609c7b4cacebb14/lean/Erdos249257/AllBaseReciprocalSupportIrrationality.lean#L395}{kernel-checked Lean proof}.
...
therefore uses a different argument; see Theorem~\ref{thm:full-support}.
...
hypothesis is also hereditary under passage to infinite subsets.  The
theorem does not cover all infinite supports and gives no quantitative
irrationality measure; no priority is claimed for it.
...
\subsection{Finite rules for prime witnesses}\label{sec:257-finite-witness-rules}
Theorem~\ref{thm:257-weighted} asks for one prime set $\mathcal P$ with
finite weighted mass.  A single host can make the answer follow any
monotone rule on a finite set of primes, simultaneously at every base.
In the next statement $\mathcal P=\varnothing$ is allowed, with $h(a)=1$.

\begin{prop}[realising finite monotone witness rules]
\label{prop:257-finite-witness-rule}
Let $E$ be a finite set of primes, and let $\mathcal U$ be an
upward-closed family of subsets of $E$ with $E\in\mathcal U$ and
$\varnothing\notin\mathcal U$.  There is a set
$A_{\mathcal U}\subseteq\mathbb N_{>0}$ with
$\sum_{a\in A_{\mathcal U}}1/a=\infty$ such that, for every integer
$b\ge2$ and every finite set $\mathcal P$ of primes,
\begin{equation}\label{eq:257-finite-witness-rule}
 W_{b,\mathcal P}(A_{\mathcal U})<\infty
 \quad\Longleftrightarrow\quad \mathcal P\cap E\in\mathcal U.
\end{equation}
Moreover $X_B(b)$ is irrational for every infinite
$B\subseteq A_{\mathcal U}$ and every integer $b\ge2$.
\end{prop}

\begin{proof}
For each nonempty $C\subseteq E$ put $D_C=\prod_{p\in C}p$ and
\[
 B_C=\{D_C^km:k\ge1,\ 1\le m\le2^{D_C^k},\ \gcd(m,D_C)=1\},
\]
and let $g=\prod_{p\in C\cap\mathcal P}p$.  The weighted mass of the
$k$th layer lies between positive multiples, depending only on
$C\cup\mathcal P$, of $g^k/(b^{g^k}-1)$.  Above: an element $D_C^km$ has
$\mathcal P$-part $g^ku$, and raising the $\mathcal P$-part from $g^k$ to
$g^ku$ multiplies the weight by
$u/(1+b^{g^k}+\cdots+b^{(u-1)g^k})\le1$, while
$\sum_{m\le2^r}1/m\le2r$.  Below: the cofactors $m\equiv1\pmod Q$, with
$Q=\prod_{p\in C\cup\mathcal P}p$, have $\mathcal P$-part exactly $g^k$,
and grouping $1,\dots,2^r$ into blocks of at most $Q$ integers gives
reciprocal sum at least $H_{2^r}/Q\ge r/(2Q)$, where $r=D_C^k$ and
$H_n=\sum_{j\le n}1/j$.  The comparison terms equal $1/(b-1)$ when $g=1$
and are summable when $g\ge2$, since consecutive ratios are at most
$g/b^{g^k}\to0$.  Hence $B_C$ has finite weighted mass exactly when
$\mathcal P$ meets $C$, and the case $Q=D_C$ shows that each layer adds at
least $1/(2D_C)$ to the ordinary reciprocal sum.

Let $A_{\mathcal U}$ be the union of the blocks $B_{E\setminus M}$ over
the forbidden sets $M\subseteq E$, $M\notin\mathcal U$; each clause
$E\setminus M$ is nonempty because $E\in\mathcal U$.  A subset of $E$ lies
in $\mathcal U$ exactly when it meets every clause: an allowed set cannot
lie inside a forbidden one, and a forbidden set misses its own clause.
This finite correspondence is standard~\cite[Section~2]{sedaghatstephenchindelevitch}.
Nonnegative weights make summability on a finite union equivalent to
summability on each block, which gives~\eqref{eq:257-finite-witness-rule}
and reciprocal divergence.  Since $E\in\mathcal U$,
\eqref{eq:257-finite-witness-rule} with $\mathcal P=E$ gives
$W_{b,E}(B)\le W_{b,E}(A_{\mathcal U})<\infty$ for every infinite
$B\subseteq A_{\mathcal U}$ and every $b\ge2$, so
Theorem~\ref{thm:257-weighted} makes $X_B(b)$ irrational.
\end{proof}

For the subsets of $\{2,3,5\}$ with at least two elements, no single
prime is mandatory and none is sufficient.  The maximal forbidden sets
alone give the clauses $\{2,3\}$, $\{2,5\}$, $\{3,5\}$, which are also the
minimal witnesses; that smaller host has the same classification, while
the Lean statement uses every forbidden set.  The novelty of the
arithmetic realisation has not yet been assessed.
...
The argument displayed here is an ordinary proof; the estimate is also
\href{https://github.com/wcook04/plectis-erdos/blob/c91562bd574a387cde904481e609c7b4cacebb14/lean/ErdosProblems/Erdos257/PaperCompleteR8/DyadicKernel.lean\#L206}{proved in Lean}.
...
This ordinary argument is sharper than the factor-two bound in the Lean
source's
\href{https://github.com/wcook04/plectis-erdos-lean/blob/52f29ad173b04e3bac941b3663f2b9aebe5de0bb/ErdosProblems/Erdos257/PaperCompleteR7/CoverKernel.lean#L232}{scalar comparison between bases}.
...
The positive-weight variant of
Theorem~\ref{thm:257-variable-fractional-cover}, with its hereditary
all-base conclusion, also has a
\href{https://github.com/wcook04/plectis-erdos/blob/c91562bd574a387cde904481e609c7b4cacebb14/lean/ErdosProblems/Erdos257/PaperCompleteR8/ArbitraryWeightMixedClaim.lean#L19}{kernel-checked Lean proof}.
Theorem~\ref{thm:257-mixed-supports} alone does not exhibit a host outside
both individual classes.  The short paper now supplies the missing ordinary
constructions: one infinite host has a finite-prime weighted witness at base
two but no strengthened positive cover, while a fresh-prime host has such a
cover but no finite-prime weighted witness at any integer base.  Their union
satisfies neither individual criterion, and this theorem gives every
infinite subset an irrational sum at every integer base.  Those exact host
constructions and the strictness corollary have not been formalised in Lean
or independently reviewed; the mixed implication itself is kernel-checked.
Universal irrationality and the quantitative and prime-power thinning claims
remain open.

The class of subsets of mixed hosts is closed under finite unions and finite
changes even with the literal dyadic cover cost in
\eqref{eq:257-strengthened-cover}.  For two covers, interleave their $j$th
finite sets at positions $2j-1,2j$, halve each exponent, and keep the
nonnegative coefficient columns.  Integer divisor counts satisfy
$f^{\alpha/2}\le f^\alpha$, while the full reindexed cost of each column
grows by at most $1+\sqrt2$.  For $r$ covers, positions at most $rj$ and
exponents divided by $r$ give a cost ratio below $2r$.  For weighted
supports combine the finite prime witnesses: the prime part grows and
$h/(2^h-1)$ decreases.  Subsets inherit the same hosts, and finite sets
have finite weighted mass.
...
finite certificate of nonmembership and an unbounded survival condition.
...
\hline
\textbf{Support class} & \textbf{Exact hypotheses} & \textbf{Formal proof}  \\
\hline
\endhead
...
The Lean sources contain no theorem for $A=$ the primes, but this is a
...
is derived from it.  This record has no general theorem for an
...
\]
Then $\dim_H S_J=d$.  Here is an ordinary proof, separate from the
formal statements above.  If $J$ is finite, both sides are zero,
...
For $d=0$ the lower bound is automatic.

\begin{rem}[Achievement-set terminology and sources]
...
the topology and measure used here.  The
volume dichotomy, injectivity, and greedy-membership statements above and
below are proved in Lean.  The sparse power-series criteria
...
recorded after Theorem~\ref{thm:257-weighted}.
\end{rem}
...
The following finite checks use exact integer arithmetic.  Each has a
\href{https://github.com/wcook04/plectis-erdos/tree/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257}{published program and saved result},
including the scan to row $200{,}000$ and the grouped computation to
depth $13{,}548{,}057$.  Decimal margins summarise those integer data and
give no asymptotic estimate.
...
target inequality failing at exactly one reset row $6\le r\le200{,}000$, namely $r = 7$; maximal pure-R run $19$, following
the reset at row $158{,}096$.

Independently, rows $6 \le n \le 2500$: $1209$ resets, zero classification
anomalies, minimum in-scope margin $+1.1119$ bits at row $14$ growing to $\approx 1246$ bits at row
$2499$ (mean $627$), never re-approaching the threshold.
...
It is the smallest take-margin reported in this finite scan. The finite greedy prefix through $m=200{,}000$, certified rank by rank
with integer interval bounds
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/greedy-prefix-200000.json}{saved run}),
satisfies
$0\le 1/2-\sum_{d\in G\cap[1,m]}w_d<2^{-199999}$.
The sum here is finite; this statement does not evaluate the infinite greedy sum.

A further exact finite computation, not used in the arguments here,
regenerates the forced half-prefix support to depth $13{,}548{,}057$
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/half_prefix_coefficient_macro_verify.cpp}{program},
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/coefficient-macro-13548057.json}{saved run}).
The reset rows $13$--$30$ are also
verified in Lean by direct evaluation
...
does~\cite[Theorem~1, p.~12]{campbell2026}.
\end{rem}
...
exclusions, not an assertion about the actual greedy word.
...
\begin{rem}[A sufficient condition that is not necessary]
The natural sufficient condition via fractional split-mass is \emph{not} necessary on real crossing
cores: the example $D = \{2,3\}$, $c = 5$ is an explicit counterexample
...
\textbf{Scope limit.} Item (a) does \emph{not} extend to every $A$ with
$1 \notin A$. Dropping the greedy-specific fact $x_G \le 1/2$ yields only the one-sided statement
that the bound in (a), with $A$ in place of $G$, is equivalent to $1/2 - x_A \le 0$. Thus the conclusion for a general support is a one-sided inequality,
...
cannot distinguish them. (iii) For any
...
Proposition~\ref{prop:2adic-nogo}, so these fixed-precision symbols alone do not exclude a completion in
that specified family.  This says nothing about extra constraints imposed
by divisor counts from one common support.
\end{prop}
...
rules out every argument using prime-counting information.
...
The certificate proof of Theorem~\ref{thm:pairwise-coprime} uses the tail
budget in one step.  Deleting that assumption
does not complete the proof: one needs a replacement estimate for that step.
...
the absence of a \emph{Lean theorem from this certificate argument}; it does not make the
...
\item \emph{Band avoidance at resets, already proved to imply membership.}
  For every upper reset $d \ge 13$ (branch $\mathrm U$ of
  Theorem~\ref{thm:dynamics}) and every $j \le d$, the reset charge
...
length $19$ after the reset at row $158{,}096$, far below the proposed threshold
...
Section~\ref{bar:scale}; it remains unproved.
...
one of:
\begin{enumerate}[nosep,label=(\alph*)]
\item \emph{critical quotient supply}: for every below-half core $D$ bounded by $[2,c)$
...
orbit, the half-target greedy prefix, not searched over $D$; and by the recorded sharp-capacity
biconditional it is exactly the $c-2$ bit capacity test
$\ensuremath{S}\,D\,1\,(2c-2) < 2^{c-2}$. The example $D=\{2,3\}$, $c=5$ shows that the fractional-mass
...
analytic and is cited here; it is not formalised in Lean.
...
this certificate theorem, not a boundary of the mathematics.  A
windowed-tail repair of that theorem can test whether the Lean
certificate theorem reproduces the known theorem; reproducing a cited
result is not new progress on \#257.  After Theorem~1.3 the authors state the
...
The longer record has two different reading modes, and they should not be
confused.
...
\paragraph{Use as reference.} The catalogue in Part~III may be entered by
topic: the certificate method and universal-direction record;
the greedy and achievement-set geometry; the integer-quotient model, quotient identity, and
dynamics; the half-carry and Boolean--M\"obius formulations; and the support
...
\paragraph{Finite ranges and unbounded statements.} The directly verified
instances stay in finite ranges: rungs $J \le 22$, seam rows
$\le 200{,}000$, ranks $\le 3000$, and depth $\le 13{,}548{,}057$ in the
further computation; the open obligations require arbitrarily large
parameters.  Theorem~\ref{thm:one-sided} shows that extending a
bounded range does not establish such a statement.  Likewise, an obstruction
proved in one representation, for example the greedy recurrence, the
integer quotients or the Diophantine formulation, does not transfer to another
representation without a proved map.
% ; - part a257_p0 ; -
\clearpage
...
weights $\{x_n\}_{n \ge 1}$. \textbf{Is $1/2 \in \mathcal{A}$?} This is \textbf{OPEN}.
\end{defn}
...
The base-$2$ instance recovering the classical Erd\H{o}s--Borwein constant's irrationality
...
Luca and Tachiya~\cite[Theorem~A, p.~139]{lucatachiya2017}; these cases also have Lean proofs
(margin):

\begin{thm}[Purely periodic support]\label{thm:purely-periodic-support}
...
$m$-periodic, $\exists a > 0,\ a \in A$.
\end{thm}

\begin{thm}[Eventually periodic support]\label{thm:eventually-periodic-support}
...
\begin{thm}[Residue-class support]\label{thm:residue-class-support}
For integers $b\ge2$, $m\ge1$ and $c$, the series
$\sum_{n\ge1,\ n\equiv c\pmod m}(b^n-1)^{-1}$ is irrational.
\end{thm}

\begin{thm}[Odd support]\label{thm:odd-support}
...
Strict separation and summability give the compactness, unique coding, and Cantor topology, and
...
the inequalities at all ranks give a representation.
\end{thm}
...
This gives (I).  \textbf{Certification:} rows $6\le n\le200$
rechecked with zero mismatches by an exact
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/check_master_identity.py}{program}
(\S\ref{ssec:anchors}).\\
...
The following values and finite ranges come from exact integer
computations for the quotient-greedy sequence of Section~\ref{ssec:seam-model}.
They provide test cases for the formulas above. The low-row remainders
at $n=14,15,32$ are exact integer values. Each larger certified range has a
\href{https://github.com/wcook04/plectis-erdos/tree/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257}{published program and saved run}.
...
-429921291$.\\
\textit{Source:} the values at rows $14$--$31$ are proved in Lean by direct
evaluation (Theorem~\ref{thm:upper-reset-band}), which uses them to verify the
band-escape condition for reset indices $13\le d\le30$
\lean{seamUpperResetDyadicBandEscape\_through\_thirty}{HalfCylinderUpperResetBandCertificates.lean:78}.
The two ranges differ by one: the remainder used for reset $d$ is at
row $d+1$. The take set $D_{14}$, $\mathrm{rem}(32)$ and $\Delta_{32}$ are
exact integer computations with the greedy rule of Section~\ref{ssec:seam-model}.
\end{obs}

\begin{obs}[Depths of the reported computations]
\label{obs:anchors-depth}
The quotient identity (Theorem~\ref{thm:master-identity}) is verified at rows $6 \le n \le 200$ with zero
mismatches
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/quotient-identity-200.json}{saved run}),
and the quotient-greedy orbit itself is certified row by row to row $2500$
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-reset-crossing-2500.json}{saved run})
and through the exact branch recursion to row $200{,}000$
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-orbit-200000.json}{saved run}; branch counts $\mathrm{R{:}M{:}U} =
100197{:}49899{:}49898$, the target inequality failing at exactly one reset row $6\le r\le 200{,}000$,
namely $r=7$).
\end{obs}

\begin{obs}[Four computations side by side]
...
survive, not merely computed); quotient row $6 \le n \le 2500$
...
(an exploratory scan, which is not certified); sharp tail margin, ranks $2$ through
$3000$ (no fatal skip at any rank; take fraction $0.4992$; tightest certified take-margin
$0.0340$ bits at rank $7$, the near-tie $1/126 \ge 1/127$).\\
...
Every entry states exact hypotheses and an exact conclusion; the Lean
proof of each is linked in the margin.  None of these results decides \#257.  Entries
...
\end{proof}
Convergence itself is automatic; proving that its limit is zero is
exactly the unresolved membership question.
...
membership is proved in Lean; no ordinary proof is printed here.  Neither all-row hypothesis
...
The upper-reset dyadic-band escape condition requires the following at every
...
residue-class and odd-support statements below follow from their theorem; their Lean proofs are
separate and use the certificate method described here.
...
The Lean source calls $A_r$ the M\"obius numerator; $B(r)$ is a finite
...
\end{proof}
The Lean proof of the bound uses natural-number division $t/2$,
...
is odd.  The Lean statement of the exact denominator uses an
equivalent formula in its own numerator notation.  Neither denominator growth nor
...
fixed lower prefix $u$, the resulting cofinite-support value stays strictly below $1/2$; the matching
alternative ``upper competitor'' word gives a strict excess \emph{above} $1/2$. Neither of these two cofinite continuations represents $1/2$.
...
reduced hypothesis at the single index $j_*$ is proved logically equivalent to
...
of the Lean proof (margin) gives $C_B(2D+1)=C_D+3$.  If $C_D=-3$, this is zero.
The recurrence
...
the positive-index digit map), hence closed; it is also perfect, totally
...
Definitions and conditional implications are separated below.  An
unproved hypothesis is not established merely because the
...
\par\noindent   \lean{ExactLocalMersenneHalfRow}{BooleanMobiusCofinalExactRows.lean:31}
\end{defn}
...
\par\noindent   \lean{localPrefixQuotient}{BooleanMobiusLocalRepair.lean:78} \lean{localMersenneQuotient}{BooleanMobiusLocalRepair.lean:20} \lean{localRepairInteger}{BooleanMobiusLocalRepair.lean:123}
\end{defn}
...
\par\noindent   \lean{localBinarySuffix}{BooleanMobiusLocalRepair.lean:102}
\end{defn}
...
\par\noindent   \lean{CofinalExactLocalMersenneHalfRows}{BooleanMobiusCofinalExactRows.lean:38}
\end{defn}
...
statements.
\end{thm}
The existence of rows satisfying these four conditions for every $n\ge2$
is not proved.
...
every step: each protected exact row either doubles below half
(unconditional) or recycles at its first crossing rank $e > $ cutoff, giving endpoint
$2e-2 > $ previous endpoint; protection (endpoint $< 2\cdot$cutoff, new ranks $>$
...
arithmetic, so the supply is needed at essentially every step. Its conclusion is
the cofinal exact-row condition of Definition~\ref{record:257bm-d4}.
\end{thm}
...
The Lean proofs use the theorem in exactly this first-crossing form.
...
assuming it, a Lean theorem proves
$1/2\in\ensuremath{\mathcal A}$.
\par\noindent   \lean{HalfGreedySkippedFullShellNonnegative}{HalfCylinderFullShellSeamBridge.lean:604} \lean{half\_mem\_mersenneAchievementSet\_of\_skippedFullShellNonnegative}{HalfCylinderFullShellSeamBridge.lean:633}
\end{defn}
...
Comparing this carry with the carry obtained by fixing a finite prefix introduces a contribution from later omitted ranks. No equivalence with the finite-support hypothesis
of Theorem~\ref{record:257bm-c7} follows without controlling that
additional contribution. The hypothesis is not established.
\end{thm}
...
generic tempered-orbit recurrence $u(N+1)=2u(N)-v\cdot c(N+1)$ from the
tempered-orbit criterion (Theorem~\ref{record:257bm-i-t7} below), specialised to Mersenne
...
\par\noindent   \lean{localBinarySuffix\_two\_mul\_sub\_two\_lt\_criticalCapacity\_iff}{BooleanMobiusSkippedCoreCriticalCapacity.lean:22}
\end{defn}
...
ranks $6$ and $7$, whose quotients are $4$ and $2$.
...
The set $\{2,3,6\}$ satisfies
\[
\ensuremath{Q}(\{2,3,6\},6) = 2^5-1 = 31,\qquad \ensuremath{V}(\{2,3,6\}) < 1/2,
\]
hence it is an exact row at depth $6$ (Definition~\ref{record:257bm-d1}).
Both facts are direct numerical checks.
...
\par\noindent   \lean{exactRowSixSupport}{BooleanMobiusExactRowSeed.lean:14} \lean{exactLocalMersenneHalfRow\_six}{BooleanMobiusExactRowSeed.lean:34}
\end{defn}
...
For a finite set $E$, its crossing ranks are
\[
...
\par\noindent   \lean{localMersenneCrossingRanks}{BooleanMobiusExactRowCrossing.lean:24}
\end{defn}
...
\par\noindent   \lean{mersenneGap}{GreedyAchievementSet.lean:2337} \lean{mersenneGap\_pos}{GreedyAchievementSet.lean:2346}
\end{defn}
...
For $k\ge1$ put $\ell_3(k)=1/2^k + 1/(3\cdot4^k) + 1/(7\cdot8^k)$.  Then
$\ell_3(k)<\ensuremath{R}(k)$: this is a strict rational lower bound obtained by truncating
the Lambert-type series at three terms.
\par\noindent   \lean{mersenneTailLB3}{HalfGreedyFatalGap.lean:56} \lean{three\_div\_lt\_mersenneTailLB3}{HalfGreedyFatalGap.lean:70} \lean{mersenneTailLB3\_lt\_mersenneTail}{HalfGreedyFatalGap.lean:216}
\end{defn}
...
These are statements about the current step, not infinite survival.
\end{prop}
...
bound, induction (base case at row 5 verified by direct computation) propagates it to the universal
two-sided bound
\[
...
satisfies exactly the same two-branch transition shape (seed at 6, and the
model reproduces the $\vee$ shape by always taking
...
Observation~\ref{record:257bm-k11}; this says nothing about equality
...
\[
 ...
 =\frac{757}{651}>1.
\]
The linked Lean proof checks this example by direct computation.  Thus the
...
\par\noindent   \lean{exists\_exactRowStrictUpperExtension\_two\_mul\_sub\_one\_of\_exact\_below}{BooleanMobiusExactRowDoubling.lean:185}
\end{obs}

\begin{thm}[The returning endpoint need not be larger]\label{record:257bm-k4}
Theorem~\ref{record:257bm-c10} is unconditional, but its
witness is $\exists c\le n$, not $\exists c$ large: $2c-2$ may be $\le n$, so the endpoint need
not grow. The countermodel of Theorem~\ref{record:257bm-k1} is the
...
$0<\ensuremath{r}(1/2,c{-}1)$, holds unconditionally: the greedy remainder is $1/2$ minus a
finite Mersenne sum, which is nonzero by the odd-denominator parity fact of
Observation~\ref{record:257bm-k11}. Hence the hypothesis of Theorem~\ref{record:257bm-c2}
holds if and only if the set of ranks skipped by the greedy rule for $1/2$ is infinite,
which Theorem~\ref{record:257bm-c20} proves equivalent to $1/2\in\ensuremath{\mathcal A}$. Proving Theorem~\ref{record:257bm-c2}'s hypothesis would therefore settle
...
\par\noindent   \lean{localMersennePrefixValue\_halfGreedy\_lt\_half}{BooleanMobiusCriticalCapacityCofinal.lean:138}
\end{obs}
...
hypotheses.
\par\noindent   \lean{HalfCarryCofinalTerminalOnlyStrip}{TerminalOnlyCofinal.lean:34}
\end{obs}

\begin{obs}[An unsafe example in the two-thirds interval]\label{record:257hg-k8}
Write a greedy residual as $\mathrm{rem}=1/R$; a skip at rank $k$ is dyadically safe if and
only if $R\ge2^k$.  Two unconditional corollaries hold: an integral reciprocal $R$ is never
unsafe, and an unsafe run with odd $p,D,q$ forces $p\ge7$.  Dyadic safety is sufficient but
not necessary for the true greedy process to survive: the true test uses the actual tail
$\sum_{j>k}w_j$, which exceeds $2^{-k}$.  Nothing here asserts that the actual half-greedy
orbit avoids the band: $(p,D,b)=(17,41,3)$ is an explicit odd-coprime example that is
dyadically unsafe.  Theorem~\ref{record:257hg-i4}'s criterion $2u\le3a$ narrows exactly this
gap in the single-skip case; whether the fatal region meets a band reachable by the actual
greedy trajectory is not settled.
\par\noindent   \lean{TwoThirdsBand}{HalfGreedyTwoThirdsBand.lean:120} \lean{not\_twoThirdsBand\_of\_int}{HalfGreedyTwoThirdsBand.lean:185} \lean{seven\_le\_of\_intBand\_odd}{HalfGreedyTwoThirdsBand.lean:231}
\end{obs}
...
\par\noindent   \lean{localBinarySuffix\_two\_mul\_sub\_two\_lt\_upperHalfCapacity}{BooleanMobiusSkippedCoreExactRow.lean:123}
\end{obs}

\begin{obs}[Odd denominators of finite sums]\label{record:257bm-k11}
...
Proposition~\ref{record:257bm-i10}.
\par\noindent   \lean{finiteErdosSum\_den\_odd}{DyadicPrefixCompression.lean:1513}
\end{obs}
...
membership results.  They concern the
...
\begin{prop}[The unsafe middle range is exactly three integers]\label{prop:unsafe-middle-range-exactly-three}
...
\]
\end{prop}
The complement of the two safe integer ranges is precisely
...
\[
 4Z_s\le c.
\]
This inequality has a Lean proof
(\lean{seamMiddleCell\_four\_mul\_floorZ\_le}{HalfCylinderMiddleCarryLowerBound.lean:4142}).
On a middle transition $D_{s+1}=D_s$.  Writing
...
A parallel equivalence holds for the right branch: an overshoot witness $\le2^s$ produces a
pulse leak above $2^{s+2}$ exactly when the overshoot lies within an explicit distance $k$ of
$2^s$ with $4k<\mathrm{abovePulse}$
(\lean{seamRightPulseLeak\_iff\_distanceCell}{HalfCylinderMiddleCarryLowerBound.lean:4202}).
The remainder left by the integer greedy rule with the row-$s$ quotient weights and target
$2^{2s-1}$ (\lean{seamPureHalfPrefixRemainder}{HalfCylinderMiddleCarryLowerBound.lean:4232})
has an exact case-split formula
(\lean{seamPureHalfPrefixRemainder\_eq\_ite}{HalfCylinderMiddleCarryLowerBound.lean:4238}),
and a bound on it is proved equivalent to the two-sided bound
(\lean{seamPureHalfPrefixRemainder\_le\_iff\_twoSided}{HalfCylinderMiddleCarryLowerBound.lean:4356}).

\begin{obs}[The reset-band hypothesis has a verified base case]
The upper-reset dyadic-band escape condition (discussed after Section~\ref{sec:o4}) is
reduced by three lemmas to a quarter-scale overshoot exclusion: an equivalence
(\lean{seamUpperReset\_band\_iff\_successorRemainder\_avoids}{HalfCylinderMiddleCarryLowerBound.lean:4469})
rewrites the reset-charge band as a successor-remainder avoidance; an identity
(\lean{upperResetCharge\_dyadicDistance\_add\_abovePulse}{HalfCylinderMiddleCarryLowerBound.lean:4490})
rescales it by a factor of $4$; and a third lemma
(\lean{seamUpperReset\_band\_of\_overshoot\_band}{HalfCylinderMiddleCarryLowerBound.lean:4514})
...
\emph{every} band simultaneously: its successor remainder is exactly $392$, by exact
computation
(\lean{seamUpperResetDyadicBandEscape\_at\_thirteen}{HalfCylinderMiddleCarryLowerBound.lean:4547}).
This is one verified row of a hypothesis required for every $d\ge13$; it shows only that the
base case is not itself an obstruction.
\end{obs}
...
\begin{prop}\label{prop:finite-approximations-without-compatibility}
Every shared-prefix family just defined at depth $N$ contains a finite
support $A$ with $|K_A(N)|\le B(N)$, and at a feedback row this survives \emph{both} outputs of
...
\end{prop}
Four concrete instances, at depths $51$--$54$, are verified by exact computation of the
underlying stages
...
This is a finite chain of witnesses; it does not reach unbounded depths.
...
with a positive-support refinement using the same odd-denominator lemma
(\lean{finite\_boolSupport\_ne\_half}{HalfCarryReachability.lean:589};
\lean{exists\_infinite\_positive\_support\_half\_of\_cofinalCylinderStages}{SuffixCylinderTerminalOnlyBridge.lean:287}).
The hypothesis requires these finite families at arbitrarily large
depths and is unproved.  The terminal bound then implies half-membership.
...
Five Lean declarations reduce the cases that remain to be checked in the
square-root lower bound on the producer carry at late middle rows (discussed in
Part~IV), using the proved exclusion of the $-3$ cell.  No strict separation of the
existence conditions is inferred from dropping that case.

\begin{defn}
The \emph{middle tail condition} is the hypothesis that at every genuine middle
...
The \emph{reduced middle tail condition} is the same hypothesis restricted to cells
$\ne-3$ (\lean{SeamMiddleProducerTailEscapeExceptNegThree}{HalfCylinderLastProducerContradiction.lean:54}).
\end{defn}
The middle tail condition trivially implies the reduced one
(\lean{SeamMiddleProducerTailEscape.toExceptNegThree}{HalfCylinderLastProducerContradiction.lean:76});
the substance is in the converse direction. The reduced condition, with cell
$-3$ discharged by the already proved exclusion
(\lean{middleProducer\_neg\_three\_not\_last}{HalfCylinderLastProducerContradiction.lean:364}),
already suffices for half-membership:
\[
...
The unreduced condition and the square-root bound both imply it
(\lean{half\_mem\_mersenneAchievementSet\_of\_middleProducerTailEscape}{HalfCylinderLastProducerContradiction.lean:451}).
So the square-root bound implies the middle tail condition, which implies the reduced
condition, which implies $1/2\in\mathcal A$: a chain of sufficient implications.  The
arrows do not prove that any implication is strict; all three antecedents remain unproved.

\subsubsection{An eventual nonnegative margin}

The finite-horizon condition can be stated directly.  Recall the integer
margin $F_k(J)$ from Theorem~\ref{thm:frozen-margin} and its equivalence with a
negative excess
...
Indeed, the finite coefficient window is at most its
...
the bounded and unbounded horizons is asserted.
...
This is a local normal form for the two-sheet boundary.  It does
not produce adjacent stages at unbounded depths and therefore decides neither
half-membership nor universal \#257.

\subsubsection{Further restrictions on represented values}

These results concern achievement-set geometry and do not settle the universal
irrationality problem.  For an arbitrary support restriction $J\subseteq\mathbb{N}$, let
$\mathcal A_J$ be the set of Mersenne digit values supported on $J$
(\lean{supportedMersenneAchievementSet}{ErdosProblems/Erdos257/MersenneSubseriesRigidity.lean:76}). It is compact
(\lean{isCompact\_supportedMersenneAchievementSet}{ErdosProblems/Erdos257/MersenneSubseriesRigidity.lean:90}),
...
and perfect, that is compact with no isolated points, whenever $J$ is infinite
(\lean{perfect\_supportedMersenneAchievementSet}{ErdosProblems/Erdos257/MersenneSubseriesRigidity.lean:167}), via a
Cantor-space no-isolated-point argument on the coding space itself
(\lean{preperfect\_supportedMersenneDigitSet}{ErdosProblems/Erdos257/MersenneSubseriesRigidity.lean:120}).

\begin{prop}[Exact Lebesgue-measure dichotomy]\label{prop:exact-lebesgue-measure-dichotomy}
Either $J=F^c$ for a finite $F$, and $\mathrm{vol}(\mathcal A_J)=2^{-|F|}$
exactly, or $J^c$ is infinite and the volume is exactly $0$.
\end{prop}
The finite case is an exact doubling induction over inserted
...
using disjointness of the two coordinate-split faces
(\lean{disjoint\_supportedMersenneAchievementSet\_translate}{ErdosProblems/Erdos257/MersenneSubseriesRigidity.lean:262});
this is where unique binary coding is used. The dichotomy is the case of retained coordinates of
Hornich's strict-tail measure formula~\cite[Theorem~4(1), p.~9]{nitecki2013}: the retained Mersenne
weights still exceed their tails, and the Mersenne tail estimate supplies the values $2^{-|F|}$ and
$0$.  None of this concerns irrationality of any subseries value.

\subsubsection{Conditional counterexamples at rational targets}

Finite supports whose scaled terminal carries tend to zero give an infinite $A$ with
$X_{A}(2)=1/2$
...
and hence refute the universal statement
(\lean{not\_universal\_of\_terminalScaledVanishing}{ErdosProblems/Erdos257/HalfCounterexampleFrontier.lean:39};
the universal statement itself has its own
\lean{UniversalMersenneSubseriesIrrationality}{ErdosProblems/Erdos257/HalfCounterexampleFrontier.lean:26}).
The required finite supports have only been checked to a large finite cutoff.  This restates
the open half-membership question; it is not a new result, since any representation of $1/2$
already refutes the universal statement.

A Lean theorem also proves an unconditional finite-support obstruction at
...
The denominator-order identity forces
...
Consider the primitive representations $2p+3q=n$ with $p,q>0$ and $\gcd(p,q)=1$
(\lean{IsPrimitive23Solution}{Primitive23Multiplicity.lean:20}).  Every $n\ge11$
has such a representation
(\lean{exists\_primitive23\_solution\_of\_eleven\_le}{Primitive23Multiplicity.lean:52}),
while $n=10$ has none
(\lean{no\_primitive23\_solution\_ten}{Primitive23Multiplicity.lean:26}).
The value $n=11$ already has two distinct representations
(\lean{exists\_two\_primitive23\_solutions\_eleven}{Primitive23Multiplicity.lean:38}),
and so does every $n=10k$ with $k\ge2$
(\lean{exists\_two\_primitive23\_solutions\_mul\_ten}{Primitive23Multiplicity.lean:86}).
These facts explain an approach and give no counterexample: nothing here proves
that the associated integer-multiplicity expansion of $1/21$ can be made Boolean.  The recurring multiplicities are precisely
...
$\mathcal F_{21}$ denote the explicit branch consisting
...
an infinite-support rational counterexample to universal \#257; it remains
open.
% ; - part a257_p2 ; -
\clearpage
...
Two kinds of information matter to a reader deciding whether a result can be used as a
premise: the kind of support behind it (a Lean proof, an exact finite computation, an
ordinary argument, a cited theorem, or none, for an unproved hypothesis) and the range of
its parameters.  A result may concern one specified value; a finite tested range, or a
witness bounded in terms of the input; every index in its stated range, with its hypotheses
retained, which need not make it unconditional; or unboundedly many indices, in the form
$\forall N\,\exists n\ge N,\ P(n)$.  Compatibility between witnesses of the last kind is an
additional requirement only when explicitly imposed.
...
as open assumptions.  The range alone determines neither the support nor
the truth status; the stated predicate and premises do.

\begingroup
\small
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{}p{0.60\linewidth}p{0.37\linewidth}@{}}
\toprule
\papertablehead
\textbf{Result} & \textbf{Quantifier prefix} \\
\midrule
\endfirsthead
\toprule
\papertablehead
...
\bottomrule
\endfoot
\papertablehead\multicolumn{2}{l}{\emph{One concrete witness or value}} \\
Seed for continuation from $n\ge6$: support $\{2,3,6\}$ satisfies
$\ensuremath{Q}=2^5-1$ (exact computation). \lean{exactLocalMersenneHalfRow\_six}{BooleanMobiusExactRowSeed.lean:34}
& $n=6$ \\
One-point countermodel: the double-or-recycle transition shape alone, even seeded, does not
give exact rows at unbounded depths. \lean{boundedDoubleOrRecycleModel\_not\_cofinal}{BooleanMobiusExactRowDichotomy.lean:79}
& $n=6$ (free-standing model) \\
The combined-residue-mass $\le 1$ sufficient test for sharp capacity is not necessary:
example $D=\{2,3\}$, $c=5$ (exact computation). \lean{splitFractionMass\_one\_bound\_not\_necessary\_fixture}{BooleanMobiusSkippedCoreCriticalCapacity.lean:183}
& $D=\{2,3\},\ c=5$ \\
Exact residual: $\tfrac12-W(\{2,3,6,7\})=\tfrac1{16002}$ (exact computation).
\lean{half\_sub\_four\_term\_prefix\_eq}{HalfCutLocator.lean:664}
& none (numeric instance) \\
Closed-form M\"obius--Lambert value $\sum_{d\ge1}\mu(d)/(2^d-1)=1/2$ (classical identity, cited).
\lean{tsum\_moebius\_div\_two\_pow\_sub\_one\_eq\_half}{MersenneLambertLadder.lean:587}
& none (closed-form value) \\
The one natural negative-M\"obius candidate support overshoots: $1/2 <
\sum_{d\in N}1/(2^d-1)$ for $N=\{d\ge2:\mu(d)=-1\}$.
\lean{half\_lt\_tsum\_negativeMobius}{MobiusSignSupportNoGo.lean:164}
& one fixed candidate support $N$ \\
\papertablehead\multicolumn{2}{l}{\emph{Finite ranges and witnesses bounded by the input}} \\
...
& $\forall n\ge6$, given a row at $n$: $(\ldots)\lor\exists c\le n(\ldots)$ \\
Every above-half bounded support recycles into a genuine exact row at some $2c-2$ with $c\le n$.
\lean{exists\_skippedCoreExactRow\_of\_value\_above}{BooleanMobiusExactRowCrossing.lean:155}
& $E\subseteq[2,n]$, $X_E(2)>1/2$: $\exists c\le n$ \\
(A cross-check for Problem~\#249.) $\sum\varphi(n)/2^n$ matches no rational whose denominator divides
$2^{14}(2^h-1)$, for some $1\le h\le16$ (exact computation).
\lean{totient\_series\_ne\_rat\_of\_den\_dvd\_upto\_sixteen}{CarrySurvivorExtinction.lean:587}
& $1\le h\le16$ (explicit bound) \\
...
ordinary argument with exact computation
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/notes/truncation-rung-ladder-2026-07-24.md}{working note},
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/check_truncation_rung_ladder.py}{program}).
& $3\le J\le22$ (finite table, exhaustive) \\
\papertablehead\multicolumn{2}{l}{\emph{Uniform assertions and conditional implications; premises retained}} \\
Membership equals greedy survival.
\lean{mem\_mersenneAchievementSet\_iff\_greedy\_survival}{GreedyAchievementSet.lean:1458}
& $\forall x\ge0$, membership iff survival at every rank \\
Membership iff every actually skipped rank survives.
\lean{no\_lastHalfGreedySkip\_iff\_every\_skip\_survives}{HalfCylinderFixedTailSocket.lean:57}
& $\forall M\in$ skipped support \\
...
& $c\ge4$, $D\subseteq[2,c)$, $0<1/2-X_D(2)<w_c$ \\
Largest-skip conditional implication used in the reset argument.
\lean{half\_mem\_mersenneAchievementSet\_of\_largestSkipLateStepSocket}{HalfCylinderLargestSkipInduction.lean:167}
& $\forall s\ge14$, given the hypothesis \\
Band avoidance verified by exact computation at reset indices $13\le r\le30$, using
...
& $\forall b\ge2$ \\
Erd\H{o}s 1968: any infinite, pairwise-coprime, reciprocal-summable index set gives an irrational
support series. \lean{irrational\_erdosSupportSeries\_pairwise\_coprime}{CertificateKernel.lean:10776}
& $\forall b\ge2\ \forall A$ (infinite, coprime, summable) \\
Block-certificate criterion, independent of the problem: a weighted-coefficient series is
irrational given a divisibility and window certificate at every precision.
\lean{irrational\_coeff\_series\_of\_weighted\_coeff\_block\_certificates}{CertificateKernel.lean:8665}
& $\forall b\ge2\ \forall c\,(\forall m,c(m)\le m)$, given $\forall q\,\exists N,K,L,C$ \\
...
& $\forall n$ \\
\papertablehead\multicolumn{2}{c}{\textbf{Unproved hypotheses and the proved implications that use them}} \\
\papertablehead\multicolumn{2}{l}{\emph{Unbounded-witness and all-reset conditions have different quantifiers}} \\
The exact-row existence condition: exact rows recur at arbitrarily large endpoints, with no coherence
required between witnesses (unproved). \lean{CofinalExactLocalMersenneHalfRows}{BooleanMobiusCofinalExactRows.lean:38}
& $\forall N\ \exists n\ge N$: an exact row at depth $n$ \\
The preceding condition implies $1/2\in\mathcal A$ by compactness (theorem proved;
hypothesis open). \lean{half\_mem\_mersenneAchievementSet\_of\_cofinalExactLocalRows}{BooleanMobiusCofinalExactRows.lean:71}
& given $\forall N\,\exists n\ge N(\ldots)$ \\
An equivalent membership condition: the canonical rational half-greedy orbit itself has infinitely many
positive skips (unproved). \lean{CofinalPositiveHalfGreedySkips}{BooleanMobiusSkipRowCofinal.lean:22}
& $\forall N\ \exists c\ge\max(N,4),\ 0<\mathrm{rem}(c{-}1)<w_c$ \\
Infinitely many positive greedy skips imply $1/2\in\mathcal A$ (theorem proved; hypothesis open).
\lean{half\_mem\_mersenneAchievementSet\_of\_positiveHalfGreedySkips}{BooleanMobiusSkipRowCofinal.lean:97}
& given $\forall N\,\exists c\ge N(\ldots)$ \\
Membership iff the greedy skip set is infinite (equivalence proved; both sides assert
unboundedly many indices).
\lean{half\_mem\_mersenneAchievementSet\_iff\_greedySkippedSupport\_infinite}{GreedyAchievementSet.lean:2583}
& the skipped support is infinite \\
Seven equivalent forms of membership (Theorem~\ref{thm:nine-way-hub}): the integer-greedy
sequence is not eventually always on branch $\mathrm R$, the terminal rank is omitted at
unboundedly many rows, there are cofinally many skipped ranks, \emph{etc.}
...
& every right-hand side asserts unboundedly many indices \\
Ladder dichotomy: either infinitely many rungs survive (\#257 false, explicit limit support) or an
eventual bad-rank obstruction closes only this route (ordinary argument,
Corollary~8 of the
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/notes/truncation-rung-ladder-2026-07-24.md}{truncation-rung note}).
& open branch (a): $\exists^\infty J,\ \mathrm{HalfRung}(J)$ \\
Reset square-root escape, a sufficient all-reset hypothesis (unproved).
Theorem~\ref{thm:lower-bound-every-reset} gives the conditional
implication; the run-length
comparison is discussed separately in Section~\ref{sec:o4}.
& $\forall r\ge10$ that are resets,
$|\mathrm{rem}(r+1)-2^{r+1}|>2^{(r+5)/2}$ \\
\caption{Quantifiers for the stated results.  The groups
...
An earlier review examined eighteen possible implications from recorded
results to the required unbounded-witness conditions.  It found no
such implication.  This is a report about those attempts, not a theorem
excluding other arguments.
...
What decides the matter is the precise premise and conclusion, whatever
tactic a finite proof uses.
\end{rem}
...
the truncated weights. Certified for $3\le J\le22$ (Table~\ref{tab:scale-ladder}, finite ranges;
...
reset-crossing classification, and the underlying orbit separately certified to row
$200{,}000$, with exactly one violation of the target inequality at reset rows $6\le r\le200{,}000$, at $r=7$
(saved runs to
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-reset-crossing-2500.json}{row $2500$}
and to
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-orbit-200000.json}{row $200{,}000$}).
\item \textbf{Integer margin $m_c$.} An exploratory, unverified scan of the integer-greedy
margin out to $5\times10^5$, tracking how far the certified region sits from the danger
threshold.
...
logarithms of the corresponding positive slack multiplied by $4^k$
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/tail-margins-3000.json}{saved run};
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/notes/reset-crossing-unification-2026-07-24.md}{working note},
section~8).
\end{itemize}
...
No conclusion about the exploratory margin scan follows without specifying
...
$3\le J\le22$.  The checking program uses exact integers: it verifies
witness intervals covering the ranks beyond $B(J)$, proves for $J\le16$
that the listed bad ranks have no witness, and tests the greedy remainders
through $B(J)$.  In particular, it reproduces $B(19)=1008$ and
$B(20)=B(21)=B(22)=1530$.
The
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/check_truncation_rung_ladder.py}{checking program}
and its saved outputs for
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/truncation-ladder-3-16.json}{$3\le J\le16$}
and
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/truncation-ladder-17-22.json}{$17\le J\le22$}
are published.  For $J\ge17$ a floating-point filter proposes witnesses
and each accepted witness is rechecked with integers, so a missed witness
could only lengthen the exact greedy check.  These are finite
...
This section compares the stated results with the remaining hypotheses.  Each result is
tested against three exact open obligations: what it yields, where it falls short of the
obligation (a stronger hypothesis, a different parameter range, a different quantifier
order, a different expression of the same condition, or several of these), and the exact
auxiliary statement that would close the gap.  Nothing here decides \#249 or \#257.  A
conditional reduction is a result with a hypothesis, not a proof of that hypothesis, and a
finite verified list does not supply witnesses at unbounded depths.
...
For every $N$ there is $n\ge N$ with an exact row at depth $n$
(Definition~\ref{record:257bm-d1}); that is, there are finite sets at unboundedly many depths
whose quotient sums land exactly on the half-value target.  This condition implies
...
be mutually incompatible; only their depths need tend to infinity. Requiring compatibility would add a condition on the witnesses.
...
\begin{obs}[Remainders along a right-branch run]
...
\begin{obs}[The affine excess recurrence]
...
\begin{obs}[A bound at later rows]
...
\begin{obs}[Exclusion of equality at a branch boundary]
...
\begin{obs}[A sharper test for a fatal greedy gap]
...
\begin{obs}[Exclusion of a linear-width band at rows 13--30]
\lean{seamUpperResetDyadicBandEscape\_through\_thirty}{HalfCylinderUpperResetBandCertificates.lean:78}.
...
\lean{SeamUpperResetDyadicBandEscape}{HalfCylinderMiddleCarryLowerBound.lean:4566}, whose full
form would give half-membership by
\lean{half\_mem\_mersenneAchievementSet\_of\_upperResetDyadicBandEscape}{HalfCylinderMiddleCarryLowerBound.lean:4790},
and so refute the universal statement.
\textbf{Parameter range.} Proved at 18 explicit rows by exact computation checked in the Lean
kernel; the obligation needs all $d \ge 13$. The excluded band has width $O(d)$, whereas the reset condition uses
...
Only $O(d)$ separation is required here; separation at the square root of the dyadic scale is not needed.
\end{obs}

\begin{obs}[Middle predecessors and right-branch runs]
...
bound this upgrades to $2^{s+1}+(s+1) \le \mathrm{rem}(s+1)$. A minimal-counterexample argument
in the same Lean source then shows that any row whose remainder is smaller than its index must have an upper-reset (U) ancestor, since M-ancestors
...
$O(d)$-wide band escape of the preceding observation.  The 257-reset statement demands escape
at the square root of the dyadic scale, whereas linear-width escape at U-resets alone suffices
for this argument.
\textbf{Missing step.} The single remaining channel: for every $d \ge 13$ with successor
carries, the reset charge avoids the width-$2(d+j_0)$ band at its critical dyadic index.  Then
the remainder is at least the row index at every late middle row, which is the hypothesis of
the next observation, and hence $1/2 \in \ensuremath{\mathcal A}$; each of these implications is
already proved in Lean.
\end{obs}

\begin{obs}[A linear lower bound at middle transitions]
...
(via its cardinality and tail forms, \lean{}{HalfCylinderMiddleCarryLowerBound.lean:1510}).
\textbf{Conclusion.} If at every late middle row ($s \ge 13$, no carry, middle branch)
$s \le \mathrm{rem}(s)$, then $1/2 \in \ensuremath{\mathcal A}$.  The hypothesis is much
weaker than an exponential bound, but stronger than the square-root-scale carry bound.
...
to this linear bound; no converse is asserted.
\end{obs}

\begin{obs}[A square-root lower bound in the row index]
\lean{SeamMiddleProducerSqrtEscape}{HalfCylinderLastProducerContradiction.lean:84},
its implication to the exact tail bound,
\lean{half\_mem\_mersenneAchievementSet\_of\_middleProducerSqrtEscape}{HalfCylinderLastProducerContradiction.lean:458}.
\textbf{Conclusion.} If at every late middle row $s$ the producer carry exceeds $2\sqrt{2s+2}+4$, then
$1/2 \in \ensuremath{\mathcal A}$; the producer carry equals the middle coordinate
...
\textbf{Comparison.} This is a square root of the \emph{row index}
$s$ (a tail majorant for the divisor counts), not the square root of the \emph{dyadic scale}
$2^{(r+5)/2}$.  The two square roots in this record are exponentially far apart and easy to
conflate; this hypothesis is a bound in the row index.  The cited conditional theorem uses it to rule out a
...
\begin{obs}[The remaining interval of possible integer values]
...
\begin{obs}[Excluding three integer values]
...
\begin{obs}[An upper bound on the remainder]
...
\begin{obs}[Reported computation through row 200{,}000]
\textbf{Conclusion.} Exact integer arithmetic to row 200{,}000
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-orbit-200000.json}{saved run}); branch counts $R\!:\!M\!:\!U =
100197\!:\!49899\!:\!49898$; for $r\ge6$ the obligation \emph{fails only at} $r=7$; maximum R-run $19$ after the reset at row
$158{,}096$ against the required threshold $\approx (r-3)/2 \approx 79{,}000$ (margin
$\sim\!4000\times$). Independently: 1209 resets checked to row 2500
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-reset-crossing-2500.json}{saved run}), zero classification
anomalies, exactly two crossing cells in rows $6\le s\le2500$ ($s{=}7,d{=}5$ and $s{=}10,d{=}7$), both below hypothesis
scope.
\textbf{Parameter range.} fixed-scale exhaustive verification versus a statement
at every reset $r \ge 31$.  This is the difference between a finite and an unbounded
parameter range: a finite verified list supplies no witnesses at unbounded depths, however
large the margin.
\textbf{Missing step.} Nothing computational; only a theorem. The certificate's value is fixing the
true constant ($r \ge 31$, sole violation among reset rows $6\le r\le200{,}000$ at $r=7$) so the theorem's scope can be stated exactly.
\end{obs}

\begin{obs}[Structure of resets after long runs]
...
Pinning to two integers is not yet a contradiction: no theorem excludes those two values.  This is
an ordinary argument; it is not formalised.
...
\begin{obs}[The sign condition and growth of the margin]
...
1.11 bits at $r=14$ to $\sim$1246 bits at $r=2499$. The record isolates this as a separate,
possibly more tractable target than the full square-root escape.
...
\begin{obs}[Comparison of finite truncation lengths]
...
an infinite statement from a long finite run.  Saved runs for
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/truncation-ladder-3-16.json}{$3\le J\le16$} and
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/truncation-ladder-17-22.json}{$17\le J\le22$} reproduce it.
...
\begin{obs}[Exact rows from a sharp capacity bound]
...
$\ensuremath{S}\,D\,1\,(2c{-}2) < 2^{c-2}$, and this record supplies that
input only through the crossing-deficit route, which
\lean{eq\_halfGreedyPrefixSupport\_of\_critical\_crossing}{BooleanMobiusCriticalCapacityCofinal.lean:50}
...
\begin{obs}[A conditional induction for unbounded depths]
...
(\lean{ProtectedExactLocalMersenneRow}{BooleanMobiusCriticalCapacityCofinal.lean:1087}; strict
progress \lean{}{BooleanMobiusExactRowCrossing.lean:233}).
...
endpoint.
\textbf{Scope of the hypothesis.} The recycle branch uses the critical quotient supply
hypothesis whenever recycling is required.
...
\begin{obs}[Unconditional capacity, off by one bit]
...
\begin{obs}[Doubling extension, fails at the only recorded seed]
\lean{exists\_exactRowStrictUpperExtension\_two\_mul\_sub\_one\_of\_exact\_below}{BooleanMobiusExactRowDoubling.lean:185};
rank two is automatic
(\lean{two\_mem\_of\_exact\_localMersenneQuotient}{BooleanMobiusExactRowRankTwo.lean:23}).
\textbf{Conclusion.} $\forall n \ge 6$: a below-half exact row at $n$ produces an exact row at
$2n{-}1$, support-extending, all new ranks $>n$.  This is the only unconditional
endpoint-doubling step in this record.
...
exact at $n$ with $\sum_{d\in D}2^{n\bmod d}/(2^d-1) < 1$. A single such row at each of cofinally
many $n$ makes doubling unnecessary; a self-reproducing one closes everything.
\end{obs}

\begin{obs}[Consecutive-skip precritical bound]
\lean{halfGreedy\_precriticalSuffix\_lt\_of\_next\_skip}{BooleanMobiusCriticalCapacityCofinal.lean:682}
(used in \lean{halfGreedySkippedCriticalQuotientSupply\_of\_precriticalSuffix}{BooleanMobiusCriticalCapacityCofinal.lean:1001}).
...
$2c{-}2$, with no supply hypothesis.
\textbf{Scope of the hypothesis.} It needs two \emph{consecutive} skipped ranks.
...
with $c=4,5$ already settled by direct computation.
...
\begin{obs}[Bounded-gap precritical bound]
...
\begin{obs}[Frozen margin, ineffective horizon]
...
(convergence of the finite coefficient windows, then an existence extraction), so $J$ is
unbounded.
...
$|E_k|/D_k > (2k{+}1)/2^{k-2}$ at every skipped rank.  This uses only lemmas already proved.
\end{obs}

\begin{obs}[A nonnegative margin at an unspecified horizon]
...
\textbf{Open property and proved conditional implication.} The open property asks
that every skipped rank $n\ge3$ satisfy
$0\le\ensuremath{F}(n{-}1)\,n$. Assuming this property, a Lean theorem proves
$1/2\in\ensuremath{\mathcal A}$.
\textbf{Order of quantifiers.} The horizon is exactly three doubling steps short.
...
+\mathrm{sc}(2k{+}2)$, where $\mathrm{sc}$ is the divisor count, bounded by $\tau\le 2\sqrt{2k+2}$. Since
...
an explicit $O(\sqrt k)$ divisor-count quantity, on the same square-root scale as the carry
bounds above.
\end{obs}

\begin{obs}[Cofinal positive skips \emph{is} the obligation]
...
\lean{half\_mem\_mersenneAchievementSet\_iff\_greedySkippedSupport\_infinite}{GreedyAchievementSet.lean:2583},
\lean{}{GreedyAchievementSet.lean:1515}; positivity
...
\textbf{Scope of the hypothesis.} It is equivalent to the problem itself. The positivity
conjunct $0 < \ensuremath{r}(1/2)(c{-}1)$ holds unconditionally by the odd-denominator parity
fact, so the hypothesis says exactly that the greedy rule for $1/2$ skips infinitely many
ranks, which Theorem~\ref{record:257bm-c20} proves equivalent to
...
\begin{obs}[Terminal bounds and restrictions on finite support]
...
quotient is exactly $2^{M-d}$.  The essential difference is where the support may sit.

\par\medskip

\textbf{Missing step.} Confinement of the strip witnesses to the lower half: $\forall N\
...
\begin{obs}[The live greedy carry and the carry of a fixed prefix]
\lean{GreedyHalfCarryCofinalStripReturn}{CofinalStripReturn.lean:76},
\lean{greedy\_half\_infinite\_of\_cofinalStripReturn}{CofinalStripReturn.lean:130},
...
\textbf{Conclusion.} Cofinal returns of the actual greedy carry to the square-root strip
(Theorem~\ref{record:257rig-c18}), or a pointwise $2\sqrt N+4$ bound on the M\"obius-centred
carry (Theorem~\ref{record:257rig-c16}), each give an infinite support with value exactly $1/2$.
\textbf{Comparison.} These bound the carry of the \emph{infinite}
greedy support $G$ of $1/2$; sharp capacity is about the \emph{frozen
...
sharp capacity $\Leftrightarrow \ensuremath{C}\,G\,(2c{-}4) \le \Phi_c$, where
$\Phi_c=\sum_{i=1}^{c-3}2^{c-3-i}\mathbf 1[c+i\notin G]$ is the future-skip capacity;
transport costs exactly this term.
\textbf{Missing step.} Either a strip bound stated for the frozen prefix rather than the live orbit,
or $\Phi_c \ge \ensuremath{B}(2c{-}3)$; holds as
...
\begin{obs}[Above-support recycling, no growth guarantee]
\lean{exists\_skippedCoreExactRow\_of\_value\_above}{BooleanMobiusExactRowCrossing.lean:155},
...
The countermodel of Theorem~\ref{record:257bm-k1}
(\lean{exists\_seeded\_bounded\_double\_or\_recycle\_model}{BooleanMobiusExactRowDichotomy.lean:91})
satisfies the same transition schema plus a seed and does \emph{not} give rows at unbounded
depths.  Growth is recovered only for protected exact rows, whose invariants force
$c>$cutoff, hence $2c{-}2>$ endpoint; and maintaining those invariants is precisely what needs the sharp-capacity fill
...
\begin{obs}[Double-or-recycle dichotomy, proved insufficient]
\lean{exactLocalMersenneHalfRow\_double\_or\_recycle}{BooleanMobiusExactRowDichotomy.lean:26},
...
of the witness value (odd denominators exclude equality); the countermodel pins the
...
\begin{obs}[A sufficient block-certificate condition]
...
\textbf{Scope of the hypothesis.} The criterion is already universally quantified over $A$;
the universal quantifier is \emph{not} the gap.  The certificate hypothesis must hold at every
precision, and it is supplied only for named classes; certificate existence for an arbitrary
...
\begin{obs}[Rational targets and infinitely many greedy skips]
\lean{half\_mem\_mersenneAchievementSet\_iff\_greedySkippedSupport\_infinite}{GreedyAchievementSet.lean:2583}
...
value.  The Lean declarations above concern the half target; the
extension to every rational target is an ordinary argument and is not
formalised.
\end{obs}

\begin{obs}[Finite sums exclude every rational with even denominator]
\lean{finite\_boolSupport\_ne\_half}{HalfCarryReachability.lean:589}; the underlying parity
lemma has its own \lean{finiteErdosSum\_den\_odd}{DyadicPrefixCompression.lean:1513}.
\textbf{Conclusion.} $\forall A$ finite with $0\notin A$: $X_{A}(2) \ne
1/2$; any support achieving exactly $1/2$ must be infinite.
\textbf{Parameter range.} The statement is specific to the target $1/2$, while the underlying
parity lemma (the reduced denominator of a finite sum is odd) holds for every finite support.
\textbf{The same argument in general.} For every rational $t$ with even reduced denominator
and every finite $A$ with $0\notin A$, $X_{A}(2) \ne t$; the proof is the same. Thus a represented rational with even reduced denominator would refute universal
\#257 at $b=2$. The converse is not established. If $\mathcal F$ denotes the set of finite-support
values, the exact unrestricted condition is
$(\mathbb Q\cap\ensuremath{\mathcal A})\smallsetminus\mathcal F\ne\varnothing$.
\end{obs}

\begin{obs}[The denominator in the reciprocal-mass estimate]
\lean{dyadic\_support\_fraction\_reciprocalMass\_diverges\_or\_gt\_one}{RationalSupportCarrySkeleton.lean:2210};
the version for general $v$ has its own
...
rational): $\sum_{a\in A}1/a$ diverges or exceeds $1$.
\textbf{Comparison.} The result is restricted to denominators $2^c$ ($v{=}1$), while
the obligation needs every rational $p/(2^c v)$.  The restriction is one of application, not
of proof: every supporting lemma allows an arbitrary positive $v$, and the general-$v$
conclusion is already recorded for arbitrary $(v,h,L,F)$.
...
\mathrm{doublingWrapCount}(p,v,h)/h + \lfloor|F|/2\rfloor/L$ (equivalently: the
mean least residue of $p\cdot2^n \bmod v$ is $\ge v(1-|F|/2L)$).
\end{obs}

\begin{obs}[Zero windows of a fixed support]
\lean{supportCoeffZeroWindow\_length\_le\_eps\_logb}{SublogDivisorCoverage.lean:435}.
\textbf{Conclusion.} For any $A$ with a positive element and
$X_{A}(2)=p/(2^c v)$, $v{>}0$ (any rational, not just dyadic):
for every $\varepsilon>0$ there is $B\ge0$ such that, for all $N\ge1$, every run of $h$ zero
divisor counts starting after $c+N$ has $h \le \varepsilon\log_2 N + B$. The constant $B$ may depend on the fixed support and the other parameters.
...
\begin{obs}[Prime support and the reciprocal-sum hypothesis]
...
$\sum_p1/p$ diverges, so this Lean theorem does not apply.  It is false,
...
\textbf{What remains is a test of the method:} replacing the global tail budget by a
windowed one would ask whether this particular certificate theorem can recover
the known theorem.  It is no longer an open mathematical target or a claimed
new route for \#257.
\end{obs}

\begin{obs}[The two hypotheses of the sunflower criterion]
\lean{irrational\_erdosSupportSeries\_of\_orthogonalPetalBouquet}{SupportSunflowerDichotomy.lean:540}.
...
1/\mathrm{petal}_i<\infty$: if $A$ satisfies the tail-selection hypothesis below, then its
series is irrational.

\par\medskip

\textbf{Different restrictions.} Two gaps. (i) On the support: the bouquet class strictly
generalises pairwise-coprime supports but still demands globally pairwise-coprime, summable
petals; it inherits the exclusion of the primes noted in the preceding observation and adds
nothing in the divisor-dense regime. (ii) On the selector: the tail-selection hypothesis must
hold at every precision and is unproved.

\par\medskip

\textbf{Missing step.} For the selector, $\forall K{>}0\ \exists N$, $2^K \mid$ the carried first
block $\wedge\ \sum_{j\ge1}c_{A}(N{+}K{+}j)2^{-j}\le16$, from the
bouquet's persistent-reduced-modulus averaging. For the class: the assertion that every infinite $A$ admits a
bouquet structure is \emph{false} (take $A=$ all multiples of 6: there is no coprime
...
\begin{obs}[What eventual periodicity supplies]
...
that the periodic sieve manufactures one block certificate. The periodic argument already
needs periodicity only across the certificate window, so this is the closest genuine
widening; but proving that periodic windows exist at unboundedly many scales for arbitrary $A$
is a new statement.
\end{obs}

\begin{obs}[The reduced denominator in a gap criterion]
...
\begin{obs}[Rationality expressed by the integer carry]
\lean{erdosSupportSeries\_rational\_iff\_exists\_temperedCarry}{BooleanMobiusCarry.lean:384};
the general criterion of Theorem~\ref{record:257bm-i-t7} has its own
\lean{binaryCoeffSeries\_rational\_iff\_exists\_temperedBinaryOrbit}{GenericTailOrbitRigidity.lean:426}.
\textbf{Conclusion.} For every $A\subseteq\mathbb N$, with no hypothesis:
$X_{A}(2)$ is rational if and only if there are $q>0$ and $U:\mathbb N\to\mathbb Z$ with
$U(N+1)=2U(N)-q\,c_A(N+1)$ and $U(N)/2^N\to0$.
\textbf{Comparison.} This is already universal over $A$; it is the exact form in which the
obligation should be stated, and the argument literally shared with \#249 (take
$c=\varphi$ in Theorem~\ref{record:257bm-i-t7}). It does not refute the orbit:
...
\textbf{Missing step.} A support-specific non-existence theorem: for every infinite $A$ and
every $q>0$, no such $U$ exists.  Because the criterion is an equivalence and the orbit is
unique, this is logically equivalent to the obligation; its value is to name the single object
(the tempered carry orbit of $c_{A}$) whose non-existence is the entire content, for every $A$
at once.
\end{obs}

\begin{obs}[Why unbounded carry alone is insufficient]
\lean{shifted\_state\_unbounded\_of\_infinite\_support}{RationalSupportCarrySkeleton.lean:2327},
...
\begin{obs}[A finite-gap criterion for every target in the convex hull]
...
the exclusion for a general support has its own
...
\begin{obs}[Why the signed periodic theorem still requires periodicity]
...
unconditionally as soon as $c_w$ is one-signed and frequently nonzero.
...
required; this is the signed version of the periodic-support theorem and keeps its periodicity hypothesis.
(ii) The one-sidedness clause is automatic for \#257 (a $0/1$ support indicator gives
$c_{A}\ge0$), so the sign conditions add nothing here; they were designed for the signed
\#249 argument.
...
so removing it again requires the missing supply of certificates for arbitrary supports.
\end{obs}

\subsection{Three combined conditional arguments}

Entry A has a Lean proof, and entry B is an ordinary argument about what
its hypotheses would imply.  The facts behind entry C, the survival equivalence of
Theorem~\ref{thm:greedy-survival-record} and the one-sided certificates,
are proved in Lean.  None of the three establishes an unproved orbit
hypothesis or decides~\#257.

\begin{thm}[A: a sufficient lower bound at every reset]\label{thm:lower-bound-every-reset}
Assume that
...
\]
For $r\ge10$, one has $B(r)^2\le2^{r+5}$.
...
A dangerous reset (one within $2^{(r+5)/2}$ of the square-root escape threshold) that is preceded by a
...
$n{-}1$ nested exact integer conditions, none free to vary once the pulse stream is fixed. Failure of square-root escape therefore
...
values.  This is an ordinary argument.
\end{rem}

\begin{thm}[C: the one-sided finite decision boundary]\label{thm:one-sided-finite-decision-boundary}
The equivalence of Theorem~\ref{thm:greedy-survival-record} separates two logically different kinds of evidence. A fatal greedy gap found at a finite rank is
a finite certificate that $1/2\notin\mathcal A$. By contrast, membership requires survival at
every rank; this paper supplies no finite certificate that the orbit survives forever and no
...
finite ranks; this is the quantifier boundary of Theorem~\ref{thm:one-sided}.
\end{thm}
...
\begin{rem}[The centred-carry bound is equivalent to the conclusion, for the greedy support only]
...
assume the bound $C_A(N)\le2\sqrt N+4$ for every $N$. For the greedy support $A=G$ of $1/2$
specifically, this bound is logically \emph{equivalent} to the theorem's own conclusion
$X_{A}(2)=1/2$, not a strictly weaker sufficient condition. This follows
from the exact residual identity of Lemma~\ref{lem:collapse-mech}, the unconditional
square-root tail bound
...
$X_{A}(2)\le1/2$. \textbf{The scope limit is exact:} this equivalence does \emph{not} extend to
every $A$ with $1\notin A$; dropping the greedy-specific fact yields only that the bound is
equivalent to $\delta\le0$ (a one-sided condition), not to $\delta=0$. For the related hypothesis
\lean{GreedyHalfCarryCofinalStripReturn}{CofinalStripReturn.lean:76}, the former gap between
the real and the integer square root is removed by restricting cofinal witness depths
to perfect squares. Any remaining gap lies in the hypothesis itself, not in rounding.
\end{rem}
...
conversion raises no further issue; the open content is the hypothesis itself.
\end{rem}

\begin{rem}[Positive greedy skips and half-membership]
As noted above, the positivity conjunct of
\lean{CofinalPositiveHalfGreedySkips}{BooleanMobiusSkipRowCofinal.lean:22} holds
unconditionally, by parity and greedy nonnegativity
(\lean{}{BooleanMobiusCriticalCapacityCofinal.lean:138}).  The hypothesis therefore says
exactly that the greedy rule for $1/2$ skips infinitely many ranks, which is
\emph{equivalent} to $1/2\in\ensuremath{\mathcal A}$
(\lean{}{GreedyAchievementSet.lean:2514}).  A proof or disproof of this statement settles
the half-target question, and conversely; it is not an intermediate step toward it.
\end{rem}
...
deviation of Theorem~\ref{thm:dynamics}, and $\mathcal A$ the Mersenne achievement set. Reset square-root escape
from row 10 is the statement $|\mathrm{rem}(r+1)-2^{r+1}| > 2^{(r+5)/2}$ for every reset row $r\ge 10$.
It is a sufficient input for the cited
...
\textbf{Route as conceived.} The universal recursion $K(M+1) = 2K(M) - (\tau(M+1)-1)$ doubles at every step and subtracts the divisor-excess $\tau(M+1)-1$. At a prime $M+1=p$,
...
values illustrate.  This table is a new finite arithmetic check; the
longer earlier run search was not repeated.
...
is conditional.  It propagates by induction from a local hypothesis which at every row
...
the transition at row $s$ is on branch $\mathrm U$ of Theorem~\ref{thm:dynamics}.
Then
\[
...
\textbf{Object or representation:} about the \emph{object}
under the stated hypothesis ;  a genuine arithmetic consequence of the all-right-tail assumption
via the fatal-gap orbit, not a coordinate artifact.
\end{thm}
...
This is the reduced middle tail condition of Part~III, and half-membership follows from it
(\lean{half\_mem\_mersenneAchievementSet\_of\_middleProducerTailEscapeExceptNegThree}{HalfCylinderLastProducerContradiction.lean:436}).
...
without the exception.  Their Lean forms are
...
\textbf{Object or representation:} about a \emph{representation}
...
\textbf{Object or representation:} about the \emph{object} ;  a genuine
...
\textbf{Scope.} The Lean declarations used in this comparison
...
\textbf{Object or representation:} about a
...
\begin{rem}[What this does \emph{not} show]
\label{rem:band-caveat}
Definition~\ref{defn:band-escape} is a $\forall d\ge13$ statement; Observation~\ref{cert:band-through-31}
is a finite verification in the cited Lean source for $13\le d\le30$. \textbf{The
finite certificate does not establish the universal hypothesis.} Nothing here proves the band
condition for all $d\ge13$; it is the unproved hypothesis of
Theorem~\ref{thm:upper-reset-band}, stated here in its own form.
...
and unproved proposals are distinguished in the text.  No single necessary obstruction is claimed
...
cases.  An exact computation
(\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-orbit-200000.json}{saved run})
gives a maximum run of length
$19$ through row $200{,}000$, following the reset at row $158{,}096$.
That observation is finite evidence, not an asymptotic theorem.

\emph{Form (ii), band avoidance, already proved to imply membership.}  For every
upper reset $d\ge13$ and every $j\le d$,
\[
...
\]
A finite recomputation (\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-reset-crossing-2500.json}{saved run}) verifies these signs
...
only on $x \bmod d$.
\end{proof}
The identity was also checked by exact rational computation at
$(M,L)\in\{1000,10001,123456\}\times\{12,20\}$.
...
equivalence, or an unproved bound for all later rows.  This conditional
calculation is an ordinary argument.
...
\end{tabular}
\end{center}
The experiment samples consecutive integers, not reset rows.  Its median distance
...
In particular, no membership conclusion is drawn from small
expected counts computed in such a model.  The counts are
exact finite computations; the error and probability inequalities are
ordinary arguments; an application to resets is unproved.
...
says every exact row $n\ge6$ either doubles to $2n-1$ or recycles to $2c-2$ for some
$4\le c\le n$.  Exact rows at unbounded depths, which give $1/2\in\mathcal A$, need the recycle
...
all proofs. The record already has the sharp
...
exhibits the one-point predicate $P(n):=(n=6)$, which satisfies the seed
...
proof must use arithmetic content of exact rows that the abstract
schema does not see.

There is a second, more specific trap, and it is also formalised. The natural sufficient
condition via fractional split mass is \emph{not necessary}: the example at $D=\{2,3\}$, $c=5$
(\lean{splitFractionMass_one_bound_not_necessary_fixture}{BooleanMobiusSkippedCoreCriticalCapacity.lean:183})
exhibits a real crossing core violating it. Thus the fractional-mass bound is not necessary for an individual
...
which forces $D=G\cap\{1,\ldots,c-1\}$. So the $\forall D$ collapses
...
analytic result, not a theorem formalised in Lean.

The existing pairwise-coprime Lean theorem does not recover it because that
theorem assumes $\sum_{a\in A}1/a<\infty$, whereas $\sum_p1/p$ diverges.  The
block-certificate criterion behind it would also demand, in the prime case,
\[
...
Thus the contribution of this analysis is negative and specific to
the method: it identifies why the present formal certificate
criterion fails to reproduce a theorem already available by quantitative
...
The forward directions of (2)--(5) are proved in Lean
(\lean{half_mem_mersenneAchievementSet_of_positiveHalfGreedySkips}{BooleanMobiusSkipRowCofinal.lean:97},
...
\lean{half_mem_mersenneAchievementSet_of_cofinalTerminalOnlyStrip}{TerminalOnlyCofinal.lean:134}). The reverse implications are ordinary deductions in
...
\end{lem}
\begin{proof}
By \lean{integerHalfCarry_eq_scaled_residual_add_tail}{HalfCarryReachability.lean:871},
\[
...
\lean{binaryCoeffTail_supportCoeff_le_two_sqrt_add_four}{BooleanMobiusCarry.lean:290}
...
= 2\lfloor\sqrt n\rfloor+4$, which at $n=k^{2}$ equals $2k+4$ exactly. Non-negativity of the tail
(\lean{binaryCoeffTail_nonneg}{GenericTailOrbitRigidity.lean:78}) gives the lower side.
\end{proof}

The difference between the real and the integer square root disappears when cofinal witness
depths are restricted to perfect squares: at $n=k^{2}$ the two roots coincide
...
equivalence of the bound in (1), with $A$ in place of $G$, and $1/2-S_A\le0$. The mechanism in all five cases is the same and
...
\lean{mem_mersenneAchievementSet_iff_greedy_survival}{GreedyAchievementSet.lean:1458}.
...
The dense branch as stated is \emph{false}: an explicit, natural, divisor-dense support refutes
it.  This also corrects an earlier claim that universal \#257 reduces with no loss to the
existence of certificates for every infinite support.

\begin{prop}[Squarefree support: a limit of the certificate method, not an open value]
...
The dependence on the choice of support is exact.  Adjoin $1$ to the support.  The series
...
Everything in this subsection is inference from the recorded failures, not a theorem.  Where
the evidence does not determine the answer, the text says so.

\subsubsection*{Finite observations and their limits}
The recorded obstructions concern specified representations.  Fixed
...
$R:M:U=100197:49899:49898$, which a
\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-orbit-200000.json}{saved run}
reproduces, a roughly geometric run-length histogram, and a
...
$2.5$.  The other figures and the external digit comparison come from
that log alone.  They are descriptive reports, not certificates of
...
The exceptional row $r=7$ and the crossing $(s,d)=(10,7)$ are likewise
statements about the tested ranges only: reset rows $6\le r\le200{,}000$
and rows $s\le2500$ (\href{https://github.com/wcook04/plectis-erdos/blob/f757115e03bebe5c04431a147da559015c680a37/research/experiments/erdos257/receipts/seam-reset-crossing-2500.json}{saved run}).  No claim that this is
the only exceptional row of the infinite orbit is made.

These representations all descend from the same greedy carry;
they are not independent tests of a common random hypothesis.  Strict
tail domination gives an injective coding, but says neither that its
...
certificate method first meets a parity obstruction that depends on the choice of support.
...
without changing irrationality.  Thus the obstruction concerns this method; it
does not concern the value or the universal problem.
...
bounds the length of a successful one.
...
Instantiating the present block-certificate method at a general support is blocked, on the
...
all probabilistic, measure-theoretic or averaging methods.
% ; - part a257_p5 ; -
\clearpage
...
have only finite computational checks.  This section states the exact open targets, their Lean
sources where formalised, and their logical relationships.  The implication table in
\S\ref{sec:v-table} shows that several apparent intermediate steps are \emph{equivalent} to the
full problem.

\subsection{Notation fixed for this section}

The base-$2$ support series is
...
coefficient whose generating tail enters every certificate below.
$\mathcal{A}=\{X_A(2):A\subseteq\mathbb N_{\ge1}\}$
(\lean{mersenneAchievementSet}{GreedyAchievementSet.lean:571}).
$G$ is the greedy support for target $1/2$;
...
\]
\end{defn}
This is the literal statement of Erd\H{o}s \#257; it is open.  The sufficient condition used by
the certificate method of this record is
\[
...
the theorem is universally quantified over $A$, and only its hypothesis is missing).  Therefore the stronger statement
\[
...
This is a limit of the method, not a counterexample to irrationality:
Duverney--Tachiya prove the squarefree value irrational at every
power-of-two base.  Likewise the pairwise-coprime theorem does not cover the
primes because $\sum_p1/p$ diverges, even though Tao--Ter\"av\"ainen prove
the base-$2$ prime-support value irrational.  The universal target is
...
(\lean{finite_boolSupport_ne_half}{HalfCarryReachability.lean:589};
...
different target $t\in\mathbb{Q}$.  $\mathrm{HALF}$ is open.  It is logically equivalent, by an
unconditional Lean theorem, to a purely combinatorial statement about one fixed orbit:
\[
...
the reverse direction holds for every target, and the forward direction uses only that the target
is rational, so the same theorem holds verbatim for every rational $t\ge 0$ in place of $1/2$).
...
(an odd-denominator parity fact), hence deletable without changing
...
\]
This is the exact terminal-strip condition.
...
\end{prop}
The existence condition is therefore not logically weaker than half-membership.
...
\[
 \sum_{d\in D}\left\lfloor\frac{2^n}{2^d-1}\right\rfloor
       =2^{n-1}-1.
\]
(\lean{CofinalExactLocalMersenneHalfRows}{BooleanMobiusCofinalExactRows.lean:38}).  This
condition is unproved.  Supports at different depths need not agree.
This is the condition in Definition~\ref{record:257bm-d4};
no strict weakening of half-membership is asserted.
\end{defn}
The implication from this condition to $\mathrm{HALF}$ is proved unconditionally
via compactness of $\mathcal{A}$ (a continuous image of the space of $0/1$ sequences under the
digit-coding map, hence closed)
(\lean{half_mem_mersenneAchievementSet_of_cofinalExactLocalRows}{BooleanMobiusCofinalExactRows.lean:71}).
The cited theorem already proves that a representing support exists,
...
This describes an ordinary existence proof; no additional
...
this condition as logically equivalent to $1/2\in\mathcal{A}$.
...
extraction theorem does not make this condition a strictly weaker substitute for $\mathrm{HALF}$.
The remaining work here is to supply exact rows at unbounded depths, or any equivalent
...
assumes $(\ast\ast)$ for a below-half core
...
The complete induction from the endpoint-six seed to exact sums at unbounded depths is already
formalised
(\lean{cofinalExactLocalMersenneHalfRows_of_criticalQuotientSupply}{BooleanMobiusCriticalCapacityCofinal.lean:1309});
it uses the sharp capacity input, in its reduced canonical form
\[
...
\]
(\lean{HalfGreedySkippedCriticalQuotientSupply}{BooleanMobiusCriticalCapacityCofinal.lean:178}),
at essentially every step; this input is unproved.  This condition is required along the specified greedy prefixes.  That
...
$B(r)=2^{\lfloor(r+4)/2\rfloor}+2r+3$ described in Theorem~\ref{thm:lower-bound-every-reset}:
...
the first is proved in Theorem~\ref{thm:lower-bound-every-reset}.
...
\emph{Finite evidence.} Exact integer arithmetic to row $200{,}000$ shows that the
target inequality holds at every reset row $6\le r\le200{,}000$ except $r=7$; the maximum observed R-run is $19$, after
the reset at row $158{,}096$, against a required threshold there of $\approx79{,}046$, a margin of about
$4000$ times.  This is a finite verified list; it supplies no witnesses at unbounded depths.
...
grows from $1.11$ bits at $r=14$ to $\sim1246$ bits by row $2500$ (exact computation). Proving the sign
...
shows the failure mode is a tower of exact integer coincidences, not drift; it is an ordinary
argument, and it does \emph{not} by itself exclude the two pinned values; an isolated dangerous reset after a
...
\begin{defn}
The \emph{two-sided cell condition} excludes three integer values of the middle coordinate,
...
\end{defn}
Granted this, induction from a base case at row $5$, checked by direct computation, propagates
$\forall s\ge5,\ \mathrm{rem}(s)\le2^s\vee\mathrm{overshoot}(s)\le2^s$
(\lean{SeamTwoSidedDyadicCellEscape.twoSided}{HalfCylinderMiddleCarryLowerBound.lean:4448},
given the hypothesis). One of the three cells, $-3$, is already
closed unconditionally in the all-right-tail configuration
(\lean{finalMiddleCell_neg_three_not_last}{HalfCylinderFinalMiddleCellEscape.lean:587}).
The two remaining cells, $-2$ and $-1$, are not excluded: what would complete this route is their
exclusion for the
...
\[
 ...
 E_d+2(d+j)\le2^{d-j+1}.
\]
This is the upper-reset band condition of Theorem~\ref{thm:upper-reset-band}
(\lean{SeamUpperResetDyadicBandEscape}{HalfCylinderMiddleCarryLowerBound.lean:4566}).  It is
unproved in general, and verified by exact finite computation for rows $13\le d\le30$
(\lean{seamUpperResetDyadicBandEscape_through_thirty}{HalfCylinderUpperResetBandCertificates.lean:78}).
...
weakening.  The linear-width condition suffices for half-membership
(\lean{half_mem_mersenneAchievementSet_of_upperResetDyadicBandEscape}{HalfCylinderMiddleCarryLowerBound.lean:4790}).
It is proved logically equivalent to a single-critical-index check per row
(\lean{seamUpperResetCriticalBandEscape_iff}{HalfUpperResetCriticalBand.lean:883}), which is a genuine
...
\]
(\lean{one\_div\_twenty\_one\_mem\_iff\_not\_fatalAlignedBranch}{TwentyOneQuotientGreedy.lean:3507}).
The fatal branch $\mathcal F_{21}$ means a finite fatal greedy gap followed
...
$\Leftrightarrow$ records an equivalence.  Lean proofs and ordinary
arguments are distinguished as in the text.  ``No implication stated'' means that
...
Infinitely many greedy skips (Proposition~\ref{prop:cpgs-equiv}) & $\Leftrightarrow$ half-membership & open, equivalent condition \\
Terminal-only strip (Prop.~\ref{prop:strip-equiv}) & $\Leftrightarrow$ half-membership & open, equivalent condition \\
The centred square-root bound for $G$ & $\Leftrightarrow$ half-membership & open, equivalent condition \\
Exact finite sums at unbounded depths & $\Leftrightarrow$ half-membership (Prop.~\ref{prop:collapsed-list}(c)); Lean proves $\Rightarrow$ & open, equivalent condition \\
...
\textbf{The universal statement.} The question is $\mathrm{U257}$ itself. The stronger schema
...
digitwise nor the carry-aware block certificate exists, while Duverney--Tachiya still
prove that squarefree value irrational. Supplying $\mathrm{Cert}(A)$ for arbitrary infinite
supports is therefore not an open equivalent normal form of $\mathrm{U257}$.  The remaining work
is the universal irrationality statement, or a different method that covers supports the
certificate schemas cannot.

\textbf{Half-membership, exact sums and middle cells.} The sharp-capacity gap $(\ast)\to(\ast\ast)$ and the
...
\textbf{Reset separation.} The specified quotient-row argument
...
$\mathrm{U257}$ or the two rational membership targets $1/2$ and
$1/21$.  The conditions surveyed here have different logical roles.
Those equivalent to half-membership, including exact sums at unbounded
depths by Proposition~\ref{prop:collapsed-list}(c), do not establish
membership merely by reformulating it.  The sufficient conditions (the
reset bound, the remaining middle-cell exclusions and the capacity band)
have not been established at unboundedly many indices, and
no converse is proved for them.  The proposed universal certificate
...
initial-interval estimate here; the short paper records their consequences
beside its bound for the family $F(q,P)$.

The proofs in this section are ordinary arguments, first worked out in
an AI-assisted note of 17 September 2026 and given here in full.  All
three results also have Lean proofs, each compared independently
(margin); the ordinary proofs have not had independent human review.
...
\eqref{eq:257-mixed-finite-kernel}.  The short paper's logarithmic lower bound
and its bound for $F(q,P)$ concern the same fractional-cover cost; their
...
the single set $F$ with exponent one.  This is the fractional cost of
Theorem~\ref{thm:257-variable-fractional-cover},
...
arithmetic restriction.  An irrationality criterion with logarithmic
divisor costs in place of fractional ones therefore does not follow from
this proof; it is not disproved either.
...
finite-period noncollapse results are not affected.  No conclusion here
decides universal \#257 or membership of $1/2$ or $1/21$.

\clearpage
\section*{Two proof details and sources}
...
Their theorem formulations used here were read in the author-written
overview; the two original publisher PDFs were not retrieved.  Van Assche's nonvanishing and
...
\paragraph{Artefact and data availability.} The Lean declarations, the fixed
toolchain and the library versions are given in the evidence record linked in the
introduction.

\paragraph{Scope of the formal checks.} Kernel checking establishes that a proposition
was proved; it does not validate the exposition or the literature judgements, and it
does not solve Problem 257.
...
\bibitem{sedaghatstephenchindelevitch} N.~Sedaghat, T.~Stephen, and
  L.~Chindelevitch, \href{https://doi.org/10.4230/LIPIcs.SEA.2018.6}
  {\emph{Speeding up dualization in the Fredman--Khachiyan Algorithm B}},
  17th International Symposium on Experimental Algorithms (SEA 2018),
  LIPIcs 103 (2018), 6:1--6:13.
