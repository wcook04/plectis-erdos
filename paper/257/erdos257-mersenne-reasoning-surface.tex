...
The following finite checks are reported from earlier computations.
...
not rerun for this record.  The quotient calculation through row
...
A further exact finite computation, unpublished and not relied on here,
reaches depth $13{,}548{,}057$.  The reset rows $13$--$30$ are also
...
The following values and finite ranges are reported from earlier
...
at $n=14,15,32$ were reproduced by exact integer arithmetic for this
record. The larger computation ranges remain reports from the earlier
computations; they were not rerun here.
...
certified to row $2500$ (and separately, in a longer single run of $207.7$ seconds, to row
$200{,}000$: branch counts $\mathrm{R{:}M{:}U} =
100197{:}49899{:}49898$, the target inequality failing at exactly one reset row $\le 200{,}000$,
namely $r=7$).
\end{obs}
...
The large computation ranges in this section are taken from the earlier
computations named above and were not rerun here.
% ; - part a257_p1a ; -
...
ordinary argument with exact computation, from an unpublished working note.
...
eventual bad-rank obstruction closes only this route (ordinary argument, from an
...
$200{,}000$, with exactly one violation of the target inequality in that range, at $r=7$
(both from unpublished working notes).
...
logarithms of the corresponding positive slack multiplied by $4^k$ (from an unpublished
working note).
\end{itemize}
...
$B(20)=B(21)=B(22)=1530$.
The certificate data and an independent verifier accompany that
computation; they are not reproduced here.  These are finite
...
cases.  An earlier computation reports a maximum run of length
...
for this record.  They are descriptive reports, not certificates of
...
target inequality holds at every reset row except $r=7$; the maximum observed R-run is $19$, at
...
They have not had independent review.
