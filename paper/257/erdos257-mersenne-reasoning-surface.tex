...
has a
\href{https://github.com/wcook04/plectis-erdos/blob/065e09523286894dfb57ba205e69666843817009/lean/Erdos249257/AllBaseReciprocalSupportIrrationality.lean#L395}{kernel-checked Lean proof}.
...
The argument displayed here is an ordinary proof.  Lean sources exist for
\href{https://github.com/wcook04/plectis-erdos-lean/blob/52f29ad173b04e3bac941b3663f2b9aebe5de0bb/ErdosProblems/Erdos257/PaperCompleteR8/DyadicKernel.lean#L207}{the full estimate}
and for
\href{https://github.com/wcook04/plectis-erdos-lean/blob/52f29ad173b04e3bac941b3663f2b9aebe5de0bb/ErdosProblems/Erdos257/PaperCompleteR7/CoverKernel.lean#L130}{its complete-period and no-wrap ingredients};
no new compilation of them is claimed here.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f3f54798508b29ded6e854e0fa5784a560bd6cc7/lean/ErdosProblems/Erdos257/PaperCompleteR8/ArbitraryWeightMixedClaim.lean#L19}{kernel-checked Lean proof}.
Theorem~\ref{thm:257-mixed-supports} does not establish strict enlargement
over both individual classes.  A Lean source
...
a support with such a cover but no finite weighted mass is not constructed
here.  The theorem also leaves universal irrationality and the quantitative
...
\textbf{A probabilistic heuristic.}
An earlier heuristic calculation reports an expected count of about
...
The cited proof uses the tail budget in one step.  Deleting that assumption
...
An analytic supplement that is not available here claims a second
criterion admitting reciprocal-divergent supports.
...
nor the claimed sharper rational-phase lower bound is available here.
...
\item \emph{Band avoidance at resets, already proved to imply membership.}
  For every reset $d \ge 13$ whose successor carries satisfy the hypotheses
  of Section~\ref{sec:o4} and every $j \le d$, the reset charge
...
this certificate theorem, not a boundary of the mathematics.  The previously
proposed windowed-tail repair is therefore withdrawn as a research
contribution.  It may remain useful as a test of whether the existing Lean
certificate theorem can reproduce the known theorem, but reproducing a cited
...
Lean sources give the corresponding
...
they were inspected for this proof, and no new compilation of them is
claimed.
\end{proof}
...
at $n=14,15,32$ are exact integer values. Each larger range has a
...
row $d+1$. The Lean proof evaluates these values directly in the
kernel; it was not recompiled for this record.
\end{obs}
...
A Lean source gives the conditional implication; the run-length
...
100197\!:\!49899\!:\!49898$; for $r\ge6$ the obligation \emph{fails only at} $r=7$; maximum R-run $19$ at row
...
an infinite statement from a long finite run.  The certificates accompany
that computation.
...
hold at every precision and is unproved.  Despite the name of the source file, it contains no
dichotomy theorem.
...
argument is supposed to give. For the class: the assertion that every infinite $A$ admits a
...
The first conditional implication also has a Lean source, identified
below.  The other two entries are
...
\]
The finite recomputation for this record verifies these signs
...
\end{tabular}
\end{center}
These counts were reproduced for this record; the experiment
...
small expected counts reported in the earlier record.  The counts are
...
$w_{s+1}=4w_s-4-p_s$.
The same middle and upper deviation identities appear in a
\href{https://github.com/wcook04/plectis-erdos-lean/blob/52f29ad173b04e3bac941b3663f2b9aebe5de0bb/Erdos257PeriodNoncollapse/HalfResetSqrtEscapeScaleProducers.lean#L79-L144}{Lean source},
which was not recompiled for this record.  They separate the
...
all probabilistic, measure-theoretic or averaging methods.  Each recorded obstruction, unlike a
failed attempt, can be reused; that is what this part contributes.
% ; - part a257_p5 ; -
\clearpage
...
\]
\end{prop}
The proof is an ordinary argument.  The cited implication
...
\end{prop}
The proof is an ordinary argument.  This corrects the claim that the existence condition is logically weaker.
...
target inequality holds at every reset row $6\le r\le200{,}000$ except $r=7$; the maximum observed R-run is $19$, at
row $158{,}096$, against a required threshold there of $\approx79{,}046$, a margin of about
...
(O3-supply)) have not been established at unboundedly many indices, and
...
an AI-assisted note of 17 September 2026 and given here in full; they
have not had independent review.
