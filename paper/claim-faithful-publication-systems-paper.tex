...
claim stronger. Among the systems compared here, none implements this full
route from a question to the strongest public claim its evidence supports.
...
well. Comparator checks selected propositions and their axiom boundary;
Palomar selects result families for review. Neither tool decides novelty,
importance, acceptance, or whether an open problem has been solved.
...
status of a mathematical statement. Among the systems compared in this paper,
none spans this complete chain. This is a
bounded claim over the cited systems, not a proof of universal priority or an
...
maps, Lean gates, Comparator, Palomar records, and release checks are
...
\centering
\begin{tikzpicture}[
  box/.style={draw=gray!58, rounded corners=3pt, align=center, fill=white,
    text width=2.15cm, minimum height=1.28cm, inner sep=5pt},
  ...
  back/.style={-{Stealth[length=2.2mm]}, densely dashed, draw=Violet!75!black},
  lab/.style={font=\tiny\sffamily\bfseries, text=gray!70!black}
]
\node[private] (intent) at (0,0) {\textbf{Intent and route}\\[-1pt]\scriptsize question, scope, work claim};
\node[private, right=4mm of intent] (reason) {\textbf{Reason and test}\\[-1pt]\scriptsize candidates, experiments, dead ends};
\node[formal, right=4mm of reason] (lean) {\textbf{Formalise}\\[-1pt]\scriptsize statement, proof, kernel check};
\node[public, right=4mm of lean] (claim) {\textbf{Explain}\\[-1pt]\scriptsize graph, claim, paper, limits};
\node[assure, right=4mm of claim] (review) {\textbf{Assure and release}\\[-1pt]\scriptsize Comparator, Palomar, checks};
\draw[arr] (intent) -- (reason);
\draw[arr] (reason) -- (lean);
\draw[arr] (lean) -- (claim);
\draw[arr] (claim) -- (review);
\draw[back] (review.south) to[out=-115,in=-65,looseness=1.15]
  node[lab, below=4pt] {review, failures, and reusable lessons return; authority does not} (reason.south);
...
\draw[densely dashed, gray!60] ($(reason.east)!0.5!(lean.west)+(0,-8mm)$) --
  ($(reason.east)!0.5!(lean.west)+(0,12mm)$);
\end{tikzpicture}
\caption{The end-to-end architecture. The dashed vertical line is a release
boundary, not a network connection: the public repository must remain usable
without the private workbench.}
\label{fig:lifecycle}
\end{figure}
...
\centering
\begin{tikzpicture}[
  job/.style={draw=gray!58, rounded corners=2.5pt, align=center, fill=white,
    text width=2.35cm, minimum height=1.02cm, inner sep=4pt},
  arr/.style={-{Stealth[length=2.1mm]}, semithick, draw=gray!78!black}
]
\node[job] (invoke) at (0,0) {\textbf{Invoke}\\[-1pt]\scriptsize choose a bounded route};
\node[job, right=4mm of invoke] (work) {\textbf{Work}\\[-1pt]\scriptsize continue or suspend};
\node[job, right=4mm of work] (validate) {\textbf{Validate}\\[-1pt]\scriptsize computation or Lean};
\node[job, right=4mm of validate] (propagate) {\textbf{Propagate}\\[-1pt]\scriptsize classify every consumer};
\node[job, below=7mm of propagate] (package) {\textbf{Package}\\[-1pt]\scriptsize evidence and provenance};
\node[job, left=4mm of package] (propose) {\textbf{Propose}\\[-1pt]\scriptsize pull request or issue};
\node[job, left=4mm of propose] (adopt) {\textbf{Review and adopt}\\[-1pt]\scriptsize reconcile, accept, credit};
...
and legal next actions. A reader can then drill from a one-line flag to a short
card, a working context, and finally source evidence. This ``small map before
large source'' rule reduces blind search without pretending that a summary is
proof.
...
The router is not merely a prompt template. It projects a typed option surface
over skills, standards, paper modules, live work, and source artefacts, then
...
\centering
\begin{tikzpicture}[
  role/.style={draw=gray!58, rounded corners=3pt, align=center, fill=white,
    text width=3.05cm, minimum height=1.25cm, inner sep=5pt},
  object/.style={role, fill=TealBlue!6, draw=TealBlue!60!black,
    text width=2.72cm},
  output/.style={role, fill=ForestGreen!6, draw=ForestGreen!55!black,
    text width=3.15cm},
  arr/.style={-{Stealth[length=2.2mm]}, semithick, draw=gray!78!black},
  feedback/.style={-{Stealth[length=2.2mm]}, densely dashed,
    draw=Violet!75!black}
]
\node[role, fill=MidnightBlue!5, draw=MidnightBlue!55] (mine) at (0,0)
  {\textbf{Discovery goal}\\[-1pt]\scriptsize read, compute, conjecture, prove, falsify};
\node[object, right=7mm of mine] (delta)
  {\textbf{Landed delta}\\[-1pt]\scriptsize theorem, no-go, computation, boundary};
\node[role, fill=BurntOrange!7, draw=BurntOrange!65!black,
  right=7mm of delta] (steward)
  {\textbf{Stewardship goal}\\[-1pt]\scriptsize appraise, compose, reconcile, explain};
\node[output, below=9mm of delta] (surfaces)
  {\textbf{Current public}\\[-1pt]\textbf{picture}\\[-1pt]\scriptsize paper order, Comparator, Palomar, frontier};
\draw[arr] (mine) -- (delta);
...
The working memory is federated rather than a copied mega-index. A compact
descriptor carries separate identities for proof-bearing source, navigation
generation, content digests, and source-input fingerprints. An attached
exhaustive atlas is opened only after those identities reconcile and the
bounded route cannot answer. Retrieval can move between global frontier
structure, concept neighbourhoods, and premise or obligation ancestry. Exact
imports, authored arguments, generated navigation, lexical references, and
semantic inference remain visibly different edge classes.
...
Comparator's roster is an evidence inventory, not a ranking. Palomar adds the
missing editorial layer. It groups declarations into result families, ranks
them by mathematical signal, keeps the hard mechanism and surviving boundary
adjacent, and selects a small review portfolio. A qualification check can say
that the local packet satisfies its structural requirements. Submission,
registration, acceptance, publication, and community uptake remain separate
events.

This separation matters under proof abundance. Counting theorems rewards
generated volume and routine closure. Palomar instead asks which result changes
...
\subsection{Applying Tao's proof-abundance programme}
...
claim records support exposition. Palomar triages scarce expert attention, and
...
value. In this sense the repository is not merely consistent with Tao's
recommendations. It is an implementation experiment in how a small project can
...
footnotes to be discarded after the repair. This is a coverage boundary, not a
reliability score. The post-repair witness accepts the current README and
...
nearest stronger open statement. Palomar can allocate scarce expert attention
to those packets; Comparator can protect selected exact interfaces; neither is
replaced by scale.
...
authority mode, rather than merely by its command string or working directory.
...
Palomar-style ranking governs the fourth. Fan-in joins their outputs only after
...
transfer across unrelated mathematical domains, not only across neighbouring
...
reviewed claims, Comparator interfaces, Palomar selection, and release
...
until an explicit fan-in gate. Together these make a claim-transition operating
system rather than merely an agent wrapper around a prover.
...
Comparator checks exact statement interfaces. Palomar directs scarce review
attention. The public repository then packages source, claims, papers, and
...
The public repository is the reproducible object for this paper. A fresh clone
can inspect the architecture guide, query the paper and problem corpus, build
the Lean roots, check the Comparator packet, inspect the Palomar qualification
surface, and run the release checks. The exact commands and current file
identities live in \path{ARCHITECTURE.md}, \path{docs/publication_contract.json},
and the generated query routes; volatile counts are intentionally omitted here.

The worked-example evidence is recorded in
