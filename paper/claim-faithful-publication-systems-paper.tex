...
context budget. Among the systems compared here, none implements this full
...
\centering
\begin{tikzpicture}[
  box/.style={paper stage, text width=2.3cm, minimum height=24mm, inner xsep=4pt},
...
% Five equal stages in one row.  Two tinted bands behind them mark what is
% private and what ships; the dashed seam between the bands is the release
% boundary.  Information returns along one rectilinear rail beneath the
% bands; authority only ever moves along the top row.
\node[private] (intent) at (0,0) {\papereyebrow{1 \; SELECT}\\[3pt]
  \textbf{Intent and route}\\[2pt]\scriptsize question, scope, claim};
\node[private] (reason) at (2.75,0) {\papereyebrow{2 \; TEST}\\[3pt]
  \textbf{Reason and test}\\[2pt]\scriptsize candidates and failures};
\node[formal] (lean) at (5.5,0) {\papereyebrow{3 \; CHECK}\\[3pt]
  \textbf{Formalise}\\[2pt]\scriptsize proof and kernel verdict};
\node[public] (claim) at (8.25,0) {\papereyebrow{4 \; EXPLAIN}\\[3pt]
  \textbf{Public claim}\\[2pt]\scriptsize result, evidence, limit};
\node[assure] (review) at (11,0) {\papereyebrow{5 \; RELEASE}\\[3pt]
  \textbf{Assure}\\[2pt]\scriptsize replay and boundary checks};
\draw[arr] (intent) -- (reason);
\draw[arr] (reason) -- (lean);
\draw[arr] (lean) -- (claim);
\draw[arr] (claim) -- (review);
% Bands and their names, on the background layer.
\begin{pgfonlayer}{paperbackground}
  \fill[paper band, fill=MidnightBlue!7]
    ([xshift=-3mm,yshift=-3.5mm]intent.south west) rectangle
    ([xshift=2.6mm,yshift=9.5mm]reason.north east);
  \fill[paper band, fill=ForestGreen!7]
    ([xshift=-2.6mm,yshift=-3.5mm]lean.south west) rectangle
    ([xshift=3mm,yshift=9.5mm]review.north east);
\end{pgfonlayer}
\node[paper band label, text=MidnightBlue!75!black]
  at ($(intent.north)!0.5!(reason.north)+(0,6.3mm)$) {PRIVATE WORKBENCH};
\node[paper band label, text=ForestGreen!55!black]
  at ($(lean.north)!0.5!(review.north)+(0,6.3mm)$) {SELF-CONTAINED PUBLIC ARTEFACT};
\coordinate (seam) at ($(reason.east)!0.5!(lean.west)$);
\draw[paper boundary, draw=gray!75]
  ([yshift=-5.5mm]seam |- intent.south) -- ([yshift=11.5mm]seam |- intent.north);
\node[paper label, above] at ([yshift=11.5mm]seam |- intent.north) {release boundary};
% Return rail.
\coordinate (rail) at (0,-1.75);
\draw[back, rounded corners=3pt] (review.south) -- (review.south |- rail)
  -- node[paper label, below, pos=.5]
     {failures and review return as evidence; authority stays put}
  (reason.south |- rail) -- (reason.south);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
  title/.style={font=\small\bfseries, align=center, text width=2.1cm,
                paper nohyphen, anchor=south, inner sep=0pt},
  note/.style={font=\scriptsize, align=center, text width=2.05cm,
               paper nohyphen, anchor=north, inner sep=0pt},
  station/.style={paper step dot}
]
% Seven stations on one timeline, read left to right without a row break.
% Station colours follow the earlier grouping: research (1--2), validation
% and propagation (3--4), packaging and proposal (5--6), maintainer adoption.
\draw[paper spine] (-0.3,0) -- (13.3,0);
\foreach \i/\x/\c/\t/\n in {
  1/0.35/MidnightBlue/Invoke/bounded route,
  2/2.4/MidnightBlue/Work/continue or suspend,
  3/4.45/TealBlue/Validate/computation or Lean,
  4/6.5/TealBlue/Propagate/classify consequences,
  5/8.55/ForestGreen/Package/evidence and provenance,
  6/10.6/ForestGreen/Propose/pull request or issue,
  7/12.65/BurntOrange/Review and adopt/reconcile{,} accept{,} credit}{
  \node[station, fill=\c!9, draw=\c!60!black, text=\c!75!black] (s\i) at (\x,0) {\i};
  \node[title] at ([yshift=3.2mm]s\i.north) {\t};
  \node[note] at ([yshift=-3.2mm]s\i.south) {\n};
}
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
  role/.style={paper stage, text width=2.9cm, minimum height=3.0cm},
  object/.style={paper stage, text width=4.3cm, minimum height=13mm,
    inner ysep=4pt},
  arr/.style={paper arrow},
  feedback/.style={paper feedback}
]
% A loop with two roles on the sides and the two objects they exchange on
% the top and bottom edges.  The top edge is the hand-off that wakes
% stewardship; the bottom edge is the appraisal that wakes discovery.
\node[role, fill=MidnightBlue!5, draw=MidnightBlue!55] (mine) at (0,0)
  {\papereyebrow{DISCOVERY}\\[3pt]\textbf{Test the frontier}\\[2pt]
   \scriptsize compute, prove, or falsify};
\node[role, fill=BurntOrange!7, draw=BurntOrange!65!black] (steward) at (9.6,0)
  {\papereyebrow{STEWARDSHIP}\\[3pt]\textbf{Place the result}\\[2pt]
   \scriptsize appraise, reconcile, explain};
\node[object, fill=TealBlue!6, draw=TealBlue!60!black] (delta) at (4.8,0.8)
  {\papereyebrow{HANDOFF}\\[2pt]\textbf{Landed delta}\\[1pt]
   \scriptsize theorem, no-go, computation};
\node[object, fill=ForestGreen!6, draw=ForestGreen!55!black] (surfaces) at (4.8,-0.8)
  {\papereyebrow{PUBLIC SURFACE}\\[2pt]\textbf{Current mathematical picture}\\[1pt]
   \scriptsize paper order, evidence, frontier};
\draw[arr] (mine.east |- delta) -- (delta.west);
\draw[arr] (delta.east) -- (steward.west |- delta);
\draw[feedback] (steward.west |- surfaces) -- (surfaces.east);
\draw[feedback] (surfaces.west) -- (mine.east |- surfaces);
\node[paper label, above=1.5mm of delta] {a landed delta wakes stewardship};
\node[paper label, below=1.5mm of surfaces, text width=6.2cm, align=center]
  {a ranked next question, missing evidence, or a discriminating test
   wakes discovery};
\end{tikzpicture}
