...
problems, all still open. The implementation and author-operated checks are
available; whether this organisation improves research productivity has yet
to be tested with outside contributors.
\end{abstract}
...
those recorded relationships when files change. We call this a
\emph{claim-transition architecture}; the phrase names this connection between
an attempted result, its evidence, and its published statement.
...
All eight problems remain open. This paper does not claim a solution to any of
the eight problems or a measured improvement in discovery rate. Its evidence
is the implementation and the worked cases below. Independent contributor use
and gains in research productivity remain questions for evaluation.
...
The private packet builder prepares for two ways of reading. A browser-only
...
The working-memory compiler selects a task-specific reading packet before an
agent attempts the mathematics. Each attached corpus retains its own index;
nothing is copied into a central index. Compact descriptors and content
fingerprints identify the sources. The compiler opens a larger atlas when the
bounded query does not supply enough context.

The packet starts with the target statement, established results, and missing
premises. It distinguishes formal dependencies from authored interpretations
and suggested connections. Its inference workspace groups premises into
candidate arguments and records tests or counterexamples that could reject
them. A freshness check compares the recorded fingerprints with current
sources and requests recompilation when they differ. The workspace plans
inference and concludes nothing.

A request that names no theorem, declaration, or claim is reported as unanchored
before a proof route is proposed. If a packet exceeds its context budget, the
compiler reports the requested and estimated size, the omitted material, and
how to retrieve it; it exits non-zero rather than silently truncating. These
checks make the selected context inspectable. Whether it leads to better
mathematics requires the comparison proposed in Section~\ref{sec:related}.
...
CI-gated proof work \cite{leanmarathon}. This directly overlaps the durability
and statement-fidelity concerns here. DreamProver learns a compact reusable lemma library
...
fidelity; HEP already records hypotheses and refutations. These overlaps make
component-level comparison more informative than a claim to cover the most
stages. The evidence reported here is a design and worked reconstruction from
one evolving corpus, not a controlled comparison of research productivity.
