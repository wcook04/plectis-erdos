...
files in a public Git repository that keeps one research problem together with
its established results, failed routes, experiments, literature and exact open
...
eight Erd\H{o}s problems this way. Its results include a kernel-checked
...
proving Lean theorems, shows how the two designs compose. The paper closes with
the controlled comparison that will measure whether such records reduce the
work of understanding, checking and continuing a result.
\end{abstract}
...
Each of these concerns what a proof does not carry by itself. A proof found in a
session arrives without the session: the routes that failed, the computations
that chose the route, the literature it relied on and the precise statement that
remains open. Whoever continues, a person or a model, must rebuild that record or
work without it.
...
work, the \emph{problem-sized world}: one research problem together with
everything learned about it. A world is written for two readers at once. Its short paper
...
with credit (Section~\ref{sec:cycle}); and a public instance on eight
...
organised around one research question. It keeps its established results and
remaining obligations as the question is refined or resolved.
Table~\ref{tab:world} lists its parts.
...
hypotheses. The last two mark routes that later work need not repeat, and a
no-go theorem shows that a proof of the endpoint must leave the excluded class.
For Problem 257, a proposed estimate said that the greedy test
...
permitted axioms? & Comparator and the Lean kernel, with nanoda as a second
kernel in the corpus replay & The Comparator ledger and evidence records \\
\addlinespace
...
change has evidence and every consequence has a recorded disposition; a problem
closes only when a proof or a refutation settles its original statement.}
\label{tab:cycle}
\end{table}
...
synthesis paper and three systems papers. The papers assert 674 results,
counting a result stated in both a short paper and a long record twice. Of
these, 671 carry a Lean mark: 506 link a declaration that proves the result
exactly, 140 one that proves a stronger form and 25 one that proves it from a
named input. Comparator has compared 633, and 13 more are queued for its next
replay; every receipt in the pinned corpus replay was accepted by both the Lean
kernel and nanoda. The claim registry holds 150 public claims in seven
statuses, from proved here and formalised here to conditional reduction and
open, and states 19 remaining obligations exactly, one of them the Problem 1041
correspondence review. Continuous integration builds
...
OpenAI's GPT models, concurrently on one shared file
...
\paragraph{Proof-state continuation.} The public clone carries the validation
queue and a \repolink{docs/agents/PROOF_STATE_COMPILER.md}{proof-state
compiler}. Static queries nominate declarations that might close a goal; the
compiler asks the pinned Lean environment what each application does, keeps the
subgoals Lean leaves as the exact remaining obligation, and records a proof only
when none remain. In its pilot, applying a Problem 249 totient-tail lemma without
its divisibility hypothesis leaves exactly that hypothesis open, and supplying
...
record and its checks can pass every structural check. By 24 September 2026 no
outside human contributor had opened a pull request or issue in the repository;
its outside
...
comparative. With the source revision, tasks, model and budget held fixed,
agents receive the plain repository, an information-equivalent written briefing,
or the world with its continuation tools. Independent domain readers grade
whether each recovers the strongest supported claim, its hypotheses, its
attribution and the remaining obligation, and review one returned change, with
scope errors, reviewer time and total work, maintenance included, recorded.
Repeating the frozen tasks with a stronger model measures what the record adds
as models improve.
...
public repository show that the record can be kept persistent,
