...
checking. At the audited revision, a cold clone exposes a public proof cockpit
that composes checkout and toolchain identity, corpus scale, exact open
propositions, problem obligations, and committed workbench receipts. Beneath
that card sit a bounded tour card, a mathematical map of ten thematic
programmes across eight indexed Erd\H{o}s problems, a
reviewed public claim map with explicit open propositions, and an
elaborated loaded-root reference graph joined to source coordinates. These are
committed navigation products: they require no Lean build and make omissions
and authority boundaries explicit.
...
probes, and claims. The notary derives probe verdicts from the pinned Lean
process; an agent cannot supply them through its interface. Claims must cite
an accepted probe, and replay reruns
...
repeating a full environment export. A recorded case study contains six reasoning
notes, one accepted probe, and two claims, and its probe replays at the audited
revision.

The architecture combines five functions without confusing their roles:
complete inventory, selective interpretation, short intent-based routes,
recorded reasoning, and incremental validation. It is not an autonomous theorem
prover, a portability study, or evidence that the agent's reasoning was optimal
or mathematically novel outside the repository history. It is a prototype
audited in the author's environment. As of 14 September 2026, no completed
external cold-clone use had been recorded.
\end{abstract}

\begin{paperresult}{What this paper establishes}
\textbf{Result.} A cold clone exposes the library's complete declaration
inventory, reviewed mathematical routes, public claim boundaries, and exact
source links before Lean is built. The notary derives accepted proof-probe
verdicts from pinned Lean execution and can replay the stored bytes.
\textbf{Evidence.}
The audited repository contains 1,019 modules and 153,253 declarations; the
paper includes a replayed session case study.  \textbf{Limit.} This is a
single-environment prototype with no recorded external cold-clone use.
\end{paperresult}

\section{The cold-clone problem}\label{sec:problem}
\papersectiontarget{cold-clone-problem}
...
Poor first-contact navigation changes the mathematics an agent attempts. An
agent that cannot see a library's option surface may rediscover existing
lemmas, confuse a finite result with its open unbounded neighbour, or build a
new local index because the existing one was not discoverable. Loading every declaration,
...
The \href{\repobase}{development studied here} indexes eight Erd\H{o}s
problem programmes. The ten map entries are thematic routes across these
problems, not ten separately indexed problems. The claim registry now covers
selected statements from all eight; a paper's inclusion does not give every
statement in it formal
status. The degree-seven counterexample due to the erdosproblems.com
contributor
\href{https://www.erdosproblems.com/forum/thread/1041\#post-8861}{\texttt{ani}},
formalised in Lean in this repository, refutes the Formal Conjectures 1041
one-dimensional Hausdorff path-image claim, while correspondence with its
historical curve-length question remains unreviewed and the other seven
targets remain unresolved here.
The detailed navigation examples in this paper concentrate on Problems 249
and 257: exact
...
The paper first separates the repository's four navigation layers and then
shows how a bounded tour expands into exact source and dependency routes.
Sections~\ref{sec:authority} and~\ref{sec:incremental} mark the transition from
navigation to kernel authority and explain the incremental validation path.
Section~\ref{sec:dogfood} gives a replayed case study; the remaining sections
compare related systems and state the limits of transfer.

This paper owns the navigation and validation case study. The wider
\papersectionlink{claim-faithful-publication-systems-paper.pdf}%
{systems-lifecycle}{claim-transition lifecycle} is described in the systems
paper, while the
\papersectionlink{open-source-mathematics-strategy.pdf}%
{strategy-protocol}{contribution protocol} specifies how an outsider returns a
change. These links lead to named destinations in the sibling PDFs rather than
to page numbers.

The same proof-state compiler also supports bounded inspection of a selected
elaborated proof. The existing feedback experiment uses that route to expose
admissible construction choices and emits finite obligations for the same
workbench to replay. Comparator can check the separately stated continuation
contract through a named unit of its existing runner. The implementation,
literature attribution and finite controls are described in the systems
paper's discussion of executable construction choices. These operations
extend the existing research route and do not automatically extract a new
contract or establish an infinite claim from a finite run.

\section{Four views of the mathematical library}\label{sec:layers}
...
\centering
\begin{tikzpicture}[
  wide/.style={paper box, text width=10.7cm},
  branch/.style={paper box, text width=4.75cm, minimum height=2.6cm},
  arr/.style={paper arrow},
  note/.style={paper label}
]
\node[wide, fill=MidnightBlue!5] (atlas) at (0,0)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} SOURCE INVENTORY}\\[3pt]
   {\large\textbf{Exhaustive declaration atlas}}\hfill
   {\small\bfseries\color{MidnightBlue!85}1,019 modules \(\cdot\) 153,253 declarations}\\[3pt]
   \footnotesize Every live declaration has source coordinates, a kind, a
   module role, and a route.};
\node[branch, fill=BurntOrange!6, below=11mm of atlas, xshift=-2.82cm] (semantic)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} AUTHORED MEANING}\\[3pt]
   {\normalsize\textbf{Tiered semantic graph}}\\[4pt]
   {\small\bfseries\color{BurntOrange!80!black}\SemanticAuthoredTheoremLike\ linked theorem-like declarations}\\[4pt]
   \footnotesize Authored interpretations are distinguished from exact
   source-structural families.};
\node[branch, fill=TealBlue!5, below=11mm of atlas, xshift=2.82cm] (dep)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} FORMAL STRUCTURE}\\[3pt]
   {\normalsize\textbf{Loaded-root dependency index}}\\[4pt]
   {\small\bfseries\color{TealBlue!72!black}exact reference edges}\\[4pt]
   \footnotesize Type and value references are joined to source; unresolved
   rows remain explicit.};
\node[wide, fill=ForestGreen!6, below=11mm of $(semantic.south)!0.5!(dep.south)$] (claims)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} PUBLICATION SURFACE}\\[3pt]
   {\large\textbf{Reviewed public claim map}}\hfill
   {\small\bfseries\color{ForestGreen!68!black}101 claims \(\cdot\) 5 open propositions}\\[3pt]
   \footnotesize Public wording records status, evidence, the claim ceiling,
   and the nearest stronger statement.};
\coordinate (fork) at ($(atlas.south)+(0,-4mm)$);
\draw[arr] (atlas.south) -- (fork) -| node[note,pos=.72] {authored selection} (semantic.north);
\draw[arr] (fork) -| node[note,pos=.72] {formal slice} (dep.north);
\draw[arr] (semantic.south) -- ++(0,-5mm) -|
  node[note,pos=.28] {reviewed selection} ([xshift=-2.4cm]claims.north);
\draw[arr] (dep.south) -- ++(0,-5mm) -| ([xshift=2.4cm]claims.north);
\end{tikzpicture}
...
\paragraph{Authored mathematical interpretation.}
...
\SemanticAuthoredTheoremLike\ theorem-like declarations in the source scope
are linked.
...
The declaration count does not measure distinct discoveries, human-authored
proofs, or individually reviewed mathematical interpretations.

\paragraph{Reviewed public claims.}
At the audited revision the claim map selected 101 results for public exposition.
It recorded 37 as proved here, 8 as formalised here, 5 as unconditional progress,
39 as conditional reductions, 7 as verified finite instances, 3 as cited only,
and 2 as open. Five explicit frontier propositions described the stronger
obligations that survived; the current registry is larger. These labels are maintainer-reviewed public meaning,
not outputs inferred by the proof kernel.

\section{A short tour with exact follow-up queries}\label{sec:tour}

The public entry command is
\texttt{python3 scripts/proof\_cockpit.py}.  Its compact card combines the
checkout, pinned toolchain, corpus scale, exact open frontier, problem-local
obligations, recorded workbench outcomes, and the next typed commands.  Every
field comes from the public clone.  No private task ledger, provider trace,
memory, or operator state is required.  The cockpit is an observation and
routing surface: even its \texttt{--check} mode runs public structural checks,
not Lean, and cannot promote a navigation result or workbench note into a
theorem.

The underlying tour command returns a short card rather than a database dump. It
derives the first result to inspect, its contribution class and boundary, the
eight-problem map with its open count, and the next route commands from the committed
...
\section{From navigation to proof authority}\label{sec:authority}
\papersectiontarget{cold-clone-authority}
...
\centering
\begin{tikzpicture}[
  stagebox/.style={paper stage, text width=1.88cm},
...
\node[nav] (clone)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} 1 \; CLONE}\\[4pt]
   \textbf{Cold clone}\\[2pt]\footnotesize committed projections};
\node[nav, right=4mm of clone] (tour)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} 2 \; ROUTE}\\[4pt]
   \textbf{Bounded tour}\\[2pt]\footnotesize intent and scope};
\node[nav, right=4mm of tour] (cone)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} 3 \; FOCUS}\\[4pt]
   \textbf{Proof cone}\\[2pt]\footnotesize exact declaration};
\node[exact, right=4mm of cone] (probe)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} 4 \; VERIFY}\\[4pt]
   \textbf{Lean probe}\\[2pt]\footnotesize kernel verdict};
\node[exact, right=4mm of probe] (claim)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} 5 \; BIND}\\[4pt]
   \textbf{Cited claim}\\[2pt]\footnotesize replay receipt};
\draw[arr] (clone) -- (tour);
\draw[arr] (tour) -- (cone);
\draw[arr] (cone) -- (probe);
\draw[arr] (probe) -- (claim);
\draw[paper boundary] ($(cone.east)!0.5!(probe.west)+(0,-12mm)$) --
  ($(cone.east)!0.5!(probe.west)+(0,13mm)$);
\node[font=\scriptsize\sffamily\bfseries, text=gray!70!black,
  above=14mm of $(cone.east)!0.5!(probe.west)$] {AUTHORITY BOUNDARY};
...
The agent chooses the route; the notary derives the verdict from pinned Lean
execution.}
\label{fig:sequence}
\end{figure}
...
agent cannot supply a verdict through the supported notary interface. A claim
must cite an accepted probe receipt; the notary rejects weaker citations.
Replay reruns every stored
probe and compares the current result with the recorded verdict.

Workflow-provenance systems already record what ran, when and where it ran,
how tasks and data were connected, and who initiated them; they also
distinguish a planned workflow from the trace of an executed one
\cite{workflowprovenance}. That is the closest established role for the
notary's execution metadata. The notary adds a different contract: its ledger
types an agent's conjectures, abandonments, probes, and claims, while the
notary derives probe verdicts from the pinned Lean process. This interface
rule does not prevent an actor with filesystem access from changing the
implementation or stored record. The comparison does not show that the
resulting record improves the agent's research choices.
...
\section{A replayed case study}\label{sec:dogfood}
...
Repository question answering provides a second comparison. Abedu,
Khatoonabadi, and Shihab combine an LLM with a knowledge graph built from
users, commits, issues, and files; their evaluation spans five open-source
repositories and a 20-participant task study~\cite{repokg}. They explicitly
leave code entities and program-analysis edges to future work. The tour here
does not evaluate a conversational interface. It commits code-level Lean
declarations, elaborated dependencies, authored mathematical interpretations,
and claim-status boundaries so that a reader can inspect the route before an
LLM explains it.
...
\section{From a proof receipt to a public return}\label{sec:return}
\papersectiontarget{cold-clone-return-lifecycle}

A proof receipt closes only the validation job. It does not say whether a
claim record, paper, computation, open frontier, query route, or contributor
surface has become stale. The clone therefore continues through an explicit
sequence: invoke a bounded route with \texttt{mine-open-problem}; continue,
suspend, or close the research run at its stated stopping condition; validate
the stable delta; classify its downstream consumers with
\texttt{propagate-research-consequences}; package the evidence and provenance
with \texttt{erdos-research-return}; and prepare the proposed upstream change
with \texttt{submit-pull-request}. Pushing a branch or opening the pull request
still requires the contributor's authorisation.

The router also has to serve a request that names no target.  The task
``read the corpus and find something worth developing across the papers''
matched no cue and fell through to the repository-explanation fallback, so an
agent asked to think was handed a tour.  The repair followed the method of
this paper: the observed request became a routing fixture, a lane for open
reading was added, and the \texttt{explore-the-corpus} skill now hands an
emerged question to \texttt{mine-open-problem} and the sequence above.  The
same research instruction is published without a clone in the
\href{\repobase/blob/main/docs/reading-edition/README.md}{reading edition}.

The sequence is deliberately independent of the agent harness. A contributor
may use Codex, Claude Code, Cursor, another runner, ordinary scripts, or no
agent. The shared contract is the starting commit, changed source, evidence,
claim ceiling, surviving limitation, consequence dispositions, and stop or
re-entry condition. A suspended run can therefore be resumed without being
misreported as either failure or proof.

The starting commit also permits work from an older clone to survive current
development. Git retains the common ancestor and hence the original delta.
Maintainers first reproduce that delta in its stated context, then reconcile
the reviewed substance with current main. Validation and consequence
propagation are repeated on the integration delta. Material conflict
resolution is recorded and credited as new work; the contributor's original
authorship is preserved. The complete governance sequence appears in the
\papersectionlink{open-source-mathematics-strategy.pdf}%
{strategy-protocol}{companion contribution protocol}, while the wider job map
appears in the
\papersectionlink{claim-faithful-publication-systems-paper.pdf}%
{systems-job-lifecycle}{systems lifecycle}.

\section{Limits and transfer conditions}\label{sec:limits}
\papersectiontarget{cold-clone-limits}
...
The audit was author-operated. No external contributor had yet completed the
tour, attempted the documented return path, or reported where the first-contact
explanation failed. Cold-clone usability and maintainer burden are therefore
open empirical questions, not properties established by the internal replay.
...
\section{What the architecture guarantees}\label{sec:conclusion}

An agent-native formal repository needs to reveal, from a cold clone, what
mathematics exists, what each navigation
...
projections, a public proof cockpit, a bounded intent tour, exact loaded-root
dependency data, a session notary, focused builds, and cache-bound validation
receipts.
...
\appendix
\section{How to reproduce the audit}\label{app:repro}

The public repository commits the tour, route, projections, workbench session,
and validators used in this paper. The shortest reproduction route has four
bounded tasks.

\paragraph{1. Orient without compiling Lean.}
...
\end{routeblock}
The cockpit check validates the public claim registry, cold-clone contract, and
generated-orientation freshness; it does not run Lean.  The full tour packet is
obtained by omitting the format flag. A declaration name returned by inventory
can be expanded into a direct neighbourhood or bounded proof cone.

\paragraph{2. Check a proof.}
...
\paragraph{3. Check reuse of the dependency index.}
...
\paragraph{4. Inspect the return lifecycle.}
...
\end{routeblock}
These commands inspect and test the route. They do not push a branch, open a
pull request, accept a contribution, or alter mathematical status.
...
\bibitem{workflowprovenance} R.~Souza, T.~Poteet, B.~Etz, D.~Rosendo,
A.~Gueroudji, W.~Shin, P.~Balaprakash, and R.~Ferreira da Silva,
\emph{LLM Agents for Interactive Workflow Provenance: Reference Architecture
and Evaluation Methodology}, WORKS at the ACM/IEEE International Conference
for High Performance Computing, Networking, Storage and Analysis, 2025,
\href{https://doi.org/10.1145/3731599.3767582}{DOI}.
\bibitem{repokg} S.~Abedu, S.~Khatoonabadi, and E.~Shihab,
\emph{Synergizing LLMs and Knowledge Graphs: A Novel Approach to Software
Repository-Related Question Answering}, 2024, revised 2025,
\href{https://arxiv.org/abs/2412.03815}{arXiv:2412.03815}.
\end{thebibliography}
\end{multicols}
}
