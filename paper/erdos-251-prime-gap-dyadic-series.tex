...
catalogue~\cite{erdosproblems}.  The problem is open.  Numerically
...
the remaining obligation.  The formal statements are checked by Lean~4~\cite{lean4} against
...
Maynard and Tao~\cite[Theorem~1, p.~66]{fgkmt2018} bounds the largest single
consecutive-prime gap below $X$, whereas \eqref{eq:truncation} needs the joint
dyadic distribution of a whole block of consecutive gap differences.  The same
introduction notes a separate sequel on chains of large gaps.  The cited theorem
itself supplies no residue-sensitive block estimate of the kind needed here.
