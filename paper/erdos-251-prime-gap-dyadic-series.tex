...
$\ph(d)$.  More precisely, the initial state is rational if and only if one
...
equivalently when every fixed shift is non-integral cofinally.
...
\begin{proof}
Theorem~\ref{res:collapse} gives \textup{(i)}$\Rightarrow$\textup{(iii)}, and
...
Theorem~\ref{res:escape-irrational} identifies Problem~\#251 exactly with cofinal
shift escape for $\mathcal T$.  The concrete tail, its recurrence,
