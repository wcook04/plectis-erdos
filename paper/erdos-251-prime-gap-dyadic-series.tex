...
$g_n=p_{n+1}-p_n$.  We formalise in Lean the elementary bound
$p_n\le1250(n+1)^4$, hence convergence of both dyadic series, as well as the
exact finite identity
\[
 \sum_{i=0}^{n}\frac{p_i}{2^{\,i+1}}
 \;=\;2+\sum_{i=0}^{n-1}\frac{g_i}{2^{\,i+1}}-\frac{p_n}{2^{\,n+1}},
\]
and the unconditional infinite identity and irrationality equivalence
\[
...
\]
The prime number theorem remains useful context, but it is not a proof
dependency of the formal statements in this note.
...
For every rational candidate, the checked denominator argument selects a
positive fixed shift which is eventually integral but not eventually confined
to $(-1,1)$.  This is compatible with rationality, not a contradiction: it
identifies a shift for which eventual smallness fails.  No theorem supplies
either eventual smallness or cofinally many adjacent small mismatches for that
same shift.  A telescoping
...
Concrete prime-gap tail recurrence and rational-candidate bridge & Lean-checked unconditionally & The actual scaled tail is represented at every index under non-irrationality. \\
...
\]
The Lean-checked polynomial prime bound gives convergence, and an index shift
gives
\[
...
shift escape is proved for $\mathcal T$.  The concrete tail, its recurrence,
and the bridge from a hypothetical rational value to every scaled real tail
are kernel-checked.  What is not proved is the cofinal non-integrality needed
by Theorem~\ref{res:escape-irrational}.
...
interval
(\lword{Erdos251/PrimeGapDyadicTail.lean}{1101}{rationalPrimeGapTail_has_positive_shift_not_eventually_small}{not eventually small}).
