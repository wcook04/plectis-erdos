...
although both Lean-checked, do not supply either property.  Indeed, an
explicit unbounded, non-eventually-periodic integer sequence has a rational
dyadic sum, so the missing argument must control the full prime-gap tails
rather than the coefficients alone.  The source-current coefficient-only
interface makes this warning precise: for the synthetic linear carry $K_n=n$,
...
\externalreviewobject{The paper proves the exact prime/prime-gap summation
identity, classifies rationality through integral tail shifts, proves that
actual prime gaps are unbounded, and gives, at paper level, a nonperiodic
rational countermodel to a tempting coefficient-only route.}{These results expose the missing
cofinal nonintegrality statement for the actual prime gaps without proving it.
Problem~\#251 remains open.}{Comparator selects the prime-gap unboundedness
theorem, the exact irrationality equivalence between the two infinite series,
and the conditional actual-prime-gap small-mismatch specialization, with the
summability and recurrence hypotheses retained; its coefficient-only no-go
transport checks only finite identities and nonperiodicity, leaving every
cofinal producer and the infinite-limit step open.}
