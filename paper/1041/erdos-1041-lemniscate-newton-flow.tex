...
\begin{abstract}
Every monic trinomial $z^n+az^m+b$, $1\le m<n$, with all roots in the
open unit disc has every root-to-origin segment inside $\{|f|<1\}$, so
any two roots are joined through the origin with length less than $2$.
The proof reduces each radial image to two controlled values.
For arbitrary squarefree monic polynomials of degree $n\ge2$, writing
$\mu=\min_{f'(c)=0}|f(c)|$, we also obtain a connector shorter than $2$
when $\mu\le13/25$, and by scaling a connector shorter than
$(5/2)\mu^{1/n}$ in $\{|f|<(25/13)\mu\}$ without a root-location assumption.
A separate inverse-map argument treats an isolated simple critical value,
while a Poisson identity gives the sharp critical-value mean
$\sum_{j=1}^{n-1}|f(c_j)|^{2/(n-1)}\le(n-1)R^{2n/(n-1)}$
for roots in a closed disc of radius $R$.
The solved families, sufficient conditions and obstruction examples do not
settle the unrestricted root-connector problem.
\end{abstract}
...
complete degree-four case.  His proof separates a close-pair chord case from
a radial construction: after normalising a smallest enclosing disc, a
four-point angular lemma selects two safe spokes.  That vertex need not be
a critical point.  The results below concern their stated polynomial families
and sufficient conditions; no assessment of the current unrestricted problem
status or priority is intended.
...
area.  The scaled $5/2$ consequence is stronger in both length and containment,
since $25/13<2$; the area-based construction is retained for its proof method
and its component-sensitive refinements.
...
\]
P\'olya's inequality $A(T)\le\pi T^{2/n}$ \cite[Theorem~1]{crane} and the change of measure
...
Lean checks the proposition as
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1041/PaperStraightObstructions.lean#L178}{the complete straight-path obstructions}.  The first defeats a
...
and
\lword{Erdos1041/PrimitiveQuinticInteriorTail.lean}{272}{primitiveInterior_exists_two_tailEnergy_lt_one}{two interior tail energies below one}.
The attached source snapshot also contains \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperPrimitiveCompletionR10.lean#L235}{a complete primitive-quintic path declaration}.
...
against the standard axioms.  The finite checked kernels and the full
geometric theorem remain distinct layers: the former were already in the
root, and the latter is now a checked implementation rather than an
...
proper, and $U$ is simply connected by the maximum principle.  The
component-wise Riemann--Hurwitz count in the proof of
\cite[Proposition~2.1]{eks2010}, applied to $(P-w_0)/S$, gives
$\deg(P|_U)=2$ from the one simple ramification point.  Thus $1-P$ has only its
...
The Bergman segment inequality is the following estimate.  Let
$\Phi:\mathbb D\to U$ be conformal with $\operatorname{Area}(U)<\infty$, and
let $0<q<1$.  The monomials $\sqrt{(m+1)/\pi}\,z^m$ are orthonormal for area
measure on $\mathbb D$, so the Bergman space $A^2(\mathbb D)$ has reproducing
kernel $K(z,w)=\pi^{-1}(1-z\overline w)^{-2}$, and
$\|\Phi'\|_{A^2}^2=\operatorname{Area}(U)$.  Choose a measurable unimodular
$\theta$ with $\theta\Phi'=|\Phi'|$ on $[-q,q]$.  The reproducing property,
Cauchy--Schwarz and the positivity of $K$ on $[-q,q]^2$ give
\[
...
\]
Here $(1+q^2)/(1-q^2)=(S^2+S+p)/(S^2-S+p)$.  Applying this estimate to
$\Phi=Z\circ\zeta^{-1}$ gives
\[
...
$\operatorname{Area}(K)\le\pi\operatorname{cap}(K)^2$
\cite[printed pp.~280--282]{polya1928}, in the form
\cite[Theorem~6]{crane}, then gives
...
Dubinin's relative area inequality \cite[Theorem~1]{dubinin} assumes a full
$n$-fold covering of an annulus; the proof above uses the absolute
inequality only.
\end{proof}
...
multiple-saddle strata, or proves the unrestricted problem.
...
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1041/PaperSeparationCounterexample.lean#L189}{the complete separation counterexample}; the
...
An arbitrary one-root component has no degree-uniform perimeter bound:
the Runge-approximation construction in the accompanying research addendum
\cite[Proposition~2]{revision2026} produces monic level-one examples with
arbitrarily long analytic boundaries.  That construction does not require
all roots in the unit disc.  The narrower question relevant here requires
\emph{every} inverse branch to be unramified at the chosen level:
$0<\sigma<\mu(f)$.  For a branch $f(\phi(w))=\sigma w$, it asks whether
$\int_0^{2\pi}|\phi'(e^{it})|\,dt\le\beta\sigma^{1/n}$ with an absolute
$\beta$.  Necessarily $\beta\ge2\pi$, as shown by $z^N-az$ with fixed $a>1$
and $N\to\infty$, followed by $a\downarrow1$.  The family $z^N-z$ at level
$1$ does not supply the required isolated component.  Neither general path
theorem uses the rejected arbitrary-component bound.
...
Kozen and Stef\'ansson record the following identity as a lemma of Shub,
Tischler and Williams \cite[Lemma~2.1]{kozen-stefansson1997}.
...
endpoint behaviour of maximal trajectories is classified there as well
\cite[Lemma~2.2]{kozen-stefansson1997}.
...
$n/2$ of the zeros.  Applied to $q$, after the reflection $w\mapsto-w$ if the
centroid is negative, this gives for $n\ge3$ two consecutive zeros joined by
...
points $c_i$ chosen in the proof are therefore the critical points of $q$, and
the gap maxima are the moduli of its critical sequence
$(q(c_1),\ldots,q(c_{n-1}))$.  A real polynomial with real zeros is determined
by its critical sequence up to an increasing real affine change of variable
\cite[Theorem~1]{eremenko-yuditskii}; the proof above uses only the
alternation count.  In this language, the bound of
Theorem~\ref{res:sharp-collinear-root-diameter} says that a real polynomial of
degree $n$ with $n$ distinct real zeros, leading coefficient $a$ and zero
diameter $D$ has a critical value of modulus at most $C_n|a|(D/2)^n$.
...
recalled in \cite[\S1]{eremenko-yuditskii}.  All $n-1$ of its critical values
have modulus $C_n$, so every critical point of $q_*$ lies on the level curve
$\{|q_*|=C_n\}$.  Extremals with this property also occur in the level-curve
length problem, where some extremal polynomial has all its critical points on
$\{|p|=1\}$ \cite[Lemma~5]{eremenko-hayman}, and in the sharp bound for $|f'|$
on a connected sublevel set $\{|f|\le1\}$ of a monic polynomial, proved by
Eremenko and Lempert \cite[Theorem~1]{eremenko-lempert}, whose equality cases are
$e^{-in\theta}T_n(2^{1/n-1}e^{i\theta}z+b)$ with $\theta$ real
\cite[Theorem~A]{eremenko-markov}.
...
that product bound by AM--GM; the converse implication does not hold.
...
The proof above explains the analytic mechanism.  The supplied formal index
at commit \texttt{3d6d938d696f} places \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperCriticalValueMeanR10.lean#L101}{the complete critical-value mean}
and the complete weighted free-point theorem in its checked build closure.
The critical-value endpoint includes arbitrary centre, $R=0$, and critical
points counted with multiplicity.  This is evidence recorded in the attached
index, not a fresh kernel replay in this revision.  Older candidate headers
and narrower comparator selections do not supersede those complete targets.
...
critical point $c$; its two halves are Newton-flow trajectories from $c$ to
roots, that is, edges of the Newtonian graph
\cite[Definition~2.1]{kozen-stefansson1997}.  Only critical points with
$|f(c)|\le1$ give admissible
...
The previous return also supplies a near-regular-pentagon family for which
\emph{all} origin spokes fail \cite[Proposition~A of the preserved earlier
note]{revision2026}.  The accompanying addendum analyses every critical point
of that family and gives an ordinary counterargument to a critical
\emph{straight}-hub selector even locally near the regular pentagon.
That new argument is separated from the paper's registered formal results
and requires independent review.  It does not exclude a noncritical vertex,
curved inverse-ray arms or an arbitrary contained connector.
...
The evidence belongs to each statement, not to a blanket label for the
whole paper.  The supplied index at commit \texttt{3d6d938d696f} records
complete checked targets for the critical-value mean, weighted free-point
inequality and translated cubic-fibre path.  The trinomial radial bounds and
radius sum, together with the named finite selectors and algebraic kernels,
have their own cited formal statements.  A comparator is a selected
interface, not an inventory of every formal theorem.

The complete degree-three and primitive-quintic path declarations also occur
in the supplied source, but are indexed outside the checked build.  Their
ordinary proofs remain mathematical arguments; absence of an attached build
receipt is not an absence of an ordinary proof or a formal-source file.

The $13/25$ theorem uses ordinary analysis and an exact rational certificate;
its scaled corollary, the independent $71/10$ argument, inverse-map geometry
and slit-sheet topology are not certified merely by the neighbouring finite
Lean kernels.  The new research addendum is ordinary, unreviewed work, with
its rational sign checks explicitly separated from its analytic steps.
No fresh Lean replay or independent human review is claimed in this revision.

Fixed-degree lower semicontinuity reduces the closed class to a dense generic
metric bound.  The admissible \emph{curved} canonical-arc estimate is a
sufficient target, not an identity for the unrestricted path infimum.
Raster searches are finite-sample computations, not proofs of that target.
