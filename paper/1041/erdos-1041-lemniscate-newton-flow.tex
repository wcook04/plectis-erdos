...
\input{evidence/erdos-1041-lemniscate-newton-flow}
...
\hypersetup{
  pdftitle={Paths in polynomial lemniscates: a degree-seven counterexample and two short-path criteria},
  pdfsubject={Erdős Problem Notes: Problem 1041},
  pdfkeywords={lemniscate, counterexample, Hausdorff measure, critical values, trinomials, weighted Poisson inequality, Lean 4},
}

% Source targets retain their original pinned identifiers.

\runninghead{Paths in polynomial lemniscates: a counterexample and two criteria}
\title{Paths in Polynomial Lemniscates:\\A Degree-Seven Counterexample\\and Two Short-Path Criteria}
\subtitle{Erd\H{o}s Problem~\#1041, with a Lean proof for the polynomial
constructed by \texttt{ani}}
\author{Will Cook}
...
curve-length wording is not recorded. The rest of the note concerns two
sufficient conditions for a short path; neither applies to that polynomial.
...
segments. Second, a squarefree monic polynomial has a contained path of
length less than $2$ when its least critical-value modulus is at most
$13/25$, without a root-location hypothesis. Its proof combines an
area-growth argument with an exact rational certificate computed by a
program; the main note outlines that proof and states its scale-free
consequence, and the companion record gives it in full. Independent
estimates and limitations of other constructions are collected in the
supplementary sections.
\end{abstract}

\section{The question and a degree-seven counterexample}\label{sec:problem}
...
statement.\footnote{Lean proves the
\href{https://github.com/wcook04/plectis-erdos/blob/7380b7871687b6bcc41ca0143c61f232e8af6500/lean/ErdosProblems/Erdos1041/Counterexample/HausdorffLength.lean\#L283}{preconnected-set theorem} and the
\href{https://github.com/wcook04/plectis-erdos/blob/7380b7871687b6bcc41ca0143c61f232e8af6500/lean/ErdosProblems/Erdos1041/Counterexample/HausdorffLength.lean\#L407}{negation of the Formal Conjectures statement}, and a
\href{https://github.com/wcook04/plectis-erdos/blob/7380b7871687b6bcc41ca0143c61f232e8af6500/lean/ErdosProblems/Erdos1041/Counterexample/HausdorffLength.lean\#L449}{third Lean theorem} proves the \texttt{answer(False)} form.
...
The small-parameter family that \texttt{ani} reported is not formalised, and independent human
review has not adjudicated correspondence with the 1958 wording.}

The next two sections give sufficient conditions under which two roots
are joined by a short path. Each is a hypothesis on $f$, and neither
applies to the polynomial of \texttt{ani}: it has eight nonzero
coefficients, and all its critical values lie very near the unit circle.
Section~\ref{sec:trinomial} treats trinomials; its proof prescribes a
path for every pair of distinct roots. Section~\ref{sec:constant-factor}
requires the least critical-value modulus to be at most $13/25$, imposes
no coefficient pattern or root-location hypothesis, and selects some pair
of roots. The \hyperlink{supplementary-results}{supplementary sections}
retain the other estimates and the obstructions to prescribed
constructions.
...
different root or a different joining point.
...
time $X=635762889599/10^{12}$ recorded by the certificate. The computation
that certifies $X$, done by an exact-arithmetic program, and the analytic
argument in degrees three and higher are not checked in Lean.}
...
No optimality of either cutoff is asserted.
...
\]
The \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperFiniteDual.lean#L68}{Lean lemma for this last implication}
assumes the packing bound and boundedness; it does not cover the
preceding hyperbolic geometry.
...
the simpler stated constant. A coarser run of the same program gives
$X=664373027131/10^{12}$ and the weaker threshold $51/100$.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/UnconditionalConstantFactorBound.md}{area argument} and on
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/MinimalHubWindowJoin.md}{components with two roots}.
...
critical level. The second note keeps these assumptions
...
\begin{corollary}[two nearest roots]\label{res:two-nearest-roots}
...
not a curved path. Lean checks the whole proposition: the polynomials,
the root locations, the critical point, uniqueness of the nearest root
and both escapes.
...
and
\lword{Erdos1041/SharpCollinearChebyshev.lean}{133}{exists_peak_le_comparisonBound}{the resulting Chebyshev bound}.
...
Bergman length bound, and the capacity bound for the area of the
...
Theorem~\ref{res:critical-value-separation}. The numerical inequalities,
including the cubic constant $6/5$, are checked without it.}
...
\end{corollary}
\evidenceremark{The Lean proof assumes
Theorem~\ref{res:critical-value-separation}.}
...
$z^3+(3/100)z-3/4$ shows in particular that the condition
$|1-f(d)/v|\ge2$ for every other critical point $d$ does not cover all
polynomials.
...
$U'$ contain two zeros of $P-w_0$. Theorem~2 and Corollary~3 of a
\href{https://github.com/wcook04/plectis-erdos/blob/8efbccc235df64a38d83f5dc7b1949e2ad18270d/research_corpus/Erdos1041/problem/ExteriorBlaschkeFibreCapacityGap.md}{research note on component capacity}
give $\operatorname{cap}(\overline{U'})^n/S'<1/(n-1)$.
...
\paragraph{Scope.}\label{bdry:critical-value-separation}
The proof above is also written out in a
\href{https://github.com/wcook04/plectis-erdos/blob/8efbccc235df64a38d83f5dc7b1949e2ad18270d/research_corpus/Erdos1041/problem/DiskFamilyCriticalValueSeparation.md}{research note}.
...
The rational calculations are
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/RevisionR2ExactCores.lean}{checked in Lean}, and a
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/ExactObstructionsR2.md}{research note}
gives the ordinary arguments.
...
\noindent This cubic example is distinct from the quartic example discussed
in the long record.
...
For this monic cubic, its normalised capacity is its capacity divided by
$(2\mu)^{1/3}$. Here the capacity of a compact set is its transfinite
diameter, the limit as $N\to\infty$ of the largest geometric mean of the
$N(N-1)/2$ distances between $N$ of its points; by the Fekete--Szeg\H{o}
theorem~\cite[p.~153]{ransford1995} it equals the logarithmic capacity. The
whole filled lemniscate has capacity $(2\mu)^{1/3}$
\cite[Theorem~5.2.5]{ransford1995}, so a normalised value of $1$ gives no
improvement over the whole set.
...
\end{proposition}
\evidenceremark{The Lean proof assumes the classical value $t^{1/n}$ of
the transfinite diameter of the filled lemniscate $\{|p|\le t\}$ of a
monic polynomial $p$ of degree $n$, and uses it only in the capacity
clause; the root count $k_0=2$ is proved outright.}
...
give the displayed identity.  Lean checks both conclusions. The
\lword{Erdos1041/NewtonFlowRaySeparation.lean}{50}{newtonFlow_value_hasDerivAt}{local chain rule}
and the
\lword{Erdos1041/NewtonFlowRaySeparation.lean}{64}{newtonFlow_scaledValue_hasDerivAt_zero}{zero derivative of $e^{t}f(z(t))$}
are also separate Lean lemmas.
...
endpoint.  Lean checks the whole corollary, including the endpoint
relation $f(z(b))=e^{a-b}f(z(a))$, assuming only that $f\circ z$ is
continuous on $[a,b]$. Its last step, that values on distinct positive
rays cannot satisfy this relation, is also a
\lword{Erdos1041/NewtonFlowRaySeparation.lean}{334}{no_newtonConnection_of_not_samePositiveRay}{separate Lean lemma}.
...
\noindent Each pair of critical values contributes such a forbidden subset.
...
A \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/ReciprocalNewtonExpansion.md}{research note}
gives recurrences for these reciprocal power sums and tests based on the
first two derivatives for a locally contained piecewise path.
...
Proposition~12 of the manuscript posted by \texttt{shtuka} in March 2026
\cite{march2026} claims the following estimate.  For $u=-\log|f|$, a connected component $V$ of $\{u>c\}$ carrying
...
The exact finite inequality and the contradiction it gives for the
proposed tree-length bound also have a \href{https://github.com/wcook04/plectis-erdos/blob/e6c2d8f77ac24753c5216a49f4daf7f7388b309f/research_corpus/Erdos1041/CassiniTreeBudget.lean\#L117}{Lean proof}, in the
repository's research corpus, a library the checked build does not
compile. The level-length majorant is an ordinary analytic calculation.
...
\begin{theorem}[complementary binomial chords]\label{res:complementary-binomial-chords}
...
The outer chord has length $2r\sin(\pi/n)\le2r<2$. The argument
distinguishes two prescribed polygonal constructions; it is not a
shortest-path result or a claim of priority.
...
degree two is the exact boundary case. The result does not say that every adjacent gap
...
and handles equality at this endpoint. Lean does not check this
refinement.
...
Theorem~\ref{res:fp-to-s} states the quadratic specialisation, which is
the case checked in Lean; the fourth-power consequence rests on the
...
roots with small total logarithmic contribution are given in a
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/AttachmentAgeLifetimeOrlicz.md}{research note}.
Those inequalities do not by themselves construct pieces of a contained
path that meet one another.

\section{Fixed-degree limits and the earlier approach}\label{sec:open}
...
\href{erdos1041-lemniscate-reasoning-surface.pdf\#closed-class-limits}{The fixed-degree limits subsection of the companion} gives the full argument.
...
historical reduction. They also show that the bound fails in degree
seven: if $R<1$ exceeds the root moduli of the polynomial $f$ of
\texttt{ani} in Section~\ref{sec:problem}, then $R^{-7}f(Rz)$ lies in $\mathcal K_7$ and
has $\Lambda>2$, because a contained curve of length at most $2$ would
rescale to a curve in $\{|f|<1\}$ of length below $2$, whose image has
one-dimensional Hausdorff measure below $2$. The reduction remains
available on separately specified polynomial classes.
...
of a
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/AttachmentAwareReeb.md}{research note}.
...
\emph{all} origin segments escape \cite[Proposition~A of the earlier
...
but its proof is not available.
...
\section{Sources and adjacent results}\label{sec:1041-sources}
...
\bibitem{ransford1995} T.~Ransford,
  \emph{Potential Theory in the Complex Plane}, London Mathematical Society
  Student Texts~28, Cambridge University Press, Cambridge, 1995,
  doi:\href{https://doi.org/10.1017/CBO9780511623776}{10.1017/CBO9780511623776}.
\end{thebibliography}
