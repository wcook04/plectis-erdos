...
\begin{abstract}
For a monic polynomial with roots in the open unit disc, Erd\H{o}s,
Herzog and Piranian ask for a root-to-root path of length less than $2$
inside the open unit lemniscate.  This record gives the arguments behind
several sufficient conditions and solved families, beginning with the
all-degree trinomial theorem: every root-to-origin segment is contained.
For squarefree monic polynomials, with no root-location restriction,
$\mu=\min_{f'(c)=0}|f(c)|\le13/25$ guarantees such a path; scaling gives
length less than $(5/2)\mu^{1/n}$ in the open level $(25/13)\mu$.
An independent construction gives $(71/10)\mu^{1/n}$ at level $2\mu$,
and a two-sheeted inverse-map argument treats a separated simple critical
value.  The record also contains degree three, collinear roots, sparse
quintics and quotient-fibre families, and the sharp all-degree
critical-value mean for roots in a disc.  Classical product inequalities,
inverse-sheet constructions and area estimates are credited at the point
of use.  Counterexamples distinguish metric selection from a scalar
critical-value budget.  The unrestricted conclusion is not established
here, and formal statements, supplied build evidence, ordinary arguments
and computations are recorded separately.
\end{abstract}

\paragraph{How to read the record.}
The short note is the entry point; this record supplies hypotheses, proofs,
counterexamples and evidence boundaries.  Statements sharing a label across
the two papers are still separate formalisation obligations.  The final
source guide names complete targets as well as partial kernels.  The former
front-page inventories and the exact perturbation-interface descriptions
are preserved verbatim in the editorial transfer archive rather than repeated
before the first argument.

\paragraph{The principal arguments and their dependencies.}
The trinomial proof in Section~\ref{sec:trinomials} uses the root equation
to control an entire radial segment; it does not use Newton flow.
The low-critical theorem in
Section~\ref{sec:low-critical-closure} follows a different chain:
absence of a short connector forces separated points in a conformal disc,
circle slices bound how many can occur at each radius, and the resulting
root count forces too much area growth.  The finite dual inequality is the
summation step of that argument, not a replacement for its geometric inputs.
The later solved families and obstructions can be consulted independently.
Sections~\ref{sec:newton}--\ref{sec:gap} distinguish the scalar Newton
identity from the global topology and metric estimates that it does not
supply.  This record retains those distinctions when a short-paper proof
refers here for detail.
...
\cite{bloom}.  This record does not adjudicate announcements or the current
status of the unrestricted problem.
...
$\dot z=-f(z)/f'(z)$ the Newton flow and observes that $f$ maps its solution
curves to straight lines emanating from the origin
\cite[p.~42]{sutherland1992}.  The underlying identity
$f(z(t))=e^{-t}f(z(0))$ is a lemma of Shub, Tischler and Williams, recorded by
Kozen and Stef\'ansson \cite[Lemma~2.1]{kozen-stefansson1997}, and the flow
curves joining roots and critical points form the Newtonian graph, which has
finitely many edges and is connected \cite[\S2]{kozen-stefansson1997}.  The
arguments below use only the value-space identity.
...
with the Cassini obstruction to the printed global tree budget.  That
obstruction refutes the load-bearing metric estimate, not Erdős \#1041.  Pendyala's independent June 2026
...
inside the supplied checked closure and an exact finite certificate are
separate kinds of evidence.  The theorem-level boundaries are given beside
the proofs and in the final source guide.  The former front status table,
including superseded source descriptions, is preserved verbatim in the
editorial transfer archive rather than duplicated here.
...
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L123}{abel control polygon}, the
constant-term bound as
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L258}{norm const lt one of roots lt one},
the radial estimate as
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L219}{trinomial radial norm lt one},
and their combination with the metric budget as
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L330}{trinomial erdos1041 conclusion}.
Example~\ref{ex:sextic-spoke} is checked as
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L555}{sextic guardrail}.  What the
...
a finite Blaschke product of degree $k$, as in \cite[(2.2)]{eks2010}.

For a Riemann map the derivative has Bergman squared norm
$\operatorname{Area}(C_t)$.  The Bergman kernel
$K(z,w)=\pi^{-1}(1-z\overline w)^{-2}$ of the disc is positive on the real
square, so the reproducing property, a measurable unimodular phase on $[0,r]$
and Cauchy--Schwarz give
$\operatorname{length}(\varphi([0,r]))^2\le\operatorname{Area}(C_t)
\int_0^r\!\int_0^rK(x,y)\,dx\,dy=a\log(1/(1-r^2))$.  Moving
...
\end{proof}
This is the scale-free consequence already displayed in the short note.
It improves both the length constant and the containment level of the next
theorem: $5/2<71/10$ and $25/13<2$.  The older proof is retained for its
independent area mechanism and component-sensitive estimates.

\section{An unconditional all-degree constant-factor path theorem}
\label{sec:constant-factor}

The scaled low-critical corollary above supersedes the universal constant
in this section, with a smaller containment level as well.  We retain the
independent area-based construction because its proof exposes root-count and
component-capacity dependence.  Its high-arity and capacity-defect corollaries
are also retained, although their stated $\mu\le1/2$ ranges are already covered
by the stronger $\mu\le13/25$ theorem.  They describe the area method, not an
additional uncovered part of the problem.  For a monic degree-$n$ polynomial put
\[
...
If $f$ has a repeated zero, the constant path between two occurrences
proves the theorem.  Otherwise $f$ is squarefree and $\mu>0$.
...
\]
P\'olya's area inequality
$\operatorname{Area}(K_T)\le\pi T^{2/n}$ \cite[Theorem~1]{crane} then selects
$t$ with a
...
The three contributions are, explicitly,
\[
\begin{aligned}
...
\end{aligned}
\]
The low bound holds on every ray, the high bound on the ray chosen from
its mean, and the boundary bound at the level selected before either choice.
They therefore hold together. Adding and substituting
$T^{1/n}=\lambda^{1/n}\rho$ recovers~(CF).
...
Green defect $\Sigma<n\log(40/39)$ at the top arity.  This is a limitation of
...
The unrestricted one-component assertion previously posed here is false.
The supplied previous-return construction, reproduced in
\cite[Proposition~2]{revision2026}, uses Runge approximation and monicisation
to make a level-one component with one simple zero and arbitrarily long
analytic boundary.  It imposes neither a unit root disc nor a global lower
bound on critical-value moduli.  The useful surviving question is narrower.

\begin{problem}[globally subcritical one-lobe perimeter]\label{prob:conjecture-p}
...
\]
Equivalently, each of the $n$ univalent inverse branches
$f(\phi_j(w))=\sigma w$ should have
...
\end{problem}
All these components contain one zero.  The simultaneous condition
$\sigma<\mu$ is essential: local univalence of just one selected component
is not enough.  For $1<R<\mu/\sigma$, the area estimate gives
\[
...
For $f=z^2-d^2$ one has $\mu=d^2$ and $\rho=d$.  For $\sigma<d^2$ the component
of $\{|f|\le\sigma\}$ around $d$ contains one root, and as $\sigma\uparrow d^2$
its boundary increases to one loop of Bernoulli's lemniscate $|z^2-d^2|=d^2$,
...
set is a single component containing both roots.  Hence
$\beta_*\ge\sqrt2\,\varpi=\Gamma(1/4)^2/(2\sqrt\pi)=3.70814935\ldots$, where
$\varpi$ is the lemniscate constant.  On $z^n-r^n$ the corresponding closed
form $(2^{1/n}/n)\sqrt\pi\,\Gamma(1/(2n))/\Gamma(1/(2n)+1/2)$ decreases from
$3.7081$ at $n=2$ to $2$ as $n\to\infty$.

The quadratic loop is exceeded in degree eight.  For $p(z)=z^8-(3/2)z$ and
$\sigma=1$, the component $C$ of $\{|p|\le1\}$ containing the origin lies in
...
$\mathcal H^1(\partial C)>5\pi/4>(\pi/2)(1+\sqrt2)\ge\sqrt2\,\varpi$.  Moreover,
...
for every $0<\sigma<\mu$, where $f$ is squarefree and monic of degree
$n\ge2$, then two distinct roots of $f$ are joined inside $K_\mu$
...
\begin{proof}
At the first critical level a critical point $c_*$ belongs to the closures
of at least two distinct one-root components of $\{|f|<\mu\}$; choose
$U_a,U_b$ among them.  No simplicity assumption is needed.  Each inverse
map extends continuously to its algebraic boundary.  Its boundary values
are injective, since $f(\phi(e^{it}))=\mu e^{it}$; hence the boundary is a
Jordan curve.  The subcritical perimeter bound passes to that boundary by
lower semicontinuity of length under uniform convergence.  It therefore
suffices to bound the intrinsic distance from $a$ to $c_*$ inside
$\overline{U_a}$.  Lift from $a$ along a ray to $\partial U_a$ and
...
Lean-checked part.  Its inputs are the capacity identity for polynomial
preimages \cite[\S2]{crane}, P\'olya's area inequality \cite[Theorem~1]{crane},
Cauchy--Schwarz, the Koebe distortion theorem for univalent maps, and a finite
graph exit lemma.  The public source
of record is
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/UnconditionalConstantFactorBound.md}{unconditional constant factor bound}, which
carries every rational verification quoted above.  Its
containment level is $2\mu$ and its constant is $71/10$.
...
inequalities.  The former finite perimeter samples concerned the superseded
auxiliary formulation and are retained only in the editorial history.
They do not support the corrected global subcritical endpoint question.
...
Erd\H{o}s--Herzog--Piranian component lemma
\cite[remark before Problem~5, p.~139]{ehp1958} supplies a
...
The supplied snapshot contains \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperCubicCompletion.lean#L297}{the complete degree-three path implementation} and its supporting cubic source.
The index classifies this endpoint as \texttt{outside\_checked\_build}.
Thus the ordinary theorem is proved above and a complete formal-source
implementation is supplied, but this revision does not claim an attached
successful build for that endpoint or a fresh kernel replay.
Multiplicity and the distinction between different root occurrences and
different complex values must be read from the precise target statement.
...
\begin{theorem}[disk-family critical-value separation]
...
point in $U$.  A component of a polynomial lemniscate is simply connected, by
the maximum principle.  The proper map $P:U\to Q$ therefore has degree two:
the component-wise Riemann--Hurwitz count in the proof of
\cite[Proposition~2.1]{eks2010}, applied to $(P-w_0)/S$, gives
$1=\deg(P|_U)-1$ because the simple critical point contributes the only unit
of ramification.
...
\]
Here $\widetilde Q$ is the connected simply connected double cover of
$D(a,S)$ branched at $0$.  Hence $\xi$ is a conformal bijection; its inverse
$Z$ is the continued saddle branch.  Since $[-1,1]\subset\widetilde Q$, the
curve $Z([-1,1])$ joins the two distinct points over $P=0$, and
$P(Z(\xi))=1-\xi^2\in[0,1]$ gives its containment.

The next change of coordinate puts the exact curve, rather than a larger
disc, into Bergman normal form.  The map
\[
 M(w)=\frac{Sw}{S^2+aw-a^2}
\]
sends $D(a,S)$ conformally to the unit disc and fixes $0$.  Choosing the
analytic square root gives a conformal bijection
\[
...
It remains to bound the area of this particular component.  For
$1-w_0<S'<S$, let $U'$ be the component of
$\{|P-w_0|<S'\}$ containing $0$.  It is a proper regular component carrying
exactly two zeros of $P-w_0$, counted with multiplicity.  The exterior-fibre
capacity gap for a proper $k$-zero component, here with $k=2$, gives
\[
...
then yields
$\operatorname{Area}(U')<\pi(S'/(n-1))^{2/n}$.  These components exhaust
$U$ as $S'\nearrow S$, so
\[
...
polynomial, set $z=c+\rho w$, where $\rho=|v|^{1/n}$; then
$P(w)=f(z)/v$ has leading coefficient $|v|/v$, of modulus one.  Scaling the
connector multiplies its length by $\rho<1$
and takes $\{|P|\le1\}$ to $\{|f|\le|v|\}\subset\{|f|<1\}$.
\end{proof}

Taking $S=2$ gives the earlier branch-centred criterion
$|1-f(d)/v|\ge2$ in every degree $n\ge3$.  If $c$ minimises the nonzero
critical-value modulus, the Fekete--resultant bound gives
$|v|\le R^n<1$ when all roots lie in a disc of radius $R<1$; for a general
selected hub, the condition $|v|<1$ must remain explicit.
In degree three the branch-centred choice $w_0=1$ already works at
$S=6/5$, since
\[
...
constrains critical values only.  Roots and critical points can remain
uniformly separated while two distinct critical values coalesce.  An exact
example is
\[
...
inequality, the exterior-fibre capacity gap, P\'olya's area--capacity
inequality and the exhaustion in the proof are ordinary mathematics.  Their
complete source proof is the immutable
\href{https://github.com/wcook04/plectis-erdos/blob/8efbccc235df64a38d83f5dc7b1949e2ad18270d/research_corpus/Erdos1041/problem/DiskFamilyCriticalValueSeparation.md}{disk-family critical-value separation record}.
The separate
...
The method stops when no disk centred on $[0,1]$ isolates the selected
critical value, and it assumes the selected saddle is simple.  It therefore
leaves the near-tie and multiple-saddle strata, makes no sharpness claim for
the uniform threshold $S=4/3$, asserts no existence of a separated disk, and
does not prove an unrestricted
...
on one proper component, after the exterior-fibre argument supplies its strict
...
$q$, after the reflection $w\mapsto-w$ if the centroid is negative.  For
...
its critical sequence $(q(c_1),\ldots,q(c_{n-1}))$.  A real polynomial with
real zeros is determined by its critical sequence up to an increasing real
affine change of variable \cite[Theorem~1]{eremenkoyuditskii2012}; the proof
uses only the alternation count.  In this language, \emph{(9)} says that a
...
The finite moment and tail-selection kernels are formally supplied.
The attached snapshot additionally contains \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperPrimitiveCompletionR10.lean#L235}{a complete primitive-quintic implementation}, including the geometric assembly.
This revision admitted that module to the checked \texttt{ErdosProblems}
root; \texttt{AxiomAudit} prints \texttt{complete\_primitive\_quintic}
against the standard axioms \texttt{propext}, \texttt{Classical.choice},
and \texttt{Quot.sound}.  The ordinary proof above remains the paper
argument; the Lean module is now a checked implementation of it.
...
Lean checks the charge identity, safe-spoke selection and quartic falsifier,
and also \href{https://github.com/wcook04/plectis-erdos/blob/27c2fc5fe3c55fe547feaeb4c9bb68b3cf63a5bf/lean/ErdosProblems/Erdos1041/PaperCubicFibres.lean#L240}{the complete translated cubic fibre theorem}.  The latter supplies the finite fibre
construction, root-of-unity average, nonzero-root selection, pullback and
explicit rectifiable hub path under the hypotheses displayed above.
...
The selected Comparator declarations route to the finite collinear,
primitive-tail and cubic-safe-spoke interfaces.  They do not exhaust formal
coverage: the complete translated cubic-fibre path is in the supplied
checked closure, while the complete primitive-quintic implementation is
indexed outside it.  The sharper $2\sqrt{1-|h|^2}$ reading and the quartic
quotient-lift geometry retain the ordinary proof boundaries stated locally.
Each paper statement therefore requires its own declaration and hypothesis
comparison; a same-named comparator is not automatically the whole theorem.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/FRONTIER.md}{the \#1041 frontier note}.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/NearFeketeRadialAngularSplit.md}{near fekete radial angular split}
...
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/ConcyclicAlternation.md}{concyclic alternation}.
...
budget, the tetranomial case is not covered by this argument; this family does not solve
...
nontrivial cyclic power.  Let $P$ be a monic quartic, let $q\ge2$, and set
\[
  f(z)=P((z-h)^q).
\]
...
the quotient geometry, in two cases.  If two quotient roots satisfy
...
\paragraph{The quartic input is geometric, not a critical-hub assertion.}
In \cite[Lemma~1 and the proof of Theorem~1, pp.~1--3]{june2026},
Pendyala first treats a close pair by a contained chord; otherwise a
smallest-enclosing-disc normalisation and a balanced-vertex angular lemma
supply two radial arms.  The proof of a power-map pullback must specify
which vertex those arms use.  An arbitrary broken line cannot inherit a
single-chord endpoint budget merely by being called a quartic connector.
The two cases above retain that distinction.
...
by AM--GM, whereas the product bound does not bound a positive moment.
...
\cite[Section~3]{revision2026} reduces its maximum to a boundary-root
configuration without solving that angular optimisation.
...
$R^{n/m}$ and $R$.  Thus the older exponent is retained as a consequence,
rather than as the principal theorem.

The quadratic mean and both lower-exponent consequences are checked as
...
and critical points counted with multiplicity.  The exact source hash and
the directly audited target alias match \href{https://github.com/wcook04/plectis-erdos/blob/27c2fc5fe3c55fe547feaeb4c9bb68b3cf63a5bf/verification/erdos1041-returned-r18-v5-full-audit-evidence.json}{the source-bound Lean build and axiom-audit record}; the source
header's earlier candidate status predates that successful run.

The stronger exponent controls concentration of the critical values.
For $R>0$, write $r_j=|f(c_j)|^{1/n}$ and $p=2n/(n-1)$. Then
\[
...
are constant on positive support. Equal weights therefore give the quadratic
finite inequality above for every $m\ge1$, with equality only at the zero
configuration. The
three-point and four-point specialisations follow with their equality
cases.  The supplied snapshot records the complete weighted target in
\texttt{PaperWeightedRefinementsR10.lean}; a dated working-tree footnote
is not a reliable description of its current attached source coverage.

The previous three-point H\"older proof is withdrawn as an alternative proof
from this record. Its
scalar defect identity
...
supplied checked closure.  Their exact statements, dependencies and status
...
This checks a central equal-weight case.  The complete all-disc weighted
target has greater mathematical scope and is itself in the supplied checked
closure; the restricted proof remains an independent mechanism.
...
The stronger critical straight-hub assertion formerly called (SPOKE-5)
is withdrawn as an open target.  For the family
\[
...
\]
the previous return obstructs all origin spokes as the roots approach a
regular pentagon.  The accompanying ordinary counterargument
\cite[Proposition~1]{revision2026} shows that, for sufficiently small
positive $\epsilon$, no critical point supplies two contained straight arms
of total length at most $2$.  That new proof is not a registered Lean
endpoint; it is provided separately for independent review.  It invalidates
the universal critical \emph{straight}-hub target, not the original path
problem or a selector using curved inverse-ray arms.
...
proper components and connecting it to the parent theorem are not established here.  The
degree-five statements and their evidence classes are recorded in
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/Degree5AssemblyAndSharpenedCuts.md}{degree5 assembly and sharpened cuts}.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/MinimalHubArmBudgetRefutation.md}{minimal hub arm budget refutation}
and
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/SeparatrixAggregateReduction.md}{separatrix aggregate reduction}.

Third, the origin is not a uniform near-Fekete hub.  Beyond the isolated
rational witness in the original record, the previous return gives an
explicit sequence $F_\epsilon$ approaching the regular pentagon for which
all five origin spokes escape \cite[Proposition~A of the preserved earlier
note]{revision2026}.  Thus failure occurs arbitrarily near the equality
configuration, not just at one fixed positive defect.  The extension to
all critical straight hubs is the separate new counterargument just stated.
Neither result excludes curved canonical arcs or an arbitrary connector.
The former isolated-witness account is preserved verbatim in the editorial
transfer archive as superseded evidence, not as the current frontier.

\subsection*{The surviving carrier and the actual open endpoint}

The sufficient canonical target uses \emph{curved} inverse-ray arcs,
not straight segments.  On a generic ray-separated class let $L_f(c)$ be
the length of the two-root descent arc at an admissible critical point,
where $|f(c)|\le1$.  Then
\[
 \min_{c\ \mathrm{admissible}}L_f(c)\le2
\]
would imply the root-connector conclusion by the stated generic-closure
...
The straight-hub obstruction supplies no proof or disproof of this curved
arc estimate.  Finite sampled rescues are not a universal selector.
The obligations are joint: admissibility, containment and metric cost
must be achieved by the same selected competitor.
...
\lref{Erdos1041/NewtonFlowRaySeparation.lean}{50}{newtonFlow_value_hasDerivAt}, together with the
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/NewtonFlowTrajectory.lean}{newton flow real value decay} in \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/NewtonFlowTrajectory.lean}{newton flow trajectory}.
...
in \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/PaperNewtonEndpoints.lean}{newton endpoints} make these
...
The level set $\{u=\mu\}$ is a lemniscate graph with a vertex of degree four
...
There is nevertheless a complete ordinary topology theorem on the generic
stratum where the critical values separate both by angle and by level.  Its
...
\begin{theorem}[ray-separated slit-sheet decomposition]
\label{res:attachment-aware-reeb}
Let $f$ be monic, and let $U$ be a component of $\{|f|<1\}$ containing
$k\ge2$ roots, counted with
multiplicity.  Suppose every critical point of $f$ in $U$ is simple, its
critical value is nonzero, and these critical values have pairwise distinct
arguments and pairwise distinct moduli.  Then:
\begin{enumerate}
...
\item cutting the value disc along the complete critical rays decomposes the
preimage into conformal strips;
...
\item for each critical point $c$, the two inverse lifts of
$[0,f(c)]$ join two roots through $c$ inside $\{|f|\le|f(c)|\}$, and the union
of these arcs is an embedded geometric realisation of that tree.
\end{enumerate}
...
\begin{proof}
A polynomial sublevel component is simply connected, and
$f:U\to\mathbb D$ is proper of degree $k$.  Riemann--Hurwitz gives exactly
$k-1$ simple critical points.  The holomorphic Morse lemma makes them
nondegenerate saddles of $-\log|f|$; the distinct moduli make their levels
distinct.  Properness here is for the displayed codomain $(0,\infty)$: a
compact positive range stays away from both the level-one boundary and the
deleted roots.
Removing all critical rays leaves simply
connected sectors without branch values, so every component of their
preimage maps biholomorphically to its sector; logarithmic polar coordinates
turn these components into strips.

For the sheet tree, remove only the outward slits
$\{tf(c):1\le t<1/|f(c)|\}$.  Distinct critical-value arguments make the
remaining value domain star-shaped about $0$, hence simply connected.  Its
preimage splits into $k$ sheets labelled by the roots.  Local monodromy at a
simple critical value is a transposition.  The monodromy action before the
slits are made is transitive because the punctured domain upstairs is
connected; therefore the graph of the $k-1$ transpositions is connected.  A
connected graph with $k$ vertices and $k-1$ edges is a tree.

The inward segment from $f(c)$ to $0$ contains no other critical value:
distinct arguments exclude a different ray, and distinct moduli separate
critical levels on the same ray.  Its two inverse lifts converge to $c$ by
the holomorphic Morse coordinate and start at two roots.  Lifts belonging to
...
an embedded tree.  The same local Morse coordinate identifies a value disc of
radius $\delta$ with a spatial neighbourhood of diameter
$O(\sqrt\delta)$.
\end{proof}

The complete ordinary argument is recorded in
\href{https://github.com/wcook04/plectis-erdos/blob/a729f05c40398663fd586c9da487ad874d898639/research_corpus/Erdos1041/AttachmentAwareReeb.md}{the attachment-aware Reeb source}.
...
Corollary~\ref{res:ray} shows that the distinct-argument hypothesis also
excludes finite nonsingular saddle-to-saddle Newton connections.  The theorem
does not cover coincident arguments, simultaneous
critical levels, multiple saddles, boundary tangencies in a general compact
band, or non-Hausdorff orbit spaces, and it gives no target-scale metric
gluing estimate.  Those broader questions remain below.
...
\subsection*{3. Metric fan-in after the printed spanning-tree coefficient is excluded}
...
Those identities explain why a per-strip average does not automatically yield a
global coefficient.  They do not reopen~\eqref{eq:prop12-bound}.  Cassini already
...
\tag{6.1}\label{eq:metric-fanin1041}
 \end{equation*}
for an explicit constant $C>1/(2\pi)$ that accounts for attachment cost and for
a valid allocation of the strip fluxes $\Phi_S$.
...
\]
used the same excluded coefficient.  It is not remaining budget on an otherwise
...
Classical Newton-flow theory supplies the global part of this programme.  Away
from the roots and critical points, every orbit satisfies
\[
 p(z(t))=e^{-(t-t_0)}p(z(t_0)),\qquad
 u(z(t))=u(z(t_0))+t-t_0,\qquad u=-\log|p|,
\]
by the lemma of Shub, Tischler and Williams
\cite[Lemma~2.1]{kozen-stefansson1997}; every maximal orbit has one of four
types, with finite ends at critical points, an infinite forward end at a root
and an infinite backward end at the point at infinity
\cite[Lemma~2.2]{kozen-stefansson1997}; and the orbits joining roots and
critical points form a connected Newtonian graph with finitely many edges
\cite[Definition~2.1 and \S2]{kozen-stefansson1997}.
...
\]
adapt this theory to the band: describe how maximal orbits cross the two
boundary curves, and show that the orbit space of the chosen strip
decomposition is Hausdorff.  Under pairwise ray separation of the critical
values, decide whether the flow is Morse--Smale relative to the boundary or
whether the strongest valid conclusion is only absence of saddle-to-saddle
connections.
\end{problem}

The checked algebra proves the pointwise value equation and the endpoint-ray
consumer; the global solution theory and the finite graph are the classical
results cited above.  The open content is their relative form on the band,
together with the quantitative slack and length control needed for gluing.  A
...
missing sufficient canonical estimate is metric, and the general curve
infimum need not equal the canonical-arc minimum.
...
priority.  The supplied index places the complete weighted free-point,
critical-value mean and translated cubic-fibre targets in its checked build
closure.  Complete cubic and primitive-quintic implementations are also
supplied but indexed outside that closure.  Finite comparators are selected
interfaces, not a substitute for a statement-by-statement comparison.
The analytic path arguments and the new research addendum retain their
ordinary proof boundaries.  No fresh kernel replay or independent human
review was performed for this revision.

\appendix
\section{Guide to the formal sources}\label{app:sources}

The attached declaration index, not a single Newton-flow module or a
shared historical macro, is the source map for this revision.  At snapshot
\texttt{3d6d938d696f}, the following distinctions are essential.
\begin{itemize}
\item Complete targets in the supplied checked closure: \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperCriticalValueMeanR10.lean#L101}{critical-value mean}, \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperWeightedRefinementsR10.lean#L17}{weighted free-point inequality}, and \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperCubicFibres.lean#L240}{translated cubic-fibre path}.
\item Complete implementations indexed outside that closure: \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperCubicCompletion.lean#L297}{degree-three path}.
\item Admitted to the checked \texttt{ErdosProblems} root in this revision: \href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperPrimitiveCompletionR10.lean#L235}{primitive-quintic path}.
\item Finite selected kernels, source-only release declarations and ordinary
analytic assemblies have separate rows in the accompanying formal-scope map.
The older Newton declaration list is preserved verbatim in the editorial
transfer archive; it remains useful for those lemmas, not as complete paper
coverage.
\end{itemize}
