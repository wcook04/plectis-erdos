...
$c$ is a simple critical point, $v=f(c)\ne0$, and some real centre
$w_0\in[0,1]$ satisfies $|f(d)/v-w_0|\ge4/3$ at every other critical point,
then two zeros are joined inside $\{|f|<1\}$ by a curve of length below $2$ in
every degree $n\ge3$, provided $|v|<1$.

The separation theorem resolves the saddle by a square-root coordinate.  A
disk centred at $w_0$ and containing the value segment $[0,1]$ isolates the
selected simple branch point.  The resulting two-sheeted component is
uniformised by $\xi^2=1-P$, then mapped to the unit disc so that the connector
becomes a diameter.  The Bergman segment inequality and the exterior-fibre
capacity gap give
\[
...
\]
The threshold $S=4/3$ reaches every degree $n\ge3$ and every real centre;
$S=2$ remains an immediate weaker corollary.
...
$(71/10)\rho$, and when $\mu\le1/2$ they are joined inside $\{|f|<1\}$ with
length at most $5.7$.  The same construction settles the
...
$\sum_{j=1}^{n-1}|f(c_j)|^{2/(n-1)}\le(n-1)R^{2n/(n-1)}$ holds in every degree
$n\ge2$ for zeros in a closed disc of radius $R\ge0$; the previously used
exponents follow from this quadratic mean.

These results do not settle the unrestricted problem.  Critical values near a tie and multiple
...
degree, with a prescribed path and an unrestricted middle coefficient.  The
statement uses checked radial inequalities and the radius budget from the
pinned Abel control-polygon source; the final conversion to one rectifiable
path is ordinary mathematics (Section~\ref{sec:trinomials}).
...
conclusion at separation $S=4/3$ in every degree $n\ge3$, for any real centre
of the normalised value segment.
...
\externalreviewobject{The paper gives an all-degree path theorem for every monic trinomial,
a sharp critical-value mean checked in Lean, a
quantitative connector at a separated simple critical value whose threshold
$S=4/3$ reaches every degree $n\ge3$, a short contained connector whenever the
least critical-value modulus is at most $13/25$, a constant-factor connector
of length at most $(71/10)\mu^{1/n}$ inside $\{|f|\le2\mu\}$, the full
degree-three case, a sharp all-degree collinear theorem, a complete primitive
sparse quintic theorem, and translated cubic quotient-fibre theorems in every
degree $3q$.  Lean checks exponential Newton-value decay, positive-ray
collision interfaces, finite translation avoidance, and quantified root
retention.  Exact obstruction examples, including a Cassini counterexample to
the printed global tree budget, show why those estimates cannot be promoted to
a general connector theorem.}{The critical-value mean and translated cubic fibre path have complete
Lean statements.  The trinomial radial inequalities and radius budget are
checked at their own public pin, with the final rectifiable-path interpretation
ordinary.  The analytic low-critical and separation arguments retain the
source boundaries stated in their sections; the regime $13/25<\mu<1$ is
untouched by the threshold argument.  The checked
dynamical and perturbative inputs do not construct unrestricted connectors, so
the unrestricted conclusion is not established here.  A reported degree-seven counterexample
announcement is a separate external claim, not a theorem of this record.}
{The selected solved-family Comparator interfaces are separate from the
full proof coverage: they check the registered kernels.  The complete
critical-value mean and translated cubic fibre path have the source-bound
Lean evidence cited in their sections.  Those checks provide neither a
general connector theorem nor an assessment of novelty or significance.}
...
Critical-value separation, general disk & Ordinary all-degree theorem & A simple nonzero hub, $S>\max(w_0,1-w_0)$ and the coefficient inequality in Theorem~\ref{res:critical-value-separation}; the covering, Bergman and capacity arguments remain ordinary. \\
Critical-value budget in every degree & Lean-checked theorem & $\sum_j|f(c_j)|^{1/n}\le(n-1)R$ for every $n\ge2$ and $R\ge0$, with the sharper Poisson mean $\sum_j|f(c_j)|^{2/(n-1)}\le n-1$ on the closed unit disc.  The complete mean is checked in \texttt{PaperCriticalValueMeanR10.lean}, with matching source-bound build evidence cited below.  No inverse-ray length estimate is implied. \\
...
Unrestricted proof of \cite[Theorem~1, p.~1]{march2026} & Printed tree budget false & Proposition~12 uses a false three-ended local saddle block, and the Cassini polynomial $z^2-a^2$ at $a=9/10$ refutes the printed $1/(2\pi)$ spanning-tree estimate.  A different route cannot retain that coefficient.  This does not refute Erdős \#1041. \\
...
by the symmetry $g(\infty,a)=G(a)$.  A remaining
...
root of $f$.  Determine whether a constant $\beta$, independent of $f$, of its
degree $n$ and of $\sigma$, satisfies
\[
...
Problem~\ref{prob:conjecture-p} is the one-root per-component analogue of the
Erd\H{o}s--Herzog--Piranian lemniscate-length problem, whose per-root constant
is $2$, attained asymptotically by $z^n-1$.
...
For Problem~\ref{prob:conjecture-p}, $120$ sampled configurations over degrees
$2$ through $10$ and five families gave a largest ratio
$\mathcal H^1(\partial U)/(\sqrt2\varpi\rho)$ of $0.9999997747$, at
quadratics.  The degree-eight example above exceeds the quadratic constant, so
the sampled maximum is a local one.  The adjacent classical literature on
...
large $n$ \cite{tao2025}.  These results bound the length of a level curve; a
dated search on 2 September 2026 located no constant-factor root-pair bound in
arbitrary degree.
...
The
checked analytic assembly
is accompanied by an
80-declaration audit.
...
surface of the inverse function into sheets by slits along rays from its branch
points, which Dubinin uses to estimate capacities
\cite[\S2]{dubinin2006capacity}.  The theorem below specifies the sheets,
...
containment $\{|f|<1\}$ with length at most $5.7$ needs only $\mu\le1/2$,
and both bounds fall short of the target constant $2$.
...
uniform threshold $S=4/3$ reaching every degree $n\ge3$ and every real centre
of the normalised value segment by
Corollary~\ref{res:critical-value-thresholds}.
Theorem~\ref{res:trinomial-all-degree} settles the whole trinomial family in
every degree by an ordinary radial argument.  Their formal coverage differs: the translated cubic fibres have a complete
rectifiable-path theorem; the trinomial source checks both radial bounds and
the radius sum.  The remaining companions check the named numerical, algebraic and
...
and the disk uniformisation, Bergman estimate, capacity gap, area bounds and
certificate chain remain ordinary mathematics.  The source now publicly verifies the Newton
kernel, finite ray avoidance and quantitative constant-translation root
control.  The ray-separated generic stratum has the ordinary slit-sheet
decomposition of Theorem~\ref{res:attachment-aware-reeb}; coefficient
perturbation, the exceptional compact-band decompositions and metric gluing
remain unresolved.
...
kernel accepts. The checked core is the local Newton value equation, the zero-derivative form
of its exponentially scaled value, the algebraic consumer from an assumed
exponential endpoint relation, the finite planar-avoidance theorem, quantitative
constant-translation root retention, and the ray-collision parameterisation.
The real-time first integral and finite-trajectory ray exclusion use the ordinary
interval-integration and endpoint-continuity argument given in the paper.  The
decomposition and length statements of \S\ref{sec:open} are not proved.  The
diagnosis of Proposition~12 in \S\ref{sec:gap} records both the invalid local
saddle block and the Cassini obstruction to the printed global tree budget.
The search results of \S\ref{sec:finite} are computations.
