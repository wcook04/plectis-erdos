...
statement is ordinary mathematics; \texttt{PaperTrinomial.lean} is not in this
checkout.
...
\externalreviewobject{The paper gives an ordinary all-degree path theorem for
every monic trinomial, a separately ordinary sharp critical-value mean, a
...
a general connector theorem.}{The trinomial paths, the Poisson critical-value
mean, the separation regime, the two threshold-free theorems, the
degree-three theorem and the solved-family path assemblies are ordinary
mathematics.  The $13/25$ threshold, the $(71/10)$ constant and the
degree-three theorem have no formal endpoint in the pinned corpus, and the
regime $13/25<\mu<1$ is untouched by the threshold argument.  The checked
dynamical and perturbative inputs do not construct unrestricted connectors, so
Problem~\#1041 remains open.  A reported degree-seven counterexample
announcement is a separate external claim, not a theorem of this record.}
{Comparator checks three solved-family kernels (collinear, primitive quintic,
translated cubic fibres), not the ordinary length-below-two assemblies.
Critical-value separation, including its threshold analysis, has no
corresponding formal endpoint in the pinned corpus.  \texttt{PaperTrinomial.lean}
and \texttt{PaperCriticalValueMeanR10.lean} are not cited as kernel-checked
complete statements.}
...
Critical-value budget in every degree & Ordinary theorem & $\sum_j|f(c_j)|^{1/n}\le(n-1)R$ for every $n\ge2$ and $R\ge0$, with the sharper Poisson mean $\sum_j|f(c_j)|^{2/(n-1)}\le n-1$ on the closed unit disc.  The complete mean is ordinary mathematics, not a kernel-checked assembly of \texttt{PaperCriticalValueMeanR10.lean}.  No inverse-ray length estimate is implied. \\
...
\[
 \sum_{j=1}^{n-1}|f(c_j)|^{2/(n-1)}\le(n-1)R^{2n/(n-1)},
\]
are ordinary mathematics in this record.
\texttt{PaperCriticalValueMeanR10.lean} is not cited as a kernel-checked
complete statement.
...
every degree by an ordinary radial argument.  All five are
