...
\input{evidence/erdos1041-lemniscate-reasoning-surface}
...
\renewcommand{\externalreviewobject}[3]{%
  ...
  \end{quote}}
\hypersetup{
  pdftitle={Paths in polynomial lemniscates: a degree-seven counterexample and two short-path criteria, proofs and examples for Erdős Problem 1041},
...
\title{Paths in Polynomial Lemniscates:\\A Degree-Seven Counterexample and Two Short-Path Criteria}
\subtitle{Proofs and examples for Erd\H{o}s Problem~\#1041, with a Lean proof for the polynomial constructed by \texttt{ani}}
\author{Will Cook}
...
$\{|f|<1\}$ containing two distinct roots has one-dimensional Hausdorff
measure greater than $2$.  This refutes the exact path-image-length
formulation in Formal Conjectures, as well as the total-variation
formulation.  Independent human review of correspondence with the 1958
curve-length wording is not recorded. The remaining results give short
paths under additional hypotheses.
...
argument, closed by an exact rational certificate computed by a program,
gives such a path when its least critical-value modulus $\mu$
...
The \href{erdos-1041-lemniscate-newton-flow.pdf}{short note} states the
counterexample and two sufficient conditions for a short path: the
trinomial identity of Section~\ref{sec:trinomials}, whose proof does not
use Newton flow, and the small-critical-value criterion, whose complete
proof is Section~\ref{sec:low-critical-closure}. The other constructions
are independent of that proof. This record provides their derivations and
...
\section{The question and the degree-seven counterexample}\label{sec:problem}
...
squarefree. The problem numbering follows Bloom's Erd\H{o}s problem catalogue
\cite{bloom}.

\phantomsection\label{res:ani-degree-seven-counterexample-long}
...
\href{https://github.com/google-deepmind/formal-conjectures/tree/398958d3964d738886bd24433918c365df4a2aab/FormalConjectures/ErdosProblems}{Formal Conjectures} is a public Lean repository of formal
statements of open problems, among them many Erd\H{o}s problems.
Lean separately proves the
\href{https://github.com/wcook04/plectis-erdos/blob/f70679d47178bf1174309cb29f5b31748609cc04/lean/ErdosProblems/Erdos1041/Counterexample/CatalogueAdapter.lean\#L25}{total-variation formulation}.
...
Erd\H{o}s--Herzog--Piranian \cite[Problem~5, p.~139]{ehp1958}; the paragraph
before it there records the known fact that one component of the
lemniscate contains at least two zeros.
...
obstruction refutes the metric estimate used in the final proof; by itself it does not answer the problem.  Pendyala's independent June 2026
...
with the cubic theorem proved here, it settles degrees three and four.
The all-degree estimates below either impose a bound on the least
...
Lean checks the
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L123}{identity}~\eqref{eq:abel-control-polygon}, the
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L258}{constant-term bound},
the
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L219}{radial estimate},
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L330}{their combination with the length bound}
and the
\href{https://github.com/wcook04/plectis-erdos/blob/0ba585f632fbbbb43af2ee9532d4e83752af3f67/ErdosProblems/Erdos1041/AbelControlPolygon.lean\#L555}{evaluation} in Example~\ref{ex:sextic-spoke}.  No novelty claim is made for this elementary trinomial argument.
...
any component.
\end{theorem}
\evidenceremark{No Lean proof covers the whole theorem. Lean checks the
degree-two case, and the closing inequality $(13/25)e^X<1$ at the stopping
time $X$ certified below. The computation that certifies $X$, done by an
exact-arithmetic program, and the analytic argument in degrees three and
higher are not checked in Lean.}
...
\end{corollary}
\evidenceremark{The Lean proof assumes
Theorem~\ref{res:low-critical-thirteen-twentyfifths}.}
...
\end{theorem}
\evidenceremark{The Lean proof takes the separation bound
\eqref{eq:lc-separation} and the radius and budget bounds
\eqref{eq:lc-radius-and-budget} as hypotheses; their derivation from the
failure assumption, given above, is not checked in Lean.}
...
choice with room to spare, not an optimality claim. A
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/scripts/check_erdos1041_angular_budget_closure.py}{program} evaluates
...
suffice to verify them. Its full run uses $18$ area-table levels,
...
A coarser run gives $X=664373027131/1000000000000$
...
pairs of the coarser run at $a=1$, rounded to six decimal places, are
...
\]
Thus the full certificate in fact gives the path conclusion for
$\mu\le529/1000$, without changing the geometric argument or its hypotheses.
This is an arithmetic consequence of the certified stopping time. We retain $13/25$ in the theorem and its scaling
...
Their derivations above are part of the analytic proof. No independent
review of this theorem is recorded. Prior art for
...
and the hyperbolic law of cosines.  A
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos1041/AngularBudgetLowCriticalClosure.md}{research note}
carries the full analytic argument, the table of certified weighted bounds
and the instructions for running the certificate. The stopping-time test
...
\end{corollary}
\evidenceremark{The Lean proof assumes
Theorem~\ref{res:low-critical-thirteen-twentyfifths} in degrees above two;
the degree-two case is checked without it.}
\begin{proof}
...
length at most $5.7$.
\end{theorem}
\evidenceremark{The Lean proof assumes the two-parameter bound (CF) below
in degrees $n\ge3$, which the proof obtains by averaging over levels and
value directions. The degenerate case, degree two and the numerical
constants are checked without that input.}
...
\]
\end{corollary}
\evidenceremark{The Lean proof assumes the bound (CF) from the proof of
Theorem~\ref{res:constant-factor-path}, for a component containing at least
$k_0$ roots; the rational inequalities are checked.}
...
\end{corollary}
\evidenceremark{The Lean proof assumes the averaging construction in the
proof of Theorem~\ref{res:constant-factor-path}, rerun with the
area--capacity inequality for the area of $C$; the rational cutoffs are
checked.}
...
Its proof is not available, and
...
\end{theorem}
\evidenceremark{The Lean proof assumes the two-component split at the
first critical level that the proof below constructs: two one-root
components whose closures meet at a critical point, each with boundary
length at most $\beta\rho$ and each joining its root to that point within
half its perimeter.}
...
The inputs of the construction in the proof of
Theorem~\ref{res:constant-factor-path} are the capacity identity for polynomial
...
\end{equation*}
\end{theorem}
\evidenceremark{The Lean proof assumes the analytic construction in the
proof below: the connector obtained through the square-root map, its
Bergman length bound (7), and the capacity bound for the area of the
two-sheeted component.}
...
\end{corollary}
\evidenceremark{The Lean proof assumes the same construction as
Theorem~\ref{res:critical-value-separation} for the second assertion.
Inequality~\eqref{eq:disk-family-coefficient} and every numerical step are
checked without it.}
...
This theorem is the finite alternation inequality; it does not construct
...
The sharpness assertion concerns the maximum of $|f|$ on the selected adjacent-root
...
The theorem selects two distinct indices.  It does not assert that the
corresponding complex root values are distinct.
...
The theorem gives length below $2$. Lean does not check the sharper bound
$2\sqrt{1-|h|^2}$ obtained in the proof.
...
cases.  Lean checks
\href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos1041/PaperWeightedRefinementsR10.lean\#L17}{the quadratic weighted inequality}.
The hypotheses permit repeated centres and zero weights;
...
polynomial is $(z-h)^n$. Lean does not check this classification.
...
Lean checks only the quadratic case; the analytic argument reaches
further. To estimate $G^p$ for $p>0$, we need an
...
moments. Lean does not check the higher-power conclusions in this
paragraph; the Lean proof of Theorem~\ref{res:critical-value-budget} has
exponent $2/(n-1)$.

\paragraph{Lean proofs of the Poisson and integration steps.}
The square-expansion proof above applies whenever $g$ is holomorphic on
the disc and continuous on its closure, with $|c|<1$. The Lean
...
\[
 ...
 \qquad |c_j|\le1,
\]
with equality if and only if all four centres vanish. Equality in this
linear form forces equality in the quadratic inequality, by the variance
identity above; Lean proves
\href{https://github.com/wcook04/plectis-erdos/blob/e6c2d8f77ac24753c5216a49f4daf7f7388b309f/lean/ErdosProblems/Erdos1041/PaperWeightedRefinementsR10.lean\#L17-L21}{that equality case}.
In particular, the
...
A
\href{https://github.com/wcook04/plectis-erdos/blob/a729f05c40398663fd586c9da487ad874d898639/research_corpus/Erdos1041/GlobalCriticalTwoNearestBudget.md}{research note} gives the same argument. Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/52d6c45ad203ba619cb5fe6ba485c0b5400519ea/lean/ErdosProblems/Erdos1041/PaperCompleteR20/TwoNearestPolynomial.lean\#L31}{strict distance bound from the critical polynomial}, including the root-disc reduction and the choice of the two nearest indices.
...
straight arms of total length at most $2$. Its proof is not available.
...
with uniquely smallest value modulus. A
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/MinimalHubArmBudgetRefutation.md}{computational record}
gives an unnormalised
...
Second, a
\href{https://github.com/wcook04/plectis-erdos/blob/f214a6b45528dc5eefe20ffadc35f2e981627d4c/research_corpus/Erdos1041/SeparatrixAggregateReduction.md}{second computational record}
reports numerical violations of
...
\cite[Proposition~A of the earlier note]{revision2026}; an
...
or the smaller exponent $2/7$ in Theorem~\ref{res:critical-value-budget}.
...
by evaluating $-f/f'$ at a critical endpoint. The corollary constructs no
trajectories and supplies no global monodromy theorem.
...
\noindent A finite union of these loci has empty interior. Adding a constant to
...
The exact numerical inequality and the resulting contradiction for a
tree-length bound also have a \href{https://github.com/wcook04/plectis-erdos/blob/e6c2d8f77ac24753c5216a49f4daf7f7388b309f/research_corpus/Erdos1041/CassiniTreeBudget.lean\#L117}{Lean proof}, in the repository's
research corpus, a library the checked build does not compile. The
level-length majorant is an explicit analytic input.
...
\end{theorem}
\evidenceremark{No Lean proof covers the whole theorem. Lean checks the
ray-disjointness, level-separation and saddle-scale steps of the proof; the
Morse, monodromy and strip statements are not checked.}
...
The complete argument is also written out in
...
Once a finite critical-value family is injective, Lean
\lword{Erdos1041/NewtonFlowRaySeparation.lean}{197}{exists_small_translation_separating_arguments}{proves}
that an arbitrarily small translation makes every value nonzero and
pairwise positive-ray separated.  Lean also
\lword{Erdos1041/NewtonFlowRaySeparation.lean}{287}{constant_perturbation_roots_in_unitDisk}{proves}
the explicit root-retention estimate: for a monic polynomial $f$ of degree
...
\]
Adding a constant leaves the difference of two critical values unchanged,
...
open unit disc. The factor $n+1$ in the Lean estimate is not
...
is given in the \href{\evidencerecordurl}{evidence record}.
\begin{center}\small
...
\bibitem{revision2026} W.~Cook / Plectis,
  ...
  unreviewed research note, Sections~1--3, together with the earlier
  note \emph{Structural obstructions}.
