...
Land has since posted a proof of irrationality, and Ringer a proof that
$\sum_n p_nB^{-n}$ is normal to base~$B$ for each integer $B\ge2$, both assuming
Kuperberg's uniform Hardy-Littlewood prime-tuples conjecture and both
formalised in Lean~\cite[Conjecture~1 and \S1]{land2026}%
\cite[Corollary~1.2]{ringer2026}.  Land's route constructs weighted
...
series by filling an interval of attainable values.  For a nonincreasing
...
Theorem~\ref{long251:res:sparse-rationalisation} uses both devices: equally
spaced digit choices in the nested covering, and a chain of factorial moduli
that preserves every fixed eventual congruence of the corrections.
...
Theorem~2~\cite{schlagepuchta2011}, which treats base-$b$ digit strings formed
by concatenating a slowly growing integer sequence under a rationality hypothesis,
and is a different object, and the sweep of the dyadic
...
has sum $y$.  The same nested covering proves~\cite[Lemma~5.1]{kovactao2024},
where the choices at each stage are reciprocals of the integers in an
interval.  Equation~\eqref{long251:eq:sparse-baseline} therefore realises
...
unconditional bound
$\liminf_n(p_{n+1}-p_n)\le600$~\cite[p.~385]{maynard2015}.  The large-gap
...
\bibitem{kovactao2024} V.~Kova\v{c} and T.~Tao,
  ...
  (2025), 572--608, arXiv:\href{https://arxiv.org/abs/2406.17593}{2406.17593}.
...
\bibitem{vandoornkovac2025} W.~van Doorn and V.~Kova\v{c},
  ...
  arXiv:\href{https://arxiv.org/abs/2509.24971}{2509.24971}.
...
\bibitem{ringer2026} S.~Ringer,
  ...
  telescoping, and normality}}, preprint dated 11~September~2026, with a Lean
  formalisation whose conditional endpoint theorems take Kuperberg's
  Conjecture~1.3 as an explicit hypothesis; submitted as a partial proof claim
  at \texttt{erdosproblems.com/251} on 13~September~2026.  Theorem~1.1
  classifies periodic local polynomial series in the prime gaps, each rational
  or normal, under a positive-comparison condition on prime-tuple counts that
  Conjecture~1.3 implies through a one-sided averaged hypothesis, and
  Corollary~1.2 deduces from Conjecture~1.3 the normality of
  $\sum_n p_nB^{-n}$ to base~$B$ for each integer $B\ge2$, hence for $B=2$ its
  irrationality.  Appendix~C
  proves unconditionally that $\sum_n p_nB^{-S_n}$ with
  $S_n=\sum_{j\le n}\lceil\log_B\log(j+3)\rceil$ is normal to base~$B$.
\end{thebibliography}
