...
The Lean proof uses a different construction: at each stage it changes
one coordinate, where the proof above changes a triple.  Like the
printed statement, it fixes the permitted set, the interval and every
congruence cutoff before the target.
...
displayed normalisation and has equivalent irrationality status; the
statement is left unproved there.
...
\noindent Far stronger lower bounds for large gaps are
known~\cite[Theorem~1]{fgkmt2018}; the
...
of the gaps} is all that is needed.
...
the same polynomial estimate for the omitted prime terms.  Both are exact
certificates, finitely many integer comparisons checked by the Lean
kernel; the first is small enough to print in full in
Appendix~\ref{long251:app:certificates}.  Both bounds are finite; neither
...
\noindent The Lean kernel evaluates the four inequalities itself, on the
same integers, after recomputing the primes below $10^4$ by trial
division; no compiled code is trusted.  Its analytic input is the same
enclosure, derived from the
\href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean\#L360}{polynomial
bound} $p_n\le1250(n+1)^4$.  The four integers $u,v,u',v'$ have $177$
and $178$ digits.
...
The Lean kernel checks every step of this proof and evaluates every
computation itself; no compiled code is trusted.  It recomputes $c$ and
$A$ from the definition of primality, scanning the integers below
$1\,023\,068$ in $250$ consecutive blocks and checking the prime count and
partial numerator at the end of each block.  It decides primality by trial
division below $1012$.  On $[1012,1013^2)$, which contains the rest of the
scan because $1013^2=1\,026\,169$, it uses the
...
then decides the four comparisons, together with $10^{59}<2^{196}$ and
$10^{23}<2^{78}$; the enclosure and the determinant step are proved in
general form.  Lean proves the
...
and derives the theorem as stated from it, using only Lean's three
standard axioms.

A second, independent certificate was computed outside Lean by an
integer-arithmetic program that uses no floating-point arithmetic.  It
...
$1\,200\,000$; the added term bounds the omitted terms by
...
$L2^{-80000}\le\Pi\le U2^{-80000}$.  The continued fractions of these two
endpoints agree in their first $23369$ partial quotients, and their last two
common convergents, ordered as $s/t<s'/t'$, satisfy
\[
 s't-st'=1,\qquad
...
The constants $2^{39997}$ and $10^{12040}$ in the statement come from the
original computation, which used the best-approximation property of
convergents \cite[\S6, Theorems~16 and~17]{khinchin1964}; Short gives an
accessible statement and proof~\cite[Theorem~1.1]{short2009}.  Its
\href{https://github.com/wcook04/plectis-erdos/blob/dd6cc708f650f061de06594eba6ba43878f4de31/research/experiments/erdos251/receipts/certified-cf.json}{recorded output}
lists the generating program and its digest and a last convergent whose
denominator has bit length $39998$, hence is at least $2^{39997}$; the
program also checked its arithmetic against the known expansions of $e$
and $\pi$.  That bit length describes one convergent only; the bounds for
every rational in the enclosure come from the certified sums $v+v'$ and
$t+t'$ above, and $2^{39997}>10^{12040}$ is a separate integer comparison.
...
\noindent Hence a property shared by every allowed perturbation cannot by
itself force irrationality, since one such perturbation has a rational sum;
this consequence is not stated separately in Lean.  Lean also checks the construction at the actual consecutive prime gaps,
for every $M\ge1$ and every $K$: the
\href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos251/BoundedPerturbationCountermodel.lean\#L208}{perturbed
prime-gap series is rational}, and each perturbed gap lies in
$[g_n,g_n+M]$ and is
\href{https://github.com/wcook04/plectis-erdos/blob/3d6d938d696fed0fb71dd55115a18a73738ff223/lean/ErdosProblems/Erdos251/BoundedPerturbationCountermodel.lean\#L234}{congruent
to $g_n$ modulo $M$}.  For each fixed $k\ge1$, the sum of $k$ consecutive gaps
...
$P_n\sim n\log n$.
\end{corollary}
\evidenceremark{The Lean proof assumes Schlage-Puchta's Lemma~4
\cite{schlagepuchta2011} and the prime number theorem.}
...
\end{proof}
These conclusions follow from the sparse theorem, Schlage-Puchta's
Lemma~4 and the prime number theorem.  In fact, all conclusions also hold for every
...
\noindent Taking $K_0=\tfrac52$ and $K_n=2n+2$ for $n\ge1$ gives
...
tails.  Both compute the tails in double-precision arithmetic with no
bound on the accumulated rounding error, so neither a count nor an
individual hit is exact; a bound for the omitted infinite tail would not
by itself bound that error.  Neither search says anything beyond its
range.  The exact certificates are
Proposition~\ref{long251:res:finite-smallpair} and the two denominator
bounds of Section~\ref{long251:sec:denominator-floor}, all checked by the
Lean kernel.  The searches have not been rerun for this edition.
...
$0.13148$ in the last.
This finite observation establishes no asymptotic counting law and no
occurrence beyond every cutoff.  The
\href{https://github.com/wcook04/plectis-erdos/blob/dd6cc708f650f061de06594eba6ba43878f4de31/research/experiments/erdos251/receipts/adjacent-mismatch.json}{search output}
is public.
...
check in floating-point arithmetic.  The scan supplies neither arbitrary
moduli nor witnesses beyond every prescribed cutoff.  Its
\href{https://github.com/wcook04/plectis-erdos/blob/dd6cc708f650f061de06594eba6ba43878f4de31/research/experiments/erdos251/receipts/free-pair.json}{search output}
is public.
The \href{https://github.com/wcook04/plectis-erdos/blob/dd6cc708f650f061de06594eba6ba43878f4de31/research/experiments/erdos251/README.md}{computation guide}
gives the programs, dependencies and commands to rerun both searches and
the original continued-fraction calculation of
Section~\ref{long251:sec:denominator-floor}.
Appendix~\ref{long251:sec:xr-corrections} corrects the stated
justification of the tail bound in the earlier $4096$-bit program.
...
environment; the margin marks link each result to its Lean proof, and the
\href{\evidencerecordurl}{evidence record} lists every declaration and
check.  Formal checking does not establish novelty or attribution.  The
programs for the searches of Section~\ref{long251:sec:measurements} are
listed in the
\href{https://github.com/wcook04/plectis-erdos/blob/dd6cc708f650f061de06594eba6ba43878f4de31/research/experiments/erdos251/README.md}{computation guide}.
For this edition the program in Appendix~\ref{long251:app:certificates},
which checks the adjacent pair and the bound $2^{589}$, and the
independent program of Section~\ref{long251:sec:denominator-floor} for
the $80000$-bit enclosure were rerun; the frequency scans were not.
...
and the transfer of nonconcentration come from earlier working materials
of this project, which were not an independent mathematical review.
...
inequality used in Theorem~\ref{long251:res:denominatorfloor}.  The four
integers are the certificate that the Lean kernel checks for that theorem;
their use here requires no floating-point approximation.  Adjacent quoted strings are concatenated by
Python.
{\footnotesize
\begin{verbatim}
...
The margin marks link each result to its Lean proof, and the
\href{\evidencerecordurl}{evidence record} lists every declaration.  The
links after selected proofs identify further formal definitions and steps
at fixed revisions and line numbers.

The summation-by-parts identity and the polynomial prime bound concern the
actual primes.  The recurrence results apply to arbitrary integer
coefficients and, as stated, to rational or real sequences; substituting
the actual prime-gap tail requires its convergence and boundary condition.
...
and construct no new primes.

Three results assume published inputs that the Lean development does not
prove.  Theorem~\ref{long251:res:sparse} assumes Schlage-Puchta's
Lemma~4 \cite[Lemma~4, pp.~5--6]{schlagepuchta2011}; for a fixed nonzero
integer polynomial in a fixed block of consecutive prime gaps, it states
that the set of indices where the polynomial vanishes has density zero.
Corollaries~\ref{long251:res:nonconc-primes}
and~\ref{long251:res:jointcountermodel} assume that lemma and the prime
number theorem in the form $p_n\sim n\log n$~\cite[Chapter~6]{mv2007}.
What each statement adds to these inputs is checked, including the
estimate that the corrected sequence of
Corollary~\ref{long251:res:jointcountermodel} differs from the prime gaps
at $O_\varepsilon(n/\log\log n)$ indices below $n$, which uses no
...
\paragraph{An earlier exclusion record.}
An earlier internal record gave a denominator exclusion at $10^{602}$.
Theorem~\ref{long251:res:cfexclusion}, whose certificate the Lean kernel
checks, supersedes it; the original continued-fraction program was not
...
printed argument; the Lean proof covers the rational case stated in the
theorem.
