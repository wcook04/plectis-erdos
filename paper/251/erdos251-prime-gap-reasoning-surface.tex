...
\]
The identity follows from summation by parts and the elementary polynomial
bound $p_n\le1250(n+1)^4$ proved in Appendix~\ref{long251:app:prime-bound}.
The substantive limitation comes from rationalising perturbations: bounded
residue-preserving changes to the actual gaps can make their dyadic value
rational.  The record recovers this construction, the exact tail criteria,
and the finite denominator exclusions; the identity provides their common
normalisation.
...
Assuming Kuperberg's uniform Hardy--Littlewood prime-tuples
conjecture~\cite[Conjecture~1.3]{kuperberg2023}, Land has since posted a
research draft dated 5 September 2026 that proves
irrationality~\cite[Conjecture~1 and Theorem~2]{land2026}, and Ringer a
preprint dated 11 September 2026 that proves $\sum_n p_nB^{-n}$ normal to
base~$B$ for each integer $B\ge2$~\cite[Corollary~1.2]{ringer2026}; both
arguments are formalised in Lean.  Land's route constructs weighted
...
unconditional statement either argument would need.
...
$1<q_1\le q_2\le\cdots$ be integers satisfying, for some $k>0$, the article's
printed condition (5), $q_n>o(n/\log^{k}n)$. We retain that nonstandard
asymptotic typography without imposing a modern quantified interpretation.
Under its stated hypotheses, the article proves that
$\sum_{n\ge1}p_n/(q_1\cdots q_n)$
...
and strict increase is not assumed. The endpoint $q=1$ is attained: if
$G_n=\prod_{j=1}^{n}(p_j+1)$ then $p_n/G_n=1/G_{n-1}-1/G_n$, so the
corresponding series telescopes to $1$. Neither this historical classification
nor Kova\v{c}'s counterexample addresses the fixed-denominator dyadic
series studied here.
...
and~\ref{long251:res:sparse-rationalisation} below produce rational perturbed
series by filling an interval of attainable values.  The first uses only a
binary expansion.  The second uses variable-digit covering, which is
classical: for nonincreasing weights $r_i\to0$ and digit bounds $k_i$, Fridy
showed that every number in $(0,\sum_ik_ir_i)$ is a sum $\sum_ia_ir_i$ with
integers $0\le a_i\le k_i$ exactly when $r_n\le\sum_{i>n}k_ir_i$ for every
$n$~\cite[Lemma, p.~194]{fridy1966}.  For a nonincreasing
summable sequence of positive terms, the sums of subseries form a finite union
of closed intervals exactly when every late term is at most the sum of all
later terms, a criterion going back to Kakeya~\cite[Remark~3.1]{kovactao2024}.
Kova\v{c} and Tao show by nested covering that if $\sum_n1/\min J_n<\infty$
and, from some stage on, consecutive reciprocals of the integers in $J_n$ are
no further apart than the combined spread of the reciprocals over all later
intervals, then the sums $\sum_n1/x_n$ with $x_n\in J_n$ contain an interval.
They deduce that bounded perturbations of the denominators $2^n$ can give a
rational sum, for instance $\sum_{n\ge1}1/(2^n+b_n)=3/4$ with every
$b_n\in\{1,\ldots,5\}$, answering a question of Erd\H{o}s and Graham on
irrationality sequences~\cite[Lemma~5.1, Theorem~2.5 and \S5]{kovactao2024}.
Van Doorn and Kova\v{c} combine dense filling with a divisibility chain among
distinguished terms that eventually absorbs every positive integer, so that
finite reciprocal sums take every rational value in an
interval~\cite[Lemma~7 and Proposition~8]{vandoornkovac2025}.
Theorem~\ref{long251:res:sparse-rationalisation} uses the same two devices:
equally spaced digit choices in a nested covering, whose overlap condition is
Fridy's inequality without his monotonicity assumption, and a chain of
factorial moduli that preserves every fixed eventual congruence of the
corrections.
...
\subsection{Paired corrections and cumulative congruences}
...
\begin{theorem}[sparse rationalisation with cumulative congruences]
...
every integer $q\ge1$, there is an $N_q$ such that
\[
 q\mid e_n\quad\hbox{and}\quad q\mid\sum_{i<n}e_i
 \qquad(n\ge N_q).
\]
For every $0<\varepsilon<1$, the construction can instead be chosen with
...
\noindent This theorem has an ordinary proof below.  The general interval
and pair algebra is covered by the elementary Lean core; the explicit
schedule and its density and size estimates are not claimed to be
kernel-checked.
...
has sum $y$.  This is the variable-digit covering of Fridy's lemma
\cite[Lemma, p.~194]{fridy1966}, whose condition is the overlap
$F_{j+1}\ge w_j$; Fridy assumes nonincreasing weights, and the argument above
uses only the overlap.  Kova\v{c} and Tao use the same nested covering
in~\cite[Lemma~5.1]{kovactao2024}, where the choices at each stage are
reciprocals of the integers in an interval.
Equation~\eqref{long251:eq:sparse-baseline} therefore realises
every $r$ in the fixed interval $I=(A+\beta,A+\beta+F_0)\subset(A,\infty)$.
...
The pair identities, modular repair, and conditional interval filling are the
elementary core in
\path{lean/ErdosProblems/Erdos251/SparseRationalisationCore.lean}.
Lean checks the theorem itself, including upper Banach density and block-law
transfer:
...
The schedules
\eqref{long251:eq:sparse-general-schedule}--\eqref{long251:eq:sparse-polylog-schedule}
belong to the ordinary proof above, and the theorem has no Comparator entry.  Applied to
...
unconditional bound
$\liminf_n(p_{n+1}-p_n)\le600$~\cite[Theorem~1.3, p.~385]{maynard2015}.  The large-gap
...
Propositions~\ref{long251:res:explicit-remainder} and~\ref{long251:res:finite-smallpair} and the
continued-fraction exclusion of Theorem~\ref{long251:res:cfexclusion} rest on
exact arithmetic replay.  The nonconcentration transfer of
Theorem~\ref{long251:res:nonconcentration} with its corollary, the sparsity
statement of Theorem~\ref{long251:res:sparse}, and the construction of
Theorem~\ref{long251:res:polignacfail} are ordinary mathematical proofs.  None of
these carries a Lean declaration.  The other linked declarations are checked by
the kernel at the source revisions named in their links.
...
placeholders and no project-defined axioms; Lean
...
Theorem~\ref{long251:res:cfexclusion}, together with their generating programs,
are public at the immutable revision linked there.  The
...
records dependencies, replay commands, source provenance, and the exact versus
floating-point evidence boundary.
...
\textbf{Reader position.} 4 of 70; display band: front door.  Formal editorial disposition: promote.  These are separate classifications.

\textbf{Reader entry.} Exact finite summation by parts, the unconditional infinite prime-gap identity, and irrationality equivalence.
...
\textbf{Reader position.} 11 of 70; display band: major result.  Formal editorial disposition: merge.  These are separate classifications.

\textbf{Reader entry.} For any integer-digit dyadic tail recurrence T(N+1) = 2*T(N) - g(N+1), the rational h-shift is integral exactly when the reduced denominator of T(N) divides 2\textasciicircum{}h - 1, equivalently when 2\textasciicircum{}h is congruent to 1 modulo that denominator. For the real recurrence, the load-bearing normal form says that irrationality of T(0) is equivalent to every positive tail shift being nonintegral; the eventual-integrality equivalence and denominator/order laws are supporting mechanism evidence.
...
\textbf{Result-atom population.} 10 of 681 public coordinates. The public atom catalog groups them under this family.
...
\textbf{Reader position.} 27 of 70; display band: mechanism.  Formal editorial disposition: merge.  These are separate classifications.

\textbf{Reader entry.} Block identities and exact denominator criteria classify rationality through integral positive tail shifts.
...
\textbf{Reader position.} 51 of 70; display band: frontier.  Formal editorial disposition: hold.  These are separate classifications.

\textbf{Reader entry.} The exact finite carry identity and the two non-eventual-periodicity statements expose the coefficient-only barrier: irregularity and growth of a coefficient stream do not by themselves force irrationality, while the synthetic carry stream remains distinct from the actual prime-gap stream.
...
\textbf{Reader position.} 60 of 70; display band: technical support.  Formal editorial disposition: merge.  These are separate classifications.

\textbf{Reader entry.} Odd rational denominators give totient-length integral shifts, and integrality propagates through the recurrence.
