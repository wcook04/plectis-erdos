...
between zero and the remaining capacity, which tends to zero.  The same
nested covering proves~\cite[Lemma~5.1]{kovactao2024}, where the choices at
each stage are reciprocals of the integers in an interval; here they are the
$D_j+1$ equally spaced values $dw_j$.  Take
$I=(A+\beta,A+\beta+F)$.
...
and gaps are unbounded.  Land posted a proof of irrationality~\cite{land2026},
and Ringer a proof that $\sum_n p_nB^{-n}$ is normal to base~$B$ for each
integer $B\ge2$~\cite{ringer2026}, both assuming Kuperberg's uniform
Hardy-Littlewood prime-tuples conjecture and formalised in Lean.  Nothing in this note
...
Proposition~\ref{res:sparserationalisation} has recent precedents for sums of
...
The identity is the already-known summation-by-parts (Tao on the
erdosproblems.com/251 thread, 7~October~2025); this checkout checks it
and does not claim priority.
...
the prime number theorem remains useful
...
of every fixed size; \cite[p.~385]{maynard2015} gives the explicit
unconditional bound $\liminf_n(p_{n+1}-p_n)\le 600$.  The large-gap theorem of
Ford, Green, Konyagin, Maynard and Tao
\cite[Theorem~1, p.~66]{fgkmt2018} gives an effective lower bound for the
...
\bibitem{ringer2026} S.~Ringer,
  ...
  telescoping, and normality}}, preprint with a Lean formalisation,
  11~September~2026, Corollary~1.2; submitted as a partial proof claim at
  \texttt{erdosproblems.com/251} on 13~September~2026.
\end{thebibliography}
