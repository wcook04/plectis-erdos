...
\begin{abstract}
A sparse perturbation of the actual prime gaps can make their dyadic sum
rational while preserving every fixed eventual congruence of the gaps and
cumulative sums, and the empirical distributions of all unnormalised gap
blocks of length $o(\log\log X)$.  For every $0<\varepsilon<1$, its
corrections can be bounded by $(\log n)^\varepsilon$ eventually.  The
construction uses pairs with fixed unweighted total and variable dyadic
weight.  Lean checks its pair identities and the proposition it proves, and
the scheduling argument written here is ordinary mathematics.  The simpler positive even word
$a_n=2(n^2+4n+2)$ has complete dyadic tails $U_N=2(N+4)^2$ and rational sum
$\sum_{j\ge1}a_j2^{-j}=32$.  We identify the actual complete prime-gap tails
and classify rationality by integrality of their shifts.  Two adjacent shifts in $(-1,1)$ with unequal
...
$n\log n$ scale that the prime number theorem gives for
$p_n$~\cite[Chapter~6]{mv2007}.  Taking $M$ even retains parity as well.
Thus a bounded
...
A single changed coefficient can alter every later cumulative residue.
We therefore pair corrections: changing their allocation changes the dyadic
value while leaving their unweighted total fixed.  Sparsity has a separate
job.  Most short blocks then meet no correction at all, which preserves
their empirical distributions without requiring the corrected positions
to be prime.
...
dividing both $e_n$ and $\sum_{i<n}e_i$.  For the actual prime gaps and the
specific choice $f(n)=(\log(n+3))^\varepsilon$, $0<\varepsilon<1$, one
can in addition require
...
Put $D_j=2^{s_j}-1$.  Starting from $C_0=0$, define
\[
...
size bound and summability of the correction.  The spacings tend to
infinity, so the union of triples has upper Banach density zero.
...
\]
The digit intervals therefore overlap.  Greedy selection fills the complete
interval $[0,F]$, where $F=\sum_jD_jw_j>0$: at each step the remainder is
between zero and the remaining capacity, which tends to zero.  This is the
variable-digit covering of Fridy's lemma on generalised bases, whose
condition $r_n\le\sum_{i>n}k_ir_i$ is the overlap inequality above
\cite[Lemma, p.~194]{fridy1966}; Fridy assumes nonincreasing weights, and the
argument here uses only the overlap.  Kova\v{c} and Tao use the same nested
covering in~\cite[Lemma~5.1]{kovactao2024}, where the choices at each stage
are reciprocals of the integers in an interval; here they are the $D_j+1$
equally spaced values $dw_j$.  Take
$I=(A+\beta,A+\beta+F)$.
...
The complete argument and the all-index congruence check are expanded in
the companion record, subsection ``Paired corrections and cumulative
congruences''.  The pair, buffer and conditional interval-filling identities
are the Lean-checked core; the scheduling and density arguments are ordinary
mathematics.
\end{proof}
...
The first gaps are
\lword{Erdos251/PrimeGapDyadicTail.lean}{47}{primeGap0_zero}{$1$},
\lword{Erdos251/PrimeGapDyadicTail.lean}{51}{primeGap0_one}{$2$},
$2,4,2,4,2,4,6,2$; only $g_0$ is odd.
...
and gaps are unbounded.  Assuming Kuperberg's uniform Hardy--Littlewood
prime-tuples conjecture~\cite[Conjecture~1.3]{kuperberg2023}, Land posted a
research draft dated 5 September 2026 that proves
irrationality~\cite[Theorem~2]{land2026}, and Ringer a preprint dated
11 September 2026 that proves $\sum_n p_nB^{-n}$ normal to base~$B$ for each
integer $B\ge2$~\cite[Corollary~1.2]{ringer2026}; both arguments are
formalised in Lean.  Nothing in this note
assumes a prime-tuples hypothesis, and Erd\H{o}s Problem~\#251 remains open
unconditionally.  Interval filling of the kind used in
Proposition~\ref{res:sparserationalisation} goes back to variable-digit
expansions: for nonincreasing weights $r_i\to0$ and digit bounds $k_i$,
Fridy showed that every number in $(0,\sum_ik_ir_i)$ is a sum
$\sum_ia_ir_i$ with integers $0\le a_i\le k_i$ exactly when
$r_n\le\sum_{i>n}k_ir_i$ for every $n$~\cite[Lemma, p.~194]{fridy1966}.  It
also has recent precedents for sums of
unit fractions.  Kova\v{c} and Tao use it to show that bounded perturbations
of the denominators $2^n$ can give a rational sum, for instance
$\sum_{n\ge1}1/(2^n+b_n)=3/4$ with every $b_n\in\{1,\ldots,5\}$, answering a
question of Erd\H{o}s and Graham on irrationality
sequences~\cite[Lemma~5.1, Theorem~2.5 and \S5]{kovactao2024}.  Van Doorn and
Kova\v{c} combine dense filling with a divisibility chain among distinguished
terms that eventually absorbs every positive integer, so that finite
reciprocal sums take every rational value in an
interval~\cite[Lemma~7 and Proposition~8]{vandoornkovac2025}.  The
corrections in Proposition~\ref{res:sparserationalisation} use a chain of
factorial moduli with the same absorption property to preserve every fixed
eventual congruence.  The detailed historical comparisons and their
...
We first record the recurrence-level conclusions.  Modulo one the recurrence
is multiplication by two, and the classifications below use only reduced
denominators and Euler's congruence $d\mid2^{\ph(d)}-1$ for odd $d$.  They
apply to arbitrary integer digits and therefore separate the finite algebra
from the prime-specific input still missing in Problem~\#251.
...
\noindent The statement is \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/OrderLatticeDiagonal.lean#L153}{irrational initial iff all lcm diagonal nonintegral}.
...
\noindent The rational normal form is \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/AffineShiftEscape.lean#L113}{adjacent small mismatch iff signed two window}.
The same equivalence for real states is
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L135}{real signed two window}.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L241}{affine circularity bundle}.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L27}{polynomial countermodel}.
...
\begin{proof}
Expand the difference sum and shift its first index.  All interior
coefficients of $P(i)$ become $2^{-i-1}$; the remaining terms are $P(0)$
and the displayed endpoint $-P(n)2^{-n-1}$.  This also covers $n=0$.
\end{proof}

\noindent This is the
\lword{Erdos251/PrimeGapDyadicTail.lean}{138}{dyadicPartialSumQ_eq_start_add_differences}{finite
summation by parts}.
...
The displayed partial sums with denominators $2^i$ are twice these, by the
...
The identity is summation by parts specialised to the primes; Tao posted this
reduction on the problem's forum thread on 7~October~2025
\cite[comment of 7 October 2025]{erdosproblems251thread}.
...
\]
for the
\lword{Erdos251/PrimeGapDyadicTail.lean}{183}{primeDyadicTerm}{prime dyadic
terms} and the
\lword{Erdos251/PrimeGapDyadicTail.lean}{192}{primeGapDyadicTerm}{prime-gap
dyadic terms}, so that
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L47}{infinite prime gap identity} rather than a hypothesis carried by the
reader.
...
\]
The corresponding
\lword{Erdos251/PrimeGapDyadicTail.lean}{188}{primeDisplayedDyadicTerm}{zero-based series
with denominator $2^n$} is
$4+2\sum_{n\ge0}v_n$ and has the same irrationality status.
Termwise it is twice the normalised series, by the
\lword{Erdos251/PrimeGapDyadicTail.lean}{196}{primeDisplayedDyadicTerm_eq_two_mul}{infinite
factor-of-two identity}.
\end{corollary}
...
declaration \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L56}{irrationality reformulation}.
...
infinite prime-gap identity}
removes the summability hypothesis;
the prime number theorem is therefore needed neither for convergence nor for
this identity.  It enters this note only through the $n\log n$ scale of the
primes~\cite[Chapter~6]{mv2007}.  The open content is
...
\begin{proof}
Induct on $h$ using the recursions for $T$ and $B$, then subtract $T_N$
to obtain the shift identity.
\end{proof}

\noindent The first identity is the
\lword{Erdos251/PrimeGapDyadicTail.lean}{653}{tail_iterate_eq_pow_mul_sub_block}{iterated
block identity}; the second is the
\lword{Erdos251/PrimeGapDyadicTail.lean}{667}{tailShift_eq_scaled_sub_block}{scaled
shift identity}.  Both are also assembled in
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L68}{block identity}, and the real-state form is
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L76}{real block identity}.

The shifts themselves satisfy the
\lword{Erdos251/PrimeGapDyadicTail.lean}{563}{tailShift_succ}{shift step identity}
\[
...
subtracting an integer does not change integrality, by the
...
The Euler multiplier itself is the
\lword{Erdos251/PrimeGapDyadicTail.lean}{695}{ratIntegral_totientMultiplier_of_odd_den}{totient
multiplier at odd denominator}.
\end{proof}
...
an integer combination of an integer and two digits; this is the
\lword{Erdos251/PrimeGapDyadicTail.lean}{888}{tailShift_integral_succ}{one-step
propagation}, and the claim follows by induction on $k$.
\end{proof}
...
\noindent Denominator factorisation and cancellation are the
\lword{Erdos251/PrimeGapDyadicTail.lean}{723}{ratIntegral_scaled_of_den_eq_pow_two_mul_odd}{power-of-two/odd
denominator scaling}
and the
\lword{Erdos251/PrimeGapDyadicTail.lean}{756}{ratIntegral_multiplier_tail_at_twoExponent}{multiplier
at the two-adic exponent},
packaged in the
\lword{Erdos251/PrimeGapDyadicTail.lean}{830}{rationalPrimeGapTailShift_eventuallyIntegral_of_den_eq}{fixed-denominator
theorem}; the quantified conclusion is the
...
indexed form is \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L84}{eventual integral positive shift}.
...
actual-tail state}
obeys the
\lword{Erdos251/PrimeGapDyadicTail.lean}{527}{rationalPrimeGapTailState_recurrence}{tail
recurrence},
and a hypothetical rational value of the gap series supplies
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L94}{rationality classification}.
...
\]
are already identified, with no rationality hypothesis.
The polynomial prime bound gives convergence, and
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L267}{rational small pair bundle}.
...
\begin{proof}
The shift identity holds over $\R$, and the only integer in $(-1,1)$
is still zero.  Thus the proof of Theorem~\ref{res:smallpair} applies
without change, including its cofinal conclusion.
\end{proof}

\noindent Corollary~\ref{res:smallpair-real} is
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L282}{real small pair bundle}, and the resulting implication for the
actual prime series is \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L298}{real small pair prime endpoint}.  These live
...
of the actual gaps} and the
\lword{Erdos251/PrimeGapDyadicTail.lean}{1023}{primeGap0_not_eventually_periodic}{non-eventual
periodicity} of the gap sequence.  The statement at the index convention displayed above is
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L175}{prime gaps not eventually periodic}.  Combining nonperiodicity with eventual strict smallness of one
...
For a bounded canonical base-two digit expansion, rationality forces
eventual periodicity.  Prime gaps are unbounded coefficients, so that
criterion does not apply.  The following telescoping construction shows
explicitly how nonperiodic coefficients can have a rational sum.
...
Problem~\#251 is open.  Unconditional
convergence of both dyadic series, the
\lword{Erdos251/PrimeGapDyadicTail.lean}{427}{tsum_primeDyadicTerm_eq_two_add_primeGap_unconditional}{unconditional
infinite prime-gap identity},
and the
\lword{Erdos251/PrimeGapDyadicTail.lean}{510}{exists_rationalPrimeGapTailState_representation_of_not_irrational}{real-to-rational
scaled-tail representation}
are proved.  Every rational candidate supplies one positive fixed shift
which is integral at every sufficiently late tail index: the
\lword{Erdos251/PrimeGapDyadicTail.lean}{853}{rationalPrimeGapTailShift_eventuallyIntegral}{eventual
...
Forward propagation is Theorem~\ref{res:propagate}.
...
simultaneous integrality; Theorem~\ref{res:smallpair} is the
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperTailBoundsR7.lean#L203}{not real integral of error bound}, and the proposition itself is
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperTailBoundsR7.lean#L261}{cofinal escape of finite truncation}.  For instance, if
...
series is \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperTailBoundsR7.lean#L273}{irrational prime series of finite truncation}.
...
we make no progress on it here.  The following results on prime gaps address a
...
\cite[Theorem~1]{fgkmt2018} gives an effective lower bound for the
largest single consecutive-prime gap below $X$.  None of these results supplies the joint dyadic distribution
...
approach in the companion record uses the growth $p_n\asymp n\log n$
\cite[Chapter~6]{mv2007} to obtain
\[
...
The three finite-computation receipts and their generating programs are public
in the
...
at an immutable revision.  That guide records dependencies, replay commands,
source provenance, and the exact versus floating-point evidence boundary.

Lean~4~\cite{lean4} checks each proof term against the fixed version of the
mathlib library~\cite{mathlib}, and the sources linked here contain no proof
placeholders and no
...
\texttt{SparseRationalisationCore.lean}.  Lean checks the proposition itself.
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparseAmbientR9.lean#L217}{Arbitrary word sparse rationalisation}
gives, for every $f\to\infty$, a support in $[K,\infty)$ of upper Banach density
zero, an interval above $A$, and a correction realising each point of that
interval;
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparsePaperR11.lean#L53}{arbitrary word sparse rational target}
realises a rational value above $A$; and
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparsePaperR11.lean#L78}{polylogarithmic word interval}
gives the polylogarithmic clause, with block limits
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/GrowingBlocksR11.lean#L139}{growing block TV} and
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/GrowingBlocksR11.lean#L152}{growing block tests uniform}.
The explicit three-coordinate schedule in the construction is the ordinary
proof, expanded in the companion record, subsection ``Paired corrections and
cumulative congruences''.  The proposition has no Comparator entry.
