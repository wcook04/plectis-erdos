...
%
% PROVENANCE: superseded local copy.  The published successor is
% paper/problem-note-preamble.tex in the public release repository
% wcook04/plectis-erdos at 8df0dda280 (release/final-20260905),
% which additionally loads longtable.  Builds in this directory use the public
% copy through the tectonic search path; do not export or promote from here.
%
...
% ---------------------------------------------------------------------------
\newcommand{\commit}{8e58c395e9807ef3374d7365c62cf04d20ee6331}
\newcommand{\commitshort}{9e231ce4371f}
...
\usepackage{longtable}  % the notes' declaration tables; loaded here so the assembled records inherit it
...
% ---------------------------------------------------------------------------
% Statement-level proof links.  \commit above predates the move of the Lean
% roots under lean/, and the exact endpoints the coverage ledger binds were
% written after it, so a second immutable pin carries them.  \ledgercommit is
% the public revision whose Lean sources the ledger resolved its declarations
% against; every file it names is byte-identical to that revision.  The note
% printed under each statement is generated from that ledger and verified by
% scripts/check_generated_lean_notes.py.  Do not edit a \leannote by hand:
% regenerate it.
% ---------------------------------------------------------------------------
...
\newcommand{\lproof}[3]{\href{\ledgerbase/lean/#1\#L#2}{\leanlabel{#3}}}
...
% The mathematical notes remain standalone. These links supply optional
% system context without making a systems manuscript authority for a theorem.
\newcommand{\companionpapercontext}{%
  \par\smallskip\noindent\emph{Companion system context.}
  The \papersectionlink{claim-faithful-publication-systems-paper.pdf}%
  {systems-trust}{claim and trust boundary},
  \papersectionlink{cold-clone-to-proof-receipt.pdf}%
  {cold-clone-authority}{cold-clone route to proof authority}, and
  \papersectionlink{open-source-mathematics-strategy.pdf}%
  {strategy-protocol}{public contribution protocol} are described in sibling
  papers. Those descriptions do not change the mathematical status of this
  note.}

% Compact opening card for the separately stated Comparator theorem selected
% from each problem note.  The prose supplied by each note owns the
% contribution and relevance judgements; the shared final sentence states the
% mechanical assurance boundary once and points to the public evidence owner.
\newcommand{\externalreviewobject}[3]{%
  \begin{quote}\small
