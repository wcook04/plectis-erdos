...
for the integer in $Z_m=mZ_{m-1}+1-b_m$.  The central formal equivalence is
\[
  \Ser\notin\mathbb Q
...
$m=300000$; the corresponding Lean theorem proves that
every representation $\Ser=a/q$ with $q>0$ has $q\ge300000$.

The carry recurrence also closes a tempting two-step route.  Under two
consecutive unit carries, the exact cleared width-one window factors through
the same positive two-step normalizer in its numerator and denominator.
Cancelling that factor leaves only the universal reduced-fraction inequality
$0<U_{m+2}\le V_{m+2}$.  Thus prime-power or quotient-gcd information
extracted only after assuming the adjacent pair cannot exclude it.  This is
an exact no-go theorem, not a proof that adjacent unit carries never occur.
...
zero.  The lcm obstruction also has a quantitative endpoint.  If a positive
moment $M$ is divisible by every channel modulus through $D=2t^2$ and
$M<(R+1)!-1$, then, for $t\ge2^{32}$,
\[
  3t^3<2(R+1).
\]
Thus a family satisfying these hypotheses cannot have
$2(R(t)+1)\le3t^3$ eventually.  This is a checked cubic radius obstruction,
not a construction of the required moments or channel cancellations.
Separately, endpoint congruences turn sufficiently large residues or
disagreement between two projections into finite exclusions.

There is also a sharp finite breakpoint hidden inside this channel picture:
within one quotient band $kd\le i_j<(k+1)d$, every factorial extracts the same
power $(d!)^k$.  Thus a vanishing $d$-channel forces the factorial moment to
vanish; in particular, a nonzero moment with all indices at least $d$ must
cross the first breakpoint and include some index at least $2d$.  This is a
finite support obstruction, not a cofinal construction.

The evidence route is explicit rather than heuristic: the cofinal carry
equivalence is kernel-checked at
\lword{Erdos68/FactorialZeroPlateau.lean}{971}{irrational_factorialGapSeries_iff_cofinal_nonunit_carries}{the carry equivalence};
the quotient-band obstruction at
\lword{Erdos68/ChannelBreakpointRigidity.lean}{91}{factorialMoment_eq_zero_of_channelNumerator_eq_zero_band}{the band-cancellation theorem};
and the remote finite reduction at
\lword{Erdos68/PrimeUnitTranslator.lean}{1657}{exists_remote_factorialGrid_primeTranslator_reduction}{the remote construction}.
The first identifies the exact missing cofinal input, while the latter two
remain finite mechanisms and do not supply it.
...
\externalreviewobject{The paper proves exact carry and divisibility
equivalences for the factorial-gap series and records a finite denominator
exclusion through $q<300000$, whose arithmetic certificate is checked outside
Lean.}{The equivalences identify the missing cofinal input without supplying
it.  A finite denominator exclusion does not change the quantifier, and
Problem~\#68 remains open.}{Comparator selects both exact equivalences:
cofinal failure of the strict factorial-successor divisibility test and
cofinally many non-unit carries.  The mixed Lean-plus-certificate denominator
computation is reported separately.}

For release review, the selected Comparator rows are exact.  The declaration
\texttt{irrational\_factorialGapSeries\_iff\_cofinal\_strictFacTopRat\_misses}
states
\[
  \operatorname{Irr}(\Ser)\iff(\forall B)(\exists m>B)\;
  m\nmid Z_m,
\]
with $Z_m=\operatorname{strictFacTopRat}(\operatorname{factorialGapPrefix}(m),m)$;
the declaration
\texttt{irrational\_factorialGapSeries\_iff\_cofinal\_nonunit\_carries}
states the equivalent condition $(\forall B)(\exists m>B)\;b_m\ne1$.
Both rows expose the missing cofinal input and neither supplies it, so the
irrationality boundary for Problem~\#68 is unchanged.
...
termination theorem}; they hold for every real $x$,
not only for $\Ser$.
...
The primitive lcm divisibility after cofactor removal is
...
\noindent The theorem holds for every prime, not merely for sampled primes.  Its use is
...
The Mathlib-only Comparator surface
\texttt{ExternalVerification68PrimeUnitTranslator} now carries this family at
its natural strength: the complete prime-translator profile, the exact integer
translation law \pdecl{channelResidual_appendPrimeTranslator}, and the
unconditional remote Cramer-grid reduction above.  Its deliberate negative
fixture changes the theorem type by an extra hypothesis.  Comparator replay or
Palomar acceptance would check those declarations under the configured trust
budget; neither would prove the missing strict nonvanishing statement or turn
the construction into a solution of Problem~68.
...
\section{The missing cofinal inputs}\label{sec:open}
...
but it is not itself a Lean evaluation.  The converse direction of the
...
for this note.  The release snapshot contains fifteen cited modules:
...
auxiliary digit-rigidity file is cited or projected.  The release root imports
