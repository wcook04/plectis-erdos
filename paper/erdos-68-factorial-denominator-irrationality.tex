...
\hypersetup{
  pdftitle={Integral channels and the prime unit translator: a denominator-obstruction programme for Erdős Problem 68},
...
\runninghead{Erd\H{o}s \#68: integral channels and the prime unit translator}
\title{Integral Channels and the Prime Unit Translator}
\subtitle{A denominator-obstruction programme for Erd\H{o}s Problem~\#68}
\author{Will Cook}
\date{July 2026}
...
$Z_m=\lfloor m!H_m\rfloor+1$ be its strict factorial successor.  The cleanest
checked contribution is the exact integral normal form
\[
  \Ser\notin\mathbb Q
...
$m=300000$; the Lean-checked consumer therefore proves unconditionally that
every representation $\Ser=a/q$ with $q>0$ has $q\ge300000$.

Two further checked layers expose possible closing mechanisms.  Integral
channels impose an explicit lcm tax, while a uniform remote-support
construction and a prime translator produce a nonzero-moment kernel whose
residual has absolute value at most $1/2$.  Weighted projection rigidity
excludes a bounded endpoint provided the endpoint congruence holds and two
divisor moduli exceed the endpoint bound with unequal projected residues.
What is not proved is equally precise: no theorem supplies cofinally many
integral misses, makes the rounded residual strictly nonzero cofinally, or
establishes the two remaining projection scale bounds.  Thus this note gives
an exact normal form, checked structural reductions, and a finite denominator
exclusion---not an irrationality proof.
\end{abstract}
...
Structural reductions & Checked channel/translator and endpoint-projection consumers isolate explicit missing inputs. \\
...
identifies a useful exact interface: Mahler's elementary rationality floor is
...
$1-b_m\ne0$ infinitely often---precisely the cofinal non-unit-carry producer
...
The key point is that Theorem~\ref{res:congruence} is a tax, not a tool.  A
localiser that kills the low channels in order to isolate the tail pays $\Lc_D$
in its moment, and $\Lc_D$ grows faster than the tail shrinks.  The returned
analysis proposes the quantitative form
\[
...
that is meant to produce such primitive kernels remains an advisory returned
reduction pending Lean formalisation; the checked divisibility must not be read
as checking that producer.
...
in the $p$-channel.  Adding an integer multiple of the translator to any
...
There is a second, more arithmetic producer at doubled prime indices.  For
...
criterion, not yet the missing cofinal producer.

The formal consumer is not restricted to those hand-reduced indices.  A
...
Its at-most-one consumer cuts the normalized valuation below $e$, and its
...
prime range at and above the block parameter.  It remains an interface:
...
The irrationality consumer also works for every natural block parameter at
least three, not only prime parameters.  What remains open is the arithmetic
producer.  Wilson supplies cofinal prefix-private factors without analytic
...
obligation in this interface is therefore to prove both sides of this exact
...
sufficient producer for~\eqref{eq:exact-frontier68}; none is presented as an
...
The first inequality is the local collision-core half of the exact unit-factor
...
\]
or, closer to the endpoint consumer, that with $m=n+1$ and $d=A_n$,
\[
...
\subsection*{3. Escape from the lower endpoint cylinder}
...
\begin{problem}[cofinal lower-cylinder escape]\label{prob:lower-cylinder68}
...
Any positive answer feeds the exact carry consumer directly and proves
...
carry does not meet this consumer; the theorem must couple them.

\subsection*{5. The finite floor-discontinuous Cramer block}
...
intermediate block is floor-discontinuous and has genuine sign changes.
...
Two broader programmes remain after the five quantified producers: find an
...
substitutes for one of the displayed producer theorems.
...
hash-bound; combined with the checked carry consumer it gives $q\ge300000$,
