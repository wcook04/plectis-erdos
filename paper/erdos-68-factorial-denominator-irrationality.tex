...
\]
is irrational.  Put $H_m=\sum_{2\le n\le m}(n!-1)^{-1}$,
$Z_m=\lfloor m!H_m\rfloor+1$, and define the integer carry $b_m$ by
$Z_m=mZ_{m-1}+1-b_m$.  The main theorem is the exact equivalence
\[
  \Ser\notin\mathbb Q
...
\]
The proof passes from the normalized strict successors $Z_m/m!$ to eventual
unit carries, and then contraposes.  An independently regenerated exact GMP
certificate gives $b_{300000}\ne1$; combined with the Lean theorem, it shows
that every rational representation $\Ser=a/q$ with $q>0$ has
$q\ge300000$.

A second group of results concerns finite factorial congruences.  In channel
$d$, the channel numerator is congruent to its factorial moment modulo
$d!-1$.  Hence cancellation through $D$ forces an explicit least common
multiple to divide that moment.  If a positive moment $M$ is divisible by
every channel modulus through $D=2t^2$ and $M<(R+1)!-1$, then, for
$t\ge2^{32}$,
\[
  3t^3<2(R+1).
\]
Within a single quotient band $kd\le i_j<(k+1)d$, cancellation in channel
$d$ forces the factorial moment itself to vanish.  A two-term correction
isolates one prime channel, while a Cramer construction cancels any prescribed
finite collection of channels on arbitrarily remote support.  Its rounded
residual can, however, be zero.  These are finite obstructions, not a cofinal
construction.

The carry equivalence is checked at
...
and the remote construction at
...
Under two consecutive unit carries, the complete two-step window cancels to
the universal inequality $0<U_{m+2}\le V_{m+2}$, so information obtained only
after assuming that adjacent pair cannot exclude it.  No theorem here produces
cofinally many non-unit carries, cofinally many nonzero translated residuals,
or the residue and scale estimates needed by the endpoint arguments.
Problem~\#68 therefore remains open.
\end{abstract}

\begin{paperresult}{Main results and exact open boundary}
\textbf{New exact reduction.} Irrationality is equivalent to cofinally many
non-unit carries, and also to cofinally many failures of $m\mid Z_m$.
\textbf{Finite consequence.} The checked witness $b_{300000}\ne1$ excludes
every rational representation with denominator below $300000$.
\textbf{Further result.} Factorial-channel cancellation obeys explicit moment
divisibility and quotient-band rigidity.  \textbf{Open boundary.} No argument
here produces the required cofinal non-unit carries or nonzero translated
residuals.
\end{paperresult}
