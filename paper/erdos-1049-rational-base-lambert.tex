...
% Problem-specific source pin for the #1049 endpoint-congruence results.
\renewcommand{\commit}{76b5b0a7ed5da7ebf3b9ed3bfd2fb480b6c38ee0}
\renewcommand{\commitshort}{76b5b0a7ed5d}
\hypersetup{
  pdftitle={Arithmetic boundaries at base 3/2: coordinatewise clearing and endpoint congruences for Erdős Problem 1049},
...
\runninghead{Erd\H{o}s \#1049: arithmetic boundaries at base 3/2}
\title{Arithmetic Boundaries at Base $3/2$}
\subtitle{Coordinatewise clearing and endpoint congruences for Erd\H{o}s
Problem~\#1049}
\author{Will Cook}
\date{July 2026}
...
two scoped arithmetic obstructions and formalise them in Lean: one concerns
primitive coefficient rows and their endpoint congruences, and the other a
coordinatewise clearing scheme.  Neither is a statement about every possible
linear-form construction.  Rowwise content (the greatest common divisor of the
...
We also publish the exact elementary boundary of a stronger source-backed
height region: $\log b/\log a<81/200$.  Lean checks that $31/4$ and all its
positive powers lie in that enlarged region and outside the earlier one, while
$3/2$ lies outside both because
$81/200<\log2/\log3$.  The source theorem supplying irrationality inside the
region remains external; the formal claims here are the parameter
specialisations and exclusions.
...
Membership of $31/4$ and its powers in the $81/200$ height region & Proved here & Exact elementary specialisation; the source analytic theorem remains external. \\
Exclusion of $3/2$ from both recorded height regions & Proved here & $3^{81}<2^{200}$ gives $81/200<\log2/\log3$. \\
Rectangular Hermite--Pad\'e threshold does not improve the classical margin & Proved here & Newly public exact no-go, with equality only at the classical endpoint. \\
...
not formalised of two external routes, including the stronger $81/200$
height baseline.  Section~\ref{sec:open} separates kernel escape from
...
\subsection{The enlarged $81/200$ height region}

A stronger source-backed hypergeometric criterion reaches the rational-height
region
\[
 \frac{\log b}{\log a}<\frac{81}{200}
 \qquad(a>b>0).
\]
The analytic criterion remains an external source theorem; the newly public
Lean module checks its elementary specialisation and its exact boundary.  In
particular,
\[
 4^{200}<31^{81}
 \quad\Longrightarrow\quad
 \frac{\log4}{\log31}<\frac{81}{200},
\]
so $31/4$, and every positive power $(31/4)^r$, lies in the enlarged region
(\lword{Erdos1049/ZudilinHeightRegion.lean}{45}{thirtyoneFour_mem_zudilinHeightRegion}{checked},
\lword{Erdos1049/ZudilinHeightRegion.lean}{62}{thirtyoneFour_power_mem_zudilinHeightRegion}{power family}).
The same base lies strictly outside the earlier
Bundschuh--V\"a\"an\"anen region
(\lword{Erdos1049/ZudilinHeightRegion.lean}{94}{thirtyoneFour_outside_bundschuhVaananenHeightRegion}{checked}),
so this is a genuine enlargement rather than a second certificate for the
same range.
...
region
(\lword{Erdos1049/ZudilinHeightRegion.lean}{108}{eightyOneTwoHundredths_lt_threeHalves_log_ratio}{boundary},
\lword{Erdos1049/ZudilinHeightRegion.lean}{125}{threeHalves_outside_zudilinHeightRegion}{exclusion}).
Thus $81/200$ is the strongest established baseline used below, while the
threshold needed to reach $3/2$ is strictly larger than
...
Problem~\#1049 remains open.  The remaining linear-form programme has two
...
beat its height.  We state those gates separately.
...
Here “primitive” means that the common coefficient content of the pair in
$\Z[X]^2$ is divided out before specialisation.  The specialised row content
\[
 g^{\mathrm{spec}}_{n,j}
...
\[
 K_n^{\mathrm{poly}}=
 \left\{\lambda:
   \sum_j\lambda_jU_{n,j}=0,\quad
   \sum_j\lambda_jV_{n,j}=0\right\},
 \qquad
 K_n^{\mathrm{rem}}=
 \left\{\lambda:
...
\begin{problem}[four-jet kernel escape]\label{prob:kernel}
For one literal primitive family satisfying~\eqref{eq:exact-jet-threshold},
...
nonvanishing nor remainder nonvanishing.  Problem~\ref{prob:kernel} is finite
algebra and normality; it makes no asymptotic product-formula claim.

\subsection{The first falsifiable families}

The current research packet names a contiguous $a_0$-shift determinant at
$r_n=13n+2$, after reporting failure of lower ranks $r\le13n+1$.  Those rank
claims and the approximate exterior exponents are advisory computations, not
Lean theorems.  More importantly, the current public artefact does not contain
a literal definition of the matrix $A_{n,r}$, so writing merely
“$\det A_{n,13n+2}\ne0$” would not yet be a public mathematical question.

\begin{problem}[publish and decide the first saturated determinant]
...
This formulation makes the matrix definition itself part of the publication
gate rather than treating private notation as inherited authority.
...
Prove the architecture-appropriate analogue of
\[
 \limsup_{n\to\infty}
...
nullspace.  The advisory adjacent-exterior calculation leaves a positive
normalised exponent of about $110.850\,n^2$; this number is a regression target,
not a checked theorem.  A proposed rescue must remove its explicit deficit
rather than merely exhibit some extra divisibility.

\subsection{One exact alternative-criterion test}

Different criteria consume different objects, so they are not bundled into one
method list.  The smallest Mahler-side applicability test is the following.

\begin{problem}[finite Mahler-system test]
For
\[
 \mathcal L(z)=\sum_{n\ge1}\frac{z^n}{1-z^n},
\]
determine whether $\mathcal L$ belongs to a finite-dimensional
$\Q(z)$-vector space stable under both $z\mapsto z^2$ and $z\mapsto z^3$,
with a regular functional system at $z=2/3$ to which a stated special-value
theorem applies.  Supply the literal system and verify its hypotheses, or prove
that no such finite-dimensional regular system exists in the specified class.
\end{problem}

This question has a concrete positive and negative outcome.  It does not infer
a special-value theorem from the already known fact that $\mathcal L$ is not a
rational function.

\subsection{The quantitative height frontier}

The newly public rectangular Hermite--Pad\'e no-go proves that, throughout its
admissible parameter range, the rectangular two-function threshold never
exceeds the classical one-function margin, with equality only at the classical
endpoint
(\lword{Erdos1049/HermitePadeNoGo.lean}{99}{rectangular_hp_threshold_le_classical}{bound},
\lword{Erdos1049/HermitePadeNoGo.lean}{121}{rectangular_hp_threshold_eq_classical_iff}{equality}).
The strongest positive baseline used here is the separate source-backed
$81/200$ region of the preceding section, and Lean proves
\[
 \frac{81}{200}<\frac{\log2}{\log3}.
\]
...
Let $\mathcal A$ be a specified class of primitive, noncollapsed constructions,
excluding scalar row rescaling, proportional permutation-orbit forms, pure
row-content gain and the rectangular family just excluded, and define
\[
