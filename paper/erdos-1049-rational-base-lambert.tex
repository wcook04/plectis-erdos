...
\hypersetup{
  pdftitle={Arithmetic obstructions at the rational base 3/2},
...
\title{Arithmetic Obstructions at the Rational Base $3/2$}
\subtitle{Two precise limits of integer-base methods for Erd\H{o}s
Problem~\#1049}
\author{Will Cook}
\date{August 2026}
...
\begin{abstract}
\noindent
Let $F(t)=\sum_{n\ge1}(t^n-1)^{-1}$.  The irrationality of $F(3/2)$ remains
open.  We isolate two exact arithmetic obstructions that explain why familiar
integer-base mechanisms do not transfer automatically to this rational base.

First, a cut-and-clear argument at $r/s$ leaves the denominator contribution
$s^{N+1}$ inside its forcing term.  For a precisely stated coordinatewise
corridor, we prove
\[
 s^{N+K+1}<r(N+K).
\]
At $3/2$ this inequality is impossible for every nonempty shift and cleared
window.  The associated cleared-tail recurrence
$U_{N+1}=rU_N-Bc(N+1)s^{N+1}$ makes the integer-base boundary explicit: the
geometric factor disappears when $s=1$ and survives when $s\ge2$.

Second, we study integer polynomial rows specialised homogeneously at $(3,2)$.
Dividing rows by their scalar contents is exactly neutral in the
local-to-Archimedean determinant balance.  Unit endpoint coefficients also
exclude both $2$ and $3$ from a common divisor of the two specialised
evaluations.  Hence neither device supplies the required local gain.  An
additive alternative remains: with $M\ge4R+2S$ coefficient pairs and $R>0$, a
nonzero $\{-1,0,1\}$ relation cancels all four endpoint jets.  This counting
theorem does not show that the resulting polynomial pair or analytic remainder
is nonzero.

Lean checks these finite algebraic statements.  We separately verify the
elementary height inequality used by the published $7/2$ criterion and record
denominator-exponent identities for a homogenised Pad\'e construction; the
analytic irrationality theorems remain external.  None of these results
decides the arithmetic nature of $F(3/2)$.
\end{abstract}
...
$t\ge2$~\cite{erdos1948}.  Bloom's catalogue currently records the rational-base
problem as open~\cite{erdosproblems}; we use it for numbering and reported
status, and the original papers for the mathematical claims.  Some individual
non-integral bases are known, including $7/2$ below.
...
Erd\H{o}s proved irrationality at every positive integer base in
1948~\cite{erdos1948}.  Pad\'e methods later gave quantitative refinements at
integer bases: Borwein treated shifted Lambert series~\cite{borwein1991}, Van
Assche used little $q$-Legendre approximants~\cite{vanassche2001}, and Zudilin
obtained a uniform irrationality-measure bound through Heine's transform and a
permutation group~\cite{zudilin2004}.  All retain an integer base.  For
non-integral rational bases, the criterion of Bundschuh and V\"a\"an\"anen
contains $7/2$ but not $3/2$~\cite{bv1994}.

Rivin's nonrationality theorem for the underlying Lambert function is a
functional statement and does not decide a value at a particular rational
point~\cite{rivin2026}.  The \emph{Formal Conjectures} source checks the
Lambert-series identity but not the irrationality statements
~\cite{formalconjectures1049}.  Likewise, no theorem here derives the endpoint
lemmas from the coefficient family of Zudilin or Rhin--Viola
~\cite{rhinviola1996}; that application theorem is genuinely missing.
...
arbitrarily long zero blocks in the base-$b$ expansion; tail bounds and
positivity show that the expansion does not terminate.  Hence it has
arbitrarily long zero blocks without being eventually zero and is irrational
...
$q\tau(N+1)/b$.  We isolate additional corridor hypotheses under which a
bounded-window version would work and show that they fail at $3/2$.
...
\paragraph{Two theorem packages.}
The first concerns a precisely defined cut-and-clear corridor.  Its exact
recurrence retains a forcing term of size at least $2^{N+1}$ at every
non-integral rational base, and the corridor inequality is impossible at
$3/2$ for all $N,K\ge1$ (Theorems~\ref{res:forcing} and
\ref{res:nocorridor}).  This excludes that predicate, not every argument one
might informally call ``clearing denominators''.

The second concerns polynomial coefficient rows.  Row content is neutral in
the determinant balance, and unit endpoints prevent a common divisor of two
specialised evaluations from carrying either local prime
(Theorem~\ref{res:content}, Proposition~\ref{res:commonmult}).  The surviving
alternative is additive: Theorem~\ref{res:jetkernel} gives a nonzero
$\{-1,0,1\}$ relation cancelling four endpoint jets once $M\ge4R+2S$ and
$R>0$.  It does not prove that the resulting polynomial pair or analytic
remainder is nonzero.  Problem~\ref{prob:kernel} records that application gate.

\paragraph{Sharpness.}
The corridor bound is exponential on the left and linear on the right, so for
fixed $r$ and $s\ge2$ it can hold only for bounded $N+K$.  At $3/2$ the crossing
has already occurred at the smallest admissible window.  The published
$7/2$ result rests on a different height criterion~\cite{bv1994}; nothing here
transfers that theorem to $3/2$.

\statusboundary

The Lean development proves finite algebra: determinant identities, endpoint
congruences, collision counts, the corridor exclusion, and the cleared-tail
recurrence.  The $7/2$ irrationality theorem is cited from
Bundschuh--V\"a\"an\"anen; only its elementary height inequality is checked
here.  The Pad\'e section proves exponent arithmetic but supplies no remainder
estimate.  No theorem in this note decides $F(3/2)$.

\paragraph{Structure.}
Sections~\ref{sec:primitive}--\ref{sec:endpoints} treat polynomial rows and
their endpoint jets.  Sections~\ref{sec:corridor}--\ref{sec:tail} treat the
cut-and-clear corridor.  The final sections separate the external $7/2$
criterion, Pad\'e bookkeeping, and the open additive gate.  Linked phrases open
the corresponding Lean declaration at revision~\commitshort.
