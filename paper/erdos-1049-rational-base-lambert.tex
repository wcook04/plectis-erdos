...
% Problem-specific source pin for all linked #1049 declarations.
...
nor a common divisor of the two specialised evaluations supplies $2$- or
$3$-primary gain.  Thus any argument based on this particular local-divisibility
strategy must obtain that gain after primitive normalisation.  A possible next
step is a growing-rank additive kernel
...
We also isolate the elementary logarithmic region
$\log b/\log a<81/200$.  Lean checks that $31/4$ and all its positive powers
lie in this region; it checks separately that $31/4$ lies outside the earlier
region.  The base $3/2$ lies outside both because
$81/200<\log2/\log3$.  No analytic irrationality theorem for the $81/200$
region is cited or formalised here: the formal claims are only the displayed
inequalities, memberships, and exclusions.
...
A separate possible route is Mahler's method.  Its first applicability test is
the following.
...
that no such finite-dimensional regular system exists.
\end{problem}
...
two-parameter exponent model.  Within that model, the displayed threshold
never exceeds the classical one-function margin, with equality only at the
classical endpoint
...
This is an algebraic theorem about the four defined exponent expressions.  It
does not construct polynomials or remainders, prove integrality or a
determinant estimate, or establish the asymptotics of a Hermite--Pad\'e family.
...
revision~\commitshort.  The declarations of this note live in five modules:
\texttt{RationalBaseLambert}, \texttt{RationalPadeArithmetic},
\texttt{ZudilinConeArithmetic}, \texttt{ZudilinHeightRegion}, and
\texttt{HermitePadeNoGo}.  The first contains the corridor, cleared-tail
recurrence, and elementary $7/2$ certificate; the remaining four separate the
Pad\'e exponent arithmetic, endpoint arithmetic, logarithmic comparisons, and
rectangular exponent model.  The link
...
positive-power membership & \lrefx{Erdos1049/ZudilinHeightRegion.lean}{62}{thirtyoneFour_power_mem_zudilinHeightRegion}\\
$31/4$ outside the earlier region & \lrefx{Erdos1049/ZudilinHeightRegion.lean}{94}{thirtyoneFour_outside_bundschuhVaananenHeightRegion}\\
$81/200$ below the $3/2$ log ratio & \lrefx{Erdos1049/ZudilinHeightRegion.lean}{108}{eightyOneTwoHundredths_lt_threeHalves_log_ratio}\\
$3/2$ outside the $81/200$ region & \lrefx{Erdos1049/ZudilinHeightRegion.lean}{125}{threeHalves_outside_zudilinHeightRegion}\\
rectangular exponent-model bound & \lrefx{Erdos1049/HermitePadeNoGo.lean}{99}{rectangular_hp_threshold_le_classical}\\
equality case for the exponent model & \lrefx{Erdos1049/HermitePadeNoGo.lean}{121}{rectangular_hp_threshold_eq_classical_iff}\\
\end{tabular}
\fi
