...
% Problem-specific source pin: #1049's endpoint-jet obstruction advances
% independently of the corpus default.
\renewcommand{\commit}{551bcf6c2525efa6e2b3d29ca6d78db41c50c232}
\renewcommand{\commitshort}{551bcf6c2525}
\hypersetup{
...
Heine--Zudilin route we isolate a sharper obstruction: homogeneous
specialisation at $(3,2)$ sees the constant endpoint modulo $3$ and the top
endpoint modulo $2$.  Unit endpoints therefore prevent scalar forms from
acquiring the common local content they need.  The surviving programme is a
growing-rank additive kernel that cancels both endpoint jets in both
coefficient channels while preserving a nonzero, sufficiently small
remainder.  We count the four-jet target exactly and prove the concrete
sufficient threshold \(n\ge 4R+2S\) at positive bottom depth.
\end{abstract}
...
\lword{Erdos1049/ZudilinConeArithmetic.lean}{302}{three_two_scalar_margin_neg}{three-halves
...
\(4R_n+2S_n\) Heine--Zudilin coefficient pairs from a genuinely non-collapsed
deformation, and prove that at least one resulting checked collision satisfies
\[
...
\(4R_n+2S_n\) coefficient pairs, plus a proof that some checked collision has
nonzero polynomial pair, nonzero remainder, and sufficient local gain.  A
scalar ray, a multiplicative cyclotomic factor, or another primitive-content
computation is not responsive to the surviving obstruction.
...
\(3\) and \(2\); the source integer-base ratio inequality implies a strictly
negative scalar margin at \(3/2\); and a large enough binary selector space
forces a nonzero \(\{-1,0,1\}\) four-jet collision.  The exact target size is
\((3^{R})^2(2^{S})^2\), and \(n\ge4R+2S\) suffices when \(R>0\).  None of
these statements produces a sufficiently large non-collapsed family or proves
a collision polynomial pair or remainder nonzero.

\paragraph{Most useful answer.}
Give the first actual family with \(r(n)\ge4R_n+2S_n\), then show that its
collision fibre is not contained in the polynomial-pair or remainder
nullspace.  If every available family is too small or degenerate, a rank
theorem identifying that obstruction is more valuable than further low-rank
enumeration.

\paragraph{If answered.}
A positive construction supplies the missing local-content producer to the
existing Archimedean consumer.  A negative rank theorem removes the additive
endpoint route and exposes the next genuinely different deformation.
...
irrationality-exponent inequality & \lrefx{Erdos1049/ZudilinConeArithmetic.lean}{290}{twice_le_of_irrationalityExponent_bounds}\\
three-halves scalar margin & \lrefx{Erdos1049/ZudilinConeArithmetic.lean}{302}{three_two_scalar_margin_neg}\\
\end{tabular}
\fi
