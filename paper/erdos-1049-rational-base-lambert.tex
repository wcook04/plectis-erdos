...
\renewcommand{\commit}{0b33a8f5430c9c68a0acba6027adf4bb828a5d25}
\renewcommand{\commitshort}{0b33a8f5430c}
\hypersetup{
  pdftitle={Sharp height and four-jet obstructions at base 3/2 for Erdős Problem 1049},
...
\runninghead{Erd\H{o}s \#1049: sharp obstructions at base 3/2}
\title{Sharp Height and Four-Jet Obstructions at Base $3/2$}
\subtitle{An exact $41/65$ certificate for Erd\H{o}s
Problem~\#1049}
\author{Will Cook}
\date{July 2026}
...
\begin{abstract}
\noindent
Let $F(t)=\sum_{n\ge1}(t^{n}-1)^{-1}$.  Problem~\#1049 asks whether $F(t)$ is
irrational for every rational $t>1$; it remains open, and we prove no
irrationality result.  The main arithmetic input is the sharp integer bracket
\[
 2^{64}<3^{41}<2^{65}.
\]
It gives a uniform gap greater than $3/13$ between $\log2/\log3$ and every
threshold in the admissible rectangular Hermite--Pad\'e model considered here.
It also shows that a cubic Hankel construction must remove more than the
$39/41$ charge used below, and that Zudilin's source scalar factor and the
universally forced first-order residual border remain strictly below this
requirement at every nontrivial rank.  These are unconditional exact
inequalities.  They exclude the stated height mechanisms, not all
Hermite--Pad\'e or determinant constructions.

The same rational scale sharpens the four-jet count.  At bottom depth $R=41T$, binary
selectors collide once $M\ge130T+2S$.  For $T=1$ this integer threshold is
exact by the additional direct comparison $2^{129}<3^{82}$: with only
$129+2S$ selectors the four-jet target is still larger than the selector
space.  Exact optimality is asserted only for this unit block;
$130T+2S$ is a uniform sufficient bound for general $T$.  A bounded-fibre
lemma then supplies the missing finite escape principle: if every analytic
remainder value has multiplicity at most $k$ and the selector space has size
greater than $k$ times the four-jet target, some equal-jet selector pair has
different remainders.  The required multiplicity bound for an actual
$q$-Ap\'ery or Zudilin family is not proved.

A distinct conditional improvement appears when the two endpoint coordinates
are already Pl\"ucker-collinear modulo $2^S3^R$.  If the second coordinate of
every tail row is a unit and every adjacent minor vanishes, then every pairwise
minor vanishes.  A B\'ezout shear reduces the selector image from two residue
coordinates to one, and the sufficient width drops from $2S+4R$ to $S+2R$.
This is an exact halving of the ambient exponent budget under its hypotheses;
the paper does not prove those hypotheses for an actual approximation family.

At the rational base $3/2$ we also retain two earlier scoped arithmetic
obstructions.  One quantitative result concerns an explicit
rectangular two-function exponent model, whose threshold is no larger than the
classical one-function margin, with equality only at $\rho=0$ and
$\sigma=1$ under $0\le\rho$ and $1+\rho\le\sigma$
(\lword{Erdos1049/HermitePadeNoGo.lean}{103}
      {rectangular_hp_threshold_le_classical}{threshold bound};
\lword{Erdos1049/HermitePadeNoGo.lean}{126}
      {rectangular_hp_threshold_eq_classical_iff}{equality case}).
The hard step is the gap-sign calculation: after writing
$\sigma=1+\rho+u$ with $u\ge0$, every term in the cleared gap is
nonpositive, and its unique zero forces $\rho=u=0$.
This retires only that model-specific source of an improved height margin: it
constructs no approximating polynomials or remainders, is not a universal
Pad\'e or Hermite--Pad\'e no-go, and proves no irrationality at $3/2$.
A separate obstruction concerns a common-width
integer-linear-form approach.  Integer scalar multiplication changes the
analytic error and the exterior determinant by the same factors, including
determinant height, so scalar content alone gives no local-to-Archimedean gain.
Homogeneous specialisation at $(3,2)$ detects the constant endpoint modulo $3$
and the top endpoint modulo $2$; under unit-endpoint hypotheses, neither prime
can divide both specialised evaluations.  Thus an argument using this
particular local-divisibility strategy must obtain its $2$- and $3$-primary
gain after primitive normalisation.  These results exclude only those
mechanisms; they neither prove that local gain is necessary nor rule out
polynomial factors before specialisation, cross-row arithmetic, or another
proof architecture.  We also give a finite-group collision lemma: for positive
bottom depth $R$, any $M\ge4R+2S$ coefficient pairs admit a nonzero selector
difference cancelling four prescribed endpoint residues, that is, both endpoint
jets in both sequences.  This constructs only the signed relation; it does not
ensure that the resulting polynomial pair or analytic remainder is nonzero.

The pigeonhole count has a sharper trigger than this convenient bound.
For four-jet depths $R,S$, it guarantees a collision whenever
$M>2R\log_2 3+2S$; if $N=2^M$ and
$Q=3^{2R}2^{2S}$, convexity then gives at least
$\tfrac12(N^2/Q-N)$ equal-signature selector pairs.  The surviving finite
problem is therefore to find one collision outside both the polynomial-pair
nullspace and the $3/2$-remainder nullspace.  This separates finite normality
from nonvanishing and from the later analytic height margin; none of those
remaining gates is proved here.

The two nearby little-$q$-Legendre Pad\'e diagonals used in the integer-base
literature are not interchangeable: substituting the Van Assche diagonal into
the Amdeberhan--Zeilberger recurrence leaves a nonzero residual already at
$n=0$ for every $p>1$.  A shared kernel therefore does not license transfer
of that recurrence, endpoint, lattice, or valuation argument.

Finally, we isolate a six-condition coordinatewise clearing scheme at a
rational base $r/s$.  Its hypotheses force
$s^{N+K+1}<r(N+K)$, which is impossible at $3/2$ when the shift and cleared
window are nonempty.  The associated cleared-tail recurrence
$U_{N+1}=rU_N-Bc(N+1)s^{N+1}$ is recorded as an explanatory identity; its
forcing term satisfies $2^{N+1}\le Bc(N+1)s^{N+1}$ for $s\ge2$ and positive
data, whereas at $s=1$ that term is $Bc(N+1)$.  The recurrence is
not derived from rationality and does not exclude other clearing or
linear-form constructions.

For comparison, Bundschuh and V\"a\"an\"anen's published theorem settles a
restricted family containing $7/2$; in its $K=\mathbb Q$, $\alpha=-1$ case the
criterion is $\lambda<(1/2+1/\pi^2)^{-1}$, with
$\lambda=\log 7/\log(7/2)$.  Lean checks only the elementary height inequality
required by that 1994 criterion~\cite{bv1994}, leaving its analytic
irrationality argument external.  Polynomial denominator-exponent bounds for a
homogenised Pad\'e construction likewise leave the analytic remainder
untreated.  We also verify elementary membership
and exclusion statements for the logarithmic region
$\log b/\log a<81/200$, but cite no analytic irrationality theorem for that
cutoff.  Problem~\#1049 remains open: nothing here proves rationality or
irrationality at $3/2$, or supplies a method-wide obstruction.
\end{abstract}

\externalreviewobject{The paper proves the sharp $2^{64}<3^{41}<2^{65}$
certificate, its rectangular height and Hankel-charge consequences, the
Zudilin scalar and scalar-plus-border no-go inequalities, the rank-$41$
four-jet threshold with its unit-block optimality statement, the abstract
bounded-fibre escape theorem, and the conditional B\'ezout--Pl\"ucker reduction
from the ambient $2S+4R$ selector width to $S+2R$.  It also retains the exact rational-base tail
recurrence, the literal coordinatewise-clearing exclusion at $3/2$, and the
earlier endpoint and Pad\'e arithmetic.}{These are exact scoped exclusions and
finite counting theorems.  They do not supply a remainder-fibre bound for an
actual approximation family, an analytic decay estimate, or an irrationality
result; Problem~\#1049 remains open.}{A dedicated AdelicHeightBridge Comparator
package binds the sharp power, height, charge, rank-$41$, and bounded-fibre
interfaces.  A separate RationalBaseBarrier package binds the exact
cleared-tail recurrence, its exponential forcing bound, and the literal
coordinatewise-corridor exclusion at $3/2$.  A BezoutPluckerJets package binds
the minor-propagation and halved-threshold family.  Registration of any package
in the public portfolio is a separate release step.}

The dedicated RationalBaseBarrier Comparator package exposes three parts of
one mechanism.  Its cleared-tail row is an exact algebraic interface.  For
arbitrary rationals $r,s,B,F$, an arbitrary coefficient stream
$\mathit{coeff}\colon\mathbb N\to\mathbb Q$, $r\ne0$, and every $N\in\mathbb N$,
\texttt{rationalBaseClearedTailQ\_succ} states
\[
 Q_{N+1}=rQ_N-B\,\mathit{coeff}(N+1)s^{N+1},
\]
where $Q_N$ is the rational-base cleared-tail state.  Its forcing term is
therefore part of the exact identity, not an approximation or an irrationality
claim.  The second row,
\texttt{twoPow\_le\_rationalBaseForcingNat}, proves that this forcing magnitude
is at least $2^{N+1}$ when $s\ge2$ and the integer data are positive.  The
third row,
\texttt{threeHalves\_no\_coordinatewiseCorridor} assumes only $N\ge1$ and
$K\ge1$ and excludes the specific predicate
\texttt{CoordinatewiseCorridor}$(3,2,N,K,Q,\mathit{digit})$.  Together these
rows rule out this literal clearing shape at $3/2$ while leaving other
linear-form or clearing architectures, and the irrationality question, open.
...
non-transfer of that recurrence.  Lean checks the
...
threshold among the sources compared here for this value.  It is not the only
later source admitting
non-integral rational bases: Zudilin's Pad\'e-and-Hankel treatment of the
...
$c>0$~\cite{zudilin2016}.  No value of $c$ is computed there, and we do not
compute one; we record the remark because it shows the rational-base extension
was already contemplated, and because any threshold of that shape is a
statement about $\log s/\log r$ of exactly the kind
Section~\ref{sec:sevenhalves} treats.
No claim of priority is made for anything below,
...
The linear-form route uses polynomial pairs before specialisation and integer
...
the height route, the exact bracket $2^{64}<3^{41}<2^{65}$ gives a gap greater
...
For the additive route, the same bracket replaces the generic
...
(Theorem~\ref{res:scalar}).  One candidate pursued here is additive: an
...
Integer scalar content is neutral for the local-to-Archimedean balance & Proved here & Theorem~\ref{res:content}, including the absolute determinant height. \\
...
No gain from integer scalar content or from the stated common divisor at $3/2$ & Deduced here & Corollary~\ref{res:nomult}; an ordinary deduction from the two preceding rows, not separately formalised. \\
...
Sharp bracket $2^{64}<3^{41}<2^{65}$ & Proved here & Theorem~\ref{res:powerbracket}; both integer inequalities are exact. \\
Rectangular height deficit greater than $3/13$ & Proved here & Corollary~\ref{res:sharpgaps}; uniform for $0\le\rho$ and $1+\rho\le\sigma$. \\
Scalar and scalar-plus-border charge ceilings miss $39/41$ & Proved here & Theorem~\ref{res:chargeceilings}; conditional only on the displayed source-facing degree ceilings. \\
Rank-$41$ four-jet collision at $M\ge130T+2S$ & Proved here & Corollary~\ref{res:rankfortyone}; uniform in $T>0$, and exactly optimal only when $T=1$. \\
Bounded-fibre escape from the remainder nullspace & Proved here & Theorem~\ref{res:boundedfibre}; abstract finite counting, with no source-family fibre bound supplied. \\
...
Common-width simultaneous endpoint-jet construction & Candidate sufficient route & Problem~\ref{prob:kernel}; not necessary for every proof and not supplied by the collision theorem alone. \\
Homogenised cyclotomic values are coprime to $ab$ & Proved here & Proposition~\ref{res:cyclounit}; an imported cyclotomic factor supplies no $2$- or $3$-primary gain at $3/2$. \\
No finite $\Q(z)$-space containing $\mathcal L$ stable under $z\mapsto z^2$ and $z\mapsto z^3$ & Deduced here from cited theorems & Proposition~\ref{res:nomahler}; closes the simultaneous Mahler route only, and uses no property of the point $2/3$. \\
\bottomrule
\end{tabular}
\end{table}
...
than an imported cyclotomic factor, and it is one reason the exclusions above
are not merely local accidents of the two primes involved.
...
analytic remainders.  The exact missing interface is a bounded-fibre estimate.
...
irrationality theorem for $31/4$.
...
The remaining linear-form programme therefore has two
...
Problem~\ref{prob:kernel} imposes.  The specialised row content
\[
 g^{\mathrm{spec}}_{n,j}
...
\subsection{The first falsifiable families}
...
Any proposed improvement must remove this explicit deficit rather than merely
exhibit some extra divisibility.

\subsection{One exact alternative-criterion test}

A separate possible route is Mahler's method.  Its first applicability test has
...
\subsection{The quantitative height frontier}
...
$\log2/\log3\approx0.6309$, not merely an improvement over $81/200=0.405$.
