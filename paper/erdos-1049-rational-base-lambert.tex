...
formalise them in Lean.  Rowwise content scales the analytic error and the
...
cancelling both endpoint jets in both specialised coefficient sequences while
preserving a nonzero, sufficiently small remainder; we count the four-jet
target exactly and prove the concrete sufficient threshold \(n\ge 4R+2S\) at
positive bottom depth.
...
the coordinatewise clearing scheme the leftover is the forcing term, of size at
least $2^{N+1}$ once the data are positive (Theorem~\ref{res:forcing}), and the
scheme itself is excluded at $3/2$ once the shift and the cleared window are
both nonempty (Theorem~\ref{res:nocorridor}).  For the linear-form
...
Scalar-cone product-formula margin & Negative under source input & Theorem~\ref{res:scalar}; the source inequality is assumed. \\
...
local gain, and gives the four-jet collision count.
...
The Heine--Zudilin forms have integral coefficient polynomials before
specialisation.  For \(P(X)=\sum_i p_iX^i\in\Z[X]\) and a declared width \(W\),
put
\[
...
For every integral polynomial \(P\),
\[
...
If \(U\) has unit top endpoint, \(V\) has unit constant endpoint, and an integer
...
Corollary~\ref{res:nomult} closes the two multiplicative devices above, and
...
For higher powers define the bottom and top jets by reducing \(H_W(P)\) modulo
\(3^R\) and \(2^S\), respectively.  Their vanishing is exactly the requested
divisibility:
\[
...
divisibility criterion}.  They turn the missing local divisor into an additive
...
Let \((U_j,V_j)_{j<n}\) be any \(n\) integral coefficient pairs.  If the
...
\noindent The inequality is immediate from \(\log3<2\log2\) and \(C_1>0\), and
is the checked
...
scheme leaves behind, in a form that does not mention the series.
...
pp.~176--177]{bv1994}.  In its special case $\alpha=-1$, the printed
hypothesis is
\[
...
\noindent The Lean proof factors the estimate into the
...
construction at \(3/2\) would have to supply.
...
A scalar ray, growth in the row contents, a multiplicative cyclotomic factor,
or a further common-multiplier computation does not address the obstruction
that survives Corollary~\ref{res:nomult}.  A theorem showing that every
