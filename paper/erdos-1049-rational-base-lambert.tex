...
two scoped arithmetic obstructions.  First, rowwise content scales both the
analytic error and the exterior determinant by the same factors.  Homogeneous
specialisation at $(3,2)$ detects the constant endpoint modulo $3$ and the top
endpoint modulo $2$; under unit-endpoint hypotheses, neither prime can divide
both specialised evaluations.  Thus an argument using this particular
local-divisibility strategy must obtain its $2$- and $3$-primary gain after
primitive normalisation.  We also give a finite-group collision lemma that
cancels four prescribed endpoint residues once $M\ge4R+2S$.  This congruence
statement does not ensure that the resulting polynomial pair or analytic
...
$U_{N+1}=rU_N-Bc(N+1)s^{N+1}$ is recorded as an explanatory identity; it is
...
inequality needed for that application.  We also verify elementary membership
