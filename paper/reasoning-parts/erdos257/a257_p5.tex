...
\ev{Lean}). The Lean-landed theorem uses closedness of $\mathcal{A}$ and discards the producing
rows, so it does not exhibit a single limiting support $A$. A genuine sequential-compactness
extraction (Cantor-space diagonal argument over $(\mathrm{Fin}\,2)^{\mathbb{N}}$, using the file's
own
...
upgrade is itself unformalised (Survivor 1, \ev{Math, adversarially verified}). That is a
constructive extraction task, not a proof that O3 is a strictly weaker logical condition.
...
\midrule
O1 U257 (universal) & O1 $\Rightarrow$ everything below is moot; $\mathrm{O1'}$ is false & \ev{Open} \\
...
O3 CofinalExactLocalMersenneHalfRows & $\Leftrightarrow$ O2 (Prop.~\ref{prop:collapsed-list}(c)); Lean lands $\Rightarrow$ & \ev{Open}, no shortcut \\
...
\textbf{O1 (universal).} The honest target is $\mathrm{U257}$ itself. The stronger schema
...
digitwise nor the carry-aware block-certificate producer exists, while Duverney--Tachiya still
...
resolution $2^{(r+5)/2}$. No proof of this input is identified in this record. The same shape of
input, a totient-window discrepancy anti-concentration bound, is needed on the other side of the
Erd\H{o}s \#249/\#257 pair to close \#249's certificate-supply obligation (Part IV). The two open problems, in their
...
Erd\H{o}s \#257 is open in the universal form (O1${}={}$U257), and both rational
membership targets $1/2$ (O2) and $1/21$ (O2b) remain open. Every route surveyed above either
(a) is proved equivalent to O2, including O3 by Proposition~\ref{prop:collapsed-list}(c), and
therefore offers no logical reduction, (b) is a sufficient condition not known to be equivalent
(O4, O5, O3-supply) whose own supply is unproved at cofinal scale, (c) is the rejected
certificate strengthening $\mathrm{O1'}$, already false on squarefree support, or (d) is certified
only on a finite initial segment, however large a margin that segment exhibits. Nothing in this
paper closes any of these gaps, and no claim above should be read as progress toward doing so.
...
Van Doorn--Kova\v{c} distinguish finite reciprocal sums from achievement sets
and prove an interval-representation threshold in a different, freely chosen
lacunary-denominator setting~\cite[Theorem~1, Lemma~4 and Corollary~5]{vandoornkovac}.
...
\end{equation}
over finite or countable families of finite positive frames $F_j$ covering
...
The frames need not be disjoint or contained in $F$.  Covers of infinite
cost do not lower the infimum, and a finite-cost cover exists, for example
one frame $F$ with exponent one.  This is the manuscript's fractional cost,
...
squarefree; different parent primes give disjoint frames, since no parent
prime occurs in any other frame.  Their minimum is greater than
...
for all finite nonempty $F$, even after all frames, exponents, coefficients
...
A fixed $n$ can witness fewer than or equal to $g(n)/a$ integer values of
...
not successfully retrieved in this pass.  Van Assche's nonvanishing and
