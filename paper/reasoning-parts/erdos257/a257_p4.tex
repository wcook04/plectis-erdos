...
\textbf{Scope.} This parity identity supplies no restriction for this recursion shape: it follows
formally from doubling minus forcing. That precise obstruction also applies to a structurally
identical recurrence. It does not rule out averaging across shifts, residue classes, or
observation scales.
...
\textbf{Exact denominator budget.} Cardinality alone gives only
$|\mathrm{Skip}_n|\le n-2$. For the denominator $D_n$ of the corresponding rational,
\[
 \log_2 D_n\le\sum_{d\in\mathrm{Skip}_n}\log_2(2^d-1)
 <\sum_{d\in\mathrm{Skip}_n}d\le \frac{n(n-1)}2-1.
\]
An $n^2/4$ asymptotic would require additional distribution information. This weighted budget
is too coarse to yield the required $2^{-3n/2}$ separation from the known irrationality measure.

Zudilin's bound $\mu(E)\le 2.42343562\ldots$ (Math.\ Notes 72 (2002) 858--862; the cited
erratum gives $2.46497868\ldots$) does not repair this quadratic-versus-linear budget.

\textbf{Scope.} The displayed denominator estimate combined with a fixed irrationality-measure
exponent does not derive the wall. A sharper denominator analysis using the actual skip
distribution, or a simultaneous approximation theorem with additional structure, is outside this
calculation and is not excluded.
...
Combining Proposition~\ref{prop:upper-unconditional} with Theorem~\ref{thm:cd-neg3-impossible},
only $C_D\in\{-2,-1\}$ remain in the final-middle-then-all-right scenario. In the unrestricted
late-middle-row induction, all three cells $\{-3,-2,-1\}$ remain to be excluded unless another
argument supplies the final-producer premise. No Lean theorem in the tree at the time of writing
excludes $C_D=-2$ or $C_D=-1$ in either scope.
