...
places $T$ beside the level-1 sibling identity
$L(\mu):=\sum_d \mu(d)/(2^d-1) = \tfrac12$ (rational, trivially) and
$L(1) = \sum_d 1/(2^d-1) = E$, the Erd\H{o}s--Borwein constant, whose
irrationality Erd\H{o}s proved in 1948~\cite{erdos1948lambert} and which
\emph{is} also proved irrational in this same kernel
...
$\varphi(n)=\Theta(n)$. A near-integer irrationality criterion
...
\ev{Lean}, \coord{n/a}, fully coordinate-free) applies more plausibly to $T$,
whose weight is bounded, than to the raw $\varphi(n)/2^n$ series.
