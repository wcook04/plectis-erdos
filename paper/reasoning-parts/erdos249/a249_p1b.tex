...
(\texttt{\breakcode{...TerminalCarryCorridorEscapeSupply}}) closes the entire cluster and hence \#249.
...
Define \[\adjustbox{max width=\linewidth}{$\displaystyle \mathtt{fixedRankSecondDifference}\ H\ j := \varphi(3H+j)-2\varphi(2H+j)+\varphi(H+j)$}\].
...
On the square-root clean window $j^2\le2^a$, $a\ge4$: \[\adjustbox{max width=\linewidth}{$\displaystyle 2\varphi(j) \mid \mathtt{fixedRankSecondDifference}(\mathtt{periodLcm}(2^a))\ j$}\].
...
(matches project memory \texttt{\breakcode{feedback\_erdos\_reductions\_rejected\_bank\_real\_results}}): only
...
\[\adjustbox{max width=\linewidth}{$\displaystyle \mathtt{certifiedRank2Kill}\ h\ N\ L \implies$}\] the second difference
