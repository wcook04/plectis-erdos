...
\begin{prop}[The five-link implication chain below the residue-gap target]\label{prop:TE-05}
Each of the following cofinal supplies is proved \emph{sufficient} for
Prop.~\ref{prop:TE-04}'s target, with the displayed proved implications:
\[
\begin{aligned}
...
\end{aligned}
\]
All six named predicates are open; none is proved. Each arrow points from a
proved sufficient condition to the statement it implies, so proving the last
link (\texttt{...TerminalCarryCorridorEscapeSupply}) implies every link above
it and hence \#249.
\scale{cofinal}\ \ev{Open}\ \coord{mobius-mersenne}\\
...
\]
would contradict this lower bound and close \#249 if proved with the
necessary additional totient-specific hypotheses. The generic assertion
for arbitrary rational coefficient series is false: the $5/4$ control in
\texttt{ParityPerturbedRationalControl.lean} has a tempered integral carry
with the same exponential rank floor. The compressed-adjoint and
shift-polynomial obstructions (Observation~\ref{prop:B4b-kill}) remain
separate no-go results. None is a generic rationality-driven rank ceiling.
The evidence label below applies to the proved rank floor and no-go results,
...
Nesterenko~\cite[Cor.~2, p.~1320]{nesterenko1996} (cited, not formalised). Any proof of \#249 sits on this Dirichlet-convolution
...
the Mersenne-Lambert primitive weight (so $S=L(A)$, Prop.~\ref{prop:D7-inv}).
The coordinates $A(n)/n$ are the M\"obius-inverted orbit counts of $\varphi$,
whose integrality, together with nonnegativity, characterises the sequences
realisable as periodic-point counts~\cite[Lemma~3.1, p.~5]{everest-periodic};
only the integrality part is used here. Any $D$ clearing
