...
\scale{cofinal}\ \ev{Open}\ \coord{mobius-mersenne}\\
\lean{irrational_totient_series_of_lcm_cone_window_kill_supply}{CertificateKernel.lean:18967}
\end{prop}
...
\scale{cofinal}\ \ev{Open}\ \coord{mobius-mersenne}\\
\lean{irrational_totient_series_of_lcm_cone_nonflat_supply}{CertificateKernel.lean:19093}
\end{prop}
...
\begin{prop}[Finite-rank compression — a third independent wall]\label{prop:D5cons}
...
carry orbit with $\mathbb Q$-rank $\ge 2^e-1$ at every level (Prop.~\ref{prop:CP-02}). A theorem
\[
\text{bounding the dyadic-section rank of \emph{every} rationality-supplied tempered carry}
\]
would contradict the unconditional lower bound and close \#249 by a linear-algebraic coordinate
wholly distinct from the binary-window certificate coordinate of Props.~\ref{prop:A10}-\ref{prop:B10}.
...
to cross, and it is the exact hypothesis feeding Prop.~\ref{prop:D5cons}'s third independent wall.
...
\lean{totientTail_eq_tsum_shiftedMobiusPulse}{TotientShiftedMobiusPulse.lean:535}
\end{prop}
...
(Prop.~\ref{prop:B7}).
\scale{n/a}\ \ev{Lean}\ \coord{mobius-mersenne}\\
\lean{rational_totient_series_forces_lcm_cone_flatness}{CertificateKernel.lean:18955}
\end{prop}
...
\lean{card_totientCanonicalIndex}{TotientMahlerDefect.lean:51}
\end{prop}
...
\lean{tsum_totient_div_pow_two_eq_pnat_half_pow}{CertificateKernel.lean:18368}
\end{prop}
...
(f) $\sum_n' c(n)/2^n = 3/2$, a \emph{rational} number. \textbf{Kills}: any \#249 proof strategy
...
\emph{visible} aperiodic structure while summing to a fixed rational. \textbf{Doctrine consequence}
...
\lean{totient_tail_rank_two_kill_sound_but_not_shallower_cell}{CertificateKernel.lean:19043}
\end{obs}
...
\lean{squareCRT_clean_block_can_vanish}{SquareCRTCube.lean:285}
\end{obs}
...
\lean{CertifiedGreedyMersenneDeath}{GreedyAchievementSet.lean:1720}
\end{obs}
