...
(\texttt{\breakcode{TotientParityCoboundaryCountermodel}})}
...
(project memory \texttt{\breakcode{feedback\_erdos\_reductions\_rejected\_bank\_real\_results}}):
...
Integral coordinates are relative to the \emph{generated} lattice $L_e$.
They do not say that $L_e$ contains every integer-valued sequence in its
rational span.  For $e\ge2$, the sequence $\varphi(4n+3)/2$ is integer-valued,
since $4n+3\ge3$ and its totient is even.  Its retained-channel coordinate
is $1/2$ at $F_{2,3}$, so uniqueness excludes it from $L_e$, although its
double lies in $L_e$.  Thus the lattice is not saturated in that ambient
integer-valued group.  The unit-pivot argument is elementary module
elimination applied to the arithmetic reductions and the independence
proved above; it requires no additional prime-distribution input.
...
257-cofinal-rows & \texttt{\breakcode{greedyHalfFrozenMargin\_nonneg}},
...
of anything on disk.
\item \textbf{scale\_only} (5 rows: SEP-02, \texttt{\breakcode{greedyHalfFrozenMargin\_nonneg}},
