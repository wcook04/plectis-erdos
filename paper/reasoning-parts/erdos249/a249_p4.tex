...
explicit splice construction. Choose an integer \(P>B\), set
...
\(\mathrm{Sep}(h,N,L)\) with \(N+h+L\le B\) exactly as \(\varphi\) does. Yet
...
survival/cancellation law: the reduced denominator of \(a/D\) is
\(D/\gcd(a,D)\), so a divisor \(m\mid D\) coprime to \(a\) divides it
(coprimality is sufficient and not necessary: \(2/8=1/4\) keeps \(m=2\)); after
scaling the numerator by \(h\), such a divisor \(C\mid D\) still leaves the
divisor \(C/\gcd(C,h)\) of the reduced denominator. \ev{Lean} \scale{uniform}
...
Let \(e:\mathrm{Fin}\,d\to\mathbb N\) and \(z:\mathrm{Fin}\,d\to\mathbb C\),
with \(z(j)\ne0\) for every \(j\), and fix a row \(i_0\) with \(e(i_0)=1\).
Use the locked column weights \(W(j)=z(j)^{-1}\), so that
\[
 \mathrm{residualMonomialMatrix}(e,W,z)(i,j)
 =W(j)z(j)^{e(i)}.
\]
If \(\det(\mathrm{phasePowerMatrix}(e,z))\ne0\), then
\(\det(\mathrm{residualMonomialMatrix}(e,W,z))\ne0\), and every entry
of row \(i_0\) is \(1\).
\end{prop}
...
contradiction (or a genuine rank obstruction) might be extracted.  Such a
compression would make the totient $2$-regular, which
Coons~\cite[Theorem~3.2]{coons} had already
excluded for every base $k\ge2$ \ev{Cited}.  Martin's Theorem~1 already implies
the linear independence proved below~\cite[Theorem~1]{martin-phi-inequalities};
the argument recorded here is a separate CRT--Dirichlet proof.

\textbf{(b) Exact mechanism.}
\begin{thm}[Infinite dyadic totient-kernel rank]
For every depth \(e\ge1\), the canonical family of \(2^e+1\) dyadic
...
canonical index then still contains the two zero-residue channels
...
spanning family of all channels already matters for regular sequences, where
matrices associated to a basis compute the growth exponent through their joint
spectral radius and a spanning family gives only an upper bound
...
equivalent contradiction.''  The infinite-rank conclusion is the case $k=2$
of Coons's theorem cited in~(a); the formal record adds the exact rank
$2^e+1$ at every level $e\ge1$, the explicit basis, and the integral relation
module.  Martin's Theorem~1 gives linear independence for any family of
totient channels along pairwise nonproportional affine forms with positive
slopes~\cite[Theorem~1]{martin-phi-inequalities}
\ev{Cited}; in the canonical family only $\varphi(n)$ and $\varphi(2n)$ have
proportional forms, and one further evaluation separates them, as in the short
note.  The Lean proof does not use Martin's theorem.
...
True} on this first pass. The eighteen promotion claims were then checked
against the proofs, with the review depth stated per row; in particular the
NM-12 endpoint proof bodies were not read in that pass. The audit table
below reports the verdict for all eighteen.

Of the eighteen, \textbf{sixteen are recorded as NOT\_PROMOTABLE}. For
fifteen of them, reading the proof body exposes a documented arithmetic,
logical, or coordinate obstruction that a routine strengthening cannot cross,
and the row's own record states exactly what new input would be required;
NM-12 is unverified, since its endpoint proofs were not read.
...
So, at the level that matters for this paper's obligations,
\textbf{no verified promotion candidate in the audited set supplies missing
arithmetic content toward closing \#249 or \#257}; NM-12 remains an unverified
widening. The two mechanically-widenable rows
...
\subsection*{Classification of the blockers}

Reading the proof bodies, where the table records them as read, in place of the \texttt{yields}
...
this most-favourable subset yields fifteen verified blockers, one unverified row and only two
