...
(\lean{squareCRT\_clean\_block\_can\_vanish}{SquareCRTCube.lean:285}, kernel-\texttt{decide}d;
cleanliness witnessed by \lean{squareCRT\_vanishing\_countermodel\_is\_clean}{SquareCRTCube.lean:294}).
\item \emph{Nonvanishing witness.} A separate, nearby clean configuration at
\(n=27\) has the same block \emph{nonzero}
(\lean{squareCRT\_clean\_block\_can\_be\_nonzero}{SquareCRTCube.lean:303}; cleanliness witnessed by
\lean{squareCRT\_nonzero\_countermodel\_is\_clean}{SquareCRTCube.lean:311}).
\end{itemize}
Both witnesses are exhibited alongside the positive achievability result
\lean{exists\_squareCRT\_clean\_horizon}{SquareCRTCube.lean:228} (a bounded common base satisfying prescribed
...
(\lean{totient\_mul\_prime\_cocycle}{SquareCRTCube.lean:34}) and the CRT-witness apparatus
(\lean{exists\_squareCRT\_base}{SquareCRTCube.lean:131}, \lean{exists\_large\_squareCRT\_base}{SquareCRTCube.lean:164}) remain fully reusable for
...
\lean{alternating\_powerset\_sum\_eq\_zero\_of\_insert\_invariant}{SquareCRTCube.lean:264} — ``if inserting one
...
\lean{positiveRat\_numDivisor\_mul\_lt\_two\_mul\_den}{AdelicHeightObstruction.lean:77}). A companion rigidity fact
...
fixed \(w_0\) —
\lean{Erdos249257.AdelicHeightObstruction.linearDescender\_eq\_smul\_eval}{AdelicHeightObstruction.lean:120}. The upstream
...
\lean{tsum\_totient\_div\_pow\_two\_ne\_ratCast\_of\_den\_le\_79639646646701375323355774875831053}{CertificateKernel.lean:18337}, the strongest
...
\end{prop}
\lean{rational\_totient\_series\_forces\_lcm\_cone\_flatness}{CertificateKernel.lean:18955} (body
...
\lean{irrational\_totient\_series\_of\_lcm\_cone\_window\_kill\_supply}{CertificateKernel.lean:18967} shows any
...
\lean{irrational\_totient\_series\_of\_lcm\_cone\_nonflat\_supply}{CertificateKernel.lean:19093}). \ev{Lean}
...
unbounded scale has been supplied. Neither direction decides \#249: flatness
...
nonintegral tail difference at cofinally many scales — is unchanged. Note
...
\lean{totient\_tail\_rank\_two\_kill\_sound\_but\_not\_shallower\_cell}{CertificateKernel.lean:19043}) — an
...
\lean{card\_totientCanonicalIndex}{TotientMahlerDefect.lean:51}) is linearly independent over
...
large power of \(2\)).
\end{thm}
\lean{linearIndependent\_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:900}; consequently the full
infinite family spans an infinite-dimensional \(\mathbb Q\)-space,
\lean{not\_finiteDimensional\_span\_fullTotientKernel}{TotientMahlerDefect.lean:1014}. A companion
...
\lean{false\_of\_compressedAdjointCertificate}{TotientMahlerDefect.lean:1051}. \ev{Lean}
...
equivalent contradiction.'' The companion necessary-consequence-of-rationality
...
(\lean{not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}{TotientCarryKernelRigidity.lean:284}),
...
entire open content here. A \emph{third}, independent coordinate exhibits
...
\textbf{(d) Salvage.} \lean{SeparatedMinorCertificate}{TotientMahlerDefect.lean:73} and
\lean{linearIndependent\_of\_separatedMinorCertificate}{TotientMahlerDefect.lean:81} are fully generic finite-rank
...
\begin{longtable}{p{0.16\linewidth}p{0.40\linewidth}p{0.36\linewidth}}
...
\lean{Erdos249257.intWeightedCoeff\_periodFourSignWeight\_eq\_zero\_of\_mod\_four\_eq\_three}{CertificateKernel.lean:14179}.
...
(\lean{greedyMersenneDeath\_not\_mem}{GreedyAchievementSet.lean:1711}; witness
\lean{three\_fourths\_certifiedGreedyMersenneDeath}{GreedyAchievementSet.lean:1742}, \(3/4\) certified dead at
...
record states exactly what new input would be required. \textbf{Two}
...
estimate) that every genuine closure of \(\#249\) or \(\#257\) requires. So
...
\begin{longtable}{p{0.10\linewidth}p{0.14\linewidth}p{0.10\linewidth}p{0.58\linewidth}}
\textbf{Obligation} & \textbf{Row / site} & \textbf{Gap kind} &
\textbf{Verdict and exact blocker} \\
\hline\endhead
249-supply & e1-companion,
\lean{exists\_certifiedKill\_of\_first\_harmonic\_gap}{FirstHarmonicGap.lean:128} & hypothesis\_strength &
NOT\_PROMOTABLE. The consumer already has the obligation's exact quantifier
...
249-supply & SGN-01,
\lean{actualLcmTailDiff\_shift\_pos}{TotientActualLcmOrbitSign.lean:39} & hypothesis\_strength &
NOT\_PROMOTABLE. Positivity is exactly HALF of the needed certificate: the
...
249-supply & TE-04,
\lean{actualLcmTailDiff\_notMem\_int\_of\_topEdgeResidueGap}{TotientActualLcmTopEdgeStaircase.lean:1260} & hypothesis\_strength &
NOT\_PROMOTABLE. The consumer side is finished and cofinal (proved for
...
\lean{PowerTwoFlexibleActualTerminalDominanceSupply}{TotientActualLcmTopEdgeStaircase.lean:1908} & hypothesis\_strength &
NOT\_PROMOTABLE. The exact identity pinning the target shows the dominance
...
249-supply & SEP-02,
\lean{abs\_actualLcmTailOrbit\_sub\_rawApprox\_lt}{TotientActualLcmOrbitSeparation.lean:141} & scale\_only &
NOT\_PROMOTABLE. The analytic tail is fully discharged with an explicit,
...
\(a=6\) is proved. \\
249-supply & b7,
\lean{upperHalfMersenneProduct\_lower\_bound}{MersenneShadowDenominatorGrowth.lean:60} & coordinate\_only &
NOT\_PROMOTABLE. The only proved unbounded-growth quantity in the
...
249-supply & d4/d5,
\lean{linearIndependent\_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:900}, \lean{not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}{TotientCarryKernelRigidity.lean:284} &
coordinate\_only & NOT\_PROMOTABLE. The scale side of a second, independent
...
(\lean{false\_of\_compressedAdjointCertificate}{TotientMahlerDefect.lean:1051}). \\
...
NOT\_PROMOTABLE. The excess-side run-length law is only half-assembled: an
...
257-reset & E1,
\lean{SeamGreedyRemainderGapBound}{HalfCylinderSeamLimit.lean:192} & coordinate\_only &
NOT\_PROMOTABLE. Supplies an UPPER bound on the remainder (the ceiling half
...
multiple & NOT\_PROMOTABLE. Two gaps at once: the sign law is
...
\lean{exists\_exactRowStrictUpperFill\_of\_skippedCoreSharpCapacity}{BooleanMobiusSkipRow.lean:238} & hypothesis\_strength &
NOT\_PROMOTABLE. Unconditional and strictly more general than every actual
...
hypothesis\_strength & NOT\_PROMOTABLE. Unconditional and uniform in both
...
\lean{exists\_greedyHalfFrozenMargin\_nonneg\_of\_excess\_neg}{HalfCylinderFiniteShadow.lean:1198} & scale\_only &
NOT\_PROMOTABLE. The margin-nonnegativity horizon is produced by a
...
NOT\_PROMOTABLE. Trades off against the obligation in the opposite
...
257-universal & NM-02,
\lean{infinite\_greedyMersenneSkippedSupport\_of\_half\_mem}{GreedyAchievementSet.lean:2482} & scale\_only &
...
257-universal & NM-03,
\lean{finite\_boolSupport\_ne\_half}{HalfCarryReachability.lean:589} & scale\_only &
...
a resolution. \\
257-universal & NM-04,
\lean{dyadic\_support\_fraction\_reciprocalMass\_diverges\_or\_gt\_one}{RationalSupportCarrySkeleton.lean:2200} & coordinate\_only &
NOT\_PROMOTABLE (partial mechanical content only). The dyadic restriction
\(v=1\) is a call-site choice, not a proof constraint — the general-\(v\)
conclusion is already landed for arbitrary \((v,h,L,F)\) at
\lean{shiftedBooleanCollision\_wrap\_bound\_of\_common\_multiple}{RationalSupportCarrySkeleton.lean:1963} — but
...
257-universal & NM-12,
\lean{half\_mem\_mersenneAchievementSet\_or\_exists\_fatal\_gap}{HalfCutLocator.lean:623} & scale\_only &
NOT\_PROMOTABLE (unverified, not merely blocked). The killer and the
