...
\textbf{(b) Exact mechanism that closed it.} Theorem~\ref{thm:gamma} supplies
the construction, and this subsection carries no separate one. For a fixed
\(B\) and a prime \(P>B\), the stream \(\gamma(n)=\varphi(n)\) for \(n\le B\)
and \(\gamma(n)=n-\mathbf 1_{P\mid n}\) for \(n>B\) has
\(T_\gamma = D - 1/(2^{P}-1)\in\Q\) and agrees with \(\varphi\) on every index
\(\le B\). It therefore returns the same verdict as \(\varphi\) on every
instance of \(\mathrm{Sep}(h,N,L)\) whose inspection horizon \(N+h+L\) is at
most \(B\). The shift \(h\) belongs in that horizon: a tail-difference
certificate reads through index \(N+h+L\).

An earlier version of this subsection instead proposed a lacunary
zero-valued coboundary edit \(2/2^m-4/2^{m+1}=0\) beyond \(B\). That
description is withdrawn. A zero-valued edit preserves the value of the
series, so it cannot make an unknown totient sum rational, and it is not the
mechanism of Theorem~\ref{thm:gamma}, whose edit changes the value by the
explicit amount \(1/(2^{P}-1)\).
...
stated in Corollary~\ref{cor:b1}: no proof rule uniform over all coefficient
sequences \(c(n)\le n\) can pass from one fixed prefix
\(\{c(n):n\le B\}\) to the cofinal supply. The construction gives a different
\(\gamma_B\) for each \(B\), so it says nothing against a theorem that uses
the fixed arithmetic sequence \(\varphi\) and supplies actual totient
certificates at unbounded horizons. That theorem is precisely the open
obligation, and Remark~\ref{rem:b1-scope} states the boundary.

\textbf{(d) Standing consequence.} Wherever a proof strategy in this corpus
proposes to certify a supply by exhaustive finite search (the diagonal
deposits at \(t\in\{1,\dots,64\}\),
\lean{Erdos249257.certifiedKill\_diagonal\_all\_imported\_through\_t64}{DiagonalPincerCertificatesT64.lean:1967};
the row-200{,}000 sqrt-escape census on the \#257 side),
Theorem~\ref{thm:gamma} is the standing reason such a search is evidence and
not proof. The Lean-formalised countermodel of
\S\ref{sec:parity-countermodel} is a separate construction with a separate
purpose, and the two should not be described as one mechanism.
\ev{Math} \scale{n/a} \coord{binary-digit}.
...
\lean{tsum\_totient\_div\_pow\_two\_ne\_ratCast\_of\_den\_le\_79639646646701375323355774875831053}{CertificateKernel.lean:18572}, the strongest
...
\end{prop}
\lean{rational\_totient\_series\_forces\_lcm\_cone\_flatness}{CertificateKernel.lean:19190} (body
...
\lean{irrational\_totient\_series\_of\_lcm\_cone\_window\_kill\_supply}{CertificateKernel.lean:19202} shows any
...
\lean{irrational\_totient\_series\_of\_lcm\_cone\_nonflat\_supply}{CertificateKernel.lean:19328}). \ev{Lean}
...
\lean{totient\_tail\_rank\_two\_kill\_sound\_but\_not\_shallower\_cell}{CertificateKernel.lean:19278}) — an
...
\textbf{(b) Exact mechanism.}
\begin{thm}[The dyadic totient basis and finite-level ranks]
The sequences $\varphi(n)$, $\varphi(2n)$, and
$\varphi(2^j n+r)$ for $j\ge1$ and odd $0<r<2^j$ form a
$\mathbb Q$-basis of the span of the full dyadic totient kernel.
The sections through level $e\ge1$ span a space of dimension $2^e+1$;
the level-zero span has dimension one.
\end{thm}
\lean{linearIndependent_oddCoreTotientKernelFamily}{TotientMahlerDefect.lean:1265}
\lean{span_range_fullTotientKernel_eq_span_range_oddCore}{TotientMahlerDefect.lean:1380}
\lean{finrank_totientKernelThroughLevelFamily_eq}{TotientMahlerDefect.lean:1085}

\begin{proof}[Proof mechanism]
Fix $e$.  Restrict the canonical finite family to odd $n$, where
$\varphi(2n)=\varphi(n)$, and merge these two columns.  The remaining
arguments $L_i(n)=a_i n+b_i$ are $n$ and the odd-residue affine forms;
their slopes and residues are coprime and their pairwise cross determinants
are nonzero.  For each target $i$, CRT prescribes an odd residue for $n$
and a root of each other $L_j$ modulo a distinct large prime
$q_j\equiv1\pmod{2^{e+3}}$.  Choosing $q_j$ larger than the nonzero
cross determinant ensures that the target does not vanish modulo $q_j$.
Dirichlet's theorem then makes the target value prime in the resulting
reduced progression.  The odd residue of $n$ fixes
$v_2(\varphi(L_i(n)))=d_i<e+2$, whereas each off-target totient is divisible
by $q_j-1$, hence by $2^{e+2}$.

Choose one such input $n_i$ per target and form
$M_{ij}=\varphi(L_j(n_i))$.  Dividing column $j$ by $2^{d_j}$ gives an
integer matrix congruent to the identity modulo $2$, so its determinant is
nonzero.  This proves independence of the merged family on odd inputs
\lean{exists_separatedMinorCertificate_totientAffineOddFamily}{TotientMahlerDefect.lean:893}.
In a relation among the original columns, their two zero-residue coefficients
$\alpha,\beta$ therefore satisfy $\alpha+\beta=0$, and all other coefficients
vanish.  Evaluation at $n=2$ gives $\alpha+2\beta=0$, proving full independence
\lean{linearIndependent_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:946}.

The identities
$\varphi(2^{j+1}n)=2^j\varphi(2n)$ and
$\varphi(2^j n+2^{t+1}s)=2^t\varphi(2^{j-t-1}n+s)$ for odd
$s$ and $0<2^{t+1}s<2^j$ reduce all remaining sections to these basis vectors.
Finite character gives independence of their union.  The count through level
$e\ge1$ is $2+\sum_{j=1}^{e}2^{j-1}=2^e+1$.
\end{proof}

The full-kernel infinite-dimensionality follows, but the explicit basis and
finite-level ranks are the sharper statements explained here.
\lean{not_finiteDimensional_span_fullTotientKernel}{TotientMahlerDefect.lean:1146}
A companion
...
(\lean{not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}{TotientCarryKernelRigidity.lean:301}),
...
\lean{Erdos249257.intWeightedCoeff\_periodFourSignWeight\_eq\_zero\_of\_mod\_four\_eq\_three}{CertificateKernel.lean:14414}.
...
(\lean{greedyMersenneDeath\_not\_mem}{GreedyAchievementSet.lean:1745}; witness
\lean{three\_fourths\_certifiedGreedyMersenneDeath}{GreedyAchievementSet.lean:1776}, \(3/4\) certified dead at
...
Not promotable. Positivity excludes the nonnegative true survivor.
Under the longer-window and modulus-room bounds, integrality forces the
residue to the upper endpoint band. This leaves an arithmetic obstruction;
it does not supply a central-band certificate inequality. \\
249-supply & TE-04,
...
249-supply & d4/d5,
\lean{linearIndependent\_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:946}, \lean{not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}{TotientCarryKernelRigidity.lean:301} &
...
\lean{one\_third\_lt\_scaledMersenneTail\_floorWall}{HalfCylinderMiddleCarryLowerBound.lean:2518} & multiple &
...
257-reset & E1,
\lean{SeamGreedyRemainderGapBound}{HalfCylinderSeamLimit.lean:266} & coordinate only &
...
\lean{exists\_greedyHalfFrozenMargin\_nonneg\_of\_excess\_neg}{HalfCylinderFiniteShadow.lean:1196} & scale only &
...
257-universal & NM-02,
\lean{infinite\_greedyMersenneSkippedSupport\_of\_half\_mem}{GreedyAchievementSet.lean:2545} & scale only &
