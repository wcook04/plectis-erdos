...
\begin{thm}[cert:a9 --- \texttt{\breakcode{eventual\_period\_of\_not\_irrational}}, THE TAIL-PERIOD LAW]
...
\begin{thm}[cert:d5 --- \texttt{\breakcode{not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}}]
...
\begin{thm}[cert:d4 --- \texttt{\breakcode{linearIndependent\_canonicalTotientKernelFamily}}]
...
\begin{prop}[mob:b5 --- top fibre survives, T3, \texttt{\breakcode{mobiusNumerator\_gcd\_cyclotomicValue}}]
...
For $t \ge 5$ (Bertrand's postulate supplies nonemptiness of $\mathrm{upperHalfPrimes}(t)$): \[\adjustbox{max width=\linewidth}{$\displaystyle 2^{t/2} \le \prod_{p \in \mathrm{upperHalfPrimes}(t)} \mathrm{mersenne}(p) \le \big(\mathrm{lcmHeight}(t)\cdot\mathrm{numericMobiusShadow}(\mathrm{lcmHeight}(t))\big).\mathrm{den}$}\]. An exponential-in-$t/2$ growth lower bound for the reduced denominator at every LCM height, produced from mob:b6's explicit surviving channel product. Denominator-only: does not by itself rule out cancellation by a foreign-defect term, hence does not by itself prove \#249.
...
For every $t$ (no lower bound on $t$ needed for this direction): \[\adjustbox{max width=\linewidth}{$\displaystyle \big(\mathrm{lcmHeight}(t)\cdot\mathrm{numericMobiusShadow}(\mathrm{lcmHeight}(t))\big).\mathrm{den} = \mathrm{mersenne}(\mathrm{lcmRadical}(t)) / \gcd\big(\mathrm{mersenne}(\mathrm{lcmRadical}(t)),\ \mathrm{lcmScale}(t)\cdot|\mathrm{oddJordanScalar}(\mathrm{lcmRadical}(t))|\big)$}\]. A fully closed form for the reduced denominator at every scale, produced (not merely bounded) as an explicit rational function of $t$.
...
$\forall h \in [1,8],\ \mathrm{certifiedKill}\ h\ 12\ 16$ — a finite, fully unconditional \texttt{decide}-checked family of 8 certificate witnesses at the fixed point $N=12$, $L=16$ (256 explicit totient values below 37 evaluated). Consequence, via cert:a6: \[\adjustbox{max width=\linewidth}{$\displaystyle \forall r:\mathbb{Q},\ 1\le h\le 8,\ r.\mathrm{den}\mid 2^{12}\cdot(2^h-1) \to S \ne r$}\].
...
\begin{prop}[cert:a6 --- certificate soundness, the kernel's converter direction, \texttt{\breakcode{tail\_diff\_notMem\_int\_of\_certifiedKill}}]
...
\begin{thm}[cert:a7 --- certificate completeness, \texttt{\breakcode{exists\_certifiedKill\_iff\_tail\_diff\_notMem\_int}}]
...
\begin{prop}[cert:d1 --- \texttt{\breakcode{irrational\_of\_den\_mul\_abs\_sub\_tendsto\_zero}}, generic Dirichlet-gap criterion]
...
\begin{prop}[cert:d2 --- \texttt{\breakcode{irrational\_of\_int\_mul\_near\_int}}, classical near-integer criterion]
...
\begin{prop}[cert:d9 --- \texttt{\breakcode{positive\_rational\_difference\_lower\_bound}}]
For $\mathrm{pfx} < \mathrm{whole}:\mathbb{Q}$ (strict): $1/(\mathrm{whole.den}\cdot\mathrm{pfx.den}) \le (\mathrm{whole}:\mathbb{R}) - (\mathrm{pfx}:\mathbb{R})$. A positive rational difference is bounded below by the reciprocal of the product of the two \emph{actual} reduced denominators, not a displayed/guessed one. Fully general; feeds \texttt{\breakcode{prefixDenominator\_shell\_power\_bound\_of\_rational\_difference}} and \texttt{\breakcode{nextSupport\_power\_bound\_of\_rational\_difference}}, converting any analytic upper bound on a rational gap into a denominator-growth lower bound — plug in any tail estimate from either open problem's coordinate.
...
\begin{prop}[mob:a2 --- squared-Lambert transfer engine, \texttt{\breakcode{tsum\_lambert\_linear\_weight\_sq\_pure}}]
...
\begin{prop}[mob:a4 --- $L_2(1)$, the known $q$-zeta anchor rung, \texttt{\breakcode{tsum\_one\_div\_mersenne\_sq\_eq\_sigma\_sub\_tau\_series}}]
...
\begin{prop}[mob:a5 --- $L_2(\varphi)$, the Pillai gcd-moment rung, \texttt{\breakcode{tsum\_totient\_div\_mersenne\_sq\_eq\_gcd\_moment\_series}}]
...
\begin{prop}[mob:a7 --- gcd-divisibility factorises, \texttt{\breakcode{tsum\_pos\_pair\_both\_dvd\_half\_eq\_inv\_mersenne\_sq}}]
...
\begin{prop}[mob:a8 --- reduced-direction law, \texttt{\breakcode{tsum\_pos\_coprime\_inv\_mersenne\_eq\_one}}]
...
\begin{prop}[mob:b1 --- repunit gcd word, T1, \texttt{\breakcode{mobiusNumeratorPolynomial\_eq\_gcdWord}}]
For $r$ squarefree, the divisor-signed polynomial \[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{mobiusNumeratorPolynomial}(r) := \sum_{d\mid r} \mu(d)\cdot(r/d)\cdot\mathrm{spacedRepunit}(d,r/d)$}\] equals $\mathrm{gcdWord}(r)$, whose $X^k$ coefficient ($k<r$) is $\mathrm{gcdWordCoeff}(r,k) = (r/\gcd(r,k))\cdot\varphi(\gcd(r,k))$, strictly positive for $k<r$ and zero for $k\ge r$. The signed repunit numerator is exactly a positive gcd word; the classical polynomial identity underlying it (signed spaced repunit $=$ gcd word) is base-independent, though the application here is Mersenne-specific.
...
\begin{prop}[mob:b2 --- evaluation at $2$ recovers the integer numerator, \texttt{\breakcode{mobiusNumeratorPolynomial\_eval\_two}}]
\[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{mobiusNumeratorPolynomial}(r).\mathrm{eval}\,2 = \mathrm{mobiusNumerator}(r)$}\] for $r$ squarefree, where \[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{mobiusNumerator}(r) := \sum_{s \subseteq \mathrm{primeFactors}(r)} (-1)^{|s|}\cdot(r/d)\cdot(\mathrm{mersenne}(r)/\mathrm{mersenne}(d))$}\], $d = \prod s$. Bridges the polynomial (mob:b1) world to the integer arithmetic used by every denominator-survival theorem below.
...
$\mathrm{baseMobiusShadow}(r) := \mathrm{mobiusNumerator}(r)/(2^r-1)$ (unscaled); \[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{numericMobiusShadow}(H) := \mathrm{baseMobiusShadow}(\mathrm{rad}(H))/\mathrm{rad}(H)$}\]; exactly \[\adjustbox{max width=\linewidth}{$\displaystyle H\cdot\mathrm{numericMobiusShadow}(H) = (H/\mathrm{rad}(H))\cdot\mathrm{baseMobiusShadow}(\mathrm{rad}(H))$}\] for $H>0$. Reduced-denominator identity for the unscaled shadow: \[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{baseMobiusShadow}(r).\mathrm{den} = \mathrm{mersenne}(r)/\gcd(|\mathrm{mobiusNumerator}(r)|, \mathrm{mersenne}(r))$}\] for $r>0$ — exact and generic, no coprimality assumed, no channel-survival hidden.
...
\begin{prop}[mob:b4 --- cyclotomic congruence, T2, \texttt{\breakcode{cyclotomic\_dvd\_mobiusNumeratorPolynomial\_sub}}]
...
\begin{longtable}{@{}p{.12\linewidth}p{.38\linewidth}p{.44\linewidth}@{}}
\caption{The Mersenne--Lambert ladder five-row status table (docstring @ \texttt{\breakcode{CertificateKernel.lean:18063--18083}}, body \texttt{MersenneLambertLadder.lean}).}\\
\toprule
\papertablehead
...
\begin{thm}[cert:b2 --- multiple-period collapse, wave-22, \texttt{\breakcode{irrational\_totient\_series\_of\_multiple\_certificate\_supply}}]
...
\begin{lem}[cert:b4 --- lcm-window structure, \texttt{\breakcode{eq\_prime\_pow\_of\_not\_dvd\_periodLcm}}]
...
\begin{thm}[cert:b6 --- lcm-cone flatness law, wave-24, \texttt{\breakcode{rational\_totient\_series\_forces\_lcm\_cone\_flatness}}]
...
\begin{thm}[cert:b7 --- cone collapse, wave-24, annihilator umbrella, \texttt{\breakcode{irrational\_totient\_series\_of\_lcm\_cone\_window\_kill\_supply}}]
...
\begin{prop}[cert:b9a --- second-difference certificates, sound but measured not shallower, \texttt{\breakcode{second\_diff\_notMem\_int\_of\_certifiedRank2Kill}}]
...
\begin{thm}[cert:b10a --- cone non-flatness menu refuter, wave-25, sharper than pairwise, \texttt{\breakcode{exists\_nonintegral\_pair\_of\_coneNonflatCert}}]
...
\begin{thm}[cert:b10b --- cone non-flat supply, wave-25, \texttt{\breakcode{irrational\_totient\_series\_of\_lcm\_cone\_nonflat\_supply}}]
...
\begin{thm}[mob:e1 --- first-harmonic norm-gap supply, \texttt{\breakcode{irrational\_totient\_series\_of\_first\_harmonic\_norm\_gap}}]
Hypothesis \[\adjustbox{max width=\linewidth}{$\displaystyle \mathrm{DTWFirstHarmonicNormGap} := \forall h>0,\ \forall X_0,\ \exists X\ge\max(X_0,1),\ \exists L,\ 16(2X+h+L+2)\le 2^L \wedge \big\|\sum_{N\in[X,2X)} \mathrm{windowFirstExp}(h,N,L)\big\| \le (21/25)\cdot X$}\]. If this holds then $\mathrm{Irrational}(S)$. A genuinely cofinal first-harmonic exponential-sum (Weyl-sum) cancellation statement is sufficient for \#249; unconditional companion \texttt{\breakcode{exists\_certifiedKill\_of\_first\_harmonic\_gap}} shows any constant-saving first-harmonic gap on one dyadic block forces a finite kill certificate (via $\cos(\pi/8)>9/10$ and a pigeonhole/averaging argument).
...
\begin{thm}[mob:e2 --- prime-jump sharp-kill supply, \texttt{\breakcode{irrational\_totient\_series\_of\_primeJumpSharpKill\_supply}}]
