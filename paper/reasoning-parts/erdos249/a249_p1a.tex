...
For $t \ge 5$ (Bertrand's postulate supplies nonemptiness of $\mathrm{upperHalfPrimes}(t)$):
\[
\begin{aligned}
2^{t/2}
...
For every $t$ (no lower bound on $t$ needed for this direction):
\[
\begin{aligned}
...
The unscaled and numerical shadows are
\[
\begin{aligned}
...
\noindent The constant in mob:b4 is the value of the gcd word at a primitive
$m$-th root of unity.  Since the $X^k$ coefficient of that word depends only on
...
sums~\cite[Theorem and (2), p.~2]{schramm-gcd}; mob:b4 specialises this to the
