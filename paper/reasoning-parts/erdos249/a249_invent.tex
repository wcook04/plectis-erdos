...
so by
\lean{Erdos249257.\allowbreak TotientTailPeriodKiller.\allowbreak windowFirstCos_le_\allowbreak add_of_tailOrbitGap}{PivotAntiReconstruction.lean:221},
...
\lean{Erdos249257.\allowbreak dtwWindowSeparatedPairs_iff_\allowbreak irrational_totient_series}{PivotAntiReconstruction.lean:1775}
...
\subsection{Route 5: the rationality-side rank bound is refuted}
...
\lean{Erdos249257.\allowbreak not_irrational_totientSeries_\allowbreak implies_unbounded_carryRank_unconditional}{TotientCarryKernelRigidity.lean:301}
...
\lean{Erdos249257.\allowbreak not_finiteDimensional_\allowbreak span_fullTotientKernel}{TotientMahlerDefect.lean:1146}
...
sections can contribute up to $e$ new directions, and no fixed $O(1)$ error
bound is available on that route.  That observation stands, and the earlier
greedy set construction is still inconclusive. \ev{Gap} \scale{uniform}
\coord{other:carry-kernel}.

The route is nevertheless closed, by a construction that does not need the
invalid bridge.  There is $c:\N\to\N$ with $c(n)\le n$, with
$c(n)=\varphi(n)$ at every odd $n$, with $|c(n)-\varphi(n)|\le2$, and with
$\sum_{n\ge1}c(n)2^{-n}=5/4$
(\lean{parity\_perturbed\_rational\_control}{ErdosProblems/Erdos249/ParityPerturbedRationalControl.lean:396}),
whose tempered orbit satisfies $\operatorname{rk}_e(u)\ge2^{e}-1$ for every
$e$
(\lean{control\_temperedOrbit\_carryRank\_unbounded}{ErdosProblems/Erdos249/ParityPerturbedRationalControl.lean:478}).
It transfers the independent odd totient channels directly
(\lean{finrank\_canonicalCarryKernel\_ge\_of\_oddAgree}{ErdosProblems/Erdos249/ParityPerturbedRationalControl.lean:441}),
so the omitted zero-residue sections never enter.
Every version of the wanted statement is therefore false, including a bound
allowed to depend on the sequence and the orbit.
Section~\ref{ssec:rankupper-retired} records the retirement in full.
\ev{Lean}

The Lean theorem for $\varphi$ remains valid: it uses the special structure
and independently proved linear independence of the totient kernel.  The
honest status is thus asymmetric.  The large $\varphi$-specific rank floor is
proved; the universal rationality-driven ceiling is refuted; and what stays
open is a ceiling proved from arithmetic of $\varphi$ that the control above
fails.
