...
\emph{Weyl differencing and van der Corput} do not simplify this phase. The
van der Corput inequality holds for arbitrary bounded complex
sequences~\cite[Theorem~2.1]{edeko-vdc}, so it applies here, but it leaves the
correlations of the phase with its shifts to be estimated; Weyl differencing
gains when a difference operator lowers the degree of a polynomial phase.
Here $N\mapsto D(h,N,L)/2^{L}$ is, by Lemma~\ref{lem:orbit}, the
...
problem and lowers nothing. This is
...
$E=\sum_n 1/(2^n-1)=\sum_N d(N)/2^N$~\cite{erdos1948lambert}, is the right shape for the
...
quantitatively. Erd\H{o}s's argument for $E$ solves congruences that make the
divisor counts along a block divisible by increasing powers of $2$, and a tail
bound then forces a long run of zero digits~\cite[pp.~63--66]{erdos1948lambert}.
That uses the large divisibility available to $d(N)$: over
$N\le Y$ the ratio of its maximum to its typical value is
$Y^{(\log 2+o(1))/\log\log Y}$. The coefficient here is
...
that is, $\log\log Y$, far below $Y^{c/\log\log Y}$.
...
\begin{thm}[Within the class $0\le c(n)\le n$, the gap is strictly stronger than irrationality]\label{thm:lacunary}
...
collapse mechanism using only irrationality and the bound $0\le c(n)\le n$ can
exist''. It also says precisely what the missing
...
for Lebesgue-almost every phase that is the law of the iterated logarithm of
Erd\H{o}s and G\'al for lacunary series~\cite[p.~65]{erdos-gal}, which says
nothing at the particular phase carried by $S$
...
needed to finish the numerical budget, namely
$\Psi(x,x^{1/u})\sim\rho(u)x$ for fixed $u$~\cite[(1.1), p.~268]{granville-smooth}
with $\rho(u)=1-\log u$ on $[1,2]$~\cite[(1.3), p.~268]{granville-smooth}.  In particular the earlier identity
...
\noindent
\ev{Math} \quad \scale{cofinal}. The least element of $B(1/5)$ is
$2\cdot3\cdot5\cdot7\cdot11\cdot13=30030$, with
$\varphi(m)/m=5760/30030=0.19181$, and the logarithmic density of $B(1/5)$ up
to $3\cdot10^{5}$ is $3\cdot 10^{-5}$ \ev{Cert}. Using $\eta=1/5$ itself would
require a rigorous upper bound for the limiting density $D(1/5)$, including
the part of $B(1/5)$ beyond the enumeration; the finite count does not supply
one.
...
family concern bounded multiplicative functions at fixed shifts, and they are
averaged: Matom\"aki and Radziwi\l{}\l{} compare short and long averages of
real multiplicative functions bounded by $1$, outside an exceptional set of
short intervals~\cite[Theorem~1]{matomaki-radziwill}; Tao proves the
logarithmically averaged two-point Chowla and Elliott
conjectures~\cite[Theorems~1.2--1.3]{tao-elliott}; and Tao and Ter\"av\"ainen
...
\paragraph{A distinction from digit-complexity methods.}
...
\paragraph{A single-phase heuristic, not a probability proof.}
...
\subsection{A counterfactual rank criterion, not a live route}

\paragraph{The exact statement that was considered.}
\[
...
\quad \ev{Lean}.  No such rationality-side upper bound is proved.  It is a
logically sufficient theorem schema for $\varphi$, but the generic schema
as displayed is false. The rational control in
\texttt{ParityPerturbedRationalControl.lean} satisfies $0\le c(n)\le n$,
agrees with $\varphi(n)$ at every odd $n$, and has binary sum $5/4$.
It has a tempered integral carry of rank at least $2^e-1$ for every $e$
(\texttt{control\_series}, line~379, and
\texttt{control\_temperedOrbit\_carryRank\_unbounded}, line~478, in the
supplied separate Lean release). Thus neither a uniform nor a subexponential
generic carry-rank ceiling follows from rationality and these size bounds.
This $5/4$ control is distinct from the bounded $3/2$ parity control in
...
proves the case $k=2$ independently.  An attempted generic counterexample
...
greedy set construction therefore supplies neither a counterexample nor
support for the criterion, and it does not restore the generic shortcut.
\ev{Gap} \scale{uniform}
...
invalid generic $O(1)$ bridge. The greedy construction above remains a
record of an unsuccessful argument. It is not the current status of the
generic ceiling: the separate $5/4$ control refutes that ceiling directly.
Any replacement upper bound must use additional totient-specific hypotheses
that this control does not satisfy.
...
\paragraph{First, a fact rather than an inference: the four live routes and
one retired shortcut concern the same digit sequence.}
...
5 (retired) & a counterfactual rationality-side upper bound on the carry-orbit kernel rank &
...
wall''. The four live routes measure the binary digit sequence of $S$ and of
its multiples $(2^{h}-1)S$ at different positions and margins.  The fifth row
is retained only to show why a generic rank-compression proof does not supply
another route.
...
$2^{d}$ for which no functional equation, modular structure or
continued-fraction structure is known.
...
proceeds by a crude uniform margin.
