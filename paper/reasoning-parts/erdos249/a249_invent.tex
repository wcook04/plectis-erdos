...
$b$-ary expansion, which shows that the block complexity $p(n)$ of the base-$b$
expansion of an irrational algebraic number satisfies $\liminf_n
p(n)/n=+\infty$~\cite[Theorem~1, p.~549]{adamczewski-bugeaud} \ev{Cited}. The
shape fits exactly. The hypothesis does
...
problem. No version of the method is known that runs from an analytic
...
logically sufficient new theorem schema, but the preceding mathematics does
not isolate it as an open proposition, so it is not counted among the surviving
routes.
...
only a counterfactual sufficient input; and the generic countermodel attempt
is inconclusive.
