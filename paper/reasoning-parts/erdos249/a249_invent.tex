...
\texttt{\breakcode{ErdosProblems/Erdos249/File.lean:line}}; the hyperlink target for
...
\lean{scaled tail-difference identity}{PivotAntiReconstruction.lean:54}
and
\lean{tail-orbit first-exponential identity}{PivotAntiReconstruction.lean:66}
...
\lean{approximation of the tail-orbit phase}{PivotAntiReconstruction.lean:202}
gives
$\bigl\|\,\mathrm{windowFirstExp}(h,N,L)-e(2^N\alpha_h)\,\bigr\|
 < 2\pi(N+L+h+2)/2^{L} \le 2\pi/1024 < 1/100$,
so by
\lean{cosine comparison from the tail-orbit gap}{PivotAntiReconstruction.lean:220},
...
$t$, with $m=N+t$,
\begin{equation}\adjustbox{max width=.98\linewidth}{$\displaystyle \label{eq:window}
  \frac{D(h,N,L)}{2^{L}}
...
\begin{equation}\adjustbox{max width=.98\linewidth}{$\displaystyle \label{eq:mobmers}
...
\begin{equation}\adjustbox{max width=.98\linewidth}{$\displaystyle \label{eq:primefibre}
...
(\lean{absolute tail-difference bound}{CarrySurvivorExtinction.lean:79},
