...
\#249 asks whether $S$ is irrational. The strongest unconditional theorem
recorded here is Theorem~\ref{thm:residueobservable}: for $m \ge 2$ and an
integer-valued letter map $f$ with $f(0) = 0$ and $f(r) \ne 0$ at some $r < m$
with $\gcd(r+1, m) = 1$, the series $\sum_n f(\varphi(n) \bmod m)/2^n$ is
irrational, so $A_m = \sum_n (\varphi(n) \bmod m)/2^n$ is irrational for every
$m \ge 3$. That settles the totient word at every fixed resolution. It decides
no instance of \#249, whose coefficients are the unreduced values
$\varphi(n)$, and $S$ itself is open. Everything after that theorem is
negative knowledge: this document assembles every recorded failure,
obstruction, countermodel and dead route the programme has produced against
\#249, and classifies each one by the exact class of argument it eliminates.
The claim being made is that those failures are not independent. They are
repeated measurements of a single obstruction, and that obstruction has a
shape which can be stated.
...
which it is actually proved. The first is the strongest unconditional theorem
in the programme about a series driven by $\varphi$ itself, and it is stated
first for that reason. Everything after it is negative knowledge about $S$:
obstructions, reformulations, countermodels and finite exclusions.

\begin{thm}[Irrationality of the fixed-resolution totient observables]
\label{thm:residueobservable}
Let $m \ge 2$, let $f : \N \to \Z$ satisfy $f(0) = 0$, and let $r < m$ satisfy
$\gcd(r+1, m) = 1$ and $f(r) \ne 0$. Then $\sum_{n} f(\varphi(n) \bmod m)/2^{n}$
is irrational. In particular $A_m = \sum_n (\varphi(n) \bmod m)/2^n$ is
irrational for every $m \ge 3$, and $m \ge 3$ is sharp, since $A_1 = 0$ and
$A_2 = 3/4$. The mechanism is the isolated-pulse argument of Erd\H{o}s 1948,
and reducing $\varphi$ modulo $m$ is what makes the pulses isolated; the
prior-art audit located no exact antecedent for the statement.

This decides no instance of \#249. The coefficients of $S$ are the unreduced
values $\varphi(n)$, and $S$ remains open. The Lean development lives in the
private authoring tree at
\texttt{ErdosProblems/Erdos249/ResidueClassTotientSeries.lean}, with
\texttt{irrational\_totientObservable} at line 502, the dyadic form
\texttt{fixed\_resolution\_observable\_irrational} at line 544 and the corollary
\texttt{residue\_series\_irrational} at line 570. It is absent from the release
tree at this checkpoint, so no link into the checkpoint is given.
\coord{binary-digit} \scale{uniform} \ev{Lean}
\end{thm}

\begin{thm}[Sharp positive rank-one floor]
\label{thm:rankonefloor}
Put $\Theta_2=\sum_{d\ge1}\mu(d)/(2^d-1)^2$, which equals $S-1/2$ by the
convolution identity $\ph=\mu*\mathrm{id}$ recorded by Steve Fan in his
comment of 16 May 2026 on \texttt{erdosproblems.com/249}.  For $e\ge1$ and
$Y\ge4$ set
\[
  Q(e,Y)=\frac{\bigl(\sum_{d=1}^{Y}\mu(d)/(2^d-1)^{e+2}\bigr)^2}
               {\sum_{d=1}^{Y}\mu(d)/(2^d-1)^{2e+2}} .
\]
Then $Q(e,Y)\ge Q(1,5)$ for every admissible pair with equality exactly at
$(1,5)$; $Q(e,Y)-\Theta_2>21/320$ for every admissible pair; $1/16$ is a valid
uniform floor and $1/15$ fails already at $(1,5)$; every nonempty finite
positive weighted average of admissible quotients keeps the same strict
$21/320$ separation; and $|q\Theta_2-p|>21q/320$ whenever $Q(e,Y)=p/q$ with
$q\ge1$.  The complete positive rank-one and positive direct-sum
approximation mechanism is therefore closed with an explicit constant.  Signed
cancellation and genuinely coupled higher-rank approximants lie outside every
statement here.  Novelty of the family is unassessed.
\coord{mobius-mersenne} \scale{uniform} \ev{Lean}
\lean{rankOneSubrankQuotient\_eq\_one\_five\_iff}{ErdosProblems/Erdos249/RankOneSharpFloor.lean:534}
\lean{rankOneSubrankQuotient\_sub\_theta\_two\_gt\_twentyOne\_div\_threeTwenty}{ErdosProblems/Erdos249/RankOneSharpFloor.lean:624}
\lean{positive\_direct\_sum\_sub\_theta\_two\_gt\_twentyOne\_div\_threeTwenty}{ErdosProblems/Erdos249/RankOneSharpFloor.lean:655}
\lean{primitive\_form\_abs\_gt\_twentyOne\_div\_threeTwenty}{ErdosProblems/Erdos249/RankOneSharpFloor.lean:693}
\end{thm}

\begin{thm}[The remaining analytic obligation, stated exactly]
\label{thm:normgap}
Write $D_{h,N,L}=\sum_{j<L}(\ph(N+h+1+j)-\ph(N+1+j))2^{L-1-j}$, which is
$\mathtt{windowDiscrepancy}(h,N,L)$.  Suppose that for every $h>0$ and every
$X_0$ there are $X\ge\max(X_0,1)$ and $L$ with $16(2X+h+L+2)\le2^L$ and
\[
  \Bigl\|\sum_{N=X}^{2X-1}e^{2\pi iD_{h,N,L}/2^L}\Bigr\|\le\tfrac{21}{25}X .
\]
Then $S$ is irrational.  The chain is checked with no remaining gap: the
complex norm saving dominates the real-part saving, a constant-saving
first-harmonic bound on one dyadic block forces a certified non-integrality
witness inside that block, and a cofinal supply of such witnesses is
irrationality.  The hypothesis is the whole remaining obligation and is one
exponential-sum cancellation bound at the resolution $2^L$ that
$\mathtt{certifiedKill\_depth\_floor}$ forces.  Section~\ref{sec:wall} shows
this hypothesis is inequivalent to $\mathrm{Irrational}(S)$, so a proof of it
is strictly stronger than the target.
\coord{binary-digit} \scale{cofinal} \ev{Lean} for the implication,
\ev{Open} for the hypothesis.
...
\begin{thm}[Certified continued-fraction denominator floor]
\label{thm:cfdenom}
Bracket $S$ by exact integers $\mathrm{lo}\le2^BS\le\mathrm{hi}$ at
$B=80\,000$ bits and run the interval continued fraction, emitting a partial
quotient only when both endpoints agree on it.  This forces $23\,438$ partial
quotients, and if $S=a/q$ in lowest terms with $q>0$ then $q\ge2^{39989}$ and
$q>10^{12038}$.  The two power bounds are checked separately against the
integer denominator of the last certified convergent.  The bit length
$39\,990$ and the decimal digit count $12\,039$ of that denominator are
lengths rather than exponents; printing them as exponents overstates the
result by one in each base, and earlier corpus prose did exactly that.  The
preceding convergent is separated from the enclosure and the arithmetic
reproduces the known expansions of $e$ and $\pi$.  The largest partial
quotient in the prefix is $29\,403$ at index $15\,470$, so the prefix shows
no anomalous rational approach.  This is an exact finite computation with no
floating-point decision anywhere in it and with no Lean proof; it is a
strictly larger floor than Theorem~\ref{thm:denom} and a strictly weaker
evidence band.
\coord{continued-fraction} \scale{bounded} \ev{Cert}
Verifier \codeid{formal_math/probes/certified_continued_fraction.py}, receipt
\codeid{state/formal_math/probes/erdos249_certified_cf_receipt.json}.
\end{thm}

The theorem is Theorem~\ref{thm:residueobservable}, stated at the head of this record; its Lean development is cited there.

\begin{thm}[Denominator exclusion, the headline kernel-checked fact]
\label{thm:denom}
...
neighbours.  This is the strongest kernel-checked size exclusion.  The
strongest size exclusion of any band is Theorem~\ref{thm:cfdenom}, at
$q>10^{12038}$, which carries no Lean proof.
\coord{farey} \scale{bounded} \ev{Lean}
\lean{Erdos249257.tsum\_totient\_div\_pow\_two\_ne\_ratCast\_of\_den\_le\_79639646646701375323355774875831053}{CertificateKernel.lean:18572}
...
\begin{prop}[Cofinal positivity and the endpoint obstruction]
\label{prop:sign}
...
This fixes the sign of the true survivor, but does not construct a
central-band certificate.  The top-edge residue remains compatible with
integrality and needs an independent arithmetic exclusion.
...
(\lean{Erdos249257.not\_irrational\_totientSeries\_implies\_unbounded\_carryRank\_unconditional}{TotientCarryKernelRigidity.lean:301}).
...
(\lean{Erdos249257.linearIndependent\_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:946}),
so the full family spans an infinite-dimensional space
(\lean{Erdos249257.not\_finiteDimensional\_span\_fullTotientKernel}{TotientMahlerDefect.lean:1146}).
...
(\lean{Erdos249257.dtwWindowSeparatedPairs\_iff\_irrational\_totient\_series}{PivotAntiReconstruction.lean:1775}).
\coord{binary-digit} \scale{cofinal} \ev{Lean}
\end{prop}
...
should carry into the next section: cofinal positivity leaves an endpoint-residue obstruction, the rank information
runs the wrong way, its
...
unbounded range. Proposition~\ref{prop:sign} illustrates why such sign
information can leave the arithmetic endpoint obstruction unresolved.
...
\subsection{Survivor 5, retired: the rationality-side rank upper bound is
false}

\begin{defn}[The refuted coordinate-disjoint obligation]
\label{defn:rankupper}
The obligation formerly listed here asked for a constant $C$ such that for
every $c : \N \to \N$ with $c(n) \le n$ and
...
$C$. Both versions are false. Section~\ref{ssec:rankupper-retired} exhibits a
rational coefficient sequence satisfying every hypothesis whose tempered orbit
has rank at least $2^{e}-1$ at every level
(\lean{control\_temperedOrbit\_carryRank\_unbounded}{ErdosProblems/Erdos249/ParityPerturbedRationalControl.lean:478}).
\coord{carry-rank} \scale{uniform} \ev{Lean}
\end{defn}

The counterexample is much closer to $\ph$ than a generic digit
construction. Its coefficient sequence agrees with $\ph$ at every odd
argument and differs from it by at most $2$ everywhere, so no rank argument
that reads only the odd channels, the size constraint $c(n)\le n$, or the
rationality of the value can supply the missing ceiling. The surviving
question is the totient-specific one: an upper bound for the carry orbits
that rationality of $S$ itself would supply, proved from arithmetic of $\ph$
that the control above fails to satisfy. That question is untouched by the
counterexample, and it remains open. \ev{Open}

The $\ph$-specific lower bound is unaffected (Proposition~\ref{prop:rank}),
as is the earlier finding that the most natural route to a ceiling is dead
(\lean{Erdos249257.false\_of\_compressedAdjointCertificate}{TotientMahlerDefect.lean:1183})
and that uniform eventual periodicity of the carry's dyadic sections mod $v$
does not promote to a $\Q$-rank bound
(Proposition~\ref{prop:period-not-rank}).
