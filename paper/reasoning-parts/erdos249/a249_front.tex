\section{Totient sections, tail differences, and the irrationality question}

The dyadic sections $n\mapsto\varphi(2^jn+r)$ through each level $e\ge1$
have exact rational rank $2^e+1$. The two zero-residue sections and the
odd-residue sections give independent coordinates; scalar reductions recover
the omitted sections. The short paper proves the all-base theorem
$\dim_{\Q}V_{k,e}=k^e+1$ for $e\ge1$, its integral normal form and the
evaluation-matrix argument. For composite $k$ the retained condition is
$k\nmid r$, not $\gcd(k,r)=1$. This record supplies
the tail identities, countermodels, and individual arithmetic constructions
needed to compare that structure with the irrationality question.
...
\begin{rem}[Literature]
Erd\H{o}s and Graham record the irrationality question for
$S$~\cite[p.~61]{erdosgraham1980}. Coons proved that $\varphi$
is not $k$-regular for any $k\ge2$~\cite[Theorem~3.2]{coons}, and Martin's
...
asymptotics for shifted convolutions of the divisor-convolution class that
contains $\varphi(n)/n$~\cite[Theorem~2.2]{bgs2016}; that theorem does not
supply the dyadic residue small-ball or phase anti-concentration required by
$\mathrm{Sep}$.  The remaining sources are cited where their hypotheses are
used.

At the generating-function level, the characteristic-zero multiplicative
classification of Bell and Smertnig gives $k$-Mahler $\Rightarrow$
$k$-regular, and hence rules out $k$-Mahler equations for the totient series
in every base~\cite[Thm.~1.3 and p.~3]{bell-smertnig2026}.
...
\[
  ...
  \mathrm{certifiedKill}(h,N,L).
\]
$\mathrm{Sep} \Longleftrightarrow \mathrm{Irrational}(S)$
...
and eight further period deposits at $N = 300$. These are historical
sub-banks. The supplied source also proves a complete diagonal band for
...
Note the quantifier order: the simultaneous small-shift deposit
...
\begin{prop}[Cofinal positivity and the remaining top-edge condition]
\label{prop:sign}
...
Positivity excludes the nonnegative endpoint survivor in this coordinate;
under integrality the negative survivor and its top-edge residue remain.
Thus positivity alone does not establish the central-residue inequalities.
This is a statement about these hypotheses, not an obstruction to every
possible use of the actual-LCM coordinate.
...
$2^{e}+1$ canonical dyadic channels is linearly independent over $\Q$
for $e\ge1$, via CRT plus Dirichlet
...
If $S$ is rational, its carry has both uniform eventual periodicity of its
dyadic sections modulo $v$ and unbounded rational section rank
...
\texttt{control\_temperedOrbit\_carryRank\_unbounded}, line~478 of
...
a top-edge condition; a large rational carry rank is not a contradiction;
and exact equivalences still require a certificate supply. The stated size
...
\section{What the countermodels exclude}
\label{sec:wall}
...
\subsection{Range, resolution, and the scope of an obstruction}
...
\item Fixed-prefix information alone is excluded by
  Theorem~\ref{thm:gamma} (B1): $\varphi$ may be pinned exactly on $[1,B]$ and
  the series can still be rational.
\item Parity information alone is excluded by
  Proposition~\ref{prop:parity} (B7): $\varphi$'s parity may be
...
\item B5 treats the stated bounds on carry state and precision. B6 treats
  the displayed linear relations and truncations. Neither is a theorem
  against every finite-state or compressed argument.
\item B4 records the cost of the specified congruence construction:
  prescribing divisibility can increase the index, which also increases
  the certificate's tail bound.
\item B2 identifies exact equivalences; their value depends on what new
  estimate the changed coordinates permit. B3 records finite checks and
  does not supply their missing universal quantifier.
\end{itemize}

The finite certificates in Proposition~\ref{prop:deposits} and the $K=240$
Farey calculation in Theorem~\ref{thm:denom} have bounded ranges. B1 excludes
...
B1's real contribution is narrower but still load-bearing: no finite deposit
by itself proves the cofinal assertion. Extending the deposit is not a
substitute for a uniform arithmetic argument; a new theorem that propagates
finite information could change the situation. The gap between the census and
the obligation is therefore a gap in kind for the method actually used, not a
proof that every possible proof is impossible. It is not a selector.
\end{rem}
...
Its proof first partitions the block into supplier and non-supplier bases,
then partitions the supplier bases according to the good-cofactor condition.
On the good part, subtracting the mean residual on each pivot fibre gives
the centred-correlation term and leaves exactly the fibre-mean term.  Thus
the four displayed quantities sum to the original first-harmonic block;
the identity itself provides no estimate for any of them.
...
None of the four displayed inequalities is currently proved.  The fibre-mean,
bad-cofactor and non-supplier terms have the form of counting estimates, while
the first is a genuine correlation estimate and is needed only in real part.
...
Survivor 1's burden; it is listed separately because its decomposition and
canonical-fibre identities are already exact on disk. The module asserts no prime-distribution
...
\[
  ...
  \;<\; 2^{td} - (N + 2td + 2).
\]
It is equivalent to $\mathrm{Irrational}(S)$.
...
For the actual totient series, $\mathrm{ApFullDepthEscape}$ is equivalent to
...
arithmetic input is a nonintegral seed for every relevant pair, not a converse
implication for the depth-locked condition. This remains an exact
...
is still on disk; it is not the current classification.
...
\subsection{Retired shortcut: a rationality-side rank upper bound}
...
$C$. Either assertion would contradict the proved floor of
Proposition~\ref{prop:rank} and would therefore close \#249.
...
\texttt{ParityPerturbedRationalControl.lean} has $0\le c(n)\le n$ and a
tempered integral carry of rank at least $2^e-1$ at every depth
(\texttt{control\_temperedOrbit\_carryRank\_unbounded}, line~478).
...
\textbf{The generic shortcut is retired.} The most natural approach to the
upper bound is dead
(\lean{Erdos249257.false\_of\_compressedAdjointCertificate}{TotientMahlerDefect.lean:1183}),
...
bound (Proposition~\ref{prop:period-not-rank}). Any viable rank argument would
therefore need new arithmetic specific to the actual totient carry.  The
criterion is retained to record that logical possibility and the no-go result,
not as a fifth surviving frontier.
