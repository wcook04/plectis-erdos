...
\begin{thm}[Fixed-resolution observables of the totient word are irrational]
\label{thm:residueobservable}
Let $m\ge2$, let $f:\N\to\Z$ satisfy $f(0)=0$, and let $r<m$ satisfy
$\gcd(r+1,m)=1$ and $f(r)\ne0$.  Then $\sum_{n}f(\ph(n)\bmod m)/2^n$ is
irrational.  At $m=2^k$ the unit condition holds automatically at every even
residue.  In particular $A_m=\sum_n(\ph(n)\bmod m)/2^n$ is irrational for
every $m\ge3$, and $m\ge3$ is sharp because $A_1=0$ and $A_2=3/4$.  The proof
supplies cofinally many two-sided isolated pulses by the Chinese remainder
theorem with Dirichlet's theorem, converts one isolated pulse into a
quantitative separation of $qT$ from every integer, and closes through a
number-theory-free dyadic criterion for bounded integer coefficient sequences.
The mechanism is the isolated-pulse argument of Erd\H{o}s 1948, and reducing
$\ph$ modulo $m$ is what makes the pulses isolated; the prior-art audit
located no exact antecedent for the statement.

Two boundaries.  This says nothing about $S$, whose coefficients are the
unreduced values $\ph(n)$, so the parent question stays open and stays
distinct.  And the Lean development lives in the private authoring tree, at
\codeid{ErdosProblems/Erdos249/ResidueClassTotientSeries.lean} with
\codeid{irrational_totientObservable} at line 502,
\codeid{fixed_resolution_observable_irrational} at line 544 and
\codeid{residue_series_irrational} at line 570.  It is absent from the pinned
formal-source checkpoint of this paper and is marked for the next formal-source
cycle, so no link is given here and none would resolve.
\coord{residue-observable} \scale{uniform} \ev{Lean} in the authoring tree,
