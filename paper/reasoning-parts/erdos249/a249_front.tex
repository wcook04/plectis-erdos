...
(\lean{totient tail}{TotientTailPeriodKiller.lean:57},
\lean{dyadic scaling identity for the totient series}{TotientTailPeriodKiller.lean:150}).
...
$N \ge N_0$
(\lean{eventual period forced by rationality}{TotientTailPeriodKiller.lean:358},
...
(\lean{dyadic window discrepancy}{TotientTailPeriodKiller.lean:66},
\lean{certified interior residue}{TotientTailPeriodKiller.lean:72}).
...
non-integrality
(\lean{certificate existence characterizes tail nonintegrality}{LcmConeFlatness.lean:316}).
...
(\lean{cofinal certificates imply irrationality}{TotientTailPeriodKiller.lean:394}).
...
(\lean{certificate depth lower bound}{TotientTailPeriodKiller.lean:79},
...
$Q_0 := 79\,\allowbreak 639\,\allowbreak 646\,\allowbreak 646\,\allowbreak 701\,\allowbreak 375\,\allowbreak 323\,\allowbreak 355\,\allowbreak 774\,\allowbreak 875\,\allowbreak 831\,\allowbreak 053 \approx 7.96 \times 10^{34}$.
...
\lean{denominator exclusion through the stated Farey bound}{CertificateKernel.lean:18384}
\lean{first denominator beyond the Farey certificate}{GapFareyBound.lean:225}
\end{thm}

This is obtained from a classical Stern--Brocot gap lemma
(\lean{Farey interval bound}{GapFareyBound.lean:51}) applied at window $K = 240$, the
...
(\lean{all diagonal certificates through depth 64}{DiagonalPincerCertificatesT64.lean:1967},
endpoint \lean{depth-64 diagonal certificate}{DiagonalPincerCertificatesT64.lean:1928});
all shifts $h \in [1,16]$ simultaneously at $(N,L) = (14,9)$, by
\texttt{decide}
(\lean{simultaneous certificates for shifts through 16}{CarrySurvivorExtinction.lean:574});
...
(\lean{positivity of shifted actual-LCM tail differences}{TotientActualLcmOrbitSign.lean:39},
\lean{positivity of the actual-LCM tail orbit}{TotientActualLcmOrbitSign.lean:144}).
In the same coordinate, integrality of the orbit forces the residue to the
\emph{top edge}, exactly $2^{K} - e$
(\lean{integral actual-LCM orbit forces the top-edge residue}{TotientActualLcmOrbitSign.lean:211}).
...
(\lean{rationality forces unbounded carry rank}{TotientCarryKernelRigidity.lean:300}).
...
(\lean{linear independence of canonical totient kernels}{TotientMahlerDefect.lean:935}),
so the full family spans an infinite-dimensional space
(\lean{infinite dimensionality of the full totient-kernel span}{TotientMahlerDefect.lean:1145}).
...
(\lean{rationality gives periodic reductions but unbounded rank}{TotientTailCarryPeriod.lean:224}).
\coord{carry-rank} \scale{uniform} \ev{Lean}
\end{prop}
...
kill supply
(\lean{period-multiple certificate characterization}{ErdosProblems/Erdos249/PeriodMultipleEscape.lean:432}),
the base certificate supply and its lcm-diagonal normal form
(\lean{cofinal certificate characterization of irrationality}{LcmConeFlatness.lean:412};
\lean{LCM-diagonal certificate characterization of irrationality}{LcmConeFlatness.lean:426}),
the directed-certificate supply and its LCM specialisation
(\lean{cofinal directed LCM certificate condition}{TotientTailCarryPeriod.lean:866}),
and the window-separated-pairs predicate
(\lean{separated-window characterization of irrationality}{PivotAntiReconstruction.lean:1765}).
\coord{binary-digit} \scale{cofinal} \ev{Lean}
\end{prop}
...
(\lean{rational parity countermodel with arbitrarily many separated carries}{TotientParityCoboundaryCountermodel.lean:637};
...
\lean{tail estimate}{TotientTailPeriodKiller.lean:198} and used
there to prove
\lean{certified residue forces a nonintegral tail difference}{TotientTailPeriodKiller.lean:262}.
...
\lean{tempered-orbit characterization of rational binary series}{GenericTailOrbitRigidity.lean:426};
...
\item \emph{Sample choice.} Any weakening in which the prover may \emph{choose}
  ...
  (\texttt{\breakcode{windowSeparatedPairsAt\_of\_cofinally\_scaled\_adjacent\_chord}},
...
\item \emph{Multi-point enrichment.} Adding vertices, higher-order differences,
  ...
  primality used
  (\lean{prime-adjunction diamond reduction}{FullTargetPrimeAdjunctionNoGo.lean:196}).
\end{enumerate}
...
(\texttt{\breakcode{totient\_tail\_rank\_two\_kill\_sound\_but\_not\_shallower\_cell}},
...
\lean{prime two-adic pulse-block construction}{TotientTwoAdicPulseBlock.lean:211}
\lean{half-turn congruence of the pulse discrepancy}{TotientTwoAdicPulseBlock.lean:262}
\end{prop}
...
by it, hence $0$
(\lean{impossibility of a roomy terminal dyadic staircase}{TotientActualLcmTopEdgeStaircase.lean:251});
...
(\texttt{\breakcode{puncturedDyadicStaircase\_penultimate\_eq\_half}},
\texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1187}}). The repair the barrier
...
\item Bounded local $2$-adic valuation-unit data at fixed precision excludes
  ...
  (\lean{fixed-precision tropical obstruction}{TropicalCurvatureCarry.lean:137}).
\end{enumerate}
...
\item \emph{Dyadic.} The canonical family of $2^{e}+1$ dyadic channels is
  ...
  (\lean{impossibility of a compressed adjoint certificate}{TotientMahlerDefect.lean:1183},
...
\item \emph{M\"obius incidence.} $U_N(i,j) = \mu((i{+}1)/(j{+}1))$ when
  ...
  (\lean{unit determinant of the incidence Mobius matrix}{IncidenceQuotientHermitePade.lean:56},
...
\item \emph{Adjugate reconstruction.} Any finite rational row exactly isolating
  ...
  (\lean{lower bound for the totient adjugate tail cost}{TotientTailCarryPeriod.lean:266},
...
\item \emph{Shift-polynomial.} An explicit nonzero all-horizon countermodel
  ...
  (\texttt{\breakcode{LcmFactorIdealPulseObstruction.lean}}; module docstring, theorem
...
(\lean{uniform rank-one subrank quotient gap}{ErdosProblems/Erdos249/RankOneSubrankObstruction.lean:238}).
...
\lean{first-harmonic norm-gap criterion}{FirstHarmonicPivot.lean:75}
\lean{first-harmonic norm gap implies irrationality}{FirstHarmonicPivot.lean:83}
\end{defn}
...
(\lean{first-harmonic average gap supplies a certificate}{FirstHarmonicGap.lean:128},
the elementary unconditional engine; the $21/25 \to 9/10$ bridge is
\lean{first-harmonic norm gap supplies a certificate}{FirstHarmonicPivot.lean:53}).
...
\item \textbf{B2 --- the decisive point.} The proved collapse
  ...
  (\lean{failure of certification forces a large first cosine}{FirstHarmonicGap.lean:44}),
  ...
  (\lean{subset first-harmonic gap supplies a certificate}{FirstHarmonicGap.lean:104})
...
It implies $\mathrm{Irrational}(S)$.
\coord{first-harmonic} \scale{cofinal} \ev{Lean}
\lean{pivot-residual decorrelation criterion}{FirstHarmonicPivot.lean:569}
\lean{pivot-residual decorrelation implies irrationality}{FirstHarmonicPivot.lean:577}
\end{defn}
...
(\lean{pivot decomposition of the first-exponential sum}{FirstHarmonicPivot.lean:514},
budget consumer
\lean{pivot budget implies the first-harmonic gap}{FirstHarmonicPivot.lean:549}).
...
(\lean{one-overlap characterization of a pivot fibre}{FirstHarmonicPivot.lean:423}),
...
(\lean{locked reconstruction preserves a nonzero minor}{ResidualGaugeObstruction.lean:94}).
...
\lean{natural-prime tail-orbit gap criterion}{ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean:157}
\lean{natural-prime tail-orbit gap implies irrationality}{ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean:524}
\end{defn}
...
\lean{full-depth arithmetic-progression escape criterion}{ErdosProblems/Erdos249/PeriodMultipleEscape.lean:441}
\lean{full-depth arithmetic-progression escape implies irrationality}{ErdosProblems/Erdos249/PeriodMultipleEscape.lean:450}
\end{defn}
...
(\lean{full-depth escape supplies period-multiple certificates}{ErdosProblems/Erdos249/PeriodMultipleEscape.lean:444}),
...
\lean{self-window discrepancy identity}{ErdosProblems/Erdos249/PeriodMultipleEscape.lean:67},
