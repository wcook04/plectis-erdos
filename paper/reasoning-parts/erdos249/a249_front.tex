...
the omitted sections. The short paper proves the corresponding integral
normal form and explains the evaluation-matrix argument. This record supplies
...
prove at present~\cite[p.~61]{erdosgraham1980}.  Coons proved that $\varphi$
...
slopes~\cite[Theorem~1]{martin-phi-inequalities}; the exact kernel ranks of
\S\ref{sec:mahler-defect} refine both.  Erick Wong posted a proof that
...
reformulations are equivalences rather than reductions, and the listed size
bound, parity and aperiodicity of the coefficient word are satisfied by a
...
sufficient theorem schema in a coordinate disjoint from the residue and
certificate statements, and B2's certificate completeness cannot reach it.
The proved scale side is uniform in $e$ (Proposition~\ref{prop:rank}), but the
present mathematics supplies no reason for rationality to impose the opposite
rank bound.
