...
$\mathtt{actualLcmTailOrbit}\ a = R_{2H}-R_H
...
\begin{prop}[Five sufficient links, each closing \#249 if proved]
\label{prop:te-chain}
...
same module. Each changes the form of the target by dropping a hypothesis, widening a band, or
asking only for magnitude in place of a signed inequality; no strict separation between them is
proved.
...
$1\le h\le f(N)$. This implies the obligation immediately (fix $h\ge1$, take $N_0$ large enough that
...
\nolinkurl{rational_totient_series_forces_lcm_cone_flatness}$\to$contradiction, the natural
...
residue avoids the arc,'' which does not exist in the corpus. Route (i) needs truncation errors
with $\mathrm{den}(u_t)\cdot|S-u_t|\to0$; a larger lower bound for the surviving denominator,
such as $2^{t/2}$, makes this requirement harder.
\end{prop}
...
\begin{prop}[The extra hypothesis needed by a rank argument]
...
same file family. Five sufficient forms are already proved, related by the implications of
Proposition~\ref{prop:te-chain}, and proving any one of them closes \#249.

Third, the retired d4/d5 rank shortcut is structurally independent of the
residue-coordinate rows above.  An upper rank bound would contradict the
proved $2^e{-}1$ lower bound, but the corpus supplies no reason for such a
bound, and a natural finite-rank strengthening is already proved impossible.
It is therefore a counterfactual sufficient input and a useful no-go record,
not a third live obligation.

No row in this index is a solution, a partial solution in the sense of resolved cases, or evidence
that \#249 is likely true or false. Every actual open producer listed is exactly as open as the
original Erd\H{o}s--Borwein question it reduces to; the d4/d5 row is retained only as the
explicitly retired shortcut. What the index adds is the exact remaining content, verified against
the live Lean tree, of each reduction.
