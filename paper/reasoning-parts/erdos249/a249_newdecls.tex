...
unconditional order-two Hankel (log-concavity) theorem for every rung $r\ge 1$.
...
\begin{thm}[The first two rungs: a rational value and the totient offset]
...
\begin{thm}[Full ladder: unconditional strict log-concavity, every rung $r\ge 1$]
...
\texttt{RankOneSubrankObstruction} is a uniform proved barrier for primitive
rational linear forms obtained from rank-one strict-subrank monomial quotients
of finite Möbius--Mersenne prefixes.  It proves that no construction in this
class can approximate \(\Theta_2\) with a small linear form.
...
One of the eight is certified at its own locked depth $L=h=67$, a concrete
instance of the depth-equals-period form $\mathrm{ApFullDepthEscape}$
(equivalent to irrationality; this is one finite certificate, not a seed for
every pair).
