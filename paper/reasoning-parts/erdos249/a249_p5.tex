...
\paragraph{Five further links, each proved sufficient.}
The same file proves five further cofinal predicates, each of which implies
\texttt{PowerTwoActualLcmTopEdgeResidueGapSupply} and has a direct \#249
endpoint of its own; proving any one of them closes \#249.
...
\paragraph{A sixth link, and the exact identity pinning it.}
A sixth predicate is the disjunction of (v)'s inequality with the
opposite-direction escape:
\[
...
$\mathrm{lcmRayArithmeticLetter}(t,j) = \varphi(2H_t+j)-\varphi(H_t+j)$, with
$H_t=\mathrm{periodLcm}(t)$, for
\emph{every} offset $j$
...
\]
(or any bound growing slower than $2^e-1$). For arbitrary rational
coefficient series this schema is refuted by the $5/4$ control and
\texttt{control\_temperedOrbit\_carryRank\_unbounded}, line~478 of
\texttt{ParityPerturbedRationalControl.lean}. An upper bound for the actual
totient carry would therefore require additional arithmetic hypotheses;
rationality and periodic denominator data alone are insufficient.
...
= \varphi(2H{+}2q{+}2)-\varphi(H{+}2q{+}2)$ measured against twice a centred
