...
\ev{Open}: nothing in this corpus decides it. Every attack recorded in Parts
...
cannot supply the predicate. This section renders every such obligation
...
\lean{Erdos249257.totient\_tail\_window\_kill\_exists\_iff\_tail\_diff\_not\_int}{CertificateKernel.lean:18978})
...
witness already at $t=t_0$.  $P(t)$ is verified for every $t\le82$
...
(\lean{Erdos249257.certifiedKill_diagonal_all_imported}{DiagonalPincerCertificates.lean:2860},
...
\[
  ...
  \ \Longrightarrow\ \mathrm{Irrational}(S).
\]
\lean{Erdos249257.irrational_totient_series_of_lcm_cone_window_kill_supply}{CertificateKernel.lean:18967}
...
(\lean{Erdos249257.rational_totient_series_forces_lcm_cone_flatness}{CertificateKernel.lean:18955},
\ev{Lean}, unconditional). A sharper cone-form producer,
...
\lean{Erdos249257.irrational_totient_series_of_lcm_cone_nonflat_supply}{CertificateKernel.lean:19093}.
...
\[
  ...
  \ \Longrightarrow\ \mathrm{Irrational}(S),
\]
\lean{Erdos249257.irrational_totient_series_of_lcm_diagonal_nonintegrality_supply}{CertificateKernel.lean:18987}
(cone analogue at
\lean{Erdos249257.irrational\_totient\_series\_of\_lcm\_cone\_nonintegrality\_supply}{CertificateKernel.lean:18999})
...
$h,X,L$. A strictly stronger \emph{subset} form (a saving on any nonempty
...
and averaging). \emph{What supplying this form would take:} genuine
...
\ev{Lean}, unconditional, no rationality hypothesis). \emph{Do not attempt a
...
\quad \scale{bounded} \quad \coord{mobius-mersenne}. Note this is a
...
corpus, needing no rationality assumption at all. Its companion shows that
...
\lean{Erdos257PeriodNoncollapse.DiagonalFreshLossBridge.PowerTwoOddWindowAffine.actualLcmTailOrbit_notMem_int_of_actualTerminalDominance}{TotientActualLcmTopEdgeStaircase.lean:1880}. This is the sharpest
finite arithmetic statement of ``what remains to prove'' anywhere in the
...
\ev{Cert} \scale{fixed}. The bounded stub
...
\subsection{Two further, logically independent obligations}

Two more open statements would \emph{also} close \#249, each living in a
genuinely different coordinate from the certificate/residue coordinate
above, and each finished on exactly one side of an inequality.

\paragraph{Obligation 2 (rank compression).}
...
\lean{Erdos249257.linearIndependent_canonicalTotientKernelFamily}{TotientMahlerDefect.lean:900}
...
\]
\lean{Erdos249257.not_irrational_totientSeries_implies_unbounded_carryRank_unconditional}{TotientCarryKernelRigidity.lean:284}
\quad \ev{Lean} \quad \scale{uniform} \quad \coord{other:carry-kernel-rank}.
The missing input is the opposite inequality — a rationality-side rank
\emph{upper} bound:
\[
...
\]
(or any bound growing slower than $2^e-1$). \ev{Open}
\scale{cofinal} \coord{other:carry-kernel-rank}. One natural route to it is
proved dead: no \texttt{CompressedAdjointCertificate} ($Q\cdot v\cdot A =
\mathrm{boundary}$ with $|\mathrm{boundary}|<Q\cdot v$ and $A\ne0$) can
exist —
\lean{Erdos249257.false_of_compressedAdjointCertificate}{TotientMahlerDefect.lean:1051}
\quad \ev{Lean} — and the full family is proved infinite-dimensional in
span:
\lean{Erdos249257.not_finiteDimensional_span_fullTotientKernel}{TotientMahlerDefect.lean:1014}
\quad \ev{Lean}. What rationality \emph{does} buy — uniform eventual
...
\[
  ...
  \ \Longrightarrow\ S\ne p.
\]
\lean{Erdos249257.tsum_totient_div_pow_two_ne_ratCast_of_den_le_79639646646701375323355774875831053}{CertificateKernel.lean:18337}
...
continued-fraction convergents, both hard-coded numerals. The cofinal
...
constant, uniform in $K$. \ev{Open} \scale{cofinal} \coord{farey}. One
