...
(wrapped as \texttt{\breakcode{totient\_tail\_window\_kill\_exists\_iff\_tail\_diff\allowbreak\_not\_int}} at
...
rational, \texttt{\breakcode{eventual\_period\_of\_not\_irrational}}
...
$R_{N+h}-R_N\notin\mathbb{Z}$, a contradiction via \texttt{\breakcode{tail\_diff\_notMem\_int\_of\_certifiedKill}}
...
than \texttt{\breakcode{PowerTwoActualLcmTopEdgeResidueGapSupply}} (each implies it, so
...
$}\]
where \[\adjustbox{max width=\linewidth}{$\displaystyle d=\mathrm{diagonalWindowIncrement}(2^a,2q{+}2)$}\],
...
\begin{center}
\begin{longtable}{p{0.28\linewidth} p{0.18\linewidth} p{0.40\linewidth}}
\textbf{Form} & \textbf{Relation to Obligation 1} & \textbf{Site} \\
\hline
\endhead
Sep$(h,N,L)$ supply (Obl.\ 1, base form) & \textbf{equivalent} to Irrational$(S)$ & \texttt{\breakcode{LcmConeFlatness.lean:412}} \\
multiple-period supply & propositionally equivalent via Obl.\ 1; explicit directions are Obl.\ 1 $\Rightarrow$ multiple and multiple $\Rightarrow$ irrationality & \texttt{\breakcode{CarrySurvivorExtinction.lean:502}} \\
lcm-diagonal supply $P(t)$ & \textbf{equivalent} to Irrational$(S)$; single free parameter & \texttt{\breakcode{LcmConeFlatness.lean:426}} \\
lcm-cone supply & propositionally equivalent via Obl.\ 1; $P(t)$ is the cell $q{=}m{=}1$ & \texttt{\breakcode{CertificateKernel.lean:18686}} \\
cone-menu (\texttt{coneNonflatCert}) supply & weaker; half the pairwise radius & \texttt{\breakcode{CertificateKernel.lean:18812}} \\
pure non-integrality (diagonal/cone) & \textbf{equivalent} to the corresp.\ certificate form, via the completeness iff & \texttt{\breakcode{CertificateKernel.lean:18706}}, :18718 \\
\texttt{\breakcode{DTWFirstHarmonicNormGap}} & sufficient for Obl.\ 1; neither comparison direction is established & \texttt{\breakcode{FirstHarmonicPivot.lean:83}} \\
actual-orbit nonintegrality supply & \textbf{equivalent} to Irrational$(S)$ itself & \texttt{\breakcode{TotientActualLcmOrbitNonintegrality.lean:37}} \\
short-arithmetic-kill supply & special case of $P(t)$: powers of two, short window & \texttt{\breakcode{TotientActualLcmOrbitArithmetic.lean:2107}} \\
top-edge residue-gap supply & sufficient for actual-orbit supply; one-sided & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1325}} \\
adjacent-suffix midband supply & weaker than top-edge residue-gap & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1334}} \\
odd-guard half-word band supply & weaker again; \textbf{equivalent} to final-magnitude form & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1349}} \\
actual final top-edge magnitude supply & \textbf{equivalent} to odd-guard half-word band & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1471}} \\
flexible top-edge magnitude supply & weaker than midband; feeds midband, not corridor-escape & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1927}} \\
flexible terminal-dominance supply & weaker again; one-sided & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1908}} \\
flexible terminal carry-corridor-escape supply & \textbf{weakest known} in this cluster & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:1895}} \\
raw-approximant separation supply & alternative (Diophantine) sufficient form, same target & \texttt{\breakcode{TotientActualLcmOrbitSeparation.lean:305}} \\
rank-compression upper bound (Obl.\ 2) & independent obligation, different coordinate & \texttt{\breakcode{TotientCarryKernelRigidity.lean:284}} \\
Farey growth law (Obl.\ 3) & independent obligation, different coordinate & \texttt{\breakcode{GapFareyBound.lean}}, \texttt{\breakcode{CertificateKernel.lean:18056}} \\
guard-cylinder witness & problem-agnostic normal form of \emph{any} certificate, not scale-comparable & \texttt{\breakcode{TotientActualLcmTopEdgeStaircase.lean:844}} \\
\end{longtable}
\end{center}
