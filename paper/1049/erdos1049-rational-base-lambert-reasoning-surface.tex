...
claimed order. This verifies the displayed decimal truncations. The
program that carried out the calculation is not published with this
edition. The proofs use the exact definitions and never these decimal values.
...
margins and the four fixed-denominator comparisons below; its program
is not published with this edition. The
...
and a second implementation repeats it with $512$ tail terms and a
different enumeration order. Neither program is published with this
edition. The calculation proves optimality only in the displayed finite box. No
...
comparison for every tuple in its finite box. The programs for these
two exact checks are not published with this edition. The historical
...
A computer-algebra calculation reconstructs the forms of
...
were computed by a program that also repeats the direct determinant
checks; it is not published with this edition. At ranks six to eight
