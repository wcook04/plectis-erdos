...
is ordinary mathematics. Lean source for the all-row statement is published
outside the checked library build and has never been compiled by public CI.
...
\paragraph{Evidence decomposition.}
The proof above is ordinary mathematics.  One part of it is kernel-checked
in this library: the all-depth associated-graded recurrence
\lword{Erdos1049/AdelicHeightBridge.lean}{183}{hankelAssociatedCoeff_eq_reciprocal}{checked}.
The initial monomial of transformed row $1$ in every column, of order exactly
$l+1$ and coefficient exactly $-6$, and the two closed forms
$6\operatorname{ord}=N(N-1)(2N-1)$ and
$2^{N}\mathrm{lc}=(N!)^{2}(N+1)!$, are ordinary in this record.  Lean source
in \texttt{Erdos1049/AllRow} contains statements and proof scripts for the
initial monomial of every transformed row and for the order and leading
coefficient of $V_N^{*}$ at every rank, with no row hypothesis.  That source and
\texttt{ZudilinSharpHankelCoefficient.lean}, which it imports, lie outside the
checked library build, because the normalised moment definitions they use are
absent from the published \texttt{AdelicHeightBridge.lean}.  Public CI has never
compiled either file, so this record treats them as unchecked.  Independently of the proof, the order and the
...
one.  Lemma~\ref{long1049:res:allrowinitial} and~\eqref{long1049:eq:row-initial} give the initial monomial of every row, and the Lean
source in \texttt{Erdos1049/AllRow} states that identity for every row.  The
formal check still requires a bridge module carrying the moment definitions, a
successful compilation of that source against it, and an import into the
supported library root.  Nothing in
...
are proved in Lean at the linked revision, including the
