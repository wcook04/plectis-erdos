...
\begin{abstract}
Specialising Zudilin's 2004 construction for \(q\)-harmonic series to rational
bases, we prove that \(F(a/b)=\sum_{n\ge1}((a/b)^n-1)^{-1}\) is irrational for
every pair of coprime integers
...
printed by Zudilin for his \((14,12,14;27)\) direction, whose ratio is
the integer-base irrationality-exponent bound proved there. The linear forms,
their permutation-group integrality, the direction and the constants come
from Zudilin's 2004 paper; the rational specialisation, the cancelled-degree
accounting and the remainder estimates are this paper's own work. The region
contains \(31/4\) and every power \((31/4)^r\), \(r\ge1\), which lie outside
the published sufficient region of Bundschuh and V\"a\"an\"anen.
The same construction gives
\(\mu_{\rm irr}(F((a/b)^r))\le(1-\theta)/(\theta^{*}-\theta)\), uniformly for
every integer $r\ge1$, and in particular a bound below $301$ for every power
of $31/4$.

The proof cancels cyclotomic factors before clearing the rational-base
denominator. The degree of the cancelled polynomial pair measures the cost,
and positivity prevents the remainder from vanishing. This is an ordinary
proof using the polynomial conclusion of Zudilin's Lemma~7. Lean checks the
irrationality statements, with the source supply applied.

A separate degree-budget theorem bounds the sufficient cutoff delivered by
the stated base-uniform estimates by \(1/2\). It does not exclude every
approximation family at \(3/2\).
...
leading coefficient \((N!)^{2}(N+1)!/2^{N}\), sharpening Zudilin's lower
bound for the order. After a row transformation, one determinant term has
uniquely least order, so its coefficient cannot cancel. This complete identification
is ordinary mathematics, and Lean checks the order and the leading coefficient
at every rank.
The record also gives a separate positive-measure proof of
fixed-real-base size estimates.

At \(3/2\), local residue collisions yield divisible row combinations. To
prove irrationality one still needs combinations whose divided integral
linear forms are nonzero and tend to zero as real numbers. The record
preserves the source-specific obstructions and quantitative criterion for
that missing step. The universal rational-base assertion remains open.
\end{abstract}

\begin{paperresult}{What the paper proves}
\textbf{A rational-base region from Zudilin's forms.} $F(a/b)$ is irrational for coprime
...
(Theorem~\ref{long1049:res:31over4}).  \textbf{Evidence.} An ordinary proof that
specialises Zudilin's forms, using his Lemma~7 with the inputs of its proof,
his direction and his constants, with the finite height
...
the stated scalar mechanism and the displayed first-order exponent model there.
...
integer-base theorem~\cite{erdosproblems}.  Specialising Zudilin's 2004
...
\paragraph{Structure.}
The rational-base theorem and its cyclotomic degree calculation are
Theorem~\ref{long1049:res:region}; the base $31/4$, which lies outside the
Bundschuh--V\"a\"an\"anen region, is Theorem~\ref{long1049:res:31over4}.  The independent determinant argument is
Theorem~\ref{long1049:res:zudilin-sharp-qorder}, whose proof identifies the
unique least-order term at every rank.  The result-family catalogue at the end
lists the older families admitted to the public review registry.  It is not an
exhaustive index of the theorems proved in this record, so these three labels
are the direct route to its two principal arguments.
...
generalises Vandehey's Theorem~1.2 \cite[p.~3]{duverneytachiya2019}.  Each of
these results assumes an integer base.
...
specialise the construction of \cite{zudilin2004} to rational bases.  The linear
forms, the integrality of Lemma~7 there with the inputs of that lemma's own
proof, the direction and the constants come from that paper, and the
cyclotomic limits are its Lemmas~1--2 \cite[p.~155]{zudilin2004}, proved by
the method of \cite[Lemma~1, p.~466]{zudilin2002}.  The rational
specialisation, the cancelled-degree accounting and the remainder estimates
are this paper's own work and are proved in Section~\ref{long1049:sec:region}.
Lean checks both theorems, with the source supply applied rather than assumed:
the region theorem is
...
The linked Lean declarations establish exact inequalities,
...
inside the enlarged logarithmic region and outside the earlier one.
Bundschuh and V\"a\"an\"anen's irrationality theorem
...
\cite[Sec.~5, pp.~161--162]{zudilin2004} and put
\[
...
rational specialisation, the exact degrees, positivity and the size estimates
are this paper's own work.
...
proves them by that method with an explicit truncation estimate.  Kernel-checked are the finite height comparisons of
...
reconstructions of $U_n$ and $V_n$ for $n\le4$; the values of $J$, $C_0$ and
$\theta^{*}$ are numerical evaluations.  Both are computational receipts, and
the reconstruction is described in Section~\ref{long1049:sec:receipts}.
...
Four finite computations support statements above.  The reconstruction and
...
and, at every rank with no row hypothesis, its conclusions.  The order and the
...
vanish, all minors vanish, the sharper $N<2^k$ collision threshold applies,
and at modulus $2^S3^R$ with $R>0$ the depth $k\ge S+2R$
suffices.  These three statements are proved in this record.  Their
Lean proofs are at the revision linked in Section~\ref{long1049:sec:endpoints}; the pinned
revision~\commitshort\ does not contain those declarations.  Neither coordinate must be a unit;
$(2,3)$ modulo six is an example.
