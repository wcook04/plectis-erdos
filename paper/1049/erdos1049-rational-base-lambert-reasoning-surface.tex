...
\begin{abstract}
We prove that \(F(a/b)=\sum_{n\ge1}((a/b)^n-1)^{-1}\) is irrational for every pair of coprime integers
...
the integer-base irrationality-exponent bound proved there. This region
contains \(31/4\) and every power \((31/4)^r\), \(r\ge1\), beyond the
published sufficient region of Bundschuh and V\"a\"an\"anen.
...
inequality alone. After a row transformation, one determinant term has
...
Theorem~\ref{long1049:res:region}; the first newly covered base is isolated in
Theorem~\ref{long1049:res:31over4}.  The independent determinant argument is
...
and every further step is proved in Section~\ref{long1049:sec:region}.  Lean checks
both statements, with the source supply applied rather than assumed: the region
theorem is
...
The construction of the approximations is Zudilin's
\cite{zudilin2004,zudilin2016}; what is checked here is the Lean formalisation
of its consequence.  The linked Lean declarations also establish exact inequalities,
...
identity~(9)--(11), all in \cite[pp.~156--161]{zudilin2004}.  Everything else above is
proved here:
...
on $|V_N^{*}|$ \cite[Sec.~4, p.~7]{zudilin2016}.  Equality is the statement that no
...
The classification of Bell and Smertnig implies that $\mathcal L$ is not
$k$-Mahler for any $k\ge2$ \cite[Theorem~1.3 and p.~3]{bellsmertnig2026}, so the
simultaneous obstruction is subsumed for this scalar function.  The proposition
remains useful as an elementary proof isolating simultaneous closure.  A
...
suffices.  These three statements are ordinary in this record;
those Lean names are not in this checkout.  Neither coordinate must be a unit;
