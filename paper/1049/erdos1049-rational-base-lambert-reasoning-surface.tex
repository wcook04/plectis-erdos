...
\begin{abstract}
We specialise Zudilin's 2004 $q$-harmonic linear forms at rational bases.
For coprime integers $a>b\ge1$, put $\theta=\log b/\log a$.
Then $F(a/b)=\sum_{n\ge1}((a/b)^n-1)^{-1}$ is irrational if
$\theta<\theta^*=0.4056830213840605\ldots$, and
\[
 \mu_{\rm irr}\!\left(F((a/b)^r)\right)
 \le\frac{1-\theta}{\theta^* -\theta}\qquad(r\ge1).
\]
In particular, every positive integral power of $31/4$ is covered, with
bound less than $301$. The forms, permutation-group integrality, direction
and constants are inherited from Zudilin. The proof here cancels before
homogenising and establishes the cancelled degree and positive remainder
needed for rational specialisation.
For the different normalised Hankel family in Zudilin's 2016 construction,
we prove at every rank
\[
 \operatorname{ord}_q V_N^*=\frac{N(N-1)(2N-1)}6,
 \qquad [q^{\operatorname{ord}_q V_N^*}]V_N^*
       =\frac{(N!)^2(N+1)!}{2^N}.
\]
A separate positive-measure argument gives fixed-real-base positivity and
size estimates. These statements do not determine cyclotomic content or
positivity of the coefficient sequence. We formulate those two questions
for an explicit polynomial pencil and provide reproducible finite-rank
calculations through rank eight. These certify coefficient-matrix
positivity and real pencil roots below the Lambert value through that rank,
without an all-rank moment representation.
The remaining sections prove restrictions for specified clearing,
rank-two and endpoint-jet constructions. They separate a divisible
selector relation from a small nonzero integral form. No irrationality
statement at $3/2$, or universal rational-base theorem, is obtained.
The principal irrationality and formal-order results have source-applied
Lean declarations; the ordinary proofs and their dependencies are given here.
\end{abstract}

\begin{paperresult}{Reading map}
The two principal arguments are independent.
For rational specialisation, read Theorem~\ref{long1049:res:region}, the
cancelled-degree calculation and the positive remainder, then
Theorem~\ref{long1049:res:31over4} and the irrationality-exponent bound.
For the 2016 Hankel family, read
Theorem~\ref{long1049:res:zudilin-sharp-qorder} and the separate
positive-measure argument. Section~\ref{long1049:sec:coefficient-questions}
then distinguishes coefficient moments from cyclotomic content.
The degree-budget theorem concerns only its displayed base-uniform
rank-two hypotheses. The endpoint-jet sections give necessary further
nonvanishing and size obligations at $3/2$, not a proof at that base.
For finite coefficient positivity and pencil roots, read the certificate
statement and Proposition~\ref{long1049:res:finite-pencil}. The registry
appendix is provenance material, not the theorem index.
\end{paperresult}
...
absolute convergence for every real $t>1$. The coefficient of $t^{-m}$
is the number of divisors of $m$.
The question is a conjecture of Chowla; Erd\H{o}s proved it for every integer
$t\ge2$~\cite{erdos1948}.  The catalogue snapshot cited here records that rational-base question and
its attribution to Chowla~\cite{erdosproblems}; it is not a guarantee that
the bibliography is complete. Specialising Zudilin's 2004
construction~\cite{zudilin2004} to rational bases, this note proves $F(a/b)$
...
$31/4$ and every power $(31/4)^{r}$, $r\ge1$.  This paper does not establish the universal conjecture over all rational
$t>1$. The cited literature already covers non-integral rational bases,
including $7/2$ below; no claim of a first non-integral example is made.

Write $t=r/s$ in lowest terms with $r>s\ge1$, so that $s=1$ is exactly the
integer case Erd\H{o}s settled.  Among reduced non-integral rational numbers greater than one, $3/2$ has
the least naive height $H(r/s)=\max(r,s)$. It is the particular base whose
remaining approximation obligations are examined here.  A published height criterion of Bundschuh and
...
\paragraph{Dependencies and conventions.}
The rational-base proof uses the 2004 coefficient polynomials; the Hankel
proof uses the different 2016 normalised remainder sequence. The first is
not a change of notation for the second. We use $q=p^{-1}$ for a fixed real
base $p>1$, and Gaussian polynomials ${m\brack k}_p$ have base $p$.
The approximation scale is $n$ and the Hankel rank is $N$.
Formal order as $q\to0$, real asymptotics at fixed $q$, polynomial content
in $\Q[p]$, and specialised integer-row content are separate notions.
A cited construction, an ordinary deduction, an exact finite computation
and a kernel-checked declaration are identified where used, rather than
collected under a single claim of verification.
Empty products and determinants have value $1$. All asymptotic estimates
with a subscripted base hold with that base fixed. In the 2004 construction
alone, $N=15n$ denotes the cyclotomic cutoff; elsewhere $N$ is a Hankel
rank. The symbol $\mu=C_1/C_0$ is a construction parameter, whereas
$\mu_{\rm irr}(\xi)$ denotes the exponent of a particular irrational value.


\paragraph{Formal identity and historical integer-base results.}
...
\paragraph{Related kernels and non-transfer of recurrences.}
...
compared here; Theorem~\ref{long1049:res:region} below enlarges the region that
...
$q$-logarithm of that paper.  Section~\ref{long1049:sec:region} carries out such an
extension for $F$ itself: specialising the 2004 forms at a rational base and
carrying out the denominator accounting shows that
...
$(3,2)$.  Here a \emph{jet} means a
residue of a specialised coefficient modulo a prime power, not a derivative jet: the bottom jet is
...
\paragraph{Sharpness.}
The following sharpness claims concern the stated models, not all
approximation families. Two questions of scope are worth isolating.  The corridor bound of
...
\paragraph{Scope of the present results.}
The results below concern the displayed propositions and the supplied
formal-source snapshot. The paper does not prove irrationality at $3/2$.
An exact finite calculation, a conditional formal interface and a
source-applied all-rank declaration have different scopes; the appendix
records their separate scopes and dependencies.

\paragraph{Results and boundary.}
Theorems~\ref{long1049:res:region} and~\ref{long1049:res:31over4}
specialise the construction of~\cite{zudilin2004}. Its Lemma~7, (23),
p.~161 is a polynomial integrality statement, prior to (24), p.~162,
where an integer base is substituted. The direction and constants are
also those of that source. The cancelled-degree and positive-remainder
arguments needed here are proved in Section~\ref{long1049:sec:region};
the reciprocal-interval summation follows
\cite[Lemma~1, p.~466]{zudilin2002}.
The supplied public snapshot contains the source-applied declarations
\texttt{PaperR17.rational\_base\_region} and
\texttt{PaperR17.thirtyone\_four\_powers}, rather than just conditional
interfaces or finite height comparisons. The formal-source appendix
specifies their evidence boundary. The cited Bundschuh--V\"a\"an\"anen
analytic criterion at $7/2$ and the functional nonrationality and Mahler
theorems remain external inputs. At $3/2$, an actual noncollapsed family
with sufficiently small nonzero divided remainder is still missing.
...
asymptotic adequacy and states the remaining obligations.  Linked phrases identify formal-source declarations. The source map records
the supplied snapshot and the historical direct-link revisions separately;
a link is not itself evidence of a fresh Lean build.
...
\]
from Zudilin \cite[Sec.~5, pp.~161--162]{zudilin2004}, and put
\[
...
The constants are defined by these exact expressions, rather than by their
decimal displays. The rational certificate accompanying this revision gives
\[
\begin{aligned}
...
\end{aligned}
\]
Put $\mu=1/\theta^*$. The direction, intervals and constants, including
...
$c=(13,14,12,14,15,13)$ of the fixed direction; it gives the exponents
...
The proof occupies Sections~\ref{long1049:sec:source-forms}--\ref{long1049:sec:integer-forms}.  Its
integrality input is the polynomial conclusion of Zudilin's Lemma~7 together
with the coefficientwise argument that proves it \cite[pp.~156--161]{zudilin2004},
and it uses his direction and constants.  The cyclotomic limits of
Section~\ref{long1049:sec:cyclotomic-limit} are his Lemmas~1--2, proved there by the
method of \cite[Lemma~1, p.~466]{zudilin2002} for the degree count.  The
rational specialisation and the accompanying degree and positive-remainder
arguments are supplied here. This describes the division of work in the
proof; it is not an exhaustive priority claim about rational specialisations.

\subsection{The source forms as polynomial
identities}\label{long1049:sec:source-forms}

The strict threshold is essential to this proof: at
$\log b/\log a=\theta^*$ the leading quadratic balance is zero, and the
error estimates stated here do not decide its sign.
...
denote the denominator of the base, and $M_n$ is the integer of that
paper's~(16) on p.~157.  With
...
\end{equation}
whose last denominator has the length $\beta_n-a_2$ given by the gamma
expression~(7) and the residues~(8) of \cite[p.~156]{zudilin2004},
and \cite[(8)--(11), pp.~156--157]{zudilin2004} gives $H_n=A_nF-B_n$ with
...
supplies the polynomial form.  Its Lemma~4 (pp.~157--159) clears the two
rational coefficients separately by Gaussian-binomial identities in $\Z[X]$,
its Lemma~5 (p.~160) is the stability of the normalised series under the
order-twelve group generated by the parameter map that Heine's transform~(1)
induces and the exchange of $a_1$ and $a_2$, and Lemma~7 (p.~161) removes
the cyclotomic factors by polynomial divisibility, $\Omega_n$ being monic.  All
three precede the source's evaluation at an integer $p$ in its~(24) on p.~162,
and all three remain valid with an
...
Four arithmetic checks in this step are represented by the supplied Lean declarations:
...
The degree calculation fixes $n$ and varies $x$; the decay calculation fixes
$x$ and varies $n$. Polynomial integrality was established before the
first calculation, so an analytic bound at infinity legitimately bounds
the polynomial degree rather than merely a rational-function growth rate.
...
For the Archimedean side, fix a real $x>1$. Put
$B(x)=\sum_{d\ge1}-\log(1-x^{-d})/d<\infty$.
The M\"obius product formula and a divisor reindexing give
\[
...
is the rational-specialisation denominator-clearing step. The inherited
polynomial integrality and the preceding cyclotomic cancellation are
...
For $31/4$, the exact rational interval calculation accompanying this
revision gives
\[
\begin{gathered}
...
The script \texttt{computations/intervals.py} records the rational endpoints;
this enclosure is exact computer arithmetic, not a new Lean theorem.
\end{proof}
...
Theorem~\ref{long1049:res:region}.  The sharper interval in the preceding proof is supplied by a separate
exact-arithmetic certificate with $256$ summands per trigamma difference.
The old thousand-term decimal evaluation is not used as a proof or as a
freshly reproduced receipt. The rational-bracket Lean statements and this
new interval certificate have different verification histories.
...
proves them by that method with an explicit truncation estimate.  The supplied public source also contains the source-applied irrationality
and measure declarations, in addition to the checked finite height
comparisons, the rational bracket of
Section~\ref{long1049:sec:regionbracket}, and the degree identities of
Section~\ref{long1049:sec:degrees}. The formal-source appendix distinguishes
these endpoints from exact computer algebra and numerical evaluation. The supplied earlier record reports exact reconstructions of $U_n,V_n$
for $n\le4$, described in Section~\ref{long1049:sec:receipts}; that
reconstruction was not rerun for this revision. Fresh exact computations
here are the interval enclosure above and the finite coefficient-moment
and cyclotomic-residue certificates in
Section~\ref{long1049:sec:coefficient-questions}. These receipts do not
constitute a new Lean build.
...
with the source supply applied rather than assumed.
...
$29/4$.  The source-applied irrationality and measure declarations are present in
\texttt{PaperR17/SourceConsumers.lean} in the supplied public snapshot.
The formalisation is therefore not limited to integer degree bookkeeping.
This does not settle the missing approximation family at $3/2$.
...
\begin{proof}
The only point not immediate from homogenisation is the cap
$\sigma\le\delta$.  We apply the forms at arbitrarily large integer bases
$p$: the upper bounds for their values and the two-sided remainder asymptotic
give an upper bound for the irrationality exponent of $F(p)$, while
Dirichlet's theorem gives the lower bound $2$.  Letting $p$ grow removes the
...
To prove $\sigma\le\delta$, fix an integer $p\ge2$, write $\xi=F(p)$,
...
\paragraph{The bounded direction search.}
The direction $(14,12,14;27)$ is Zudilin's \cite[p.~162]{zudilin2004}; the
scan below compares it with the other directions in a bounded box.
...
(25) and (26) has largest recorded value at $(14,12,14;27)$ and its group image
...
and the floating-point comparisons were not interval-certified, so this is
numerical evidence over that box rather than an exact optimality theorem.
The enumeration ranges over integer tuples
$(\alpha_0,\alpha_1,\alpha_2;\beta)$ with $1\le\alpha_j\le30$,
$1\le\beta\le30$, satisfying the cone conditions of \cite[Sec.~5, p.~161]{zudilin2004}
and taken up to the common factor of the four entries; the objective is the
ratio $C_0/C_1$ of the source's (25) and (26), evaluated from the exact rational
breakpoints of the step-function weight and trigamma values at thirty decimal
digits.  The two leading tuples are exchanged by the source's permutation
group, so they are one direction.

The rank-two condition should be distinguished from simultaneous
approximation. Postelmans and Van Assche prove the $\Q$-linear independence
...
That theorem neither satisfies a universal rank-two template nor removes
the integer inverse-base restriction. It is a comparison of constructions,
not an application at $3/2$.

\subsection{Finite receipts}\label{long1049:sec:receipts}

Four finite computations support statements above. This paragraph records the earlier supplied
receipts; those historical runs were not repeated in the present revision.
The new rank-eight and interval certificates are listed separately below.  The reconstruction and
degree calculations use exact rational or integer arithmetic; the constant
evaluation and direction scan use the numerical precision stated below.  None
...
\href{https://github.com/wcook04/plectis-erdos/blob/39a0a2078c8f9fbe7fc24f912654815cc24685df/research/experiments/erdos1049/receipts/direction-search-bound30.json}{tracked receipt}.
The receipt records mpmath~1.3.0 at thirty decimal digits.  Its exact enumeration
...
precision \cite[p.~162]{zudilin2004}. The new rational interval certificate,
not these reported digits, supports the numerical inequality in this revision.
...
The supplied earlier record reports that the order and leading coefficient
of $V_N^{*}$ were computed exactly for
$1\le N\le7$, with the following reported results:
\[
...
threshold below, while the lower inequality shows that $65$ is the least
exponent $q$ for which $3^{41}<2^{q}$.  Exact failure of the rank-$41$
selector count one row earlier uses the additional integer comparison
$2^{129}<3^{82}$ below.
...
The charge comparison of Corollary~\ref{long1049:res:sharpgaps} concerns the stated
denominator model.  The next theorem determines a different quantity, the
formal $q$-order of Zudilin's normalised Hankel determinant at $x=z=1$;
converting a $q$-order into a denominator saving at a rational specialisation
needs a separate integral normalisation and divisibility argument.  Zudilin
...
on $|V_N^{*}|$ \cite[Sec.~4, p.~7]{zudilin2016}.  Equality rules out any further
initial power of $q$ in this formal determinant at $x=z=1$.  The size at a
fixed real $q$ is a separate statement; the positive-measure estimate below
proves that $V_N^{*}(q)>0$ and that $\log V_N^{*}(q)$ differs from the
logarithm of its leading term by $O_q(N)$.
...
monotone convergence of the nonnegative tuple sums gives Heine's expansion of
a Hankel determinant of moments, whose integral form is reproved in
...
The fixed-$q$ constants give an error linear in $N$ after taking logarithms;
they are not uniform as $q\uparrow1$. The measure depends on $q$ but not on
the moment index $m$ or the Hankel rank $N$.
  This is a separate analytic proof; it supplies no
...
\paragraph{Evidence decomposition.}
The positive-measure proof above is ordinary mathematics and is not a Lean
claim. Separately, the formal-order proof has the supplied Lean check of
\lword{Erdos1049/AdelicHeightBridge.lean}{183}{hankelAssociatedCoeff_eq_reciprocal}{its all-depth associated-graded recurrence}
and source-applied all-rank conclusions for the formal order and initial
coefficient. These are not conclusions about the real measure.  The order and the
leading coefficient of $V_N^{*}$ are
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1049/AllRow/Producer.lean#L163}{the sharp Hankel order and coefficient},
and the two closed forms $6\operatorname{ord}=N(N-1)(2N-1)$ and
$2^{N}\mathrm{lc}=(N!)^{2}(N+1)!$ are
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1049/AllRow/Producer.lean#L173}{the closed order} and
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1049/AllRow/Producer.lean#L185}{the closed leading coefficient}.
The Lean proof composes the initial monomial of every transformed row, which
...
Lemma~\ref{long1049:res:allrowinitial} and~\eqref{long1049:eq:row-initial} give the initial monomial of every row, and Lean
states that identity for every row as
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos1049/AllRow/Producer.lean#L143}{the all-row initial monomial}.  Nothing in
...
\[
 ...
 \quad(p\downarrow1).
\]
The cubed factorial is therefore not an inessential positive factor:
entrywise multiplication by a positive sequence need not repair a failed
moment condition. At $p=1$, $s_m(1)=(m!)^3$ is the moment sequence of a
...
\end{array}
\]
The complete coefficient lists, polynomial definitions and an exact
reproduction script accompany the revision. This is finite symbolic
verification, not sampling of bases and not a Lean theorem. It is also
not a test of every Hankel minor or of arbitrary rank.

The degrees admit a separate all-rank proof. The Gaussian binomial
${u\brack v}_p$ is monic of degree $v(u-v)$. In the signed sum for $R_m$,
...
The classical Stieltjes criterion requires positive semidefiniteness of
both Hankel families at all ranks \cite[Lemma~2.1, p.~4]{wangzhu2016}.
...
Write $A_N=(\alpha_{i+j})$ and $B_N=(\beta_{i+j})$.
Let $c=p(p-1)^3$ and $D=\operatorname{diag}(1,c,\ldots,c^{N-1})$.
...
$D_{k,0}(p)$ for $1\le k\le N$; their certified positivity and Sylvester's
criterion give positive definiteness. The diagonal congruence gives
...
gives non-strict interlacing.
\end{proof}
The proposition uses the finite certificates, not an all-rank coefficient
measure. Such a measure of infinite support would extend the same proof
to all ranks. The largest root is a nondecreasing lower bound over the
certified ranks, but convergence to $F(p)$, simple roots, strict
interlacing and denominator control do not follow. At $p=1$ the
congruence degenerates and $F$ diverges, so the proposition excludes that
endpoint. This pencil is not automatically the Jacobi matrix of the
coefficient moments.

\paragraph{A more concrete coefficient-moment question.}
Write $D_n=D_{n,0}$, $E_n=D_{n,1}$, with $D_0=E_0=1$.
The classical formal Stieltjes continued fraction has determinant ratios
\[
 \lambda_{2n-1}=\frac{E_nD_{n-1}}{D_nE_{n-1}},\qquad
 \lambda_{2n}=\frac{D_{n+1}E_{n-1}}{E_nD_n}\quad(n\ge1).
\]
The classical correspondence is recalled in \cite[pp.~1--2]{sw2024}.
Our certificates imply $\lambda_1,\ldots,\lambda_{15}>0$ for every
$p\ge1$. Seeking a positive formula or production mechanism for these
ratios at every index is more specific than asking whether a measure
might exist. The generating function in this correspondence is formal:
at $p=1$ its coefficients $(m!)^3$ already give radius of convergence zero.

The same two positive $8\times8$ moment matrices give a finite positive
quadrature realisation for $s_0,\ldots,s_{15}$, by the classical
orthogonal-polynomial/Jacobi construction; the computation of nodes and
weights from that matrix is described by Golub and Welsch
\cite[Sec.~2, (2.6); Sec.~4]{golubwelsch1969}.
This is a truncated moment statement, not a measure representing all
$s_m$. The Jacobi construction here must not be identified with the
different coefficient pencil $(A_N,B_N)$.
...
\paragraph{What would count as extra cyclotomic divisibility.}
...
\]
with $v_{\Phi_d}(0)=\infty$. The valuation of a polynomial in $Y$ means
the minimum valuation of its coefficients. Each determinant term is
...
A useful determinant form of this test is available whenever integers
$u_i,v_j$ satisfy $u_i+v_j\le t_{i+j,d}$ and
$\sum_i u_i+\sum_jv_j=e_{N,d}$. Divide row $i$ by $\Phi_d^{u_i}$ and
column $j$ by $\Phi_d^{v_j}$ in the local fraction field, then reduce
modulo $\Phi_d$. Only entries with $u_i+v_j=t_{i+j,d}$ survive.
The determinant of this reduced matrix is $\Gamma_{N,d}$, since every
non-minimal permutation vanishes on reduction. Thus a root-of-unity
operator annihilating the surviving matrix would explain excess
valuation; its hypotheses must be checked for this specific pencil.
This assertion is conditional on the displayed potentials and does not
assume that the matrix has rank one.


The monic contents in $\Q[p]$, computed exactly, are
\[
\begin{array}{c|l}
...
\end{array}
\]
They attain the assignment bound for every $d$ at these five ranks:
$d\le8$ was tested directly, and the full displayed factorisations
exclude other cyclotomic factors. The power of $p$ is not cyclotomic
content. Since $\deg_Y\mathcal H_N\le N$, its coefficient gcd over
$\Q[p]$ equals the gcd of its values at $Y=0,\ldots,N$, by invertibility
of the rational Vandermonde matrix. This is the computation used here;
it does not certify specialised integer contents or all-rank behaviour.

A separate minimum-cost subset recurrence now gives nonzero residue
witnesses for every pair
\[
 1\le N\le8,\qquad 1\le d\le\max(8,2N-2).
\]
There are $76$ such pairs. For each, the supplied record gives an integer
$Y_0\in\{0,\ldots,N\}$ and a nonzero residue
$\Gamma_{N,d}(Y_0)\bmod\Phi_d$, which certifies equality with the
assignment bound. At ranks six to eight this is a finite cyclotomic-index
window, not a complete content factorisation. The complete contents above
remain certified only through rank five.
...
moving-diagonal pencil. The finite witnesses show no excess in their
stated range, not the impossibility of excess at larger ranks or other
cyclotomic indices. Whether minimum-valuation residues ever cancel
...
explicit rational approximation: one constructs nonzero linear forms in
$1$ and $F(\beta)$ with integer coefficients whose absolute values tend to
zero. Bounds on coefficient height help construct those forms or bound an
...
Zudilin's treatment of \(q\)-harmonic series~\cite[Secs.~3--5]{zudilin2004} builds linear
...
integrality and quantifies over arbitrary elements of \(\Z[X]\), and the tools
are elementary congruences, the pigeonhole principle and B\'ezout's identity.
The exception is
...
cyclotomic divisor of Zudilin~\cite[Sec.~4, pp.~159--161]{zudilin2004}, produce their gain as
...
\href{https://github.com/wcook04/plectis-erdos/blob/7148c5ae4a4a2f3802cccdeed62c394f64593340/lean/ErdosProblems/Erdos1049/BezoutPluckerJets.lean\#L191}{adjacent-to-pairwise determinant propagation},
the resulting
\href{https://github.com/wcook04/plectis-erdos/blob/7148c5ae4a4a2f3802cccdeed62c394f64593340/lean/ErdosProblems/Erdos1049/BezoutPluckerJets.lean\#L230}{modular selector collision},
and the
...
The rowwise-coprime statement proved above is the stronger one.
...
source-applied declarations. The scalar statement in this paragraph is
...
In the model, the common denominator exponent is $E_n=(3n^{2}-n)/2$.  Since only doubled exponents
...
The universal assertion of Problem~\#1049 is not settled here. Theorem~\ref{long1049:res:jetkernel} suggests the following precise sufficient
...
\paragraph{The specialised lattice is a separate calculation.}
...
$\Z[p]$. It measures the actual image, rather than the ambient four-jet
target. If two independent divided rows have coordinate height at most
...
\subsection{Determinant families: definition before conjecture}
The earlier exploratory record mentions a contiguous $a_0$-shift matrix
at rank $13n+2$, but does not give its entries, its width or its relation
to the number of coefficient pairs. Consequently the assertion
$\det A_{n,13n+2}\ne0$ is not yet a well-defined public conjecture.
Before further computation, a family specification must fix the
entrywise formula, allowed shifts, normalisation, width and remainder
identity. Only then is it meaningful to test the first nonzero minor,
row contents and jets, or to ask whether adding one $a_1$-shift increases
rank. The earlier proposed questions are retained in the archival edit
record, not presented as if their missing matrix were already defined.
...
height--remainder architecture, not a necessary irrationality criterion.
First fix one such architecture and define its height $H_n>0$, undivided
...
nullspace.  An earlier exploratory adjacent-exterior calculation reports a positive
normalised exponent of about $110.850\,n^2$. It is not a checked theorem
or a reproduced certificate of this revision, and it supplies no rigorous
numerical target until that architecture and estimate are established.
A proposed divisor saving must be compared with the height and remainder
of the same explicitly normalised objects.

\subsection{One exact alternative-criterion test}

A separate possible method is Mahler's method, and its first applicability test
has a negative answer.  Bell and Smertnig's classification shows that the
divisor generating series $\mathcal L$ below is not $k$-Mahler for any $k\ge2$
\cite[Thm.~1.3 and the consequences on p.~3]{bellsmertnig2026}.  The
proposition below proves the simultaneous $2$/$3$ case directly from theorems
of Adamczewski and Bell and of Rivin.
...
For this scalar function the single-base classification of Bell and Smertnig
contains the proposition; the proof above isolates simultaneous closure.  A
...
an obstruction to every approximation method.  The classification and the two
external ingredients in the proposition are cited, not proved here.
...
\]
A useful result would be an explicit admissible construction with
$\theta_*>\theta^*$, an exact value of $\theta_*$, or a converse bound
$\theta_*<\log2/\log3$. Another possibility is a class theorem reducing
all members to one of the specified scalar or rectangular mechanisms.
Here $\theta^*$ is the exactly defined cutoff of
Theorem~\ref{long1049:res:region}; no new rounded threshold is intended.
\end{problem}
...
The public source snapshot used for this editorial audit is
\texttt{3d6d938d696fed0fb71dd55115a18a73738ff223}; the parallel release
snapshot is \texttt{52f29ad173b04e3bac941b3663f2b9aebe5de0bb}.
...
The main source-applied irrationality and measure endpoints are in
\texttt{Erdos1049/PaperR17/SourceConsumers.lean}. The file constructs
\texttt{actual\_cancelled\_supply} before applying it in
\texttt{rational\_base\_region}, \texttt{thirtyone\_four\_powers} and
\texttt{rational\_base\_power\_measure}.
The all-rank formal-order endpoints are in
\texttt{Erdos1049/AllRow/Producer.lean}; the producer proves the actual
source-row initial monomials and applies the determinant consumer.
In particular these are not merely interfaces assuming the missing rows.

Other modules check corridor and recurrence identities, degree
bookkeeping, endpoint congruences, finite collision principles and
specified exponent models. The stronger ordinary unimodular-row
B\'ezout--Pl\"ucker statement must be distinguished from the checked
unit-coordinate version. The finite coefficient-moment and polynomial
content calculations in Section~\ref{long1049:sec:coefficient-questions}
are exact computer algebra, not Lean declarations.

The accompanying audit reports the packet's CI evidence and a static
inspection of the supplied sources. It is not a fresh kernel run or an
axiom audit. Canonical links should be checked by declaration name as
well as line number when this text is integrated; historical linked
revisions are retained where no same-declaration mapping was established.
...
\paragraph{Four-jet collision family.}
