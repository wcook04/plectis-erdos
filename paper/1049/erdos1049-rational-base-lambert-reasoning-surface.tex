...
\[
 ...
 \qquad [q^{\operatorname{ord}_q V_N^*}]V_N^*
       =\frac{(N!)^2(N+1)!}{2^N}.
\]
For each fixed real $0<q<1$ a separate positive-measure argument proves
positivity at every rank and, carried further, an asymptotic formula as
the rank grows, with an explicit constant.

Two further kinds of result are finite. Lean checks a coefficient
pencil through rank eight only: positive definiteness, real roots below
$F(p)$ and non-strict interlacing. Exact computations outside Lean treat
shifted coefficient matrices and common factors at fixed ranks. The
final sections explain the limits of the stated clearing and congruence
arguments at $3/2$. No small nonzero sequence of divided remainders is
constructed at that base, and the full rational-base conjecture remains
open.
\end{abstract}
...
$4^{200}<31^{81}$, which Lean
\lword{Erdos1049/ZudilinHeightRegion.lean}{26}{thirtyoneFour_power_certificate}{checks},
and Lean also checks that it places
\lword{Erdos1049/ZudilinHeightRegion.lean}{45}{thirtyoneFour_mem_zudilinHeightRegion}{$31/4$ in the region $\log b/\log a<81/200$}.
The ratio $\log b/\log a$ is invariant under $(a,b)\mapsto(a^{r},b^{r})$,
which places
\lword{Erdos1049/ZudilinHeightRegion.lean}{59}{thirtyoneFour_power_mem_zudilinHeightRegion}{every
power of $31/4$} in the same region, and $(31^{r},4^{r})$ are coprime.
Lean also checks that
...
displayed decimal approximation. Lean checks the coarser calculation,
including $2981509/9909<301$, and proves enclosures of $\theta^*$ and
$\rho$ sharper than the finer ones displayed above
(Section~\ref{long1049:sec:regionbracket} and
Theorem~\ref{long1049:res:31over4}); the $256$-summand calculation is an
...
Lean checks the definitions of $C_0$, $C_1$ and $\theta^*$ together
with the
...
These comparisons use the defined constant; the analytic estimates belong
to the proof of Theorem~\ref{long1049:res:region}, which Lean also checks.
Lean also proves
$0.40568302138406054100\le\theta^*\le0.40568302138406054104$ and
enclosures of $J$ and $C_0$ (\href{\evidencerecordurl}{evidence record},
Theorem~\ref{long1049:res:region}). The sharper
interval in the preceding proof comes from the stated
...
certify all digits printed in the cutoff. Lean's enclosure of $\theta^*$
implies the following bounds, and a separate integer-arithmetic
calculation reproduces them independently of Lean:
\[
\begin{aligned}
...
\end{aligned}
\]
Here is the complete error bound used in that calculation. For each of the thirteen
...
claimed order. This verifies the displayed decimal truncations; \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/threshold_digits.py}{the program} and
its \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/threshold-digits.json}{recorded result} are public. The proofs use the exact definitions and never these decimal values.

\subsection{Proofs, earlier work and limitations}

\paragraph{Formal verification and computations.}
The printed proofs of Theorems~\ref{long1049:res:region}
and~\ref{long1049:res:31over4} use Zudilin's construction: his Lemma~7 in its polynomial reading, together with the
...
proves them by that method with an explicit truncation estimate.  Lean
proves both theorems and the measure bound of
Corollary~\ref{long1049:cor:rational-base-measure}; it constructs the
polynomials and proves their remainder estimates.  The Lean sources also
contain the finite comparisons, the rational bracket in
...
Exact computations outside Lean certify the rational interval
bounds, $U_n,V_n$ for $1\le n\le4$, the sixteen coefficient-positivity
tests through rank eight, and the finite parameter search. The five complete
...
A calculation with rational bounds for logarithms and for the cutoff
checks all $1101$ reduced fractions in the stated range, these rounded
margins and the four fixed-denominator comparisons below
(\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/rational_base_examples.py}{program}, \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/rational-base-examples.json}{result}). The
...
A calculation compares every tuple of the box with interval bounds,
and a second implementation repeats it with $512$ summands before the
tail and a different enumeration order; both are in
\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/parameter_box.py}{one program}, with its \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/parameter-box.json}{recorded result}.
The calculation proves optimality only in the displayed finite box. No
...
numerical reports. The polynomial reconstruction below covers
$n=1,2,3,4$, and the preceding parameter search has an exact
comparison for every tuple in its finite box. Both exact checks are
public, with the other finite calculations of this paper, in the
\href{https://github.com/wcook04/plectis-erdos/blob/f5c9e560df38cd32e9b49473a409384aab00d9ae/research/experiments/erdos1049/README.md}{reproduction guide}, and a
\href{https://github.com/wcook04/plectis-erdos/blob/f5c9e560df38cd32e9b49473a409384aab00d9ae/research/experiments/erdos1049/replay.py}{single script}
reruns them and compares each with its recorded result. The historical
...
The \href{https://github.com/wcook04/plectis-erdos/blob/f5c9e560df38cd32e9b49473a409384aab00d9ae/research/experiments/erdos1049/README.md}{public reproduction guide} gives the command and dependency pin for
...
An exact integer calculation (\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/source_polynomials.py}{program}, \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/source-polynomials.json}{every coefficient}) reconstructs the forms of
Section~\ref{long1049:sec:source-forms}
directly in $\Z[X]$ from the displayed source formulas. It checks
division by $\Omega_n$ and $X^{M_n}$ with zero remainder and computes
all coefficients of $U_n,V_n$. The exact degrees are
\[
\begin{array}{c|rrrr}
 n&1&2&3&4\\\hline
...
for every $1\le n\le400$, as verified by exact rational logarithm
enclosures and exact totient sums (\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/main_term_31_4.py}{program}, \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/main-term-31-4.json}{result}). Dividing by $n^2$ gives, to three
...
An application of these degree bounds must separately
...
and is not formalised. The formal-series argument has a checked
\lword{Erdos1049/AdelicHeightBridge.lean}{200}{hankelAssociatedCoeff_eq_reciprocal}{recurrence for the leading coefficients}.
The Lean proof of Theorem~\ref{long1049:res:zudilin-sharp-qorder} uses
the first nonzero term of every transformed row, which
for row $1$ has order exactly $l+1$ and coefficient exactly $-6$ in every
column.  Independently of the proof, the order and the
leading coefficient were computed exactly for $1\le N\le7$
(\href{https://github.com/wcook04/plectis-erdos/blob/f5c9e560df38cd32e9b49473a409384aab00d9ae/research/experiments/erdos1049/hankel_qorder.py}{program}, \href{https://github.com/wcook04/plectis-erdos/blob/f5c9e560df38cd32e9b49473a409384aab00d9ae/research/experiments/erdos1049/receipts/hankel-qorder.json}{result}), giving orders
...
give that row calculation.  Nothing in
...
in the sixteen polynomials are strictly positive. A
\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/computations/finite_coefficients_generate.py}{program}
computes the
\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/computations/finite_coefficients.json}{full lists}.
An
\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/computations/check_finite_coefficients.py}{independent checker}
...
\end{proposition}
\evidenceremark{The Lean proof checks all three conclusions for every
real $p>1$ and every rank $N\le8$; it states nothing beyond rank eight.}
\begin{proof}
...
An exact calculation outside Lean gives the following monic contents.
...
For the table, an exact
\href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/cyclotomic_content.py}{program}
computes the full polynomials
...
The source coefficients, determinant-value polynomials and witnesses
are computed by that program, which also repeats the direct
determinant checks; its \href{https://github.com/wcook04/plectis-erdos/blob/4beca4ad0c00a626def9c73f2420e20ebf8d1968/research/experiments/erdos1049/receipts/cyclotomic-content.json}{recorded result} lists every witness. At ranks six to eight
...
\noindent Scalar multiplication can introduce powers of $2$ and $3$,
...
\]
Lean checks both, as the
\lword{Erdos1049/ZudilinConeArithmetic.lean}{191}{bottomJet3_eq_zero_iff_dvd}{criterion for divisibility by $3^R$} and the
...
\noindent This conditional theorem is stronger than the ambient count of possible residue vectors only
...
\noindent Lean checks the
\lword{Erdos1049/ZudilinConeArithmetic.lean}{131}{fourJetSignature_card}{cardinality of the residue space}, the
\lword{Erdos1049/ZudilinConeArithmetic.lean}{142}{exists_distinct_binary_selectors_same_fourJet}{pigeonhole argument for the residue map} and the
...
Lean checks the
\lword{Erdos1049/ZudilinConeArithmetic.lean}{428}{three_two_scalar_margin_neg}{inequality}
and the
...
$4^{200}<31^{81}$; the last comes from $3^{81}<2^{200}$.  Lean checks the
\lword{Erdos1049/ZudilinHeightRegion.lean}{65}{twoFifths_lt_thirtyoneFour_log_ratio}{lower
bound}.  Consequently $31/4$
\lword{Erdos1049/ZudilinHeightRegion.lean}{45}{thirtyoneFour_mem_zudilinHeightRegion}{lies in the enlarged region},
and so does
\lword{Erdos1049/ZudilinHeightRegion.lean}{59}{thirtyoneFour_power_mem_zudilinHeightRegion}{every positive power $(31/4)^r$}.
The same base
\lword{Erdos1049/ZudilinHeightRegion.lean}{91}{thirtyoneFour_outside_bundschuhVaananenHeightRegion}{lies strictly outside}
the earlier Bundschuh--V\"a\"an\"anen region, so the $81/200$ inequality defines a strict set-theoretic enlargement of the
...
check. The mathematical proofs are given in the text; ordinary arguments
and finite computations are identified separately.
...
\href{\evidencerecordurl}{evidence record} lists every declaration and
check. The formal results are of two kinds, and both are distinct from
the finite computations.

\paragraph{Statements for every rank or every base.}
Lean constructs the cancelled polynomial forms of
Section~\ref{long1049:sec:region} and proves irrationality in the
stated region, irrationality at every positive integral power of $31/4$
and the irrationality-exponent bound. It proves the first nonzero term
of every transformed row of the 2016 determinant, deduces the order and
leading coefficient of the determinant at every rank, and proves the
fixed-base asymptotic of Section~\ref{long1049:sec:sharp-fixed-base}.
Both constructions therefore supply their mathematical objects; neither
is assumed to exist. Proposition~\ref{long1049:res:nomahler} is checked
...
Bell~\cite[Thm.~1.1, p.~6]{adamczewskibell2013} cited in the printed
proof, as explained after that proof.

\paragraph{Statements through rank eight.}
Proposition~\ref{long1049:res:finite-pencil} is checked in Lean for
every real $p>1$ and every rank $N\le8$. The Lean proof includes the
identity $F(p)A_N-B_N=(v^*_{i+j})$, the positivity of the eight unshifted
determinants $D_{k,0}$ by the certificate described in
Section~\ref{long1049:sec:coefficient-questions}, the reality of the
pencil roots, their position below $F(p)$ and their interlacing. It has
no all-rank coefficient-positivity or root-convergence conclusion.

\paragraph{Computations outside Lean.}
The shifted determinants $D_{k,1}$ and the strict positivity of every
coefficient rest on the sixteen-polynomial computation of
Section~\ref{long1049:sec:coefficient-questions}. The reconstruction of
$U_n,V_n$ for $n\le4$ in Section~\ref{long1049:sec:receipts}, the five
complete polynomial contents and the seventy-six cyclotomic residue
witnesses have exact reproduction scripts and direct determinant checks.
None of these computations is a Lean declaration.
...
formal proofs used here are those described above.
