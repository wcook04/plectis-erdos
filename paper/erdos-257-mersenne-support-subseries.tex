...
open.  The leading support-family theorem covers every infinite
...
reciprocal mass.  At base \(2\) this is an independent checked proof of the
no-coprimality conclusion stated without proof by Erd\H{o}s; the all-base
strengthening and its transfer argument are proved locally.  No identification
...
Theorem~\ref{res:terminalhalf} proves that terminal scaled vanishing---finite
...
Erd\H{o}s proves the pairwise-coprime reciprocal-summable case at base \(2\)
and then says that pairwise coprimality can be removed by a more complicated
argument~\cite[p.~222]{erdos1968}; the argument is not printed there, and the
boundary is repeated on p.~226.  The base-\(2\) instance of
Theorem~\ref{res:reciprocal-support} is an independent Lean-checked
reconstruction of that stated conclusion.  The uniform binary-to-radix
transfer proves the stronger all-base statement locally.  We do not identify
this proof with Erd\H{o}s's omitted proof or make a theorem-priority claim.
...
The first row records the independently reconstructed, Lean-checked base-$2$
proof of that stated conclusion together with the checked uniform transfer to
every integer base.  The sparse-support row separately records Erd\H{o}s's
printed pairwise-coprime theorem and its all-base formalisation.  These are not
conflated: the historical no-coprimality statement is base~$2$, while the
all-base strengthening is a local theorem and carries no priority claim.
