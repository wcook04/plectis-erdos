...
\hypersetup{
  pdftitle={Exact denominator periods for finite Lambert subseries (Erdős Problem 257)},
...
\runninghead{Exact denominator periods for finite Lambert subseries}
\title{Exact Denominator Periods for Finite Lambert Subseries}
\subtitle{Erd\H{o}s Problem~\#257}
\author{Will Cook}
\date{July 2026}
...
are not known to be contradictory.  We also record the achievement-set
geometry and the exact status of the candidate $1/2$: membership would refute
\#257, while non-membership would eliminate only this one candidate.
\end{abstract}
...
$F_s(E)$ and the monomial images
  ...
  $L=\lcm(1,\dots,\ell)$ &
  Duverney--Tachiya~\cite{duverneytachiya}, Cor.~1.2;
...
\noindent For scope, the squarefree value at base~$2$ is settled
by~\cite{duverneytachiya} (Corollary~\ref{res:sfsettled}).  Their cited
...
\bibitem{erdos1968} P.~Erd\H{o}s, \emph{On the irrationality of certain
  series}, Math.\ Student 36 (1968), 222--226.  The theorem on p.~222 treats
  pairwise-coprime support with convergent reciprocal sum at every integer
  base $b\ge2$.
