...
\(A\subseteq\Npos\).  We prove irrationality for several classes that
allow divergent reciprocal mass: supports satisfying a divisibility-weighted
summability criterion, supports whose reciprocal mass grows more slowly than
an iterated logarithm, and supports admitting summable positive fractional
divisor covers.  Each criterion is hereditary under passage to infinite
subsets and works at every integer base \(b\ge2\).  A separate argument,
using Tao--Ter\"av\"ainen's correlation theorem, treats every infinite
subset of the prime powers, including fixed dilations and finite modifications.

Every infinite reciprocal-summable support is already covered without
a coprimality assumption: binary LCM--Ces\`aro close returns transfer to
every integer radix, where an exact integer-orbit gap excludes rationality.
...
satisfies \(\operatorname{ord}_{D_F}(b)=\operatorname{lcm}(F)\).
...
Section~\ref{sec:eight-return-extensions}.  The accompanying Lean development
checks arbitrary reciprocal-summable support at every integer base, the
classical full-support and periodic-weight theorems, the denominator identity, the geometric
...
\textbf{Infinite supports.} Every infinite reciprocal-summable support has
irrational value at every integer base, without coprimality. Divisibility-weighted summability, sub-log-star
...
argument.  These results include supports with divergent reciprocal mass.
...
rationality.  The theorem leaves precisely the reciprocal-divergent supports
in the universal question.  Erd\H{o}s proved the pairwise-coprime special
case and stated without details that coprimality could be removed; the checked
argument here is not identified with that omitted proof, and its novelty and
priority have not been assessed.
...
The \href{https://github.com/wcook04/plectis-erdos/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/GreedyRepairCriterion.lean#L192}{uniform repair equivalence} is now kernel checked for every real target $x\ge0$.
...
denominator.  Theorem~\ref{res:period} answers the second exactly and,
...
scaled residual is at least $2^N\delta$ thereafter.  This is the hard step in
the reverse bounded-return implication: an orbit with even one bounded
cofinal subsequence cannot lie outside $\Ach$.
