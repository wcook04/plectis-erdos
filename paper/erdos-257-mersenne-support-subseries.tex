...
\renewcommand{\commit}{ec787e648a672e2a501b7e5453230a5e6d16d766}
\renewcommand{\commitshort}{ec787e648a67}
\hypersetup{
...
counts and need not be binary digits.

The strongest direct consequence in the checked spine is the classical
full-support theorem: for every integer base $b\ge2$,
\[
  \sum_{k\ge0}\frac1{b^{k+1}-1}
\]
is irrational.  This is the case $A=\Npos$, attributed to Erd\H{o}s~\cite{erdos1948}
and formalised here at
\mword{Erdos249257/CertificateKernel.lean}{8328}{irrational_erdosSum_full_support}{full-support irrationality theorem}.
Its hard step is a completed weighted certificate machine: a bounded
Bertrand--CRT first block, middle-window divisor-pair averaging with
pigeonhole selection, weighted tail control, and explicit parameter closure.
It is a complete family theorem, not the universal assertion for arbitrary
infinite supports.

A distinct structured-support theorem says that, for every integer base
$b\ge2$, every infinite pairwise-coprime support $A$ with
$\sum_{a\in A}1/a<\infty$ has irrational support series.  More generally,
the checked nonnegative rational coefficient theorem says that an
eventually-periodic coefficient sequence, positive somewhere in its periodic
region, gives an irrational
\(\sum_{a\ge0}\gamma(a)/(b^a-1)\).  The pairwise-coprime mechanism is an
adaptive CRT certificate: small support elements are paid for in the first
block, while summable reciprocal mass controls the unselected strays.  The
periodic mechanism is a divisor-orbit sieve followed by finite rational-prefix
transfer.  The exact checked routes are
...
and
\mword{Erdos249257/CertificateKernel.lean}{12811}{irrational_ratWeightSeries_eventuallyPeriodic}{eventually-periodic coefficient theorem}.

If $X_A$ is rational, its integral
...
The endpoint-proximate coordinate change is exact: for $p\in\mathbb Z$ and
$q\ge1$, a support with $0\notin A$, a positive element, and $X_A=p/q$
exists if and only if there is a positive, square-root-bounded integer orbit
whose scaled quotient has Boolean M\"obius transform; that transform
reconstructs $A$.  The checked
\mword{Erdos249257/BooleanMobiusCarry.lean}{949}{exists_normalized_support_fraction_iff_exists_booleanMobiusCarry}{Boolean--M\"obius certificate equivalence}
therefore turns rational-support search into a precise orbit problem.  It is
an equivalence over infinite objects, not a contradiction: finite supports
already supply certificates, and no universal irrationality conclusion
follows.

At the half endpoint, a new exact-row transport makes the conditional route
more concrete.  If the rational greedy remainder after rank $c-1$ is positive
but smaller than $\wgt c$, the checked skipped-core construction fills the
upper half by a Boolean word and produces an exact finite quotient row at
endpoint $2c-2$.  Rows at arbitrarily large endpoints have values converging
to $1/2$; the closed-set consumer therefore gives $1/2\in\Ach$ even when the
finite row witnesses are mutually incompatible.  The needed supply is the
unproved hypothesis that such positive skips occur cofinally.  Thus this is a
conditional membership/refutation route, and finite rows do not provide the
missing cofinal supply.

For finite nonempty $F\subseteq\Npos$ at base $b\ge2$, let $D_F$ be the
reduced denominator of $\sum_{n\in F}(b^n-1)^{-1}$.  The finite theorem is
$\operatorname{ord}_{D_F}(b)=\lcm(F)$.  If $\lcm(F)\ge2$, then
$\lcm(F)<D_F$; the case $b=2$, $F=\{1\}$ shows why that hypothesis is needed.

The squarefree support exposes a sharper boundary.  A cited theorem of
Duverney--Tachiya gives joint $Q$-linear independence of $1$ and
$X_{Asf}(2^j)$ for every finite collection of $jge1$, hence irrationality at
each power-of-two base; this citation is not formalised here.  Yet the exact
incidence formula $scA_{Asf}(n)=2^{\omega(n)}-1$ makes both digitwise and
carry-aware block-certificate hypotheses impossible at every even base,
already at precision $q=b^2$.  Adjoining exponent $1$ shifts the value by the
rational $1/(b-1)$ while changing the incidence from odd to even, showing
that this no-go is coordinate-dependent rather than an obstruction in the
value itself.
...
On the multiplicative side, a composite support element obeys an exact
dilation identity with an explicit foreign-divisor defect.  For an orthogonal
petal bouquet that defect is bounded by the finite exceptional frame plus the
petal incidence, while prime supports have zero defect.  This is a local
interface for a correlation argument, not a correlation theorem.

There is one further conditional countermodel frontier.  A sequence of finite
terminal-only words whose depths tend to infinity and whose scaled terminal
carry tends to zero would converge to the rational value $1/2$ through actual
achievement-set points.  It would therefore produce an infinite rational
support and refute the universal irrationality assertion.  No such sequence
is constructed here.
...
\externalreviewobject{The paper formalises the classical full-support
irrationality theorem for every integer base $b\ge2$, and proves
finite-support denominator-period noncollapse, exact injective coding and a
measure dichotomy for restricted achievement sets.}{These are distinct contribution
...
For release review, the selected full-support Comparator row is exact:
\texttt{irrational\_erdosSum\_full\_support} states that for every integer
base $b\ge2$,
\[
  \operatorname{Irr}\!\left(\sum_{k\ge0}
    \frac{1}{b^{k+1}-1}\right).
\]
This is the full-support case $A=\Npos$ (with the index shifted to $k\ge0$),
not a theorem for every infinite support; the universal Problem~\#257 boundary
therefore remains unchanged.

The selected finite-period Comparator row is exact:
\texttt{finite\_period\_noncollapse\_rat\_den\_interface} states that for
finite nonempty $F$ with $0\notin F$, integer $b\ge2$, and
\(\gcd(b,\operatorname{den}(\operatorname{finiteErdosSum}(F,b)))=1\),
\[
  \operatorname{orderOf}(\operatorname{unitOfCoprime}(b,h_{\rm cop}))
    = \operatorname{lcm}(F).
\]
This is a reduced-denominator/$\operatorname{ZMod}$ order interface for
finite supports; it does not extend the finite theorem to arbitrary infinite
supports or settle Problem~\#257.
...
The new source-current route exposes the mechanism behind one conditional
...
\mword{Erdos249257/BooleanMobiusSkipRowCofinal.lean}{31}{exactLocalMersenneHalfRow_of_positiveHalfGreedySkip}{the local row constructor}.
...
\mword{Erdos249257/BooleanMobiusSkipRowCofinal.lean}{60}{cofinalExactLocalMersenneHalfRows_of_positiveHalfGreedySkips}{the row fan-in}.
The row-value estimate makes the discrepancy at endpoint $n$ at most
$(n+1)/2^n$, and the closed-set consumer then places $1/2$ in $\Ach$,
\mword{Erdos249257/BooleanMobiusCofinalExactRows.lean}{71}{half_mem_mersenneAchievementSet_of_cofinalExactLocalRows}{the closed-set consumer}.
No compatibility between the finite witnesses is assumed.  The global
\mword{Erdos249257/BooleanMobiusSkipRowCofinal.lean}{73}{half_mem_mersenneAchievementSet_of_positiveHalfGreedySkips}{conditional endpoint theorem}
therefore remains conditional: \texttt{CofinalPositiveHalfGreedySkips} is not
proved, and neither one finite row nor finitely many exact rows imply it.
This route gives a conditional $1/2$-membership (and hence conditional
counterexample) criterion, not an unconditional solution of Problem~\#257.

\begin{theorem}[terminal scaled-vanishing countermodel frontier]
...
conditional countermodel route, not a construction of $S$ and not a solution
of Problem~\#257.
\end{theorem}

\noindent The terminal-only consumer first places $1/2$ in the achievement
...
\mword{Erdos249257/TerminalOnlyScaledVanishing.lean}{165}{half_mem_mersenneAchievementSet_of_terminalScaledVanishing}{achievement-set consumer},
...
\mword{Erdos257/HalfCounterexampleFrontier.lean}{31}{exists_rational_half_counterexample_of_terminalScaledVanishing}{rational half counterexample},
\mword{Erdos257/HalfCounterexampleFrontier.lean}{39}{not_universal_of_terminalScaledVanishing}{universal-claim refutation}.
...
Two further conditional routes now sharpen the boundary around this half-value
problem.  They are logically distinct from the terminal scaled-vanishing
route: one seeks irrationality from a structured support, while the other
would produce a rational counterexample from cofinal geometric stages.

\begin{theorem}[orthogonal-petal sunflower route]
\label{res:sunflower}
...
argument: the selector forces a carry supply, and the support-series consumer
...
\begin{theorem}[cofinal-cylinder half countermodel route]
...
this cofinal-stage hypothesis.  The formal route does not construct the
cofinal family of full-cylinder stages.
\end{theorem}

\noindent The bridge first converts each full-cylinder stage into a weaker
terminal-only strip, then invokes the closed half-achievement-set consumer
...
This is a conditional countermodel frontier, not evidence that such stages
exist; proving or refuting their cofinality is the surviving task for this
route.
