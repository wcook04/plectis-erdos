...
\(A\subseteq\Npos\).  The lead theorem proved here is that for every integer
base \(b\ge2\) and every infinite \(A\subseteq\Npos\) with
\(\sum_{a\in A}a^{-1}<\infty\) the value \(X_A(b)\) is irrational, with no
coprimality hypothesis, and the complete argument is checked by the Lean
kernel.  Erd\H{o}s printed the pairwise-coprime case for every integer base
\(b\ge2\) in Math.\ Student 36 (1968), 222--226 and wrote on the same page
that the coprimality condition is superfluous, giving no argument; the
statement is his, and the proof and its check are the contribution here.
The theorem removes the entire convergent-reciprocal-mass regime, so the
exact remaining obligation in the universal problem is the
reciprocal-divergent supports.

Three further kernel-checked statements describe the hypothetical
counterexample rather than sampling supports.  A Boolean--M\"obius
correspondence makes existence of a normalised support of value \(p/q\)
equivalent to existence of one square-root-bounded integer orbit, and
reconstructs the support from that orbit.  A uniform repair criterion decides
membership in the subsum set for every real target through one finite
square-root window, so a single strictly increasing window refutes
membership.  A scaled greedy recursion identifies the subsum set with the
non-escaping set of an explicit nearly doubling map, with exponential escape
off it.  The classical full-support theorem of Erd\H{o}s at every integer
base is formalised together with its certificate machine, which also supplies
arbitrarily long zero blocks in the base-\(b\) expansion of that value.

Beyond reciprocal summability we prove irrationality, by ordinary arguments,
for several classes that allow divergent reciprocal mass: supports satisfying
a divisibility-weighted summability criterion, supports whose reciprocal mass
grows more slowly than an iterated logarithm, and supports admitting summable
positive fractional divisor covers.  Each criterion is hereditary under
passage to infinite subsets and works at every integer base \(b\ge2\).  A
separate argument, using Tao--Ter\"av\"ainen's correlation theorem, treats
every infinite subset of the prime powers, including fixed dilations and
finite modifications.

The three extension arguments share a mechanism.  Freeze a finite set of
exponents by a common multiple, then average the remaining shifted atoms to
produce arbitrarily small positive displacements.  Rationality forces a
fixed positive gap, giving the contradiction.  Keeping the incomplete-period
error in this average yields a quantitative necessary condition: a rational
value forces reciprocal mass to grow at least at an explicit multiple of
log-star.  Positive fractional covers extend the argument to moving divisor
frames, with exponents that may tend to zero.

For finite nonempty supports \(F\), the reduced denominator \(D_F\)
satisfies \(\operatorname{ord}_{D_F}(b)=\operatorname{lcm}(F)\); that
finite-support theorem supplies no unbounded-support certificate, and it is
developed in Section~\ref{sec:period}.
For infinite supports, the achievement set is compact, perfect, nowhere
dense, and has Lebesgue measure \(1\); restricting the exponents gives an
exact volume dichotomy.  At the rational targets \(1/2\) and \(1/21\),
finite-support exclusion turns membership into a potential counterexample,
and exact greedy criteria locate the remaining infinite-orbit question.

The analytic support criteria have ordinary proofs in
Section~\ref{sec:eight-return-extensions} and carry no formal check.  The
accompanying Lean development checks arbitrary reciprocal-summable support at
every integer base, the classical full-support and periodic-weight theorems,
the Boolean--M\"obius correspondence, the uniform repair criterion, the
scaled greedy-trap dynamics, the denominator identity, the geometric
classification, and the stated orbit reductions.  The unrestricted problem
and the two rational membership questions remain open.
\end{abstract}

\begin{paperresult}{Main results}
\textbf{Lead theorem.} Every infinite reciprocal-summable support has
irrational value at every integer base, with no coprimality hypothesis; the
whole proof is kernel checked.  Erd\H{o}s stated this extension of his
printed pairwise-coprime theorem without proof, so the statement is his and
the proof is local.  What remains of the universal problem is exactly the
reciprocal-divergent supports.
\textbf{Description of every rational value.} A Boolean--M\"obius
correspondence, a uniform square-root repair criterion valid at every real
target, and an exact greedy-trap dynamics for the subsum set are each
kernel checked, and each quantifies over every hypothetical counterexample.
\textbf{Classical theorems formalised.} The full-support theorem of
Erd\H{o}s at every integer base, with zero-block digit witnesses, and the
periodic and pairwise-coprime families.
\textbf{Ordinary extensions.} Divisibility-weighted summability, sub-log-star
reciprocal mass, and positive fractional divisor covers each imply
irrationality at every integer base, hereditarily.  Every infinite
prime-power sub-support also has irrational value, by a separate analytic
argument.  These four have ordinary proofs and no formal check, and they
include supports with divergent reciprocal mass.
\textbf{Finite supports.} The reduced denominator has multiplicative order
exactly the least common multiple of the exponents.
\textbf{Geometry and rational targets.} The subsum set has measure $1$ and
an exact restricted-support volume law; finite-support exclusion and greedy
criteria isolate the unresolved membership questions at $1/2$ and $1/21$.
\end{paperresult}
...
Erd\H{o}s~\cite[p.~105]{erdos1988}.  Bloom's catalogue records the problem as open~\cite{erdosproblems}.
...
of the problem enters.  The datum is the $0/1$ indicator of $A$.
Equation~\eqref{eq:incidence} contains its divisor transform, a nonnegative
integer sequence bounded by the divisor function,
...
\begin{theorem}[reciprocal-summable supports]
\label{res:reciprocal-summable-support}
Let $b\ge2$ be an integer and let $A\subseteq\Npos$ be infinite.  If
$\sum_{a\in A}a^{-1}<\infty$, then $X_A(b)$ is irrational.
\end{theorem}

\noindent The complete implication, from the two displayed hypotheses to
irrationality, is
\mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{395}{irrational_erdosSupportSeries_of_summable_reciprocal}{checked by the Lean kernel};
no ordinary step and no finite certificate is left outside that check.
The proof first obtains binary LCM--Ces\`aro close returns for the shifted
support atoms.  A pointwise displacement bound transfers those returns to
every integer radix, where an exact integral tail orbit contradicts
rationality.

\emph{Attribution.}  On p.~222 of~\cite{erdos1968} Erd\H{o}s prints the
theorem for pairwise-coprime supports with convergent reciprocal sum, at every
integer base $b\ge2$, and on the same page he says that pairwise coprimality
can be removed by a more complicated argument, giving no details; p.~226
repeats that boundary.  The statement of
Theorem~\ref{res:reciprocal-summable-support} is therefore his stated
extension.  What is local is the proof and its checked development.  The
argument here is not identified with the one he omitted, and its novelty and
priority have not been assessed.  The printed pairwise-coprime theorem is
retained below as an attributed specialisation.

\emph{Where the coprimality hypothesis is not needed.}  The proof works in the
shifted-atom coordinate $\sum_{d\in A}b^{N\bmod d}/(b^{d}-1)$, the
\mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{34}{shiftedRadixSupportAtom}{support-restricted
radix atom} summed over $d$.  Each element of $A$ contributes one periodic
orbit of its own period, and the sum records no relation between distinct
elements.  Pairwise coprimality is a hypothesis about such relations, so this
coordinate gives it no place to enter.  Reciprocal summability is consumed
instead, at exactly one point.  The close return at shift $N$ comes from the
LCM-prefix limit of the gcd-orbit means, then a Ces\`aro limit at the selected
positive LCM step, then a single term no larger than that finite average, which
is
\mword{Erdos249257/ReciprocalSupportIrrationality.lean}{957}{exists_shiftedSupportAtom_closeReturn_of_summable_reciprocal}{checked}.
Infinitude of $A$ supplies the strict minimality of the shift-$0$ value
(\mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{246}{shiftedRadixSupportAtom_zero_strictMinimum}{checked}),
a displacement bound with constant $2$ uniform in $b$, $N$ and $d$ carries the
binary returns to every integer radix
(\mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{270}{shiftedRadixAtom_sub_zero_le_two_mul_binary_sub_zero}{checked}),
and the integer orbit $u_{N+1}=bu_N-v\,\scA_A(N+1)$ forced by a rational value
then has $u_N-u_0$ strictly between $0$ and $1$.  The unresolved regime sits
outside this argument at the same single point: for an infinite support with
$\sum_{a\in A}a^{-1}=\infty$ the averaging step has no summable majorant and
supplies no close return.  Section~\ref{sec:eight-return-extensions} reaches
subclasses of that regime by separate ordinary criteria, and
Section~\ref{sec:forced} constrains a hypothetical rational value there without
excluding one.

\emph{Remaining obligation.}  Theorem~\ref{res:reciprocal-summable-support}
settles every infinite support with $\sum_{a\in A}a^{-1}<\infty$.  The exact
statement still to be proved for Problem~\ref{res:problem} is irrationality
of $X_A$ for every infinite $A$ with $\sum_{a\in A}a^{-1}=\infty$.
Sections~\ref{sec:eight-return-extensions} and~\ref{sec:forced} work inside
that regime, and nothing below exhausts it.

\paragraph{The classical full support, formalised.}
The support $A=\Npos$ carries the oldest theorem in this circle, and the
certificate machine that proves it is the engine every support family here
rides.

\begin{theorem}[full support at every integer base]
\label{res:full-support-all-base}
Let $b\ge2$ be an integer.  Then $\sum_{n\ge1}(b^{n}-1)^{-1}$ is irrational.
Moreover, for every $K\in\N$ there are $N\in\N$ and $z\in\Z$ with
\[
  0<b^{N}\sum_{n\ge1}\frac1{b^{n}-1}-z<b^{-K},
\]
so the base-$b$ expansion of that value contains arbitrarily long blocks of
zero digits.
\end{theorem}

\noindent Irrationality is Erd\H{o}s's 1948 theorem~\cite{erdos1948}; the
statement and the mathematics are his, and what is local is the formalisation.
Both displayed clauses are inside the kernel check.  Irrationality is
\mword{Erdos249257/CertificateKernel.lean}{8516}{irrational_erdosSum_full_support}{checked},
with the base-$2$ instance for the Erd\H{o}s--Borwein constant at
\mword{Erdos249257/CertificateKernel.lean}{8523}{irrational_erdosBorwein_series}{checked}.
The engine is a bounded Bertrand and Chinese-remainder first-block frame, a
middle-window divisor-pair average with pigeonhole selection, and an explicit
parameter closure, supplied as
\mword{Erdos249257/CertificateKernel.lean}{8375}{exists_weighted_block_certificates}{one checked certificate family},
and the coefficient abstraction is checked to lose nothing at
\mword{Erdos249257/CertificateKernel.lean}{8877}{irrational_erdosSum_full_support_via_coeff_engine}{the coefficient-engine form}.
The zero-block clause is
\mword{Erdos249257/CertificateKernel.lean}{8920}{zero_block_witnesses_erdosSum_full_support}{checked},
as the precision-$b^{K}$ specialisation of the near-integer witnesses
\mword{Erdos249257/CertificateKernel.lean}{8896}{near_int_witnesses_erdosSum_full_support}{checked}.
Two boundaries.  The digit clause concerns zero blocks alone; it gives no
...
result~\cite[Theorem~1, p.~12; proof pp.~12--24]{campbell2026}.  And
$A=\Npos$ is one support, so this theorem leaves the remaining obligation
above untouched.

\paragraph{A description of every rational value.}
The next theorem replaces the search for a rational-valued support by a search
for one integer orbit, and it reconstructs the support from that orbit.  It is
possible because the support can always be read back off the incidence
sequence: with $\mu$ the M\"obius function, $*$ Dirichlet convolution
...
\noindent The equivalence and the reconstruction are
\mword{Erdos249257/BooleanMobiusCarry.lean}{949}{exists_normalized_support_fraction_iff_exists_booleanMobiusCarry}{checked},
with the two directions at
\mword{Erdos249257/BooleanMobiusCarry.lean}{836}{exists_booleanMobiusCarry_of_support_fraction}{support to orbit}
and
\mword{Erdos249257/BooleanMobiusCarry.lean}{859}{support_fraction_of_booleanMobiusCarry}{orbit to support}.
The whole statement is inside the kernel check, with no ordinary step and no
finite certificate outside it.  Prior art for this correspondence has not been
assessed; the register records its novelty as unassessed, and no priority is
claimed.
...
$\{2,3\}$ through that range.  This finite calculation is a computation and
illustrates the correspondence; the admissible scaled-tail sequence itself is
an infinite object, and the finite support is not a counterexample to
Problem~\ref{res:problem}.

\emph{The remaining obligation after Theorem~\ref{res:bmc}.}  The support the
theorem produces is not required to be infinite, and a finite support supplies
an admissible scaled-tail sequence for its own value, so existence of such a
sequence is by itself no counterexample to Problem~\ref{res:problem}.  What
the equivalence changes is the search space.  The obligation it leaves is to
decide whether an admissible sequence exists whose reconstructed support is
infinite.

\paragraph{The subsum set and the greedy expansion.}
The next two theorems are about the set of values obtainable from the weights
$\wgt n$, so we fix that object here and return to its geometry in
Section~\ref{sec:geometry}.  Write
\[
  \Ach=\Bigl\{\sum_{n\ge1}\varepsilon_n\wgt n:\varepsilon_n\in\{0,1\}\Bigr\}
\]
for the achievement set, the set of all values of finite and infinite
selections alike.  For a real $x\ge0$ the \emph{greedy expansion} runs through
$n=1,2,\dots$ carrying a remainder, initially $x$, and at level $n$ subtracts
$\wgt n$ from the remainder when $\wgt n$ does not exceed it, leaving the
remainder unchanged otherwise; a level at which nothing is subtracted is
\emph{skipped}, and $A_x$ denotes the set of levels at which a subtraction
happens.  Write $r_N(x)$ for the remainder after level $N$.

\begin{theorem}[uniform repair criterion]\label{res:generic-repair}
For every real $x\ge0$, put
\[
 P_0(x)=0,\qquad P_{N+1}(x)=2P_N(x)+\scA_{A_x}(N+1),\qquad
...
$Q_N(x)\le\scA_{A_x}(N+1)$ cofinally; equivalently, $Q_N(x)\le N+1$
cofinally. One strictly increasing window excludes membership.
\end{theorem}

\begin{proof}
If $x\in\Ach$, the full tail identity and divisor-pair bound give
...
floor digit give
\[
 2Q_N(x)-\scA_{A_x}(N+1)\le Q_{N+1}(x).
\]
A repair therefore forces $Q_N(x)\le\scA_{A_x}(N+1)\le N+1$.
If $x\notin\Ach$, a positive fatal gap $\delta$ persists in the actual
...
\noindent The displayed square-root window equivalence is
\lword{Erdos257/GreedyRepairCriterion.lean}{192}{mem_iff_greedyBinaryDefect_sqrt_windows}{checked}
for every real $x\ge0$, with the three further equivalent forms at
\lword{Erdos257/GreedyRepairCriterion.lean}{175}{mem_iff_greedyBinaryDefect_cofinal_repairs}{cofinal repairs},
\lword{Erdos257/GreedyRepairCriterion.lean}{207}{mem_iff_greedyBinaryDefect_cofinal_load_bound}{cofinal load bound}
and
\lword{Erdos257/GreedyRepairCriterion.lean}{222}{mem_iff_greedyBinaryDefect_cofinal_linear_bound}{cofinal linear bound},
and the exclusion clause at
\lword{Erdos257/GreedyRepairCriterion.lean}{232}{not_mem_of_greedyBinaryDefect_strict_sqrt_window}{one strict window}.
The displayed proof above is the ordinary reading of that checked statement,
and every clause of the theorem is inside the kernel check.  Prior art is
unassessed and no priority is claimed.
...
rank $4096$ finds no strict window for that target, which is a computation
and not a theorem.

\emph{The remaining obligation after Theorem~\ref{res:generic-repair}.}  The
theorem identifies the certificate to seek at any given target, and it
supplies no occurrence of one.  Deciding $1/2$ or $1/21$ or $4/9$ still
requires either one strictly increasing window or a cofinal supply of
repairs, and neither is proved for any of those targets.

\begin{theorem}[scaled-greedy trap, escape, and cofinal return]
\label{res:scaled-greedy-trap}
For every nonnegative real $x$, writing $y_N(x)=2^N r_N(x)$ for the scaled
greedy remainder,
\[
 x\in\Ach\quad\Longleftrightarrow\quad
...
\noindent With $c_{n}=2^{n}/(2^{n}-1)$, the scaled recursion is the explicit
nearly doubling map sending $y_N$ to $2y_N-c_{N+1}$ when $c_{N+1}\le2y_N$ and
to $2y_N$ otherwise
(\mword{Erdos249257/GreedyTrapDynamics.lean}{72}{scaledGreedyRemainder_succ}{checked}),
so $\Ach$ is the filled non-escaping set of that map.  The trap-set equality is
\mword{Erdos249257/GreedyTrapDynamics.lean}{262}{mersenneAchievementSet_eq_scaledGreedyTrap}{checked},
its membership form is
\mword{Erdos249257/GreedyTrapDynamics.lean}{245}{mem_mersenneAchievementSet_iff_forall_scaledRemainder_lt_two}{checked},
the escape theorem is
\mword{Erdos249257/GreedyTrapDynamics.lean}{189}{scaledGreedyRemainder_tendsto_atTop_of_not_mem}{checked},
and the bounded-cofinal-return equivalence is
\mword{Erdos249257/GreedyTrapDynamics.lean}{225}{mem_mersenneAchievementSet_iff_scaledRemainder_cofinallyBounded}{checked}.
Every clause is inside the kernel check.  The escape proof locates a fatal
rank at which the residual exceeds the complete remaining tail by some
$\delta>0$.  That excess persists, so the scaled residual is at least
$2^N\delta$ thereafter.  This is the hard step in the reverse bounded-return
implication: an orbit with even one bounded cofinal subsequence cannot lie
outside $\Ach$.  Section~\ref{sec:geometry} records that $\Ach$ is compact,
perfect, totally disconnected, nowhere dense and of Lebesgue measure $1$, so
the theorem exhibits a fat Cantor set as a non-escaping set.  The compactness,
perfectness and nowhere density are classical, and Kakeya~\cite[p.~251]{kakeya1914}
and Kova\v{c} and Tao~\cite[Remark~4.1, p.~13]{kovactao} are the antecedents
named in Section~\ref{sec:geometry}.  Novelty for the dynamical identification
itself has not been assessed.

\emph{The remaining obligation after Theorem~\ref{res:scaled-greedy-trap}.}
The theorem gives no orbit.  Deciding whether $1/2$ or $1/21$ lies in $\Ach$
still requires one bounded cofinal return or one escape, and neither is
proved; the rational specialisations are in Section~\ref{sec:geometry}.

\statusboundary

\paragraph{Structure.}  Section~\ref{sec:eight-return-extensions} gives the
ordinary analytic criteria that reach supports of divergent reciprocal mass.
Section~\ref{sec:period} states and develops the finite-support denominator
theorem, which settles no infinite support.  Section~\ref{sec:forced} collects
the further necessary conditions that rationality would force on an arbitrary
infinite support.
Section~\ref{sec:map} places Theorem~\ref{res:reciprocal-summable-support}
beside representative supports already known to give irrational values,
grouped by the argument that reaches them.
...
The following ordinary proofs exclude several classes with divergent
reciprocal mass.  Complete proofs, examples and countermodels appear in
\href{https://github.com/wcook04/plectis-erdos/blob/e8458cdce502436d27af8a77d1e4d6742d3bcb69/docs/research/erdos257-eight-return-proofs.md}{the analytic proof supplement}.
...
\begin{theorem}[divisibility-weighted supports]\label{thm:weighted-criterion}
Let $P$ be a finite nonempty set of primes, let $A\subseteq\Npos$ be infinite,
and suppose \eqref{eq:weighted-return} holds.  Then
$\sum_{a\in A}(b^a-1)^{-1}$ is irrational for every integer $b\ge2$, and the
same conclusion holds for every infinite subset of $A$.
\end{theorem}

\noindent The proof is ordinary and is given below and in the linked
supplement; no formal check covers the assembled criterion.  What the kernel
covers is its analytic ingredients, listed at the end of this note.  A
fixed-base version replaces $2$ by $b$ in the
condition; arbitrary nested divisibility chains have the same criterion
with $h(a)$ the largest chain element dividing $a$.  The remaining obligation
is unchanged: the criterion selects a class of reciprocal-divergent supports
and leaves the rest of that regime open.
...
The formal development also checks the \href{https://github.com/wcook04/plectis-erdos/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/WeightedSupportLimits.lean#L111}{summability of the dyadic observation mass}, its \href{https://github.com/wcook04/plectis-erdos/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/WeightedSupportLimits.lean#L81}{vanishing mean}, and the \href{https://github.com/wcook04/plectis-erdos/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/BatchReturnSynthesis.lean#L151}{irrationality consumer from close returns}. These are the checked analytic ingredients; the complete weighted criterion above still uses the ordinary assembly in the supplement.
...
\begin{theorem}[sub-log-star reciprocal mass]\label{thm:logstar-mass}
Let $b\ge2$ be an integer and let $A\subseteq\Npos$ be infinite.  If
$\sum_{a\in A}(b^a-1)^{-1}$ is the reduced rational $p/q$, then
\[
 \liminf_{x\to\infty}\frac{H_A(x)}{\ell(x)}\ge\frac\lambda2,
 \tag{Q}\label{eq:logstar-mass}
\]
with $\lambda$ as above.  Consequently $H_A(x)=o(\ell(x))$ implies
irrationality at every integer base, and the conclusion is hereditary under
passage to infinite subsets.
\end{theorem}

\noindent The proof is ordinary and is the displayed argument below; no formal
check covers it.  Indeed, at suitable multiples of the order of $b$ modulo $q_c$, rationality
...
decomposition.  This does not exclude other fixed-exponent covers of
the same union.  Positivity matters: the divisor expansion for
$F=\{2,3\}$ has coefficient $2^\alpha-2<0$ at $6$ when $\alpha<1$.

\paragraph{The prime-power regime.}

\begin{theorem}[prime-power supports]\label{thm:prime-power-supports}
Let $A$ be an infinite subset of the prime powers, let $b\ge2$ be an integer,
and let $A'$ be a fixed dilation of $A$ or a finite modification of one.  Then
$\sum_{a\in A'}(b^a-1)^{-1}$ is irrational.
\end{theorem}

\noindent The argument is ordinary and carries no formal check.  Its analytic
input is
Tao--Ter\"av\"ainen~\cite[Theorem~3.1]{taoteravainen2025}, so the analytic
authority for this theorem is theirs and the thinning to every infinite subset
is what is added here.
...
are due to Tao and Ter\"av\"ainen.

\subsection{Fixed repair schedules, and what they decide}
The repair mechanism requires neither periodic digits nor a rational target,
and its uniform form is Theorem~\ref{res:generic-repair} in
Section~\ref{sec:problem}, stated there for every real $x\ge0$ and kernel
checked.  The finite quantities below are computations against that
criterion, and each one decides a schedule rather than a target.
...
\href{https://github.com/wcook04/plectis-erdos/blob/e8458cdce502436d27af8a77d1e4d6742d3bcb69/docs/research/erdos257-eight-return-synthesis.md}{synthesis supplement}.


\section{Finite-support denominator periods}\label{sec:period}

A finite support has a rational value.  The relevant arithmetic questions are
the size of its denominator and the position of the base within that
denominator.
...
Every clause is inside the kernel check.  This theorem answers the second
...
quantities are both~$1$.  It sits here, and not among the leading results,
because it settles no infinite support: it supplies no unbounded-support
certificate, and no limit of it does.
...
mechanism is that each selected exponent $n$ contributes, through cyclotomic
factorisation, a prime-power modulus dividing $b^{n}-1$ on which $b$ has order exactly
...
The results here instead quantify over \emph{every} infinite support whose
value is rational, which is the quantifier in Problem~\ref{res:problem}.
The base is $2$ throughout, and each statement carries its own hypotheses.
...
sequence is then
the scaled tail
$u(n)=v\sum_{j\ge1}f(n+j)2^{-j}$; hence exactly one such orbit exists
...
\paragraph{The Boolean--M\"obius characterisation, stated earlier.}
The theorems of this section give necessary conditions.  The sharpest
description of rational-valued supports the development has is an equivalence,
and it is stated as Theorem~\ref{res:bmc} in Section~\ref{sec:problem},
together with the admissibility conditions~\eqref{eq:bmc}, the reconstruction
$A=\{n:(\mu*Q_U)(n)=1\}$ and the finite instance at $A=\{2,3\}$.  It belongs
with this section's material and is placed among the leading results because
it removes the support from the description altogether.
...
sublogarithmic divisor-coverage gaps.  By
Theorem~\ref{res:reciprocal-summable-support}, its reciprocal mass must
diverge.  Theorems~\ref{res:mass} and~\ref{res:dyadicmass} remain useful
conditional estimates for convergent reciprocal mass, but that regime can no
longer contain a counterexample.  Theorem~\ref{res:bmc} reconstructs every
...
\paragraph{Why periodic weights give divisibility.}
The full-support and periodic-support results share one argument in the
coefficients of~\eqref{eq:incidence}.  To see it, first take $m\ge1$ and a purely
$m$-periodic weight $w\colon\Npos\to\N$ and put
\[
  c_w(n)=\sum_{d\mid n}w(d).
\]
Fix a prime $p\nmid m$ and write a positive integer as
$n=p^k u$ with $p\nmid u$.
Grouping divisors by their $p$-adic valuation gives
\[
  c_w(n)=\sum_{d\mid u}\sum_{j=0}^{k}w(dp^j).
\]
Euler's congruence $p^{\ph(m)}\equiv1\pmod m$ makes each inner
sequence periodic with a period dividing $\ph(m)$.  Choosing
$k=b\ph(m)-1$ therefore gives $b$ copies of a complete cycle:
\[
  \sum_{j=0}^{b\ph(m)-1}w(dp^j)
     =b\sum_{j=0}^{\ph(m)-1}w(dp^j).
\]
Thus $b\mid c_w(n)$.  With $r$ distinct primes at these exact valuations,
the divisor sum separates into $r$ such coordinates and gains a factor
$b^r$.  This is the arithmetic step that lets the certificate argument
reach periodic supports: periodicity of the weights forces divisibility of
their divisor transform.  The full-support case is $w\equiv1$.

Divisibility at one integer is not yet an irrationality proof.  The checked
argument arranges blocks of these valuations by the Chinese remainder
theorem and controls the remaining weighted tail to obtain the required
certificates.  Its weighted form uses
$w(d)=\sum_{i=0}^{W-1}\mathbf1_{\{w(d)>i\}}$ for a bound $W$ on $w$:
each indicator layer is periodic, so the same divisor argument applies.
The theorem cited in the table then clears a common denominator for rational
weights and removes a finite prefix.  Removing that prefix changes the
series by a rational number; the periodic tail must still contain a positive
weight.  Without that hypothesis, a nonzero finitely supported sequence
would give a rational counterexample.
...
\midrule
\multicolumn{3}{@{}l}{\emph{Reciprocal-summable supports}}\\
arbitrary infinite $A$ with $\sum_{a\in A}a^{-1}<\infty$ & every $b\ge2$ &
  Theorem~\ref{res:reciprocal-summable-support};
  \mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{395}{irrational_erdosSupportSeries_of_summable_reciprocal}{proved here} \\
pairwise coprime, $\sum_{a\in A}a^{-1}<\infty$ & every $b\ge2$ &
  Erd\H{o}s~\cite{erdos1968}, theorem on p.~222; retained as a separately
  formalised specialisation
  (\mword{Erdos249257/CertificateKernel.lean}{10964}{irrational_erdosSupportSeries_pairwise_coprime}{formalised here}) \\
\addlinespace
\multicolumn{3}{@{}l}{\emph{Classical full support, and base change into it}}\\
all of $\Npos$ & every $b\ge2$ & Erd\H{o}s 1948~\cite{erdos1948};
  \mword{Erdos249257/CertificateKernel.lean}{9233}{irrational_erdosSupportSeries_univ}{formalised here} \\
multiples of a fixed $d$ & every $b\ge2$ & dilation: the multiples series at
  base $b$ \emph{is} the full-support series at base $b^d$
  (\mword{Erdos249257/CertificateKernel.lean}{9291}{irrational_erdosSupportSeries_multiples}{checked}) \\
\addlinespace
\multicolumn{3}{@{}l}{\emph{Periodicity of the coefficient sequence}}\\
infinite eventually periodic & every $b\ge2$ &
  \mword{Erdos249257/CertificateKernel.lean}{12999}{irrational_ratWeightSeries_eventuallyPeriodic}{checked here};
...
a residue class; the odd numbers & every $b\ge2$ & special cases of the row
  above (\mword{Erdos249257/CertificateKernel.lean}{11860}{irrational_erdosSupportSeries_residueClass}{residue class},
  \mword{Erdos249257/CertificateKernel.lean}{11874}{irrational_erdosSupportSeries_odd}{odd});
...
factorials $\{n!\}$ & every $b\ge2$ &
  \mword{Erdos249257/CertificateKernel.lean}{6223}{irrational_erdosSum_factorial_support}{checked here} \\
powers of two $\{2^k\}$ & every $b\ge2$ &
  \mword{Erdos249257/CertificateKernel.lean}{6247}{irrational_erdosSum_two_pow_support}{checked here} \\
infinite pairwise coprime, $\sum_{a\in A}a^{-1}<\infty$ & every $b\ge2$ &
...
\midrule
arbitrary infinite $A$ with $\sum_{a\in A}a^{-1}=\infty$ & --- &
  \textbf{Open} (the remaining part of Problem~\ref{res:problem}) \\
\bottomrule
\end{tabular}
\end{table}
...
The first row above is a local Lean theorem with its own LCM--Ces\`aro and
integer-orbit proof.  Novelty and priority are unassessed, and this checked
proof is not identified with Erd\H{o}s's omitted argument.  The printed
pairwise-coprime theorem remains represented as an attributed specialisation.
...
\mword{Erdos249257/CertificateKernel.lean}{8516}{irrational_erdosSum_full_support}{the exact checked statement};
...
rows do not.  The distinction is intrinsic: the
...
(\mword{Erdos249257/CertificateKernel.lean}{6460}{lcm_gap_hypothesis_fails_full_support}{checked}).
...
The full-support result formalises Erd\H{o}s's theorem; Luca and Tachiya's RIMS paper already proves the
...
(\mword{Erdos249257/CertificateKernel.lean}{14363}{irrational_or_bpow_mul_eq_intCast_intWeightedErdosSeries_periodic}{checked}).
...
\section{Structured supports: preserving the remaining divisibility}
\label{sec:bouquet}

The periodic argument manufactures long divisible blocks.  For a sparse
composite support the difficulty is different: after finitely many support
elements have been absorbed into a congruence modulus, what arithmetic freedom
remains in the others?  A finite-core bouquet makes that question exact.
Write
\[
 A=E\cup\{c_i p_i:i\ge0\},\qquad c_i\mid Q,\quad e\mid Q\ (e\in E),
\]
where $Q,c_i,e$ are positive, $E$ is finite, the $p_i>1$ are pairwise coprime
and coprime to $Q$, and $\sum_i p_i^{-1}<\infty$.  The petals $p_i$ need not
be prime.  For every finite set $F$ of indices, put
$M_F=Q\prod_{j\in F}p_j$.  Then
\[
 \frac{c_i p_i}{\gcd(c_i p_i,M_F)}=p_i\qquad(i\notin F).
\]
Thus a new ray retains its entire petal as the modulus governing its hits
along a progression with step $M_F$
(\mword{Erdos249257/SupportSunflowerDichotomy.lean}{192}{ray_div_gcd_frameStep}{reduced-modulus identity}).

There are concrete infinite examples.  Let $r_0,r_1,\ldots=5,7,11,13,\ldots$
be the primes after $3$, take $Q=6$, $E=\varnothing$, $p_i=r_i^2$, and
alternate $c_i=2,3$.  This gives
$A=\{50,147,242,507,\ldots\}$; every member has exactly three prime factors
counted with multiplicity.  The bouquet, infinitude and factor count are
\mword{Erdos249257/SupportSunflowerDichotomy.lean}{319}{alternatingPrimeSquareBouquet}{constructed},
\mword{Erdos249257/SupportSunflowerDichotomy.lean}{337}{alternatingPrimeSquareSupport_infinite}{proved}, and
\mword{Erdos249257/SupportSunflowerDichotomy.lean}{343}{alternatingPrimeSquareSupport_cardFactors_eq_three}{checked}, respectively.
The reciprocal-summable support theorem already settles these examples.
The reduced-modulus identity describes their arithmetic structure; no
additional block selector is needed for irrationality. The all-base argument
is proved in \href{https://github.com/wcook04/plectis-erdos/blob/e8458cdce502436d27af8a77d1e4d6742d3bcb69/docs/research/erdos257-eight-return-proofs.md}{the support-averaging supplement}.

\begin{theorem}[unconditional orthogonal-petal criterion]
\label{res:sunflower}
Let $A$ be a bouquet as above. Then
$\sum_{a\in A}(b^a-1)^{-1}$ is irrational for every integer $b\ge2$.
\end{theorem}

\begin{proof}
Each positive core satisfies $c_i\ge1$. The support decomposition gives
\[
 \sum_{a\in A}\frac1a
 \le \sum_{a\in E}\frac1a+\sum_i\frac1{c_ip_i}
 \le \sum_{a\in E}\frac1a+\sum_i\frac1{p_i}<\infty.
\]
The exceptional sum is finite, and the last series is a defining bouquet
hypothesis. The rays are distinct, so $A$ is infinite. Apply the all-base
reciprocal-summable support theorem. In particular, the alternating-core
example above gives an irrational sum at every integer base.
\end{proof}

The \href{https://github.com/wcook04/plectis-erdos/blob/43c36af8b9d5795d0a965825ec01110b87159df6/Erdos249257/BouquetReciprocalIrrationality.lean#L85}{unconditional all-base bouquet theorem} is now kernel checked. The older forced-slot selector is unnecessary for this corollary; it belongs to a stronger certificate construction.
...
that local budget. The unconditional irrationality in
Theorem~\ref{res:sunflower} follows instead from reciprocal summability,
without estimating those orbit defects.
...
\mword{Erdos249257/CertificateKernel.lean}{9655}{irrational_erdosSupportSeries_of_tail}{Lean}
and
\mword{Erdos249257/CertificateKernel.lean}{9664}{irrational_erdosSupportSeries_tail_of_irrational}{Lean}.
...
$\Ach=\{\sum_{n\ge1}\varepsilon_n\wgt n:\varepsilon_n\in\{0,1\}\}$ of
Section~\ref{sec:problem}, and this section is about its geometry.
...
\mword{Erdos249257/GreedyAchievementSet.lean}{1654}{perfect_mersenneAchievementSet}{perfect},
\mword{Erdos249257/GreedyAchievementSet.lean}{1670}{isTotallyDisconnected_mersenneAchievementSet}{totally disconnected},
\mword{Erdos249257/GreedyAchievementSet.lean}{1679}{isNowhereDense_mersenneAchievementSet}{nowhere dense},
and
...
(\mword{Erdos249257/GreedyAchievementSet.lean}{1782}{three_fourths_not_mem_mersenneAchievementSet}{Lean}).
...
\lword{Erdos257/MersenneSubseriesRigidity.lean}{418}{volume_supportedMersenneAchievementSet_dichotomy}{the formal dichotomy}.
...
\noindent Checked as
\mword{Erdos249257/GreedyAchievementSet.lean}{2582}{half_mem_mersenneAchievementSet_iff_greedySkippedSupport_infinite}{the greedy form},
...
\mword{Erdos249257/BooleanMobiusCofinalExactRows.lean}{431}{half_mem_mersenneAchievementSet_of_cofinalExactLocalRows}{the closed-set step}.
...
\lword{Erdos257/HalfCounterexampleFrontier.lean}{40}{exists_rational_half_counterexample_of_terminalScaledVanishing}{rational half counterexample},
\lword{Erdos257/HalfCounterexampleFrontier.lean}{48}{not_universal_of_terminalScaledVanishing}{universal-claim refutation}.
...
A different conditional argument would produce an infinite support of value
$1/2$ from cofinal geometric stages.
...
\subsection{Five layers proved insufficient at \texorpdfstring{$1/2$}{1/2}}

The $1/2$ question has an exact reduction and an exact record of what cannot
close it.  With $\rho_s$ the seam integer greedy remainder, membership follows
from $\rho_s\le2^s$ holding infinitely often
(\lword{Bit/R2.lean}{306}{half_mem_of_remainderReachesHalfPointCofinally}{checked}),
and membership is equivalent to a cofinal late largest skip
(\lword{Skip/D2.lean}{307}{half_mem_iff_unboundedLargestSkipLate}{checked}).

\begin{remark}[what has been proved insufficient]\label{res:half-insufficiency}
Five separate mechanisms are proved unable to close that reduction, each by a
checked countermodel or bound: the row recursion is simultaneously silent at
$s=16$ while row $17$ is good through skipped rank $14$
(\lword{Rem/C1.lean}{436}{rowLaw_blind_at_16}{checked}); the model
$f(s)=2^s+2$ satisfies every row law and ceiling while never returning to the
half point
(\lword{Rem/C1.lean}{206}{exists_persistent_surplus_model}{checked}); every
ceiling growing at rate at least $4$ is compatible with the descent law
forever
(\lword{Rem/C1.lean}{109}{le_of_descent_of_quadrupling}{checked}); every
ceiling propagating by one-step interval induction is $\Omega(4^{s})$, so the
target $\rho_s<2^{s+1}$ is out of its reach
(\lword{Three/T1.lean}{542}{IntervalCeiling.four_pow_le}{checked}); and
$2^s+3$ is odd at every row, obeys the step law, and never reaches the half
point, so no congruence modulo $2$ empties a band of positive width
(\lword{Three/T3.lean}{618}{exists_persistent_odd_surplus_model}{checked}).
A sixth observation removes the dyadic dichotomy: $2^{s+1}y_s=\rho_s+2^s$ is
an integer at every row
(\lword{Three/T2.lean}{746}{two_pow_mul_y}{checked}), so the orbit point is a
dyadic rational at every finite row.
\end{remark}

\noindent The measured trigger density of $\rho_s\le2^s$ is $0.49967$ over
$s\in[5,6000]$, with geometric gaps and maximum gap $13$ at $s=5372$.  That
figure is a computation over a bounded range and proves no cofinality.  One
correction is on record and is repeated here because it is the kind of
statement a finite scan gets wrong: the residual invariant
$\forall s\,d,\ \operatorname{seamResidualUpto}(s,d)<2^{2s-d}$ was reported as
empirically true with maximum ratio $0.99989$ over $s\in[6,3000]$, and it is
false at $s=13$, $d=7$, where the residual is $524419$ against the ceiling
$2^{19}=524288$
(\lword{Three/T1.lean}{159}{not_residualInvariant}{checked}).  An independent
rescan found that the sole failure in the stated range.

These are exclusions of named mechanisms.  None of them decides whether
$1/2\in\Ach$, and the question stays open on both sides.
...
\lword{Erdos257/HalfCounterexampleFrontier.lean}{1041}{finiteErdosSum_ne_one_div_twenty_one}{the formal theorem}.
...
Theorem~\ref{res:scaled-greedy-trap} in Section~\ref{sec:problem} gives the
trap equality, the exponential escape and the bounded cofinal return, for
every nonnegative real target and with every clause kernel checked.  At a
rational target the lower branch supplies a further equivalent form, which is
the one the $1/21$ discussion uses.

\begin{theorem}[rational lower-separatrix criterion]
\label{res:scaled-lower-separatrix}
Let $q$ be a nonnegative rational and write
$y_N(q)=2^N\operatorname{greedyMersenneRemainder}(q,N)$.  Then
\[
 q\in\Ach\quad\Longleftrightarrow\quad
...
\noindent The general rational criterion is
...
Problem~\ref{res:problem} remains the frame.  Theorem~\ref{res:reciprocal-summable-support}
settles every infinite support of convergent reciprocal mass, so the exact
remaining universal boundary is the reciprocal-divergent case.  The forced
conditions in Section~\ref{sec:forced} are not known to be contradictory there.
...
\begin{enumerate}
\item \textbf{Problem~\ref{res:problem} itself}, now only for infinite $A$
  with $\sum_{a\in A}a^{-1}=\infty$.  Section~\ref{sec:forced} constrains every hypothetical
  ...
  admissible Boolean--M\"obius scaled-tail sequence.  No contradiction among
  the applicable constraints is known.
...
\noindent This condition is both necessary and sufficient.  It is the
$x=1/21$ specialization of the general membership equivalence in
...
$Q_N\le1$
(\mword{Erdos249257/BooleanMobiusCarry.lean}{2842}{one_div_twenty_one_mem_mersenneAchievementSet_of_defect_le_one_cofinally}{sufficient condition})
...
corollary applies to power-of-two bases.

\section*{Statements and declarations}

The inline links identify the exact formal statements used in each section.
The analytic criteria in Section~\ref{sec:eight-return-extensions} and the
all-base bouquet composition in Theorem~\ref{res:sunflower} have ordinary
proofs above and in the linked supplement. The displayed proof of
Corollary~\ref{res:blind}, the
...
\paragraph{Checked declarations behind the reviewed support families.}
Every finite-core orthogonal-petal bouquet has irrational reciprocal-power
value at every integer base at least two.  Its defining summable petal
reciprocals bound the actual support reciprocals,
\mword{Erdos249257/BouquetReciprocalIrrationality.lean}{19}{summable_reciprocalSupportTerm}{support
summability}, so no forced-slot selector is needed:
\mword{Erdos249257/BouquetReciprocalIrrationality.lean}{85}{irrational_erdosSupportSeries_of_orthogonalPetalBouquet_allBase}{all-base
bouquet corollary}, specialising the base-two statement
\mword{Erdos249257/SupportSunflowerDichotomy.lean}{540}{irrational_erdosSupportSeries_of_orthogonalPetalBouquet}{base-two
bouquet theorem}.

For the divisibility-weighted family the ordinary proof carries the full
theorem; the kernel checks its analytic ingredients.  The finite orbit
estimate is
\lword{Erdos257/WeightedSupportAveraging.lean}{49}{cesaroMean_shiftedMersenneAtom_mul_le_gcdMean_add_error}{Ces\`aro
orbit estimate} with the dyadic observation bound
\lword{Erdos257/WeightedSupportAveraging.lean}{148}{dyadic_observation_sum_le}{dyadic
observation bound}; for infinite supports the observation mass is bounded,
\lword{Erdos257/WeightedSupportLimits.lean}{37}{dyadic_supportObservationMass_sum_le}{mass
bound}, summable,
\lword{Erdos257/WeightedSupportLimits.lean}{111}{summable_dyadic_supportObservationMass}{mass
summability}, and has a mean limit,
\lword{Erdos257/WeightedSupportLimits.lean}{81}{tendsto_dyadic_supportObservationMass_mean}{mean
limit}.  The radix close-return endgame is
\lword{Erdos257/BatchReturnSynthesis.lean}{151}{irrational_erdosSupportSeries_of_radix_closeReturn}{close-return
irrationality}.  The prime-part split and the diagonal scale producer remain
ordinary steps.

For every real $x\ge0$, with $Q_N$ the difference between $\lfloor 2^N x\rfloor$
and the Lambert prefix numerator of the actual greedy support, membership in
the Mersenne achievement set is equivalent to a nonincrease of $Q$ in every
window $[K,K+2\lfloor\sqrt K\rfloor+12)$,
\lword{Erdos257/GreedyRepairCriterion.lean}{192}{mem_iff_greedyBinaryDefect_sqrt_windows}{square-root
window criterion}, to cofinal nonincreases,
\lword{Erdos257/GreedyRepairCriterion.lean}{175}{mem_iff_greedyBinaryDefect_cofinal_repairs}{cofinal
repair criterion}, and to cofinal bounds by the next divisor load,
\lword{Erdos257/GreedyRepairCriterion.lean}{207}{mem_iff_greedyBinaryDefect_cofinal_load_bound}{load
bound criterion}, or by $N+1$,
\lword{Erdos257/GreedyRepairCriterion.lean}{222}{mem_iff_greedyBinaryDefect_cofinal_linear_bound}{linear
bound criterion}.  One strict-increase window excludes membership,
\lword{Erdos257/GreedyRepairCriterion.lean}{232}{not_mem_of_greedyBinaryDefect_strict_sqrt_window}{strict-window
exclusion}; the target $4/9$ is a specialisation.
