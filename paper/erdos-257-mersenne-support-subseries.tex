...
\renewcommand{\commit}{e5fd26a6d1b1a346430205eab5385b4c9565d004}
\renewcommand{\commitshort}{e02206e2685b}
\hypersetup{
...
At even depth $2R$, let $s_R$ be the terminal remainder of the descending
integer-greedy algorithm on the scaled quotient weights, and let
$D_R\subseteq\{2,\ldots,R\}$ be its Boolean support.  Write
$\mathcal F_{21}$ for the explicit fatal branch in which the real greedy
orbit has a fatal witness, only finitely many skipped exponents, eventual
agreement with the quotient-greedy rows, and eventual occupation of every
doubling block.
...
closed Boolean row is exactly the canonical quotient-greedy row, including
...
contradict the permanent affine-supercapacity regime.  Membership of $1/21$
therefore remains open at precisely that recurrence.
...
Equivalently, contradict its eventual permanent affine-supercapacity
recurrence or force an unbounded sequence of closed canonical quotient rows.
\end{problem}
...
\item \textbf{Problem~\ref{prob:one-over-twenty-one-membership}.}
  ...
  to excluding $\mathcal F_{21}$.  Equivalently, it remains to contradict the
  eventual permanent affine-supercapacity recurrence or to produce
  arbitrarily deep closed canonical rows.  Neither input is known.
\end{enumerate}
