...
At the half endpoint, every finite rational greedy remainder is strictly
positive, so a skipped rank cannot hide a prematurely exhausted expansion.
If that remainder is smaller than $\wgt c$, the skipped-core construction
fills the upper half by a Boolean word and produces an exact finite quotient
row at endpoint $2c-2$.  More sharply, cofinal positive skips occur if and
only if $1/2\in\Ach$.  The proposed producer is therefore not a weaker local
condition waiting for an estimate: it is an exact reformulation of the open
endpoint.  This route-identification theorem proves neither side.

The same greedy orbit has a sharper general dynamical form.  After rescaling
the remainder to $y_N(x)=2^N r_N(x)$, the achievement set is exactly the set
of nonnegative initial values whose orbit stays below the universal barrier
$2$ at every rank.  For every nonnegative rational $q$, membership is
equivalent to crossing the moving lower separatrix
\[
 2y_N(q)<\frac{2^{N+1}}{2^{N+1}-1}
\]
beyond every cutoff.  In particular this is the source-current canonical
$1/21$ frontier.  It does not prove that those crossings occur.
...
$1/2$ nor $1/21$: membership of either rational target is equivalent to
cofinal crossings of the exact scaled lower separatrix.  At $1/21$ the older
fatal-aligned-branch coordinate remains an equivalent negative classification,
and its eventual affine-supercapacity regime remains possible.  For
...
\begin{theorem}[the positive-skip route is exact]
...
Thus cofinal positive skips are not an independently weaker producer:
...
\subsection{The canonical scaled-greedy frontier at \texorpdfstring{$1/21$}{1/21}}
...
\begin{theorem}[scaled-greedy trap and rational lower separatrix]
...
\mword{Erdos249257/GreedyTrapDynamics.lean}{302}{mersenneAchievementSet_eq_scaledGreedyTrap}{kernel checked},
the general rational criterion is
\mword{Erdos249257/GreedyTrapDynamics.lean}{163}{rat_mem_mersenneAchievementSet_iff_scaledLowerBranchCofinally}{kernel checked},
