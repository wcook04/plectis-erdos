...
\renewcommand{\commitshort}{e5fd26a6d1b1}
\hypersetup{
...
Erd\H{o}s~\#257 asks whether every infinite-support value
$X_A=\sum_{a\in A}(2^a-1)^{-1}$ is irrational.  It remains open.  This note's
checked contribution is an exact structural record: hypothetical rational
values have an unbounded scaled-tail recurrence, sublogarithmic zero windows,
and Boolean--M\"obius reconstruction; every restriction to allowed exponents
$J$ preserves unique coding, compactness, and nowhere density, is perfect for
infinite $J$, and has measure $2^{-|F|}$ when $J^c=F$ is finite and zero when
$J^c$ is infinite.  For squarefree support the incidence is
$2^{\omega(n)}-1$; at every even base two divisibility-first certificate
hypotheses fail already at precision $b^2$, while adjoining $1$ preserves
irrationality and restores arbitrarily long base-$2$ opening blocks.  Finite
support represents neither $1/2$ nor $1/21$: the first target is equivalent to
unbounded greedy skips, and the second to excluding one explicit fatal aligned
branch whose eventual affine-supercapacity regime remains possible.
Tao--Ter\"av\"ainen's base-$2$ prime-support theorem and Duverney--Tachiya's
power-of-two squarefree theorem are cited prior work, not formalised here.
These classifications and scoped frontiers do not solve \#257 or either target.
\end{abstract}
