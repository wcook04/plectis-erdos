...
tempered integer carry orbit, that is, to an integer solution $u$ of the
long-division recurrence with $u(n)=o(2^n)$; for infinite $A$ that orbit is
unbounded; the zero windows of $\scA_A$, the runs of consecutive integers with
no divisor in $A$, then have length at most $\varepsilon\log_2 N+O(1)$
uniformly in $A$; a summable support has reciprocal mass $\sum_{a\in A}1/a$ at
least $1/\operatorname{ord}_v(2)$, where $v$ is the odd part of the
denominator; and rational values correspond exactly to Boolean--M\"obius carry
certificates, from which the support is reconstructed rather than guessed.
These are necessary conditions on a counterexample.  Jointly they are not yet
contradictory, and we do not present them as progress towards the universal
statement.

The squarefree support is not open at base~$2$: Duverney and Tachiya proved
irrationality there in 2019.  Its divisor incidence is $2^{\omega(n)}-1$, where
$\omega(n)$ is the number of distinct prime factors of $n$, and this is odd for
every $n\ge2$; the two block-certificate arguments that require first-block
divisibility therefore have no instance at any even base, and at $b=2$ they
fail on a value known to be irrational.  Adjoining $1$ to the support changes
the value by the rational
$1/(b-1)$ and the incidence by one, so the obstruction is a property of a
normalisation and not of the value.  Rationality is invariant under every
finite change of support, which is the general form of that observation.
\end{abstract}
...
\paragraph{Structure.}  Section~\ref{sec:period} settles the finite supports:
the multiplicative order of the base modulo their reduced denominators is
determined exactly.  Section~\ref{sec:forced} collects what
...
Section~\ref{sec:map} groups the supports already known to give irrational
values by the argument that reaches them.  Section~\ref{sec:squarefree} treats
...
the first by a strict lower bound.
...
Five instances at $b=2$, which also show what the hypothesis
$\lcm(F)\ge2$ is for:
\begin{center}
\begin{tabular}{@{}lllll@{}}
\toprule
...
\end{tabular}
\end{center}
The last two columns agree in every row, which is the order statement.  The
...
coefficients.  Call an integer sequence $u\colon\N\to\Z$ a \emph{tempered carry
orbit} for $f$ with multiplier $v$ if
\[
...
a tempered carry orbit exists for some $v\ge1$, and every such orbit is then
the scaled tail
...
bound forced on the tail by the carry recurrence with an upper bound for
...
\[
  A=\{n:(\mu*Q_U)(n)=1\},
\]
where $Q_U$ is the carry quotient of $U$, namely $Q_U(0)=0$ and
$Q_U(n)=\bigl(2U(n-1)-U(n)\bigr)/q$ for $n\ge1$.
\end{theorem}

\noindent Checked as
\mword{Erdos249257/BooleanMobiusCarry.lean}{948}{exists_normalized_support_fraction_iff_exists_booleanMobiusCarry}{Lean}.
The certificate asks for an orbit starting at $U(0)=p$, positive at every
index, obeying the carry recurrence, obeying a square-root growth bound, and
whose M\"obius quotient $\mu*Q_U$ takes only the values $0$ and $1$.  That
Boolean condition is exactly the statement that the quotient is the indicator
of a set, and M\"obius inversion then returns that set.  So the search for a
rational-valued Mersenne subseries is, without loss, a search for one integer
orbit with a divisibility-flavoured Boolean condition.

One boundary on the equivalence.  The support it produces is not required to be
infinite, and a finite support supplies a certificate for its own value, so a
certificate is not by itself a counterexample to Problem~\ref{res:problem}.
What the equivalence changes is the search space, not the difficulty.

\paragraph{The four conditions together.}
...
by a theorem of Duverney and Tachiya from 2019 recalled below; an earlier
version of this note listed it as open, having missed that reference.  What
this section establishes is that two of the block-certificate arguments used
here cannot reach that support at any even base, and that the reason lies in a
normalisation rather than in the value.
...
The infinite supports handled in this development are reached by
\emph{block-certificate} arguments, and it is worth saying plainly what one of
those is before showing that two of them cannot apply here.  Such an argument
fixes a base $b$ and asks, at every precision $q$, for a block of consecutive
coefficients beyond some point $N$ subject to: a divisibility condition on the
first $K$ coefficients of the block, an upper bound $C$ on the weighted sum of
the next $L-K$, a nonzero coefficient beyond the block, and the inequality
$q(C+N+L+2)<b^{L}$, which makes $b^{L}$ large compared with the precision and
the rest of the data.  Apart from the requirement that some coefficient beyond
the block be nonzero, each of these involves only finitely many terms of the
coefficient sequence, and the data $N$, $K$, $L$ and $C$ are supplied
separately at each precision.  Two of these arguments share the same first-block divisibility
condition, and the parity of Theorem~\ref{res:sfcount} refutes it outright.
...
and for a rational target the check is exact rational arithmetic, as for
$3/4\notin\Ach$
(\mword{Erdos249257/GreedyAchievementSet.lean}{1748}{three_fourths_not_mem_mersenneAchievementSet}{Lean}).

The coding, meaning the map sending a $0/1$ string $(\varepsilon_n)$ to
$\sum_n\varepsilon_n\wgt n$, which the strict tail inequality makes injective,
is also hereditary.  That matters because Problem~\ref{res:problem} quantifies
...
Theorem~\ref{res:half} classifies its membership exactly, in four equivalent
forms, with two dual forms for non-membership.
...
Of the forms listed above, only the greedy form and the fatal-gap form are used
below.  These are
equivalences and we mark them as such.  Proving statements equivalent decides
neither.  Their value is that they fix what a
...
\item \textbf{Problem~\ref{res:problem} itself}, for arbitrary infinite $A$.
  ...
  counterexample without excluding one.  Whether those constraints can be made
  jointly contradictory is itself open: an infinite support carrying an
  unbounded Boolean--M\"obius carry certificate, with sublogarithmic
  divisor-coverage gaps and satisfying the reciprocal-mass lower bound of
  Theorem~\ref{res:mass}, is not known to be impossible, and showing that it is
  would settle Problem~\ref{res:problem}.
...
\noindent Two questions listed here in an earlier version are withdrawn.  The
squarefree value at base~$2$ is settled by~\cite{duverneytachiya}
(Corollary~\ref{res:sfsettled}).  The reach of the analytic method
of~\cite{taoteravainen2025} is a question its own authors raise, and belongs to
them rather than to this note.  The squarefree value at bases $b\ge3$ is, as far as we know, genuinely
...
zero-window examples, the reciprocal-mass instance, and the greedy expansion of
$1/2$ through level~$21$ --- are direct computations, reported as computations
and not as theorems.  Results attributed to Duverney and Tachiya, to Tao and
