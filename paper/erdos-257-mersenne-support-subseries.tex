...
The strongest unconditional infinite-support consequences come before the
finite-support diagnostics.  For every integer base $b\ge2$, every infinite
pairwise-coprime support $A$ with $\sum_{a\in A}1/a<\infty$ has irrational
support series.  More generally, the checked nonnegative rational
coefficient theorem says that an eventually-periodic coefficient sequence,
positive somewhere in its periodic region, gives an irrational
\(\sum_{a\ge0}\gamma(a)/(b^a-1)\).  These are complete family theorems, not
the universal assertion for arbitrary infinite supports.  The pairwise-
coprime mechanism is an adaptive CRT certificate: small support elements are
paid for in the first block, while summable reciprocal mass controls the
unselected strays.  The periodic mechanism is a divisor-orbit sieve followed
by finite rational-prefix transfer.  The exact checked routes are
