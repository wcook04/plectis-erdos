...
\renewcommand{\commit}{c94b23bb94fd29186bece8aea70de195f81fd8ab}
\renewcommand{\commitshort}{c94b23bb94fd}
\hypersetup{
...
reciprocal mass.  Erd\H{o}s printed this theorem in 1968 for pairwise-coprime
...
quantifier is therefore his.  What is proved here is the case he withheld: the
same conclusion with no arithmetic hypothesis on the support at all.  No
identification with Erd\H{o}s's omitted argument and no theorem-priority claim
are made.
...
\textbf{Arithmetic.} Every infinite support of convergent reciprocal mass
gives an irrational value at every integer base $b\ge2$, with no coprimality,
periodicity or density hypothesis.  The full-support series and all nonzero
eventually periodic nonnegative rational weight sequences are irrational at
every integer base.  Finite subsums have exact denominator order.  \textbf{Geometry.} The achievement set is
...
\]
For fixed \(Q>0\), the Cesaro means of the complete shifted sums
\(T_{(k+1)Q}^{(2)}\) tend to \(\sum_{d\in A}M_Q(d)\).  Choosing \(P\), then a
sufficiently long finite average, and finally one term no larger than that
average proves
\[
...
arbitrary infinite $A$ with $\sum_{a\in A}a^{-1}<\infty$ & every $b\ge2$ &
  ...
  \mword{Erdos249257/AllBaseReciprocalSupportIrrationality.lean}{395}{irrational_erdosSupportSeries_of_summable_reciprocal}{checked here},
  with exact boundary in Section~\ref{sec:reciprocal-support} \\
\addlinespace
