...
open.  We prove that, for every integer \(b\ge2\), the sum
\(\sum_{a\in A}(b^a-1)^{-1}\) is irrational whenever \(A\) is infinite and
\(\sum_{a\in A}a^{-1}<\infty\).  No coprimality, periodicity, density, or
powerful-support hypothesis is used.  The proof transfers binary
LCM--Ces\`aro close returns to every radix and ends with an exact
integer-orbit contradiction.  Thus the unresolved part of Problem~\#257 is
the reciprocal-divergent case.  Novelty and priority are unassessed, and this
proof is not identified with Erd\H{o}s's omitted coprimality-free argument.
The development also retains the classical full-support theorem and proves
irrationality for every nonzero eventually periodic sequence of nonnegative
rational weights.
...
\textbf{Arithmetic.} Every infinite reciprocal-summable support gives an
irrational series at every integer base $b\ge2$; this contains the classical
pairwise-coprime theorem as a special case.  The full-support series and all
nonzero eventually periodic nonnegative rational weight sequences are also
irrational.  Finite subsums
...
$1/2$ and $1/21$ remain undecided; any universal counterexample must have
divergent reciprocal mass, and the geometric classification does not decide
the arithmetic nature of individual subsums.
\end{paperresult}
