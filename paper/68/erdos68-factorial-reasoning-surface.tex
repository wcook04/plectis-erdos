...
$J=\{n:2\le n\le M,\ v_p(d_n)=e\}$ and write $d_n=p^eu_n$ on $J$.
Specialising the reciprocal-sum valuation formula of Louwsma and Martino gives
\[
...
cancel the primes $139$ and $2593$ completely.  The common denominator before reduction satisfies the quantitative bound
\[
...
statements concern opposite sides of reduction, and the cancellations at $139$ and
...
$300000$, and $q\ge2^{39990}>10^{12038}$ from an exact continued-fraction
enclosure computed outside Lean. The
...
\paragraph{Reading route and dependencies.}
For the main mechanism, read the survival criterion in
\S\ref{long68:sec:prime-powers}, the lcm argument in
\S\ref{long68:sec:lcm}, and the comparison in
\S\ref{long68:sec:residue}.  For the strongest finite conclusions go directly
to \S\ref{long68:sec:finite}.  The channel lattice in
\S\ref{long68:sec:channels} is an independent explanation of a limitation,
not an input to the finite computations.  The research targets in
\S\ref{long68:sec:open} are explicitly conditional.  The appendices retain
longer deductions; source and execution status are recorded separately.
...
\cite[p.~102]{erdos1988}.  That is conjectural context.  Numbering follows
\href{https://www.erdosproblems.com/68}{Bloom's Erd\H{o}s problem catalogue}
\cite{bloom}.  The catalogue reference retains the manuscript's historical
access date; the current page could not be rechecked during this revision.
No irrationality proof is supplied here.
...
answer is a specialisation of the reciprocal-sum valuation formula of Louwsma
and Martino \cite[Lemma~4.1, p.~10]{louwsma-martino}; the proof below writes
it out for the denominators $d_n$.

The general valuation mechanism is inherited.  The explicit factorial
specialisation, its cancellation witnesses, and its role in the scale
comparison below are the contributions examined here; no priority claim
for a new general reciprocal-sum valuation theorem is intended.
...
What is specific here is the behaviour of the actual family $d_n=n!-1$, and
that behaviour is not generic.
...
which occurs at neither prime among the tested indices $2\le n\le p-1$.  That enumeration is a finite calculation
...
are exactly one.  Both examples concern complete cancellation because
the maximal exponent is one.

A prime is \emph{prefix-private at $m$} when it divides $d_m$ and divides
none of $d_2,\ldots,d_{m-1}$.

The word private is relative to that prefix, not to all future factorial
gaps.  Later hits can destroy uniqueness in a larger block.
...
identity $(q-1-n)!\,n!\equiv(-1)^{n+1}\pmod q$ gives the reflected hit
$q-n-1$, which differs from $n$ when $q\ne2n+1$ and is at least $2$ when
$n\le q-3$.  The reflection identity is classical; see Stewart
...
Its formal counterpart is recorded in the source ledger; the status of that
...
a uniform $O(N^{2/3})$ bound for a fixed nonzero factorial fibre in an
interval of length $N$ below a prime modulus.  The weaker lcm deduction
from it is retained in \S\ref{long68:sec:ext-superseded}, not used here.
The revision makes no exhaustive priority claim for the lcm estimate;
...
Fix $P\in\Z[X]\setminus\{0\}$.  By Lemma~3 of Luca and
...
all $n\ge n_0$ and $h\ge1$.  The proof of Theorem~\ref{long68:res:lcm-growth}
then gives
\[
 \liminf_{N\to\infty}
...
$\den(H_N)$.  Consecutive denominators $(N-1)!-1$ and $N!-1$ are already
coprime for $N\ge3$, so the qualitative failure of common-denominator
clearing follows from two terms; the estimate is the quantitative growth
statement.  Theorem~\ref{long68:res:prime-pole} and the cancellations at $139$ and
$2593$ show why the substitution fails: a prime may occupy the common
denominator and be absent from the reduced one.  One cancelled prime is not
an asymptotic cancellation theorem.

For completeness, the consecutive coprimality follows because their gcd
divides $N-1$, whereas $(N-1)!-1$ is coprime to $N-1$.  Thus the two-term
argument is qualitative and does not depend on the asymptotic constant.
...
$H_N$.  The prime-pole analysis describes which factors survive reduction,
...
\end{equation}
The proof of the next theorem feeds this estimate into the factorial-tail
integrality argument that Han\v{c}l and Tijdeman use for factorial series
with integer coefficients \cite[Lemma~2.1 and the following remark,
p.~385]{hancl-tijdeman}.
...
$x$ is rational exactly when they vanish from some index on.  The criterion
...
For $t\ne0$ this paper supplies no cofinal estimate for the displayed orbit.
...
In this section $p$ is a natural parameter, not necessarily a prime.
Prime divisors used in the valuation argument are separate variables.
The distinction matters when the parameter is tailored to a first hit.
...
\noindent This natural-parameter form is also the
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos68/PaperCompleteExisting.lean\#L156-L168}{source form of the global complementary criterion}.

\begin{proof}
Suppose $\Ser=a/q$ with $q\ge1$, and take such a parameter with
$p>\max(q,3)$
...
\]
It is a joint budget for collision loss and the real gap.  The bound
$\rho_p\ge1$ gives only $\eta_p\ge1/R_p$ and may be far too small.
...
\noindent This is a direct consequence of the constant quotient; no valuation
estimate enters.  A nonzero moment therefore cannot be hidden entirely below $2d$
...
Fix $D\ge2$.  The low-channel equations, the support equation and the
residual identity answer different questions: solvability, admissibility
on $n\ge2$, and the remaining fractional obstruction, respectively.

The integral divisor basis makes the preceding divisibility statement exact.
For the coordinate calculation only, enlarge the coefficient space to
finitely supported integer vectors on $n\ge1$, with $M$ and $\chn d{\cdot}$
given by the same formulas; this introduces no term $1/(1!-1)$ into $\Ser$.
Set
\[
...
\]
The quotient $\lfloor i/d\rfloor$ changes between $i=n-1$ and $i=n$ exactly
when $d\mid n$, so $\chn d{T_n}=(d!-1)W_{d,n}\mathbf 1_{d\mid n}$, and
induction gives $M(U_n)=0$ and $\chn d{U_n}=(d!-1)\mathbf 1_{d=n}$.  Hence
$M(K_D)=\Lc_D$ and $\chn d{K_D}=0$ for $2\le d\le D$.  The change from
$e_1,T_2,T_3,\ldots$ to $e_1,U_2,U_3,\ldots$ is integral triangular with unit
diagonal, so $e_1,U_2,U_3,\ldots$ is an integral basis, and applying $M$ and
each $\chn d{\cdot}$ gives the unique expansion
\[
 c=M(c)\,e_1+\sum_{d\ge2}\frac{\chn dc-M(c)}{d!-1}\,U_d .
\]
Consequently every vector of the enlarged space whose channels $2,\ldots,D$
vanish has a unique expression
\begin{equation}\label{long68:eq:low-channel-basis}
 c=tK_D+\sum_{n>D}z_nU_n,
 \qquad M(c)=tL_D,
\end{equation}
with finitely many nonzero integers $z_n$.  Write $a_D=[e_1]K_D$ and
$u_n=[e_1]U_n$.  Requiring support on $n\ge2$ adds precisely the scalar
equation
\begin{equation}\label{long68:eq:low-channel-support}
 ta_D+\sum_{n>D}z_nu_n=0.
\end{equation}
The scalar coefficients satisfy $u_2=2$ and
$u_n=-\sum_{d\mid n,\ 2\le d<n}W_{d,n}u_d$ for $n>2$; in particular
$u_{2p}=-(2p)!/2^{p-1}$ for every prime $p$.  If $p$ is a prime with
...
and a divisor $d\le D$ of $n$ one has $n/d\ge2p$, and $(n/d)!$ divides
$W_{d,n}$, the quotient counting partitions of $n$ points into $n/d$
unordered blocks of size $d$; thus $g\mid W_{d,n}$.  Strong induction in the
...
determination as its Theorem~3.2.
...
carry the asymptotic deduction.  Lean checks the corollary as
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos68/PaperCompleteLiminf.lean#L54}{asymptotic radius constant liminf}.
...
Neither condition implies the other.  A prime between $299999$ and $599998$
...
\path{044ef27a60f7294c39b8279b7f04e22e1234a942e2862e9e8c64eb66505af29b},
recording the parameters above, the digests of the driver and backend used on
its own run, the event-trace digest and the final enclosure.  The source packet names the driver
\path{scripts/check_erdos68_strict_successor.py} and backend
\path{scripts/check_erdos68_strict_successor_gmp.cpp}.  This revision records a fresh full-range carry replay.  The published
driver digest is \path{f25b5bd8ffbfd66bdbb42ca6a07a936aab9b4eb7d3a57446725be55acb642191} and the backend digest is
\path{43308ddc3902dd537b789ae6e0648054a0d752adf07e5d6e1d829811e9342083}; the payload digest is \path{044ef27a60f7294c39b8279b7f04e22e1234a942e2862e9e8c64eb66505af29b}.  The event-trace
and unit-carry certificate match the previously retained receipt.  The inherited independent replay at
...
The first exclusion can also be written $\kappa(q)\ge300000$, where
$\kappa(q)=\min\{r\ge1:q\mid r!\}$ is the factorial index used by Sondow
\cite[\S3, Theorem~1]{sondow2006}.  This is not a smoothness exclusion and
uses no approximation bound for $e$.
...
makes the tail from $N$ smaller than $2/(N!-1)$, so $\ell/D<\Ser<u/D$.
...
eventual unit-carry tail.  The continued-fraction identities are classical
...
\eqref{long68:eq:lower-escape}.

All sufficient conditions below are to be proved for the actual
factorial sequence.  A model obeying selected congruences, or a successful
bounded computation, is not a substitute for that requirement.  The routes below are sufficient conditions for it,
and their quantitative hypotheses are unproved.  None of them is an
equivalent reformulation of the problem.

\paragraph{A recent framework, and a missing hypothesis.}
Garaev, Luca and Shparlinski's harmonic-sum estimate
\cite[Theorem~1 and (5)]{garaev-luca-shparlinski-harmonic} concerns
unconditioned harmonic sums, not the fibre on which $n!\equiv1\pmod q$.
...
A large value set alone does not upper-bound a prescribed fibre.
Hu's September preprint \cite[Theorem~3.1]{hu-september2026} gives
$|A|\gg\min\{M,p\}^{8/15}\mu^{-4/15}$ for distinct reciprocal-affine maps
with at least $M$ internal transitions, at most $|A|$ parameters, and
nonidentity quotient multiplicity at most $\mu$.  Its incidence input is
Stevens and de Zeeuw \cite[Theorem~4]{stevens-dezeeuw2017}.
A proposed transfer to a factorial-conditioned harmonic fibre must first
construct such internal transitions and bound their multiplicity; neither
is supplied by an unconditioned value-set estimate.
...
subsequences would not prove the combined criterion.  For a prime $r$ absent from the base $F_p$, the contribution
...
\paragraph{Two narrower exploratory reductions.}
For $F_h(X)=\prod_{j=1}^h(X+j)$, joint factorial--harmonic hits a distance
$h$ apart require $F_h(x)=1$ and $F'_h(x)=0$.  An exact calculation gives
\[
 \gcd(F_{12}-1,F'_{12})
 =(X-4626)(X-7848)\quad\hbox{in }\mathbb F_{12487}[X].
\]
Thus the earlier small-prime observation of at most one critical root is
not a uniform theorem.  These two roots have factorial values $442$ and
$6300$, not $1$, so the unconditioned critical-root count is genuinely a
relaxation of the desired fibre count.  The derivation, compressed even-gap
polynomial and exact replay are in the accompanying research note.

There is also a useful distinction for the gap products in Stewart's
method \cite[Lemma~2, (21)--(24)]{stewart2004}.  Products from adjacent
gaps $i<j<k$ are unequal: equality $j!/i!=k!/j!$ would make $k!/i!$ a
square, contrary to the classical non-power theorem
\cite[Theorem~1]{erdos-selfridge1975}.  Selecting two short gaps with nearly
equal lengths does not preserve adjacency.  The wider equal-product
problem has a separate history \cite{macleod-barrodale1970}; the
doubled-length theorem discussed by Nguyen Xuan Tho
\cite[Theorem~1]{nguyen-tho2026} is another special case, not arbitrary
near-equal-length nonvanishing.  No stronger lcm exponent is inferred from
...
The two branches are disjunctive alternatives for divisibility.  To force
its failure one must rule out both at each chosen prime, not rule out one
branch on each of two different subsequences.
...
Duverney's Theorem~3.1 assumes the quadratic growth condition (1.3),
$cu_n^2\le u_{n+1}\le c'u_n^2$, whereas $u_{n+1}/u_n^2\to0$ for
$u_n=n!-1$; the summability condition (3.6) of his Corollary~3.2,
$\sum_n|u_{n+1}/u_n^2-1|<\infty$, also fails, since its summands tend to one
here \cite[pp.~275, 285--287]{duverney}.  For a single denominator sequence
...
target these papers identify: a low-height clearing subsequence, or enough
exact cancellation in the least common multiple.
...
The remaining task is a cofinal estimate, not a larger finite census.
...
and Lean checks them as
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos68/PaperCompleteLiminf.lean#L42}{common denominator growth liminf} and
\href{https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos68/PaperCompleteLiminf.lean#L54}{asymptotic radius constant liminf}.  The finite-block inequality is the separate Lean source
\href{https://github.com/wcook04/plectis-erdos/blob/d788dd4b8c59f2246000f2ed98fffb8a5e8ac72e/lean/ErdosProblems/Erdos68/ChannelIntegralCongruence.lean#L524-L529}{finite terminal-block inequality}.  The two prefix cancellations, the index-$52$
...
method ceiling ordinary in this record
(\nolinkurl{sharp_radius_satisfies_square_log_constraint} is not in this checkout)\\
...
criteria for Cantor series \cite[Ch.~II, \S2.1]{galambos1976}.  The additions
here are the telescope $C=\Ser-e+2$, the identification of the digit value
$m-2$, and the transport to the shifted family.  The tail-integrality step is
the argument of Han\v{c}l and Tijdeman \cite[Lemma~2.1 and the following
remark, p.~385]{hancl-tijdeman}.  The multiplicity bound is Garaev,
...
those of \cite[\S1.12(ii)]{nist-dlmf}.  No antecedent was located for the
Wilson cofinality packaging, the finite radius bound or
Theorem~\ref{long68:res:lcm-growth}.
...
reported interval data contain canonical zero digits at $m=5$ and $m=23$,
while the zero-branch list is empty through $m=100000$.
...
that equality from the integrality condition $np_ns_n\in\mathbb N_0$ and the
...
two denominators already obstruct the tail bound:
\[
...
\cite[(1.3), p.~275; Thm.~3.1, pp.~285--286]{duverney}; his Corollary~3.2,
which allows signs $a_n\in\{-1,1\}$, requires the summability condition (3.6),
$\sum_n|u_{n+1}/u_n^2-1|<\infty$, whereas those summands tend to one
\cite[Cor.~3.2, p.~287]{duverney}.  Both the ratio limit
and the nonsummability are kernel-checked.  The ratio proof divides numerator
and denominator by $(n!)^2$; the resulting terms tend to zero while
$(1-1/n!)^2$ tends to one.

For a single denominator sequence, Erd\H{o}s proved that $\sum_k1/n_k$ is
irrational when $\limsup_kn_k^{1/2^k}=\infty$ and $n_k>k^{1+\varepsilon}$
eventually \cite[Theorem~1, p.~1]{erdos1975}.  Barreto, Kang, Kim, Kova\v{c}
and Zhang treat products of consecutive denominators and weighted extensions
\cite[Thms.~2--3 and Rem.~4, pp.~2--5]{barreto-et-al}.  For $a_n=n!-1$ the
root-growth expressions tend to one, $(n!-1)^{1/2^n}\to1$, so neither the
limsup hypothesis of Erd\H{o}s nor the growth hypotheses of their Theorems~2
and~3 hold; the limit is kernel-checked through the bound
$0\le\log(n!-1)\le n^2$.  Their proof of Theorem~3 uses the classical
...
individual number $N!-1$ does not occur.  The criterion of Oppenheim that they
reproduce \cite[Lem.~2.2, p.~385]{hancl-tijdeman} states that, for
coefficients $c_n$ with $|c_n|<an+b$ eventually and $\liminf_n|c_n|/n=0$, the
sum is rational exactly when $c_n=0$ eventually.  For the carry defects
$c_m=1-b_m$ the first condition holds, since $-1\le b_m\le m-1$, and the
second is not established; the needed cofinal equivalence is
Theorem~\ref{long68:res:carry-equivalence}, which is proved directly.
...
theorem.  The deduction is retained because it is the only place a
...
Write $D(S)=\lcm_{i<j,\ i,j\in S}\gcd(d_i,d_j)$, with $D(S)=1$
...
$D(S)\lcm_{i\in S}d_i\mid\prod_{i\in S}d_i$ prime by prime.

The collision core has an exact incremental law. For the positive
factorial-gap denominators, adjoining a new index $a\notin S$ gives
\[
 D(S\cup\{a\})=\lcm\!\Bigl(D(S),\ \gcd\bigl(d_a,\lcm_{j\in S}d_j\bigr)\Bigr),
\]
...
The base cancellation is exact prime by prime. With $F_p=(p-1)!$,
...
\]
For $e>0$, $r^e\mid\widetilde C_p$ exactly when the pairwise core carries
...
The sharp surviving valuation cap
...
and transferred to $n!-1$ in the text \cite[p.~464]{stewart2004}.  The source
controls $q$ relative to the later
...
channels $2$ through $D$ vanish, the factorial moment is $\Lc_D$, and the
coefficient content is one.  They lie outside the space of vectors supported
...
keeps the coordinate $a_9$ and has content one.  By
...
channels $2$ and $3$, has moment $600$, and satisfies
\[
...
zero-branch event occurs at any $m\le100000$; its executable and source
digest are not available, so that classification remains external finite
evidence.  The strict-successor census through $300000$ used in the short
note is a fresh exact GMP replay in this revision; \S\ref{long68:sec:finite}
records the published driver, backend and payload digests and describes the
separate replay through $4000$.  The full range is external
computational evidence outside the Lean development.
...
\item For the genus-zero product $E(z)=\prod_{n\ge2}(1-z/n!)$, local uniform
  convergence and logarithmic differentiation give $-E'(1)/E(1)=\Ser$.  The
  natural termwise clearing factor $Q_N=\prod_{2\le n\le N}(n!-1)$ satisfies
  $Q_N(\Ser-H_N)\to\infty$, so this clearing choice yields no small positive
  integer linear form.  The calculation concerns this clearing factor alone;
  Hermite--Pad\'e systems with other denominators are left open.
...
The cofinal statement in the short note is a case of the following immediate
consequence of Cantor's termination criterion.  For a real
