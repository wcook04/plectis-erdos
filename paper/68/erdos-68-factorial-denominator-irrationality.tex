...
For an integer-coefficient factorial series, the standard tail-integrality
argument appears in Han\v{c}l and Tijdeman~\cite[Lemma~2.1 and its
remark]{hancl-tijdeman}: a rational sum has integral normalized tails
once its denominator divides the preceding factorial.  Their subsequent
finite-difference criteria impose regularity on the coefficients.  In the
present series the denominators are $n!-1$; the companion expansion and
...
\]
The same subtraction bounds a common divisor in the proof of Lemma~5 of
Luca and Shparlinski~\cite{luca-shparlinski}, and a common prime-power
divisor at display~(2.5) of Lai~\cite{lai}.  For $n_0\le i<j\le N$ the
...
power of that prime survives reduction.
...
\bibitem{lai-lupu-sprang}
  ...
  Numbered as in arXiv v1: Lemma~2.1, p.~3; \S5, pp.~10--14; Lemma~7.3, p.~20;
  (8.1), p.~22.
\end{thebibliography}
