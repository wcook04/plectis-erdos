...
the no-wrap case.  For a factorial series $\sum b_n/n!$ with integer
coefficients, a rational sum has integral normalised tails once its
denominator divides the preceding factorial
\cite[Lemma~2.1 and the following remark, p.~385]{hancl-tijdeman}, and
Han\v{c}l and Tijdeman classify the rational sums with polynomial
coefficients \cite[Theorem~3.1 and Corollary~3.1, pp.~390--391]{hancl-tijdeman}.
In the present series the denominators are $n!-1$; the companion expansion and
the two wrap terms in \eqref{eq:companion-wrap} account for that change.
...
\end{equation}
The LCM asymptotic is proved here by an ordinary mathematical argument.
The asymptotic and liminf Lean files of \texttt{wcook04/plectis-erdos} linked
later in this note are proof candidates and are not being cited as kernel
verification.  The separate public Lean release
...
by $j!/i!$ and subtracting $j!-1$.  It is the case $P=-1$ of the subtraction
for $n!+P(n)$ that Luca and Shparlinski use in the proof of their
Lemma~5~\cite{luca-shparlinski} and that Lai uses for a common prime-power
divisor \cite[proof of Lemma~2.4, display~(2.5)]{lai}.
...
These are common-denominator estimates.  Consecutive denominators
$(N-1)!-1$ and $N!-1$ are already coprime for $N\ge3$, so the qualitative
failure of common-denominator clearing follows from two terms; the estimate
above is the quantitative growth statement.  In particular,
...
The range through $300000$ rests on the retained GMP receipt and lies outside
the Lean development.  The long record gives the receipt's digests and a
separate replay through $4000$; a byte-identical regeneration of the
full-range receipt with the current driver is outstanding.
...
terminal-block inequality cited above.  The associated
\href{https://github.com/wcook04/plectis-erdos/blob/d788dd4b8c59f2246000f2ed98fffb8a5e8ac72e/lean/ErdosProblems/Erdos68/PaperCompleteAsymptotics.lean}{asymptotic}
and
\href{https://github.com/wcook04/plectis-erdos/blob/d788dd4b8c59f2246000f2ed98fffb8a5e8ac72e/lean/ErdosProblems/Erdos68/PaperCompleteLiminf.lean}{liminf}
Lean files are proof candidates and are not being cited as kernel verification.
...
\bibitem{koepf-schmersau} W.~Koepf and D.~Schmersau,
  \emph{Irrationality of certain infinite series II}, Analysis \textbf{31}
  (2011), 117--124,
  doi:\href{https://doi.org/10.1524/anly.2011.1094}{10.1524/anly.2011.1094}.
  Example~3.2, p.~121.
\bibitem{hancl-tijdeman} J.~Han\v{c}l and R.~Tijdeman,
  \emph{On the irrationality of factorial series}, Acta Arith.
  \textbf{118} (2005), no.~4, 383--401,
...
\bibitem{luca-shparlinski}
  F.~Luca and I.~E. Shparlinski, \emph{Prime divisors of shifted factorials},
