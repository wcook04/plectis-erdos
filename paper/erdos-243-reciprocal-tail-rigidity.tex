...
\hypersetup{
  pdftitle={Weighted LCM records and reciprocal-tail rigidity for Erdős Problem 243},
...
\runninghead{Erd\H{o}s \#243: weighted LCM records}
\title{Weighted LCM Records and Reciprocal-Tail Rigidity}
...
\begin{abstract}
For a strictly increasing positive integer sequence with rational reciprocal
sum and $a_{n+1}/a_n^2\to1$, eventual Sylvester recursion is equivalent to
finite weighted excess at the records of its LCM-cleared tail numerator.
The weight may be any finite nonnegative decreasing function with divergent
integral, and a fixed baseline may be subtracted from each record jump.
Pairwise coprime record digits create a progression of forbidden heights;
each first crossing charges its weight to the excess jump. The divergent
integral then rules out every nonterminal orbit with finite record budget.
The criterion includes the bounded-negative-part and bounded-defect
consequences. Its summability condition remains unproved for the
unrestricted Erd\H{o}s problem.
\end{abstract}

\section{Introduction}\label{sec:problem}

\begin{theorem}[weighted record excess]\label{res:weightedrecord}
Let $(a_n)_{n\ge0}$ be strictly increasing positive integers with
$a_{n+1}/a_n^2\to1$ and $\sum_{n\ge0}a_n^{-1}=p/q$, where $p\in\Z$
and $q\in\N$ is positive. Define
\[
...
$\int_1^\infty f(t)\,dt=\infty$. Then $a_{n+1}=a_n^2-a_n+1$
eventually if and only if there exists an integer $B\ge0$ such that
\[
 \sum_{n\in\mathcal R}(-V_n-B)_+f(U_n)<\infty.
\]
\end{theorem}

Here $U_n$ is a positive integer and $V_n$ is an integer, since $L_n$
clears the rational tail. No coprimality of $p,q$ is assumed.
The proof in Section~\ref{sec:lcmrecords} charges first crossings of a
Chinese-remainder progression to record excess. It uses the actual LCM
orbit; no additional centring or growth hypothesis is imposed.
The full statement is \href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/ErdosProblems/Erdos243/PaperCompleteR8/CanonicalWeightedRecords.lean#L268}{kernel-checked in Lean}.


The next corollary retains a simpler, stronger sufficient hypothesis in the
...
\noindent Lean checks the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/ProductDefect.lean#L211}{eventual quadratic recurrence}.
...
\noindent Lean checks both \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Arithmetic.lean#L21}{exact error identities}.
...
\noindent Lean checks \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Arithmetic.lean#L120}{absorption and descent}.
...
\noindent Lean checks the local and eventual forms of \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Arithmetic.lean#L138}{error vanishing}.
...
\noindent Lean checks all three \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Reduction.lean#L16}{persistent coprimality conclusions}.
...
\noindent Lean checks \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Reduction.lean#L103}{gcd stabilisation and exact-tail reduction}.
...
This removes the normalised-vanishing premise from the
\lword{Erdos243/SparseResetRecovery.lean}{510}{sylvesterNext_eventually_of_summable_negativeRelativeMass}{earlier exact-orbit criterion}.
...
with divergent integral. This section proves the leading equivalence.
...
We now prove Theorem~\ref{res:weightedrecord}.
...
next-index LCM expression \cite[Theorem~3, p.~132]{erdosstraus1964}. Here any finite upper
...
\end{problem}
\noindent Lean records the \href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos243/PaperCompleteR7/Frontier.lean#L151}{necessary profile for a counterexample}.
...
For the constant-error orbit $C_n=n+1$, $E_n=-1$, the defining
identity $E_n=D_n-(a_n-1)C_n$ reads $D_n+1=(a_n-1)(n+1)$. Each multiplier is
...
predicate}.  For example, the forced orbit from $a=3$ begins as follows: $\fnum(0,3)=6$ is divisible by $2$ and gives $a_1=3$, while
...
evidence and not a proof; the exact constant-negative exclusion is
\lword{Erdos243/ReciprocalTailRigidity.lean}{286}{no_constantNegative_orbit}{proved for every seed and at every scale}.  The
