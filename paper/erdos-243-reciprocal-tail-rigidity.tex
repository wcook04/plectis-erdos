...
% Problem-specific source pin: #243's finite-negative-mass results advance
% independently of the corpus default.
\renewcommand{\commit}{64f33f3a134d3414c18717d37ab8201e1dd1c98a}
\renewcommand{\commitshort}{64f33f3a134d}
\hypersetup{
...
conditional signed form of the
problem itself: if
$\sum_{n\ge0}\left|a_{n+1}/a_n^{2}-1\right|$ converges, then
the reciprocal sum, even with numerators in $\{-1,1\}$, is rational if and
only if the corresponding signed recurrence holds for all large
$n$~\cite[Corollary~3.2, p.~287]{duverney2001}.  The all-positive
specialisation is the form relevant here.  Neither result is formalised here,
and the results below do not recover either of them.
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{2092}{tailGcd_strictGrowthCount_sublinear_of_normalizedVanishes}{sublinear
strict-growth theorem}; the second is the
\lword{Erdos243/ReciprocalTailRigidity.lean}{2108}{tailGcd_exists_arbitrarilyLate_constBlock_of_normalizedVanishes}{arbitrarily
...
\]
The checked
\lword{Erdos243/ReciprocalTailRigidity.lean}{2076}{tailState_subexponential_of_normalizedVanishes}{tail-height estimate}
...
numerators and absolute convergence hypothesis, and then recover the
