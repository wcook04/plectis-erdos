...
hypothesis, finite normalised negative mass is also impossible.  The
source-current recovery interface makes this a separate, endpoint-near
criterion: under the exact $C/D$ dynamics, positivity, the strict centred-step
equation, division-free normalised vanishing, and summability of
$\sum_n(-E_n)_+/C_n$, the centred state vanishes eventually and the Sylvester
...
The summability hypothesis is not supplied for the canonical Erd\H{o}s orbit,
so this remains conditional and does not prove reciprocal-tail irrationality.

For dynamically reduced recovery intervals we also prove an exact
repair-entropy inequality.  If a family of old moduli is deleted during a
length-$L$ recovery, then the square of its transversal least common multiple
is charged against the same finite product that controls the recovery:
\[
 K^L\lcm(m_q:q\in R)^2<(K+1)^L.
\]
Thus independently repaired moduli have a quantitative cardinality bound, and
normalised vanishing forces every sufficiently late recovery of a fixed length
to be cancellation-free.  This does not exclude recoveries whose lengths tend
to infinity; that escape is the remaining obstruction.
...
magnitudes, and conditionally excludes both the cofinally bounded-negative
branch and the finite normalised-negative-mass branch.}{The two recovery
criteria retain their respective positivity, dynamics, centring,
normalised-vanishing, boundedness, and summability hypotheses.  The mixed-sign
and unbounded-negative regimes keep Problem~\#243 open.}{Comparator selects the
eventually periodic negative-orbit obstruction, the conditional exclusion of a
cofinally bounded negative state, and the exact finite-negative-mass recovery
interface
\lword{Erdos243/SparseResetRecovery.lean}{175}
      {sylvesterNext_eventually_of_summable_negativeRelativeMass}
      {summable negative-mass recovery}.
The latter is a conditional state-system implication, not a prime-specific
producer or an unconditional \#243 endpoint; the missing summability of the
canonical orbit remains explicit.}
...
The source-current \texttt{negative\_mass\_recovery} row adds the distinct
criterion obtained from summability of $\sum_n(-E_n)_+/C_n$, with exact
declarations
\texttt{eventually\_zero\_of\_summable\_negativeRelativeMass} and
\texttt{sylvesterNext\_eventually\_of\_summable\_negativeRelativeMass}.
It assumes the displayed dynamics, positivity, strict centred step,
normalised vanishing, and summability; it supplies no canonical-orbit
summability or prime-specific producer.  These are conditional exclusions
only; mixed-sign and unbounded-negative regimes remain outside their
conclusions.
