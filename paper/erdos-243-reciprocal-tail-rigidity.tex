...
\hypersetup{
  pdftitle={An exact record-excess criterion for Erdős Problem 243},
...
\runninghead{Erd\H{o}s \#243: an exact record-excess criterion}
\title{An Exact Record-Excess Criterion}
\subtitle{Rigidity criteria for Erd\H{o}s Problem~\#243}
\author{Will Cook}
\date{July 2026}
...
eventually.  The leading result of this note is an exact criterion carrying
those hypotheses and no others.  Write $(C_n,D_n,E_n)$ for the integer state
...
That criterion is an ordinary mathematical proof.  Its finite first-crossing
...
place of their $\limsup_nQ_n\le0$.  One is an unconditional irrationality
criterion at the cubic rate $a_n^2/a_{n+1}=1+3/n+o(n^{-3})$.  The last two
...
the criterion is an exact reformulation of the problem rather than a solution
of it.
...
\smallskip
\textbf{Strongest exclusion of an unconditional kind.}
At the cubic rate $a_n^2/a_{n+1}=1+3/n+o(n^{-3})$ the reciprocal sum is
irrational (Theorem~\ref{res:cubicrate}).  This assumes no rationality and no
state hypothesis, and it lies outside the regime of
Problem~\ref{res:problem} only through its rate.
...
statements should read Sections~\ref{sec:lcmrecords} and~\ref{sec:records}, and
...
\subsection{An unconditional irrationality criterion}
...
hypothesis.  Its rate is faster than the regime of Problem~\ref{res:problem}
allows to be decided by the earlier sections, and it is the only unconditional
irrationality statement in this note.
