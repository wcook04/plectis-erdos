...
\renewcommand{\commit}{ab66e4f78724fe10bfe246e50353bc1d6043c189}
\renewcommand{\commitshort}{ab66e4f78724}
\hypersetup{
...
denominator-clearing error $E_n$.  Problem~\#243 remains open: our results do
not classify mixed-sign errors or exclude the remaining unbounded-negative
regime.

We formalise in Lean three increasingly general exclusions of a negative $E$.
It cannot be constantly $-m$, at any magnitude and any scale:
...
periodic in the regime $|E_n|<a_n$ with positive drift, by the same pigeonhole
after a strong induction that removes common scale.  And, under eventual strict
centring and normalised vanishing, it cannot have a bounded negative part:
...
and coprime to every later numerator, which a Chinese-remainder block of
consecutive multiples forbids.  That last step is a statement about integer
sequences alone: a sequence tending to infinity with upward steps bounded by
$B$ cannot remain coprime to a fresh pairwise-coprime modulus at every stage.
Together with an absorption lemma making
...
The stated eventual strict-centring hypothesis is redundant, since normalised
vanishing yields it after a finite shift.  Under the same normalised-vanishing
hypothesis, finite normalised negative mass is also impossible.

The one-sided boundary is unconditional and simpler: when the centred error is
natural-valued, the update law makes the tail state nonincreasing, so it
stabilises and forces $E_n=0$ eventually.  The kernel route is
\lword{Erdos243/ReciprocalTailRigidity.lean}{1843}{centeredState_eventually_zero}{nonnegative-error descent},
followed by
\lword{Erdos243/ReciprocalTailRigidity.lean}{1805}{sylvesterNext_eventually_of_centered_zero}{eventual Sylvester recurrence}.
This is the exact easy branch; the unresolved problem begins when negative
excursions are allowed.

There is also a weaker conclusion from normalised vanishing alone.  It makes
strict increases of $\gcd(C_n,D_n)$ sublinear in number and forces constant-gcd
blocks of every prescribed finite length arbitrarily far out.  It does not
make the gcd eventually constant; that stronger conclusion uses cofinally
bounded negative magnitudes.

At the local arithmetic level, we also isolate the exact mechanism that a
reduced two-step orbit must obey.  The cancellation factors survive in a
non-autonomous second-order recurrence, and a separate feedback identity
shows exactly when a square-residue calculation is realised by the next
multiplier.  These are structural interfaces, not a termination theorem.

Problem~\#243 is open and this note does not close it.  The last theorem is
conditional: its centring and normalised-vanishing hypotheses are analytic
inputs that we assume rather than derive.  For the canonical orbit attached to
a sequence in Erd\H{o}s's problem, Koizumi's results provide normalised
vanishing and a stronger centring range, so the bounded-negative theorem
applies conditionally, with boundedness of the negative part as the missing
hypothesis.  What the results remove is the bounded case and, more
...
zero.  Any surviving canonical orbit must therefore have negative excursions
unbounded along a cofinal set and divergent normalised negative mass.
\end{abstract}

\externalreviewobject{The paper develops the exact centred-state recurrence,
proves zero-state absorption, excludes constant and periodic negative
magnitudes, and conditionally excludes the entire cofinally bounded negative
branch.}{The last theorem retains positivity, bounded-rise, and
normalised-vanishing hypotheses.  The mixed-sign and unbounded-negative
regimes keep Problem~\#243 open.}{Comparator selects the eventually periodic
negative-orbit obstruction and the conditional exclusion of a cofinally
bounded negative state, with every hypothesis present in the challenge
statement.}

For release review, the selected Comparator rows are exact.  The declaration
\texttt{no\_eventuallyPeriodicNegative\_orbit} excludes a positive-period,
positive-drift negative orbit under $2\le a_n$, $0<e_{N+n}<a_{N+n}$, the
displayed denominator and centred-state recurrences, and the eventual period
and phase-drift equations.  The declaration
\texttt{no\_cofinallyBoundedNegative\_of\_normalizedVanishes} excludes a
cofinally bounded negative magnitude under positivity, bounded rise, the same
recurrences, normalised vanishing, and the cofinal negative-state relation.
These are conditional exclusions only; mixed-sign and unbounded-negative
regimes remain outside their conclusions.
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{38}{sylvesterNext}{Sylvester
successor}.
...
the problem~\cite[Remark~21, p.~13]{koizumi2025}.  Duverney proved a
...
relevant here.  Badea's positive-term criterion is adjacent but different:
...
$e_n$ at an index where the centred state is negative.  His Theorem~16
...
\texttt{sorry}.  We use it as statement-level prior art, not as proof
authority; the development below is independent of it.
What is machine-checked below concerns the state system rather than the
analytic problem: the exclusion of constant and
...
Periodic negative magnitude & Excluded here & Theorem~\ref{res:periodic}, in the regime $e_n<a_n$ and with positive drift $M>0$. \\
...
Normalised vanishing alone & Gcd changes are sparse & Proposition~\ref{res:gcdsparse}; arbitrarily late finite constant blocks, not eventual constancy. \\
Finite normalised negative mass & Excluded here; conditional & Theorem~\ref{res:mass}, Section~\ref{sec:mass}; normalised vanishing remains an input. \\
Factorial residue reduction & Proved here; superseded for its original purpose & Theorem~\ref{res:residue}, Appendix~\ref{app:residue}. \\
Normalised vanishing & Assumed here; supplied by prior art & Section~\ref{sec:bounded}; derived in~\cite{koizumi2025}, see Section~\ref{sec:problem}.  The separate strict-centring hypothesis retained in the Lean-matching statement follows eventually by taking $K=1$. \\
Unbounded, divergent-mass negative excursions & Necessary conditions proved here; orbit-level case open & Section~\ref{sec:open}: Proposition~\ref{res:frontier} gives conditions any counterexample must satisfy, not a separate open problem; Problem~\ref{res:excursions} is the surviving orbit-level obstruction. \\
Erd\H{o}s--Straus criterion; Duverney & Proved elsewhere & Cited, not formalised; the Erd\H{o}s--Straus hypothesis is implied by that of Theorem~\ref{res:descent}. \\
State system equals reciprocal tails & Not formalised & Section~\ref{sec:state}. \\
\bottomrule
\end{tabular}
\end{table}

\noindent Taken together the exclusions constrain the shape of any
counterexample.  By Theorem~\ref{res:absorb} a counterexample has $E_n\ne0$ for
...
statements should read Sections~\ref{sec:bounded} and~\ref{sec:mass}, and then
Section~\ref{sec:open}.  Appendix~\ref{app:index} is a guide to the formal
...
\]
the
\lword{Erdos243/ReciprocalTailRigidity.lean}{42}{nextDenState}{denominator
update}, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{46}{nextTailState}{tail update},
and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{50}{centeredState}{centred state}.
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{58}{nextTailState_eq_sub_centered}{update
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{1869}{natTail_eq_nextTailState}{tail
realisation}, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1881}{natDen_eq_nextDenState}{denominator
realisation}, and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1890}{natTail_eq_sub_centeredState}{signed
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{66}{nextDenState_scale}{denominator
equivariance}, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{71}{nextTailState_scale}{tail
equivariance}, and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{76}{centeredState_scale}{centred
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{54}{sylvesterDefect}{Sylvester
defect}.
...
primitive recurrence}.  Its hard point is not a formal induction: it keeps
both changing cancellation factors visible.  Thus it is stronger than a
one-step congruence, but it is not an autonomous recurrence and cannot by
itself force $a_1=a^2-a+1$.
...
These three identities do not manufacture the hypotheses of the orbit.  In
particular, they do not prove that cancellation is eventually absent, that
the curvature has one sign, or that the feedback gate is met cofinally; the
global mixed-sign and unbounded-negative regimes of Problem~\#243 remain
open.
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{2139}{centeredState_zero_absorbing}{absorption
of a vanishing centred state}, with the companion
\lword{Erdos243/ReciprocalTailRigidity.lean}{2166}{eventually_nonzero_of_zero_absorbing}{contrapositive
...
once and is finished, or it is never zero at all.  Sections~\ref{sec:constant}
and~\ref{sec:periodic} then restrict to the special all-negative case; this is
not a without-loss reduction for a mixed-sign tail.  The mixed-sign analysis
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{148}{no_scalePrimitiveConstantNegative_orbit}{scale-primitive
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{101}{no_normalizedConstantNegative_orbit}{normalised
exclusion}.
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{287}{no_constantNegative_orbit}{exclusion
at every scale}, and the eventual form, obtained by shifting the orbit to the
first constant index, as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{351}{no_eventuallyConstantNegative_orbit}{eventual
exclusion}.
...
state, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{383}{base_dvd_denState_later}{persistence
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{403}{divisorLocksTailSucc}{one-step
lock}, and once present in both $D$ and $C$ it stays in every later $D$, $C$
and magnitude, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{430}{divisorLock_persists}{persistence
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{495}{repeatedDivisor_forces_periodicCommonScale}{repeated-divisor
transport}, using that both the period relation and the drift iterate over any
number of periods, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{462}{periodic_add_mul}{iterated
period} and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{477}{phaseDrift_add_mul}{iterated
drift}.
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{549}{no_phasePrimitivePeriodicNegative_orbit}{phase-primitive
exclusion}, the induction is the
\lword{Erdos243/ReciprocalTailRigidity.lean}{691}{no_periodicNegative_orbit}{exclusion
at every scale}, and the eventual form is the
\lword{Erdos243/ReciprocalTailRigidity.lean}{797}{no_eventuallyPeriodicNegative_orbit}{eventual
exclusion}.
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{835}{exists_shifted_consecutiveMultiples}{shifted
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{899}{no_boundedRise_of_tailAvoidance}{bounded-rise
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{985}{reducedStep_coprime_currentFactor}{step
coprimality}, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1012}{reducedTail_pairwiseCoprime}{pairwise
coprimality}, and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1026}{reducedTail_wholeModulusAvoidance}{whole-modulus
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{1037}{no_boundedRise_reducedTail}{reduced
bounded-rise exclusion}, together with its eventual form, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1062}{no_eventuallyBoundedRise_reducedTail}{eventual
reduced exclusion}.
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{1428}{tailGcd_dvd_succ}{gcd
...
$\gcd(C_n,e_n)$, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1585}{tailGcd_eq_gcd_negativeMagnitude}{gcd
...
constant, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1603}{dvdChain_eventuallyConstant_of_cofinally_bounded}{chain
stabilisation}.  Hence cofinally bounded negative magnitudes make the tail gcd
stabilise, the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1634}{tailGcd_eventuallyConstant_of_cofinally_boundedNegative}{gcd
...
exact tail, is exposition and carries no separate label.  The hypothesis is
boundedness \emph{along a cofinal set} and not
boundedness at every index.  That is what the situation gives: the tail gcd is
...
\lword{Erdos243/ReciprocalTailRigidity.lean}{2096}{tailGcd_strictGrowthCount_sublinear_of_normalizedVanishes}{sublinear
strict-growth theorem}; the second is the
\lword{Erdos243/ReciprocalTailRigidity.lean}{2114}{tailGcd_exists_arbitrarilyLate_constBlock_of_normalizedVanishes}{arbitrarily
...
of the barrier argument.  Theorem~\ref{res:mass} in the next section is a
second endpoint under a different finiteness hypothesis, and neither theorem
contains the other.  Theorem~\ref{res:mass} assumes no orbit structure, only
the state recurrence with $C_n>0$, so it is not a special case of
Theorem~\ref{res:bounded}; conversely hypothesis~(5) below does not imply
...
\noindent Formalised as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{2183}{boundedNegativePart_eventually_zero}{bounded-negative-part
rigidity}, with the eventual form, in which the centring and the bound need
only hold from some index, as the
\lword{Erdos243/ReciprocalTailRigidity.lean}{2270}{eventuallyBoundedNegativePart_eventually_zero}{eventual
bounded-negative-part rigidity}.  The two auxiliary results it uses are the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1737}{tailState_tendsto_atTop_of_nonzero_normalizedVanishes}{divergence
from normalised vanishing} and the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1753}{no_cofinallyBoundedNegative_of_normalizedVanishes}{exclusion
of cofinally bounded negative magnitudes}, the latter over the
\lword{Erdos243/ReciprocalTailRigidity.lean}{1659}{no_cofinallyBoundedNegative_of_tailTendsto}{tail-divergence
form}.
...
Hypothesis~(4) holds at every index, not merely eventually, from the centring
range $-c_n/2\le e_n<c_n/2$ of the same lemma.  Hypothesis~(6) is the vanishing
...
positive constant; the second is not a weaker target.  A positive answer is
not yet, by itself, a proof of Problem~\#243: the missing bridge is a comparison
of $M_n$ with a subexponential state or exact cancellation-payment scale.
No assertion such as $M_n\mid C_n$ or $M_n\le C_n$ is used here.  Proving such
a bridge together with Problem~\ref{res:lcmheight} would collide with the
subexponential tail-height budget forced by normalised vanishing.
...
\]
The checked
\lword{Erdos243/ReciprocalTailRigidity.lean}{2080}{tailState_subexponential_of_normalizedVanishes}{tail-height estimate}
...
$\prod_{n<N}(1+\delta_n)$; that is an explanatory reformulation, not a second
success criterion.  A negative answer must be a sequence satisfying the full
growth and rationality hypotheses, not merely a locally admissible state
orbit.
...
~\cite[p.~11]{koizumi2025} gives $|E_n|<C_n$ at every index, not
merely for all large $n$; eventual strict centring also follows directly from
...
residue reduction of Appendix~\ref{app:residue} is in
...
Sylvester successor & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{38}{sylvesterNext}\\
denominator update & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{42}{nextDenState}\\
tail update & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{46}{nextTailState}\\
centred state & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{50}{centeredState}\\
Sylvester defect & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{54}{sylvesterDefect}\\
update law & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{58}{nextTailState_eq_sub_centered}\\
denominator equivariance & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{66}{nextDenState_scale}\\
tail equivariance & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{71}{nextTailState_scale}\\
centred equivariance & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{76}{centeredState_scale}\\
normalised exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{101}{no_normalizedConstantNegative_orbit}\\
scale-primitive exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{148}{no_scalePrimitiveConstantNegative_orbit}\\
constant exclusion at every scale & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{287}{no_constantNegative_orbit}\\
eventual constant exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{351}{no_eventuallyConstantNegative_orbit}\\
persistence of a multiplier & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{383}{base_dvd_denState_later}\\
one-step lock & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{403}{divisorLocksTailSucc}\\
persistence of a lock & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{430}{divisorLock_persists}\\
iterated period & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{462}{periodic_add_mul}\\
iterated drift & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{477}{phaseDrift_add_mul}\\
repeated-divisor transport & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{495}{repeatedDivisor_forces_periodicCommonScale}\\
phase-primitive exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{549}{no_phasePrimitivePeriodicNegative_orbit}\\
periodic exclusion at every scale & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{691}{no_periodicNegative_orbit}\\
eventual periodic exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{797}{no_eventuallyPeriodicNegative_orbit}\\
shifted consecutive multiples & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{835}{exists_shifted_consecutiveMultiples}\\
bounded-rise barrier & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{899}{no_boundedRise_of_tailAvoidance}\\
step coprimality & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{985}{reducedStep_coprime_currentFactor}\\
pairwise coprimality & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1012}{reducedTail_pairwiseCoprime}\\
whole-modulus avoidance & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1026}{reducedTail_wholeModulusAvoidance}\\
reduced bounded-rise exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1037}{no_boundedRise_reducedTail}\\
eventual reduced exclusion & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1062}{no_eventuallyBoundedRise_reducedTail}\\
gcd monotonicity & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1428}{tailGcd_dvd_succ}\\
gcd at a negative index & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1585}{tailGcd_eq_gcd_negativeMagnitude}\\
chain stabilisation & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1603}{dvdChain_eventuallyConstant_of_cofinally_bounded}\\
gcd stabilisation & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1634}{tailGcd_eventuallyConstant_of_cofinally_boundedNegative}\\
tail-divergence form & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1659}{no_cofinallyBoundedNegative_of_tailTendsto}\\
divergence from normalised vanishing & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1737}{tailState_tendsto_atTop_of_nonzero_normalizedVanishes}\\
exclusion from normalised vanishing & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1753}{no_cofinallyBoundedNegative_of_normalizedVanishes}\\
...
signed update law & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{1890}{natTail_eq_sub_centeredState}\\
absorption of a vanishing centred state & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{2139}{centeredState_zero_absorbing}\\
contrapositive along a tail & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{2166}{eventually_nonzero_of_zero_absorbing}\\
bounded-negative-part rigidity & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{2183}{boundedNegativePart_eventually_zero}\\
eventual bounded-negative-part rigidity & \lrefx{Erdos243/ReciprocalTailRigidity.lean}{2270}{eventuallyBoundedNegativePart_eventually_zero}\\
