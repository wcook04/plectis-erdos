...
and rational reciprocal sum, and put $P_n=\prod_{j<n}a_j$.
We prove that
\[
 \limsup_{n\to\infty}\frac{P_n}{a_n}
 \left(\frac{a_n^2}{a_{n+1}}-1\right)<+\infty
\]
forces $a_{n+1}=a_n^2-a_n+1$ eventually.  In denominator-cleared
coordinates, the main theorem says that an eventually bounded negative
part of the integer error, together with normalised vanishing, forces that
error to vanish.  Bounded negative errors cap upward increments; gcd
stabilisation supplies persistent pairwise-coprime moduli, and a Chinese
remainder block prevents an unbounded numerator from crossing with those
increments.  The state implication is checked in Lean; the transfer to the
original sequence is an ordinary proof.  A first-crossing formulation in
LCM coordinates gives a weighted record-excess criterion and an
LCM-prefactor consequence.  Neither the bounded-negative hypothesis nor
the required record budget is known for the unrestricted problem.
\end{abstract}
...
This follows from the state theorem below and the ordinary canonical-tail
transfer in Section~\ref{sec:bounded}; it also follows from
Corollary~\ref{res:lcmbounded}, since the prefix LCM divides $P_n$.
The latter comparison is one-sided: multiply the displayed defect by a
factor in $(0,1]$, so an upper bound remains an upper bound even when the
defect changes sign.

Write $\ctr(a,D,C)=D-(a-1)C$.  The checked statement underlying this
consequence is the following.  Its centring hypothesis is redundant under
normalised vanishing, but is retained to match the source declaration.
...
The identity $C_{n+1}=C_n-E_n$ turns bounded negative error into a bound on
upward increments.  On a nonterminal tail, integrality and normalised
vanishing force $C_n\to\infty$.  At the negative indices, the bound on $E_n$
stabilises the tail gcd.  Reduction then supplies persistent coprime moduli;
a CRT block is too long for a bounded jump to cross.  The proof in
Section~\ref{sec:bounded} composes precisely these ingredients.

The LCM version in Section~\ref{sec:lcmrecords} makes the transition
constraint explicit.  At a fresh record, the jump is
$d=(a_n-1)U_n-L_n$.  Any old modulus dividing both $U_n$ and $L_n$ must
therefore divide $d$.  This is the arithmetic information that an estimate
of the error's size alone does not contain.

The same jump obstruction, with no normalised-vanishing hypothesis and with
arbitrary integer coefficients $b_n$, proves only that the LCM height is
bounded.  Stationarity still uses the limit afterwards.  That
coefficient-uniform theorem is ordinary mathematics in
\texttt{CoefficientUniformBoundedHeight.md}; it is not the selected lead,
because bounded height alone does not force vanishing (an exact $U=1$,
$V=-1$ unit-numerator orbit has reciprocal sum $1/2$).
...
\noindent The problem and classical comparisons are recorded in
Appendix~\ref{app:antecedents}.  Koizumi's canonical coordinates supply the
integer state and normalised vanishing; the extra bound on the negative
part is the hypothesis settled here, not a consequence asserted for every
sequence in the problem.
...
Sections~\ref{sec:state}--\ref{sec:bounded} give the exact state equations,
absorption, gcd stabilisation and the CRT contradiction proving
Theorem~\ref{res:bounded}.  Sections~\ref{sec:mass}
and~\ref{sec:lcmrecords} give the scalar product argument and the weighted
first-crossing criterion.  The secondary rate result follows in
Section~\ref{sec:secondaryrate}; Section~\ref{sec:open} states the remaining
estimate.  The constant and periodic exclusions, primitive feedback,
record/epoch calculations, antecedents and formal-source guide are preserved
in the appendices.  None is a prerequisite for reading the main theorem.
...
\section{A secondary original-coordinate rate theorem}\label{sec:secondaryrate}

The following separate consequence combines regular-rate extraction with the
fixed-cubic exclusion recorded in Appendix~\ref{sec:records}.  Its ordinary
proof uses the polynomial and number-field argument, not only the finite
Lean identities.
