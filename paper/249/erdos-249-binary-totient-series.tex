...
\begin{abstract}
For integers $k\ge2$ and $e\ge1$, we give a basis of the totient sections
$n\mapsto\varphi(k^jn+r)$ through level $e$. Their rational rank is
$k^e+1$. Explicit scalar reductions give unique integral coordinates and a
basis of all integral relations. The independence argument uses
CRT--Dirichlet separation, an earlier method of Yazdani credited to Shallit;
Martin's theorem already implies the requisite affine independence, and
Coons previously proved non-$k$-regularity. The contribution here is the
finite-level normal form, not those qualitative obstructions.
We also prove irrationality of $\sum_{n\ge1}(\varphi(n)\bmod m)2^{-n}$
for every $m\ge3$ and classify rational dyadic residue observables.
For $S=\sum_{n\ge1}\varphi(n)2^{-n}$, we prove a seed-to-ray amplification
and record an equivalent central-residue condition. These results do not
establish irrationality of $S$. Ordinary proofs are given in the note;
formal counterparts and their precise scope are identified separately.
\end{abstract}
...
Coons proved that $\ph$ is not $k$-regular for any $k\ge2$~\cite[Thm.~3.2]{coons},
and Martin's Theorem~1 implies that the functions $n\mapsto\ph(a_in+b_i)$ are
linearly independent along pairwise nonproportional affine forms with integer
coefficients and positive slopes~\cite[Thm.~1]{martin-phi-inequalities}.
The following theorem gives the exact rank, unique integral coordinates,
and, in Corollary~\ref{cor:integral-normal-form}, all integral relations at
every finite level. Section~\ref{sec:rank} gives a direct proof of the
independence step, with the antecedent of its separation method credited
there. No priority claim is made for affine independence.
...
\textbf{Exact finite-level rank.}  For integers $k\ge2$ and $e\ge1$,
Theorem~\ref{thm:kkernelrank} proves ordinarily that the totient $k$-kernel
truncation has rank $k^e+1$. The dyadic case is also the
\mword{Erdos249257/TotientMahlerDefect.lean}{1084}{finrank_totientKernelThroughLevelFamily_eq}{exact finite-truncation rank};
the reusable all-base Lean wrapper is
...
and retains linear independence as an explicit input. That input is discharged
by the independent CRT--Dirichlet proof in
\href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1118}{\texttt{AllBaseTotientKernel.lean}};
the \href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1239}{complete truncation rank}
therefore has no independence hypothesis. Martin supplies the broader external
antecedent, rather than an axiom of this Lean proof.
\end{enumerate}
...
The reductions explain which sections must be dependent. The difficulty is
to exclude every additional relation. We do this one coefficient at a time:
an evaluation will annihilate all the other columns modulo a prime while
leaving the chosen column nonzero. Thus no simultaneous primality assertion
for several affine forms is needed.
...
\href{https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos249/PaperCompleteR7/KernelIntegral.lean#L226}{maximal power reduction}.
...
At level zero the only channel is $\varphi(n)$, which is nonzero since
$\varphi(1)=1$, so its span has dimension one. This is an ordinary proof;
it makes no claim about an additional formal declaration.
\end{proof}
...
therefore recovers Coons's theorem by a different route and gives the
dimension at every finite level.

The generating-function context is stronger than non-regularity alone.
For multiplicative coefficients in characteristic zero, Bell and
Smertnig prove that $k$-Mahler implies $k$-regular~\cite[Thm.~1.3]{bell-smertnig2026};
their totient example therefore rules out a $k$-Mahler equation for
$F_\varphi(z)=\sum_{n\ge1}\varphi(n)z^n$ in every base $k\ge2$.
Bell, Bruin and Coons give the corresponding algebraic and $D$-finite
classification framework~\cite[Thms.~1.5--1.6]{bell-bruin-coons}.
These are restrictions on functions. Neither classification by itself
determines the arithmetic nature of $F_\varphi(1/2)$.

For a $k$-regular sequence the difference between a basis of the kernel span
and a redundant spanning family is material: matrices associated to a basis
compute the growth exponent through their joint spectral radius, whereas a
spanning family gives only an upper bound~\cite[Thm.~1, Prop.~4, Cor.~7]{coons-jsr}.
That theorem does not apply to $\ph$, but the section maps
$g(n)\mapsto g(kn+i)$ still have an explicit form here. Each sends
$F_{j,r}$ to $F_{j+1,k^ji+r}$, so it carries $V_{k,e}$ into $V_{k,e+1}$, and
no level is invariant under all of them because the dimensions increase. In
the canonical bases, a retained section with $j\ge1$ goes to a retained
section, while $F_{0,0}$ and $F_{1,0}$ go to $F_{1,i}$ and to $F_{2,ki}$, a
positive integer multiple of $F_{1,i}$. Every element of $\mathcal B_{k,e}$
is therefore sent to a positive integer multiple of an element of
$\mathcal B_{k,e+1}$.
...
This is an ordinary proof of the periodic-coefficient statement; no additional
formal declaration is asserted here.
\end{proof}
...
This does not prove irrationality of the dyadic sum. When $m>2$ the raw
residues are not binary digits, and carrying changes the digit sequence.
The residue theorem below instead forces neighbouring coefficients to
vanish while retaining one central value, then uses a bounded-tail argument.
...
proved ordinarily above. The separate formal counterpart is in
\href{https://github.com/wcook04/plectis-erdos-lean/blob/52f29ad173b04e3bac941b3663f2b9aebe5de0bb/PalomarCorpus/E249/Challenge.lean#L572}{\texttt{wcook04/plectis-erdos-lean}}
at commit \texttt{52f29ad1} states this lemma as
\texttt{irrational\_dyadicValue\_of\_pulses} in
\texttt{PalomarCorpus/E249/Challenge.lean} and proves it.  That repository's
...
ordinary in this note (that \texttt{PaperCompleteR7} file is absent).
\end{proof}
...
statement about $S$.
The complementary rationality consequence uses the same carry for both kinds
of rigidity: the
\mword{Erdos249257/TotientTailCarryPeriod.lean}{224}{not_irrational_totientSeries_implies_mod_period_and_unbounded_rank}{same-carry rank and modular-period theorem}
gives rank at least $2^e-1$ at every depth and one common eventual period for
all dyadic sections modulo a fixed positive multiplier. It supplies necessary
structure under rationality, not a contradiction between the two properties.
...
\end{theorem}
The seed-to-ray step is
\lword{Erdos249/FullDepthRayAmplifier.lean}{364}{eventually_twoSyndetic_fullDepthKillMultipliers_of_seed}{eventual two-syndetic amplification};
it is equivalent to irrationality as
\lword{Erdos249/FullDepthRayAmplifier.lean}{406}{apFullDepthEscape_iff_irrational}{global full-depth equivalence}.
\begin{proof}
...
and $v$ odd, is
ordinary in this note (that \texttt{PaperCompleteR7} file is absent).
The separate public Lean release \texttt{wcook04/plectis-erdos-lean} at
commit \texttt{52f29ad1} states the equivalence in this form, with
$vM=2^H-1$, as
\texttt{fullMersenneCanonicalBasepointResidueGapSupply\_iff\_irrational} in
\texttt{PalomarCorpus/E249/Challenge.lean} and proves it; the replay receipt
is cited at Lemma~\ref{lem:bounded-pulse}.
...
A rational control, proved in \texttt{ParityPerturbedRationalControl.lean}, has
...
compatible with rationality. For one fixed prime $p$ and integer-valued $g$, the exact laws
...
\]
The divisor-convolution identity $\ph=\mu*\mathrm{Id}$ gives the displayed
value of $\Theta_2$. Amiram Eldar posted the corresponding formula
...
Coons proved that $\ph$ is not $k$-regular~\cite[Thm.~3.2]{coons}; the exact
ranks recover this, as explained after Corollary~\ref{cor:integral-normal-form}.
Martin's Theorem~1 makes the ratios of totients on any prescribed finite list
of pairwise nonproportional affine forms with integer coefficients and positive
slopes arbitrarily separated; this implies
their linear independence~\cite[Thm.~1]{martin-phi-inequalities}.
Section~\ref{sec:rank} gives a direct determinant proof tailored to the
sections used here. The basis theorem
gives the exact finite-level normal form.
Section~\ref{sec:family} treats the residue series at base two, where Wong's
matched-base answer is credited, and classifies dyadic observables.
Erd\H{o}s and Graham record the irrationality of $S$ as probably hopeless to
prove at present~\cite[p.~61]{erdosgraham1980}.

The
\mword{Erdos249257/TotientMahlerDefect.lean}{1084}{finrank_totientKernelThroughLevelFamily_eq}{dyadic finite-level rank $2^e+1$},
residue-series irrationality,
\lword{Erdos249/FullDepthRayAmplifier.lean}{364}{eventually_twoSyndetic_fullDepthKillMultipliers_of_seed}{full-depth amplification},
canonical equivalence and the
\lword{Erdos249/RankOneSharpFloor.lean}{624}{rankOneSubrankQuotient_sub_theta_two_gt_twentyOne_div_threeTwenty}{positive rank-one floor}
are in the public snapshot.  The
unconditional all-base exact-rank theorem is also
\href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1239}{kernel-checked}:
its CRT--Dirichlet independence proof does not assume Martin's theorem.
...
remains the reusable conditional factorisation.
The
\lword{Erdos249/CyclotomicAnchoredKill.lean}{3165}{fullMersenneCanonicalBasepointResidueGapSupply_iff_irrational}{canonical gap}
is the remaining arithmetic endpoint.
The historical source links retain \texttt{\commitshort}.
...
The long record gives the Lean-checked Farey-window exclusion at window
$K=240$: $S\ne a/q$ for every integer $a$ and every $0<q\le Q_0$, where
$Q_0\approx7.96\times10^{34}$ is stated exactly there. It does not imply the
universal gap.

The remaining first-harmonic obligation is a four-term pivot budget,
not an opening statement of this note. Write
\[
 \begin{aligned}
...
\end{problem}
The quantities are defined with the
\mword{Erdos249257/FirstHarmonicPivot.lean}{543}{PivotBudgetAt}{budget condition}
in the long record. This note does not prove them.
Balasubramanian, Giri and Srivastav prove asymptotics for shifted convolutions
$\sum_nF(n)G(n-h)$ of divisor convolutions $F=f*1$, $G=g*1$ with decaying
$f,g$, a class that contains $\ph(n)/n$~\cite[Thm.~2.2]{bgs2016}; the first
bound above needs cancellation against the dyadic phase, which that theorem
does not supply.
\appendix
...
\mword{Erdos249257/TotientMahlerDefect.lean}{160}{totientKernel_zero_residue}{totient Kernel zero residue},
\mword{Erdos249257/TotientMahlerDefect.lean}{169}{totientKernel_even_residue_reduce}{totient Kernel even residue reduce},
\mword{Erdos249257/TotientTailPeriodKiller.lean}{150}{two_pow_mul_totient_series_eq}{two pow mul totient series eq},
...
\mword{Erdos249257/TotientTailPeriodKiller.lean}{404}{certifiedKill_all_small}{certified Kill all small},
\mword{Erdos249257/TotientTailPeriodKiller.lean}{407}{tail_diff_not_int_small}{tail diff not int small},
\mword{Erdos249257/GenericTailOrbitRigidity.lean}{67}{IsTemperedBinaryOrbit}{is Tempered Binary Orbit},
...
\mword{Erdos249257/CertificateKernel.lean}{8335}{irrational_erdosBorwein_series}{irrational erdos Borwein series},
\mword{Erdos249257/CertificateKernel.lean}{18434}{tsum_moebius_div_two_pow_sub_one_eq_half}{tsum moebius div two pow sub one eq half},
\mword{Erdos249257/CertificateKernel.lean}{18429}{tsum_totient_div_two_pow_sub_one_eq_two}{tsum totient div two pow sub one eq two},
\mword{Erdos249257/CertificateKernel.lean}{18454}{totient_series_eq_half_add_moebius_mersenne_square}{totient series eq half add moebius mersenne square},
\mword{Erdos249257/CertificateKernel.lean}{18557}{totient_series_eq_half_add_visible_coprime_pairs}{totient series eq half add visible coprime pairs},
\mword{Erdos249257/GcdMomentCalculus.lean}{349}{tsum_pos_coprime_inv_mersenne_eq_one}{tsum pos coprime inv mersenne eq one},
\mword{Erdos249257/GcdMomentCalculus.lean}{474}{cylinderMass_split}{cylinder Mass split},
\mword{Erdos249257/TotientMahlerDefect.lean}{823}{paritySeparatedMatrix_det_ne_zero}{parity Separated Matrix det ne zero},
\mword{Erdos249257/TotientMahlerDefect.lean}{882}{exists_separatedMinorCertificate_totientAffineOddFamily}{exists separated Minor Certificate totient Affine Odd Family},
...
\mword{Erdos249257/GapFareyBound.lean}{176}{gap_check_window_1_240_le_79639646646701375323355774875831053}{gap check window 1 240 le 79639646646701375323355774875831053},
\mword{Erdos249257/CertificateKernel.lean}{15986}{tsum_totient_div_pow_two_ne_int_div_of_totient_gap_certificate}{tsum totient div pow two ne int div of totient gap certificate},
\mword{Erdos249257/GapFareyBound.lean}{88}{gap_check_window_1_120_le_248672326362367909}{gap check window 1 120 le 248672326362367909},
\mword{Erdos249257/CertificateKernel.lean}{18572}{tsum_visible_coprime_pairs_ne_int_div_of_den_le_39819823323350687661677887437915526}{tsum visible coprime pairs ne int div of den le 39819823323350687661677887437915526},
...
\mword{Erdos249257/TropicalCurvatureCarry.lean}{137}{fixedPrecisionTropicalNoGo}{fixed Precision Tropical No Go},
...
\mword{Erdos249257/TotientTailPeriodKiller.lean}{57}{totientTail}{totient Tail},
\mword{Erdos249257/LcmConeFlatness.lean}{357}{not_irrational_of_tail_diff_mem_int}{not irrational of tail diff mem int},
...
\lword{Skip/LadderT67.lean}{71264}{exists_diagonalKill_le_82}{exists diagonal Kill le 82},
...
\mword{Erdos249257/FirstHarmonicPivot.lean}{514}{windowFirstExp_sum_eq_pivot_decomposition}{window First Exp sum eq pivot decomposition},
...
\mword{Erdos249257/TotientKernelConditional.lean}{203}{finrank_canonicalAllBaseTotientKernel_eq_of_linearIndependent}{canonical all-base rank from independence},
...
At the later public checkpoint \texttt{25ef6245d15a},
\href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1118}{\texttt{linearIndependent\_allBaseCanonicalFamily}},
\href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1231}{\texttt{finrank\_allBaseTotientKernel\_eq}}, and
\href{https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1239}{\texttt{finrank\_allBaseThroughLevelFamily\_eq}}
provide the unconditional all-base independence and both exact ranks.
