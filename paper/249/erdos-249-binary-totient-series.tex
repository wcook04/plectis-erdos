...
\begin{abstract}
For every integer $k\ge2$ and every $e\ge1$, the sections of Euler's totient
through level $e$ have rational rank $k^e+1$. The zero-residue sections and
the residues not divisible by $k$ form a basis. Explicit scalar reductions
give unique integral coordinates and a basis of all integral relations. The
arithmetic step is independence: divisibility makes an evaluation matrix
diagonal modulo a prime, with nonzero diagonal entries. This CRT--Dirichlet
argument proves the all-base theorem ordinarily in the note. Lean also checks
canonical-family independence and the complete all-base rank unconditionally.
Martin's theorem supplies a broader external
affine-independence result, while Coons proved non-$k$-regularity of $\ph$;
the exact ranks recover that theorem and quantify it level by level.
A separate ordinary argument uses isolated blocks to prove that
$\sum_{n\ge1}(\ph(n)\bmod m)2^{-n}$ is irrational for every $m\ge3$,
with a complete rationality classification for dyadic residue observables.
...
If $S$ were rational, one tempered integral carry would simultaneously have
section rank at least $2^e-1$ at every depth and one common eventual period
modulo a fixed multiplier. These necessary conditions do not decide $S$: its
irrationality is equivalent to an explicit central-residue gap for every
prospective denominator, whose arithmetic supply remains open.
\end{abstract}
...
The following theorem gives the exact rank and a unique coordinate system at
every finite level; Section~\ref{sec:rank} proves its independence step directly.
...
The level-zero clause is ordinary in this note;
\texttt{PaperCompleteR7/ArithmeticAssemblies.lean} is not in this checkout.
\end{proof}
...
The statement is ordinary in this note;
\texttt{PaperCompleteR7/PeriodicAndPulse.lean} is not in this checkout.
\end{proof}
