...
sections form a basis. Explicit scalar reductions give unique integral
coordinates and a basis of all integral relations. The arithmetic step is
independence: divisibility makes an evaluation matrix diagonal modulo a prime,
with nonzero diagonal entries. The
\mword{Erdos249257/TotientMahlerDefect.lean}{1084}{finrank_totientKernelThroughLevelFamily_eq}{dyadic rank $2^e+1$}
holds for $e\ge1$.  The all-base rank
$k^e+1$ is the
\mword{Erdos249257/TotientKernelConditional.lean}{215}{finrank_allBaseTotientKernelThroughLevelFamily_eq_of_linearIndependent}{rank transferred from linear independence},
using Martin's external independence; Coons proved non-$k$-regularity
of $\ph$. A separate ordinary argument uses isolated blocks to prove that
...
The rank and bounded-residue results do not decide $S$: its irrationality is
equivalent to an explicit central-residue gap for every prospective denominator,
whose arithmetic supply remains open.
\end{abstract}
...
and is not a Lean theorem without Martin's hypothesis.
\end{enumerate}
