...
separately: we exhibit the smallest exact obstruction to separating it as such
a product, the two sides being $1/60$ and $1/12$.  The second statement
excludes every argument that first separates the three coordinates and then
treats them one at a time.
...
Theorem~\ref{res:rank} shows that the kernel at $\{2,3,5\}$ is not a product
$f(i)g(j)h(k)$, and therefore excludes every argument that first separates the
three coordinates and then treats them one at a time.  The mechanism behind the
second is a small computation: $\hgt(6)=4\cdot3\cdot5=60$ and not $6$, because
the running least common multiple at a smooth cutoff already sees powers of the
other primes that the cutoff itself does not contain.
