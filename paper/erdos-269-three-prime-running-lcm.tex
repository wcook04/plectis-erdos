...
\hypersetup{
  pdftitle={Running least common multiples of smooth numbers: a two-prime transcendence theorem and exact three-prime obstructions},
  pdfsubject={Erdős Problem Notes: Problem 269},
  pdfkeywords={transcendence, least common multiple, smooth numbers, Sturmian words, Hecke--Mahler series, Lean 4},
}
...
\runninghead{Erd\H{o}s \#269: smooth-number running lcms}
\title{Running Least Common Multiples of Smooth Numbers}
\subtitle{A two-prime transcendence theorem and exact three-prime obstructions}
\author{Will Cook}
\date{July 2026}
...
\begin{abstract}
\noindent
For distinct primes $p$ and $q$, retain one reciprocal for each distinct value
of the running least common multiple of the $\{p,q\}$-smooth numbers.  We prove
that the resulting series is transcendental for every pair $p\ne q$.  The
ordered jumps form a shift of a lower mechanical Sturmian word.  This gives an
exact affine expression for the series in terms of a Hecke--Mahler value, to
which a theorem of Bugeaud and Laurent applies.  The result settles the
two-prime de-duplicated series, but not the original series with repetitions.

For three primes we establish exact structure rather than irrationality.  The
running least common multiple below $x$ is the product of the three largest pure
prime powers below $x$.  In the case $\{2,3,5\}$, compressing its jumps between
successive powers of two gives precisely the radix alphabet
$\{2,6,10,30\}$.  The smallest nontrivial restriction of the reciprocal kernel
has determinant $-1/15$, ruling out a single product of three one-variable
factors.  We also give an exact finite height-fibre normal form and a quadratic
shell-multiplicity bound.  These claims are linked to Lean declarations; the
two-prime transcendence argument uses an external analytic theorem and is not
formalised here.

The remaining three-prime route is conditional.  Once a bounded integral carry
with the required smooth-factor divisibility is available, a
denominator-dependent local-window residue escape forces a contradiction.  We
check the algebraic cancellation and the conditional consumer, and an
integer-only scan finds such a window for $106{,}666$ tested denominator/start
pairs.  The scan is finite evidence.  The rationality-to-carry bridge, including
the needed divisibility, and an unbounded escape producer are not proved.
Consequently Erd\H{o}s Problem~\#269 remains open.
\end{abstract}
...
For one prime, $a_n=p^{n-1}$ and the series is the rational number $p/(p-1)$.
For infinitely many primes, Erd\H{o}s calls irrationality a simple exercise
\cite[p.~106]{erdos1988}.  In a letter dated 1 January 1973 he also stated that
irrationality follows after repeated running-LCM values are deleted
\cite[p.~335]{erdos1974letter}, but the one-page letter gives no proof.

When $|P|=2$, that de-duplicated series is transcendental for every pair of
distinct primes.  Section~\ref{sec:two-prime} reads the interlacing pure-power
channels as a shifted lower mechanical Sturmian word and reduces the series to
the Hecke--Mahler value covered by Bugeaud and Laurent's Theorem~1.1
\cite[p.~61, Theorem~1.1]{bugeaudlaurent2023}.  This is an independent argument,
not a reconstruction of Erd\H{o}s's unpublished proof.  It does not cover the
original series, where a running value is repeated for every smooth integer in
the corresponding cell, and it does not extend to three primes.

For the three-prime analysis, fix pairwise distinct primes $p,q,r$.  Call $n$
\emph{smooth} when
...
height}.  Here $\lfloor\log_b x\rfloor$ is the largest $e$ for which
$b^e\le x$.  Since $a_1,\ldots,a_n$ are exactly the smooth numbers up to
$a_n$, the original summand is $1/\Lprefix(a_n)$.  At a smooth point the
reciprocal kernel is
\[
 \Kernel(i,j,k)=\frac{1}{\hgt(p^{i}q^{j}r^{k})},
\]
the
\lword{Erdos269/ThreePrimeRunningLcm.lean}{40}{threePrimeKernelQ}{lattice
kernel}.  We call the positive powers of a base its \emph{channel}.

All later formal statements use the elementary identity
$\Lprefix(x)=\hgt(x)$.  It is the finite-$P$ restriction of the classical
prime-power decomposition of $\lcm(1,\ldots,N)$, so no novelty is claimed for
the identity itself
\cite[Ex.~1.21(a), p.~22]{apostol1976}\cite[p.~183,
Exercise~6.2.7]{montgomeryvaughan2007}.  The running value is constant on
logarithmic cells and changes by one prime at each pure-power jump.  For
$\{2,3,5\}$, grouping those jumps between
successive powers of two gives the exact block alphabet $\{2,6,10,30\}$.
Separability fails at the first nontrivial rectangle: the corresponding
$2\times2$ minor of the reciprocal kernel is $-1/15$.  We also derive the
finite height-fibre normal form and its quadratic shell bound.  These are
unconditional structural statements; none is an irrationality theorem for a
three-prime series.

For the residue argument, suppose rationality produced a bounded integral carry
divisible by the $\{2,3,5\}$-smooth part of its denominator.  That factor could
then be cancelled, leaving a denominator $B$ coprime to $30$.  A suitable
consecutive window would force the reduced carry outside its bound.  Lean checks the
absorption-and-cancellation algebra and the implication from the stated
local-window escape condition to nonexistence of the reduced carry.  It does
not derive the actual carry or its divisibility from the series, and it does not
produce escaping windows cofinally.  The finite scan reported below probes the
second gap only; it cannot fill either one.

The statement of Problem~\#269 also appears as an unproved conjecture in the
\emph{Formal Conjectures} collection~\cite{formalconjectures269}.  Its
Nat-indexed normalisation includes the empty-prefix term, and its principal
assertions end in \texttt{sorry}.  The declarations linked in this note prove
auxiliary propositions about the same objects; they do not solve the
conjecture.  Kova\v{c} and Tao~\cite{kovactao2024} treat other Erd\H{o}s unit
fraction problems by elementary means, but their arguments are not used here.
We give no numerical estimate for the value of the original series.

\section{The de-duplicated two-prime sum is transcendental}
\label{sec:two-prime}

Let $P=\{p,q\}$ with $p<q$, and write
\[
 L_{p,q}(t)=p^{\lfloor\log_p t\rfloor}q^{\lfloor\log_q t\rfloor}.
\]
The elementary least-common-multiple argument used later in
Theorem~\ref{res:lcm}, with one coordinate omitted, identifies $L_{p,q}(t)$ as
the running least common multiple of the $\{p,q\}$-smooth numbers up to $t$.
Define
\[
 \mathcal D_{p,q}
 =1+\sum_{t\in\{p,p^2,\ldots\}\cup\{q,q^2,\ldots\}}
       \frac1{L_{p,q}(t)} .
\]
Thus $\mathcal D_{p,q}$ contains the initial value and exactly one reciprocal
...
$\delta_n\in\{0,1\}$.  The word $(\delta_n)_{n\ge0}$ is a shift of the lower
mechanical Sturmian word of slope $\theta$; the floor-increment formula is all
...
multiplicity to each running value, and a third prime replaces the single
Sturmian boundary by a genuinely two-dimensional ordering problem; neither is
...
Carry contradiction assuming one window per reduced denominator & \authlean & Conditional implication only; Theorem~\ref{res:windowconsumer}. \\
...
Section~\ref{sec:two-prime} proves the unconditional transcendence theorem.
Sections~\ref{sec:lcm} and~\ref{sec:cells} then identify the three-prime running
value and its jumps.  Section~\ref{sec:fibre} gives the finite three-prime
normal form, Section~\ref{sec:shell} bounds fibre multiplicity, and
...
can stay inside its bound once one window has the required residue escape
(Theorem~\ref{res:windowconsumer}).  The stronger cofinal condition introduced
below is a natural way to supply such a window.  Neither producer is proved
here, and the closing subsection reports a finite computation rather than an
unbounded theorem.
...
identification of Section~\ref{sec:open}.  Say that \emph{local-window escape}
holds when, for every $B>0$ coprime to $30$, there are $\ell$ and $h>0$ such
that
\[
 C_{\ell,h}:=|W_{\ell,h}|>0
 \quad\text{and}\quad
...
statement.  The exact minimal predicate is the
\lword{Erdos269/RestrictedFloorSum.lean}{652}{LocalWindowEscape}{local-window
escape condition}.  The stronger \emph{cofinal} version asks for such a window
after every lower bound $\ell_0$; this is the
\lword{Erdos269/RestrictedFloorSum.lean}{594}{CofinalLocalWindowEscape}{cofinal
local-window escape condition}.  Lean checks that the cofinal version implies
the minimal one by taking $\ell_0=0$.
...
of this subsection, and assume local-window escape.  Fix $B>0$ coprime to
...
\begin{proof}
Choose the window supplied for $B$.  The checked window identity
gives
\[
...
\lword{Erdos269/RestrictedFloorSum.lean}{677}{no_positive_reducedCarry_of_localWindowEscape}{minimal
reduced-carry extinction theorem}.  The
\lword{Erdos269/RestrictedFloorSum.lean}{666}{localWindowEscape_of_cofinalLocalWindowEscape}{cofinal-to-local
implication} recovers the earlier cofinal formulation.  Coprimality with $30$
is used to select a window; once a window has been chosen, the finite
contradiction does not use it.  The formalisation carries the edge cases:
...
Problem~\ref{prob:producer} implies the local hypothesis of
Theorem~\ref{res:windowconsumer}, so it rules out the corresponding positive
reduced carry.  Together with the identification above it would supply the
missing contradiction for the three-prime case.  Until both pieces are present,
Problem~\#269 remains open.

For each $B$, Problem~\ref{prob:producer} asks for a cofinal family of windows
satisfying~\eqref{eq:escape}, that is, a family containing windows that start
arbitrarily far out; it does not ask for every window to escape.  The radix word is
...
to $0$ modulo that modulus.  No equidistribution of all windows is required.
The formal consumer uses exactly one escaping window for each reduced
denominator $B$, because the carry recurrence, positivity and bound hold at
every index.  Cofinal escape is retained as the natural producer shape and is
now connected to that minimal consumer by a separate Lean theorem.
...
carry states by that factor is proved.  The local-window predicate is an
unproved hypothesis of Theorem~\ref{res:windowconsumer}; the stronger cofinal
predicate is a sufficient producer, and neither is asserted for the actual
$\{2,3,5\}$ word.
...
minimal window producer & \lrefx{Erdos269/RestrictedFloorSum.lean}{652}{LocalWindowEscape}\\
cofinal-to-minimal implication & \lrefx{Erdos269/RestrictedFloorSum.lean}{666}{localWindowEscape_of_cofinalLocalWindowEscape}\\
reduced-carry extinction & \lrefx{Erdos269/RestrictedFloorSum.lean}{677}{no_positive_reducedCarry_of_localWindowEscape}\\
\end{tabular}
\fi
