...
\begin{abstract}
\noindent
The main contribution is a complete answer when the prime set has size two.
De-duplication means retaining one reciprocal for each distinct value of the
running least common multiple, rather than repeating that reciprocal at every
smooth integer.  For every pair of distinct primes, both the de-duplicated sum
and Erd\H{o}s's original sum with repeated running-LCM values are
transcendental.  Ordering the two pure-power channels by a Beatty word
expresses the de-duplicated sum as a nonconstant algebraic affine function of a
quantity $A$ that is itself an affine function of a Hecke--Mahler value; the
repeated sum factorises as $A(1+B)$ and is a nonconstant quadratic in the same
$A$.  The transcendence engine is Loxton and van der Poorten's 1977 theorem,
of which the Bugeaud--Laurent statement quoted here is the modern form.  These
are the unconditional transcendence conclusions of the note.  Both are paper
proofs using that external theorem, not Lean-formalised results, and neither
applies to any three-prime instance.  Steve Fan publicly posted the same
reduction and conclusion on the erdosproblems.com discussion page on 26 June
2026.  The first public version of this manuscript was released on 22 July
2026 (commit \texttt{a9d3ab8}), after Fan's post.  The argument here was found
independently, and this note claims no priority for it.

For three pairwise distinct primes $p,q,r$, the contribution is exact
structure and exact reductions, not an irrationality theorem.  Writing $\Lprefix(x)$ for the least
common multiple of the $\{p,q,r\}$-smooth numbers not exceeding $x$, we
Lean-check
\[
 \Lprefix(x)=p^{\lfloor\log_p x\rfloor}q^{\lfloor\log_q x\rfloor}
 r^{\lfloor\log_r x\rfloor}\qquad(x\ge1).
\]
This yields the logarithmic cells and jump ratios, a finite height-fibre normal
form, and the shell bound $9\,\#\mathcal S\le(j+3)^2$ under the stated ordered
box hypothesis.  The kernel has a stronger phase transition than the smallest
minor suggests: for every order $n$ there are injective row and column index
families whose $n\times n$ determinant is nonzero simultaneously in every
third-coordinate layer.  Consequently no finite exact separation
$\sum_{\ell<d}f_\ell(i)G_\ell(j,k)$ exists.

For the smallest case $\{2,3,5\}$, the literal infinite dyadic shell mass is
summable and its height-normalised tail $X_a$ obeys the exact ordered affine
recurrence.  Either some $X_a$ is integral or cofinally many $X_a$ lie at
distance at least $1/31$ from every integer.  Rationality of the full shell
sum places every $X_a$ on one common $1/q$-lattice and therefore forces two
distinct scales to collide modulo one.  The remaining integral branch is not
opaque: $X_a$ satisfies an exact pinning identity, integrality is upward
closed, and any orbit that follows the same recurrence forever inside the
same shrinking anchor windows must equal the true infinite tail.  These
statements expose, but do not exclude, the integral branch.

Two further statements in $\{2,3,5\}$ are concrete.
First, group the jumps of $\Lprefix$, that is, the points at which its value
increases, into the blocks cut out by consecutive powers of two.  Each open
block $(2^{a},2^{a+1})$ contains at most one pure $3$-power and at most one
pure $5$-power, so the block radix $\beta_a$, the product of the terminal
factor $2$ with a factor $3$ when the block contains an internal $3$-power and
a factor $5$ when it contains an internal $5$-power, belongs to the exact
four-element alphabet $\{2,6,10,30\}$, rather than merely lying in a coarse
bounded interval.  All four letters already occur among the first five blocks:
$\beta_0=2$, $\beta_1=6$, $\beta_2=10$ and $\beta_4=30$.  Second, the
reciprocal of the running least common multiple at a smooth point is not a
product of functions of the three exponents separately: on the smallest
nontrivial two-by-two exponent rectangle its determinant is exactly $-1/15$.
Thus that matrix has rank two.  It is the first concrete instance of the
arbitrary-order theorem, not the endpoint of the rank analysis.

Two further exact obstructions sharpen the carry frontier.  Under channel
surjectivity and explicit $2\to3$ and $2\to5$ anchor equalities, block-nullity
forces the perturbation to vanish; a nonzero integral lift error then grows
like the product of the successive bases, so even one bound below $2^N$ is
impossible (the checked declarations
...
extinction}).  In a four-state unit-accuracy model, the same two anchors
force the first complete $2$-block defect to equal $1$, ruling out
block-nullity immediately
(\lword{Erdos269/CarryLiftExtinction.lean}{308}{no_unitAccurateLift_with_twoAnchors_and_firstTwoBlockNull}{four-state
obstruction}).  These are conditional no-go statements, not assertions that
the actual three-prime orbit supplies the anchors or a carry lift.

At the structural frontier, the carry splits exactly into a residue digit and a
coboundary.  The bounded part is literal: for a positive modulus the residue
lies in the half-open interval \(0\le c\bmod B<B\), and when the next residue
is the canonical reduction of \(b(c\bmod B)\), the residue digit lies in the
finite alphabet \(0\le d<b\)
(\lword{Erdos269/WeightedPhaseCarry.lean}{150}{carryResidue_mem_interval}{residue interval}
and \lword{Erdos269/WeightedPhaseCarry.lean}{157}{residueDigit_mem_interval}{digit interval}).
Its Cartier realisation shows that any finite-dimensional factorisation would force the realised symbolic span to be finite
(\lword{Erdos269/WeightedPhaseCarry.lean}{109}{carry_eq_residueDigit_add_coboundary}{residue--coboundary form},
\lword{Erdos269/WeightedPhaseCarry.lean}{293}{CartierRealisation}{Cartier realisation},
and \lword{Erdos269/WeightedPhaseCarry.lean}{334}{finite_realisedSpan_of_factorisation}{finite-span consequence}).
The exact recurrence still contains the uncontrolled integral coboundary:
finite residue does not give a finite-state quotient, and no theorem here
proves that the literal realised span is infinite.  This is therefore a
representation obligation, not a general analytic no-go; a scalar evaluation
alone supplies no function-faithful factorisation.

Problem~\#269 remains open from three primes onward: we prove no irrationality
...
The executable Comparator packet carries the finite consumer of this route as
the exact no-positive-reduced-carry theorem
\lword{Erdos269/RestrictedFloorSum.lean}{645}
      {no_positive_reducedCarry_of_cofinalLocalWindowEscape}
      {the conditional carry-extinction interface}.
Its hypotheses retain the cofinal local-window escape, positive multiplier
coprime to $30$, cleared recurrence, positivity, and denominator-dependent
bound.  This transport records a sharp conditional obstruction; it does not
provide the rationality-to-carry bridge or the cofinal escape producer, and so
does not settle any three-prime instance of Problem~\#269.
\end{abstract}

\externalreviewobject{The paper proves two-prime transcendence using an
external analytic theorem, then develops exact three-prime running-LCM cells,
arbitrary-order kernel rank, the actual infinite shell orbit, the all-scale
rationality lattice, integral-branch pinning, and a conditional carry
criterion.}{The two-prime result has paper-plus-external-theorem authority.
The exact three-prime reductions do not exclude the integral state or the
forced modular collision, so the three-prime problem remains open.}{Comparator
carries the exact running-LCM identity and infinite-rank obstruction, the
actual shell-orbit dichotomy, the all-scale lattice and collision target, the
integral-branch pinning family, the conditional reduced-carry extinction
interface, and the weighted-phase residue--coboundary observer
\lword{Erdos269/WeightedPhaseCarry.lean}{109}
      {carry_eq_residueDigit_add_coboundary}{the weighted-phase carry split}.
The conditional extinction interface preserves its cofinal-escape,
coprimality, recurrence, positivity, and bound hypotheses; it is not an
unconditional three-prime result.  The weighted-phase observer says that a
function-faithful finite-dimensional factorisation would force finite realised
symbolic span, but finite residue does not provide a finite-state quotient, and
no literal infinite-span theorem or analytic endpoint is claimed.  Comparator
deliberately does not badge the paper-authored two-prime transcendence
argument.}
...
$|P|=3$.  In the primary 1988 source, Erd\H{o}s states
...
\cite[p.~61, Theorem~1.1]{bugeaudlaurent2023}.  This is an independent route,
...
\toprule
\textbf{Statement} & \textbf{Authority} & \textbf{Logical reach and nearest boundary} \\
\midrule
\endfirsthead
\toprule
\textbf{Statement} & \textbf{Authority} & \textbf{Logical reach and nearest boundary} \\
...
\endlastfoot
\rowcolor{BrickRed!6}
\multicolumn{3}{@{}l}{\sffamily\bfseries\textcolor{BrickRed!75!black}{A. Open target and prior literature}}\\
Irrationality in any three-prime case & \authnone & Open. \\
Irrationality after deleting duplicate running-LCM values &
\authliterature\ (Erd\H{o}s 1974 letter) & Asserted on p.~335, but no proof is supplied there and
the letter's argument is not recovered here. \\
Transcendence after deleting duplicate values when $|P|=2$ &
\authexternal\ (Bugeaud--Laurent Theorem~1.1) & Unconditional for every pair of
distinct primes; Theorem~\ref{res:two-prime-transcendence}. \\
Transcendence of the original repeated sum when $|P|=2$ &
\authexternal\ (Bugeaud--Laurent Theorem~1.1) & Unconditional for every pair of
distinct primes; Theorem~\ref{res:two-prime-repeated-transcendence}.  Neither
two-prime theorem applies to three primes. \\
\addlinespace[2pt]
\rowcolor{MidnightBlue!6}
\multicolumn{3}{@{}l}{\sffamily\bfseries\textcolor{MidnightBlue!82!black}{B. Unconditional finite structure}}\\
$\Lprefix(x)=\hgt(x)$ & \authlean & Unconditional for distinct primes; Theorem~\ref{res:lcm}.  The unrestricted identity is classical. \\
$\hgt(x)\le x^{3}$ & \authlean & Unconditional; Proposition~\ref{res:cube}. \\
Constancy on logarithmic cells & \authlean & Unconditional; Theorem~\ref{res:cell}. \\
Single-coordinate jump ratios & \authlean & Unconditional; Theorem~\ref{res:jump}. \\
Exactly $3n+1$ jump points & \authlean & Unconditional; Theorem~\ref{res:count}. \\
Height-fibre normal form & \authlean & Exact finite identity; Theorem~\ref{res:fibre}. \\
Infinite limit of that expansion & \authnone & Not proved here; Section~\ref{sec:fibre}. \\
Shell multiplicity $9\,\#\mathcal S\le(j+3)^{2}$ & \authlean & Unconditional; Theorem~\ref{res:shell}. \\
$2\times2$ kernel restriction has rank two at $\{2,3,5\}$ & \authlean & Unconditional local obstruction; determinant $-1/15$ in Theorem~\ref{res:rank}. \\
Arbitrary-order nonsingular kernel minors and no finite exact separation & \authlean & Unconditional structural obstruction; Theorem~\ref{res:infinite-rank}. \\
Actual infinite shell recurrence and integral-or-cofinally-$1/31$-far dichotomy & \authlean & Exact for the literal $\{2,3,5\}$ orbit; Theorem~\ref{res:actual-orbit}; the integral branch remains. \\
All-scale $1/q$ lattice and forced collision under rationality & \authlean & Exact reduction; Theorem~\ref{res:all-scale-lattice}; collision exclusion remains open. \\
Pinning, upward closure and window rigidity of the integral branch & \authlean & Exact structural analysis; Theorem~\ref{res:pinning}; it does not exclude a surviving integral seed. \\
Dyadic block radix lies in $\{2,6,10,30\}$ & \authlean & Unconditional; Theorem~\ref{res:dyadic-alphabet}. \\
Canonical least-positive-residue arithmetic & \authlean & Unconditional finite lemma; Theorem~\ref{res:consumer}. \\
\addlinespace[2pt]
\rowcolor{BurntOrange!7}
\multicolumn{3}{@{}l}{\sffamily\bfseries\textcolor{BurntOrange!85!black}{C. Conditional bridge}}\\
Carry contradiction assuming~\eqref{eq:escape} & \authlean & Conditional implication only; Theorem~\ref{res:windowconsumer}. \\
Smooth-factor absorption and carry cancellation & \authlean & Conditional core; Theorem~\ref{res:denominator-reduction}.  Divisibility of the actual carry remains unproved. \\
\addlinespace[2pt]
\rowcolor{Violet!6}
\multicolumn{3}{@{}l}{\sffamily\bfseries\textcolor{Violet!80!black}{D. Bounded computation and remaining frontier}}\\
Dyadic-window scan for $B\le1000$ & \authcompute & $106{,}666$ pairs with no failures; bounded evidence, not a theorem. \\
Bridge from the actual summands to $(b_n,m_n,K)$ & \authnone & Not proved here; Section~\ref{sec:open}. \\
Local-window escape for the actual $\{2,3,5\}$ word & \authnone & Open; Problem~\ref{prob:producer}. \\
\end{longtable}
}
...
revision~\commitshort; the two transcendence theorems are analytic arguments
using the cited external theorem and are not Lean-formalised here.
...
$f(i)g(j)h(k)$ on this box.  The source-current argument does not stop there.
...
\begin{proof}
Factoring the pure row and column powers from the kernel leaves the carry
matrix associated with the fractional parts of $i\log_r p$ and
$j\log_r q$.  Distinctness of the primes makes both logarithmic rotations
nonperiodic.  For each $n$ one may choose their fractional parts in opposite
orders, giving the staircase matrix with entry $r^{-1}$ on and below the
diagonal and entry $1$ above it.  Its determinant is
$r^{-1}(r^{-1}-1)^{n-1}$, which is nonzero; the removed row and column factors
are nonzero as well, uniformly in $k$.  A representation with $d$ summands
...
The synchronized Mathlib-only Comparator package
\texttt{ExternalVerification269ThreePrimeStructure} exposes the whole checked
structural tier rather than only the final rank obstruction: the exact
running-LCM identity, logarithmic-cell constancy, the $3n$ positive-jump
count, the finite height-fibre normal form, the quadratic shell-multiplicity
bound, the determinant $-1/15$, and the arbitrary-order/no-finite-separation
endpoint.  These seven declarations sharpen the verifiable finite geometry;
none supplies the still-missing irrationality producer.
...
no seed survives.  The three theorem families in this section are
source-current Lean results; their public source registration is pending, so
the displayed arguments rather than the paper's pinned links are the present
public proof record.
...
The same finite arithmetic gives an exact classifier, not merely a
contradiction.  If $C>0$, $c>0$, $|c|\le C$, and
...
it to the series still requires two independent results: rationality
must produce a positive bounded integral carry with the required
smooth-divisibility congruence, and one must construct arbitrarily late
escaping windows.
...
packages the preceding cancellation and reduced-carry consumer into one
...
contradiction.  This removes a manual composition step; it does not prove that
the actual series supplies the factorisation, the carry recurrence, or the
...
statement is best separated from the bridge and from the possible methods for
proving it.

\subsection{The actual block data and the missing bridge}
...
\begin{problem}[actual rationality-to-carry identification]\label{prob:bridge269}
Starting from the original repeated $\{2,3,5\}$ series and a reduced
denominator $D=D_{\mathrm{sm}}B$, prove with a stated onset $a\ge a_D$ that
the literal objects in~\eqref{eq:actual-digit}--\eqref{eq:actual-tail} give
positive integral carries
\[
 c_a=DT_a,\qquad
 c_{a+1}=\beta_ac_a-Dm_a^{235},\qquad
 0<c_a\le K^{235}(D,a),\qquad D_{\mathrm{sm}}\mid c_a.
\]
Writing $c_a=D_{\mathrm{sm}}d_a$, prove the reduced recurrence and its sharp
bound
\[
...
This is the missing connection, not a notational convenience: the generic
carry theorems of Section~\ref{sec:escape} do not identify themselves with the
original series.  The problem fixes the digit, tail, onset and bound against
which a proposed proof can be tested.
...
Problem~\ref{prob:tails269}, joined to Problem~\ref{prob:bridge269}, bypasses
all residue-window machinery.
...
quantifier is present for a substantive reason: Problem~\ref{prob:bridge269}
may supply the reduced recurrence only after the denominator-dependent onset
$a_D$, and~\eqref{eq:actual-escape} then supplies a window beyond that onset.
...
\subsection{A literal two-dimensional analytic route}
...
(\lword{Erdos269/WeightedPhaseCarry.lean}{109}{carry_eq_residueDigit_add_coboundary}{residue--coboundary form},
\lword{Erdos269/WeightedPhaseCarry.lean}{293}{CartierRealisation}{system},
...
\subsection{The structural frontier}
...
one false route in four exact steps.  Under channel surjectivity, ordinary
...
and supply no independent authority. Lean checks each proof term against the fixed library
...
\bibitem{bugeaudlaurent2023} Y.~Bugeaud and M.~Laurent,
  ...
  doi:\href{https://doi.org/10.4064/aa220323-18-1}
  {10.4064/aa220323-18-1};
