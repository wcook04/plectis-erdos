...
\begin{abstract}
\noindent
For two distinct primes, retain one reciprocal for each distinct value of the
running least common multiple, rather than repeating that reciprocal at every
smooth integer.  We prove that the resulting de-duplicated sum is
transcendental.  The proof orders the two pure-power channels by a Beatty word,
writes the sum as a nonconstant algebraic affine function of a Hecke--Mahler
value, and invokes Bugeaud--Laurent's transcendence theorem.  This is the
unconditional irrationality conclusion of the note.  It applies neither to
the original series with repetitions nor to any three-prime instance.

We then turn to the smallest three-prime case.  Let $p,q,r$ be pairwise
distinct primes and let $\Lprefix(x)$ denote the least common multiple of all
$\{p,q,r\}$-smooth numbers not exceeding $x$.  We prove
...
least-positive-residue classifier}.  It remains a finite consumer.  Applying
it to the series still requires two independent upstream inputs: rationality
...
original repeated series remains open even there.  At three primes the exact
unresolved statement is best separated from the bridge and from the possible
...
Every noun in that chain is still conditional.  No theorem constructs the
