...
\renewcommand{\commit}{35c358a85bdbd12afbaac685ec331185549d3d99}
\renewcommand{\commitshort}{35c358a85bdb}
\hypersetup{
...
of $\Lprefix$ into the blocks cut out by consecutive powers of two.  Each open
...
alphabet $\{2,6,10,30\}$, rather than merely lying in a coarse bounded interval.  Second, the reciprocal of the running least common multiple
...
height, with each coefficient the cardinality of the corresponding fibre, and a
uniform bound $9\,\#\mathcal S\le(j+3)^{2}$ on the cardinality of a smooth
...
positive reduced integral carry obeying the matching bound.  Lean checks the
absorption-and-cancellation core and the resulting implication.  An
...
rather than approximate.  Theorem~\ref{res:dyadic-alphabet} determines the
alphabet of the dyadic compression of the jump word exactly: the radix
$\beta_a$ of the block between $2^{a}$ and $2^{a+1}$ takes one of the four
...
integral carry states share the factor $D_{\mathrm{sm}}$, cancel it, and obtain
a positive reduced carry $d_n$ bounded by $K(B,n)$ and satisfying
$d_{n+1}=b_nd_n-Bm_n$.  Fourth, find a window over which the accumulated base
and forcing put the residue of that carry above the bound, which is impossible.
...
\begin{proof}
The identity is a regrouping.  Partition $B$ into the fibres of the height map.  On $F(H)$ every summand is
...
the problem to one-dimensional criteria.  Theorem~\ref{res:rank} shows that
this already fails at $\{2,3,5\}$, and that the failure is exact.
...
identification of the corresponding block digit with the multiplicity forcing
...
are being considered.  Every fixed $\{2,3,5\}$-smooth factor divides the
...
just recorded.  For an integer radix word $b_n$ and forcing word $m_n$, define
\[
\begin{aligned}
 W_{\ell,0}&=1,
...
statement.  The exact unproved proposition is the
\lword{Erdos269/RestrictedFloorSum.lean}{594}{CofinalLocalWindowEscape}{cofinal
local-window escape condition}.

The next statement is the finite core of the argument.  The key point is the
