...
The middle column of the table below separates categories that are easy to
conflate, and the reader who wants only the boundary can read that column
alone.  \emph{Proved here} means that this note gives a proof and links the
corresponding checked Lean declaration; a qualifier attached to it
(\emph{finite}, \emph{conditional core}) names the restriction under which the
statement was proved.  \emph{Exact finite computation} is evidence and not a
theorem: the dyadic-window scan of Section~\ref{sec:escape} settles
$106{,}666$ instances and asserts nothing beyond them.  \emph{Not proved}
marks a statement that this note does not establish, and \emph{Open} marks one
for which no proof is known.
...
$\Lprefix(x)=\hgt(x)$ & Proved here & Theorem~\ref{res:lcm}; needs the primes distinct; classical in the unrestricted case. \\
...
$2\times2$ kernel restriction has rank two at $\{2,3,5\}$ & Proved here & Determinant $-1/15$; Theorem~\ref{res:rank}. \\
Dyadic block radix lies in $\{2,6,10,30\}$ & Proved here & Theorem~\ref{res:dyadic-alphabet}. \\
Canonical least-positive-residue arithmetic & Proved here & Theorem~\ref{res:consumer}. \\
Carry contradiction assuming~\eqref{eq:escape} & Proved here & Theorem~\ref{res:windowconsumer}; the implication only. \\
Smooth-factor absorption and carry cancellation & Proved here; conditional core & Theorem~\ref{res:denominator-reduction}; divisibility of the actual carry remains unproved. \\
Dyadic-window scan for $B\le1000$ & Exact finite computation & $106{,}666$ pairs; no failures; not a theorem. \\
Bridge from the actual summands to $(b_n,m_n,K)$ & Not proved & Section~\ref{sec:open}. \\
Local-window escape for the actual $\{2,3,5\}$ word & Open & Problem~\ref{prob:producer}. \\
\bottomrule
\end{tabular}
\caption{Status of the statements discussed in this note.}
\end{table}
