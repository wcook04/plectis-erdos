...
\title{The Running Least Common Multiple of a Three-Prime Smooth Prefix}
\subtitle{Lean-checked structure for Erd\H{o}s Problem~\#269}
\author{Will Cook}
\date{July 2026}
...
and no transcendence statement in any three-prime case.  The residue-escape
route is represented only by its final step, a finite contradiction whose
escape hypothesis is assumed and not proved; we state that hypothesis
explicitly, and it is the obligation that survives.
\end{abstract}
...
proof, in the \emph{formal-conjectures} collection.  That is a formal statement
of the question.  The declarations described below are propositions about the
objects the question is posed over, and no claim of priority is made for any of
them.
...
Residue escape occurs & Assumed, not proved & Section~\ref{sec:escape}. \\
Finite contradiction consuming it & Proved here & Theorem~\ref{res:consumer}. \\
\bottomrule
\end{tabular}
\end{table}
...
records the obstruction to separating the kernel.  Section~\ref{sec:escape} is
the one place where an unproved hypothesis appears, and it is marked as such
throughout.  Linked phrases open the corresponding Lean declaration at the
...
\section{The residue-escape route and what it assumes}\label{sec:escape}

One strategy for the three-prime case runs as follows.  Suppose the value is
rational with denominator $C$.  The jump expansion produces an integer tail
state that is positive and bounded by a polynomial window; if one can force the
state to be congruent modulo $C$ to a least positive residue lying strictly
above that window, the two facts are incompatible and the assumed rationality
fails.  The final step is elementary and we record it; the step that would make
the strategy an argument is the production of the escaping residue, and it is
not proved.

Say that a residue \emph{escapes} the window when
$\text{bound}<\text{residue}\le C$, the
\lword{Erdos269/ResidueEscape.lean}{17}{ResidueEscapesWindow}{escape condition}.

\begin{theorem}[the finite contradiction]\label{res:consumer}
Let $c,C,\text{bound},\text{residue}$ be natural numbers with $0<c$ and
$c\le\text{bound}$, and suppose $c\equiv\text{residue}\pmod C$.  If the residue
escapes the window, the hypotheses are contradictory.  Equivalently, under the
same congruence, $\text{residue}\le\text{bound}$.
\end{theorem}

\begin{proof}
From $c\le\text{bound}<\text{residue}\le C$ we get $c<C$, so $c$ and
$\text{residue}$ are each their own least residue modulo $C$ unless
$\text{residue}=C$, in which case its least residue is $0$; in either case the
congruence forces $c=\text{residue}$ or $c=0$, and both contradict
$c\le\text{bound}<\text{residue}$ and $0<c$.
\end{proof}

\noindent Formalised as the
\lword{Erdos269/ResidueEscape.lean}{23}{no_bounded_positive_state_of_residue_escape}{finite
contradiction} and the
\lword{Erdos269/ResidueEscape.lean}{44}{residue_le_bound_of_bounded_positive_state}{contrapositive
form}.

We state plainly what this is.  Theorem~\ref{res:consumer} is two lines of
arithmetic about residues and contains no information about primes, smooth
numbers, or least common multiples.  It is the trivial end of the strategy,
recorded so that the nontrivial end is visible as the only thing missing.  The
hypothesis that an escaping residue occurs, for every putative denominator and
beyond every level, is exactly the open producer; assuming it is not progress
towards it.

\section{Complements and further questions}\label{sec:open}

Problem~\#269 is open.  Three obligations survive the results above, in
decreasing order of directness.

\begin{problem}[uniform residue escape]
Show that under the rational carry recurrence some prime-power channel carries
a nonzero residue beyond every finite level, uniformly in the putative
denominator.  Theorem~\ref{res:consumer} then closes the argument;
Theorem~\ref{res:shell} supplies the polynomial window the escape must clear.
\end{problem}

\begin{problem}[a genuinely three-dimensional analytic theorem]
Prove an analytic statement for the phase cocycle attached to three
independent logarithmic coordinates, of the kind available in one dimension.
By Theorem~\ref{res:rank} no separation into one-dimensional factors is
available even at $\{2,3,5\}$.
\end{problem}

\begin{problem}[unbounded denominator exclusion]
Replace a finite denominator exclusion by a family whose exclusion bound tends
to infinity.  A finite computation excludes all denominators below its bound
and none above it, so no single such computation, at any size, bears on
irrationality.
\end{problem}

What the formalised material changes is the description of the object, not its
status.  The running least common multiple is now an explicit function of three
integer logarithms; its jumps are exactly the pure prime powers; a lattice sum
groups exactly by height with multiplicities bounded quadratically; and the
coordinates provably do not separate.  None of this produces the escaping
residue, and the problem remains open.
...
revision~\commitshort.  The structural material is in the first module and the
residue consumer in the second.  Three distinctions are worth carrying into the
source.  The height statements hold for arbitrary bases, while the statements
about $\Lprefix$ need the three primes to be distinct.  The normal form of
Section~\ref{sec:fibre} is an identity between finite sums over a rectangular
box, not over a smooth prefix.  And the escape condition of
Section~\ref{sec:escape} is a hypothesis of its theorem, not a result.
...
escape condition & \lrefx{Erdos269/ResidueEscape.lean}{17}{ResidueEscapesWindow}\\
finite contradiction & \lrefx{Erdos269/ResidueEscape.lean}{23}{no_bounded_positive_state_of_residue_escape}\\
contrapositive form & \lrefx{Erdos269/ResidueEscape.lean}{44}{residue_le_bound_of_bounded_positive_state}\\
\end{tabular}
\fi
