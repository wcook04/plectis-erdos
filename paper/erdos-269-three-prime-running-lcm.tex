...
\renewcommand{\commit}{6fe669c2c773d209dbf5d763891820a555c32cd0}
\renewcommand{\commitshort}{6fe669c2c773}
\hypersetup{
...
edge cases.  What remains unproved is the producer: the actual ordered
$\{2,3,5\}$ word must supply those escaping windows cofinally.
\end{abstract}
...
The most direct mechanism is channel-sensitive anti-concentration.  One seeks
windows ending after controlled $2$-, $3$-, or $5$-power jumps for which the
...
to $0$ modulo that modulus.  A theorem for one cofinal family of channel
windows is enough; no equidistribution of all windows is required.

Three shortcuts are already disconfirmed.  A fixed finite list of denominator
certificates does not address the universal $B$-quantifier.  A bound
independent of $B$ is not the bound delivered by denominator clearing.
...
For the actual ordered $\{2,3,5\}$ jump expansion, define the resulting radix
word $b_n$, forcing word $m_n$, and denominator-cleared bound $K(B,n)$, and
prove~\eqref{eq:escape} for every $B>0$ coprime to $30$ and every starting
index.  Equivalently, prove a cofinal subfamily of windows on which
\[
...
prime-channel statement that still emits one escaping window beyond every
level for each fixed $B$.  The answer must state all dependence on $B$ and the
window endpoint.

\paragraph{Already checked.}
Lean checks the window recurrence, the least-positive-residue interval and
congruence lemmas, the denominator-dependent producer type, and the complete
...
checked.  No theorem here constructs the actual words $b_n,m_n$, derives
$K(B,n)$ from a rational value, or proves~\eqref{eq:escape}.

\paragraph{Most useful answer.}
Give an explicit cofinal window rule---for example by a specified pattern of
prime-power channels---together with a lower bound for the canonical residue
and an upper bound for $K(B,\ell+h)$.  A negative answer is also valuable if it
constructs infinitely many $(B,\ell_0)$ for which every proposed window family
fails; that would retire the present producer rather than merely weaken it.
...
a rectangular box, not a convergence theorem.  And
\texttt{CofinalLocalWindowEscape} is an unproved proposition consumed by a
theorem; its application to the actual $\{2,3,5\}$ word is not asserted.
