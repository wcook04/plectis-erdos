...
\renewcommand{\commit}{d6d344d9557faee19276b01a1c1f20d7c08f03d5}
\renewcommand{\commitshort}{d6d344d9557f}
\hypersetup{
...
For comparison, we settle the de-duplicated problem when there are exactly two
primes.  If one retains one reciprocal for each distinct running-LCM value,
the resulting sum is transcendental for every pair of distinct primes.  The
proof orders the two pure-power channels by a Beatty word, writes the sum as a
nonconstant algebraic affine function of a Hecke--Mahler value, and invokes
Bugeaud--Laurent's transcendence theorem for that value.  This conclusion is
stronger than the irrationality asserted, without proof, in Erd\H{o}s's 1974
letter, but it applies neither to the repeated series nor to three primes.
...
the mathematical claims.  The problem is open for every finite $P$ with
$|P|\ge2$.  In the primary 1988 source, Erd\H{o}s states
...
Section~\ref{sec:two-prime} proves that this de-duplicated sum is in fact
transcendental when $|P|=2$, by reducing it to the Hecke--Mahler value covered
by Bugeaud and Laurent's Theorem~1.1
\cite[p.~61, Theorem~1.1]{bugeaudlaurent2023}.  This is an independent modern
route, not a recovery of the unprinted argument in the letter.  Sections
...
infinite passage and say nothing about the original series with repetitions.
...
distinct primes; Theorem~\ref{res:two-prime-transcendence}.  It does not apply
to repetitions or to three primes. \\
\addlinespace[2pt]
...
Section~\ref{sec:two-prime} uses that structure to prove transcendence of the
de-duplicated two-prime sum.  Section~\ref{sec:fibre} gives the finite
...
revision~\commitshort; Theorem~\ref{res:two-prime-transcendence} is an analytic
argument using the cited external theorem and is not Lean-formalised here.
...
\section{The de-duplicated two-prime sum is transcendental}
\label{sec:two-prime}
...
\cite[p.~61, Theorem~1.1]{bugeaudlaurent2023}.  We may take
...
\noindent
This proves more than the irrationality claimed for the de-duplicated sum in
Erd\H{o}s's letter, but only when $|P|=2$.  The repeated sum assigns a
multiplicity to each running value, and a third prime replaces the one
Beatty boundary by a genuinely two-dimensional ordering problem; neither is
covered by Theorem~\ref{res:two-prime-transcendence}.
...
Theorem~\ref{res:two-prime-transcendence} closes the de-duplicated question
for two primes, and does so at the stronger level of transcendence.  The
original series with repetitions remains open even there, while the
three-prime de-duplicated series is outside the one-dimensional Hecke--Mahler
reduction used in that theorem.
