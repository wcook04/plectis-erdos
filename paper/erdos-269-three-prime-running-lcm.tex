...
determinant is exactly $-1/15$.  Thus that matrix has rank two, which excludes
every argument that first separates the three coordinates and then treats them
one at a time.
...
exactly one logarithm advances by one; and exactly $3n+1$ distinct jump values
...
particular, it is not a product $f(i)g(j)h(k)$, so every argument that first
separates the three coordinates and then treats them one at a time is excluded.
...
Exactly $3n+1$ jump values & Paper; Lean source & Unconditional; Theorem~\ref{res:count}. \\
...
\section{Constancy on logarithmic cells and the jump values}\label{sec:cells}
...
separates the coordinates and then treats them one at a time.  It gives no
...
All four letters already occur among the first six blocks:
\[
...
With $b_a=\beta_a\in\{2,6,10,30\}$ the block radix of
Theorem~\ref{res:dyadic-alphabet}, and with $m_a$ and $K(B,a)$ as in the
identification above, prove~\eqref{eq:escape}: for every $B>0$ coprime to $30$
...
equidistribution of all windows is required.
...
An analytic treatment of the three logarithmic coordinates would have to be
genuinely three-dimensional: by Theorem~\ref{res:rank}, a preliminary
