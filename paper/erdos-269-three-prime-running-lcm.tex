...
\renewcommand{\commit}{c9b1a7434f558a6998de5c895cd685abf4a3ac7a}
\renewcommand{\commitshort}{c9b1a7434f55}
\hypersetup{
...
is conditional: for a positive reduced denominator $B$ coprime to $30$, a
denominator-dependent cofinal local-window residue escape rules out every
positive integral carry obeying the matching bound.  Lean checks that
implication.  An integer-only checker constructs the corresponding block
digits, reproduces three small certificates, and finds an escaping window of
length at most $18$ for each of $106{,}666$ tested denominator/start pairs.
This is finite experimental evidence, not a cofinal theorem.  Two statements
are not proved here: the identification of the actual block digits and of the
denominator-cleared carry bound with the formal recurrence, and the cofinal
escape property itself.
\end{abstract}
...
denominator $B$ coprime to $30$, clear denominators, and obtain a positive
integral carry $d_n$ bounded by $K(B,n)$ and satisfying
...
formalised here.  The instantiation in the third stage, and the cofinal
existence of windows in the fourth, are not; they are the two obligations of
Section~\ref{sec:open}.  The third and fourth stages follow a pattern standard
in irrationality proofs; what is specific here is that the radix word driving
the carry is the block alphabet of Theorem~\ref{res:dyadic-alphabet}.
...
is an identification.  The ordered $\{2,3,5\}$ jump word, its block digits
$m_a$, and the bound $K(B,a)$ obtained by clearing denominators in a
hypothetical rational value would have to be shown to produce a carry $d_n$
satisfying $d_{n+1}=b_nd_n-Bm_n$ with the radix word $b_a=\beta_a$ of
Theorem~\ref{res:dyadic-alphabet}.  The second is the Diophantine existence
statement of Problem~\ref{prob:producer}.  No Lean theorem yet identifies those
digits with the formal forcing word, derives $K(B,a)$ from a rational value, or
...
predicate is an unproved hypothesis of Theorem~\ref{res:windowconsumer}; its
application to the actual $\{2,3,5\}$ word is not asserted.
