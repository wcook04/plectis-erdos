...
posted the factorisation, reduction and conclusion first, on 26 June 2026.
The argument recorded here uses that external theorem; it is not a Lean
formalisation of the two-prime result.
...
$\Lprefix$ at a cutoff.  Fan and Lott build their van der Corput coefficients
on sums of two squares by the same kind of regrouping: up to normalisation, the
coefficient at $d$ sums a smooth weight over the representations
$d=v^2(x^2+y^2)$, and the average over $v$ keeps the associated complete sums in
a fixed sector \cite[\S3 and Corollary~4.4]{fanlott2026}.  The same Lean
...
a countable set, a phenomenon going back to Eggleston
\cite[\S3 and Lemma~3.1]{fan2026strongly}.  For an integer $B\ge1$ and $g_a=Bh_a$ $(a\ge1)$ the ratios
...
the direct smooth-number sum as an exact rational. The machine-readable witness and replay receipt for this large computation are not included in the public release, so the statement remains an archived ordinary computational certificate rather than a publicly replayed or Lean-checked result.  The recorded statistics are
...
\paragraph{Evidence.}  Steve Fan's two-prime argument is an ordinary proof over
the cited Bugeaud--Laurent theorem and carries no Lean declaration.  The
...
bound, the explicit onset $a_D$, the sharp bound $K$, and
...
applies to arbitrary coprime pairs.  The manuscript of the
