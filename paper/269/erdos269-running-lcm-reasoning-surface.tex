...
nonsingular minors of every order, with the same indices valid in every
third-coordinate layer.  Removing row and column factors leaves a binary
floor carry.  Its threshold columns give both the arbitrary-order rank
obstruction and the exact rank of a finite sample.  The normalised carry
also has a sharp uniform finite-rank approximation barrier, although the
original summable kernel admits finite-rank approximation in the summation
norm.  These are statements about representations, not irrationality of
the resulting scalar sum.

For two distinct primes, both the repeated and de-duplicated running-LCM
series are transcendental.  They are nonconstant polynomials in one
Hecke--Mahler value, whose transcendence follows from the theorem of Loxton
and van der Poorten in the form given by Bugeaud and Laurent.  Steve Fan
posted the factorisation, reduction and conclusion first, on the problem's
forum thread on 26 June 2026.  The record gives an independent proof over that
external theorem; the two-prime result is not formalised in Lean.

For the repeated $\{2,3,5\}$ series, the complete record constructs the
literal tail recurrence and proves that rationality forces eventually
positive integral tails under a quadratic bound.  Escape of the exact
window residues is equivalent to irrationality.  Finite certificates test
that criterion and exclude bounded denominator ranges, but do not supply
its unbounded quantifiers.  The record gives the proofs, exact finite
evidence and failed representation methods behind the short paper;
the repeated-series problem with three or more primes remains open.
\end{abstract}
...
catalogue records the problem as open~\cite{erdosproblems}.
...
first~\cite{fan2026comment}; the unresolved finite cases begin at $|P|=3$.
...
2026~\cite{fan2026comment}.  A reply in that thread observed that the argument
applies to arbitrary coprime pairs, and Fan's response proposed the wider
setting of nonzero algebraic numbers whose logarithms are linearly independent
over $\Q$.  The priority for the two-prime case is his.  The
proof below was found independently and is given in full because it fixes the $|P|=2$
...
primes $t\le N$, equivalently $\log\lcm(1,\ldots,N)=\psi(N)$; see
Apostol~\cite{apostol1976} and Montgomery and
Vaughan~\cite{montgomeryvaughan2007}.  Fan's forum post states the
$P$-restricted form for every $|P|$~\cite{fan2026comment}.  The pairwise distinctness hypothesis is
...
$\Lprefix$ at a cutoff.  The same Lean
module records the one-step map
$\tau(b,d,s)=b(s-d)$, the
...
\noindent For integers $p,q>1$ this is checked as the
...
If $(g_a)$ is an unbounded sequence of positive integers with bounded
successive ratios, the real $\xi$ with $\operatorname{dist}(g_a\xi,\Z)\to0$ form
a countable set.  For increasing sequences this is due to Erd\H{o}s and Taylor
...
\cite[Lemma~3.1, p.~7]{fan2026strongly}.  For an integer $B\ge1$ and $g_a=Bh_a$ $(a\ge1)$ the ratios
are the integers $b_a$ of~\eqref{long269:eq:dyadic-alphabet}, and the argument
above identifies the set: it consists of the $\xi$ for which $Bh_a\xi$ is
eventually an integer.  Since $X_a-h_aS$ is an integer for $a\ge1$
(Lemma~\ref{long269:res:all-scale-lattice}),
Theorem~\ref{long269:res:actual-cancellation} shows that the set contains $S$
for some $B$ exactly when $S$ is rational.
...
The enlarged little-$o(8^a)$ band, with the cap-domination hypothesis retained, is checked by the
\lword{Erdos269/R12/OcticWindowBand.lean}{74}{octic_window_band}{octic window-band equivalence}.  The forward direction at the fixed bound $K_0$ is
...
with $G(B,a)=o(2^{a})$, in particular to any subexponential bound above $K$,
...
produced it, and the certificate record is not part of the public source of
this release.
...
the direct smooth-number sum as an exact rational. The machine-readable witness and replay receipt for this large computation are not included in the public release, so the statement is an ordinary computational certificate that has been neither publicly replayed nor checked in Lean.  The recorded statistics are
...
depth at $a=1$.  Denominators beyond the recorded finite ranges are not
excluded by either certificate, so neither settles an instance of the problem.
...
smooth-factor cancellation, the explicit onset and the explicit quadratic carry bound all
...
exclude every finite exact separation of the kernel into rational factors,
and Theorem~\ref{long269:res:uniform-rank} excludes even approximate separation
of its normalised carry below half a jump.  The bounded-radix alternative
leaves integral tails untouched.  The residue countermodels show that coprimality
...
\paragraph{Evidence.}  The two-prime theorem has an ordinary proof in this
record over the Hecke--Mahler value theorem of Loxton and van der Poorten, in
the form of Bugeaud and Laurent, and carries no Lean declaration; Steve Fan
posted the same argument first~\cite{fan2026comment}.  The
arbitrary-order kernel minors and the exclusion of finite separation are Lean
theorems in \texttt{KernelCarryRank}.  The named source statements for the
literal recurrence and fixed-split rationality bridge are Lean theorems as
well.  The
\href{https://github.com/wcook04/plectis-erdos/blob/06f57893e0f9c6d80668048f26f096367ae4cfe7/verification/erdos269-wavea-validation.json}{source-current validation receipt}
byte-matches these modules and records their compilation inside the public
recursive import closure; its named declaration-by-declaration axiom audit is
for the R12 Wave~A results, not for these rank declarations.  Comparator names
the finite $(2,3,5)$ minor $-1/15$, the running-LCM height identity, and a
conditional carry-escape consumer; it is not the evidence for the
arbitrary-order theorem.  The actual-series reduction is given;
the source-specific cofinal escape remains unproved.  Named Lean sources below
are locators, not a blanket end-to-end coverage claim.  The quadratic tail
bound, the explicit onset $a_D$, the explicit bound $K$, and
Theorem~\ref{long269:res:uniform-rank} are ordinary proofs in this paper; the
three finite computations of Section~\ref{long269:sec:evidence} are
...
applies to arbitrary coprime pairs.  The note's two-prime argument was found
independently, and its manuscript was first released publicly on 22 July 2026,
at commit \texttt{a9d3ab8}.
...
they are recorded here in the generality in which they are checked.  They are
conditional carry lemmas in the Cantor-series tradition described after
Theorem~\ref{long269:res:actual-orbit}, and each assumes its divisibility or
escape hypothesis.
...
Each link is pinned to its own immutable revision; the exact-reference inventory
records the revisions and their comparison with the current public source commit.
...
\textbf{Reader entry.} Both two-prime running-lcm series are transcendental, by an independent deduction from the Hecke--Mahler value theorem of Loxton and van der Poorten in the form of Bugeaud and Laurent; Steve Fan posted the same argument first on the problem's forum thread.
...
\textbf{Reader entry.} A local-window checker searches $3{,}869{,}934$ denominator and start pairs, after an earlier run over $106{,}666$, and records small certificates.
