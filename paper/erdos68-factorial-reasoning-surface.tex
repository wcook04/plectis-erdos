...
continued-fraction certificate gives $q\ge2^{39991}>10^{12039}$.  For a finite
...
The definitions and claim boundary are repeated here so that the note is
self-contained.
...
converse is not yet kernel-checked here, and nothing below uses it as though it
were.
...
\]
The forward implication is the checked
\lword{Erdos68/FactorialGapPlateauCore.lean}{945}{irrational_factorialGapSeries_of_cofinal_nonunit_carries}{cofinal non-unit-carry criterion}.
In purely integral coordinates, the checked
\lword{Erdos68/FactorialGapPlateauCore.lean}{986}{irrational_factorialGapSeries_iff_cofinal_strictFacTopRat_misses}{strict-successor equivalence}
gives the same reduction without loss.  Neither theorem produces the required
cofinal witnesses.  Exact
...
payload, and receipt digests form the certificate archive.  The two
...
\]
This is a formal theorem, not a heuristic reduction.  The remaining gap is
quantified: the finite mechanisms in the preceding sections need one of the
following cofinal inputs.  The table separates those missing inputs from the
formal results that would consume them.
...
for this note.  The pinned snapshot contains seventeen cited modules:
...
\texttt{FactorialGapPlateauCore},
\texttt{FactorialZeroPlateau},
\texttt{FactorialZeroPlateauCertificates},
