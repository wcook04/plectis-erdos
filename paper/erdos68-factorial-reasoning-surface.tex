...
$300000$, and $q\ge2^{39990}>10^{12038}$ from a certified continued-fraction
enclosure.  The irrationality of $\Ser$ is open, and the remaining
arithmetic inputs are stated in the last section.
\end{abstract}

\section{Which prime powers survive reduction}\label{sec:prime-powers}
...
\begin{theorem}[maximal prime-power survival]\label{res:prime-pole}
...
\begin{equation}\label{eq:prime-pole-survival}
 v_p\bigl(\den(H_M)\bigr)=e
 \quad\Longleftrightarrow\quad
 \sum_{n\in J}u_n^{-1}\ne0\quad\hbox{in }\mathbb F_p.
\end{equation}
  The statement is checked in Lean: \lean{maximal_prime_power_survival}{ErdosProblems/Erdos68/PaperCompletePrimePole.lean:118}.
\end{theorem}
...
\begin{equation}\label{eq:prime-pole-residue}
...
displayed denominators, so it applies to any finite sum of reciprocals; its
general antecedent is unassessed.  What is specific here is the behaviour of
...
Theorem~\ref{res:prime-pole}, $139\nmid\den(H_{138})$ and
...
\begin{proposition}[cofinal first prime occurrences]\label{res:wilson-cofinality}
...
\S\ref{sec:residue} consume.  Wilson reflection limits what can be inferred
...
\section{The growth of the common denominator}\label{sec:lcm}

Theorem~\ref{res:prime-pole} concerns the denominator after reduction.  The
...
\begin{lemma}[factorial-gap gcd]\label{res:gap-gcd}
For $2\le i<j$, the integer $g=\gcd(i!-1,j!-1)$ divides $j!/i!-1$, and
$g\le j!/i!-1<j^{\,j-i}$.
  The statement is checked in Lean: \lean{factorial_gap_gcd}{ErdosProblems/Erdos68/PaperCompleteGcdSegment.lean:28}.
\end{lemma}
...
\begin{lemma}[segment inequality]\label{res:segment}
For $2\le k\le N-1$,
\begin{equation}\label{eq:segment}
 \sum_{n=N-k+1}^{N}\log(n!-1)
 \ \le\ \log \Lc_N+\binom{k+1}{3}\log N.
\end{equation}
  The statement is checked in Lean: \lean{segment_inequality}{ErdosProblems/Erdos68/PaperCompleteGcdSegment.lean:73}.
\end{lemma}

\begin{proof}
Apply Lemma~\ref{res:product-lcm} to $x_n=d_n$ for $N-k+1\le n\le N$.  Their
least common multiple divides $\Lc_N$.  By Lemma~\ref{res:gap-gcd} each
pairwise gcd is smaller than $N^{\,j-i}$, and over a block of $k$ consecutive
indices
\[
 \sum_{i<j}(j-i)=\sum_{d=1}^{k-1}d(k-d)=\binom{k+1}{3}.
\]
Taking logarithms of the resulting divisibility gives \eqref{eq:segment}.
\end{proof}

\begin{theorem}[common-denominator growth]\label{res:lcm-growth}
\[
 \liminf_{N\to\infty}\frac{\log \Lc_N}{N^{3/2}\log N}
 \ \ge\ \frac{2\sqrt2}{3}.
\]
  The statement is checked in Lean: \lean{common_denominator_growth_liminf}{ErdosProblems/Erdos68/PaperCompleteLiminf.lean:42}.
\end{theorem}
...
For the left-hand side of \eqref{eq:segment}, use $n!-1\ge n!/2$ and
...
ingredients are Lemmas~\ref{res:product-lcm} and~\ref{res:gap-gcd}, and it
...
$O(N^{2/3})$ the number of solutions of $n!\equiv a\pmod r$ in an interval of
length $N<r$; that deduction is recorded in the complete reasoning record and
...
Theorem~\ref{res:lcm-growth} is a statement about $\Lc_N$ and not about
$\den(H_N)$.  Theorem~\ref{res:prime-pole} and the cancellations at $139$ and
...
\section{Carries and the rationality boundary}\label{sec:carry}
...
\begin{equation}\label{eq:carry-recurrence}
 Z_m=mZ_{m-1}+1-b_m .
\end{equation}
...
\begin{theorem}[strict-successor characterisation]\label{res:carry-equivalence}
For $m\ge3$,
\begin{equation}\label{eq:unit-window}
 b_m=1
 \iff m\mid Z_m
 \iff 1+\varepsilon_m<m\Delta_m\le2+\varepsilon_m .
  The statement is checked in Lean: \lean{strict_successor_characterisation}{ErdosProblems/Erdos68/PaperCompleteExisting.lean:79}.
\end{equation}
Moreover
\begin{align}
 \Ser\in\Q
 &\iff b_m=1\ \hbox{for all sufficiently large }m,
 \label{eq:carry-rationality}\\
 \Ser\notin\Q
 &\iff \forall B\ \exists m>B:\ m\nmid Z_m .
 \label{eq:strict-misses}
\end{align}
...
gives both equivalences in \eqref{eq:unit-window}, endpoints included.

Suppose $\Ser=a/q$ and $q\mid(m-1)!$.  For $j=m-1$ and $j=m$ the number
$j!\,\Ser$ is an integer, and $j!H_j=j!\,\Ser-E_j$ with $0<E_j<1$ by
\eqref{eq:tail-bound}, so $Z_j=j!\,\Ser$.  Substituting into
\eqref{eq:carry-recurrence} forces $b_m=1$.  Every fixed $q$ divides
$(m-1)!$ eventually, which gives one direction of
\eqref{eq:carry-rationality} and, by contraposition, the final assertion;
...
Negating \eqref{eq:carry-rationality} gives \eqref{eq:strict-misses}.
\end{proof}
...
\begin{equation}\label{eq:companion-decomposition}
 \Ser=C+e-2 .
\end{equation}
...
\begin{proposition}[companion-orbit criterion]\label{res:companion-orbit}
\[
 \Ser\in\Q
 \quad\Longleftrightarrow\quad
...
\eqref{eq:companion-decomposition} by $m!$ and using
...
\begin{proposition}[lower-interval criterion]\label{res:lower-escape}
\begin{equation}\label{eq:lower-escape}
 \Ser\notin\Q
 \quad\Longleftrightarrow\quad
...
\end{equation}
implies the escape inequality in \eqref{eq:lower-escape}.  Cofinally many
instances of \eqref{eq:finite-escape} therefore imply $\Ser\notin\Q$.
  The statement is checked in Lean: \lean{lower_interval_criterion}{ErdosProblems/Erdos68/PaperCompleteExisting.lean:104}.
\end{proposition}
...
\eqref{eq:tail-bound} this gives $m\theta_{m-1}<1$, so $a_m(\Ser)=0$ and
...
\]
which contradicts \eqref{eq:finite-escape}.
\end{proof}

Escape at a single index does not give a non-unit carry there.  The exact
classification is
\begin{equation}\label{eq:escape-classification}
 E_m\le m\theta_{m-1}
 \quad\Longleftrightarrow\quad
...
Proposition~\ref{res:lower-escape} does not identify escape with a non-unit
carry.
...
\begin{theorem}[uniform family boundary]\label{res:shift-family}
...
at every $m\ge3$, so the boundary returns the irrationality of $e$.
  The statement is checked in Lean: \lean{uniform_family_boundary}{ErdosProblems/Erdos68/PaperCompleteExisting.lean:125}.
\end{theorem}
...
\section{From denominator structure to a scale criterion}\label{sec:residue}
...
\begin{equation}\label{eq:core-normalisation}
...
\eqref{eq:global-scale}.
\end{proof}

Cancelling $R_p$ in \eqref{eq:global-scale} through
\eqref{eq:core-normalisation} displays the quantitative issue directly:
\begin{equation}\label{eq:core-gap-scale}
...
Proposition~\ref{res:wilson-cofinality} does not close the argument.
...
\begin{equation}\label{eq:factor-split}
 ...
 (2p+1)C_pq<K_p .
 \end{cases}
  The statement is checked in Lean: \lean{moving_factor_min_split}{ErdosProblems/Erdos68/PaperCompleteSupplementary.lean:59}.
\end{equation}
Testing both entries of the minimum and using
\eqref{eq:core-normalisation} gives the equivalence, so the reduction has no
opaque floor premise left.  Cofinal instances of both inequalities imply
irrationality.  The single global condition of
Theorem~\ref{res:global-residue} is separately sufficient, so the pair is one
sufficient route among several.

\label{res:split-factor-normalized-collision}\phantomsection
\label{bdry:fixed-owner-absorption}\phantomsection
...
\section{A limitation of finite-channel cancellation}\label{sec:channels}
...
\begin{equation}\label{eq:channel-congruence}
...
\begin{theorem}[quotient-band breakpoint]\label{res:bandbreakpoint}
...
is at least $2d$.
  The statement is checked in Lean: \lean{supported_quotient_band}{ErdosProblems/Erdos68/PaperCompleteSupportedBands.lean:25}.
\end{theorem}
...
\S\ref{sec:lcm}.  Every zero-moment variation of the support changes a
...
\begin{theorem}[square-subsequence radius bound]\label{res:channel-radius}
...
$R(t)=o(t^3)$.
  The statement is checked in Lean: \lean{square_subsequence_radius}{ErdosProblems/Erdos68/PaperCompleteExisting.lean:236}.
\end{theorem}

\begin{proof}
Put $D=2t^2$, $k=2t$ and $u=D-k+1=2t^2-2t+1$.  Lemma~\ref{res:segment} at
$N=D$, with $n!-1\ge n!/2$ and $\log n!\ge n\log n-n$, and with
$\Lc_D\le M<(R+1)!$, gives
\begin{equation}\label{eq:radius-log}
...
\eqref{eq:radius-log} is at least
...
\]
Dividing by $t^3$ and rearranging, \eqref{eq:radius-log} would require
\[
...
Theorem~\ref{res:lcm-growth} raises the asymptotic constant in the same
estimate.

\begin{corollary}[asymptotic radius constant]\label{res:radius-constant}
...
\]
  The statement is checked in Lean: \lean{asymptotic_radius_constant_liminf}{ErdosProblems/Erdos68/PaperCompleteLiminf.lean:54}.
\end{corollary}

\begin{proof}
Put $r=R(t)+1$, so $\Lc_{2t^2}\le M(t)<r!$ and $\log \Lc_{2t^2}<r\log r$.
Theorem~\ref{res:lcm-growth} at $N=2t^2$ gives, for every $\varepsilon>0$ and
all large $t$,
\[
 \log \Lc_{2t^2}
...
Corollary~\ref{res:radius-constant} rests on the ordinary proof of
Theorem~\ref{res:lcm-growth}; the kernel-checked finite theorem does not
carry it.

The congruence \eqref{eq:channel-congruence} also explains the prime-channel
corrector.

\begin{theorem}[two-term prime channel corrector]\label{res:translator}
Let $p$ be prime and let $c_{p-1}=p$, $c_p=-1$, with every other coefficient
zero.  Then $M(c)=0$, $\chn pc=p!-1$, and $\chn dc=0$ for every $d\ge2$ with
$d\ne p$.
\end{theorem}

\begin{proof}
The moment is $p\,(p-1)!-p!=0$.  For $2\le d<p$ the quotient identity
...
\section{Finite denominator exclusions}\label{sec:finite}
...
\]
so $b_{300000}\ne1$.  Theorem~\ref{res:carry-equivalence} then gives
...
its own run, the event-trace digest and the final enclosure.  The driver
...
current digests postdate that run, so a byte-identical regeneration of the
$300000$ receipt is outstanding.  The same driver replayed independently at
...
closes \eqref{eq:finite-bounds}.  The
...
\section{The remaining arithmetic inputs}\label{sec:open}

The exact target is \eqref{eq:strict-misses}, equivalently
\eqref{eq:lower-escape}.  The routes below are sufficient conditions for it,
...
does not establish \eqref{eq:weighted-target}, and reflected hits may not be
...
$d=A_n$:
\begin{equation}\label{eq:amplification-target}
 \bigl((m+2)m!-2\bigr)v_m
 \le m^2(m!-1)\,(u_m\bmod d).
\end{equation}
It implies the upper alternative of \eqref{eq:finite-escape}.  A nonzero
...
reach \eqref{eq:amplification-target}.

\paragraph{Lower-interval escape.}
Prove \eqref{eq:finite-escape} at arbitrarily large indices; restricting to
...
\begin{equation}\label{eq:doubled-prime}
 p^2\mid Z_{2p}
 \quad\Longleftrightarrow\quad
 \begin{cases}
 b_{2p}=1\ \hbox{and}\ p\mid Z_{2p-1},\ \hbox{or}\\
 b_{2p}=1+p\ \hbox{and}\ p\mid2Z_{2p-1}-1 .
  The statement is checked in Lean: \lean{fixed_owner_absorption}{ErdosProblems/Erdos68/PaperCompleteSupplementary.lean:127}.
 \end{cases}
\end{equation}
...
\eqref{eq:channel-congruence} each $(\chn dc-M(c))/(d!-1)$ is an integer and
...
$u_n=n!-1$, and his all-positive specialisation additionally requires
$\sum_n|u_{n+1}/u_n^2-1|<\infty$, whose summands tend to one here
...
Theorem~\ref{res:lcm-growth} bounds that cancellation from one side and
Theorem~\ref{res:prime-pole} describes it from the other.
...
added axioms, or unchecked evaluation.  Theorem~\ref{res:lcm-growth} and
Corollary~\ref{res:radius-constant} are ordinary proofs given in full above
...
Proposition~\ref{res:wilson-cofinality} &
...
Theorem~\ref{res:carry-equivalence} &
...
Proposition~\ref{res:companion-orbit} &
...
Proposition~\ref{res:lower-escape} &
...
classification \eqref{eq:escape-classification} at
...
Theorem~\ref{res:global-residue} &
...
Theorem~\ref{res:normalform} and \eqref{eq:channel-congruence} &
...
Theorem~\ref{res:channel-radius} &
...
Equation~\eqref{eq:doubled-prime} &
...
from it is not theirs.  For the survival criterion, the Wilson cofinality
packaging, the finite radius bound and Theorem~\ref{res:lcm-growth}, no
antecedent was located, which is a negative search result, and no priority
is claimed.
...
\section{Extended record}\label{sec:extended-record}
...
\subsection{The canonical factorial digit kernel}\label{sec:ext-digits}
...
\subsection{Criteria from the literature, in full}\label{sec:ext-literature}
...
\cite[pp.~275, 285--286]{duverney}; his all-positive Corollary~3.2
additionally requires $\sum_n|u_{n+1}/u_n^2-1|<\infty$, whereas those
summands tend to one \cite[Cor.~3.2, p.~287]{duverney}.  Both the ratio limit
...
\subsection{A superseded deduction}\label{sec:ext-superseded}
...
$\log \Lc_N\gg N^{4/3}\log N$.  Theorem~\ref{res:lcm-growth} supersedes this:
...
as \eqref{eq:doubled-prime} in the short note.  The formal theorem is not
...
\subsection{The collision-core interfaces}\label{sec:ext-collision}
...
\subsection{Finite channel and carry certificates}\label{sec:ext-certificates}
...
\subsection{Limits of fixed-coordinate arguments}\label{sec:ext-nogo}
