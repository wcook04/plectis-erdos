...
\]
is irrational.  The problem remains open, and no result in this note settles it
or any instance of it.

The strongest unconditional theorems here describe the reduced denominators of
the partial sums of $\Ser$ and the primes that carry them, and both are checked
by the Lean kernel at the pinned commit.  Write $d_n=n!-1$,
$L=\operatorname{lcm}(d_2,\ldots,d_M)$ and $H_M=\sum_{2\le n\le M}d_n^{-1}$.
For a prime $p$ dividing $L$, with $e=v_p(L)$ and
$J=\{n:2\le n\le M,\ v_p(d_n)=e\}$, write $d_n=p^eu_n$ for $n\in J$.  Then the
reduced denominator of $H_M$ contains the full power $p^e$ if and only if
$\sum_{n\in J}u_n^{-1}\ne0$ in $\mathbb F_p$, and full cancellations occur at
$p=139$ and $p=2593$.  Second, for every cutoff $B$ there are a prime $q$ and
an index $m>B$ with $q\mid m!-1$ and $q$ coprime to every earlier factorial
gap, so prefix-private prime support of the denominators occurs cofinally; the
proof chooses $q\ge B!+5$ and applies Wilson's theorem, and it carries no
scale bound.  Neither theorem decides the value of $\Ser$.

The remaining results are negative knowledge, and each is labelled by what it
is.  The first is a proved class obstruction to the finite-channel strategy,
and its complete statement is checked by the Lean kernel: for all
$t,M,R\in\mathbb N$ with $t\ge2^{32}$, $M>0$, $\Lc_{2t^2}\mid M$ and
$M<(R+1)!-1$,
\[
  3t^3<2(R+1).
\]
All four hypotheses are carried inside the formal statement, the proof is
finite, and the conclusion excludes eventual $3/2$ upper bounds for the
normalized channel radius together with little-$o(t^3)$ growth in this
architecture.  The obligation it leaves is explicit: derive the divisibility
and factorial-size hypotheses from the factorial-gap series itself, and prove
that the channels through $2t^2$ vanish for the resulting family.

Two finite exclusions follow.  A GMP certificate gives $300000\nmid Z_{300000}$ and hence
$q\nmid299999!$ in any representation $\Ser=a/q$ with $q>0$; an independent
continued-fraction certificate gives $q\ge2^{39990}>10^{12038}$.  These are
two different constraints.  The first is a divisibility restriction that no
denominator magnitude discharges; the second is a magnitude bound that no
smoothness restriction discharges.  Both exponents are read from the replay
receipt \path{verification/erdos68-continued-fraction.json}, which records
scale $80000$ bits, bracket width $7053$, $23449$ certified partial quotients,
power-of-two exponent $39990$, and strict decimal exponent $12038$.

Two target-equivalent reformulations are recorded next.  For
...
failures of $m\mid Z_m$.  These equivalences restate the problem, isolate the
producer, and do not supply it.  A conditional advance completes the list: one
moving prefix-private prime, together with explicit global
complementary-residue and local collision-core scale bounds, implies
irrationality, and arbitrary factors of one moving private modulus suffice
under the corresponding normalized projection hypotheses.  Wilson supplies the
moving private factors cofinally, so the surviving obligation there is the pair
of scale bounds, and the two quantitative producers remain open.  Finally, two
consecutive unit carries reduce to the universal
inequality $0<U_{m+2}\le V_{m+2}$, so a divisibility obstruction derived only
after assuming that pair cannot exclude it.

Two further exact reformulations are recorded in the working tree and are
named here so that the frontier is visible in one place.  A shrinking-target
normal form states that irrationality is equivalent to cofinal escape of the
canonical factorial orbit from a lower endpoint cylinder whose radius is
$O(m^{-2})$.  A uniform family boundary states, for every integer $t\ge-1$,
that $\sum_{n\ge2}1/(n!+t)$ is rational exactly when
$\lceil t\,m!\,C_t\rceil\equiv2\pmod m$ for all sufficiently large $m$, with
$C_t=\sum_{n\ge2}1/(n!(n!+t))$, and it recovers the irrationality of $e$ at
$t=0$.  Both statements sit outside the formal-source commit pinned by this
note, they carry no declaration link below, and they are scheduled for the
next formalisation cycle.
\end{abstract}

\begin{paperresult}{Main results and exact open boundary}
\textbf{Unconditional theorems about the series.} The full maximal power of a
prime survives reduction of a finite prefix exactly when the reciprocal
maximal-hit cofactor sum is nonzero in $\mathbb F_p$, with full cancellations
at $p=139$ and $p=2593$; and prefix-private prime support of the factorial
gaps occurs cofinally, by Wilson.  Both are Lean-checked at the pinned commit,
and neither decides the value of $\Ser$.
\textbf{Proved class obstruction.} The square-subsequence channel radius is
...
open.  \textbf{Target-equivalent reformulations.} The fixed companion orbit and
strict successor give lossless eventual and cofinal restatements of the problem
without the missing producer; both extend the classical factorial-base rationality criterion of
Cantor \cite{cantor1869} in the form recorded by Galambos
\cite[Ch.~1]{galambos1976}.  \textbf{Evidence split.} The prime-pole law, the
Wilson cofinality, the channel radius,
the moving-factor split and the two reformulations are
Lean-checked at the pinned commit; the $299999!$ and continued-fraction
denominator exclusions are ordinary proofs whose kernel-checked part is the
implication from one non-unit carry, with the numbers supplied by replayable
integer certificates.  \textbf{Held outside the checkpoint.} The
shrinking-target normal form and the uniform boundary for
$\sum_{n\ge2}1/(n!+t)$ are proved in the working tree and are not part of the
pinned formal source.  \textbf{Open boundary.} No result here proves a
cofinal miss or irrationality of $\Ser$.
\end{paperresult}
...
Digits eventually zero $\iff$ $\Ser$ rational & Checked & The equivalence holds for the canonical factorial expansion of every real number. \\
...
\section{Leading contributions}\label{sec:lead}

Each entry below states the hypotheses it carries, separates the
kernel-checked part from the ordinary part and from any replayable
certificate, records the attribution where prior art applies, and names the
exact obligation that remains.  Every statement is repeated in full in its own
section.  Entries~1 and~2 are unconditional theorems about the object of
Erd\H{o}s~\#68, and they are stated first for that reason.  Entry~3 is a
proved class obstruction, entry~4 a conditional reduction, entries~5 and~6
target-equivalent reformulations, entry~7 a pair of finite exclusions, and
entry~8 two normal forms held outside the pinned formal source.  None of the
eight settles the problem or any instance of it.

\paragraph{1. Prime-pole cancellation law.}
\label{res:lead-prime-pole}\phantomsection
For a finite prefix, the complete maximal power $p^e$ of a prime survives
reduction exactly when the reciprocal maximal-hit cofactor sum is nonzero in
$\mathbb F_p$.  This is Proposition~\ref{res:prime-pole}.
\emph{Evidence.} Lean-checked at
\lref{Erdos68/PrimePoleCriterion.lean}{223}{factorialGapPrefix_den_full_prime_power_iff},
with the denominator-side criterion in
\path{ErdosProblems/Erdos68/PrimePoleDenominator.lean}.  The two full
cancellations at $p=139$ and $p=2593$ are exact integer computations
displayed in \S\ref{sec:finite}.
\emph{Attribution.} Recorded as unassessed.
\emph{Remaining obligation.} The law classifies survival at a finite prefix
and supplies no statement about the infinite series.

\paragraph{2. Cofinal prefix-private factorial-gap primes.}
\label{res:lead-wilson-cofinality}\phantomsection
For every cutoff $B$ there are a prime $q$ and an index $m>B$ with
$q\mid m!-1$ and $q$ coprime to $k!-1$ for every $k$ with $2\le k<m$.
\emph{Evidence.} Lean-checked at
\lref{Erdos68/PrimeZeroBranch.lean}{3097}{cofinal_prefixPrivate_factorialGap_hits}.
The proof chooses a prime $q\ge B!+5$, uses Wilson's theorem to get
$q\mid(q-2)!-1$, and takes the least hit; the least hit cannot occur at or
below $B$, since the prime would then be at most $B!-1$.
\emph{Attribution.} Wilson's theorem is classical.  The cofinality packaging is
recorded as unassessed.
\emph{Remaining obligation.} The supply is arithmetic only and carries no
scale bound relating $q$ to its least hit, which is what
entry~4 consumes.

\paragraph{3. Square-subsequence channel radius.}
\label{res:lead-channel-radius}\phantomsection
For all $t,M,R\in\mathbb N$ with $t\ge2^{32}$, $M>0$, $\Lc_{2t^2}\mid M$ and
$M<(R+1)!-1$, one has $3t^3<2(R+1)$; the sequence form says that no radius
function satisfies $2(R(t)+1)\le3t^3$ eventually, and a third form says that
$R(t)+1$ is not $o(t^3)$ under the corresponding hypotheses from $t\ge4096$
onward.  This is Theorem~\ref{res:channel-radius}.
\emph{Evidence.} The complete advertised endpoint is Lean-checked at
\lword{Erdos68/ChannelIntegralCongruence.lean}{1222}{square_subsequence_radius_three_halves_lower}{the sharp radius theorem},
with the sequence form at
\lword{Erdos68/ChannelIntegralCongruence.lean}{1241}{no_eventual_square_subsequence_three_halves_upper}{the eventual no-go theorem}
and the little-$o$ form at
\lword{Erdos68/ChannelIntegralCongruence.lean}{1043}{not_isLittleO_square_subsequence_radius}{the little-o obstruction}.
The method's own ceiling is checked separately at
\lword{Erdos68/ChannelIntegralCongruence.lean}{764}{sharp_radius_satisfies_square_log_constraint}{the logarithmic-constraint evaluation}:
for $t\ge4$ and $9(R+1)=16t^3$ the finite logarithmic constraint is already
satisfied, so that constraint alone cannot force the asymptotically optimized
constant $16/9$.  No part of this entry rests on a certificate or on an
ordinary supplement.
\emph{Attribution.} The prior-art relation for this family is recorded as
unassessed, so no novelty is asserted for it here.
\emph{Remaining obligation.} Derive $\Lc_{2t^2}\mid M$ and $M<(R+1)!-1$ from
the factorial-gap series, and prove that the channels through $2t^2$ vanish
for the resulting family.

\paragraph{4. Moving-factor scale split.}
\label{res:lead-moving-factor-split}\phantomsection
If cofinally many $m\ge4$ carry a large prefix-private prime $q$ of $m!-1$
such that, at $p=m/2+1$, both $(2p+1)L_p<2p^2(2p-1)!\,\rho_p$ and
$(2p+1)C_pq<2p^2(2p-1)!$ hold, then $\Ser$ is irrational.  This is
\S\ref{res:moving-factor-scale-split}.
\emph{Evidence.} The implication is Lean-checked at
\lref{Erdos68/PrimeZeroBranch.lean}{1574}{irrational_factorialGapSeries_of_cofinal_largePrefixPrivate_unitScaleSplit}.
The split that removes the opaque floor premise is the exact equivalence
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4766}{factorialBlock_unitFactorPairFloor_scale_iff},
resting on the floor identity
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4701}{factorialBlock_unitFactorPairFloor_eq_min}.
The arithmetic supply is unconditional and also Lean-checked:
\lref{Erdos68/PrimeZeroBranch.lean}{3097}{cofinal_prefixPrivate_factorialGap_hits}
produces, for every cutoff, a prime $q$ and an index $m$ beyond it with
$q\mid m!-1$ and $q$ coprime to every earlier factorial gap.
\emph{Attribution.} The supply proof uses Wilson's theorem, which is
classical; the cofinality packaging is recorded as unassessed.
\emph{Remaining obligation.} Prove both sides of the branch-free scale split
cofinally.  Neither side follows from the existence of the selected prime.

\paragraph{5. Fixed companion-orbit boundary.}
\label{res:lead-companion-orbit}\phantomsection
With $C=\sum_{n\ge2}1/(n!(n!-1))$, the series $\Ser$ is rational exactly when
$(\lfloor m!C\rfloor+2)\bmod m=0$ for every sufficiently large $m$, and
irrational exactly when that residue is missed cofinally.  This is
Theorem~\ref{res:companion-orbit-rationality-boundary}.
\emph{Evidence.} Lean-checked in both directions at
\path{ErdosProblems/Erdos68/CompanionOrbitRationality.lean}, endpoints
...
\emph{Attribution.} The statement extends the classical factorial-base
rationality criterion, that a real number is rational exactly when its
canonical factorial-base digits vanish from some index on, due to Cantor
\cite{cantor1869} and recorded in Galambos \cite[Ch.~1]{galambos1976}.  The
addition here is the telescope $C=\Ser-e+2$, the identification of the digit
value $m-2$, and the transport of the criterion to the factorial-gap series.
\emph{Remaining obligation.} Show that $\lfloor m!C\rfloor\bmod m$ differs
from $-2$ at arbitrarily large indices.

\paragraph{6. Strict-successor carry equivalence.}
\label{res:lead-carry-equivalence}\phantomsection
With $H_m=\sum_{2\le n\le m}(n!-1)^{-1}$ and $Z_m=\lfloor m!H_m\rfloor+1$,
irrationality of $\Ser$ is equivalent to cofinally many failures of
$m\mid Z_m$, equivalently to cofinally many non-unit carries.  This is
\S\ref{res:strict-successor-complete-characterization}.
\emph{Evidence.} Lean-checked at
\lref{Erdos68/FactorialGapPlateauCore.lean}{986}{irrational_factorialGapSeries_iff_cofinal_strictFacTopRat_misses}
with the carry form at
\lref{Erdos68/FactorialGapPlateauCore.lean}{945}{irrational_factorialGapSeries_of_cofinal_nonunit_carries}.
\emph{Attribution.} Same antecedent as entry~3.
\emph{Remaining obligation.} Produce the cofinal failures.  The equivalence
supplies no producer.

\paragraph{7. Finite denominator exclusions, two different constraints.}
\label{res:lead-denominator-exclusions}\phantomsection
If $\Ser=a/q$ with $q>0$, then $q\nmid299999!$, and independently
$q\ge2^{39990}>10^{12038}$.
\emph{Evidence.} The first exclusion is an ordinary proof with a
kernel-checked part.  The kernel-checked part is the implication from one
non-unit carry, at
\lref{Erdos68/FactorialGapPlateauCore.lean}{836}{rational_denominator_not_dvd_pred_factorial_of_nonunit_carry}
and, in the magnitude form, at
\lref{Erdos68/FactorialGapPlateauCore.lean}{812}{rational_denominator_ge_of_nonunit_carry};
the exact non-unit carry at $m=300000$ comes from a separately implemented
GMP integer computation, whose payload, driver and backend are hash-bound in
the companion research packet.  The second exclusion is an ordinary
continued-fraction proof whose input is the replayable integer certificate
\path{verification/erdos68-continued-fraction.json}, recording scale $80000$
bits, bracket width $7053$, $23449$ certified partial quotients, power-of-two
exponent $39990$, and strict decimal exponent $12038$.  A kernel-internal
instance of the same implication gives $q\ge67$ at
\lref{Erdos68/FactorialZeroPlateauCertificates.lean}{136}{sixty_seven_le_rational_denominator}.
Neither exclusion implies the other: a denominator with no prime factor above
$299999$ can still exceed $10^{12038}$, and a denominator far below that size
can still fail to divide $299999!$ by carrying one prime to a high power.
\emph{Attribution.} The continued-fraction method is textbook, and the
adjudicated disposition records the numbers as new with no antecedent
located.
\emph{Remaining obligation.} Both exclusions are finite.  Ruling out an
eventual all-unit carry tail is what would convert them into a proof.

\paragraph{Finite exclusions and the unbounded obstruction.}
Entries~3 and~7 have different reach, and the two statements are set beside
each other here.  The continued-fraction certificate establishes one inequality
about a hypothetical denominator, $q\ge2^{39990}$ and hence $q>10^{12038}$, for
a value assumed rational.  It is finite in a strict sense: every extension of
it raises the excluded range and returns a statement of the same form, so
continuation of that computation leaves irrationality undecided.  The exclusion
$q\nmid299999!$ is finite in the same sense, and it constrains the prime
support of a hypothetical denominator rather than its size.
Theorem~\ref{res:channel-radius} quantifies over an unbounded family instead.
It holds for all $t\ge2^{32}$ and all $M,R$ satisfying its hypotheses, and its
sequence form says that no radius function satisfies $2(R(t)+1)\le3t^3$
eventually, so what it excludes is an asymptotic regime and not a set of
denominators.  Its own limits are equally definite.  The hypotheses
$\Lc_{2t^2}\mid M$ and $M<(R+1)!-1$ are carried, not derived from the
factorial-gap series, and vanishing of the channels through $2t^2$ for the
resulting family is unproved.  Converting the finite exclusions into an
irrationality proof requires ruling out an eventual all-unit carry tail.
Converting the channel obstruction into one requires the unbounded strict
nonvanishing statement, which Section~\ref{sec:problem} records as open.

\paragraph{8. Two normal forms held outside the pinned formal source.}
\label{res:lead-held-outside-checkpoint}\phantomsection
The first is a shrinking-target normal form: $\Ser$ is irrational exactly
when the canonical factorial orbit escapes a lower endpoint cylinder
cofinally, and because the scaled tail lies strictly between $0$ and $2/m$
the trapping target has radius $O(m^{-2})$.  The second is a uniform boundary
for the family Erd\H{o}s stated on p.~102: for every integer $t\ge-1$, with
$C_t=\sum_{n\ge2}1/(n!(n!+t))$, the series $\sum_{n\ge2}1/(n!+t)$ is rational
exactly when $\lceil t\,m!\,C_t\rceil\equiv2\pmod m$ for all sufficiently
large $m$; the member $t=0$ is $e-2$, and its orbit never meets the residue
class, so the family boundary reproves that $e$ is irrational.
\emph{Evidence.} Both are proved in the working tree and neither is part of
the formal-source commit pinned by this note, so neither carries a
declaration link here.  They are named because the shrinking-target form is
the only recorded formulation that gives the measure-theoretic shape of the
obstruction, and the family form is the only one that covers the statement
Erd\H{o}s actually made.
\emph{Attribution.} Prior art for both is unassessed.  The $t=0$
specialisation recovers a classical fact.
\emph{Remaining obligation.} Formalise both into the pinned checkpoint in the
next cycle, and prove cofinal escape from the endpoint cylinder.
...
\lref{Erdos68/CanonicalFactorialDigits.lean}{124}{canonicalDigit_eq_floor_mul_remainder},
\lref{Erdos68/CanonicalFactorialDigits.lean}{146}{canonicalDigit_nonneg},
\lref{Erdos68/CanonicalFactorialDigits.lean}{153}{canonicalDigit_lt_radix},
\lref{Erdos68/CanonicalFactorialDigits.lean}{162}{canonicalRemainder_recurrence},
the
...
zero canonical factorial-digit expansion.
...
\]
The
\lword{Erdos68/CanonicalFactorialTermination.lean}{78}{exists_rat_eq_iff_eventually_zero_canonicalDigit}{termination
equivalence} follows from the growth of a positive remainder, through the
\lword{Erdos68/CanonicalFactorialTermination.lean}{38}{canonicalRemainder_eq_zero_of_zero_digit_tail}{zero-tail
remainder lemma} and the
\lword{Erdos68/CanonicalFactorialTermination.lean}{54}{exists_rat_eq_of_canonicalRemainder_eq_zero}{rational
witness}. If all digits after index
$N\ge1$ vanish, the recurrence gives
$\theta_{N+k}\ge(k+1)\theta_N$. Since every canonical remainder is less than
one, $\theta_N$ must vanish. Thus $x=\lfloor N!x\rfloor/N!$.
...
\lref{Erdos68/FactorialChannelCertificate.lean}{25}{factorial_pow_floor_dvd_factorial},
\lref{Erdos68/FactorialChannelCertificate.lean}{43}{channelWeight_mul_denominator}, and
\lref{Erdos68/FactorialChannelCertificate.lean}{65}{channelEvent_eq_zero_of_not_dvd}.
...
see \lref{Erdos68/PrimeUnitTranslator.lean}{1657}{exists_remote_factorialGrid_primeTranslator_reduction}.  What is not
...
also checks the \lword{Erdos68/FactorialGapPlateauCore.lean}{82}{factorialGrid_plateau}{factorial plateau theorem}: if $H<G\le \Ser$, if $n!G$ is
...
\label{res:strict-successor-complete-characterization}\phantomsection
...
\lword{Erdos68/FactorialGapPlateauCore.lean}{812}
...
\lword{Erdos68/FactorialZeroPlateauCertificates.lean}{136}
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4387}{factorialBlock_factorProjection_lcm_eq_privateModulus} and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4854}{factorialBlock_complementaryFactorPairFloor_le_global}.
\label{res:split-factor-normalized-collision}\phantomsection
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{556}{pairwiseCollisionCore_insert_gcd_lcm} and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{586}{collisionCore_insert_gcd_lcm}.  Thus each step needs only the old
...
\]
See
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{800}{collisionCore_div_base_mul_denominatorLcm_dvd_denominatorProd}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3327}{factorialBlockNormalizedCollisionCore_le_gapProd_div_gapLcm}.
...
\]
See \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{698}{collisionCore_div_base_eq_pairwiseCollisionCore_div_gcd},
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{739}{collisionCore_div_base_factorization}, and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3394}{factorialBlockNormalizedCollisionCore_factorization}.  Consequently
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3790}{factorialBlock_normalizedCollision_factorization_add_base_lt_prime},
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3982}{factorialBlock_prime_sq_gt_pred_of_dvd_normalizedCollisionCore}, and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3997}{factorialBlockNormalizedCollisionCore_coprime_factorial_of_mul_pred_le}.
...
\]
See \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5012}{factorialBlockPrime_not_dvd_base_of_upper_hit} and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5113}{factorialBlock_primePower_dvd_normalizedCollisionCore_iff_of_upper_hit}.
...
\]
See
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5144}{factorialBlock_primePower_dvd_normalizedCollisionCore_iff_one_lt_hitCount}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5187}{factorialBlock_normalizedCollisionCore_factorization_lt_of_hitCount_le_one}.
...
\]
see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5213}{factorialBlock_normalizedCollisionCore_factorization_eq_repeatedHitLayerCount}.
...
\]
See
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3668}{exists_factorialBlock_hitPair_with_normalizedPrimePower_le_gapPow}.
Consequently, if
\[
 (2p-1)^d<q^{e+v_q(B)},
\]
then some such two hits satisfy $d<j-i$; see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3698}{exists_factorialBlock_hitPair_distance_gt_of_endpointPow_lt_normalizedPrimePower}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3472}{factorialBlock_prime_hitPair_distance_gt_exponent}.  The point is
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3848}{factorialBlock_normalizedCollision_factorization_add_base_lt_blockDiameter}.
The exponent-level version is
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3817}{factorialBlock_normalizedCollision_exponent_add_base_factorization_lt_blockDiameter}.
...
\]
see \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3443}{factorialBlock_primePower_le_gapPow_of_two_hits}.  Consequently,
when $q$ is prime and $e>0$, every two $q^e$-hits in the block satisfy $e<j-i$, so the
condition holds for every pair and one chosen pair does not suffice; see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3472}{factorialBlock_prime_hitPair_distance_gt_exponent}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{3602}{factorialBlock_primePowerHitCount_mul_succ_le}.
...
\]
see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5277}{factorialBlock_normalizedCollisionCore_factorization_eq_truncatedRepeatedHitLayerCount}.
...
\]
see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5389}{factorialBlock_primePower_dvd_normalizedCollisionCore_iff_one_lt_hitCount_of_endpoint_lt_base}.
Thus an at-most-one $q^e$ incidence estimate forces
$v_q(\widetilde C_p)<e$ without first choosing an upper hit; see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5410}{factorialBlock_normalizedCollisionCore_factorization_lt_of_endpoint_lt_base_of_hitCount_le_one}.
At $e=2$ this gives the conditional squarefree conclusion
$v_q(\widetilde C_p)\le1$; see
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5436}{factorialBlock_normalizedCollisionCore_factorization_le_one_of_endpoint_lt_base_of_primeSqHitCount_le_one}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5319}{factorialBlock_primePower_dvd_normalizedCollisionCore_iff_one_lt_hitCount_of_not_dvd_base}.
...
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5344}{factorialBlock_normalizedCollisionCore_factorization_lt_of_not_dvd_base_of_hitCount_le_one}
and
\lref{Erdos68/EndpointWeightedPrivateSupport.lean}{5370}{factorialBlock_normalizedCollisionCore_factorization_le_one_of_not_dvd_base_of_primeSqHitCount_le_one}.
...
\lref{Erdos68/PrimeZeroBranch.lean}{3097}{cofinal_prefixPrivate_factorialGap_hits}.  A finite variant compares the
...
\lref{Erdos68/PrimeZeroBranch.lean}{3047}{exists_late_prefixPrivate_factorialGap_hit_of_primeProduct_lt}.
...
\lref{Erdos68/PrimeZeroBranch.lean}{3139}{prime_dvd_reflected_factorialGap_of_odd} and
\lref{Erdos68/PrimeZeroBranch.lean}{3231}{prime_dvd_factorialBlockNormalizedCollisionCore_of_odd_reflection} and
\lref{Erdos68/PrimeZeroBranch.lean}{3252}{one_lt_factorialBlockPrimeHitCount_of_odd_reflection}.  Thus a
...
\lref{Erdos68/PrimeZeroBranch.lean}{1419}{factorialGapLargePrefixPrivatePowerModulus_dvd_tailoredBlockPrivateQuotient}
and
\lref{Erdos68/PrimeZeroBranch.lean}{1524}{factorialGapLargePrefixPrivatePrimes_unit_factor_pair_tailoredBlock}.
...
\[
  \min\{\rho,R/q\};
\]
see \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4701}{factorialBlock_unitFactorPairFloor_eq_min}.  Consequently the
...
\[
 ...
 \end{cases}
\]
see \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{4766}{factorialBlock_unitFactorPairFloor_scale_iff}.  Thus the
...
\label{res:moving-factor-scale-split}\phantomsection
...
\item \phantomsection\label{bdry:fixed-owner-absorption}A fixed pair of low-index private owners cannot make the projection
  ...
  \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{2677}{factorialBlockPrivateQuotient_eq_one_of_gap_lt} and checks the
  two-owner consequence in
  \lref{Erdos68/EndpointWeightedPrivateSupport.lean}{2707}{factorialBlockFixedPairPrivateQuotients_eq_one}.  Thus the large
...
exponentiation.  The
\href{https://github.com/wcook04/plectis-erdos/blob/cd3bd6fe245867a435e15d503ccedd699c5d02e2/scripts/check_erdos68_continued_fraction.py}{integer replay}
and its
...
The reduced denominator is $L/\gcd(B,L)$; the
\href{https://github.com/wcook04/plectis-erdos/blob/0d34630e1cc9d2b1ac6edfa7cfdba83b31bdc8fe/ErdosProblems/Erdos68/PrimePoleDenominator.lean#L41}{prime-power survival criterion}
gives $p$-valuation $e$ exactly when $p\nmid B$.  Since $W$ is nonzero modulo $p$, this is the
\href{https://github.com/wcook04/plectis-erdos/blob/6e28e93fc9d8d67c382260b5f3d5fa65ceb2363d/ErdosProblems/Erdos68/PrimePoleCriterion.lean#L223}{maximal-hit cancellation criterion}.
\end{proof}
...
Under the exact rational tail enclosure $1/119<\Theta_4<1/50$, its
\lword{Erdos68/FactorialChannelCertificate.lean}{156}{lambda34_residual_bounds}{residual} lies
...
\]
Lean proves the \lword{Erdos68/PrimeZeroBranch.lean}{5604}{factorialGapRepeatedBirthAmplificationModulus_dvd_succ_den}{divisibility $A_n\mid v_{n+1}$}
and, when $A_n>1$, the
\lword{Erdos68/PrimeZeroBranch.lean}{5619}{predecessorGapNumeratorNat_mod_repeatedBirthAmplificationModulus_ne_zero}{nonzero numerator projection}
modulo the whole product.
...
For every odd prime $p$, the
\lword{Erdos68/FactorialZeroPlateauSupplement.lean}{257}{sq_dvd_double_strictFacTop_factorialGapPrefix_iff}{doubled-prime criterion} is
\[
 p^2\mid Z_{2p}
 \quad\Longleftrightarrow\quad
 \begin{cases}
...
The weighted primitive support decomposition and the determinant-quotient
reduction are returned derivations that have not
...
\item \lref{Erdos68/FactorialGapPlateauCore.lean}{812}{rational_denominator_ge_of_nonunit_carry}
\item \lref{Erdos68/FactorialZeroPlateauCertificates.lean}{136}{sixty_seven_le_rational_denominator}
\item \lref{Erdos68/FactorialGapPlateauCore.lean}{986}{irrational_factorialGapSeries_iff_cofinal_strictFacTopRat_misses}
...
\item \lref{Erdos68/ChannelIntegralCongruence.lean}{1043}{not_isLittleO_square_subsequence_radius}
\item \lref{Erdos68/ChannelIntegralCongruence.lean}{1222}{square_subsequence_radius_three_halves_lower}
\item \lref{Erdos68/ChannelIntegralCongruence.lean}{1241}{no_eventual_square_subsequence_three_halves_upper}
\end{itemize}
