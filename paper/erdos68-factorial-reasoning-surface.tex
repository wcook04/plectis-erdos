...
strategy, and its complete statement is checked by the Lean kernel: for all
...
that the channels through $2t^2$ vanish for the resulting family.  A
...
supplies the moving private factors cofinally, so the surviving obligation
there is the pair of scale bounds.  The two quantitative producers
remain open.
...
power-of-two exponent $39990$, and strict decimal exponent $12038$.  For a finite
...
successor give lossless eventual/cofinal reformulations without the missing
producer; both extend the classical factorial-base rationality criterion of
Cantor \cite{cantor1869} in the form recorded by Galambos
\cite[Ch.~1]{galambos1976}.  \textbf{Evidence split.} The channel radius,
the moving-factor split, the two exact coordinates and the prime-pole law are
...
exact obligation that remains.  The order is by strength of what is
supported, and every statement is repeated in full in its own section.

\paragraph{1. Square-subsequence channel radius.}
...
\paragraph{2. Moving-factor scale split.}
...
\paragraph{3. Fixed companion-orbit boundary.}
...
\paragraph{4. Strict-successor carry equivalence.}
...
\paragraph{5. Finite denominator exclusions, two different constraints.}
...
Entries~1 and~5 have different reach, and the two statements are set beside
...
\paragraph{6. Prime-pole cancellation law.}
...
\paragraph{7. Two normal forms held outside the pinned formal source.}
