...
\section{The problem, and what is settled}\label{sec:problem}
...
\begin{theorem}[two-prime transcendence]\label{res:lead-two-prime}
Let $p$ and $q$ be distinct primes.  Then $\mathcal R_{\{p,q\}}$ and
$\mathcal D_{\{p,q\}}$ are transcendental.
\end{theorem}

Section~\ref{sec:two-prime} proves this.  Both values are explicit polynomials
...
\cite[Theorem~8]{loxtonvdp1977} in the modern form of Bugeaud and Laurent's
...
three-prime discussion.  The identity of Theorem~\ref{res:lcm} is cited to him.

\paragraph{What the third prime does.}
Section~\ref{sec:rank} isolates the obstruction exactly.  After separating row
...
below half its own jump.  Sections~\ref{sec:blocks} to~\ref{sec:escape} take a
...
quadratic bound and a residue condition.  Section~\ref{sec:evidence} reports
the finite evidence and its ceiling, and Section~\ref{sec:open} states what
remains.
...
\section{The finite geometry of the running value}\label{sec:lcm}
...
Section~\ref{sec:rank}.  Also $\hgt(x)\le x^{3}$, since each of the three
...
relation}.  By Theorem~\ref{res:lcm} the running value depends on $x$ only
through the three integer logarithms, so it is constant on cells and moves only
where one logarithm moves.

\begin{proposition}[constancy and jump ratios]\label{res:cell}
...
\begin{proof}
Both parts are immediate from Theorem~\ref{res:lcm}: the height depends on $x$
...
\begin{proposition}[jump count]\label{res:count}
...
\begin{proposition}[finite normal form]\label{res:fibre}
...
expanded form}; the orbits analysed in Section~\ref{sec:blocks} are the actual
...
\begin{proposition}[projection and the quadratic shell bound]\label{res:drop}
\label{res:shell}
...
two coordinates, Lemma~\ref{res:short} with $b=r$ and $w=p^{i}q^{j}$ forces
...
argument of Theorem~\ref{res:lcm} with one coordinate omitted.  The
...
\begin{proof}[Proof of Theorem~\ref{res:lead-two-prime}]
Set
\[
...
\begin{equation}\label{eq:two-prime-quadratic}
 \mathcal R_{\{p,q\}}
...
\cite[Theorem~8]{loxtonvdp1977}.  Take $(\beta,\alpha)=(x,y)$: then
$|\beta|=1/p<1$ and
\[
 |xy^{\theta}|=\frac1p\left(\frac1q\right)^{\log p/\log q}=\frac1{p^{2}}<1 .
\]
So $F_\theta(x,y)$ is transcendental, and \eqref{eq:hecke-mahler-boundary}
makes $A$ transcendental.  The coefficient of $A$
in~\eqref{eq:two-prime-affine} is $(q-p)/(q-1)\ne0$ and the coefficient of
$A^{2}$ in~\eqref{eq:two-prime-quadratic} is $-(p-1)/(q-1)\ne0$, both rational.
...
\label{res:two-prime-transcendence}\label{res:two-prime-repeated-transcendence}
...
nothing; the quadratic identity~\eqref{eq:two-prime-quadratic} in the single
...
\section{The rank phase transition}\label{sec:rank}
...
\begin{proposition}[two generators separate]\label{res:two-prime-rank}
...
\begin{proof}
By the two-variable form of Theorem~\ref{res:lcm} the exponents of $p$ and of
...
Section~\ref{sec:two-prime}: the two-prime value factors because the kernel
does.
...
\begin{proposition}[non-separability at $\{2,3,5\}$]\label{res:rank}
With $(p,q,r)=(2,3,5)$,
\[
 \det\begin{pmatrix}
...
\begin{equation}\label{eq:carry-factorisation}
...
upwards from the last.  Equation~\eqref{eq:carry-factorisation} multiplies this
...
\noindent The factorisation~\eqref{eq:carry-factorisation} is the
...
Theorem~\ref{res:infinite-rank} excludes exact separations of the displayed
...
\begin{equation}\label{eq:carry-matrix}
 C(i,j)=t^{\lfloor x_i+y_j\rfloor},
 \qquad
 x_i=\{i\log_r p\},\quad y_j=\{j\log_r q\},\quad t=r^{-1},
\end{equation}
which is the factor left in~\eqref{eq:carry-factorisation} after the row and
...
\begin{theorem}[sharp uniform separated approximation]\label{res:uniform-rank}
Let $p,q,r$ be pairwise distinct primes and let $C$ be as
in~\eqref{eq:carry-matrix}.  Then
\[
...
matrix~\eqref{eq:carry-matrix}: the original kernel carries positive row and
...
What it does add to Theorem~\ref{res:infinite-rank} is robustness: no finite
...
\section{Dyadic blocks and the literal infinite tail}\label{sec:blocks}
...
Compress the jump word of Section~\ref{sec:lcm} into the blocks cut out by
...
\begin{proposition}[the dyadic block alphabet]\label{res:dyadic-alphabet}
For every $a$,
\begin{equation}\label{eq:dyadic-alphabet}
...
\begin{theorem}[the literal shell recurrence]\label{res:actual-orbit}
...
such pair Lemma~\ref{res:short} leaves at most one exponent of $2$ placing the
...
\eqref{eq:actual-digit}; at $a=0$ the shell is the single point $1$, giving
...
\begin{proposition}[integral state or cofinal separation]\label{res:actual-dichotomy}
...
to $30$: that case is the subject of Section~\ref{sec:actual-orbit}.
...
$\hi\le2^{A+1}\le2\lo$ and, by Lemma~\ref{res:short}, fixing the exponents of
...
Proposition~\ref{res:drop}; grouping the actual infinite tail by height cells
...
\begin{equation}\label{eq:endpoint-index}
...
\end{equation}
The cutoff in \eqref{eq:endpoint-index} is $2^{a}$ and it is strict.

\begin{lemma}[finite denominator clearing]\label{res:all-scale-lattice}
...
\label{res:actual-cancellation}\label{res:lead-carry-bridge}
\label{res:actual-carry-bound}\label{res:denominator-reduction}
...
Lemma~\ref{res:all-scale-lattice} both terms on the right are divisible by $M$,
so dividing by $M$ shows that $BX_a$ is an integer.  Positivity and the
recurrence come from~\eqref{eq:shell-digit-identity}, and the upper bound is
Theorem~\ref{res:actual-tail-bound} with the floor taken, since $z_a$ is an
...
\begin{proposition}[upward closure and rigidity]\label{res:pinning}
...
\section{Residue windows and the equivalence band}\label{sec:escape}
...
\begin{equation}\label{eq:window-identity}
...
\begin{proposition}[the finite residue contradiction]\label{res:consumer}
...
\begin{equation}\label{eq:actual-escape}
 \begin{gathered}
...
\begin{theorem}[the equivalence band]\label{res:actual-escape-endpoint}
\label{res:lead-escape-equivalence}\label{res:windowconsumer}
...
\]
Both $K$ of~\eqref{eq:actual-bound} and $K_0(B,a)=90B(a+1)^{2}$ lie in this
...
Theorem~\ref{res:actual-cancellation} supplies $B\ge1$ coprime to $30$, an
onset $a_D$, and positive integers $z_a=BX_a\le K(B,a)\le G(B,a)$ for
$a\ge a_D$ satisfying the cleared recurrence.  Apply~\eqref{eq:actual-escape}
with $a_0=a_D$ to obtain a window $(\ell,h)$ with $\ell\ge a_D$.
By~\eqref{eq:window-identity}, $z_{\ell+h}\equiv-BF_{\ell,h}$ modulo
$W_{\ell,h}$, while $0<z_{\ell+h}\le G(B,\ell+h)<\lpr_{W_{\ell,h}}(-BF_{\ell,h})$.
Proposition~\ref{res:consumer} is the contradiction, so $S$ is irrational.
...
multiplying~\eqref{eq:window-identity} by $B$ gives
\[
 BX_\ell-k_h=\frac{BX_{\ell+h}-r_h}{W_{\ell,h}} .
\]
Both $BX_{\ell+h}$ and $r_h$ are positive, the first at most
$BQ(n_{\ell+h})$ by Theorem~\ref{res:actual-tail-bound} and the second at most
...
Theorem~\ref{res:actual-cancellation}, both are quadratic in $a$ for fixed $B$,
...
of Theorem~\ref{res:actual-escape-endpoint} is the paper statement.

Theorem~\ref{res:actual-escape-endpoint} settles a question that a reader is
...
\begin{proposition}[window-growth law]\label{res:window-growth}
...
\begin{proof}
Telescoping \eqref{eq:dyadic-alphabet} gives
...
\begin{corollary}[no bounded-length escape at all starts]\label{res:no-bounded-length}
...
\noindent Corollary~\ref{res:no-bounded-length} is the exact reason a finite
...
Theorem~\ref{res:actual-escape-endpoint} it needs effective control of
...
\section{The finite evidence}\label{sec:evidence}
...
the inequality in~\eqref{eq:actual-escape}, so that a reader can recheck it in
...
$R=\lpr_{W}(-BF)$ and the bound $K$ of~\eqref{eq:actual-bound}.
\[
...
of~\eqref{eq:actual-escape}, since $\gcd(16,30)=2$, and is displayed to
...
Corollary~\ref{res:no-bounded-length} predicts the shortest possible window to
...
launch.  Lemma~\ref{res:all-scale-lattice} normalises by the half height
...
\section{The remaining arithmetic questions}\label{sec:open}

\begin{problem}[actual cofinal residue escape]\label{prob:producer}
Prove $\Esc(K)$ for the actual digits~\eqref{eq:actual-digit} and the bound
\eqref{eq:actual-bound}.
\end{problem}
...
Theorem~\ref{res:actual-escape-endpoint} the problem is a restatement of
Erd\H{o}s~\#269 for $P=\{2,3,5\}$.

\begin{problem}[nonintegrality of every reduced tail]\label{prob:tails269}
...
\label{res:tails-equivalence}
Statement~\eqref{eq:tail-nonintegrality}, quantified over every $B\ge1$
...
Theorem~\ref{res:actual-cancellation} produces $B$ coprime to $30$ with
...
Problem~\ref{prob:tails269} is therefore a second restatement of the target,
...
Proposition~\ref{res:actual-dichotomy} does not exclude that branch, since it
...
Theorem~\ref{res:infinite-rank} uses, and it is weaker than the joint
...
(\lword{Erdos269/WeightedPhaseCarry.lean}{110}{carry_eq_residueDigit_add_coboundary}{residue-coboundary
form},
\lword{Erdos269/WeightedPhaseCarry.lean}{294}{CartierRealisation}{symbolic
realisation},
...
(\lword{Erdos269/WeightedPhaseCarry.lean}{151}{carryResidue_mem_interval}{carry
interval},
\lword{Erdos269/WeightedPhaseCarry.lean}{158}{residueDigit_mem_interval}{digit
...
refer to the same series, and Theorem~\ref{res:actual-escape-endpoint} shows
...
Theorem~\ref{res:uniform-rank} excludes even approximate separation of its
...
Corollary~\ref{res:no-bounded-length} shows that no bounded-length search can
...
and Theorem~\ref{res:uniform-rank} are ordinary proofs in this paper; the three
finite computations of Section~\ref{sec:evidence} are computational
certificates.
...
\subsection{The historical and catalogue record}\label{long:history}
...
\label{long:denominator-reduction}
...
identity $DX_a=h_aN-Dv_a$ of Lemma~\ref{res:all-scale-lattice} is what supplies
it for the actual orbit, and Theorem~\ref{res:actual-cancellation} is the
...
the right-hand side exceeds that bound, so Proposition~\ref{res:consumer}
applies.
\end{proof}
...
\subsection{Worked finite examples}\label{long:finite-geometry}

The ten smallest values of the running least common multiple at $\{2,3,5\}$ are
tabulated in Section~\ref{sec:lcm}.  They illustrate both parts of
Proposition~\ref{res:cell}: the value is constant on $\{5,6,7\}$ and on
...
For Proposition~\ref{res:fibre} take $(p,q,r)=(2,3,5)$ and the box
...
\subsection{The finite-scan histogram}\label{long:experiments}

The scan of Section~\ref{sec:evidence} covers every $B\le5000$ coprime to $30$
...
By Corollary~\ref{res:no-bounded-length} these histograms describe a bounded
...
Proposition~\ref{res:window-growth}, which is what a reader should expect if
the residues behave like generic ones inside this range.

\subsection{Source inventory}\label{long:sources}
...
rank-two certificate & \lref{Erdos269/ThreePrimeRunningLcm.lean}{904}{kernel_235_minor_eq_neg_one_fifteen}\\
...
residue-coboundary form & \lref{Erdos269/WeightedPhaseCarry.lean}{110}{carry_eq_residueDigit_add_coboundary}\\
carry interval & \lref{Erdos269/WeightedPhaseCarry.lean}{151}{carryResidue_mem_interval}\\
digit interval & \lref{Erdos269/WeightedPhaseCarry.lean}{158}{residueDigit_mem_interval}\\
symbolic realisation & \lref{Erdos269/WeightedPhaseCarry.lean}{294}{CartierRealisation}\\
finite realised span & \lref{Erdos269/WeightedPhaseCarry.lean}{335}{finite_realisedSpan_of_factorisation}\\
...
staircase index engine & \mref{Shared/IrrationalRotationStaircase.lean}{271}{exists\_staircase\_indices}\\
\end{longtable}
