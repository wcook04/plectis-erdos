...
A conditional carry argument isolates the remaining arithmetic.  After a
smooth factor of a hypothetical denominator has been cancelled, a
denominator-dependent cofinal residue-window escape would contradict every
positive bounded reduced carry.  The finite residue contradiction and the
smooth-factor cancellation are checked exactly.  A reproducible integer scan
...
The three-prime problem remains open.  Two independent inputs are missing:
the original series must be shown to produce the stated bounded integral carry,
including the divisibility needed for cancellation, and the actual
\(\{2,3,5\}\) radix and forcing words must satisfy cofinal residue-window
escape.  No irrationality or transcendence result for any three-prime instance
is claimed.
\end{abstract}
...
structure, the radix alphabet for $\{2,3,5\}$, and the finite carry
contradiction.  A scan verifies the required escape only on $106{,}666$
finite denominator/start pairs.  \textbf{Open boundary.} The bounded carry
and cofinal residue-window escape needed for any three-prime instance remain
unproved.
\end{paperresult}
...
denominator reduction and carry extinction remain conditional on their stated
divisibility and cofinal-escape hypotheses.  The dyadic-window scan covers
