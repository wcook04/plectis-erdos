...
\newcommand{\SemanticAuthoredTheoremLike}{145,599}
\newcommand{\SemanticAuthoredInterpreted}{139,799}
\newcommand{\SemanticDirectEvidence}{3,311}
\newcommand{\SemanticContextual}{136,488}
\newcommand{\SemanticStructuralOnly}{5,800}
\newcommand{\SemanticAuthoredPercent}{96.0}
...
that card sit a bounded six-line tour, a mathematical map of ten programmes, a
reviewed public claim map whose registry carries 22 explicit remaining open
propositions in this release, and an
...
At the audited revision the inventory behind them spanned
...
separate mathematical claims.  Later revisions move the module and declaration
totals, and \path{docs/declaration_atlas.json} carries the figure for whichever
checkout the reader has.  The marked figures are a classification
...
repeating a full environment export. A recorded case study, which is an
internal agent rehearsal operated by the author, contains six reasoning
...
audited in the author's environment. As of 31 August 2026, no completed
...
paper includes a replayed session case study, which is an internal agent
rehearsal operated by the author.  \textbf{Limit.} This is a
single-environment prototype with no recorded external cold-clone use, and no
number reported here measures how an outside reader understands the corpus.
\end{paperresult}
...
\node[wide, fill=ForestGreen!6, below=11mm of $(semantic.south)!0.5!(dep.south)$] (claims)
  {{\scriptsize\sffamily\bfseries\color{gray!70!black} PUBLICATION SURFACE}\\[3pt]
   ...
   {\small\bfseries\color{ForestGreen!68!black}208 claims \(\cdot\) 22 remaining open propositions}\\[3pt]
...
The claim map selects 208 results for public exposition in this release. It
records 49 as proved here, 11 as formalised here, 52 as unconditional
progress, 67 as conditional reductions, 10 as verified finite instances, 8 as
proved by ordinary mathematics in a note rather than by a Lean declaration, 3
as cited only, and 8 as open. The ordinary-proof status is deliberately
separate: those results are not Lean-checked, and any kernel-checked
ingredient of such a result is named in its note. Twenty-two explicit frontier
propositions describe the stronger
obligations that survive. These labels are maintainer-reviewed public meaning,
not outputs inferred by the proof kernel. The counts move with the registry;
\path{docs/claims.json} is the authority for the checkout in hand.
...
The underlying tour command returns a six-line card rather than a database dump. It
...
warm two-root dogfood plan it selected no stale modules. Both figures are
internal measurements of the planner on this repository. The test suite covers
...
a synthetic benchmark. It is an internal agent rehearsal operated by the
author, and it is not an outside-contributor result. The ledger contains six reasoning notes, one
...
cold-clone validator to reject the mutation. Those tests are deterministic
mutation tests over the declared contract. This evaluates the declared
first-contact contract, not subjective comprehension. No reader was observed,
and no comprehension time was measured.
...
in \TeX{}, records the declarations that realise a statement, records the
statement and proof dependency edges its author declares, marks each node with
a formalisation-readiness state, and checks that every named declaration
exists in the project or one of its dependencies~\cite{leanblueprint}; it does
not infer those dependency edges, and it does not check that the informal and
formal statements agree.  That declaration-linking layer is prior art, and
nothing in this section claims it as new here.  LeanArchitect infers formal
...
exhaustive inventory from authored interpretation, holds Lean-checked results,
ordinary proofs, computations, and proved obstructions in one record, and
routes a reader into
...
A checkout of an earlier revision answers a narrower question than it appears
to answer. The run implemented here supplies its own authored candidate, names
the candidate declaration inside its request, exercises only the derived-layer
arm, and observes no reader and no agent. It establishes that the runtime at
the earlier revision exposes the exact remaining obligation for a stated target
and that the kernel accepts an authored candidate carrying neither the later
declaration name nor the later module name. Its clone carries the earlier
source with the dependencies of the current tree, so its receipt records the
pins that revision declared alongside the pins the build used. It does not show
that the layers present at that revision helped a reader find and interpret the
mathematics already there, and it does not show that those layers shortened the
work of producing a later result, because
later Git objects, a shared object store, generated dossiers and receipts,
network access, context supplied in the prompt, and a model's familiarity with
public mathematics can each reveal the target.
\path{docs/methodology.json} lists these channels and names the ones the
harness controls; the remainder are limitations of any historical result
reported here.
