...
\newcommand{\SemanticAuthoredTheoremLike}{143,410}
\newcommand{\SemanticAuthoredInterpreted}{139,772}
\newcommand{\SemanticDirectEvidence}{3,284}
\newcommand{\SemanticContextual}{136,488}
\newcommand{\SemanticStructuralOnly}{3,638}
\newcommand{\SemanticAuthoredPercent}{97.5}
...
\begin{figure}[!t]
\centering
\newcommand{\layercount}{\small\bfseries}%
\begin{tikzpicture}[
  wide/.style={paper box, text width=10.9cm},
  branch/.style={paper box, text width=5.0cm, minimum height=3.05cm},
  arr/.style={paper arrow},
  note/.style={paper label}
]
% One exhaustive source, two partial views side by side, one reviewed
% surface.  Each layer's count sits on its own line beneath the title so
% it can never collide with it.
\node[wide, fill=MidnightBlue!5] (atlas) at (0,0)
  {\papereyebrow{SOURCE INVENTORY}\\[3pt]
   {\large\textbf{Exhaustive declaration atlas}}\\[2pt]
   {\layercount\color{MidnightBlue!85}1,019 modules \(\cdot\) 153,253 declarations}\\[3pt]
   \footnotesize Every live declaration has source coordinates, a kind, a
   module role, and a route.};
\node[branch, fill=BurntOrange!6, below=12mm of atlas, xshift=-3.05cm] (semantic)
  {\papereyebrow{AUTHORED MEANING}\\[3pt]
   {\normalsize\textbf{Tiered semantic graph}}\\[3pt]
   {\layercount\color{BurntOrange!80!black}\SemanticAuthoredTheoremLike\ linked theorem-like declarations}\\[4pt]
   \footnotesize Authored interpretations are distinguished from exact
   source-structural families.};
\node[branch, fill=TealBlue!5, below=12mm of atlas, xshift=3.05cm] (dep)
  {\papereyebrow{FORMAL STRUCTURE}\\[3pt]
   {\normalsize\textbf{Loaded-root dependency index}}\\[3pt]
   {\layercount\color{TealBlue!72!black}exact reference edges}\\[4pt]
   \footnotesize Type and value references are joined to source; unresolved
   rows remain explicit.};
\node[wide, fill=ForestGreen!6, below=12mm of $(semantic.south)!0.5!(dep.south)$] (claims)
  {\papereyebrow{PUBLICATION SURFACE}\\[3pt]
   {\large\textbf{Reviewed public claim map}}\\[2pt]
   {\layercount\color{ForestGreen!68!black}101 claims \(\cdot\) 5 open propositions}\\[3pt]
   \footnotesize Public wording records status, evidence, the claim ceiling,
   and the nearest stronger statement.};
% Fork: one trunk from the atlas, two drops.
\coordinate (fork) at ($(atlas.south)+(0,-5mm)$);
\draw[line width=.7pt, draw=gray!72!black] (atlas.south) -- (fork);
\draw[arr, rounded corners=2pt] (fork) -| (semantic.north)
  node[note, pos=.75, left=1pt] {authored selection};
\draw[arr, rounded corners=2pt] (fork) -| (dep.north)
  node[note, pos=.75, right=1pt] {formal slice};
% Join: two drops into one trunk above the claim map.
\coordinate (join) at ($(claims.north)+(0,5mm)$);
\draw[line width=.7pt, draw=gray!72!black, rounded corners=2pt]
  (semantic.south) |- (join) node[note, pos=.25, left=1pt] {reviewed selection};
\draw[line width=.7pt, draw=gray!72!black, rounded corners=2pt]
  (dep.south) |- (join);
\draw[arr] (join) -- (claims.north);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
  stagebox/.style={paper stage, text width=2.3cm, minimum height=21.5mm, inner xsep=4pt},
...
% Five equal stages; the two tinted bands behind them are the two
% capabilities, and the dashed seam between the bands is the authority
% boundary.  Everything left of the seam is advisory; everything right of
% it is a replayable Lean process.
\node[nav] (clone) at (0,0)
  {\papereyebrow{1 \; CLONE}\\[3pt]
   \textbf{Cold clone}\\[2pt]\footnotesize committed projections};
\node[nav] (tour) at (2.75,0)
  {\papereyebrow{2 \; ROUTE}\\[3pt]
   \textbf{Bounded tour}\\[2pt]\footnotesize intent and scope};
\node[nav] (cone) at (5.5,0)
  {\papereyebrow{3 \; FOCUS}\\[3pt]
   \textbf{Proof cone}\\[2pt]\footnotesize exact declaration};
\node[exact] (probe) at (8.25,0)
  {\papereyebrow{4 \; VERIFY}\\[3pt]
   \textbf{Lean probe}\\[2pt]\footnotesize kernel verdict};
\node[exact] (claim) at (11,0)
  {\papereyebrow{5 \; BIND}\\[3pt]
   \textbf{Cited claim}\\[2pt]\footnotesize replay receipt};
\draw[arr] (clone) -- (tour);
\draw[arr] (tour) -- (cone);
\draw[arr] (cone) -- (probe);
\draw[arr] (probe) -- (claim);
\begin{pgfonlayer}{paperbackground}
  \fill[paper band, fill=MidnightBlue!7]
    ([xshift=-3mm,yshift=-12.5mm]clone.south west) rectangle
    ([xshift=2.6mm,yshift=3.5mm]cone.north east);
  \fill[paper band, fill=ForestGreen!7]
    ([xshift=-2.6mm,yshift=-12.5mm]probe.south west) rectangle
    ([xshift=3mm,yshift=3.5mm]claim.north east);
\end{pgfonlayer}
\node[paper band label, text=MidnightBlue!75!black, align=center]
  at ($(tour.south)+(0,-7.6mm)$)
  {NAVIGATION\\[1pt]{\mdseries fast, bounded, advisory}};
\node[paper band label, text=ForestGreen!55!black, align=center]
  at ($(probe.south)!0.5!(claim.south)+(0,-7.6mm)$)
  {PROOF\\[1pt]{\mdseries exact input, pinned process, replayable}};
\coordinate (seam) at ($(cone.east)!0.5!(probe.west)$);
\draw[paper boundary, draw=gray!75]
  ([yshift=-14.5mm]seam |- clone.south) -- ([yshift=5.5mm]seam |- clone.north);
\node[paper label, above] at ([yshift=5.5mm]seam |- clone.north) {authority boundary};
\end{tikzpicture}
