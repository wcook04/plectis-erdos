...
all-base independence step uses Martin externally.  The finite arithmetic
...
primes only.  The statement below combines Lean-checked arithmetic and
finite-index layers with a paper-level independence argument.  Lean proves the zero-residue
relation
...
$k^e+1$.  These are combinatorial statements about the proposed basis index;
they do not identify its coordinates with linearly independent section
functions.
...
normalisation after exact division.  Thus the zero channel, arithmetic
reduction, and finite-index/cardinality layers have kernel receipts, while
Martin remains the external authority for the last independence step.
...
\item The $k$-kernel rank statement of Theorem~\ref{thm:kkernelrank} is
  ...
  zero-residue, composite-base arithmetic, and finite-index/cardinality layers
  are Lean-checked, but the
...
infinite-dimensionality consequence that Coons already implies.  The
