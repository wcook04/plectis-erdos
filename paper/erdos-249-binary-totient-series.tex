...
\renewcommand{\commit}{234347aac64e8f186ca816281f68fa9ed1365b1b}
\renewcommand{\commitshort}{234347aac64e}
\hypersetup{
...
A second exact frontier removes an apparently free search variable.  Given a
basepoint $c$ and a positive odd $v$, choose $H>0$ with $\ph(v)\mid H$ and
$vM=2^H-1$.  Irrationality is equivalent to the existence of such $H,M$ for
which the least nonnegative residue
\[
 \bigl(-\operatorname{totientBlock}(H,c)\bigr)\bmod M
\]
lies strictly between the two endpoint intervals of width $c+H+1$
(\lword{Erdos249/CyclotomicAnchoredKill.lean}{2976}{fullMersenneCanonicalBasepointResidueGapSupply_iff_irrational}{canonical Mersenne equivalence}).
Thus the basepoint is canonically $N=c$; no remote $N$ remains to be found.
The residue evolves by an exact affine doubling recurrence.  This is an exact
reformulation, not a proof of the still-missing centered-gap supply.

A third exact frontier shows that locking certificate depth to its period
costs nothing.  One arbitrary-depth seed certificate on a $d$-ray forces
full-depth certificates on an eventually $2$-syndetic set of multipliers, so
the pointwise full-depth predicate, its cofinal version, the arbitrary-depth
period-multiple supply, and irrationality of $\Stot$ are equivalent
...
\lword{Erdos249/FullDepthRayAmplifier.lean}{406}{apFullDepthEscape_iff_irrational}{pointwise equivalence};
\lword{Erdos249/FullDepthRayAmplifier.lean}{438}{cofinalFullDepthKillSupply_iff_irrational}{cofinal equivalence}).
This amplification does not produce the initial non-integral shift or seed;
Problem~\#249 therefore remains open.
...
\externalreviewobject{The paper first exposes the exact actual-LCM,
canonical-Mersenne, and full-depth period-multiple frontiers, followed by two
distinct quantitative consumers for Problem~\#249: separation of the actual
...
open.}{A dedicated ActualLcmOrbit Comparator package carries the exact orbit
identity and the sharp equivalence
\mword{Erdos249257/TotientActualLcmOrbitNonintegrality.lean}{37}
      {irrational_totientSeries_iff_actualLcmOrbitNonintegralitySupply}
      {between irrationality and cofinal actual-LCM non-integrality}.
Separate packages carry the canonical-Mersenne and full-depth equivalences,
the odd-core basis and finite-rank rows, and the sharp rank-one/direct-sum
no-go family with its rational-linear-form endpoint.  The stronger
actual-LCM separation consumer, the first-harmonic conditional consumer, the
top-edge obstruction, and the $38=2\cdot19$ counterexample remain adjacent
source evidence rather than declarations selected by those packages.
...
The canonical Mersenne formulation removes the remote-basepoint search, and
the full-depth amplifier proves that forcing certificate depth to equal the
period loses no strength: one seed gives eventual $2$-syndeticity on its ray.
Both are exact reformulations of the open endpoint, not new producers.
...
denominator; the older centered-residue supply searched for a remote
$N\ge\max(c,N_0)$.  The canonical statement implies that older supply by
...
\lword{Erdos249/CyclotomicAnchoredKill.lean}{2185}{fullMersenneBlockResidue_succ}{residue recurrence},
\lword{Erdos249/CyclotomicAnchoredKill.lean}{2943}{fullMersenneCenteredResidueGapSupply_of_canonicalBasepoint}{dominance over the remote supply},
\lword{Erdos249/CyclotomicAnchoredKill.lean}{2976}{fullMersenneCanonicalBasepointResidueGapSupply_iff_irrational}{exact equivalence}.}
...
supply and to irrationality.  The theorem proves that the depth lock is free;
...
\subsection{Exact full-depth reduction and a stronger sufficient producer}
...
period.  The source-current amplifier removes that asymmetry completely.
...
What remains open is the producer itself: the amplifier converts one
