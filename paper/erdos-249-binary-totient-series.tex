...
\hypersetup{
  pdftitle={Irrationality of the fixed-resolution totient observables, and a basis for the 2-kernel of Euler's totient, for Erdős Problem 249},
...
\runninghead{Erd\H{o}s \#249: irrationality of the totient observables}
\title{Irrationality of the Fixed-Resolution\\Totient Observables}
\subtitle{An unconditional theorem, exact finite-level ranks, and the open
boundary of Erd\H{o}s Problem~\#249}
\author{Will Cook}
\date{July 2026}
...
\begin{abstract}
\noindent
Erd\H{o}s Problem~\#249 asks whether $\Stot=\sum_{n\ge1}\ph(n)/2^n$ is
irrational.  It is open.  Each result below carries its hypotheses, the part
of its proof a Lean kernel checked, and the part that rests on exact
computation or on ordinary mathematics.

The leading result is an unconditional irrationality theorem about the totient
word.  Let $m\ge2$, let $f:\N\to\Z$ satisfy $f(0)=0$, and let $r<m$ satisfy
$\gcd(r+1,m)=1$ and $f(r)\ne0$.  Then $\sum_nf(\ph(n)\bmod m)/2^n$ is
irrational.  In particular $A_m=\sum_n(\ph(n)\bmod m)/2^n$ is irrational for
every $m\ge3$, and $m\ge3$ is sharp because $A_1=0$ and $A_2=3/4$.  This
settles the fixed-resolution observables of the totient word completely, and it
says nothing about $\Stot$ itself, whose coefficients are unreduced;
Erd\H{o}s~\#249 is not settled here and no instance of it is.  The proof is
checked by the Lean kernel as \texttt{irrational\_totientObservable}, with the
corollary \texttt{residue\_series\_irrational}, in the private authoring tree,
and it is absent from the release tree at commit~\texttt{\commitshort};
Section~\ref{sec:results} gives its private-source coordinate and marks it for
the next formal-source cycle.  The mechanism is the isolated-pulse argument of
Erd\H{o}s 1948, and reducing $\ph$ modulo $m$ is what makes the pulses
isolated.

The second result is a proved class obstruction with an explicit constant.  Put
$\Theta_2=\sum_{d\ge1}\mu(d)/(2^d-1)^2$, which equals $\Stot-1/2$ by the
convolution identity $\ph=\mu*\mathrm{id}$ recorded by Fan~\cite{fan2026}, and
for $e\ge1$ and $Y\ge4$ put
\[
  Q(e,Y)=\frac{\bigl(\sum_{d=1}^{Y}\mu(d)/(2^d-1)^{e+2}\bigr)^2}
              {\sum_{d=1}^{Y}\mu(d)/(2^d-1)^{2e+2}}.
\]
Then $(e,Y)=(1,5)$ is the unique minimiser of $Q$, every admissible $Q(e,Y)$
satisfies $Q(e,Y)-\Theta_2>21/320$, every nonempty finite positive weighted
average of admissible values keeps the same strict separation, and
$|q\Theta_2-p|>21q/320$ whenever $Q(e,Y)=p/q$ with $q\ge1$.  Every step is
Lean-checked in the release tree.  The complete positive rank-one and positive
direct-sum approximation mechanism is closed by this constant.  Signed
cancellation and genuinely coupled higher-rank approximants lie outside the
statement.

The third result is a conditional reduction, and it names the remaining
analytic obligation exactly.  Write
\[
  D_{h,N,L}=\sum_{j=0}^{L-1}\bigl(\ph(N+h+1+j)-\ph(N+1+j)\bigr)2^{L-1-j}.
\]
Suppose that for every $h>0$ and every $X_0$ there are $X\ge\max(X_0,1)$ and
$L$ with $16(2X+h+L+2)\le2^L$ and
\[
  \Bigl\|\sum_{N=X}^{2X-1}e^{2\pi iD_{h,N,L}/2^L}\Bigr\|\le\tfrac{21}{25}X .
\]
Then $\Stot$ is irrational.  The chain from that norm gap through the
real-part consumer to certificate supply is Lean-checked with no remaining
gap, and the hypothesis is open.  It is one exponential-sum cancellation bound
over a single dyadic block, which is the form in which this note states its
request to a specialist in analytic number theory.

The fourth result is a finite exclusion.  Exact rational bracketing of $\Stot$ at
$80\,000$ bits forces $23\,438$ continued-fraction partial quotients, each
retained only when both endpoint integer parts agree, so a representation
$\Stot=a/q$ in lowest terms has $q\ge2^{39989}$ and $q>10^{12038}$.  That bound
is an exact finite computation with no floating-point decision anywhere in it
and with no Lean proof.  The strongest kernel-checked size exclusion is the
Farey-mediant bound
$q>79\,639\,646\,646\,701\,375\,323\,355\,774\,875\,831\,053\approx7.96\times10^{34}$.
The two figures are recorded separately because their evidence classes differ.

The structural results follow.  For $j\ge0$ and $0\le r<2^j$ write
$\ph_{j,r}(n)=\ph(2^jn+r)$.  The family
$\{\ph_{0,0},\ph_{1,0}\}\cup\{\ph_{j,r}:j\ge1,\ 0<r<2^j,\ r\ {\rm odd}\}$
is a $\Q$-basis for the span of the $2$-kernel of Euler's totient, so the
...
non-$2$-regularity theorem, and the linear-independence half is subsumed by
Martin's Theorem~1.
...
For $H_a=\lcm(1,\ldots,2^a)$ and
$R_a=\operatorname{totientTail}(2H_a)-\operatorname{totientTail}(H_a)$, the
exact criterion
\[
  \Stot\notin\Q
...
\]
is proved, and canonical Mersenne-residue and full-depth forms sharpen it.
None supplies the needed nonintegral seed.  Two further quantitative routes
remain conditional: one separates the actual LCM orbit from the integers, and
the other controls four terms in a first-harmonic decomposition.  Their
cofinal hypotheses are unproved.  The exact open boundary is arithmetic
control of the actual tail.
\end{abstract}

\begin{paperresult}{What is proved here}
\textbf{Sharp obstruction, Lean-checked.} The positive rank-one
M\"obius--Mersenne cone has the unique minimiser $(e,Y)=(1,5)$ and the uniform
floor $Q(e,Y)-\Theta_2>21/320$, stable under every nonempty finite positive
mixture.  \textbf{The remaining analytic obligation, named.} A cofinal
first-harmonic norm gap of size $\tfrac{21}{25}X$ over a dyadic block implies
irrationality, by a Lean-checked chain with no remaining gap; the gap itself is
open.  \textbf{Quantitative floor.} An exact rational bracket at $80\,000$
bits gives $q\ge2^{39989}$ and $q>10^{12038}$ for any representation
$\Stot=a/q$; the kernel-checked floor is $7.96\times10^{34}$.
...
have rank exactly $2^e+1$ for $e\ge1$.
...
The principal unconditional result is an irrationality theorem about the
totient word at a fixed resolution: for $m\ge2$, an integer-valued letter map
$f$ with $f(0)=0$ and $f(r)\ne0$ at some $r<m$ with $\gcd(r+1,m)=1$, the series
$\sum_nf(\ph(n)\bmod m)/2^n$ is irrational, so $A_m=\sum_n(\ph(n)\bmod m)/2^n$
is irrational for every $m\ge3$.  It carries no hypothesis on $\Stot$ and
decides no instance of Erd\H{o}s~\#249, whose coefficients are the unreduced
values $\ph(n)$; the problem stays open.  Its Lean development is in the
private authoring tree and is absent from this checkpoint, and
item~\ref{res:residueobservable} records the coordinates.

The strongest proved obstruction is the sharp determination of the
positive rank-one M\"obius--Mersenne cone: one unique minimizer at
$(e,Y)=(1,5)$, the uniform $21/320$ floor, the $1/16$ versus $1/15$ boundary,
and closure under arbitrary finite positive mixing.  It closes the whole
positive rank-one and positive direct-sum approximation mechanism with an
explicit constant, and it leaves signed cancellation and coupled higher-rank
kernels open.  The single remaining analytic obligation is stated next, as one
exponential-sum cancellation bound over a dyadic block whose consumer chain is
checked end to end.  The strongest quantitative statement about the constant
itself is a certified continued-fraction floor, $q>10^{12038}$ for any
representation $\Stot=a/q$, proved by exact rational bracketing rather than by
a Lean kernel.  A second structural result records that the two zero-residue
sections $\ph_{0,0}$ and $\ph_{1,0}$, together with the odd-residue sections,
form a rational basis, and that the two reduction identities below generate
every rational relation among the sections.  It gives exact finite-level rank
and no rationality-to-finite-rank bridge.
...
\item\label{res:residueobservable} \stag{proved in the authoring tree; absent from this checkpoint}
\textbf{Fixed-resolution observables of the totient word are irrational.}
Let $m\ge2$, let $f:\N\to\Z$ satisfy $f(0)=0$, and let $r<m$ satisfy
$\gcd(r+1,m)=1$ and $f(r)\ne0$.  Then
\[
 \sum_{n\ge0}\frac{f(\ph(n)\bmod m)}{2^n}
\]
is irrational.  At $m=2^k$ the unit condition holds automatically at every
even residue $r$, because $r+1$ is then odd.  In particular the least-residue
series $A_m=\sum_n(\ph(n)\bmod m)/2^n$ is irrational for every $m\ge3$, and
$m\ge3$ is sharp because $A_1=0$ and $A_2=3/4$.  The proof supplies cofinally
many two-sided isolated pulses by the Chinese remainder theorem together with
Dirichlet's theorem, converts one isolated pulse into a quantitative
separation of $qT$ from every integer, and feeds that into a
number-theory-free dyadic criterion.  The mechanism is the isolated-pulse
argument of Erd\H{o}s~\cite{erdos1948}; reducing $\ph$ modulo $m$ is what
makes the pulses isolated, and the prior-art audit located no exact
antecedent for the statement.

Two boundaries.  First, this says nothing about $\Stot$, whose coefficients
are the unreduced values $\ph(n)$: the parent question stays open and stays
distinct.  Second, the Lean development of this theorem lives in the private
authoring tree and is \emph{not} present in the release tree at
commit~\texttt{\commitshort}.  Its source coordinate there is
\texttt{ErdosProblems/Erdos249/Residue\-Class\-Totient\-Series.lean}, with
\texttt{irrational\_totientObservable} at line~502, the dyadic form
\texttt{fixed\_resolution\_observable\_irrational} at line~544, and the
corollary \texttt{residue\_series\_irrational} at line~570.  It is marked for
the next formal-source cycle, and no link into this checkpoint is given
because none would resolve.
{\footnotesize\emph{Registry:} no row, because the declarations are absent
from this checkpoint.}

\item\label{res:rankonefloor} \stag{checked; no registry row}
...
Every ingredient is a Lean proof term: the exact evaluation of the finite
M\"obius--Mersenne prefixes, the uniform interval bound that reduces the
two-parameter admissible family to a finite comparison, and each of the six
conclusions below.  The identity $\Theta_2=\Stot-1/2$ is Fan's, recorded in
his comment on the catalogue page~\cite{fan2026}; the floor is what is proved
here.  The remaining obligation is exactly the excluded class: an approximant
family with signed cancellation, or a genuinely coupled higher-rank kernel.
...
\emph{Registry:} no row currently owns these declarations.}

\item\label{res:firstharmonicnormgap} \stag{checked; no registry row}
\textbf{The first-harmonic norm gap, and the one estimate it needs.}
With $D_{h,N,L}$ as defined above, suppose that for every $h>0$ and every
$X_0$ there exist $X\ge\max(X_0,1)$ and $L$ with
\[
 16(2X+h+L+2)\le2^L,
 \qquad
 \Bigl\|\sum_{N=X}^{2X-1}e^{2\pi iD_{h,N,L}/2^L}\Bigr\|\le\tfrac{21}{25}X .
\]
Then $\Stot$ is irrational.  The three links of the chain are separately
checked: the complex saving dominates the real-part saving used by the
certificate theorem, a constant-saving first-harmonic bound on one dyadic
block forces a certified non-integrality witness inside that block, and a
cofinal supply of such witnesses is irrationality.  Nothing in the chain is
conditional.  The hypothesis is the whole remaining obligation, and it is one
cancellation statement for an exponential sum over a dyadic block of window
discrepancies, at the resolution $2^L$ forced by the room inequality.  The
room inequality is not decorative: a certificate at depth $L$ already
requires $2(N+h+L+2)<2^L$, so $L\gtrsim\log_2(N+h)$ and the sum is taken at
full arithmetic resolution.  Naming this estimate is more informative than
recording a generic missing producer, because it is a statement a specialist
in analytic number theory can attack directly.
{\footnotesize\emph{Checked:}
\mword{Erdos249257/FirstHarmonicPivot.lean}{75}{DTWFirstHarmonicNormGap}{the hypothesis},
\mword{Erdos249257/FirstHarmonicPivot.lean}{53}{exists_certifiedKill_of_first_harmonic_norm_gap}{one-block consumer},
\mword{Erdos249257/FirstHarmonicPivot.lean}{83}{irrational_totient_series_of_first_harmonic_norm_gap}{the reduction}.
\emph{Registry:} the \texttt{first\_harmonic\_certificate\_interface} row owns
the real-part interface in \texttt{FirstHarmonicGap.lean}; the three named here are its consumers
declarations.}

\item\label{res:cfdenominator} \stag{exact computation; no registry row}
\textbf{Certified continued-fraction denominator floor.}  Bracket $\Stot$ by
exact integers $\mathrm{lo}\le2^B\Stot\le\mathrm{hi}$ at $B=80\,000$ bits and
run the interval continued fraction, emitting a partial quotient only when
both endpoints agree on it.  This forces $23\,438$ partial quotients.  If
$\Stot=a/q$ in lowest terms with $q>0$, then
\[
 q\ge2^{39989}\qquad\hbox{and}\qquad q>10^{12038}.
\]
The two power bounds are checked separately against the integer denominator
of the last certified convergent; the bit length $39\,990$ and the decimal
digit count $12\,039$ of that denominator are lengths rather than exponents,
and printing them as exponents overstates the result by one in each base.
The preceding convergent is separated from the enclosure, and the same
arithmetic reproduces the known expansions of $e$ and $\pi$.  The evidence
class here is an exact finite computation with no floating-point decision
anywhere in it.  There is no Lean proof of this bound, and it is recorded
separately from \ref{res:denominator} for that reason.  The largest observed
partial quotient is $29\,403$, at index $15\,470$, so the prefix shows no
anomalous rational approach.
{\footnotesize\emph{Computed:} verifier
\texttt{formal\_math/probes/certified\_continued\_fraction.py}, receipt
\texttt{state/formal\_math/probes/erdos249\_certified\_cf\_receipt.json}.
\emph{Registry:} the claim taxonomy of this release has no status for an
exact computation that carries no Lean proof, so no row owns this result.}

\item\label{res:perioddeposits} \stag{checked; no registry row}
\textbf{Eight denominator exclusions on new odd classes.}  The lcm-diagonal
bank certifies kills at $\lcm(1,\ldots,t)$ through $t=64$, which covers every
odd $v$ whose multiplicative order of $2$ divides $\lcm(1,\ldots,64)$.  The
first prime-power periods it cannot reach are
$h\in\{67,81,97,101,121,125,127,128\}$.  At basepoint $N=300$ each of the
eight carries a certificate, with residues
$1654,540,7616,1270,478,762,530,1552$ at depths
$11,13,13,11,10,18,11,11$.  Each certificate yields
$\Stot\ne r$ for every rational $r$ whose denominator divides
$2^{300}(2^h-1)$, so the excluded odd classes now include the Cole factors
$2^{67}-1=193\,707\,721\cdot761\,838\,257\,287$ and the Mersenne prime
$2^{127}-1$.  The period-$67$ kill is additionally deposited at its own depth
$L=h=67$, which is the depth-locked form \ref{res:fulldepth} demands, made
concrete at one point of the ray $d=67$.  These are eight fixed instances,
and no cofinal supply follows from them.
{\footnotesize\emph{Checked:}
\lword{Erdos249/PeriodMultipleEscape.lean}{471}{certifiedKill_67_300}{period $67$},
\lword{Erdos249/PeriodMultipleEscape.lean}{490}{certifiedKill_127_300}{period $127$},
\lword{Erdos249/PeriodMultipleEscape.lean}{498}{certifiedKill_67_300_fullDepth}{period $67$ at its own depth},
\lword{Erdos249/PeriodMultipleEscape.lean}{460}{totient_series_ne_rat_of_certifiedKill}{the generic exclusion},
\lword{Erdos249/PeriodMultipleEscape.lean}{504}{totient_series_ne_rat_of_den_dvd_67_300}{the $67$ exclusion},
\lword{Erdos249/PeriodMultipleEscape.lean}{534}{totient_series_ne_rat_of_den_dvd_127_300}{the $127$ exclusion};
the remaining six certificates and six exclusions sit between these lines.
\emph{Registry:} no row currently owns these declarations.}
...
generate, a one-line consequence of the checked statements and not a further
one.
{\footnotesize\emph{Checked:}
...
reductions as well.
{\footnotesize\emph{Checked:}
\mword{Erdos249257/TotientMahlerDefect.lean}{946}{linearIndependent_canonicalTotientKernelFamily}{independence},
\mword{Erdos249257/TotientMahlerDefect.lean}{1000}{finrank_canonicalTotientKernel_eq}{dimension},
\mword{Erdos249257/TotientMahlerDefect.lean}{1085}{finrank_totientKernelThroughLevelFamily_eq}{the exact finite-truncation rank};
...
the odd-core family.
\mword{Erdos249257/TotientMahlerDefect.lean}{1146}{not_finiteDimensional_span_fullTotientKernel}{infinite-dimensionality directly}.}

\item\label{res:orbitphase} \stag{checked; no registry row}
\textbf{Qualitative orbit normal form.}  Let $\theta_{h,N}$ be the first
tail-orbit phase at shift $h$ and basepoint $N$, the unit complex number whose
argument is $2\pi$ times the fractional part of $(2^h-1)\Stot$ doubled $N$
times.  Cofinal left-half-plane visits at every positive shift, that is
$\forall h>0\ \forall N_0\ \exists N\ge N_0$ with
$\operatorname{Re}\theta_{h,N}\le0$, are equivalent to irrationality of
$\Stot$.  The reverse implication is the short one: if one tail difference
were integral, its phase would equal $1$ and would stay there at every larger
basepoint, contradicting the visit at that same shift.  The forward
implication comes from doubling expansivity applied to the non-dyadicity of
the exact initial angle, which irrationality supplies.  This carries no
quantitative parameter at all, which is what distinguishes it from the block
density and prime alignment conditions of the same file; it also supplies no
visit.
{\footnotesize\emph{Checked:}
\lword{Erdos249/TotientStrictPrimeEscape.lean}{412}{TotientTailOrbitCofinalNonpositive}{the condition},
\lword{Erdos249/TotientStrictPrimeEscape.lean}{360}{cofinally_tailOrbitFirstExp_re_nonpos_of_not_dyadic}{expansivity},
\lword{Erdos249/TotientStrictPrimeEscape.lean}{418}{totientTailOrbitCofinalNonpositive_of_irrational}{forward},
\lword{Erdos249/TotientStrictPrimeEscape.lean}{445}{totientTailOrbitCofinalNonpositive_iff_irrational}{the equivalence}.
\emph{Registry:} the \texttt{strict\_prime\_tail\_orbit\_gap} row owns the
block-density and prime-alignment declarations of this file; the four named here are its consumers.}

\item\label{res:paritycoboundary} \stag{checked; no registry row}
\textbf{Parity, boundedness and aperiodicity together force nothing.}
There is $c:\N\to\N$ with $c(n)\le6$ and $c(n)\le n$ for every $n$, with
$c(n)\equiv\ph(n)\pmod2$ for every $n$, and not eventually periodic in the
strong sense that for every $N$, $G$ and $K$ there are $K$ explicit carry
pulses beyond $N$ pairwise separated by more than $G$; and yet
$\sum_{n}c(n)/2^{n}=3/2$.  The construction replaces the pair $(4,4)$ at
$(m,m+1)$ by $(6,0)$ at each lacunary rank $m=2^{k+3}$, which is an exact
binary coboundary and therefore changes no partial-sum limit argument that
sees only the parity word.  Consequently the coarse-resolution quadrant is
closed: no proof that reads only $\ph(n)\bmod 2$, a uniform bound on the
coefficients, and failure of eventual periodicity can establish irrationality
of $\Stot$.  This is a countermodel for a proof class, and it says nothing
about $\Stot$ itself.
{\footnotesize\emph{Checked:}
\mword{Erdos249257/TotientParityCoboundaryCountermodel.lean}{637}{exists_totientParity_arbitrarilyManySeparatedCarry_rational_countermodel}{the countermodel}.
\emph{Registry:} no row currently owns this declaration.}

\item\label{res:lcmjump} \stag{checked; no registry row}
\textbf{The lcm-jump shift coincides with the diagonal at $t=2^k-1$.}
Two separate cofinal supplies were developed for \#249: kills at the lcm-jump
shift $\lcm(1,\ldots,t+1)-\lcm(1,\ldots,t)$ and kills at the diagonal shift
$\lcm(1,\ldots,t)$.  Since $\lcm(1,\ldots,t)$ grows by a factor $q$ exactly
when $t+1$ is a prime power $q$, the jump length is
$(q-1)\lcm(1,\ldots,t)$, so at $q=2$, that is at $t=2^k-1$, the two shifts are
the same natural number.  At those $t$ every deposited diagonal certificate is
a jump certificate with no new totient arithmetic.  The scales
$t=1,3,7,31$ are discharged this way, the last from the wave-$31$ diagonal
certificate at $\lcm(1,\ldots,31)=72\,201\,776\,446\,800$ and depth $49$.  The
jump hypothesis is cofinal and these deposits are finite, so the supply
obligation is unchanged; what is removed is the appearance that the jump route
is a separate unproduced obstacle.  The scales $t=15$ and $t=63$ carry no
deposit, because \texttt{decide} cannot evaluate $\ph$ near $2^{24}$ and
$2^{90}$; the emitting generator that produced the deposited scales would
supply both.
{\footnotesize\emph{Checked:}
\lword{Erdos249/LcmJumpKillFromDiagonal.lean}{62}{periodLcm_jump_eq_height_at_thirtyone}{the coincidence at $t=31$},
\lword{Erdos249/LcmJumpKillFromDiagonal.lean}{68}{certifiedKill_lcm_jump_at_seven}{$t=7$},
\lword{Erdos249/LcmJumpKillFromDiagonal.lean}{109}{certifiedKill_lcm_jump_at_thirtyone}{$t=31$}.
\emph{Registry:} no row currently owns these declarations.}
...
{\footnotesize\emph{Checked:}
\mword{Erdos249257/TotientCarryKernelRigidity.lean}{301}{not_irrational_totientSeries_implies_unbounded_carryRank_unconditional}{the barrier},
...
\item\label{res:canonicalmersenne} \stag{checked; no registry row}
...
\lword{Erdos249/CyclotomicAnchoredKill.lean}{2863}{FullMersenneCanonicalBasepointResidueGapSupply}{defined at line 2863}.
\emph{Registry:} no row currently owns these declarations.}

\item\label{res:fulldepth} \stag{checked; no registry row}
...
$q>79\,639\,646\,646\,701\,375\,323\,355\,774\,875\,831\,053\approx7.96\times10^{34}$.
This is the strongest \emph{kernel-checked} size exclusion for $\Stot$.  The
strongest size exclusion of any evidence class is the certified
continued-fraction floor $q>10^{12038}$ of \ref{res:cfdenominator}, which
carries no Lean proof; a reader comparing the two should compare their
evidence classes as well as their sizes.
...
{\footnotesize\emph{Checked:}
\mword{Erdos249257/CertificateKernel.lean}{18559}{tsum_totient_div_pow_two_ne_int_div_of_den_le_79639646646701375323355774875831053}{exclusion},
\mword{Erdos249257/CertificateKernel.lean}{18572}{tsum_totient_div_pow_two_ne_ratCast_of_den_le_79639646646701375323355774875831053}{reduced-denominator form},
...
{\footnotesize\emph{Checked:}
\mword{Erdos249257/CertificateKernel.lean}{18633}{tsum_primWeight_div_two_pow_sub_one_eq_totient_series}{identity},
...
\emph{Registry:} the Lambert-ladder row owns the neighbouring rungs; these
five declarations are owned separately, as the formal source notes below record.}
...
\mword{Erdos249257/DiagonalFreshLossBridge.lean}{2929}{irrational_totientSeries_of_canonicalAdjacentSuffixJumpSlackSupply}{conditional irrationality theorem}.}
...
\mword{Erdos249257/TotientActualLcmOrbitNonintegrality.lean}{52}{irrational_totientSeries_of_actualLcmOrbitNonintegralitySupply}{forward implication}.}
...
strictly positive.  Under the longer-window condition
$J+K+(a+6)<2\cdot2^a$, if this difference is represented by an integer
and the dyadic modulus has room for the directed endpoint strip, the carry survivor is
...
{\footnotesize\emph{Checked:}
\mword{Erdos249257/CertificateKernel.lean}{12999}{irrational_ratWeightSeries_eventuallyPeriodic}{the periodic theorem}.}
...
\mword{Erdos249257/PivotAntiReconstruction.lean}{1775}{dtwWindowSeparatedPairs_iff_irrational_totient_series}{window-separated pairs}.}
...
is not claimed.
{\footnotesize\emph{Checked:}
\mword{Erdos249257/CertificateKernel.lean}{16213}{irrational_tsum_totient_div_pow_two_of_gap_certificate_supply}{the conditional implication}.
\emph{Registry:} no row currently owns this declaration.}
...
(\mword{Erdos249257/CertificateKernel.lean}{18622}{tsum_moebius_div_two_pow_sub_one_eq_half}{checked})
and $\sum_{d\ge1}\ph(d)/(2^d-1)=2$
(\mword{Erdos249257/CertificateKernel.lean}{18617}{tsum_totient_div_two_pow_sub_one_eq_two}{checked}).
...
(\mword{Erdos249257/CertificateKernel.lean}{18642}{totient_series_eq_half_add_moebius_mersenne_square}{checked})
...
\]
(\mword{Erdos249257/CertificateKernel.lean}{18745}{totient_series_eq_half_add_visible_coprime_pairs}{checked}),
...
\mword{Erdos249257/TotientMahlerDefect.lean}{893}{exists_separatedMinorCertificate_totientAffineOddFamily}{the checked evaluation minor}.
...
(\mword{Erdos249257/TotientMahlerDefect.lean}{834}{paritySeparatedMatrix_det_ne_zero}{checked}).
...
\ref{res:rank}
(\mword{Erdos249257/TotientMahlerDefect.lean}{946}{linearIndependent_canonicalTotientKernelFamily}{checked}).
...
(\mword{Erdos249257/CertificateKernel.lean}{16174}{tsum_totient_div_pow_two_ne_int_div_of_totient_gap_certificate}{checked}).
...
(\mword{Erdos249257/CertificateKernel.lean}{18760}{tsum_visible_coprime_pairs_ne_int_div_of_den_le_39819823323350687661677887437915526}{checked}),
...
bound, not an assumption about the sign of a finite residue.  For a depth $K$
with $J+K+(a+6)<2\cdot2^a$, positivity also holds at the shifted offset
$J+K$.  In an integral orbit the carry recurrence identifies that later
difference with the carry state: the true
...
\paragraph{The independent denominator exclusions.}
...
is a computation rather than an idea.  The certified continued-fraction floor
$q\ge2^{39989}$ of \ref{res:cfdenominator} is the same kind of statement run
much further, at $80\,000$ committed bits, and it carries the same limitation:
a finite prefix leaves every later partial quotient open.  Neither is one of
the open items of Section~\ref{sec:open}.
...
\item The \mword{Erdos249257/LcmConeFlatness.lean}{426}{irrational_totient_series_iff_lcm_diagonal_certificate_supply}{lcm-diagonal}
  ...
  tail-difference certificates.  Finite instances are checked, for every
  $t\le82$ and including the historical $28$-scale bank of
...
Several of the items above are checked propositions that no row of the claim
registry currently owns.  They are \ref{res:rankonefloor},
\ref{res:firstharmonicnormgap}, \ref{res:perioddeposits},
\ref{res:orbitphase}, \ref{res:paritycoboundary}, \ref{res:lcmjump},
\ref{res:canonicalmersenne} and \ref{res:fulldepth}, together with the five
declarations of \ref{res:weight} and the supply implication of
\ref{res:supply}; \ref{res:denominator} cites three declarations of which the
registry's row owns one.  These declarations are kernel-checked and not yet
registered.  \ref{res:cfdenominator} is unowned for a different reason: the
release taxonomy has no status for an exact computation that carries no Lean
proof, so no available status describes it correctly.
\ref{res:residueobservable} is unowned because its declarations are absent
from this checkpoint.  This note prints the gap on its face rather than
assigning a neighbouring status.
...
\bibitem{fan2026} S.~Fan, comment on
  \href{https://www.erdosproblems.com/249}{\texttt{erdosproblems.com/249}},
  19:01 on 16 May 2026: ``the convolution identity $\ph=\mu*\mathrm{id}$
  yields $\sum_{n\ge1}\ph(n)/2^n=\sum_{n\ge1}\mu(n)/(2^n-1)^2+1/2$''.
  The M\"obius--Mersenne series that $\Theta_2$ heads is this object; the
  attribution is to the identity, and the rank-one floor of
  \ref{res:rankonefloor} is proved here.  Cited from the public dated
  comment; no separate publication was located.
