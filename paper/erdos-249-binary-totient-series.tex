...
The leading result is a sharp obstruction with an explicit constant.  Put
...
The third result is quantitative.  Exact rational bracketing of $\Stot$ at
...
A fourth result is proved in the private authoring tree and is absent from the
release tree at commit~\texttt{\commitshort}.  Let $m\ge2$, let $f:\N\to\Z$
satisfy $f(0)=0$, and let $r<m$ satisfy $\gcd(r+1,m)=1$ and $f(r)\ne0$.  Then
$\sum_{n}f(\ph(n)\bmod m)/2^n$ is irrational.  In particular
$A_m=\sum_n(\ph(n)\bmod m)/2^n$ is irrational for every $m\ge3$, and $m\ge3$
is sharp because $A_1=0$ and $A_2=3/4$.  This settles the fixed-resolution
observables of the totient word completely.  It says nothing about $\Stot$
itself, whose coefficients are unreduced.  Section~\ref{sec:results} gives its
private-source coordinate and marks it for the next formal-source cycle.
...
The principal unconditional result is the sharp determination of the
