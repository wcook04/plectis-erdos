...
Later work extends the checked band to every $t\le82$; that module postdates
this note's pinned snapshot, so the claim registry, rather than a source link
here, records the wider band.  The theorem through $t=82$ supplies no instance
at $t=83$ and does not discharge the cofinal quantifier.
...
\item The lcm-diagonal and clean cyclotomic-anchor statements are exact
  equivalent formulations.  Their finite certificates are checked instances,
  not evidence for the missing cofinal quantifier.
\item Full-depth period-multiple escape and the four first-harmonic pivot
  bounds are stronger sufficient producers.  Failure of either would refute
  only that route.
