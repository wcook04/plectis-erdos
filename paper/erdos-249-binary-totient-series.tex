...
\renewcommand{\commit}{599f7e50a15b288f27f9b57ece16fdd396cb6d76}
\renewcommand{\commitshort}{599f7e50a15b}
\hypersetup{
...
\externalreviewobject{The paper gives an explicit basis for the full dyadic
totient kernel, proves level rank $2^e+1$ and infinite-dimensionality, and
checks a finite denominator exclusion together with its first failing
...
the explicit odd-core basis, the exact finite-level rank theorem, and
infinite-dimensionality of the full kernel.  The finite arithmetic certificate
retains its separate evidence route.}
...
of the claim registry; see the formal source notes below.}
...
Two boundaries must be drawn more carefully than an earlier version of this
note drew them.  First, the relation to Coons is weaker than infinite-rank
language suggests.  For a family of integer-valued sequences, finite generation
of the $\mathbb Z$-module it spans is \emph{equivalent} to finite dimensionality
of its $\mathbb Q$-span: evaluating at $d$ coordinates on which the span injects
sends the module isomorphically onto a subgroup of $\mathbb Z^d$, and subgroups
of $\mathbb Z^d$ are finitely generated.  Coons's theorem therefore already
gives that the $\mathbb Q$-span of the full $2$-kernel of $\ph$ is
infinite-dimensional.  The residual content here is the exact growth $2^e+1$,
the explicit basis, and the relation normal form, not infinite-dimensionality
itself.

Second, the independence step has an antecedent.  Martin proves that for
positive affine forms $a_in+b_i$ with $a_ib_j\ne a_jb_i$ for all $i<j$, and for
\emph{any} constant $C>0$, the set of $n$ at which the consecutive ratios
$\ph(a_in+b_i)/\ph(a_{i+1}n+b_{i+1})$ all exceed $C$ has positive lower
density~\cite[Theorem~1]{martin}.  Given a vanishing rational combination
$\sum_ic_i\ph(L_i(n))=0$ with some $c_s\ne0$, ordering $L_s$ first and taking
$C>\sum_{i\ne s}|c_i|/|c_s|$ makes the $s$ term exceed the sum of the others in
absolute value, a contradiction; so totient values along pairwise
non-proportional affine forms are rationally independent.  That is the
independence used below, obtained from an existing theorem.  Martin does not
mention $k$-kernels and does not state a rank formula, and we have not located a
source recording this corollary; but the mechanism is his, and no priority is
claimed for it.  The evaluation-matrix argument of the next subsection is an
independent and more constructive route to the same independence, which is why
it is retained.
...
\]
(\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{15}{checkerboard}{mixed difference}).
If $F(i,j)=u(i)+v(j)$, then $\Delta F=0$
(\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{19}{checkerboard_separable}{separable case}).
...
(\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{25}{checkerboard_unique}{fixed-stencil uniqueness}).
...
\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{38}{fourPoint_layer_identity}{a four-point divisor-layer identity}
and
\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{55}{residueClass_step_has_centred_completion}{a centred representative for a residue class modulo an even modulus}.
...
\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{88}{PrimeRayLayerSupply}{Layer},
\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{95}{BoundedDegreeOrderConsumer}{BoundedOrder},
and
\lword{Erdos249/PrimeRayCyclotomicCurvature.lean}{102}{FinitePrimeSupportEscape}{FinitePrimeEscape}.
...
\mword{Erdos249257/FirstHarmonicPivot.lean}{405}{irrational_totient_series_of_pivotResidualDecorrelation}{the checked consumer}.
...
\begin{problem}[odd-prime totient kernels]\label{prob:oddprime}
For a fixed odd prime $\ell$, determine for every $e$ the exact dimension
\[
 \dim_{\Q}\operatorname{span}_{\Q}
 \{\,n\mapsto\ph(\ell^j n+r):0\le j\le e,
       \ 0\le r<\ell^j\,\},
\]
and give an explicit basis and a complete normal form for all rational
relations.  The first concrete case is $\ell=3$.
\end{problem}
...
\item The odd-prime kernel question is independent of the irrationality
  problem and carries no claimed answer or prior-art verdict.
\end{enumerate}
...
checked propositions.  Results attributed to
Allouche and Shallit, to Coons, to Martin, to Bell and Smertnig, to Kaneko,
Suzuki and
Tachiya, to Luca and Tachiya, and to Tao and Ter\"av\"ainen are cited from the
literature and are not formalised here.  The independence of totient values
along pairwise non-proportional affine forms is Martin's Theorem~1 and is not
claimed here; the assessment that no prior source records the resulting
$k$-kernel corollary --- the explicit basis, the exact finite-level dimension
formula, and the relation normal form --- is the outcome of a literature search
and is not a proof of non-existence.

\emph{Scope of the AI declaration.}  Large-language-model agents assisted
throughout, drafting and revising this exposition, the Lean developments it
links to, and the repository software that checks them.  Will
...
\bibitem{martin} G.~Martin, \emph{Simultaneous inequalities among values of
  the Euler $\varphi$-function}, 2006,
  \href{https://doi.org/10.48550/arXiv.math/0603053}{arXiv:math/0603053}.
  Theorem~1: for positive affine forms $a_in+b_i$ with $a_ib_j\ne a_jb_i$
  whenever $i<j$, and for any constant $C>0$, the set of $n$ at which every
  consecutive ratio $\ph(a_in+b_i)/\ph(a_{i+1}n+b_{i+1})$ exceeds $C$ has
  positive lower density.  The hypotheses are symmetric in the forms, so any
  prescribed ordering may be arranged.
