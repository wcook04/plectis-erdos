...
sets $S$; at base $3/2$ the value $1/2$ occurs.  Both facts come from one
comparison, the number of admissible choices at each level against the
contraction between levels.  Subsum sets are null when the choices fall short
...
a Cantor set of measure one.  We prove that the proportion of rationals of
bounded height that survive a fixed number of greedy steps is forced by the
equidistribution of Farey fractions, so it carries no information about
membership, and we state the membership question that remains.
\end{abstract}
...
Kova\v{c} and Tao use the higher-dimensional form of (ii) to construct
rational Ahmes series \cite[Lemma~5.1]{kovactao2024}.  We claim no novelty for
...
full sum $X_{\N_{>0}}(a/b)$ when $\log b/\log a<0.4056\ldots$, using Zudilin's
linear forms \cite[Theorem~1]{plectis1049}, \cite{zudilin2004}, and proves that
no family of forms of that shape reaches $1/2$
\cite[Theorem~5]{plectis1049}.  Thin supports reach further than the full sum
because the denominators divide one another.

At base $2$ the greedy rule decides membership: starting from $r_0=x$, take
...
\begin{theorem}[a forced statistic]\label{thm:forced}
...
The limit $2^NR_N/E$ does not depend on whether \#257 is true.  A count of
fractions that survive a fixed depth therefore says nothing about membership.
...
we repeat at each use.  Theorem~\ref{thm:chains} uses Mahler's theorem as an
...
allowance is impossible, so an unbounded one is exactly what is needed.  Hence
growth, densities of patterns and residues of $p_n$ cannot prove
$\sum p_n2^{-n}$ irrational.
...
\]
Mahler's theorem \cite{mahler1929}, in the form given in
\cite{nishioka1996}, states: if $f\in\Q[[z]]$ converges in the unit disc, is
transcendental over $\mathbb{C}(z)$, and satisfies
$f(z^d)=A(z,f(z))/B(z,f(z))$ with $A,B$ coprime polynomials of degree less than
$d$ in the second variable, then $f(\alpha)$ is transcendental for every
algebraic $\alpha$ with $0<|\alpha|<1$ such that the resultant of $A$ and $B$ in
the second variable does not vanish at any $\alpha^{d^k}$.  For $g$ we have
$d=2$, $A=(1-z)f-z$ and $B=1-z$.  They are coprime, their resultant is a power
of $1-z$, and $1-z$ is nonzero at every $\rho^{\,d2^k}$.  The function $g$ is transcendental: at a root of unity
...
positive proportion of all rationals.  It is a strong claim, and no count can
support it: by Theorem~\ref{thm:forced} every count at a fixed depth is forced.
...
removed intervals.  The expected number rejected at step $n$ is
$2^{n-1}g_n\sum_{2\le q\le Q}\varphi(q)$, asymptotically $(3Q^2/\pi^2)2^{n-1}g_n$,
which falls below $1$ once $n>2\log_2Q-3.3$.  Past
that depth a finite computation at height $Q$ cannot separate membership from
late rejection, so running deeper adds nothing.  No rational is known to have
an infinite $S$ at base $2$, and beyond the forced share $1-1/E$ no argument is
known that rejects a positive proportion of rationals.
...
The literature on rational points of Cantor sets concerns null sets such as the
middle-third set, where the expected count of rationals of height at most $Q$
is a power of $Q$ \cite{rstw2019,chowvarjuyu2024}.  The literature on subsum
sets \cite{nitecki2013} does not discuss rational points.  We found nothing on
rational points of a subsum set of positive measure, and
Problem~\ref{prob:membership} is the simplest question of that kind.
...
\item \#1049, Theorem~5 \cite{plectis1049} (ordinary proof there, with the
  contradiction step and the comparison with $1/2$ checked in Lean).  A family of
  linear forms with degrees at most $\delta n^2$ and decay $-\sigma n^2\log x$
  at every $x>1$ has $\sigma\le\delta$, so its sufficient region
...
\item \#249, Theorem~9 \cite{plectis249} (Lean-checked there).  Every
  ...
  target.
\item \#68, Section~6 \cite{plectis68} (ordinary proof).  The integer-gap
  comparison cannot hold at cutoffs within a bounded distance of the degree.
