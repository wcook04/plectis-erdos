...
to clear every rational denominator. Section~\ref{sec:capacity-proof} proves
...
\end{enumerate}
When \eqref{eq:gaps} holds, one may require $e_n=0$ before any prescribed
...
common-divisor carry obstruction of Section~\ref{sec:carry}; they do not
formalise this gap classification or its measure argument. The source and
attribution account is in Section~\ref{sec:sources}.

For example, with allowance $n^2$, every second position suffices without
congruences. Under \eqref{eq:congruence}, infinitely many omitted positions
already force a null set. With allowance $n^{2.01}$, every second position
again suffices, even under \eqref{eq:both}. Thus the strict inequality in
\eqref{eq:gaps} is essential.

\begin{corollary}\label{cor:density}
For $c>1$, the least possible asymptotic density of a fixed permitted support
that fills an interval under \eqref{eq:congruence} is
...
\section{Proof of the capacity criterion}\label{sec:capacity-proof}
...
on the target. This completes Theorem~\ref{thm:capacity}.
...
\section{Why a large gap prevents interval filling}\label{sec:negative}
...
of Theorem~\ref{thm:capacity}, with allowances
...
part of Theorem~\ref{thm:main}. If $c>1$ and (3) fails, it proves the negative
...
\begin{lemma}\label{lem:feedback}
...
Lemma~\ref{lem:feedback} fills $[1/n_0!,2/n_0!]$ using the positions $n_j$,
...
$E_0(S,c)$.
\begin{proposition}\label{prop:unrestricted}
...
\end{proposition}
\begin{proof}
For necessity repeat Section~\ref{sec:negative} with
...
\section{A common-divisor test for irrationality}\label{sec:carry}
...
from the arithmetic obstruction used in Theorem~\ref{thm:gcd}.

\section{Sources, formal correspondence, and further questions}\label{sec:sources}
...
\href{../../lean/ErdosProblems/Erdos251/ResidueFeedbackCore.lean}{\texttt{ResidueFeedbackCore.lean}}.
...
Lemma~\ref{lem:feedback}, the sharp exponential support constants, and the
...
\href{../../research/experiments/sparse_interpolation/README.md}{the accompanying research record}.
...
The formal module
\href{../../lean/ErdosProblems/Synthesis/CongruenceInterpolation.lean}{\texttt{CongruenceInterpolation.lean}}
...
\href{../../lean/ErdosProblems/Synthesis/FeedbackContinuation.lean}{\texttt{FeedbackContinuation.lean}}
