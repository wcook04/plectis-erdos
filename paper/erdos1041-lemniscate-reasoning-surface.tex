...
One attribution remains to be completed.  Dubinin's Theorem~1
\cite{dubinin} is a P\'olya-type area inequality: the area of $\{|f|<s\}$ is
at most $s^{2/n}$ times the area of $\{|f|<1\}$.  Tao identified it on the
Erd\H{o}s Problem~\#1041 discussion page on 25 March 2026 as the source of
that scaling factor.  The published text is paywalled and has been seen here
only in that paraphrase, so the inequality is used in the form just stated,
no sharper consequence is drawn from it, and reading the original before any
external submission remains an open obligation.  Corollary~\ref{res:critical-value-thresholds}
