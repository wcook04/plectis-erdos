...
$\sum_{j=1}^{n-1}|f(c_j)|^{1/(n-1)}\le(n-1)R^{n/(n-1)}$ holds through degree
five for zeros in a disc of radius $R$.
...
the broken line $\zeta_1\to0\to\zeta_2$, of length $|\zeta_1|+|\zeta_2|<2$.
  The statement is checked in Lean: \lean{complete_trinomial}{ErdosProblems/Erdos1041/PaperTrinomial.lean:38}.
\end{theorem}
...
\lref{Erdos1041/AbelControlPolygon.lean}{123}{abel_controlPolygon}, the
constant-term bound as
\lref{Erdos1041/AbelControlPolygon.lean}{258}{norm_const_lt_one_of_roots_lt_one},
the radial estimate as
\lref{Erdos1041/AbelControlPolygon.lean}{219}{trinomial_radial_norm_lt_one},
and their combination with the metric budget as
\lref{Erdos1041/AbelControlPolygon.lean}{330}{trinomial_erdos1041_conclusion}.
Example~\ref{ex:sextic-spoke} is checked as
\lref{Erdos1041/AbelControlPolygon.lean}{555}{sextic_guardrail}.  What the
...
file that lies outside the pinned formal-source library, so they carry no
pinned-kernel receipt.  The public source of record is
...
The bound comes from this defect identity.  It is not obtained by separately
...
\[
  N(z)=-\frac{f(z)}{f'(z)} .
\]
Let $I$ be an interval, let $z\colon I\to\mathbb C$ be differentiable with
...
If a solution as above runs on a compact interval and its two endpoint values
are nonzero, those values lie on the same oriented ray from $0$.
Consequently, if two critical values lie on distinct positive rays, no
finite Newton-flow connection joins the corresponding critical points.
\end{corollary}
