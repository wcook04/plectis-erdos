...
Erdős stated the no-coprimality extension in the base-two setting after
proving the pairwise-coprime case, but did not print its proof. The local
base-two proof is an independent Lean-checked reconstruction. The all-base
transfer is a local strengthening. This packet makes no theorem-priority
claim and does not identify its proof with Erdős's omitted argument.
