...
For every finite set P of at least two primes, the distinct-height sum D_P, which counts each value of the running LCM of the P-smooth integers once, is irrational (res:distinct-height-all; ordinary proof found within the project on 26 September 2026 and independently refereed by a second AI reader, no Lean proof), as Erdős asserted without an argument in 1973; for |P| = 2 it follows from the two-prime Hecke-Mahler transcendence. The case P = {2,3,5} is Lean-checked by a dyadic-block proof (res:distinct-height-235; ErdosProblems.Erdos269.distinctHeightSum235_irrational, Comparator not yet run). For the catalogue sum, both two-prime sums are transcendental by Fan's reduction to a cited Hecke-Mahler value theorem (res:two-prime-transcendence), and for every finite P with |P| >= 2 the sub-sum E_p over the powers of one prime p is irrational (res:single-prime-subsums; ordinary refereed proof, no Lean proof, literature not searched). The catalogue sum for three or more primes is unresolved in this release: for {2,3,5} the rationality-to-positive-reduced-carry bridge is checked, and the remaining endpoint is cofinal local-window escape, equivalently exclusion of the integral branch.
