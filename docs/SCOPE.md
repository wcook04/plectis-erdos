...
#1041: `python3 scripts/query_corpus.py --route erdos_1041` checks the indexed source identities and reports the recorded results and open boundaries; resume with `python3 scripts/query_route_memory.py --problem 1041`. These navigation/evidence routes do not promote research rows into reviewed claims or Comparator or close #1041.

The [complete eight-problem return matrix](SOURCE_MAP.md#complete-eight-problem-return-matrix) binds each problem to its strongest source/paper route and frontier. The [Palomar showcase](PALOMAR_RESULT_SHOWCASE.json) maps source-reviewed family dispositions without adding claims. The generated [external-verification dossier](EXTERNAL_VERIFICATION.md) gives the exact spine and boundaries; neither replaces `docs/claims.json` or Lean proof authority.
...
Machine IDs in [`docs/claims.json`](claims.json), cross-checked by `scripts/check_release.py`:
