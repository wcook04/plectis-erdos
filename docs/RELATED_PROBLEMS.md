...
|---|---|---|---|
| **#68** `∑_{n≥2} 1/(n!−1)` | open | problem-owned checked reductions and finite evidence | exact factorial-successor and carry frontiers, structural consumers, and a finite denominator exclusion; no irrationality proof |
...
| **#1041** short curves in polynomial lemniscates | open | problem-owned dynamical reductions and proof audit | Newton-flow ray separation and perturbation lemmas are checked; the global topology and metric gluing are open |
| **#1049** `∑ 1/(tⁿ−1)`, rational `t>1` | open (rational `t`) | integer-base theorem plus problem-owned rational-base obstructions | the integer-base case `b ≥ 2` is `irrational_erdosSum_full_support`; at rational noninteger bases the public modules check clearing, height-region, and Padé obstructions but no irrationality theorem |
