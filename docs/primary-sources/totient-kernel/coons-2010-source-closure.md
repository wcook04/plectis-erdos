...
PDF pages were read, and all 15 PDF pages were visually checked;
...
- **Definitions and scope:** Printed pp. 339--340 (PDF pp. 2--3) define the
  ...
  sequences.
- **Regularity framework:** Printed p. 348 (PDF p. 11) defines a
...
- **Totient theorem:** Printed p. 348 (PDF p. 11) states **Theorem 3.2**:
  “The function `φ(n)` is not `k`-regular for any `k ≥ 2`.” Its proof, on
  printed p. 349 (PDF p. 12), uses
  the identity
...
- Attribution of the theorem that Euler's totient function is not
  `k`-regular for any integer `k ≥ 2` to Coons, Theorem 3.2, stated on
  printed p. 348 and proved on p. 349.
...
rank theorem stated by Coons. The bridge is elementary for integer-valued
sequences: if the rational span had finite dimension `d`, some `d`
evaluations would embed it in `ℚ^d` and the `ℤ`-module generated by the kernel
in `ℤ^d`, so that module would be finitely generated. The #249 short note
states this equivalence in §1, where the exact ranks recover Coons's theorem.
...
- The #249 short note and the #249 reasoning record, which cite Theorem 3.2
  where the finite-level ranks recover it.
