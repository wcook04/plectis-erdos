...
Choose one statement in a short paper. Read its assumptions and the step that
does the main work, then follow the longer record if you want to see the
attempts that led there. The [results guide](RESULTS.md) keeps each result
beside its limitation; [prior work](PRIOR_ART.md) identifies earlier arguments
and attribution. You can review one argument without taking on the whole
repository.

If a hard step is compressed, a routine step gets too much space, or you cannot
tell where an idea came from, that is useful feedback. A checked proof still
needs an explanation a mathematician can understand and reuse. No software
check here substitutes for that understanding or for independent review.
