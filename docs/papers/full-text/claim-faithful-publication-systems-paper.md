...
Lean verifies that a proof establishes the formal statement written in the source. It does not check that a README or paper states that proposition faithfully. This case study separates four questions often compressed into the word “coverage”: whether declarations were inventoried, whether they were routed to a mathematical zone, whether selected declarations were interpreted as authored statement nodes, and whether selected public claims were reviewed. At the audited revision every one of 151,755 live declarations has a typed route, but only 4,348 of 141,820 authored theorem-like declarations are linked to authored statement nodes. Of those links, 2,201 are exact proposition evidence and 2,147 are contextual links. The first fact is exhaustive routing, not exhaustive interpretation.
...
The generated receipt at this revision makes the ceiling visible: 151,755 of 151,755 declarations are routed, with no duplicate routing receipts; 4,348 of 141,820 authored theorem-like declarations are linked to authored statement nodes, while 137,472 are only zone-routed. The graph distinguishes 2,201 exact proposition-evidence links from 2,147 contextual links, so attaching a large helper family to one node cannot masquerade as direct interpretation. All 300 declarations selected by the public claim ledger have the required semantic route. The command `python3 scripts/query_semantic.py coverage` derives these counts. Its checker validates source freshness, roles, node references, relation labels, endpoints, and stated bases. It does not prove that canonical prose is extensionally equivalent to its Lean proposition, that a mathematical relation is true, or that the selected graph is complete. Here *semantic coverage* means explicit statement-level interpretation, never mere inventory or routing.

Population is therefore selection, not bulk labelling. The authored papers already identify mathematically important declarations, state their scale and claim ceilings, and provide source coordinates. The `population-backlog` query resolves both public-paper and reasoning-surface links against live declaration roles, deduplicates qualified and short names, and ranks citations that still lack proposition-level semantics. A paper only nominates a packet: exact Lean signatures and proof cones adjudicate its statements, while repetitive helpers and generated certificate leaves remain compressed. This turns exposition into a source-current authoring queue without pretending that prose is proof authority or that a higher node count is better coverage.

Selected semantic subjects now carry a stronger, still limited receipt. At this revision 20 statement nodes and four typed relations have digest-bound source-to-wording reviews. Each of the eleven README headline claim families reaches at least one such node. The digest covers the exact wording, evidence coordinates, formal-source revision, and declaration-atlas fingerprint, so changing any of them invalidates the receipt. This count does not mean that every declaration or public sentence in a headline family was reviewed. The recorded reviewer type is an AI semantic-consistency review, not independent human mathematical review, Lean proof authority, or a novelty judgement. The command `python3 scripts/query_semantic.py semantic-reviews` lists the exact subjects and their individual claim ceilings.

Above that graph, the theory-laboratory projection currently records nine authored mechanisms explaining eight statement nodes, nine short transfer capsules, three typed failure receipts, and four temporal holdout items. A mechanism must name an invariant, a transformation, and the observable it controls; a failure receipt must also name the sibling routes it does not rule out. There are no benchmark results yet, so the repository makes no measured transfer claim. The commands `python3 scripts/query_semantic.py mechanisms` and `python3 scripts/query_semantic.py receipts` expose the exact records.

<a id="sec:agent-workbench"></a>
...
This is not the first declaration graph or Lean retrieval surface. LeanGraph extracts elaborator-level typed edges across multiple Lean projects, and Mathlib has separately been analysed as a multilayer declaration network \[theoremgraph; mathlibnetwork\]. LeanExplore combines semantic, lexical, and graph ranking and exposes agent-facing MCP and Python interfaces \[leanexplore\]; LeanSearch v2 targets whole-theorem premise sets with sketch–retrieve–reflect iterations \[leansearchv2\]. The contribution here is narrower: a repository-local, dependency-free first-contact contract that joins exhaustive addresses to selective authored meaning and explicit authority receipts.
...
Requirements traceability follows a requirement from its origin through development, deployment, use, and revision \[gotel\]. An assurance case sets out a system claim, the argument for it, and supporting evidence. The Goal Structuring Notation (GSN) is a graphical notation for that argument structure \[gsn\]; SACM standardises machine-readable claims, arguments, artefacts, and asserted evidence links \[sacm\]. The claim record resembles these forms, but it does not contain an argument that its Lean declarations justify the public wording. It records the approved wording, supporting declarations, scope, and limits; the justification remains a human judgement.

Likewise, digest- and revision-bound release receipts are not authenticated supply-chain attestations. In-toto cryptographically verifies actors, steps, and artefact flow, while SLSA provenance records how a subject was built \[intoto; slsa\]. This repository’s checks are unsigned and maintainer-controlled: they establish configured consistency, not builder identity, tamper resistance, or an external root of trust.
