...
The [public repository studied here](https://github.com/wcook04/plectis-erdos) contains Lean source, papers, and release machinery around eight Erdős problem programmes. The degree-seven example refutes the total-variation formulation of Problem 1041. Its correspondence with the historical curve-length question has not had independent human review; the other seven target problems remain unresolved here. The repository also contains substantial intermediate theorems, exact reformulations, conditional reductions, finite certificates, and no-go results. Private development explains the provenance; public claims rely only on the published evidence.
