...
For release review, the selected Comparator surface is exact: the declaration `exists_primeGap0_gt` states $`\forall M:\mathbb N,\ \exists n,\ M<
\mathit{primeGap0}(n)`$, and `irrational_tsum_primeDyadicTerm_iff_primeGap` states, under $`\operatorname{Summable}(\mathit{primeDyadicTerm})`$, the equivalence between irrationality of the prime-term and prime-gap-term sums. The summability hypothesis is retained; these structural rows do not prove the open irrationality claim for Problem #251.
