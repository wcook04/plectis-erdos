...
```
The digit intervals therefore overlap. Greedy selection fills the complete interval $`[0,F]`$, where $`F=\sum_jD_jw_j>0`$: at each step the remainder is between zero and the remaining capacity, which tends to zero. Take $`I=(A+\beta,A+\beta+F)`$.
