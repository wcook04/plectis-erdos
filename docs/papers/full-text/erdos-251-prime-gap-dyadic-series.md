...
A sparse perturbation of the actual prime gaps can make their dyadic sum rational while preserving every fixed eventual congruence of the gaps and cumulative sums, and the empirical distributions of all unnormalised gap blocks of length $`o(\log\log X)`$. For every $`0<\varepsilon<1`$, its corrections can be bounded by $`(\log n)^\varepsilon`$ eventually. The construction uses pairs with fixed unweighted total and variable dyadic weight. Lean checks its pair identities and the proposition it proves, and the scheduling argument written here is ordinary mathematics. The simpler positive even word $`a_n=2(n^2+4n+2)`$ has complete dyadic tails $`U_N=2(N+4)^2`$ and rational sum $`\sum_{j\ge1}a_j2^{-j}=32`$. We identify the actual complete prime-gap tails and classify rationality by integrality of their shifts. Two adjacent shifts in $`(-1,1)`$ with unequal associated gaps cannot both be integral. Their joint occurrence cofinally for every positive shift would prove irrationality of the prime series; this prime-specific assertion remains unproved.
...
```
Set $`b_n=a_n`$ for $`n<K`$ and $`b_{K+j}=a_{K+j}+M\delta_j`$. Then $`\sum_{n\ge0}b_n2^{-(n+1)}=r`$. Every coefficient retains its residue modulo $`M`$ and changes by at most $`M`$; each cumulative sum changes by at most $`Mn`$. Applied to prime gaps, this retains the cumulative $`n\log n`$ scale that the prime number theorem gives for $`p_n`$ \[mv2007, Chapter 6\]. Taking $`M`$ even retains parity as well. Thus a bounded residue-preserving perturbation can have a rational value without sharing the polynomial example’s growth. Neither construction asserts that its cumulative sums are prime.

<a id="sparse-corrections-and-local-information."></a>

#### Sparse corrections and local information.

A single changed coefficient can alter every later cumulative residue. We therefore pair corrections: changing their allocation changes the dyadic value while leaving their unweighted total fixed. Sparsity has a separate job. Most short blocks then meet no correction at all, which preserves their empirical distributions without requiring the corrected positions to be prime.

<div id="res:sparserationalisation" class="proposition">

**Proposition 1** (sparse congruence-preserving rationalisation). *Let $`a_n`$ be nonnegative integers with $`\sum_{n\ge0}a_n2^{-(n+1)}=A<\infty`$, let $`K\in\mathbb{N}`$, and let $`f:\mathbb{N}\to\mathbb{R}`$ tend to $`+\infty`$. There exist a set $`S\subseteq[K,\infty)`$ of upper Banach density zero and a nondegenerate interval $`I\subset(A,\infty)`$ such that every $`r\in I`$ is realised by a nonnegative correction $`e`$ supported on $`S`$, with $`0\le e_n\le f(n)`$ eventually, $`\sum(a_n+e_n)2^{-(n+1)}=r`$, and every fixed modulus eventually dividing both $`e_n`$ and $`\sum_{i<n}e_i`$. For the actual prime gaps and the specific choice $`f(n)=(\log(n+3))^\varepsilon`$, $`0<\varepsilon<1`$, one can in addition require $`|S\cap[X,2X)|=O_\varepsilon(X/\log\log X)`$, so that all unnormalised blocks of length $`o(\log\log X)`$ are preserved in total variation.*

</div>

<div class="proof">

*Construction.* Use disjoint triples of coordinates $`n_j-1,n_j,n_j+1`$, with spacings $`s_j=n_j-n_{j-1}\ge4`$, and moduli $`M_j`$ forming a divisibility chain. Put $`D_j=2^{s_j}-1`$. Starting from $`C_0=0`$, define
``` math
c_j=(-C_j)\bmod M_j,\qquad C_{j+1}=C_j+c_j+M_jD_j.
```
For a free digit $`0\le d_j\le D_j`$, assign all three corrections:
``` math
e_{n_j-1}=c_j,\qquad e_{n_j}=M_jd_j,\qquad
 e_{n_j+1}=M_j(D_j-d_j),
```
and set $`e_n=0`$ elsewhere. The complete triple has total $`c_j+M_jD_j`$, independent of $`d_j`$, so every buffer is fixed before the free digits are chosen. Immediately after the buffer, the cumulative correction is divisible by $`M_j`$; both pair entries preserve it. Later buffers preserve every previously imposed divisor. Thus each fixed modulus eventually divides both individual and cumulative corrections if it eventually divides $`M_j`$.
...
```
The function $`h`$ is nondecreasing, tends to infinity, and satisfies $`h(n)\le n`$. Hence $`k_j`$ is nondecreasing and tends to infinity. Each triple entry is at most $`M_j2^{s_j}\le h(n_{j-1})`$, which is at most $`f(n)`$ at every coordinate of that triple and at most $`n_{j-1}`$. This proves the size bound and summability of the correction. The spacings tend to infinity, so the union of triples has upper Banach density zero.

Put $`w_j=M_j2^{-n_j-2}`$. The weighted correction separates as
``` math
...
```
The digit intervals therefore overlap. Greedy selection fills the complete interval $`[0,F]`$, where $`F=\sum_jD_jw_j>0`$: at each step the remainder is between zero and the remaining capacity, which tends to zero. This is the variable-digit covering of Fridy’s lemma on generalised bases, whose condition $`r_n\le\sum_{i>n}k_ir_i`$ is the overlap inequality above \[fridy1966, Lemma, p. 194\]; Fridy assumes nonincreasing weights, and the argument here uses only the overlap. Kovač and Tao use the same nested covering in \[kovactao2024, Lemma 5.1\], where the choices at each stage are reciprocals of the integers in an interval; here they are the $`D_j+1`$ equally spaced values $`dw_j`$. Take $`I=(A+\beta,A+\beta+F)`$.
...
```
with $`M_j=k_j!`$, $`n_j=n_{j-1}+s_j`$ and the same $`D_j`$ and buffers. Then $`s_j\ge4`$ and $`M_j2^{s_j}\le(\log(n_{j-1}+3))^{3\varepsilon/4}`$. The preceding congruence, convergence and interval arguments still apply. Spacing comparable to $`\log\log n_j`$ gives $`|S\cap[X,2X)|=O_\varepsilon(X/\log\log X)`$. Couple the original and corrected length-$`m`$ blocks at the same starting index in $`[X,2X)`$. At most $`m|S\cap[X,2X+m)|`$ starts meet a correction, so their empirical distributions differ in total variation by $`O_\varepsilon(m/\log\log X)`$ for $`m=o(\log\log X)`$. The complete argument and the all-index congruence check are expanded in the companion record, subsection “Paired corrections and cumulative congruences”. The pair, buffer and conditional interval-filling identities are the Lean-checked core; the scheduling and density arguments are ordinary mathematics. ◻
...
```
This is the usual one-based series $`2/2+3/4+5/8+\cdots`$. The first gaps are [$`1`$](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L47), [$`2`$](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L51), $`2,4,2,4,2,4,6,2`$; only $`g_0`$ is odd. Let $`G=\sum_{n\ge0}g_n2^{-(n+1)}`$. The exact identities are
``` math
\Pi=2+G,\qquad
...
Erdős stated the fixed-denominator question in his work on irrational series \[erdos1958, p. 94\]\[erdosgraham1980, p. 62\] \[erdos1988, p. 103\]. His irrationality result for prime numerators with factorial denominators does not apply to this dyadic denominator sequence. Kovač’s counterexample to a related variable-denominator expectation likewise does not decide the fixed-denominator problem \[kovac2026\]. An adjacent dyadic theorem of Erdős and Pomerance concerns bounded digits recording comparisons of the largest prime factors of consecutive integers \[erdospomerance1978, §7\]; our prime numerators and gaps are unbounded. Assuming Kuperberg’s uniform Hardy–Littlewood prime-tuples conjecture \[kuperberg2023, Conjecture 1.3\], Land posted a research draft dated 5 September 2026 that proves irrationality \[land2026, Theorem 2\], and Ringer a preprint dated 11 September 2026 that proves $`\sum_n p_nB^{-n}`$ normal to base $`B`$ for each integer $`B\ge2`$ \[ringer2026, Corollary 1.2\]; both arguments are formalised in Lean. Nothing in this note assumes a prime-tuples hypothesis, and Erdős Problem #251 remains open unconditionally. Interval filling of the kind used in Proposition <a href="#res:sparserationalisation" data-reference-type="ref" data-reference="res:sparserationalisation">1</a> goes back to variable-digit expansions: for nonincreasing weights $`r_i\to0`$ and digit bounds $`k_i`$, Fridy showed that every number in $`(0,\sum_ik_ir_i)`$ is a sum $`\sum_ia_ir_i`$ with integers $`0\le a_i\le k_i`$ exactly when $`r_n\le\sum_{i>n}k_ir_i`$ for every $`n`$ \[fridy1966, Lemma, p. 194\]. It also has recent precedents for sums of unit fractions. Kovač and Tao use it to show that bounded perturbations of the denominators $`2^n`$ can give a rational sum, for instance $`\sum_{n\ge1}1/(2^n+b_n)=3/4`$ with every $`b_n\in\{1,\ldots,5\}`$, answering a question of Erdős and Graham on irrationality sequences \[kovactao2024, Lemma 5.1, Theorem 2.5 and §5\]. Van Doorn and Kovač combine dense filling with a divisibility chain among distinguished terms that eventually absorbs every positive integer, so that finite reciprocal sums take every rational value in an interval \[vandoornkovac2025, Lemma 7 and Proposition 8\]. The corrections in Proposition <a href="#res:sparserationalisation" data-reference-type="ref" data-reference="res:sparserationalisation">1</a> use a chain of factorial moduli with the same absorption property to preserve every fixed eventual congruence. The detailed historical comparisons and their hypotheses belong to the companion reasoning record. No priority claim is made for the elementary identities or constructions in this note.
...
We first record the recurrence-level conclusions. Modulo one the recurrence is multiplication by two, and the classifications below use only reduced denominators and Euler’s congruence $`d\mid2^{\varphi(d)}-1`$ for odd $`d`$. They apply to arbitrary integer digits and therefore separate the finite algebra from the prime-specific input still missing in Problem #251.
...
The statement is [irrational initial iff all lcm diagonal nonintegral](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/OrderLatticeDiagonal.lean#L153).
...
The rational normal form is [adjacent small mismatch iff signed two window](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/AffineShiftEscape.lean#L113). The same equivalence for real states is [real signed two window](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L135).
...
The three displayed equivalences are assembled in one declaration, [affine circularity bundle](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L241).
...
The eight displayed clauses are assembled as one statement in [polynomial countermodel](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L27).
...
```
both empty, hence zero, at $`n=0`$. We call these the [dyadic partial sum](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L101) and the [dyadic difference sum](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L121) of $`P`$.
...
*Proof.* Expand the difference sum and shift its first index. All interior coefficients of $`P(i)`$ become $`2^{-i-1}`$; the remaining terms are $`P(0)`$ and the displayed endpoint $`-P(n)2^{-n-1}`$. This also covers $`n=0`$. ◻

</div>

This is the [finite summation by parts](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L138). No positivity, monotonicity or convergence is required.

Specialising to $`P(i)=p_i`$, whose first value is $`p_0=2`$, and writing $`g_i=p_{i+1}-p_i`$ for the zero-based gaps, gives the reformulation. The displayed partial sums with denominators $`2^i`$ are twice these, by the [factor-of-two identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L111).
...
This is the [prime-gap summation by parts](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L172). The leading $`2`$ is $`p_0`$. At $`n=2`$ both sides equal $`19/8`$. The identity is summation by parts specialised to the primes; Tao posted this reduction on the problem’s forum thread on 7 October 2025 \[erdosproblems251thread, comment of 7 October 2025\].
...
u_n=\frac{p_n}{2^{\,n+1}},\qquad
 v_n=\frac{g_n}{2^{\,n+1}}
```
for the [prime dyadic terms](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L183) and the [prime-gap dyadic terms](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L192), so that $`\sum_{n\ge0}u_n=\Pi`$. The termwise identity $`v_n=2u_{n+1}-u_n`$ is the [dyadic discrete derivative](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L202). It expresses each gap term as an integer combination of two consecutive prime terms, so once $`(u_n)`$ is summable the gap series can be summed by rearranging two copies of the prime series, which is what the following proof does.
...
```
The corresponding [zero-based series with denominator $`2^n`$](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L188) is $`4+2\sum_{n\ge0}v_n`$ and has the same irrationality status. Termwise it is twice the normalised series, by the [infinite factor-of-two identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L196).*

</div>

These are the [normalised irrationality equivalence](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L435), the [displayed-series identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L444), and the [displayed-series irrationality equivalence](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L459). The three clauses are also available as the single declaration [irrationality reformulation](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L56).

<a id="the-summability-hypothesis."></a>

#### The summability hypothesis.

The formal source now proves the elementary polynomial bound $`p_n\le1250(n+1)^4`$, using prime counting and central-binomial growth, and deduces summability directly ([polynomial bound](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L360), [summability](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L379)). Thus Theorem <a href="#res:infinite" data-reference-type="ref" data-reference="res:infinite">8</a> and Corollary <a href="#res:irr-equivalence" data-reference-type="ref" data-reference="res:irr-equivalence">9</a> are now unconditional. The [unconditional infinite prime-gap identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L427) removes the summability hypothesis; the prime number theorem is therefore needed neither for convergence nor for this identity. It enters this note only through the $`n\log n`$ scale of the primes \[mv2007, Chapter 6\]. The open content is exactly irrationality of the prime-gap series. Concretely, $`\Pi=3.674643966\ldots`$ is irrational if and only if $`\sum_{n\ge0}g_n2^{-(n+1)}=1.674643966\ldots`$ is, and neither is known.
...
Iterating the recurrence $`h`$ times multiplies $`T_N`$ by $`2^{h}`$ and accumulates an explicit integer, which we now name. Define $`B_{0,N}=0`$ and $`B_{h+1,N}=2B_{h,N}+g_{N+h+1}`$, so that $`B_{h,N}=g_{N+1}2^{\,h-1}+\cdots+g_{N+h}`$: the [tail block](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L647). Thus $`B_{1,N}=g_{N+1}`$, $`B_{2,N}=2g_{N+1}+g_{N+2}`$ and $`B_{3,N}=4g_{N+1}+2g_{N+2}+g_{N+3}`$: the block puts the weights $`2^{\,h-1},\ldots,2^{0}`$ on the $`h`$ digits following index $`N`$.
...
*Proof.* Induct on $`h`$ using the recursions for $`T`$ and $`B`$, then subtract $`T_N`$ to obtain the shift identity. ◻

</div>

The first identity is the [iterated block identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L653); the second is the [scaled shift identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L667). Both are also assembled in [block identity](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L68), and the real-state form is [real block identity](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L76).

The shifts themselves satisfy the [shift step identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L563)
``` math
...
*Proof.* By Theorem <a href="#res:block" data-reference-type="ref" data-reference="res:block">11</a> the two differ by the integer $`B_{h,N}`$, and subtracting an integer does not change integrality, by the [integer-shift invariance](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L677). ◻

</div>

This is the [integral-shift criterion](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L802). Thus integrality depends only on the reduced denominator of $`T_N`$.
...
*Proof.* Since $`d`$ is odd, $`2`$ and $`d`$ are coprime, so Euler’s congruence gives $`2^{\varphi(d)}\equiv1\pmod d`$, that is $`d\mid 2^{\varphi(d)}-1`$. Writing $`2^{\varphi(d)}-1=dk`$ and $`T_N=u/d`$ in lowest terms, $`(2^{\varphi(d)}-1)T_N=ku`$ is an integer, and Theorem <a href="#res:shiftiff" data-reference-type="ref" data-reference="res:shiftiff">12</a> transfers this to the shift. The Euler multiplier itself is the [totient multiplier at odd denominator](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L695). ◻

</div>

This is the [totient shift](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L877).
...
*Proof.* By the shift step identity, $`\sigma_h(N+1)=2\sigma_h(N)-(g_{N+h+1}-g_{N+1})`$ is an integer combination of an integer and two digits; this is the [one-step propagation](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L888), and the claim follows by induction on $`k`$. ◻

</div>

This is the [propagation to every later index](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L900).
...
Denominator factorisation and cancellation are the [power-of-two/odd denominator scaling](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L723) and the [multiplier at the two-adic exponent](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L756), packaged in the [fixed-denominator theorem](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L830); the quantified conclusion is the [eventual integral-shift theorem](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L853) for the actual prime-gap tail state. The displayed indexed form is [eventual integral positive shift](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L84).
...
The [rational actual-tail state](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L489) obeys the [tail recurrence](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L527), and a hypothetical rational value of the gap series supplies [the rational-tail representation](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L510). The exact real classifiers are [one integral positive shift](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1479), [eventual integrality](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1525), [pointwise non-integrality](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1551), and [cofinal non-integrality](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1572). All four equivalences are collected in [rationality classification](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L94).
...
*Proof.* The shift identity holds over $`\mathbb{R}`$, and the only integer in $`(-1,1)`$ is still zero. Thus the proof of Theorem <a href="#res:smallpair" data-reference-type="ref" data-reference="res:smallpair">17</a> applies without change, including its cofinal conclusion. ◻

</div>

Corollary <a href="#res:smallpair-real" data-reference-type="ref" data-reference="res:smallpair-real">18</a> is [real small pair bundle](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L282), and the resulting implication for the actual prime series is [real small pair prime endpoint](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L298). These live declarations can postdate the historical public snapshot used by the source links.
...
This is the [unboundedness of the actual gaps](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L57) and the [non-eventual periodicity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1023) of the gap sequence. The statement at the index convention displayed above is [prime gaps not eventually periodic](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos251/PaperCoreR7.lean#L175). Combining nonperiodicity with eventual strict smallness of one positive shift would also exclude eventual integrality, but eventual smallness at every sufficiently large index is stronger than the cofinal adjacent-pair hypothesis in Theorem <a href="#res:smallpair" data-reference-type="ref" data-reference="res:smallpair">17</a> and is not asserted here.
...
For a bounded canonical base-two digit expansion, rationality forces eventual periodicity. Prime gaps are unbounded coefficients, so that criterion does not apply. The following telescoping construction shows explicitly how nonperiodic coefficients can have a rational sum.

The construction runs the emission of coefficients backwards. Let $`K:\mathbb{N}\to\mathbb{Q}`$ be arbitrary, read $`K_n`$ as the value carried into level $`n`$, and put $`\kappa_n=2K_n-K_{n+1}`$: the [carry coefficient](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1129). That definition is exactly the statement that
``` math
\frac{K_n}{2^{\,n}}
...
This is the [carry telescoping identity](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos251/PrimeGapDyadicTail.lean#L1137).
...
For Proposition <a href="#res:sparserationalisation" data-reference-type="ref" data-reference="res:sparserationalisation">1</a>, the pair identities, congruence buffer and abstract variable-alphabet filling are checked in `SparseRationalisationCore.lean`. Lean checks the proposition itself. [Arbitrary word sparse rationalisation](https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparseAmbientR9.lean#L217) gives, for every $`f\to\infty`$, a support in $`[K,\infty)`$ of upper Banach density zero, an interval above $`A`$, and a correction realising each point of that interval; [arbitrary word sparse rational target](https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparsePaperR11.lean#L53) realises a rational value above $`A`$; and [polylogarithmic word interval](https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/SparsePaperR11.lean#L78) gives the polylogarithmic clause, with block limits [growing block TV](https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/GrowingBlocksR11.lean#L139) and [growing block tests uniform](https://github.com/wcook04/plectis-erdos/blob/0b500c7cf8e8bb7ae343484378df02f277fb8194/lean/ErdosProblems/Erdos251/GrowingBlocksR11.lean#L152). The explicit three-coordinate schedule in the construction is the ordinary proof, expanded in the companion record, subsection “Paired corrections and cumulative congruences”. The proposition has no Comparator entry.
