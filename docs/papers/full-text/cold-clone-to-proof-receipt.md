...
The semantic graph contains authored statement nodes, typed relations, and an exact source-structural floor. At the audited revision all 144,335 authored theorem-like declarations are linked. Of these, 139,773 (96.8%) participate in authored mathematical interpretations: 3,285 are exact proposition evidence and 136,488 are bounded contextual links to digest- or module-verified families. The remaining 4,562 are grouped only by exact source module and normalised Lean proposition signature. That lower tier is useful navigation, not a mathematical paraphrase. Keeping the tiers visible prevents exhaustive linkage or bulk helper assignment from being misreported as exhaustive direct understanding. A paper-seeded population query continues to rank exact live citations whose best route is structural rather than authored.
