...
A large formal library can be mechanically exact and still be practically unreadable to the next human or reasoning agent. This paper presents a repository architecture that separates first-contact comprehension from proof checking. At the audited revision, a cold clone exposes a public proof cockpit that composes checkout and toolchain identity, corpus scale, exact open propositions, problem obligations, and committed workbench receipts. Beneath that card sit a bounded six-line tour, a mathematical map of ten programmes, a reviewed public claim map whose registry carries 22 explicit remaining open propositions in this release, and an elaborated loaded-root reference graph joined to source coordinates. These are committed navigation products: they require no Lean build and make omissions and authority boundaries explicit. At the audited revision the inventory behind them spanned 1,019 Lean modules and 153,253 declarations; 503 of those modules and 8,171 of those declarations are explicitly marked machine-generated certificate shards, counted as formal source and never as separate mathematical claims. Later revisions move the module and declaration totals, and `docs/declaration_atlas.json` carries the figure for whichever checkout the reader has. The marked figures are a classification floor, not the generated share: large emitted families predate the markers, so the true machine-generated share is substantially higher.

Navigation does not receive proof authority. A session notary records an agent’s observations, falsifiable conjectures, abandoned routes, exact Lean probes, and claims. Probe verdicts come from the pinned Lean process and cannot be authored by the agent; claims must cite an accepted probe, and replay reruns the stored bytes. Compilation is similarly separated from orientation. Lake outputs and content traces support focused or changed-cone builds, while an exact cached receipt prevents an unchanged dependency-index check from repeating a full environment export. A recorded case study, which is an internal agent rehearsal operated by the author, contains six reasoning notes, one accepted probe, and two claims, and its probe replays at the audited revision.
...
**Result.** A cold clone exposes the library’s complete declaration inventory, reviewed mathematical routes, public claim boundaries, and exact source links before Lean is built. Accepted proof probes are issued by the pinned Lean process and can be replayed byte for byte. **Evidence.** The audited repository contains 1,019 modules and 153,253 declarations; the paper includes a replayed session case study, which is an internal agent rehearsal operated by the author. **Limit.** This is a single-environment prototype with no recorded external cold-clone use, and no number reported here measures how an outside reader understands the corpus.
...
This paper owns the navigation and validation case study. The wider [claim-transition lifecycle](../../../claim-faithful-publication-systems-paper.pdf#systems-lifecycle) is described in the systems paper, while the [contribution protocol](../../../open-source-mathematics-strategy.pdf#strategy-protocol) specifies how an outsider returns a change. These links lead to named destinations in the sibling PDFs rather than to page numbers.
...
The semantic graph contains authored statement nodes, typed relations, and an exact source-structural floor. At the audited revision all 144,487 authored theorem-like declarations are linked. Of these, 139,799 (96.8%) participate in authored mathematical interpretations: 3,311 are exact proposition evidence and 136,488 are bounded contextual links to digest- or module-verified families. The remaining 4,688 are grouped only by exact source module and normalised Lean proposition signature. That lower tier is useful navigation, not a mathematical paraphrase. Keeping the tiers visible prevents exhaustive linkage or bulk helper assignment from being misreported as exhaustive direct understanding. A paper-seeded population query continues to rank exact live citations whose best route is structural rather than authored.

<a id="reviewed-public-claims."></a>

#### Reviewed public claims.

The claim map selects 208 results for public exposition in this release. It records 49 as proved here, 11 as formalised here, 52 as unconditional progress, 67 as conditional reductions, 10 as verified finite instances, 8 as proved by ordinary mathematics in a note rather than by a Lean declaration, 3 as cited only, and 8 as open. The ordinary-proof status is deliberately separate: those results are not Lean-checked, and any kernel-checked ingredient of such a result is named in its note. Twenty-two explicit frontier propositions describe the stronger obligations that survive. These labels are maintainer-reviewed public meaning, not outputs inferred by the proof kernel. The counts move with the registry; `docs/claims.json` is the authority for the checkout in hand.
...
The navigation surface was dogfooded separately as four audiences. The research-mathematics route reaches the programme map and explicit frontier; the formalisation route reaches a source coordinate and exact proof cone; the AI-lab route exposes exhaustive-versus-selective coverage and receipt authority; the contributor route reaches a focused build and release check. Adversarial tests remove or distort each protected transition and require the cold-clone validator to reject the mutation. Those tests are deterministic mutation tests over the declared contract. This evaluates the declared first-contact contract, not subjective comprehension. No reader was observed, and no comprehension time was measured.

<a id="sec:related"></a>

# Relation to prior work

Proof blueprints pair informal plans with named Lean declarations and author-supplied dependency links. `leanblueprint` stores a blueprint in TeX, records the declarations that realise a statement, records the statement and proof dependency edges its author declares, marks each node with a formalisation-readiness state, and checks that every named declaration exists in the project or one of its dependencies \[leanblueprint\]; it does not infer those dependency edges, and it does not check that the informal and formal statements agree. That declaration-linking layer is prior art, and nothing in this section claims it as new here. LeanArchitect infers formal dependencies and proof-hole status and exports synchronised blueprint material \[leanarchitect\]. The Carleson project shows a blueprint used as a large collaboration surface. Its public blueprint split the formalisation into about 180 claimable tasks, most corresponding to one blueprint lemma; formalisation feedback produced localized corrections, modifications, and extensions to the mathematical guide, including a few changes to the general setup and main theorems \[carleson\]. The present tour has a different first-contact role. It begins after the repository already contains a large corpus, distinguishes exhaustive inventory from authored interpretation, holds Lean-checked results, ordinary proofs, computations, and proved obstructions in one record, and routes a reader into exact dependency and proof-receipt tools.
...
The starting commit also permits work from an older clone to survive current development. Git retains the common ancestor and hence the original delta. Maintainers first reproduce that delta in its stated context, then reconcile the reviewed substance with current main. Validation and consequence propagation are repeated on the integration delta. Material conflict resolution is recorded and credited as new work; the contributor’s original authorship is preserved. The complete governance sequence appears in the [companion contribution protocol](../../../open-source-mathematics-strategy.pdf#strategy-protocol), while the wider job map appears in the [systems lifecycle](../../../claim-faithful-publication-systems-paper.pdf#systems-job-lifecycle).
...
A checkout of an earlier revision answers a narrower question than it appears to answer. The run implemented here supplies its own authored candidate, names the candidate declaration inside its request, exercises only the derived-layer arm, and observes no reader and no agent. It establishes that the runtime at the earlier revision exposes the exact remaining obligation for a stated target and that the kernel accepts an authored candidate carrying neither the later declaration name nor the later module name. Its clone carries the earlier source with the dependencies of the current tree, so its receipt records the pins that revision declared alongside the pins the build used. It does not show that the layers present at that revision helped a reader find and interpret the mathematics already there, and it does not show that those layers shortened the work of producing a later result, because later Git objects, a shared object store, generated dossiers and receipts, network access, context supplied in the prompt, and a model’s familiarity with public mathematics can each reveal the target. `docs/methodology.json` lists these channels and names the ones the harness controls; the remainder are limitations of any historical result reported here.
