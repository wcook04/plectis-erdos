...
<a id="what-each-lane-transfers."></a>

#### What each lane transfers.

The two exclusions and the criterion carry obligations of different kinds. Theorems <a href="#res:denominatorfloor" data-reference-type="ref" data-reference="res:denominatorfloor">1</a> and <a href="#res:cfexclusion" data-reference-type="ref" data-reference="res:cfexclusion">2</a> constrain a hypothetical rational value by excluding a range of reduced denominators. Each is finite, and extending either one raises the excluded range and reproduces the same statement form, so continuation of either lane leaves irrationality where it stands. Theorem <a href="#res:freepair" data-reference-type="ref" data-reference="res:freepair">3</a> is an equivalence, so it transfers the whole obligation, and what it changes is the shape of the input. At a fixed offset the tail condition is a correlation between two windows a bounded distance apart, which is where a Hardy-Littlewood hypothesis would enter. With $`M`$ free it is a hitting condition on an index that is otherwise unconstrained, one congruent pair for each modulus and each cutoff. The criterion also rules out two shortcuts, both recorded below. Rationality makes the tail a finite-state object at every scale, so pairs with $`T_M=T_N`$ are abundant for free and the whole difficulty sits in the nonintegrality of the difference. And under the free-pair lattice the second component of the two-condition event adds nothing, because an integer cannot lie in $`(\tfrac12,1)`$. A producer that closes the gap must supply, for every $`t`$ and every cutoff, a congruent pair whose tail difference is certified nonintegral, with uniformity in $`t`$ and in the cutoff. Each single instance of that statement is a finite computation costing $`O(\log N)`$ gaps together with the elementary prime bound. The narrowest sufficient prime-theoretic input recorded for the two-condition form is stated below, and it is not known to follow from published results.
