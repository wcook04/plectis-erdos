...
**Theorem 1** (Finite prime-part weighted mass). *Let $`b\ge2`$ be an integer, let $`\mathcal P`$ be a finite nonempty set of primes, and let $`A\subseteq\mathbb N_{>0}`$ be infinite. Write
``` math
...
*<span class="sans-serif">\[Math\]</span><span class="sans-serif">scale:uniform</span>`coord:divisibility-weighted`*
...
\label{eq:257-weighted-lattice}
 \Delta_{b,A}(m)=(b^m-1)S_b(A)-J_{b,A}(m),
 \qquad
 J_{b,A}(m)=
...
\end{equation}
```
The integer sum is finite. Moreover $`\Delta_{b,A}(m)>0`$: since $`A`$ is infinite, some $`a\in A`$ exceeds $`m`$, and its summand is positive. Thus, if $`S_b(A)=u/v`$ with $`v>0`$, equation <a href="#eq:257-weighted-lattice" data-reference-type="eqref" data-reference="eq:257-weighted-lattice">[eq:257-weighted-lattice]</a> gives $`v\Delta_{b,A}(m)\in\mathbb Z`$, whence
``` math
\begin{equation}
...
We now choose the diagonal scale. Fix $`\varepsilon>0`$ and a lower bound $`N`$. Select a finite $`F\subseteq A`$ such that
``` math
\begin{equation}
...
\end{equation}
```
The first term is paid for by <a href="#eq:257-weighted-tail-choice" data-reference-type="eqref" data-reference="eq:257-weighted-tail-choice">[eq:257-weighted-tail-choice]</a>.
...
Let
``` math
\mathcal A(Q,T)=\frac1T\sum_{t=1}^{T}\Delta_{b,A}(tQ).
```
...
0<\Delta_{b,A}(tQ)<2\varepsilon,
 \qquad tQ\ge Q\ge N.
```
The construction is valid for every positive $`\varepsilon`$; applying it with $`\varepsilon/2`$ proves the stated cofinal bound with $`\Delta_{b,A}(m)<\varepsilon`$. This contradicts <a href="#eq:257-weighted-gap" data-reference-type="eqref" data-reference="eq:257-weighted-gap">[eq:257-weighted-gap]</a> if $`S_b(A)`$ is rational. ◻
...
```
Writing $`N_k=2^{2^k}`$, the reciprocal mass of its $`k`$th layer is
``` math
...
```
and these contributions are summable. The theorem therefore proves hereditary all-base irrationality for every infinite subset of an explicit reciprocal-divergent host.
...
**Definition 2** (Universal \#257, written (U)). Is $`x_A = \sum_{n \in A} 1/(2^n-1)`$ irrational for *every* infinite $`A \subseteq \mathbb N_{>0}`$? **OPEN.** <span class="sans-serif">\[Open\]</span>

</div>

<div id="defn:H" class="defn">

**Definition 3** (The half-value question, written (H)). Let the *Mersenne achievement set* be $`\mathcal{A} := \{\, y \in \mathbb{R}: \exists A \subseteq \mathbb{N},\ 0 \notin A,\ y = \sum_{n \in A} x_n \,\}`$ (). Is $`1/2 \in \mathcal{A}`$? **OPEN.** <span class="sans-serif">\[Open\]</span>
...
**Proposition 18** (Weighted denominator budget; <span class="smallcaps">proved</span>). *For the denominator $`D_n`$ of the skip-set rational, $`\log_2D_n\le\sum_{d\in\mathrm{Skip}_n}\log_2(2^d-1)<\sum_{d\in\mathrm{Skip}_n}d
\le n(n-1)/2-1`$. This weighted quadratic upper bound is too coarse for the orbit’s $`2^{-3n/2}`$ precision. An $`n^2/4`$ asymptotic would require additional distribution information. `coord:diophantine` <span class="sans-serif">scale:cofinal</span> <span class="sans-serif">\[Math\]</span>*

</div>

**Precise obstructions.** Fixed-precision automaton state cannot determine the post-index tail, and the displayed weighted denominator budget combined with the currently cited fixed irrationality-measure exponent does not yield the required run bound. These calculations do not exclude averaging over shifts, residues, or observation scales, nor a sharper analysis of the actual skip distribution.
...
**Theorem 20** (One-sidedness; <span class="smallcaps">proved</span>). *$`1/2 \notin \mathcal{A}`$ is $`\Sigma^0_1`$: a single finite object — a fatal gap — certifies it. $`1/2 \in \mathcal{A}`$ has a $`\Pi^0_1`$ survival formulation. Survival through a tested finite depth alone does not establish membership; a uniform theorem or inductive invariant could. The arithmetical-hierarchy form by itself proves neither undecidability nor the absence of finite proofs. `coord:meta` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span> The statement is checked in Lean by the HalfCutLocator and GreedyAchievementSet declarations in the proof; `PaperCompleteR7/CatalogueAssemblies.lean` is not in this checkout.*

</div>

<div class="proof">

*Proof.* The dichotomy $`1/2 \in \mathcal{A}`$ or $`\exists`$ a fatal gap is unconditional (), and the two alternatives are exactly complementary (), with a finite witness type. In the other direction, certified death implies non-membership (, over ; $`3/4`$ is certified dead at level $`1`$, lookahead $`0`$, by ), whereas the absence of such a certificate at any depth $`D`$ is compatible with a certificate at depth $`D+1`$ and therefore proves nothing. Membership asserts survival at *every* rank (Theorem <a href="#thm:greedy-survival" data-reference-type="ref" data-reference="thm:greedy-survival">8</a>), an infinite conjunction with no finite sub-conjunction implying it. ◻
...
The direct certified instances remain fixed or bounded — rungs $`J \le 22`$, seam rows $`\le 200{,}000`$, ranks $`\le 3000`$, and macro depth $`\le 13{,}548{,}057`$ — whereas the open obligations are cofinal. Theorem <a href="#thm:one-sided" data-reference-type="ref" data-reference="thm:one-sided">20</a> shows that extending a bounded range does not establish a cofinal statement. Coordinate tags are equally literal: an obstruction in `coord:half-carry`, `coord:seam-row`, or `coord:diophantine` does not transfer to another representation without a proved map. Coordinate-free barriers are marked explicitly.
...
**Lemma 97** (Rational denominator survival: general-purpose fraction-reduction algebra). *Let $`a\in\mathbb Z`$, $`D\in\mathbb N_{>0}`$ and $`m,C,h\in\mathbb N`$. If $`m\mid D`$ and $`\gcd(m,|a|)=1`$, then $`m\mid(a/D\text{ as }
\mathrm{Rat}).\mathrm{den}`$. If $`C\mid D`$ and $`\gcd(C,|a|)=1`$, then $`C/\gcd(C,h)\mid((h\cdot a)/D\text{ as }\mathrm{Rat}).\mathrm{den}`$ — a divisor of the displayed denominator survives to at least this extent after scaling. Indeed, reduction divides $`D`$ by $`\gcd(D,|a|)`$, so no prime power in $`m`$ can be lost. After scaling, a prime $`p\mid C`$ can lose at most $`v_p(h)`$ powers from the guaranteed divisor. This is a lower bound on the reduced denominator, not an equality: for $`D=12`$, $`C=6`$, $`a=1`$, $`h=2`$, it guarantees the divisor $`3`$, while the reduced denominator is $`6`$. The lemma feeds Theorem <a href="#thm:mersenne-channel-survival" data-reference-type="ref" data-reference="thm:mersenne-channel-survival">65</a> directly.*
...
**Definition 122** (257bm:c13 — open property `HalfGreedySkippedFullShellNonnegative`).
``` math
\forall\ \text{skipped rank } n,\quad 0 \le \mathrm{greedyHalfFrozenMargin}(n-1,n).
```
This is an open property, not a proved theorem. It is the plainest sufficient socket in the family: assuming it, `half_mem_mersenneAchievementSet_of_skippedFullShellNonnegative` proves $`1/2\in\mathrm{mersenneAchievementSet}`$.
...
**Proposition 152** (257rig:i4b — `card_divisors_pow_le_divisorSubpowerConst_pow_mul`). *For positive integers $`n,k`$, $`\tau(n)^k \le (k^{2^k})^k\cdot n`$, or equivalently $`\tau(n)\le k^{2^k}n^{1/k}`$. The constant absorbs the finitely many primes below $`2^k`$; for larger primes, $`(\nu+1)^k\le p^\nu`$ controls each factor of the divisor product. This estimate can be reused independently of the support problem; a zero-window conclusion still needs the tail recurrence.*
...
Scale is the load-bearing axis of the whole corpus. Both open problems are, at bottom, cofinal statements: \#257 half-membership needs infinitely many exact rows or infinitely many positive greedy skips (Table <a href="#tab:scale-ladder" data-reference-type="ref" data-reference="tab:scale-ladder">1</a>, rows C1/C3), and the unconditional track needs a run-length bound that holds at every reset row out to infinity, not merely to a certified depth. Each row must retain its predicate, quantified variable, and every premise. An unconditional eventual assertion $`\forall n\ge N_0,\,P(n)`$ does give cofinal witnesses to that same predicate, by taking $`n=\max(N,N_0)`$. A conditional uniform consumer $`H(n)\Rightarrow C(n)`$ supplies no witness until $`H(n)`$ is established, and a checked finite band supplies no assertion beyond its endpoints. These are different logical shapes and are recorded separately below.
...
This is consistent with, not a substitute for, the general logical point above: an eventual-uniform proof of the same witness predicate already implies cofinal production. New content is required only when the uniform row is conditional, concerns another predicate, or covers only a finite band.
...
\ge 5`$: $`\mathrm{rem}(s) \le 2^s \vee \mathrm{overshoot}(s) \le 2^s`$. **Mismatch (`hypothesis_strength`):** the hypothesis is anti-concentration at *constant* resolution — the orbit must miss three named integers — where the obligation is anti-concentration at resolution $`2^{(r+5)/2}`$. Cell $`-3`$ is closed only in the all-right-tail configuration by `finalMiddleCell_neg_three_not_last`. The unrestricted induction still requires exclusion of all three cells at every late middle row. **Closes it:** exclusion of all three cells for the actual seam orbit, plus the right-pulse-leak inequality $`4\cdot\mathrm{overshoot}+\mathrm{abovePulse} \le
...
= \mathrm{lcm}(2,\dots,J)`$, a modulus of exponential scale in $`J`$. Its finite computational cost does not by itself rule out a cofinal argument.
...
*Observation 225* (G4 / full-shell margin, wrong horizon direction). <span class="sans-serif">\[Lean\]</span> , , . **Open property and proved consumer:** `HalfGreedySkippedFullShellNonnegative` asks that every skipped rank $`n\ge3`$ satisfy $`0\le\mathrm{greedyHalfFrozenMargin}(n{-}1)\,n`$. Assuming this property, `half_mem_mersenneAchievementSet_of_skippedFullShellNonnegative` proves $`1/2\in\mathrm{mersenneAchievementSet}`$. **Mismatch (`quantifier_order`):** exactly three doubling steps of horizon short. The exact-row socket needs margin$`(k,k{-}2)\ge0`$; this gives margin$`(k,k{+}1)\ge0`$. By the succ-recurrence, margin$`(k,k{+}1)=8\cdot`$margin$`(k,k{-}2)+4\,\mathrm{sc}(2k)+2\,\mathrm{sc}(2k{+}1)
...
1/2`$ — any support achieving exactly $`1/2`$ must be infinite. **Mismatch (`scale_only`):** the headline is pinned to target $`1/2`$ while its engine $`\mathrm{Odd}((\mathrm{finiteErdosSum}\,F\,2).\mathrm{den})`$ is already fully general over finite supports. **Sufficient specialization:** verbatim re-derivation — $`\forall t:\mathbb Q`$ with even reduced denominator, $`\forall A`$ finite with $`0\notin A`$, $`\mathrm{erdosSupportSeries}\,2\,A \ne (t:\mathbb R)`$. No new mathematics. Thus a represented rational with even reduced denominator would refute universal \#257 at $`b=2`$. The converse is not established. If $`\mathrm{FinVal}`$ denotes the finite-support values, the exact unrestricted condition is $`(\mathbb Q\cap\mathrm{mersenneAchievementSet})\smallsetminus\mathrm{FinVal}\ne\varnothing`$.
...
*Observation 237* (NM-07 / sunflower bouquet, unproved selector plus false class-existence). <span class="sans-serif">\[Lean\]</span> (selector def line 406; carry socket line 381; bouquet structure line 33). **Yields:** for $`A=`$ (finite exceptional set dividing $`Q`$) $`\cup \{\mathrm{core}_i \cdot
...
**Failure of the proposed inequality.** The tail already contains the contribution $`f_A(L)/2`$, so $`2T_A(L-1)\ge f_A(L)`$ identically. The proposed strict excess by $`1/v`$ is therefore impossible. This route supplies no sufficient condition until a different inequality is found.
...
\delta\le0`$ (a one-sided condition), not $`\delta=0`$. For the sibling socket , the former `Real.sqrt`/`Nat.sqrt` adapter gap is repaired by restricting cofinal witness depths to perfect squares. Any remaining gap lies in the producer hypothesis, not in rounding.

</div>

<div class="rem">

*Remark 248* (HalfCarryCofinalTerminalOnlyStrip: current status). at the original strip constant is logically equivalent to $`1/2\in\mathrm{mersenneAchievementSet}`$ after restricting cofinal witness depths to perfect squares and using the landed tail bound. The square-root conversion is no longer an open adapter; the open content is the producer itself.
...
**Scope.** This parity identity supplies no restriction for this recursion shape: it follows formally from doubling minus forcing. That precise obstruction also applies to a structurally identical recurrence. It does not rule out averaging across shifts, residue classes, or observation scales. `coord:parity-tautology`
...
**Exact denominator budget.** Cardinality alone gives only $`|\mathrm{Skip}_n|\le n-2`$. For the denominator $`D_n`$ of the corresponding rational,
``` math
\log_2 D_n\le\sum_{d\in\mathrm{Skip}_n}\log_2(2^d-1)
 <\sum_{d\in\mathrm{Skip}_n}d\le \frac{n(n-1)}2-1.
```
...
**Corollary 257** (Only $`C_D\in\{-2,-1\}`$ remain among the exceptional dyadic cells). *Combining Proposition <a href="#prop:upper-unconditional" data-reference-type="ref" data-reference="prop:upper-unconditional">255</a> with Theorem <a href="#thm:cd-neg3-impossible" data-reference-type="ref" data-reference="thm:cd-neg3-impossible">256</a>, only $`C_D\in\{-2,-1\}`$ remain in the final-middle-then-all-right scenario. In the unrestricted late-middle-row induction, all three cells $`\{-3,-2,-1\}`$ remain to be excluded unless another argument supplies the final-producer premise. No Lean theorem in the tree at the time of writing excludes $`C_D=-2`$ or $`C_D=-1`$ in either scope. `coord:final-producer/landing-excess` <span class="sans-serif">\[Lean\]</span>*
...
**Proposition 266** (Quantifier collapse: one critical index suffices, not $`d{+}1`$). *Let $`d,E\in\mathbb N`$ and assume $`E\le2^{d+1}`$. Then the purely combinatorial statement $`\mathsf{DyadicBandEscape}(d,E) \iff \exists j,\ \mathsf{CriticalDyadicBandIndex}(d,E,j)\land
E+2(d+j)\le 2^{d-j+1}`$ collapses the $`\forall j\in[0,d]`$ band-avoidance condition (formally $`d{+}1`$ separate inequalities) to checking exactly *one* nearest-boundary index $`j`$. The range assumption ensures that a dyadic threshold lies at or above $`E`$. Choose the smallest such threshold: smaller thresholds are already below $`E`$, while larger thresholds have narrower forbidden bands. Without the range assumption all bands escape automatically when $`E>2^{d+1}`$, but no critical index exists; $`(d,E)=(0,3)`$ is the smallest example. Specialised to the concrete seam reset charge, $`\mathsf{SeamUpperResetCriticalBandEscape}`$ is proved logically *equivalent* to Definition <a href="#defn:band-escape" data-reference-type="ref" data-reference="defn:band-escape">265</a>’s socket. (seam specialisation ). Zero Mersenne/seam content in the core lemma — pure $`(d,E,j)`$ arithmetic over powers of 2. `coord:dyadic-boundary, generic` <span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>*
...
```
Any bound $`L_r=o(r)`$ suffices. In the exact computation through row $`200{,}000`$, the maximal observed run has length $`19`$, attained at row $`158{,}096`$ (<span class="sans-serif">\[Cert\]</span>). This finite observation rules out smaller bounds on that range only; a global constant bound or asymptotic unboundedness requires an additional proof.
...
The former square-root adapter issue is resolved by restricting cofinal witness depths to perfect squares: at $`n=k^{2}`$ the two roots coincide exactly and no widening is needed. Applying Lemma <a href="#lem:sqwitness" data-reference-type="ref" data-reference="lem:sqwitness">271</a> along $`M=k^{2}-1`$ gives (4) directly; taking $`a`$ to be the indicator of an achieving support truncated to $`\mathtt{Fin}(M+1)`$ at $`M=k^{2}`$, and using together with $`1\notin A`$ (forced because $`\mathtt{mersenneWeight}\,1=1>1/2`$), gives the witness required by and hence (5).
...
| O3 CofinalExactLocalMersenneHalfRows | $`\Leftrightarrow`$ O2 (Prop. <a href="#prop:collapsed-list" data-reference-type="ref" data-reference="prop:collapsed-list">16</a>(c)); Lean lands $`\Rightarrow`$ | <span class="sans-serif">\[Open\]</span>, no shortcut |
