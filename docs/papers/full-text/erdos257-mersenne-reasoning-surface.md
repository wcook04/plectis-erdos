...
For $`n\in\mathbb{N}_{>0}`$ let $`w_n=(2^n-1)^{-1}`$, and for $`J\subseteq\mathbb{N}_{>0}`$ let $`\mathcal A_J=\{\sum_{n\in J}\varepsilon_nw_n:\varepsilon_n\in\{0,1\}\}`$. Erdős Problem 257 asks whether every infinite-support sum is irrational. This paper is a Lean-backed structural audit, not a solution. Its strongest unconditional theorem proves irrationality at every integer base for every infinite support with convergent reciprocal mass. The proof uses binary LCM–Cesàro close returns, a radix-transfer bound, and an exact integer-orbit contradiction; it needs no coprimality, periodicity, density, or powerful-support hypothesis. Its novelty and priority are unassessed, and it is not identified with Erdős’s omitted coprimality-free argument. Other unconditional checked content includes exact finite-denominator noncollapse; unique coding, compactness, and nowhere density for every restricted achievement set $`\mathcal A_J`$; perfectness when $`J`$ is infinite; and the measure dichotomy $`2^{-|F|}`$ when $`J^c=F`$ is finite, versus zero when $`J^c`$ is infinite. For squarefree support the divisor incidence is $`2^{\omega(n)}-1`$: at every even base this blocks two divisibility-first certificate schemas, while adjoining $`1`$ preserves the irrationality question and restores arbitrarily long base-$`2`$ opening blocks. Adjacent suffix-cylinder words also admit an exact carry-pivot normal form.

The targets $`1/2`$ and $`1/21`$ have no finite-support representation in the stated normalisation and reduce to explicit infinite-orbit alternatives; neither membership question is decided.

Published work independently settles prime support at base $`2`$ and squarefree support at every power-of-two base. Universal \#257 and both target membership questions remain open.
...
For an integer $`n \ge 1`$ set $`x_n := 1/(2^n-1)`$, and for $`A \subseteq \mathbb{N}_{>0}`$ write $`x_A := \sum_{n \in A} x_n`$; the sum always converges, since $`x_n = O(2^{-n})`$. Erdős’s Problem \#257 asks a single universally quantified question about this family.

<div id="defn:U" class="defn">

**Definition 1** (Universal \#257, written (U)). Is $`x_A = \sum_{n \in A} 1/(2^n-1)`$ irrational for *every* infinite $`A \subseteq \mathbb{N}_{>0}`$? **OPEN.** <span class="sans-serif">\[Open\]</span>
...
There are two useful mechanisms. Block certificates prescribe divisor incidences and control the remaining tail. Small-shift arguments average the actual periodic atoms and produce a positive displacement smaller than the lattice spacing of a rational value. The latter proves irrationality for every infinite reciprocal-summable support, without a coprimality assumption, and extends to the weighted and fractional-cover classes below. The table starts with the reciprocal-summable theorem and retains the earlier block-certificate families, uniformly in the base $`b\ge2`$; the further ordinary extensions follow below.
...
| Eventually periodic | $`m`$-periodic from $`N_0`$, $`A`$ infinite |  |
| Residue class | any $`m \ge 1`$, any $`c`$ |  |
| Odd support (density $`1/2`$) | — |  |
...
The full-support row includes Erdős’s classical irrationality theorem for the Erdős–Borwein constant $`E=\sum_{n\ge1}(2^n-1)^{-1}`$. The support-dependent certificate constructions explain the method requirements discussed in §<a href="#bar:box" data-reference-type="ref" data-reference="bar:box">2.7</a>. Those requirements do not limit the small-shift theorems.

The older bouquet endpoint assumes a forced-slot selector. Its conclusion is now unconditional by composition with the reciprocal-summable support theorem. Indeed, for $`A=E\cup\{c_ip_i:i\ge0\}`$ with $`E`$ finite and $`c_i\ge1`$,
``` math
\sum_{a\in A}\frac1a\le\sum_{a\in E}\frac1a+
 \sum_i\frac1{c_ip_i}\le\sum_{a\in E}\frac1a+\sum_i\frac1{p_i}<\infty.
```
Distinct bouquet rays give infinitude, so irrationality follows at every integer base. The selector remains a hypothesis of the older pinned certificate proof, not a mathematical prerequisite for this support class. <span class="sans-serif">\[Math\]</span>

<a id="the-lead-theorem-hypotheses-evidence-attribution-obligation."></a>

#### The lead theorem: hypotheses, evidence, attribution, obligation.

The first table row is the strongest statement in the universal direction, and its four components are worth separating. *Hypotheses.* An integer base $`b\ge2`$; an infinite $`A\subseteq\mathbb{N}_{>0}`$; and $`\sum_{a\in A}1/a<\infty`$. No coprimality, periodicity, density or powerful-support hypothesis appears. *Evidence.* The complete implication from those hypotheses to irrationality is kernel checked at ; no ordinary step and no finite certificate sits outside that check. <span class="sans-serif">\[Lean\]</span> *Attribution.* Erdős printed the pairwise-coprime case for every integer base $`b\ge2`$ on p. 222 of Math. Student 36 (1968), 222–226, and on the same page wrote that pairwise coprimality can be removed by a more complicated argument, giving no details; p. 226 repeats that boundary. The statement is therefore his stated extension. The proof and its checked development are local, the argument here is not identified with the one he omitted, and novelty and priority for the argument are unassessed. <span class="sans-serif">\[Math\]</span> *Remaining obligation.* What is left of (U) is exactly irrationality of $`x_A`$ for every infinite $`A`$ with $`\sum_{a\in A}1/a=\infty`$. Every later section of this document works inside that regime.

<a id="the-full-support-row-also-carries-digit-information."></a>

#### The full-support row also carries digit information.

The certificate machine behind the full-support theorem is , the abstraction is checked to lose nothing at , and the same certificates give, for every base $`b\ge2`$ and every $`K`$, some $`b^N`$-dilation of the value exceeding an integer by strictly between $`0`$ and $`b^{-K}`$ (, ). So the base-$`b`$ expansion of the full-support value contains arbitrarily long zero blocks. <span class="sans-serif">\[Lean\]</span> This is a statement about zero blocks alone: it gives no normality and no digit-frequency law, and the theorem that the binary block $`11`$ occurs infinitely often in the base-$`2`$ value is Campbell’s separate result, not formalised here. <span class="sans-serif">\[Lit\]</span>

Two one-sided rigidity results constrain a hypothetical rational-valued support, both only from the sparse side. For any $`A`$ with a positive element and $`x_A`$ rational, divisor-free windows are at most logarithmic: for every $`\varepsilon > 0`$ there is $`B`$ with every zero window of $`\operatorname{sc}_A`$ beyond the scale threshold of length $`\le \varepsilon \log_2 N + B`$, uniformly in $`A`$, $`N`$ and the window position (). <span class="sans-serif">\[Lean\]</span> This is vacuous whenever $`2 \in A`$, since then every zero window already has length $`\le 1`$. The companion is essentially tight and is best read as a restatement rather than a constraint.
...
<a id="sec:257-exact-descriptions"></a>

## Exact descriptions of a rational-valued support

Three checked statements describe every hypothetical counterexample rather than sampling supports. Each is an equivalence, each is uniform in its target, and each is entirely inside the kernel check.

<div id="thm:bmc" class="thm">

**Theorem 4** (Boolean–Möbius scaled-tail correspondence). *Let $`p\in\mathbb{Z}`$ and $`q\ge1`$. For $`U\colon\mathbb{N}\to\mathbb{Z}`$ set $`Q_U(0)=0`$ and $`Q_U(n)=(2U(n-1)-U(n))/q`$ for $`n\ge1`$, and call $`U`$ *admissible* for $`(p,q)`$ when $`U(0)=p`$; $`U(N)>0`$ and $`U(N)\le q(2\sqrt N+4)`$ for every $`N\ge0`$; $`q\mid 2U(N)-U(N+1)`$ for every $`N\ge0`$; and $`(\mu*Q_U)(n)\in\{0,1\}`$ for every $`n\ge1`$. Then a support $`A`$ with $`0\notin A`$, with some positive element, and with $`x_A=p/q`$, exists if and only if an admissible $`U`$ for $`(p,q)`$ exists, and in that case $`A=\{n:(\mu*Q_U)(n)=1\}`$. `coord:mobius-mersenne` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Sites:** , , .*

</div>

*Boundary and obligation.* The support the theorem produces is not required to be infinite, and a finite support supplies an admissible sequence for its own value, so existence of an admissible sequence is by itself no counterexample to (U). What the equivalence changes is the search space. The obligation it leaves is to decide whether an admissible sequence exists whose reconstructed support is infinite. Prior art for the correspondence is unassessed in the claim register and no priority is claimed. <span class="sans-serif">\[Math\]</span>

<div id="thm:scaled-greedy-trap" class="thm">

**Theorem 5** (Scaled greedy trap, escape, and bounded cofinal return). *Write $`r_N(x)`$ for the greedy Mersenne remainder of a real $`x\ge0`$ after rank $`N`$, put $`y_N(x)=2^N r_N(x)`$, and set $`c_n=2^n/(2^n-1)`$. The scaled recursion sends $`y_N`$ to $`2y_N-c_{N+1}`$ when $`c_{N+1}\le2y_N`$, and to $`2y_N`$ otherwise. For every real $`x\ge0`$: $`x\in\mathcal{A}`$ if and only if $`y_N(x)<2`$ for every $`N`$; if $`x\notin\mathcal{A}`$ then $`y_N(x)\to+\infty`$; and $`x\in\mathcal{A}`$ if and only if there is a real $`B`$ with $`y_N(x)\le B`$ for cofinally many $`N`$. `coord:achievement-set` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Sites:** , , , , .*

</div>

*Boundary and obligation.* By Theorem <a href="#thm:geometry" data-reference-type="ref" data-reference="thm:geometry">9</a> the set $`\mathcal{A}`$ is a compact, perfect, nowhere dense set of Lebesgue measure one, so this theorem exhibits a fat Cantor set as the filled non-escaping set of an explicit nearly doubling map. The compactness, perfectness and nowhere density are classical; novelty for the dynamical identification itself is unassessed. The theorem supplies no orbit: deciding (H), or membership of $`1/21`$, still requires one bounded cofinal return or one escape, and neither is proved. <span class="sans-serif">\[Math\]</span>

<div id="thm:rational-membership" class="thm">

**Theorem 6** (Rational membership by cofinal greedy skips). *For every rational $`q\ge0`$, $`q\in\mathcal{A}`$ if and only if the canonical greedy orbit of $`q`$ omits infinitely many positive exponents, equivalently if and only if that orbit skips at arbitrarily late ranks. `coord:greedy-orbit` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Sites:** , .*

</div>

*Attribution and obligation.* The forward implication spends irrationality of the Erdős–Borwein constant, which is Erdős’s 1948 theorem. The reverse direction is the classical greedy lemma for Cantor-type achievement sets and is folklore. The ambient achievement-set literature is recorded in the corpus prior-art adjudication as identified and unread, so the criterion is registered as new at medium confidence only. The criterion decides no individual rational; the half-value specialisation is . <span class="sans-serif">\[Math\]</span>

The third exact description is uniform over every real target, and it turns membership into one finite window.

The repair mechanism does not require periodic digits or even a rational target. Let $`x\ge0`$, let $`A_x`$ be its actual greedy support, and define
``` math
P_0(x)=0,\qquad P_{N+1}(x)=2P_N(x)+d_{A_x}(N+1),\qquad
 Q_N(x)=\lfloor2^Nx\rfloor-P_N(x).
```
Here $`d_{A_x}(n)=\#\{a\in A_x:a\mid n\}`$ is the actual divisor load. The prefix bound makes $`Q_N(x)`$ a nonnegative integer.

<div id="thm:generic-repair" class="thm">

**Theorem 7** (uniform repair criterion). *For every real $`x\ge0`$,
``` math
x\in\mathcal{A}\quad\Longleftrightarrow\quad
 \forall K\ \exists N\in[K,K+2\lfloor\sqrt K\rfloor+12):
 Q_{N+1}(x)\le Q_N(x).
```
Equivalently, repairs occur cofinally; equivalently, $`Q_N(x)\le d_{A_x}(N+1)`$ cofinally; equivalently, $`Q_N(x)\le N+1`$ cofinally. One strictly increasing window excludes membership.*

</div>

<div class="proof">

*Proof.* If $`x\in\mathcal{A}`$, the full tail identity and divisor-pair bound give $`Q_N(x)\le2\sqrt N+4`$. Put $`s=\lfloor\sqrt K\rfloor`$ and $`T=2s+12`$. Strict increase at each of the $`T`$ steps would give $`Q_{K+T}(x)\ge T`$. But $`K+T<(s+4)^2`$ gives $`Q_{K+T}(x)<2(s+4)+4=T`$, a contradiction.

For the converse, the prefix recurrence and the nonnegative next binary floor digit give
``` math
2Q_N(x)-d_{A_x}(N+1)\le Q_{N+1}(x).
```
A repair therefore forces $`Q_N(x)\le d_{A_x}(N+1)\le N+1`$. If $`x\notin\mathcal{A}`$, a positive fatal gap $`\delta`$ persists in the actual greedy remainder; the prefix-tail identity yields $`Q_N(x)\ge2^N\delta-1`$ beyond that fatal rank. This eventually exceeds $`N+1`$, contradicting a cofinal linear bound. These implications give all the stated equivalences. ◻

</div>

The [uniform repair equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/GreedyRepairCriterion.lean#L192) is now kernel checked for every real target $`x\ge0`$.

The finite deadline comes from a growth comparison: strict increases of an integer sequence accumulate linearly, whereas represented tails have only a square-root bound. For $`x=4/9`$ this recovers the original repair problem with $`Q_N=\lfloor4\cdot2^N/9\rfloor-P_N`$. Exact replay through rank $`4096`$ finds no strict window for that target. The theorem identifies the certificate to seek; it does not supply its occurrence. The displayed proof is the ordinary reading of the checked statement; the square-root window equivalence, the three further equivalent forms and the one-window exclusion are all kernel checked, at , , , and . <span class="sans-serif">\[Lean\]</span> The rank-$`4096`$ replay at $`4/9`$ is an exact computation and not a theorem. <span class="sans-serif">\[Compute\]</span> Prior art is unassessed. The obligation the criterion leaves is unchanged: deciding $`1/2`$, $`1/21`$ or $`4/9`$ still needs either one strictly increasing window or a cofinal supply of repairs, and neither is proved for any of those targets. <span class="sans-serif">\[Math\]</span>

<a id="sec:257-weighted-returns"></a>

## Divisibility-weighted supports: why averaging twice works

The following ordinary proof extends the reciprocal-summable theorem to explicit supports whose reciprocal sum diverges. Its full source, including the other analytic support criteria, is the [analytic proof supplement](https://github.com/wcook04/plectis-lean-erdos249-257/blob/e8458cdce502436d27af8a77d1e4d6742d3bcb69/docs/research/erdos257-eight-return-proofs.md).

<div id="thm:257-weighted" class="thm">

**Theorem 8** (Divisibility-weighted irrationality). *Let $`P`$ be a finite nonempty set of primes and set $`h(a)=\prod_{p\in P}p^{v_p(a)}`$. For an integer $`b\ge2`$ and an infinite $`A\subseteq\mathbb{N}_{>0}`$, suppose
``` math
W=\sum_{a\in A}\frac{h(a)}{a(b^{h(a)}-1)}<\infty.
```
Then $`S_b(A)=\sum_{a\in A}(b^a-1)^{-1}`$ is irrational. The condition at $`b=2`$ gives this conclusion at every integer base $`b\ge2`$ and for every infinite subset of $`A`$. <span class="sans-serif">\[Math\]</span>*
...
```
For fixed $`m`$ the sum converges. It is positive for $`m>0`$, because $`A`$ contains an $`a>m`$. Division of $`m`$ by each $`a`$ gives the finite integer identity
``` math
\Delta(m)=(b^m-1)S_b(A)
 -\sum_{\substack{a\in A\\a\le m}}
       \sum_{i=1}^{\lfloor m/a\rfloor}b^{m-ia}.
```
Thus $`S_b(A)=u/v`$, $`v>0`$, would force $`\Delta(m)\ge1/v`$ for every positive $`m`$. We construct arbitrarily small positive displacements.

Fix $`\varepsilon>0`$. Choose a finite $`F\subset A`$ whose complementary weighted mass is less than $`\varepsilon`$, and put $`L=\operatorname{lcm}F`$ (with $`L=1`$ if $`F`$ is empty). Let $`p_* =\max P`$, $`r=|P|`$, and, for large $`H`$, set
``` math
Q=L\prod_{p\in P}p^{\lfloor\log_p H\rfloor},\qquad
 G=\lfloor H/p_*\rfloor.
```
Then $`Q\le LH^r`$ and every atom in $`F`$ has zero displacement on $`Q\mathbb{N}`$. If $`h(a)\le H`$, then $`h(a)\mid Q`$; if $`h(a)>H`$, then $`(a,Q)\ge G`$. For the latter assertion, either all prime-power factors of $`h(a)`$ are at most $`H`$, so $`h(a)\mid Q`$, or one such factor exceeds $`H`$ and the corresponding factor of $`Q`$ is greater than $`H/p_*`$.

Put $`g=(a,Q)`$. One orbit of $`Qm\bmod a`$ visits $`0,g,\ldots,a-g`$, and the sum of its uncentred atoms is $`(b^g-1)^{-1}`$. Complete orbits and one partial orbit therefore give
``` math
\frac1T\sum_{m=1}^T\frac{b^{Qm\bmod a}-1}{b^a-1}
 \le \frac{g}{a(b^g-1)}+\frac1{T(b^g-1)}.
```
The function $`g/(b^g-1)`$ decreases for positive integral $`g`$. Splitting at $`a=QT`$ and $`h(a)=H`$ consequently gives
``` math
\begin{align*}
...
\end{align*}
```
Here the harmonic bound handles the large-$`h`$ terms. For $`a>QT`$ there is no wrap: its atom is at most $`2\,2^{Qm-a}`$; summing first over $`a`$, then over $`m`$, gives the final $`4/T`$.

The remaining incomplete-period sum need not have finite total mass. Instead average over $`T=2^j`$, $`M\le j<2M`$. For each fixed $`a`$,
``` math
\sum_{\substack{M\le j<2M\\a\le Q2^j}}2^{-j}\le\frac{2Q}{a}.
```
This is a geometric tail starting at the first admissible $`j`$. Multiplying by $`(b^{h(a)}-1)^{-1}`$ and interchanging the two finite sums charges this error at most $`2QW`$, rather than $`M`$ separate copies of a divergent sum. Hence
``` math
\frac1M\sum_{j=M}^{2M-1}\mathcal A(Q,2^j)
 \le\varepsilon+\frac{2QW}{M}
 +\frac{G(1+\log Q+2M\log2)+Q}{b^G-1}+4\,2^{-M}.
```
Choose $`M=\lfloor b^{G/2}\rfloor`$. Since $`Q\le LH^r`$ and $`G\sim H/p_*`$, every term after $`\varepsilon`$ tends to zero as $`H\to\infty`$. A finite average below $`2\varepsilon`$ contains a displacement below $`2\varepsilon`$. This contradicts the rational lattice gap. Heredity follows from positivity of the weights, and the all-base assertion follows from $`b^{h(a)}-1\ge2^{h(a)}-1`$. ◻

</div>

<a id="the-full-dyadic-error-series."></a>

#### The full dyadic error series.

The incomplete-period estimate has an infinite-support formulation that isolates exactly the needed hypothesis. For nonnegative $`\alpha_a`$ on $`A\subseteq\mathbb{N}_{>0}`$, put
``` math
C(R)=\sum_{\substack{a\in A\\a\le R}}\alpha_a,
 \qquad V=\sum_{a\in A}\frac{\alpha_a}{a}<\infty.
```
Then for every integer $`Q\ge0`$,
``` math
\sum_{j\ge0}2^{-j}C(Q2^j)\le2QV<\infty.
```
Indeed, every finite collection of scales sees a finite union of conductor cutoffs. Interchanging those finite sums and using $`\sum_{j:\,a\le Q2^j}2^{-j}\le2Q/a`$ bounds every finite partial sum by $`2QV`$; nonnegativity then gives convergence. Thus the entire error series is summable even when $`\sum_a\alpha_a`$ diverges. Taking $`\alpha_a=(b^{h(a)}-1)^{-1}`$ on $`h(a)\le H`$, and zero otherwise, supplies the small-part estimate above since $`V\le W`$. This closes the infinite-support averaging step; the prime-part split and choice of scales are the separate arithmetic steps in the full argument.

<a id="a-concrete-support-beyond-reciprocal-summability."></a>

#### A concrete support beyond reciprocal summability.

For $`k\ge0`$ put $`B_k=2^{\,2^{\,2^k-k}}`$ and $`A^*=\{a\ge1:a\le B_{v_2(a)}\}`$. In the layer $`v_2(a)=k`$, writing $`a=2^ku`$ with $`u`$ odd gives
``` math
...
```
These bounds sum to a finite number. In contrast, the reciprocal mass of the same layer is at least $`(\log B_k-\log(4\cdot2^k))/2^{k+1}`$ for large $`k`$, which tends to infinity. Every infinite subset of $`A^*`$ therefore has irrational $`S_b`$ at every integer base. The supplement also shows that consecutive elements of $`A^*`$ have gaps $`O(\log\log a)`$.

<a id="other-support-criteria-sharing-the-small-shift-endgame."></a>

#### Other support criteria sharing the small-shift endgame.

If $`H_A(x)=\sum_{a\in A,\,a\le x}1/a=o(\log^*x)`$, then every infinite subset of $`A`$ has irrational $`S_b`$ at every integer base. A second criterion uses finite frames $`F_j\subseteq\mathbb{N}_{>0}`$, their incidences $`f_F(n)=\#\{a\in F:a\mid n\}`$, exponents $`0<\alpha_j\le1`$, and nonnegative divisor majorants $`f_{F_j}(n)^{\alpha_j}\le\sum_{d\mid n}c_{j,d}`$. With $`C_j=\sum_{d\ge1}c_{j,d}/d`$ and $`B_j=2^{\alpha_j}`$, it suffices that
``` math
\sum_j C_j2^{j\alpha_j}\frac{B_j}{(B_j-1)^2}<\infty.
```
Every infinite subset of $`\bigcup_jF_j`$ then has irrational $`S_b`$ at all integer bases. Fixed-exponent summable-cost covers fit this criterion after regrouping. The short paper and supplement prove these statements, including the domination required before exchanging infinite sums and averages. The analytic statements in this subsection are ordinary proofs; the pinned formal-source table above records their separately verified predecessors.

The formal development also checks the [summability of the dyadic observation mass](https://github.com/wcook04/plectis-lean-erdos249-257/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/WeightedSupportLimits.lean#L111), its [vanishing mean](https://github.com/wcook04/plectis-lean-erdos249-257/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/WeightedSupportLimits.lean#L81), and the [irrationality consumer from close returns](https://github.com/wcook04/plectis-lean-erdos249-257/blob/43c36af8b9d5795d0a965825ec01110b87159df6/ErdosProblems/Erdos257/BatchReturnSynthesis.lean#L151). These are the checked analytic ingredients; the complete weighted criterion above still uses the ordinary assembly in the supplement.
...
**Theorem 10** (Support-restricted refinement). *For the achievement set built from a coordinate subset $`J \subseteq \mathbb{N}`$: either the omitted coordinate set is finite, and the volume is exactly $`2^{-|J^c|}`$, or infinitely many coordinates are omitted and the volume is $`0`$. Injectivity survives every restriction; perfectness is proved when $`J`$ is infinite. No perfectness claim is made for finite $`J`$, whose coding range is finite. `coord:achievement-set` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Sites:** , , , .*
...
Finally, the certified computation. All of it is exact integer arithmetic, no floating point. Truncation rungs $`J = 3,\dots,22`$ all *proved* to survive (not merely computed). Seam orbit certified to row $`200{,}000`$: branch counts $`\mathrm{R{:}M{:}U} = 100197{:}49899{:}49898`$, the target inequality failing at exactly one reset row, $`r = 7`$; maximal pure-R run $`19`$, attained at row $`158{,}096`$.
...
*Observation 14* (The problem admits no ensemble). The Mersenne weights are strictly superincreasing. Consequently the digit coding is injective (Theorem <a href="#thm:geometry" data-reference-type="ref" data-reference="thm:geometry">9</a>), the greedy orbit is the unique candidate support for any target, and distinct subset sums of a row of seam weights are separated by at least $`2^{s+1}`$. Every one of the nine coordinates the corpus has built — truncation rung $`J`$, seam row $`s`$, integer margin, sharp tail margin, half-carry $`K_n`$, reverse-carry word, Boolean–Möbius exact row, fatal-cell interval packing, Dedekind cut-locator — is therefore a coordinate on the binary digits of *one* specific real orbit. Distinct supports cannot represent the same value. `coord:meta` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span><span class="sans-serif">\[Math\]</span>\
**Sites:** , , .

</div>

The next sections separate six limitations. Canonicalisation fixes the support for a specified value (<a href="#bar:canon" data-reference-type="ref" data-reference="bar:canon">2.2</a>); several proposed producers are equivalent to the original membership statement (<a href="#bar:collapse" data-reference-type="ref" data-reference="bar:collapse">2.3</a>); current estimates leave a scale gap (<a href="#bar:scale" data-reference-type="ref" data-reference="bar:scale">2.4</a>); and bounded replay needs a uniform continuation argument (<a href="#bar:asym" data-reference-type="ref" data-reference="bar:asym">2.5</a>). Measure and category do not decide the rational targets (<a href="#bar:measure" data-reference-type="ref" data-reference="bar:measure">2.6</a>). On the universal side, some supports violate the particular block engine’s hypotheses (<a href="#bar:box" data-reference-type="ref" data-reference="bar:box">2.7</a>). These have different causes and do not form an exhaustion of possible proof methods.
...
\(2\) *Digit-cascade impossibility* is false statically: explicit words exist at $`t = 20`$ (top $`18`$ of $`20`$ bits all-$`1`$) and $`t = 22`$ (top $`20`$ of $`22`$ all-$`0`$), and an unconditional construction supplies about $`t/12`$ independent single-bit knobs, verified with zero exceptions to $`t = 601`$. <span class="sans-serif">\[Cert\]</span>

\(3\) *Integer-window static adversary search* finds nothing: zero hits, exhaustively for $`t \le 22`$ and exactly at $`t = 40, 42, 45`$ over all cut positions. <span class="sans-serif">\[Cert\]</span>

The reconciliation is the content: residues mod $`2^t`$ are adversary-tunable, but the integer *magnitude* is not, because the row weights are gap-dominated. So the static tier and the dynamic tier coincide identically, and all three families were, without knowing it, measuring the same single orbit.

<div class="rem">

*Remark 16* (A recorded hazard). The natural sufficient condition via fractional split-mass is *not* necessary on real crossing cores: the fixture $`D = \{2,3\}`$, $`c = 5`$ is an explicit counterexample (). Any attack on the capacity socket that routes through fractional mass is already refuted. <span class="sans-serif">\[Lean\]</span>
...
**Lemma 17** (Collapse mechanism; <span class="smallcaps">proved</span>). *Let $`A \subseteq \mathbb{N}`$ with $`1 \notin A`$, put $`\delta := 1/2 - x_A`$, and let $`\operatorname{ihc}(A,N)`$ denote the integer half-carry at level $`N`$. Then
``` math
\operatorname{ihc}(A,N) \;=\; 2^{N+1}\,\delta \;+\; \mathrm{T}(N+1),
  \qquad 0 \;\le\; \mathrm{T}(N+1) \;\le\; 2\sqrt{N+1} + 4 ,
```
where $`\mathrm{T}`$ is the binary coefficient tail of $`\operatorname{sc}_A`$. `coord:half-carry` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Sites:** ; ; .*
...
**Proposition 18** (Route collapse; <span class="smallcaps">proved</span>). *Any hypothesis asserting, for cofinally many $`N`$, that $`\operatorname{ihc}(A,N)`$ lies inside a strip of width $`O(\sqrt{N})`$ forces $`\delta = 0`$.*

</div>

<div class="proof">

*Proof.* By Lemma <a href="#lem:collapse-mech" data-reference-type="ref" data-reference="lem:collapse-mech">17</a>, $`2^{N+1}\delta = \operatorname{ihc}(A,N) - \mathrm{T}(N+1)`$. For the greedy support of $`1/2`$ one has $`x_G \le 1/2`$, hence $`\delta \ge 0`$ (). If $`\operatorname{ihc}(A,N) \le c\sqrt{N} + c'`$ for cofinally many $`N`$, then $`2^{N+1}\delta \le c\sqrt{N} + c'`$ along that sequence; since $`2^{N+1}`$ outgrows any $`\sqrt{N}`$ envelope and $`\delta \ge 0`$ is a fixed real, $`\delta = 0`$. Conversely $`\delta = 0`$ makes $`\operatorname{ihc}(A,N) = \mathrm{T}(N+1)`$ exactly, and the unconditional envelope of Lemma <a href="#lem:collapse-mech" data-reference-type="ref" data-reference="lem:collapse-mech">17</a> then *supplies* the strip for free. And $`\delta = 0`$ is exactly $`1/2 \in \mathcal{A}`$, by . ◻

</div>

<div id="lem:sqrt-witness" class="lem">

**Lemma 19** (Perfect-square strip witness; <span class="smallcaps">proved</span>, this pass). *Suppose $`x_A = 1/2`$ and $`1 \notin A`$. Then for every $`k \ge 1`$,
``` math
\operatorname{ihc}(A, k^2 - 1) \;=\; \mathrm{T}(k^2) \;\le\; 2k + 4 \;=\; \texttt{halfStripBound}(k^2).
```
`coord:half-carry` <span class="sans-serif">scale:cofinal</span> <span class="sans-serif">\[Math\]</span>*

</div>

<div class="proof">

*Proof.* By Lemma <a href="#lem:collapse-mech" data-reference-type="ref" data-reference="lem:collapse-mech">17</a> with $`\delta = 0`$, $`\operatorname{ihc}(A,k^2-1) = \mathrm{T}(k^2)
\le 2\sqrt{k^2} + 4`$. At a perfect square the real and truncated square roots agree exactly, $`\sqrt{k^2} = k = \lfloor \sqrt{k^2} \rfloor`$, so the real envelope coincides with the discrete strip $`\texttt{halfStripBound}(n) = 2\lfloor\sqrt n\rfloor + 4`$ () with no slack required. ◻

</div>

Lemma <a href="#lem:sqrt-witness" data-reference-type="ref" data-reference="lem:sqrt-witness">19</a> matters because it removes the last technical obstruction previously recorded against two of the sockets below — the mismatch between $`\lfloor\sqrt{\cdot}\rfloor`$ and $`\sqrt{\cdot}`$, for which the corpus carries only the wrong-direction inequality. Restricting the cofinal witness depths to perfect squares dissolves it, and no widening of the strip constant from $`4`$ to $`6`$ is needed. For the terminal-only socket, take $`a`$ to be the indicator of an achieving support truncated to $`\{0,\dots,M\}`$ at $`M = k^2`$, using and $`1 \notin A`$ (which holds since $`x_1 = 1 > 1/2`$).

<div id="prop:collapsed-list" class="prop">

**Proposition 20** (The collapsed sockets; <span class="smallcaps">proved</span>, composite). *Each of the following is logically *equivalent* to $`1/2 \in \mathcal{A}`$, not strictly weaker:*

1.  *the `hbound` hypothesis of and of ;*

2.  *;*

3.  *, including its advertised weakening that *no* compatibility is required between witness supports at different endpoints;*

4.  *;*

5.  *;*

6.  *the negation of , and all seven forms of the associated classification chain.*

*Forward directions are landed Lean (e.g. , , ). The reverse directions of (d) and (e) are derived here from Lemma <a href="#lem:sqrt-witness" data-reference-type="ref" data-reference="lem:sqrt-witness">19</a> and are *not* themselves landed Lean theorems. `coord:half-carry / exact-row` <span class="sans-serif">scale:cofinal</span> <span class="sans-serif">\[Lean\]</span><span class="sans-serif">\[Math\]</span>*
...
**Proposition 21** (Local information is void; <span class="smallcaps">proved</span>). *(i) After $`L`$ common steps the endpoint residue mod $`2^L`$ of an affine binary orbit is *independent* of the initial carry: $`u(L) - v(L) = 2^L(u_0 - v_0)`$ (). (ii) No bounded or autonomous state summarising the history before position $`m`$ determines the tail after $`m`$: for a balanced-pulse family no decoder recovers the radius, and any finite state type needs cardinality at least $`\lfloor m/2 \rfloor + 2`$, unbounded in $`m`$ (). (iii) For any starting carry and any finite word of odd valuation-unit symbols at fixed precision $`u > 0`$, a compatible centred carry completion *exists* (), so fixed-precision local data never excludes anything. `coord:carry` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>*
...
**Proposition 22** (Exponent-budget mismatch; <span class="smallcaps">proved</span>, given a cited $`\mu`$). *The skip set at row $`n`$ has $`|\mathrm{Skip}_n| \approx n^2/4`$, so an irrationality-measure exponent $`\mu`$ for the target constant yields a separation of order only $`2^{-\mu n^2/4}`$, while the required precision is $`2^{-3n/2}`$. The shortfall is a factor $`\approx 0.42\,n`$ *in the exponent*, and it grows linearly in $`n`$. Since $`\mu \ge 2`$ for every irrational, no improvement in $`\mu`$ can close it: Zudilin’s $`\mu(E) \le 2.42343562\ldots`$ is irrelevant to the gap, and so is any future bound. `coord:diophantine` <span class="sans-serif">scale:cofinal</span> <span class="sans-serif">\[Math\]</span><span class="sans-serif">\[Cited\]</span>*
...
*Remark 23* (Status of the exhaustion claim). The seven named families are each closed with mechanism. The claim that classes (a) and (b) *exhaust* the available tools is an <span class="smallcaps">observed pattern</span>, not a theorem, and is labelled as such wherever it is used below.
...
**Theorem 24** (Finite failure and all-rank survival). *The target $`1/2`$ lies outside $`\mathcal A`$ if and only if its greedy orbit has a fatal gap. It belongs to $`\mathcal A`$ if and only if the greedy remainder stays below the remaining tail at every rank. `coord:greedy-orbit` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>*

</div>

<div class="proof">

*Proof.* These are the [fatal-gap equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/HalfCutLocator.lean#L643) and the [greedy-survival criterion](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1458). A [certified greedy death](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1756) records a strict failure using a rational upper bound for the tail; [such a certificate excludes membership](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1762). For example, [$`3/4`$ has a certificate at level $`1`$ with lookahead $`0`$](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1778). ◻

</div>

For a rational target, a strict fatal gap is eventually detected by refining the rational tail bound. This gives a semidecision procedure for nonmembership. A computation that reports survival through rank $`D`$ records those finitely many comparisons; the survival criterion still requires an argument covering every later rank. Extending the replay can expose a fatal gap or suggest a useful invariant, but a successful finite replay by itself does not supply that argument.

This is a limitation of bounded replay, not an impossibility theorem for finite proofs or certificates of membership. An inductive invariant, a proved recurrence, or a finite criterion with a uniform completeness theorem could establish the all-rank statement. The existence of such a reduction is a legitimate proof target. In particular, the logical form of the survival criterion does not rule it out.

<a id="bar:measure"></a>

## Barrier 5: what measure and category leave undetermined

<div id="prop:soft" class="prop">

**Proposition 25** (Positive measure and empty interior). *The achievement set $`\mathcal A`$ has [Lebesgue measure $`1`$](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L996) and is [nowhere dense](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1681). Thus a uniformly sampled point of $`[0,E]`$ belongs to $`\mathcal A`$ with probability $`1/E`$, while a Baire-generic point lies outside it. `coord:achievement-set` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span><span class="sans-serif">\[Math\]</span>*

</div>

These are compatible statements about different notions of typicality. Neither assigns a probability to membership of the fixed rational $`1/2`$, nor decides whether any rational has an infinite support. The rational points form a countable, hence null and meagre, set, so an almost-everywhere or generic-point conclusion alone leaves their membership undetermined. The checked geometry therefore supplies context, not evidence for either answer to (U) or (H).

The [injectivity of digit coding](https://github.com/wcook04/plectis-lean-erdos249-257/blob/4ab50e144847a3393fb340341760e5d6907d702e/Erdos249257/GreedyAchievementSet.lean#L1582) likewise says that an achievable value has a unique support. It does not prohibit averaging over ranks, shifts, or finite approximations: the small-shift argument in Section <a href="#sec:257-weighted-returns" data-reference-type="ref" data-reference="sec:257-weighted-returns">1.4</a> is one such use of averaging. A probabilistic model of the greedy digits would need a separate arithmetic justification before its predictions could bear on the specified target.
...
*Observation 26* (The older certificate box; <span class="smallcaps">observed pattern</span>). The support-dependent CRT certificate constructions have two separate jobs: prescribe enough divisor incidences to control a block, and bound the remaining tail so that $`q(C+N+L+2)<b^L`$. Pairwise-coprime petals and a summable reciprocal budget are sufficient inputs in the corresponding construction. These are requirements of that producer, not necessary conditions for irrationality. The small-shift method in §<a href="#sec:257-weighted-returns" data-reference-type="ref" data-reference="sec:257-weighted-returns">1.4</a> uses neither a pairwise-coprime support nor ordinary reciprocal summability. `coord:certificate-engine` <span class="sans-serif">\[Math\]</span>

</div>

**What a general-support application still needs.** Shared factors impose compatibility conditions on CRT prescriptions; a proof must check those conditions and supply its tail estimate. The existing hypotheses cannot simply be omitted from the proof that uses them. This observation does not prove that all possible certificates or CRT constructions fail.
...
Several remaining approaches ask for arithmetic control of geometrically weighted divisor incidences. For the half-value probe one proposed input concerns $`\sum_{i\le L}(\tau(M+i)-1)2^{-i}`$; for a general support a block certificate instead concerns its own incidence $`\operatorname{sc}_A`$ and tail budget. Their similar appearance suggests useful comparisons, but does not make the two producers equivalent. The weighted and fractional-cover theorems show that small positive shifts can also be obtained without constructing either prescribed block.
...
**Sites.** with consumer ; quantifier collapse , which reduces the $`\forall j \in [0,d]`$ band check to one nearest-boundary index per row; finite verification ; alternative fan-in with consumer at `:167` and base case .

**Why it evades Barrier <a href="#bar:collapse" data-reference-type="ref" data-reference="bar:collapse">2.3</a>.** This is the one reduction in the corpus that is genuinely *sufficient* rather than equivalent. It is a quantitative bound at every reset row, while $`1/2 \in \mathcal{A}`$ is qualitative; it can be false while the goal is true. There is no implication $`1/2 \in \mathcal{A} \Rightarrow \sqrt{\ }`$-escape anywhere in the corpus, and by the shape of the two statements there cannot be. Proposition <a href="#prop:collapse" data-reference-type="ref" data-reference="prop:collapse">18</a>’s mechanism needs a strip hypothesis of width $`O(\sqrt N)`$ around the half-carry; this route asserts a lower bound on a deviation, which the collapse identity does not manufacture.
...
The local reciprocal-summable theorem still does not cover the primes because $`\sum_p1/p`$ diverges. That is the exact boundary of the new theorem, not a boundary of the mathematics. The previously proposed windowed-tail repair is therefore withdrawn as a research contribution. It may remain useful as an audit question about whether the existing Lean engine can reproduce the known theorem, but reproducing a cited result is not new progress on \#257. The authors mention broader base and prime-power extensions with modifications omitted in the cited version; this paper records only the fully proved base-$`2`$ theorem.

<a id="ssec:route-4"></a>

## Route 4: land the route-collapse equivalences

**What it needs.** Five biconditionals, each with $`1/2 \in \mathcal{A}`$ on the right-hand side: the `hbound` hypothesis of the two `mobiusCenteredHalfCarry` consumers; `CofinalPositiveHalfGreedySkips`; `CofinalExactLocalMersenneHalfRows`; `GreedyHalfCarryCofinalStripReturn`; `HalfCarryCofinalTerminalOnlyStrip`. The forward directions are landed. Every reverse direction runs through Lemma <a href="#lem:sqrt-witness" data-reference-type="ref" data-reference="lem:sqrt-witness">19</a>, stated once, whose only inputs are , , , and the evaluation of the square root at a perfect square. The scope limit of Proposition <a href="#prop:collapsed-list" data-reference-type="ref" data-reference="prop:collapsed-list">20</a> must be stated verbatim in the docstrings.
...
The direct certified instances remain fixed or bounded — rungs $`J \le 22`$, seam rows $`\le 200{,}000`$, ranks $`\le 3000`$, and macro depth $`\le 13{,}548{,}057`$ — whereas the open obligations are cofinal. Theorem <a href="#thm:one-sided" data-reference-type="ref" data-reference="thm:one-sided">24</a> identifies the all-rank obligation; extending a bounded range still requires a uniform argument for the remaining ranks. Coordinate tags are equally literal: an obstruction in `coord:half-carry`, `coord:seam-row`, or `coord:diophantine` does not transfer to another representation without a proved map. Coordinate-free barriers are marked explicitly.
...
**Definition 27** (Universal Problem \#257). **Is $`x_A = \sum_{n \in A} 1/(2^n-1)`$ irrational for *every* infinite $`A \subseteq \mathbb{N}`$?** This is a single universally-quantified statement over all infinite subsets of $`\mathbb{N}`$. It is **OPEN**. <span class="sans-serif">\[Open\]</span>
...
**Definition 28** (The distinguished half-value question). Define the *Mersenne achievement set* $`\mathcal{A} := \{\,y \in \mathbb{R} : \exists A \subseteq \mathbb{N},\ 0 \notin A,\ y =
...
**Theorem 30** (Full-support irrationality, unconditional, every base $`b \ge 2`$). *For every integer $`b \ge 2`$,
``` math
\sum_{k=0}^{\infty} \frac{1}{b^{k+1}-1} \quad \text{is irrational.}
```
**Hypotheses:** $`b \in \mathbb{N}`$, $`b \ge 2`$. **Conclusion:** unconditional irrationality of the full-support Erdős–Borwein-type series at every base, not merely base $`2`$. `coord:full-support` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** (‘$`b : \mathbb{N}`$, hb : $`2 \le b`$’), landed via a certificate machine (bounded Bertrand/CRT first-block frame, middle-window divisor-pair average with pigeonhole selection, explicit parameter closure). The base-$`2`$ instance recovering the classical Erdős–Borwein constant’s irrationality is , a one-line corollary.*

</div>

This is the base-$`b`$ generalization of Erdős’s original 1948 irrationality theorem for $`\sum 1/(2^n-1)`$ (cited, in its base-$`2`$ form, as an input at Remark <a href="#rem:no-ties" data-reference-type="ref" data-reference="rem:no-ties">38</a> below); its Lean form removes the base restriction entirely and is landed with no open hypotheses.
...
**Theorem 31** (Purely periodic support). *For every $`b \ge 2`$, every modulus $`m \ge 1`$, and every $`m`$-periodic $`A \subseteq \mathbb{N}`$ (i.e. $`n+m \in A \Leftrightarrow n \in A`$ for all $`n`$) containing a positive element, the support series $`\sum_{a \in A} 1/(b^a-1)`$ is irrational. **Hypotheses:** $`b \ge 2`$, $`m \ge 1`$, $`A`$ $`m`$-periodic, $`\exists a > 0,\ a \in A`$. `coord:periodic-support` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** .*
...
**Theorem 32** (Eventually periodic support). *For every $`b \ge 2`$: if $`A \subseteq \mathbb{N}`$ is infinite and its membership is $`m`$-periodic from some threshold $`N_0`$ on (i.e. $`n+m \in A \Leftrightarrow n \in A`$ for all $`n \ge N_0`$), then $`\sum_{a \in A} 1/(b^a-1)`$ is irrational – proved by transferring irrationality across the finite symmetric difference from the shifted purely periodic set $`A_{\mathrm{pure}} := \{n : n + m N_0 \in
A\}`$. **Hypotheses:** $`b \ge 2`$, $`m \ge 1`$, $`N_0 \in \mathbb{N}`$, eventual $`m`$-periodicity from $`N_0`$, $`A`$ infinite. `coord:eventually-periodic-support` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** .*
...
**Theorem 33** (Residue-class support). *For every $`b \ge 2`$, modulus $`m \ge 1`$, and residue $`c`$: $`\sum_{n \equiv c \pmod m} 1/(b^n-1)`$ is irrational. `coord:residue-class-support` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** (specializes purely periodic support).*

</div>

<div class="thm">

**Theorem 34** (Odd support – first density-strictly-between-0-and-1 class). *For every $`b \ge 2`$: $`\sum_{n \text{ odd}} 1/(b^n-1)`$ is irrational. This is the first landed support class with natural density strictly between $`0`$ and $`1`$ (odd $`n`$ has density $`1/2`$ inside $`\mathbb{N}`$), as opposed to the density-$`1`$ full-support case or density-$`1/m`$ residue classes for large $`m`$. `coord:odd-support` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** (specializes residue-class support at $`m=2,c=1`$).*

</div>

None of these families contains $`A`$ of the shape produced by a candidate half-value witness (the greedy support for $`1/2`$, described in §<a href="#ssec:greedy-orbit" data-reference-type="ref" data-reference="ssec:greedy-orbit">5.3</a>, is not eventually periodic in any landed sense) – so Theorem <a href="#thm:full-support" data-reference-type="ref" data-reference="thm:full-support">30</a> and its periodic-family corollaries neither prove nor disprove $`1/2 \in \mathcal{A}`$; they exhaust only the structured-support corner of universal \#257’s infinite search space.
...
**Chains with:** Theorem <a href="#thm:greedy-survival-record" data-reference-type="ref" data-reference="thm:greedy-survival-record">36</a> below (compactness is exactly what powers every “limit of achieved points is achieved” argument used downstream, including the seam-limit route of Part 2).*

</div>

<div id="thm:greedy-survival-record" class="thm">

**Theorem 36** (Membership equals greedy survival at every level). *$`x \in \mathcal{A} \iff 0 \le x \wedge \forall n,\ \rho(x,n) \le T_{n+1}`$, where $`\rho(x,n)`$ is the canonical greedy (take-if-possible) residual after rank $`n`$ and $`T_{n+1} := \sum_{j>n} x_j`$ is the full remaining tail. **Hypotheses:** none (unconditional biconditional). **Conclusion:** membership in $`\mathcal{A}`$ is exactly the statement that the greedy process never enters a fatal state at any rank – this is the master theorem underlying every sufficient-condition result for $`1/2 \in \mathcal{A}`$ used later in this paper. `coord:greedy-survival` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Lean\]</span>\
**Site:** (forward direction , reverse ).*
...
By Theorem <a href="#thm:greedy-survival-record" data-reference-type="ref" data-reference="thm:greedy-survival-record">36</a>, $`1/2 \in \mathcal{A}`$ iff no rank of this orbit is ever fatal. Two boundary degeneracies are excluded unconditionally and independently of any open hypothesis:

<div id="rem:no-ties" class="rem">

*Remark 38* (No-ties: both greedy-orbit boundaries are strict). (a) $`\rho_k \ne x_k`$ at any rank: a finite Mersenne sum has odd reduced denominator (each $`2^n-1`$ is odd), so no finite prefix mass can equal $`x_k`$ exactly, sharpening the general denominator-parity fact used in Observation <a href="#obs:half-controls-universal" data-reference-type="ref" data-reference="obs:half-controls-universal">29</a>. (b) $`\rho_k \ne T_{k+1}`$ at any rank: the tail $`T_{k+1} = E - 1 - \sum_{2 \le j \le k} x_j`$, where $`E := \sum_{n\ge1} x_n`$ is the Erdős–Borwein constant, is irrational by Erdős’s own 1948 theorem (the base-$`2`$ case of Theorem <a href="#thm:full-support" data-reference-type="ref" data-reference="thm:full-support">30</a>), while $`\rho_k`$ is always rational; equality is impossible. **Conclusion:** both greedy-orbit boundaries are provably strict at every rank, so the orbit’s decisions stabilize after finitely many strict inequalities at any fixed truncation depth. `coord:no-ties` <span class="sans-serif">scale:uniform</span> <span class="sans-serif">\[Math\]</span>
...
The seam model is a base-$`4`$ rescaled integer reformulation of Definition <a href="#defn:greedy-orbit" data-reference-type="ref" data-reference="defn:greedy-orbit">37</a>, built so that every quantity involved is an exact natural number (no real-number or limiting arithmetic). Fix a row $`n \ge 6`$. The seam weights at row $`n`$ are
``` math
w(n,d) := \left\lfloor \frac{4^n}{2^d-1} \right\rfloor, \qquad d = 2,\dots,n-1,
```
and the seam target is $`T_n := 2^{2n-1} - 2^n`$. The seam greedy process takes ranks $`d = 2,\dots,n-1`$ in ascending order against $`T_n`$ exactly as in Definition <a href="#defn:greedy-orbit" data-reference-type="ref" data-reference="defn:greedy-orbit">37</a>, producing a take set $`D_n \subseteq \{2,\dots,n-1\}`$ and skip set $`\mathrm{Skip}_n := \{2,\dots,n-1\} \mathbin{\backslash} D_n`$. Define the *seam remainder* $`\mathrm{rem}(n) := T_n - \sum_{d \in D_n} w(n,d)`$ and the *seam deviation* $`\Delta_n := \mathrm{rem}(n) - 2^n`$.

<div class="defn">

**Definition 39** (The universal recursion $`K(M)`$). For $`M \ge 2`$, define
``` math
K(M) := 2^{M-1} - \sum_{d=2}^{M} \left\lfloor \frac{2^M}{2^d-1} \right\rfloor.
```
$`K`$ is *row-independent*: it depends only on $`M`$, not on any seam row $`n`$. It satisfies the recursion $`K(2) = 1`$, $`K(M+1) = 2K(M) - (\tau(M+1)-1)`$, where $`\tau`$ is the number-of-divisors function (so $`\tau(M+1)-1`$ is the count of proper divisors of $`M+1`$). `coord:universal-recursion` <span class="sans-serif">scale:n/a</span> <span class="sans-serif">\[Cert\]</span>

</div>

<div id="thm:master-identity" class="thm">

**Theorem 40** (Master seam identity). *For every seam row $`n \ge 6`$,
``` math
\begin{equation*}
...
\end{equation*}
```
**Hypotheses:** $`n \ge 6`$; $`\mathrm{rem}(n)`$, $`D_n`$, $`\mathrm{Skip}_n`$ as in the seam greedy process for target $`T_n`$ over weights $`w(n,d) = \lfloor 4^n/(2^d-1) \rfloor`$, $`d=2,\dots,n-1`$, ascending greedy. **Conclusion:** the entire $`2n`$-bit seam deviation collapses to one universal, row-independent recursion value $`K(2n)`$ plus a short sum restricted to the skipped ranks only – every rank the greedy *takes* drops out of the identity entirely. `coord:seam-row / integer-greedy (Mersenne weights, binary digits of $K$)` <span class="sans-serif">scale:fixed</span> <span class="sans-serif">\[Math\]</span><span class="sans-serif">\[Cert\]</span>\
...
**Theorem 41** (Real (non-integer) form of the master identity). *Let $`x_d := 1/(2^d-1)`$ and define the constant
``` math
...
\end{equation*}
```
**Hypotheses:** as Theorem <a href="#thm:master-identity" data-reference-type="ref" data-reference="thm:master-identity">40</a>. **Conclusion:** escaping the wall (making $`\Delta_n`$ large, at scale $`2^{(n+5)/2}`$ or beyond – the exact target of Part 2’s Theorem A) is exactly the statement that the greedy skip-set’s real-valued partial sum $`\sum_{\mathrm{Skip}_n}
x_d`$ never approximates the fixed constant $`C`$ to within roughly $`2^{-1.5n}`$, at the natural scale $`2^{-n}`$ set by $`4^n \cdot 2^{-n} = 2^n`$. `coord:real/analytic (Erd\H{o}s--Borwein tail); bridges the
integer seam coordinate to a continuous Diophantine-approximation statement` <span class="sans-serif">scale:fixed</span> <span class="sans-serif">\[Math\]</span><span class="sans-serif">\[Cert\]</span>\
...
**Theorem 42** (Deviation dynamics and the R/M/U branch structure). *Let $`\lambda_n := \Delta_n / 2^n`$. Then
``` math
\begin{equation*}
\begin{aligned}
...
\end{equation*}
```
**Hypotheses:** as Theorem <a href="#thm:master-identity" data-reference-type="ref" data-reference="thm:master-identity">40</a>. **Conclusion:** the deviation dynamics is a doubling map on $`\lambda_n`$ with a forced signed digit $`e_n`$ (recording which of three branches – Right/Middle/Upper – fires at each row) plus a vanishing correction $`\delta_n = O(n
2^{-n})`$. **Measured bands** (rows $`14`$ through $`699`$, certified computation): branch R $`\lambda \in [-0.499189,\ 0.499846]`$; branch M $`\lambda \in [-0.998378,\ -0.501224]`$; branch U $`\lambda \in [0.501652,\ 0.999692]`$ – the three bands are disjoint and together nearly cover $`[-1,1]`$. `coord:seam-affine dynamics (doubling map with signed digit $e_n$)` <span class="sans-serif">scale:bounded</span> <span class="sans-serif">\[Math\]</span><span class="sans-serif">\[Cert\]</span>\
...
*Observation 43* (Exact low-row values). At row $`n=14`$: $`\mathrm{rem}(14) = 392`$, with take set $`D_{14} = \{2,3,6,7\}`$. At row $`n=15`$: $`\mathrm{rem}(15) = 34333`$. At row $`n=32`$: $`\mathrm{rem}(32) = 3865046005`$ and $`\Delta_{32} =
-429921291`$. `coord:seam row, exact integer` <span class="sans-serif">scale:fixed</span> <span class="sans-serif">\[Cert\]</span>\
**Site:** the row-$`14`$ through row-$`31`$ range, including $`\mathrm{rem}(14)=392`$, additionally carries independent <span class="sans-serif">\[Lean\]</span> confirmation via the kernel-computed certificate , which lists the exact seam remainder at every row $`13`$–$`31`$ by ‘decide‘-checked kernel computation and finds none violating the dyadic band-escape condition.

</div>

<div id="obs:anchors-depth" class="obs">

*Observation 44* (Certified computation depth, all four coordinates). The master identity (Theorem <a href="#thm:master-identity" data-reference-type="ref" data-reference="thm:master-identity">40</a>) is verified rows $`6 \le n \le 200`$ with zero mismatches, independently reproduced by two subagents, and the seam orbit itself is extended and certified to row $`2500`$ (and separately, in a longer single run, to row $`200{,}000`$ – §SE-7 of the underlying record, $`207.7\mathrm{s}`$ runtime, branch counts $`\mathrm{R{:}M{:}U} =
100197{:}49899{:}49898`$, TARGET failing at exactly one reset row $`\le 200{,}000`$, namely $`r=7`$). `coord:multiple: seam row, binary digit stream of $C$, greedy set $G$` <span class="sans-serif">scale:fixed</span> <span class="sans-serif">\[Cert\]</span>

</div>

<div id="obs:anchors-coords" class="obs">

*Observation 45* (The four coordinate depths, side by side). Definition <a href="#defn:greedy-orbit" data-reference-type="ref" data-reference="defn:greedy-orbit">37</a>’s single orbit is independently certified in four distinct integerizations, to the following depths: truncation rung $`J = 3,\dots,22`$ (all $`20`$ rungs proved to survive, not merely computed – Part 3 of the truncation-rung record); seam row $`6 \le n \le 2500`$ (Theorem <a href="#thm:master-identity" data-reference-type="ref" data-reference="thm:master-identity">40</a>/<a href="#thm:dynamics" data-reference-type="ref" data-reference="thm:dynamics">42</a>); integer margin scan to $`m_c = 5\times10^5`$ (advisory Type B scan, not yet promoted to a certified theorem); sharp tail margin, ranks $`2`$ through $`3000`$ (no fatal skip at any rank; take fraction $`0.4992`$; tightest certified take-margin $`0.0340`$ bits at rank $`7`$, the near-tie $`1/126 \ge 1/127`$). `coord:meta: all four coordinates` <span class="sans-serif">scale:fixed</span> <span class="sans-serif">\[Cert\]</span>\
...
**Theorem 50** (Straddle-prefix closed-set criterion). *If for every depth $`d`$ there exists a finite word $`u`$ (all exponents $`\le d`$) straddling target $`t`$, meaning $`\mathrm{value}(u)\le t\le \mathrm{value}(u)+\mathrm{mersenneTail}(d)`$ then $`t\in\mathrm{mersenneAchievementSet}`$. The proof needs only compactness of the achievement set, no coinductive machinery.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:dedekind-cut`*

</div>

<div id="thm:largest-skip-late" class="thm">

**Theorem 51** (Largest-skip-late induction: single-socket reduction). *Define *LargestSkipLateAt s* as “row $`s`$’s largest false rank $`d`$ satisfies $`2s<3d`$” and *LargestSkipLateStepSocket* as the universally quantified one-step arithmetic implication $`\forall s\ge 14,\,d,\; \mathrm{IsLargestFalseRank}(\mathrm{seamGreedyWord}\, s)\,d
...
**Theorem 52** (Middle-producer card/row escape sockets). *If, at every late middle-branch row, $`\mathrm{card}(\mathrm{support})+\mathrm{belowPulse}+5
...
**Theorem 59** (Erdős–Borwein full-support irrationality, unconditional). *For every base $`b\ge 2`$: $`\sum'_{n\ge 1} 1/(b^n-1)`$ is irrational, unconditionally. No open obligations. Base $`2`$ is the classical Erdős–Borwein constant. This is the terminal target every other rigidity/support theorem in this catalogue is measured against.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:mobius-mersenne`*

</div>

<div id="thm:reciprocal-summable-support" class="thm">

**Theorem 60** (Reciprocal-summable support irrationality, unconditional). *For every base $`b\ge2`$ and every infinite support $`A\subseteq\mathbb N`$ with $`\sum_{a\in A}1/a<\infty`$, the series $`\sum_{a\in A}1/(b^a-1)`$ is irrational. No pairwise-coprimality, periodicity, density, or powerful-support hypothesis is required. Binary LCM–Cesàro close returns transfer to every radix through a pointwise displacement bound, and an exact integral tail orbit then excludes rationality. The universal assertion remains open only in the reciprocal-divergent regime; this does not say that every support in that regime is unresolved. Novelty and priority are unassessed, and this proof is not identified with Erdős’s omitted coprimality-free argument.*
...
**Theorem 61** (Pairwise-coprime support irrationality, Erdős 1968). *For every base $`b\ge 2`$ and every infinite pairwise-coprime support $`A\subseteq\mathbb N`$ with summable reciprocals, $`\sum_{a\in A} 1/(b^a-1)`$ is irrational. This older adaptive-CRT theorem is retained as an attributed specialisation of Theorem <a href="#thm:reciprocal-summable-support" data-reference-type="ref" data-reference="thm:reciprocal-summable-support">60</a>.*
...
*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:lcm-gap`*

</div>

<div id="thm:factorial-twopow-support" class="thm">

**Theorem 64** (Factorial-support and $`2^k`$-support instances). *For every $`b\ge 2`$: $`\sum'_k 1/(b^{(k+1)!}-1)`$ is irrational, and $`\sum'_k
...
**Theorem 65** (Multiples-support irrationality via dilation). *For every $`b\ge 2`$, $`d\ge 1`$: $`\sum_{a\in d\mathbb N} 1/(b^a-1)`$ is irrational, via the exact identity $`\mathrm{erdosSupportSeries}\,b\,\{n:d\mid n\} = \sum'_k
1/((b^d)^{k+1}-1)`$, dilation to base $`b^d`$ full support, reducing directly to Theorem <a href="#thm:full-support-catalogue" data-reference-type="ref" data-reference="thm:full-support-catalogue">59</a>.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:dilation`*

</div>

<div id="thm:periodic-support" class="thm">

**Theorem 66** (Periodic-support irrationality: the periodic divisor-orbit sieve). *For every $`b\ge 2`$ and every $`m`$-periodic support $`A`$ ($`\forall n,\, n{+}m\in A
...
**Theorem 67** (Eventually-periodic support irrationality). *For every $`b\ge 2`$: an infinite support whose membership is $`m`$-periodic from some threshold $`N_0`$ on has $`\mathrm{erdosSupportSeries}\,b\,A`$ irrational.*
...
**Theorem 68** (Residue-class and odd-support irrationality). *For every $`b\ge 2`$, modulus $`m\ge 1`$, residue $`c`$: $`\sum_{n\equiv c\,(m)} 1/(b^n-1)`$ is irrational. Specializing $`m=2,c=1`$: $`\sum_{n\text{ odd}} 1/(b^n-1)`$ is irrational for every $`b\ge 2`$, the first support class of density strictly between $`0`$ and $`1`$ to land.*
...
**Theorem 69** (Signed periodic-weight dichotomy and its unconditional one-sided closures). *For base $`b\ge 2`$ and any $`m`$-periodic integer-valued signed weight $`w`$: the weighted series $`\sum w(a)/(b^a-1)`$ is either irrational or base-$`b`$ terminating ($`b^k\cdot x=z`$ for some integer $`k,z`$), unconditionally, mixed signs allowed. If additionally the divisor-coefficient *intWeightedCoeff w* is one-sided (all nonnegative, or all nonpositive) and not eventually zero, the terminating branch is excluded and the series is unconditionally irrational.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:signed-divisor-calculus`*

</div>

<div id="thm:mersenne-channel-survival" class="thm">

**Theorem 70** (Mersenne-channel denominator survival, T4). *For a finite family $`P`$ of “upper-half” primes (each $`p`$ with $`t<2p`$ for scale $`t`$, each dividing a squarefree radical $`r`$ all of whose prime factors are $`\le t`$): the pairwise-coprime Mersenne product $`C=\prod_{p\in P}(2^p-1)`$ survives fraction reduction, giving $`C/\gcd(C,h)\mid (h\cdot\mathrm{mobiusNumerator}(r)/\mathrm{mersenne}(r))\mathrm{.den}`$ exactly when $`C`$ is coprime to scale $`h`$ (or, weaker, when every prime factor of $`h`$ is $`\le t`$). This is the load-bearing quantitative step behind Theorem <a href="#thm:full-support-catalogue" data-reference-type="ref" data-reference="thm:full-support-catalogue">59</a>.*
...
*Observation 72* (Half-greedy regression fixtures: exact rational checkpoints). Three closed numeric facts, `norm_num`/`decide`-checked: $`1/2 -
...
<span class="sans-serif">\[Cert\]</span><span class="sans-serif">scale:fixed</span>`coord:regression-fixture`

</div>

<div id="obs:period-four-obstruction" class="obs">

*Observation 73* (Period-4 sign weight vanishes on a residue class). The explicit period-$`4`$ sign pattern $`1,0,-1,0`$ (*periodFourSignWeight*) has $`\mathrm{intWeightedCoeff}\,w\,n = 0`$ identically for every $`n\equiv 3\pmod 4`$, via the divisor-involution $`d\mapsto n/d`$. This is exactly why the general signed-periodic case (Theorem <a href="#thm:signed-periodic" data-reference-type="ref" data-reference="thm:signed-periodic">69</a>) only reaches a dichotomy rather than an unconditional theorem: no residue-blind selection can force a nonzero protected residue for every periodic signed weight.
...
**Theorem 74** (Möbius–Mersenne Lambert identity). *$`\sum'_{d:\mathbb N^+} \mu(d)/(2^d-1) = 1/2`$ exactly (imported as *MersenneLambertLadder.tsum_moebius_div_two_pow_sub_one_eq_half*). A signed (not Boolean) series over Mersenne denominators hitting the target value exactly, a landmark calibration point for rigidity arguments bounding how close a Boolean infinite support can get to $`1/2`$.*

*<span class="sans-serif">\[Cited\]</span><span class="sans-serif">scale:fixed</span>`coord:mobius-mersenne`*

</div>

<div id="cor:negative-mobius-overshoot" class="cor">

**Corollary 75** (Negative-Möbius Boolean support overshoots $`1/2`$). *Consequently the negative-Möbius Boolean support $`N=\{d\ge 2:\mu(d)=-1\}`$ *overshoots* $`1/2`$: $`\sum_{d\in N} 1/(2^d-1) = 1/2 + (\text{positive-M\"obius tail})
...
**Theorem 78** (The nine-way half-membership classification hub). *$`1/2\in\mathrm{mersenneAchievementSet}`$ is equivalent to *all* of: (1) $`\neg`$*SeamGreedyEventuallyRight*; (2) *SeamGreedyUnboundedTerminalFalse* (false successor terminal bits beyond every bound); (3) *SeamGreedyUnboundedUpperOrMiddle* (U/M events beyond every bound); (4) *SeamGreedyCofinalTerminalFalse* (one cofinal sequence of false terminals); (5) $`\exists\,\mathrm{rows},\,
\mathrm{SeamGreedyUnboundedSkippedRanksAlong}\,\mathrm{rows}`$; (6) the greedy skipped support is infinite (Theorem <a href="#thm:half-skip-dichotomy" data-reference-type="ref" data-reference="thm:half-skip-dichotomy">77</a>); (7) no last half-greedy skip exists (Theorem <a href="#thm:last-skip-iff-fatal" data-reference-type="ref" data-reference="thm:last-skip-iff-fatal">86</a>). The central hub of the whole half-branch: every sufficient-condition theorem elsewhere in this catalogue is secretly proving one of these seven equivalent forms; a disproof of any one (a bounded/eventually-right certificate) closes the branch.*
...
**Lemma 80** (Mersenne weight/tail: strict superincreasingness). *$`\mathrm{mersenneWeight}\,n = 1/(2^n-1)`$; $`\mathrm{mersenneTail}\,n = \sum'_k
...
**Theorem 81** (Greedy survival criterion — the master theorem). *$`x\in\mathrm{mersenneAchievementSet} \iff 0\le x \land \forall n,\,
...
**Lemma 82** (Rank-step trichotomy of the Dedekind cut). *From a depth-$`d`$ straddle of target $`t`$, exactly one of: (a) still straddles at $`d{+}1`$ without taking $`d{+}1`$; (b) straddles at $`d{+}1`$ after taking $`d{+}1`$; (c) $`t`$ sits in the fatal gap $`(\mathrm{value}(u)+\mathrm{mersenneTail}(d{+}1),\,
\mathrm{value}(u)+\mathrm{mersenneWeight}(d{+}1))`$. Specialized to $`t=1/2`$, branches (a) and (b) are proved mutually exclusive (*half_step_forced*), since both boundary equalities are impossible for $`1/2`$ (Lemma <a href="#lem:half-endpoint-kills" data-reference-type="ref" data-reference="lem:half-endpoint-kills">84</a>). A pure consequence of superincreasingness plus $`\mathrm{mersenneTail}\,n = \mathrm{mersenneWeight}(n{+}1)+\mathrm{mersenneTail}(n{+}1)`$.*
...
**Lemma 83** (Fatal-gap exclusion: kills every support, not just greedy). *Write $`P_A(d)`$ for the rank-$`d`$ prefix, $`T(d+1)`$ for the remaining tail, and $`w_{d+1}`$ for the next weight. If
``` math
P_A(d)+T(d+1)<t<P_A(d)+w_{d+1},
```
then the full support value is not $`t`$ for *every* support $`A`$ achieving that prefix, not merely the greedy orbit. Pure consequence of superincreasingness, no Mersenne-specific arithmetic beyond the weight function. The “kill certificate” primitive: any concrete numeric interval computation landing inside a fatal gap immediately falsifies half-membership via this lemma.*
...
**Lemma 84** (Half endpoint kills: no finite word hits $`1/2`$ exactly). *No finite positive-support word has value exactly $`1/2`$ (*positiveMersenneSupportValue_coe_finset_ne_half*, via the odd reduced denominator of the finite Erdős sum). No finite word plus a complete tail equals $`1/2`$ either (*half_ne_coe_finset_add_mersenneTail*, via irrationality of the Erdős–Borwein constant, same mechanism as Theorem <a href="#thm:half-skip-dichotomy" data-reference-type="ref" data-reference="thm:half-skip-dichotomy">77</a>). Discharges the two boundary branches of the rank-step trichotomy (Lemma <a href="#lem:rank-step-trichotomy" data-reference-type="ref" data-reference="lem:rank-step-trichotomy">82</a>), forcing every macrostep to be a strict trichotomy with no equality case.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:parity-irrationality`*

</div>

<div id="lem:straddle-agrees-greedy" class="lem">

**Lemma 85** (Straddle words are canonical: they agree with the greedy prefix). *Every half-straddling word agrees with the actual canonical greedy word through the same depth: $`\forall n\le d,\, n>0,\; (n\in u \leftrightarrow n\in
\mathrm{greedyMersenneSupport}(1/2))`$, by induction on $`d`$ using the backward step *erase_top*. Straddle words are forced to *be* the greedy prefix, so they do not form an independent search tree. This is what lets the word-level fatal-gap exclusion (Lemma <a href="#lem:fatal-gap-exclusion" data-reference-type="ref" data-reference="lem:fatal-gap-exclusion">83</a>, valid for any support) be applied specifically to rule out the actual greedy orbit’s half-representation.*
...
**Theorem 86** (Last-skip iff local fatality: a pointwise criterion). *Write $`G`$ for the greedy skipped support, $`R_M`$ for the greedy remainder, and $`T_M`$ for the remaining tail. A rank $`M`$ is the last skip exactly when $`M\in G`$ and the state at $`M`$ is fatal. Equivalently,
``` math
\frac12\in K \quad\Longleftrightarrow\quad \forall M\in G,\ R_M\le T_M.
```
This turns half-membership into an infinite conjunction over *actually skipped* ranks only. The reduction from a global fatal-state statement to a rank-local pointwise inequality is a generic greedy-recursion fact.*
...
**Lemma 87** (Seam upper-or-middle classification). *For row $`s\ge 5`$: the empirical “false successor terminal bit” event is exactly the arithmetic disjunction $`\mathrm{SeamGreedyUpperOrMiddleAt}\,s\,hs :=
(\mathrm{seamAdjacentCut}\,s\,hs).\mathrm{successorCarries} \lor (\neg\mathrm{carries}
\land 4\cdot\mathrm{remainder}+\mathrm{gap}-\mathrm{belowPulse} <
\mathrm{terminalWeight})`$. The empirical U/M/R label at each seam row is removable in favor of pure arithmetic on *seamAdjacentCut*; the seam-arithmetic analogue of the rank-step trichotomy (Lemma <a href="#lem:rank-step-trichotomy" data-reference-type="ref" data-reference="lem:rank-step-trichotomy">82</a>).*
...
**Theorem 90** (Final-middle-cell $`-3`$ excluded). *The first of three exceptional final-middle integer cells (coordinate $`4\cdot\mathrm{remainder}-\mathrm{belowPulse}-4=-3`$) cannot be the actual last transition before an all-right tail. Combines the analytic strict inequality of Lemma <a href="#lem:eventually-right-impossible" data-reference-type="ref" data-reference="lem:eventually-right-impossible">79</a> with a two-step Möbius-carry recurrence forcing the centred carry to zero then strictly negative within two more coefficient rows, contradicting the nonnegativity of Theorem <a href="#thm:mobius-centred-nonneg" data-reference-type="ref" data-reference="thm:mobius-centred-nonneg">56</a>. One of three exceptional cells ($`-3,-2,-1`$); the companion file records the analogous *binaryCoeffTail* inequalities for cells $`-2`$ and $`-1`$. Closing all three fully resolves the final-middle producer.*
...
**Lemma 91** (Skipped-endpoint trichotomy: frozen-margin sign classification). *At every genuinely skipped rank $`s\ge 5`$, the frozen margin $`\mathrm{greedyHalfFrozenMargin}(s{-}1)\,s`$ is in exactly one of three signed cells: negative (actual word $`=`$ seam-greedy word, remainder $`\ge 1`$, margin $`=
-\mathrm{remainder}`$); zero (actual word $`=`$ seam-greedy word, remainder $`=0`$); positive (actual word $`=`$ the seam “above” word, margin $`=\mathrm{overshoot}`$). The zero cell is provably safe; only the negative cell needs excluding, and it is exactly the case-split that Theorem <a href="#thm:final-middle-cell" data-reference-type="ref" data-reference="thm:final-middle-cell">90</a>’s three exceptional cells live inside.*
...
**Lemma 92** (Reverse carry-word overlap spacing: zero Mersenne content). *A *ReverseCarryWord* is $`(\mathrm{coeff},\mathrm{bit},\mathrm{carry}:\mathbb
...
**Lemma 93** (Relation-invariant linear channels force a rank-one determinant collapse). *If a finite family of linear channels $`\mathrm{channel}:\iota\to V\to_{\mathbb Q}
...
*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:linear-algebra`*
...
*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:rational-band`*

</div>

<div id="thm:sharp-fatal-gap" class="thm">

**Theorem 95** (Sharp two-thirds fatal gap: the true-tail-safe criterion). *For a skipped residual $`\rho=u/(2L)`$ at rank $`k`$, writing the skip margin as $`2^k u+a=2L+u`$ (so $`0<a\Leftrightarrow`$ skip): the dyadic safety test (Theorem <a href="#thm:two-thirds-band" data-reference-type="ref" data-reference="thm:two-thirds-band">94</a>) is $`u\le a`$, but the *true* tail-based safety test only needs $`2u\le 3a`$, proved via the explicit three-channel Lambert lower bound $`\mathrm{mersenneTailLB3}\,k = 1/2^k+1/(3\cdot 4^k)+1/(7\cdot 8^k) <
...
**Lemma 96** (Total gap-mass summability beyond any level). *$`\mathrm{mersenneGap}\,n := \mathrm{mersenneWeight}\,n-\mathrm{mersenneTail}\,n>0`$. The total gap mass strictly beyond level $`N`$ is summable and explicitly bounded: $`\sum'_k\mathrm{mersenneGap}(N{+}k{+}1) \le (2/9)(1/4)^N+(3/7)(1/8)^N\to 0`$, obtained by summing the pre-existing per-level asymptotic bound geometrically. **Scope caveat** (explicit in the source file header): bounds gap *mass* only: it says nothing about which reals the greedy run reaches, does not certify membership or nonmembership of any point, and in particular says nothing about $`1/2`$ directly.*
...
**Lemma 97** (Half-divisor unit drop: smallest single-bit coefficient producer). *Let $`C_a(m)`$ denote the support coefficient of the word $`a`$ at $`m`$. At row $`2(N+1)`$, changing only exponent $`N+1`$ from false to true gives
``` math
C_{a\mathbin{+}\mathrm{true}}(2N+2)=C_{a\mathbin{+}\mathrm{false}}(2N+2)+1.
```
This is the smallest possible “flip one bit, coefficient changes by exactly $`1`$” producer. The mechanism (flipping a single word-bit changes a divisor-incidence sum by exactly the number of new divisor relations created) is a generic digit/divisor-incidence fact, restating for any base-$`2`$ divisor-coefficient bookkeeping, including \#249’s own divisor sums.*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:divisor-incidence`*

</div>

<div id="thm:tempered-orbit-rigidity" class="thm">

**Theorem 98** (Tempered-orbit rigidity: the shared \#249/#257 trunk). *For any nonnegative-integer coefficient sequence $`c:\mathbb N\to\mathbb N`$ with $`c(n)\le
...
**Lemma 99** (Finite symmetric-difference tail-transfer of irrationality). *If $`\mathrm{erdosSupportSeries}\,b\,A`$ is irrational and $`A,B`$ differ by a finite symmetric difference, then $`\mathrm{erdosSupportSeries}\,b\,B`$ is irrational too. Irrationality of a support series is invariant under changing finitely many support elements. Fully general; likely reusable for \#249 tails as well. Feeds the eventually-periodic theorem (Theorem <a href="#thm:eventually-periodic" data-reference-type="ref" data-reference="thm:eventually-periodic">67</a>).*

*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:tail-transfer`*

</div>

<div id="lem:dyadic-excess-reformulation" class="lem">

**Lemma 100** (Dyadic-vs-Mersenne exact excess reformulation). *The greedy-toward-$`1/2`$ decision at rank $`n{+}1`$ (take the next Mersenne weight $`1/(2^{n+1}-1)`$ or skip it) is exactly equivalent, with no real-analytic approximation, to an integer sign test on the excess numerator $`\mathrm{nextDyadicExcessIntNumerator}\,p\,n\,L
...
**Lemma 101** (Denominator sandwich: odd-part survival, domain-neutral). *Subtracting an odd reduced fraction $`r/D`$ from a dyadic $`p/2^c`$ leaves a residual whose reduced denominator sandwiches the original: $`D\mid(\mathrm{residual}).\mathrm{den}\mid
...
*<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:uniform</span>`coord:p-adic`*

</div>

<div id="defn:achievement-set" class="defn">

**Definition 104** (The Mersenne achievement set and the support-series object). $`\mathrm{mersenneAchievementSet} := \{x\in\mathbb R \mid \exists A\subseteq\mathbb N,\,
...
(1/(b^n-1))\,n`$, with $`\mathrm{supportCoeff}\,A\,n := \#\{d\mid n : d\in A\}`$ (the Dirichlet incidence $`1_A * 1`$, \#257’s analogue of $`\tau`$); $`A=\mathrm{Set.univ}`$ recovers the full-support series of Theorem <a href="#thm:full-support-catalogue" data-reference-type="ref" data-reference="thm:full-support-catalogue">59</a>.

<span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>`coord:binary-digit`

</div>

<div id="prop:achievement-set-topology" class="prop">

**Proposition 105** (Achievement-set topology: compact, closed, perfect, measure exactly one). *$`\mathrm{mersenneAchievementSet}`$ is compact (continuous image of the binary-sequence Cantor space $`\mathbb N\to\mathrm{Fin}\,2`$ under the product topology, via $`\mathrm{positiveMersenneDigitValue}`$), hence closed; it is also perfect, totally disconnected, and nowhere dense, with Lebesgue measure exactly $`1`$. The compactness/closedness argument (binary coding $`\to`$ Cantor space $`\to`$ continuous image) is a fully generic technique for characterizing the achievement set of *any* absolutely convergent digit-weighted series, and it applies beyond Mersenne denominators: directly reusable for a \#249-style $`\varphi(n)/2^n`$ series after checking summability and superincreasingness. Closedness alone is what powers every “limit of a sequence of achieved points is achieved” argument in this catalogue (e.g. Theorem <a href="#thm:seam-limit" data-reference-type="ref" data-reference="thm:seam-limit">47</a>, Theorem <a href="#thm:straddle-closed-set" data-reference-type="ref" data-reference="thm:straddle-closed-set">50</a>).*
...
**Definition 109** (257bm:d1 — `ExactLocalMersenneHalfRow`). For $`n:\mathbb{N}`$:
``` math
...
**Definition 110** (257bm:d2 — `localPrefixQuotient`, `localMersenneQuotient`, `localRepairInteger`).
``` math
...
**Definition 111** (257bm:d3 — `localBinarySuffix`).
``` math
...
**Definition 112** (257bm:d4 — `CofinalExactLocalMersenneHalfRows`, THE MASTER PREMISE).
``` math
...
<span class="sans-serif">scale:cofinal</span> `coord:mobius-mersenne`
...
**Theorem 113** (257bm:c1 — `half_mem_mersenneAchievementSet_of_cofinalExactLocalRows`, THE PAYOFF THEOREM).
*``` math
...
*<span class="sans-serif">scale:cofinal</span> `coord:other:topological-achievement-set`*

</div>

<div class="thm">

**Theorem 114** (257bm:c2 — `cofinalExactLocalMersenneHalfRows_of_positiveHalfGreedySkips`, `half_mem_mersenneAchievementSet_of_positiveHalfGreedySkips`).
*``` math
...
*<span class="sans-serif">scale:cofinal</span> `coord:greedy-orbit`*
...
**Theorem 115** (257bm:c3 — `GlobalBooleanMobiusRepairFeasible`). *A repair $`T`$ (a bit-assignment $`\mathrm{bit}:\mathbb{N}\to\mathbb{N}\to\mathrm{Bool}`$, endpoint$`\times`$rank$`\to`$chosen bit, with $`\mathtt{bit\_stable}`$: once endpoint $`2d`$ is reached coordinate $`d`$ is frozen forever) is feasible when
``` math
...
**Theorem 116** (257bm:c4 — `SkippedCoreCriticalQuotientSupply`, THE REMAINING OPEN SOCKET).
*``` math
...
**Theorem 118** (257bm:c6 — `HalfGreedySkippedCriticalQuotientSupply`, the already-reduced canonical form).
*``` math
...
*<span class="sans-serif">scale:uniform</span> `coord:greedy-orbit`*

</div>

<div class="thm">

**Theorem 119** (257bm:c6a — `halfGreedy_precriticalSuffix_lt_of_next_skip`, unconditional two-consecutive-skips producer). *UNCONDITIONAL: if $`c\ge6`$ and BOTH rank $`c`$ and rank $`c+1`$ are skipped by the rational half-greedy orbit, then
``` math
...
**Theorem 121** (257bm:c7 — `exists_exactRowStrictUpperFill_of_skippedCoreSharpCapacity`). *For any $`c\ge4`$ and any finite $`D\subseteq[2,c)`$ with $`\mathrm{value}(D)<1/2`$ and the sharp capacity
``` math
...
**Theorem 122** (257bm:c8 — `exactLocalMersenneHalfRow_two_mul_sub_two_of_skippedCoreSharpCapacity`). *Bare-Prop corollary of 257bm:c7: the same hypotheses give $`\mathrm{ExactLocalMersenneHalfRow}(2c-2)`$ directly, dropping the support-extension data. The consumer form actually chained into 257bm:c1/c5 when the strict-upper separation is not needed downstream.*
...
**Proposition 123** (257bm:c9 — `localMersenneQuotient_eq_two_pow_sub_of_half_lt`). *In the pure upper window used by 257bm:c7’s fill, the Mersenne quotient is exactly $`\mathrm{localMersenneQuotient}(M,d) = 2^{M-d}`$ once the value condition places $`d`$ strictly above half the depth. This one identity is what lets the fill encode the missing residue as a literal Boolean word rather than a recursive repair, the arithmetic reason 257bm:c7 is possible at all.*
...
**Theorem 125** (257bm:c11 — `DyadicBandEscape` $`\Leftrightarrow`$ `CriticalDyadicBandIndex`, PROBLEM-AGNOSTIC quantifier collapse). *Pure $`\mathbb{N}`$-arithmetic, zero seam or Mersenne content: for $`d,E,j`$ with $`\mathrm{CriticalDyadicBandIndex}(d,E,j)`$ the unique nearest dyadic power $`2^{d-j+1}\ge E`$,
``` math
...
**Theorem 127** (257bm:c13 — (i) `HalfGreedySkippedFullShellNonnegative`).
*``` math
...
**Theorem 128** (257bm:c14 — (ii) `HalfGreedySkippedSeamAlignmentZero`).
*``` math
...
**Theorem 129** (257bm:c15 — (iii) `HalfGreedySkippedSeamEscape`).
*``` math
...
**Theorem 130** (257rig:c16 — a square-root carry bound implies half-membership). *Write $`G`$ for the greedy support and $`C_G(N)`$ for its centred carry. If
``` math
\forall N,\qquad C_G(N) \le 2\sqrt N + 4,
```
then the greedy skipped support is infinite and its reciprocal-Mersenne series equals $`1/2`$. Nonnegativity of $`C_G`$ is unconditional (see 257bm:i-carry below); only the upper $`2\sqrt N+4`$ bound remains open. This is independent of 257bm:c11’s dyadic-band route and 257bm:c7’s largest-skip socket.*

*<span class="sans-serif">scale:cofinal</span> `coord:other:sqrt-carry-growth`*

</div>

<div class="thm">

**Theorem 131** (257rig:c17 — cofinal terminal-strip witnesses imply half-membership).
*``` math
\begin{aligned}
...
**Theorem 134** (257bm:c20 — half-membership is equivalent to infinitely many greedy skips).
*``` math
(1/2:\mathbb{R})\in\mathrm{mersenneAchievementSet} \;\Longleftrightarrow\; (\mathrm{greedyMersenneSkippedSupport}(1/2)).\mathrm{Infinite}.
```
Odd-denominator parity supplies the positivity clause in 257bm:c2 automatically. Thus its cofinal-positive-skip hypothesis is exactly the assertion that the greedy skipped support is infinite, which the displayed theorem identifies with half-membership. See 257bm:k5 for the reason this restatement does not itself prove membership.*
...
**Theorem 136** (257bm:i1b — `localPrefixQuotient_succ`). *The rank-wise identity of 257bm:i1a summed over a fixed support $`D`$: exact recurrence for $`\mathrm{localPrefixQuotient}(D,n)\to\mathrm{localPrefixQuotient}(D,n+1)`$.*
...
**Definition 138** (257bm:i-cap — the sharp capacity test). For $`c\ge4`$, $`\forall d\in D,\ 2\le d<c`$, $`\mathrm{value}(D)<1/2`$:
``` math
\mathrm{localBinarySuffix}(D,1,2c-2) < 2^{c-2}
```
...
**Theorem 139** (257bm:i5 — `localBinarySuffix_two_mul_sub_two_lt_criticalCapacity_iff`). *Under the hypotheses of 257bm:i-cap:
``` math
...
**Theorem 140** (257bm:i6 — `localBinarySuffix_two_mul_sub_two_lt_upperHalfCapacity`, loose (one-bit) capacity, `BooleanMobiusSkippedCoreExactRow`). *Unconditionally, for every $`c\ge4`$ and every below-half core $`D\subseteq[2,c)`$ with deficit $`<\mathrm{mersenneWeightRat}(c)`$:
``` math
...
**Theorem 141** (257bm:i7 — `localBinarySuffix_two_mul_sub_one_lt_upperWindow_of_exact_below`). *For $`n\ge6`$, $`D`$ bounded $`[2,n]`$, $`2\in D`$, exact quotient at $`n`$, value $`<1/2`$:
``` math
...
**Proposition 142** (257bm:i-rank2 — `two_mem_of_exact_localMersenneQuotient`). *The side condition $`2\in D`$ used throughout the doubling branch is FREE: it holds uniformly for $`n\ge3`$ at any exact quotient, so it never needs separate discharge downstream.*
...
**Definition 143** (257bm:i-seed — `exactRowSixSupport`, the smallest exact row). $`\mathrm{exactRowSixSupport} := \{2,3,6\}`$ satisfies
``` math
...
<span class="sans-serif">scale:fixed</span> `coord:mobius-mersenne`
...
**Proposition 144** (257bm:i9 — `abs_localMersennePrefixValue_sub_half_le`). *An exact row at endpoint $`n`$ has real value within $`O(n/2^n)`$ of $`1/2`$: the quantitative bound $`|y_n-1/2|\le(n+1)/2^n`$ consumed by 257bm:c1 to turn a sequence of exact rows tending to depth infinity into a sequence of values tending to $`1/2`$.*
...
**Proposition 145** (257bm:i10 — `finite_boolSupport_ne_half`). *Cites the odd-denominator fact (`finiteErdosSum_den_odd`, 257bm:k11) to conclude directly: no finite Boolean support ever equals $`1/2`$ exactly. The corollary consumed at the very end of 257bm:c1’s chain to certify the achieved witness is infinite.*

*<span class="sans-serif">scale:uniform</span> `coord:p-adic`*
...
**Proposition 148** (257bm:i11c — `localMersenneHalfTarget_lower_word_eq_greedy_and_remainder_eq`). *Consumer form of 257bm:i11a specialised to the half-target: the lower word of any exact half-target construction is forced to equal the greedy word, remainder identity included. This is the general uniqueness principle instantiated by 257rig:i3 (the wall).*
...
**Theorem 151** (257bm:i-t7 — rationality is equivalent to a tempered binary orbit). *For $`c:\mathbb{N}\to\mathbb{N}`$ with $`\forall n,\ c\,n\le n`$:
``` math
...
```
Only the linear growth bound $`c(n)\le n`$ is required; the theorem uses no divisor, Mersenne, or \#257-specific structure. Fully problem-agnostic; instantiating $`c:=\varphi`$ gives a candidate \#249 reduction to the SAME criterion used for \#257, modulo an index-shift caveat that remains unverified.*
...
**Theorem 152** (257bm:i-mob — `moebius_mul_supportCoeffAF`, public, Möbius layer). *$`\mathrm{ArithmeticFunction.moebius} * \mathrm{supportCoeffAF}(A) = \mathrm{positiveSupportBitAF}(A)`$ for ANY $`A:\mathrm{Set}\,\mathbb{N}`$, exact Möbius recovery of a 0/1 support indicator from its divisor-count coefficient. Fully base-free; only downstream specialisations ($`\mathrm{erdosSupportSeries}\,2\,A`$) fix the base to 2.*

*<span class="sans-serif">scale:uniform</span> `coord:other:mobius-inversion`*

</div>

<div class="thm">

**Theorem 153** (257bm:i-bridge — `erdosSupportSeries_two_eq_binaryCoeffSeries`, public). *$`\mathrm{erdosSupportSeries}(2,A) = \mathrm{binaryCoeffSeries}(\mathrm{supportCoeff}\,A)`$. This identifies the \#257 support series at base 2 exactly with T7’s generic machinery, closing the loop that makes T7 the actual engine behind \#257’s rationality criterion too.*
...
**Theorem 154** (257rig:i2 — `dyadic_support_fraction_reciprocalMass_diverges_or_gt_one`). *If an INFINITE support $`A`$ has $`\mathrm{erdosSupportSeries}(2,A)`$ equal to a dyadic rational $`p/2^c`$ (any $`c`$, the easiest terminating case to rule out), then
``` math
...
*<span class="sans-serif">scale:uniform</span> `coord:other:divisor-envelope`*

</div>

<div class="prop">

**Proposition 157** (257rig:i4b — `card_divisors_pow_le_divisorSubpowerConst_pow_mul`). *$`\tau(n)^k \le (k^{2^k})^k\cdot n`$ for all $`n,k`$, a pure number-theory fact with zero support/problem content, directly reusable anywhere a $`1/k`$-power divisor-count envelope is needed. Would plausibly transfer to a \#249 argument bounding zero-runs of $`\varphi`$ itself, though $`\varphi`$’s growth is linear not polylog, so the same zero-window CONCLUSION would not transfer. Only this underlying divisor-bound lemma might, via a different route.*
...
**Definition 159** (257bm:i-cross — `localMersenneCrossingRanks`).
``` math
...
**Theorem 160** (257bm:i-cross2 — `exists_first_localMersenne_crossing`). *For $`E`$ with $`\forall d\in E,\ 2\le d`$ and $`1/2<\mathrm{localMersennePrefixValue}(E)`$:
``` math
...
**Theorem 161** (257hg:i6 — `mobiusCenteredHalfCarry_nonneg_of_supportSeries_lt_half` (G1)). *For $`\mathrm{mobiusCenteredHalfCarry}(A,N)`$ (an integer, defined via $`\mathrm{integerHalfCarry}`$): if $`1\notin A`$ and $`\mathrm{erdosSupportSeries}(2,A) < 1/2`$, then
``` math
...
**Definition 163** (257hg:i1 — the Mersenne gap). Let $`g_n:=w_n-T_n>0`$, the per-level shortfall between a single weight and the true tail sum beyond it.
...
**Theorem 164** (257hg:i2 — `mersenneGap_tail_le`, `tendsto_mersenneGap_tail_zero`). *The TOTAL gap mass strictly beyond level $`N`$ is summable and explicitly bounded:
``` math
...
*<span class="sans-serif">scale:uniform</span> `coord:other:summed-gap-mass`*
...
**Definition 165** (257hg:i3 — `mersenneTailLB3`, the three-channel Lambert lower bound). $`\mathrm{mersenneTailLB3}(k) := 1/2^k + 1/(3\cdot4^k) + 1/(7\cdot8^k) < \mathrm{mersenneTail}(k)`$. A strict rational lower bound obtained by truncating the Lambert-type series at three terms.

<span class="sans-serif">scale:n/a</span> `coord:other:lambert-bound`

</div>

<div class="thm">

**Theorem 166** (257hg:i4 — `skipSafe_of_two_mul_le_three_mul`, the sharp $`2u\le3a`$ criterion). *For a skipped residual $`\rho=u/(2L)`$ at rank $`k`$, writing the skip-margin as $`2^k u + a = 2L+u`$ (so $`0<a\Leftrightarrow`$ skip): the DYADIC safety test is $`u\le a`$, but the TRUE (tail-based) safety test only needs
``` math
2u \le 3a.
```
...
*<span class="sans-serif">scale:uniform</span> `coord:other:lambert-bound`*

</div>

<div class="prop">

**Proposition 167** (257hg:i5 — unconditional safety corollaries). *Unit numerators ($`u=1`$) are always safe. Conversely, fatality forces $`3a<2u`$, hence $`u\ge2`$, and it forces $`u\ge3`$ when $`u`$ is odd. The same conclusions hold for the actual Mersenne tail, rather than only its three-channel bound. The exact declaration names are linked below.*
...
**Theorem 168** (257bm:i13 — `SeamTwoSidedDyadicCellEscape.twoSided`). *Given a minimal local socket excluding exactly three middle "cells" plus one right-pulse-leak bound, induction (base case at row 5 verified by `decide`) propagates it to the universal two-sided bound
``` math
...
*<span class="sans-serif">scale:fixed</span> `coord:other:dyadic-boundary`*
...
*<span class="sans-serif">scale:uniform</span> `coord:other:abstract-perturbed-greedy`*

</div>

<div class="thm">

**Theorem 171** (257bm:i16 — `ReverseCarryWord`, abstract carry-overlap spacing). *`structure ReverseCarryWord` where $`\mathrm{coeff},\mathrm{bit},\mathrm{carry}:\mathbb{N}\to\mathbb{Z}`$; $`\mathrm{normalized}: \forall m,\ \mathrm{bit}(m)+2\cdot\mathrm{carry}(m) = \mathrm{coeff}(m)+
...
**Theorem 173** (257bm:k1 — `boundedDoubleOrRecycleModel_not_cofinal`, THE DICHOTOMY-SHAPE FALSIFIER, read before re-deriving cofinality from the dichotomy alone). *The dichotomy $`\mathrm{ExactLocalMersenneHalfRow}(2n{-}1) \vee \exists c,\,4\le c\le n\wedge
...
```
Packaged existentially as `exists_seeded_bounded_double_or_recycle_model`. The double-or-recycle transition shape, EVEN TOGETHER with an endpoint-six seed, is logically insufficient to conclude cofinal exact rows. A genuinely new progress input (strict endpoint growth, or an independent cofinality argument) is required. The TECHNIQUE of building a bounded one-point fixed model with the same recursion shape to show a transition schema doesn’t imply cofinality is fully general and transfers to any other double-or-recycle-shaped inductive scheme, including one built for \#249.*

*<span class="sans-serif">scale:fixed</span> `coord:other:meta-logical`*

</div>

<div class="thm">

**Theorem 174** (257bm:k-dich — `exactLocalMersenneHalfRow_double_or_recycle`, the dichotomy 257bm:k1 attacks). *For $`n\ge6`$, $`\mathrm{ExactLocalMersenneHalfRow}(n)`$:
``` math
\mathrm{ExactLocalMersenneHalfRow}(2n-1) \;\vee\; \exists c,\ 4\le c\le n \wedge \mathrm{ExactLocalMersenneHalfRow}(2c-2).
```
Every exact row of endpoint $`\ge6`$ has exactly one of two continuations: literal doubling (below-half) or recycling through a first-crossing to some $`2c-2`$ with $`c\le n`$ (the recycled endpoint is *not* claimed to exceed $`n`$). Equality is excluded by 257bm:k11 (`finiteErdosSum_den_odd`). What is missing, per 257bm:k1, is STRICT ENDPOINT PROGRESS in the recycle branch, or a proof the below-half branch recurs. Another dichotomy does not supply it. Any future attack that merely re-derives a double-or-recycle transition is already refuted.*

*<span class="sans-serif">scale:bounded</span> `coord:mobius-mersenne`*

</div>

<div class="prop">

**Proposition 175** (257bm:k2 — `splitFractionMass_one_bound_not_necessary_fixture`, sufficient-but-not-necessary fixture). *Concrete fixture $`D=\{2,3\}`$, $`c=5`$:
``` math
...
*Observation 176* (257bm:k3 — doubling immediately produces an above-half row, numeric countermodel). 257bm:i7’s doubling extension is NOT preserved iteratively: below-halfness is not preserved under one application of doubling, and demonstrably fails at the very first step. From the identity $`\mathrm{value}(E)-\tfrac12 = \big(\mathrm{localFractionMass}(E,2n{-}1)-1\big)/2^{2n-1}`$, at the seed $`D=\{2,3,6\}`$ (257bm:i-seed) with $`M=11`$ the core alone contributes
``` math
...
**Theorem 179** (257rig:k6 — `eq_halfGreedyPrefixSupport_of_critical_crossing`, THE WALL: the reduction to one universal-carry orbit). *For $`c\ge4`$, $`D`$ bounded $`[2,c)`$, below-half, with genuine crossing deficit $`\tfrac12-\mathrm{value}(D) < \mathrm{mersenneWeightRat}(c)`$:
``` math
...
**Proposition 186** (The unsafe middle range is exactly three integers). *For $`s\ge5`$, writing $`R=(\text{seamAdjacentCut } s\ hs).\mathrm{remainder}`$ and $`P=(\ldots).\mathrm{belowPulse}`$,
``` math
...
(, <span class="sans-serif">\[Lean\]</span>, <span class="sans-serif">scale:uniform</span>, `coord:mobius-mersenne`, proved by `omega` from the integer definitions). This is the exact unconditional source of the "three integer values" language used above for O5; it was previously asserted in prose only.

Two companion bounds pin the scaled rational residual inside a genuine middle transition: $`4\cdot\mathtt{seamGreedyFloorZ}\,s\le c`$ from a cell label $`4R=P+c`$ (, <span class="sans-serif">\[Lean\]</span>), and, combining it with the successor floor-$`Z`$ recurrence, $`4^{s+1}\cdot
\mathtt{seamWordRationalRemainder}(\mathtt{seamGreedyWord}(s+1))\le 2^{s+2}+c`$ (, <span class="sans-serif">\[Lean\]</span>). The module’s own remark is that for the first exceptional cell ($`c=-3`$, i.e. the cell already closed by ) this leaves margin exactly $`1`$ against a needed correction-tail slack of $`4/3+\varepsilon`$ — a concrete near-miss, not a proof, and it does not extend to the two open cells $`-2,-1`$.
...
**Proposition 188**. *Every `CylinderStage` at depth $`N`$ yields `HalfTerminalOnlyStripWitness` $`N`$ (, <span class="sans-serif">\[Lean\]</span>, `coord:support rigidity, half-carry`), and at a feedback row this survives *both* outputs of the corpus’s total feedback theorem — full-cylinder advance or a localized one-hole seam, which can delete at most one of carries $`3,4`$ (, <span class="sans-serif">\[Lean\]</span>).*
...
*Observation 189* (A new named producer socket for HALF, not yet in the implication table). The end-to-end consumer
``` math
...
**Definition 190**. $`\mathrm{SeamMiddleProducerTailEscape}`$ is the exact hypothesis that at every genuine middle transition ($`s\ge13`$, no successor carry, below the terminal-weight threshold) the terminal-augmented binary coefficient tail is strictly below the producer carry (). $`\mathrm{SeamMiddleProducerTailEscapeExceptNegThree}`$ is the same hypothesis restricted to cells $`\ne-3`$ ().

</div>

The reduction $`\mathrm{TailEscape}\Rightarrow\mathrm{ExceptNegThree}`$ is trivial by hypothesis-dropping (, <span class="sans-serif">\[Lean\]</span>); the substance is in the converse direction of the fan-in. The reduced socket, with cell $`-3`$ discharged by the already-proved , already suffices to close HALF:
``` math
\mathrm{SeamMiddleProducerTailEscapeExceptNegThree}\ \Longrightarrow\ (1/2:\mathbb{R})\in\mathcal{A}
```
(, <span class="sans-serif">\[Lean\]</span>), with the unreduced socket and the already-cited square-root majorant both routing through it (, <span class="sans-serif">\[Lean\]</span>). So the chain now reads $`\mathrm{SqrtEscape}\Rightarrow\mathrm{TailEscape}\Rightarrow
...
**Definition 191** ($`\mathrm{HalfGreedyGovernedFrozenMarginProducer}`$). For every $`k`$ with $`\mathtt{mersenneWeight}(k+1)`$ exceeding the current greedy remainder, some $`J\le k+1`$ gives a nonnegative frozen margin ().
...
**Proposition 192** (Exact Lebesgue-measure dichotomy). *Either $`J=F^c`$ for a finite $`F`$, and $`\mathrm{vol}(\mathtt{supportedMersenneAchievementSet}\,J)=2^{-|F|}`$ exactly, or $`J^c`$ is infinite and the volume is exactly $`0`$.*
...
|:---|:---|:---|:---|
|  |  |  |  |
| Smallest concrete exact row: support $`\{2,3,6\}`$ satisfies $`\mathrm{localPrefixQuotient}=2^5-1`$. <span class="sans-serif">\[Cert\]</span> | <span class="sans-serif">scale:fixed</span> | $`n=6`$ | `coord:mobius-mersenne` |
| One-point counter-model: the double-or-recycle transition shape alone, even seeded, is not cofinal. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:fixed</span> | $`n=6`$ (free-standing model) | `coord:other:meta-logical` |
| The combined-residue-mass $`\le 1`$ sufficient test for sharp capacity is not necessary: fixture $`D=\{2,3\}`$, $`c=5`$. <span class="sans-serif">\[Cert\]</span> | <span class="sans-serif">scale:fixed</span> | $`D=\{2,3\},\ c=5`$ | `coord:mobius-mersenne` |
| Exact residual fixture: $`\tfrac12-W(\{2,3,6,7\})=\tfrac1{16002}`$. <span class="sans-serif">\[Cert\]</span> | <span class="sans-serif">scale:fixed</span> | none (numeric instance) | `coord:other:regression-fixture` |
| Closed-form Möbius–Lambert value $`\sum_{d\ge1}\mu(d)/(2^d-1)=1/2`$ (imported identity). <span class="sans-serif">\[Cited\]</span> | <span class="sans-serif">scale:fixed</span> | none (closed-form value) | `coord:mobius-mersenne` |
| The one natural negative-Möbius candidate support overshoots: $`1/2 <
\sum_{d\in N}1/(2^d-1)`$ for $`N=\{d\ge2:\mu(d)=-1\}`$. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:fixed</span> | one fixed candidate support $`N`$ | `coord:mobius-mersenne` |
|  |  |  |  |
...
| Every crossing core is forced to be the canonical rational half-greedy prefix. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:uniform</span> | $`\forall c\ge4\ \forall D`$ bounded $`[2,c)`$ | `coord:greedy-orbit` |
| Theorem A payoff: reset $`\sqrt{\ }`$-escape socket $`\Rightarrow`$ membership. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:uniform</span> | $`\forall s\ge14`$, given the socket hypothesis | `coord:seam-integer` |
| Band-avoidance certified for every row $`13\le r\le31`$ (feeds the F5/Theorem A uniform hypothesis as an exhaustively-verified finite range). <span class="sans-serif">\[Cert\]</span> | <span class="sans-serif">scale:uniform</span> | $`13\le r\le31`$, exhaustive | `coord:seam-integer` |
| Critical-band-index reduction: a $`(d{+}1)`$-way check collapses to one nearest-boundary check. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:uniform</span> | $`\forall d,E`$ with $`E\le2^{d+1}`$ | `coord:other:dyadic-boundary` |
...
| Every infinite reciprocal-summable support gives an irrational support series at every integer base. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:uniform</span> | $`\forall b\ge2\ \forall A`$ (infinite, reciprocal-summable) | `coord:mobius-mersenne` |
| Erdős 1968 pairwise-coprime theorem, retained as a represented specialisation. <span class="sans-serif">\[Lean\]</span> | <span class="sans-serif">scale:uniform</span> | $`\forall b\ge2\ \forall A`$ (infinite, coprime, summable) | `coord:mobius-mersenne` |
...
**Proposition 194** (One orbit, four coordinates). *Fix the weights $`x_n=1/(2^n-1)`$ and target $`1/2`$. Sufficiently fine truncations of any one of the four integerisations above reproduce the full greedy orbit’s decisions exactly on any fixed finite prefix of ranks; consequently all four coordinates decide the *same* question, namely whether $`1/2`$ lies in the Mersenne achievement set.*
...
**Lemma 195** (No-ties lemma). *At every rank $`k`$ of the full greedy orbit for target $`1/2`$, both defining comparisons are strict: $`\rho\ne x_k`$ and $`\rho\ne T_{k+1}`$.*
...
*Take boundary excluded (parity).* Any finite Mersenne sum $`P/D:=\sum_{d\in D}x_d`$ has $`D`$ odd, since every weight’s denominator $`2^d-1`$ is odd and a product/lcm of odd numbers is odd (equivalently, , itself traced to ). Hence the residual after any finite prefix is $`\rho=\tfrac12-\tfrac{P}{D}=\tfrac{D-2P}{2D}`$: its reduced denominator is *even*. Every weight $`x_k=1/(2^k-1)`$ has *odd* denominator. An even-denominator rational can never equal an odd-denominator rational, so $`\rho=x_k`$ is impossible at any rank. The take comparison is never a tie.
...
*Remark 196*. Lemma <a href="#lem:no-ties" data-reference-type="ref" data-reference="lem:no-ties">195</a> is exactly what licenses Proposition <a href="#prop:one-orbit" data-reference-type="ref" data-reference="prop:one-orbit">194</a>: because no comparison is ever exactly on a boundary, every rank’s decision is stable under sufficiently fine truncation. There is no knife-edge case where refining $`J`$, extending $`s`$, tightening $`m_c`$, or sharpening the tail margin could flip a decision that a coarser coordinate had already settled on a fixed prefix. This is the sense in which the four bullet points above are one decision stream, not four independent producers that happen to agree so far.
...
**Lemma 197** (Forced greedy). *For every $`J\ge2`$: $`w_n^{(J)}>T_{n+1}^{(J)}`$ for all $`n`$ (the $`q=1`$ terms agree exactly, and every $`q\ge2`$ tail term is strictly smaller than the corresponding weight term). Consequently the weight sequence is strictly superincreasing, the greedy support is the unique candidate support, and $`\mathrm{HalfRung}(J)`$ holds iff the greedy orbit for $`1/2`$ under weights $`w_n^{(J)}`$ never lands in a fatal interval $`(T_{n+1}^{(J)},w_n^{(J)})`$. Rank $`1`$ is always a safe skip; ranks $`2`$ and $`3`$ are always takes.*

</div>

<div id="lem:tr-parity" class="lem">

**Lemma 198** (Parity forces infinite support). *For every $`J\ge2`$: no finite $`A`$ attains $`\mathrm{HalfRung}(J)`$. Scaling by $`2^{J\cdot\max A}`$ makes the maximal element’s contribution odd while every other element’s contribution is even, so a support achieving $`1/2`$ exactly under $`J`$-truncated weights must be infinite. (This is the rung analogue of the parity exclusion in Lemma <a href="#lem:no-ties" data-reference-type="ref" data-reference="lem:no-ties">195</a>: the mechanism is the same shape, one level down.)*

</div>

<div id="thm:tr-witness-exclusion" class="thm">

**Theorem 199** (Witness exclusion). *Define the misalignment mass
``` math
...
*Observation 207* (oa-2 / deficit-side run law). <span class="sans-serif">\[Lean\]</span> . **Yields:** $`\forall k, s.\ 5 \le s \to \mathrm{rem}(s) \le 2^s \to 2^s \le 2^k(2^s -
...
*Observation 208* (rc-2 / affine excess recurrence). <span class="sans-serif">\[Lean\]</span> , , (recurrence ). **Yields:** for an R-run of length $`k`$ from row $`S`$: $`X_k + \mathrm{charge}_k = 4^k X_0`$ exactly, with $`X_j = \mathrm{rem}(S+j) - 2^{S+j}`$ and $`\mathrm{charge}_{j+1} = 4\,\mathrm{charge}_j
...
*Observation 209* (oa-2 sibling / late-row ceiling). <span class="sans-serif">\[Lean\]</span> . **Yields:** $`\forall s \ge 5`$, at a largest false rank $`d`$ with $`2s<3d`$ (late row): $`3\,\mathrm{rem}(s) < 3\cdot 2^{s+1} + 2\cdot4^{s-d}+4`$. Unconditional, uniform in $`s,d`$; since late means $`s-d<s/3`$, the error term is $`<2^{2s/3}`$. **Mismatch (`coordinate_only`):** an upper bound on the deviation where the obligation needs a lower bound on its absolute value, the excess-side ceiling the rc-2 run law needs, not anti-concentration itself. Degrades to vacuous outside the late-row regime. **Closes it:** nothing on its own; its role is discharging the “$`X_k`$ bounded above” premise of rc-2 without invoking the open `SeamGreedyRemainderGapBound`.

</div>

<div class="obs">

*Observation 210* (G5 / no-tie). <span class="sans-serif">\[Lean\]</span> , with . **Yields:** $`\forall s \ge 5`$, the greedy decision boundary is never exactly attained. Combined with integrality this is the corpus’s *only* unconditional lower bound on the deviation: $`|\Delta_r| \ge 1`$. **Mismatch (`scale_only`):** deviation $`\ge 2^0`$ versus the required deviation $`>
...
*Observation 211* (H4, PRIVATE / sharp fatal-gap test). <span class="sans-serif">\[Lean\]</span> , , , . **Yields:** writing the residual in lowest terms $`\rho=u/(2L)`$, $`u,L`$ odd, $`a := 2L -
(2^k-1)u`$, the sharp safety test is $`2u \le 3a`$ (strictly better than the old dyadic test $`u \le
a`$). Contrapositive: fatal $`\Rightarrow 3a<2u \Rightarrow u \ge 2`$, and $`u \ge 3`$ when $`u`$ is odd. Uniform in $`k \ge 1`$. **Mismatch (`scale_only`):** same shape as the obligation, a numerator lower bound at a dangerous rank, but the bound is the constant $`3`$ where growth $`2^{(r+5)/2}`$ is needed. The constant $`3`$ is the Erdős–Borwein tail-ratio limit $`\theta_k := 1/T(k+1)-(2^k-1) < 2/3`$; adding Lambert channels sharpens $`2/3`$ toward the true limit but never produces growth in $`k`$. **Closes it:** a rank-scaling version, e.g. fatal at rank $`k \Rightarrow u \ge 2^{k/2}`$. The current mechanism provably cannot deliver this, since the ceiling is the tail ratio itself, so this needs a different arithmetic input, not a sharper truncation.

</div>

<div class="obs">

*Observation 212* (F5b / linear-width band escape, rows 13–30). <span class="sans-serif">\[Lean\]</span> . **Yields:** at every upper reset row $`d`$, $`13 \le d \le 30`$, the reset charge $`E_d =
4\cdot\mathrm{overshoot}_d + \mathrm{abovePulse}_d`$ avoids the linear-width band $`(2^{d-j+1}-2(d+j),\ 2^{d-j+1}]`$ for every $`j \le d`$, the finite segment of , which by delivers the whole \#257-false payoff. **Mismatch (`scale_only`):** proved at 18 explicit rows by `interval_cases` plus per-row `decide+kernel`; the obligation needs all $`d \ge 13`$. The band width here is only $`O(d)`$, *linear*, while sqrt-escape demands width $`2^{(r+5)/2}`$; this socket is exponentially weaker than 257-reset yet lands the same downstream conclusion. **Closes it:** a structural lower bound on the distance from $`E_d`$ to the nearest dyadic power at or above it, of width $`2(d+j_0)`$ at the single critical index $`j_0`$, already collapsed from $`\forall j`$ to one inequality by . Only $`O(d)`$ separation is required, not sqrt-of-scale.

</div>

<div class="obs">

*Observation 213* (oa-3b / M-ancestor and R-run closure). <span class="sans-serif">\[Lean\]</span> , , . **Yields:** unconditional, uniform in $`s\ge5`$: a middle (M) branch forces $`s+1 \le
...
*Observation 218* (E1 / remainder ceiling). <span class="sans-serif">\[Lean\]</span> , (weaker cofinal route ). **Yields:** if $`\forall s \ge 6`$, $`\mathrm{rem}(s) < 2^{s+1}`$, then $`1/2 \in
...
*Observation 219* (SE-7 / RC-7 / certified computation to row 200,000). <span class="sans-serif">\[Cert\]</span> **Yields:** exact integer arithmetic to row 200,000, branch counts $`R\!:\!M\!:\!U =
...
*Observation 222* (TR-5 / TR-6 / truncation-rung ladder, parallel coordinate). <span class="sans-serif">\[Cert\]</span> **Yields:** $`\mathrm{HalfRung}(J)`$ genuinely *proved* (not merely computed) for each $`3
...
*Observation 223* (Sharp-capacity fill, unconditional). <span class="sans-serif">\[Lean\]</span> , . **Yields:** for any $`c \ge 4`$ and any finite $`D \subseteq [2,c)`$ with $`\mathrm{localMersennePrefixValue}\,D < 1/2`$ and $`\mathrm{localBinarySuffix}\,D\,1\,(2c{-}2) <
2^{c-2}`$: an exact row $`E`$ at endpoint $`2c{-}2`$ with $`D \subseteq E`$, every new rank strictly above $`c`$. Unconditional, no deficit/crossing hypothesis, no reference to the greedy orbit. **Mismatch (`hypothesis_strength`):** needs a per-$`c`$ input $`\mathrm{localBinarySuffix}\,D\,1\,(2c{-}2) < 2^{c-2}`$, and the corpus only ever supplies that input through the crossing-deficit route, which then rigidly collapses onto $`D = \mathrm{halfGreedyPrefixSupport}(c{-}1)`$. The theorem itself is strictly more general than every consumer that calls it. **Closes it ($`\star`$):** $`\forall N\ \exists c \ge N,\ \exists`$ finite $`D \subseteq [2,c)`$ with $`\mathrm{localMersennePrefixValue}\,D<1/2`$ and $`\mathrm{localBinarySuffix}\,D\,1\,(2c{-}2) <
...
*Observation 224* (Sharp-capacity induction, engine complete). <span class="sans-serif">\[Lean\]</span> $`\to`$ $`\to`$ (; seed line 1023; strict progress ). **Yields:** a complete, already-formalised induction from the endpoint-six seed to the full obligation: each protected row either doubles below half unconditionally, or recycles at its first crossing rank $`e`$, giving endpoint $`2e{-}2 >`$ previous endpoint. Terminates literally at `CofinalExactLocalMersenneHalfRows`. **Mismatch (`hypothesis_strength`):** the recycle branch consumes `SkippedCoreCriticalQuotientSupply` at essentially every step (the below-half branch never fires twice, by the seed arithmetic). Protection (endpoint $`< 2\cdot`$cutoff, new ranks $`>`$ cutoff) is exactly what converts a non-growing recycle endpoint into strict progress; everything else is finished. **Closes it:** the sharp inequality at one crossing rank per step, canonical form : $`\forall c \ge 4`$ skipped by the rational half-greedy orbit, $`2^{(2c-2)-1} \le
...
*Observation 225* (Unconditional capacity, off by one bit). <span class="sans-serif">\[Lean\]</span> (word form line 228). **Yields:** unconditionally, $`\forall c \ge 4`$ and every below-half core $`D \subseteq [2,c)`$ with deficit $`< \mathrm{mersenneWeightRat}\,c`$: $`\mathrm{localBinarySuffix}\,D\,1\,(2c{-}2) <
...
*Observation 229* (Frozen margin, ineffective horizon). <span class="sans-serif">\[Lean\]</span> , (margin def line 1021, recurrence line 1036, monotonicity line 1094). **Yields:** at every dyadically safe rank $`k>0`$, $`\mathrm{greedyHalfFrozenMargin}\,k\,J \ge
0`$ for *some* horizon $`J`$: the finite coefficient window eventually covers the centred carry. Nonnegativity is monotone up in $`J`$. **Mismatch (`scale_only`):** the horizon comes from a non-effective limit argument (`tendsto_finiteCoeffWindow_atTop` plus an existence extraction), so $`J`$ is unbounded. The exact-row route needs the crossing by the specific horizon $`J=c-3`$ (, $`k=c-1`$). All ingredients to make it effective already exist: gives $`\mathrm{gap}=\mathrm{tail}(k{+}1{+}J)/2^J`$, and gives $`\mathrm{tail}(m)\le m+2`$. **Closes it:** margin$`(k,J)\ge0`$ as soon as $`(k{+}J{+}3)/2^J <
...
*Observation 231* (Cofinal positive skips *is* the obligation). <span class="sans-serif">\[Lean\]</span> , , line 60; A5 , A6 (); positivity discharge . **Yields:** cofinally many positive rational-greedy skips $`\Rightarrow`$ `CofinalExactLocalMersenneHalfRows` $`\Rightarrow 1/2 \in
...
*Observation 232* (C4a / terminal-only strip, wrong locality). <span class="sans-serif">\[Lean\]</span> , , . **Yields:** cofinally many depths $`M`$ carrying a finite normalized word $`a`$ with $`|\mathrm{integerHalfCarry}(\mathrm{wordSupport}\,a)(M{-}1)| \le \mathrm{halfStripBound}\,M
\Rightarrow \exists A`$ infinite with $`\mathrm{erdosSupportSeries}\,2\,A=1/2`$. Same architecture as the obligation: cofinal, no coherence, mutually incompatible witnesses allowed.

**Mismatch (`multiple`):** the two sockets trade off in opposite directions and neither implies the other. C4a tolerates *any* support inside the depth-$`M`$ window but demands the carry inside a $`\sim 2\sqrt M`$ strip; the obligation form ($`\star`$) tolerates a carry up to $`2^{M/2-1}`$, exponentially looser, but demands the support be confined to ranks $`<
...
**Closes it:** a lower-half confinement upgrade of the strip witnesses, $`\forall N\
...
*Observation 233* (C4c / G3 / greedy carry vs. frozen prefix carry, coordinate mismatch). <span class="sans-serif">\[Lean\]</span> , , line 157; , line 805 (nonnegativity from line 842 / ). **Yields:** cofinal returns of the actual greedy carry to the square-root strip (C4c), or a pointwise $`2\sqrt N+4`$ bound on the Möbius-centred carry (G3), each give an infinite support with value exactly $`1/2`$. **Mismatch (`coordinate_only`):** these bound the carry of the *infinite* greedy support $`\mathrm{greedyMersenneSupport}(1/2)`$; sharp capacity is about the *frozen finite prefix* $`\mathrm{halfGreedyPrefixSupport}(c{-}1)`$. The two carries agree only at index $`k`$ (inside ), not at depth $`2c{-}3`$ where the fill needs it. The bridge is already landed: : sharp capacity $`\Leftrightarrow \mathrm{mobiusCenteredHalfCarry}\,G\,(2c{-}4) \le
...
*Observation 234* (Above-support recycling, no growth guarantee). <span class="sans-serif">\[Lean\]</span> , , . **Yields:** unconditional: every finite above-half support bounded by $`n`$ recycles through its first crossing rank $`c`$ into a genuine exact row at endpoint $`2c{-}2`$, with $`4 \le c \le n`$. No capacity hypothesis at all. **Mismatch (`quantifier_order`):** the witness is $`\exists c \le n`$, not $`\exists c`$ large: $`2c{-}2`$ may be $`\le n`$, so the endpoint need not grow. is the explicit falsifier: $`\mathrm{boundedDoubleOrRecycleModel}\,n := (n{=}6)`$ satisfies the same transition schema plus a seed and is *not* cofinal. Growth is recovered only inside `ProtectedExactLocalMersenneRow`, whose invariants force $`c>`$cutoff, hence $`2c{-}2>`$ endpoint, and maintaining those invariants is precisely what needs the sharp-capacity fill rather than this general recycling. **Closes it:** a crossing-location lower bound, for the supports actually produced, the first crossing rank satisfies $`2c{-}2>n`$. Equivalently, keep protection alive without sharp capacity, e.g. a fill placing all new ranks above $`c`$ using only the landed $`c{-}1`$-bit capacity.

</div>

<div class="obs">

*Observation 235* (Double-or-recycle dichotomy, proved insufficient). <span class="sans-serif">\[Lean\]</span> , , . **Yields:** $`\forall n \ge 6`$, an exact row at $`n`$ gives an exact row at $`2n{-}1`$ or at $`2c{-}2`$ for some $`4 \le c \le n`$; plus a formal proof that this shape, even seeded at endpoint six, is logically insufficient for cofinality. **Mismatch (`quantifier_order`):** the dichotomy is proved by splitting on the sign of the witness value (`finiteErdosSum_den_odd` excludes equality); the no-go pins the deficit precisely: what is missing is strict endpoint progress in the recycle branch, or a proof that the below-half branch recurs. Another dichotomy does not supply it. **Any future attack that merely re-derives a double-or-recycle transition is already refuted by this row.** **Closes it:** either (a) the recycle branch upgraded to $`2c{-}2>n`$, or (b) cofinally many below-half exact rows so the doubling branch alone is cofinal. Nothing weaker than one of these two can work through this dichotomy.
...
*Observation 236* (NM-01 / the single certificate socket). <span class="sans-serif">\[Lean\]</span> (carry variant ). **Yields:** $`\forall b\ge2,\ \forall A\subseteq\mathbb N`$ (no infinitude, no structure hypothesis at all): if $`\forall q>0\ \exists N,K,L,C`$ with $`K\le L`$, $`b^r \mid
\mathrm{supportCoeff}\,A\,(N{+}r)`$ for $`r\in[1,K]`$, $`\sum_{r=K+1}^L \mathrm{supportCoeff}\,A\,(N{+}r)
b^{L-r}\le C`$, $`\exists t,\ 0<\mathrm{supportCoeff}\,A\,(N{+}L{+}1{+}t)`$, and $`q(C{+}N{+}L{+}2)<b^L`$, then $`\mathrm{erdosSupportSeries}\,b\,A`$ is irrational. The certificate-based \#257 families (full support, multiples, periodic, eventually periodic, residue class, odd, pairwise coprime, and the conditional sunflower bouquet) factor through this socket. The stronger reciprocal-summable theorem uses the separate close-return route. **Mismatch (`hypothesis_strength`):** already universally quantified over $`A`$: the universal quantifier is *not* the gap. The hypothesis `hcert` is a cofinal producer supplied only for named classes; the theorem’s own docstring: “certificate existence for $`f_A`$... for arbitrary infinite $`A`$ remains an open obligation.” **Closes it:** a single producer theorem, $`\forall A`$ infinite, $`\forall q>0`$, $`\exists
...
*Observation 237* (NM-02 / single-target to all-rational-targets, promotable). <span class="sans-serif">\[Lean\]</span> (forward ; reverse ). **Yields:** $`(1/2:\mathbb R)\in\mathrm{mersenneAchievementSet} \Leftrightarrow
...
(\mathrm{greedyMersenneSkippedSupport}(t:\mathbb R))\text{.Infinite})`$ with no new mathematics. **Closes it:** exactly this re-parametrisation. It converts the entire half-greedy refutation machine from a one-target statement into a statement about every rational target, the quantifier shape 257-universal actually needs.

</div>

<div class="obs">

*Observation 238* (NM-03 / target-free engine underneath a target-pinned corollary). <span class="sans-serif">\[Lean\]</span> ; engine . **Yields:** $`\forall A`$ finite with $`0\notin A`$: $`\mathrm{erdosSupportSeries}\,2\,A \ne
...
*Observation 242* (NM-07 / sunflower bouquet: selector superseded, universal class-existence false). (selector def line 406; carry socket line 381; bouquet structure line 33). **Yields:** for $`A=`$ (finite exceptional set dividing $`Q`$) $`\cup \{\mathrm{core}_i \cdot
...
**Current conclusion.** The selector is unnecessary for irrationality: the displayed decomposition gives $`\sum_{a\in A}1/a\le\sum_{a\in E}1/a+\sum_i1/p_i<\infty`$. Infinitude and the all-base reciprocal-support theorem therefore settle every such bouquet unconditionally. The [all-base composition](https://github.com/wcook04/plectis-lean-erdos249-257/blob/43c36af8b9d5795d0a965825ec01110b87159df6/Erdos249257/BouquetReciprocalIrrationality.lean#L85) is now kernel checked. This composition supersedes the conditional endpoint as a mathematical result. The pinned declaration above records the earlier certificate proof. <span class="sans-serif">\[Math\]</span>

**Remaining class boundary.** A bouquet-existence theorem $`A`$.Infinite $`\to`$ Nonempty(bouquet structure) is *false* as stated (take $`A=`$ all multiples of 6; no coprime petal decomposition), so the class can only be widened, never promoted to universal.

</div>

<div class="obs">

*Observation 243* (NM-08 / eventual periodicity and finite perturbations). <span class="sans-serif">\[Lean\]</span> (engine line 11262, producer line 11072; finite-perturbation transfer lines 9139, 9148). **Yields:** $`\forall b\ge2\ \forall m{>}0\ \forall N_0`$: $`A`$ infinite and $`m`$-periodic above $`N_0`$ $`\Rightarrow`$ irrational. Plus: the class of supports with irrational series is closed under finite symmetric difference in both directions.

**Class boundary.** Eventual periodicity covers every modulus and threshold, but is not the widest unconditional support criterion. It and the nonperiodic small-shift regimes concern different classes. Finite symmetric difference preserves eventual periodicity, so changing a finite prefix cannot enlarge this particular class; a further extension must relax its asymptotic structure.

**Closes it:** a “locally periodic at cofinally many scales” version, $`\forall q{>}0\
...
*Observation 244* (NM-09 / lcm-gap, far-sparse edge only). <span class="sans-serif">\[Lean\]</span> (instances , ). **Yields:** $`\forall b\ge2,\ \forall a:\mathbb N\to\mathbb N`$ StrictMono, $`a_0\ge1`$: if $`a_k - \mathrm{lcm}\{a_0,\dots,a_{k-1}\}\to\infty`$ then $`\sum_k 1/(b^{a_k}-1)`$ is irrational, via . **Mismatch (`hypothesis_strength`):** the $`\mathbb N`$-truncated-subtraction hypothesis requires $`a_k`$ to outgrow the lcm of all earlier elements, extreme lacunarity. Fails immediately for $`A=`$primes (lcm of first $`k`$ primes vastly exceeds $`p_k`$), squares, any positive density $`A`$. Covers only the far sparse edge, the same edge NM-05’s T11 also patrols. **Closes it:** a relaxed gap measuring the surviving denominator rather than the raw lcm: $`a_k - \log_b(\text{reduced denominator of the }k\text{-th partial sum}) \to \infty`$. The corpus already has the denominator-survival algebra to state this (`divisor_dvd_divInt_den`, `survivingDivisor_dvd_scaled_divInt_den` at , and giving $`2^{t/2}`$ growth); the growth theorem for the surviving denominator of an *arbitrary* support is what is missing.

</div>

<div class="obs">

*Observation 245* (NM-10 / the load-bearing coordinate, iff over all $`A`$). <span class="sans-serif">\[Lean\]</span> (line 376); T7 . **Yields:** $`\forall A\subseteq\mathbb N`$ (no hypothesis at all): $`\mathrm{HasRationalValue}(\mathrm{erdosSupportSeries}\,2\,A) \Leftrightarrow \exists q{>}0\
\exists U:\mathbb N\to\mathbb Z,\ \mathrm{IsTemperedBinaryOrbit}(\mathrm{supportCoeff}\,A)\,q\,U`$. **Mismatch (`coordinate_only`):** already universal over $`A`$, the exact coordinate in which the obligation should be stated, and the literal shared trunk with \#249 (instantiate T7 at $`c:=\mathrm{Nat.totient}`$). It does not refute the orbit: pins the orbit uniquely to $`u(N)=qT_c(N)`$, and $`T_c(N)\le N{+}2`$ confirms it is never ruled out on growth grounds for any $`c`$ with $`c(n)\le n`$. **Closes it:** a support-specific non-existence theorem, $`\forall A`$ infinite, $`\forall
...
*Observation 246* (NM-11 / carry unboundedness, tautological, dead end). <span class="sans-serif">\[Lean\]</span> , , . **Yields:** for every infinite $`A`$ and positive carry recurrence $`u(n{+}1)+v\cdot\mathrm{supportCoeff}\,A\,(c{+}n{+}1)=2u(n)`$: $`u`$ is unbounded, and $`1+v|F| \le
2u(L{-}c{-}1)`$ for every finite $`F\subseteq A`$ with all elements dividing $`L`$. Stated for the general denominator $`2^c v`$, not just dyadic.

**Mismatch (`multiple`):** traced to a dead end. The proof picks $`2B{+}1`$ elements of $`A`$, sets $`L=(c{+}1)\prod F`$, forcing $`\mathrm{supportCoeff}\,A\,L\ge|F|`$; then $`u(L{-}c{-}1)=v\cdot\mathrm{binaryCoeffTail}(\mathrm{supportCoeff}\,A)(L{-}1)\ge v|F|/2`$, so $`1+v|F|
\le 2u`$ holds with *no slack*. Unboundedness of the carry is a tautological restatement of “supportCoeff is unbounded on common multiples,” not a rigidity. The linear tail bound cannot rescue it either: $`L`$ is exponential in $`|F|`$ by construction, so $`N{+}2`$ is astronomically larger than $`|F|`$.

**Closes it:** a common-multiple selection with divisor economy, an infinite $`A`$ must admit $`L`$ with $`|\{a\in A: a\mid L\}| > 2\,\mathrm{binaryCoeffTail}(\mathrm{supportCoeff}\,A)(L{-}1) +
...
*Remark 252* (hbound is equivalent to the conclusion, greedy support only). and its packaged form take a hypothesis `hbound`: $`\forall N,\ \mathrm{mobiusCenteredHalfCarry}\,A\,N \le
2\sqrt N + 4`$. For the canonical greedy support $`A=\mathrm{greedyMersenneSupport}(1/2)`$ specifically, `hbound` is logically *equivalent* to the theorem’s own conclusion $`\mathrm{erdosSupportSeries}\,2\,A=1/2`$, not a strictly weaker sufficient condition. This follows from the exact residual identity at line 842, the unconditional sqrt-envelope tail bound (), the unconditional tail-nonnegativity lemma (), and the greedy-specific fact $`\mathrm{erdosSupportSeries}\,2\,A\le1/2`$ (line 859). **The scope limit is exact and must not be widened by inference:** this equivalence does *not* extend to every $`A`$ with $`1\notin
A`$: dropping the greedy-specific $`\le1/2`$ fact yields only $`\mathrm{hbound}\Leftrightarrow
\delta\le0`$ (a one-sided condition), not $`\delta=0`$. It also does *not* extend to the sibling socket : `hbound` is stated in `Real.sqrt`, while `CofinalStripReturn`’s `halfStripBound` is built on `Nat.sqrt`, and bridging the real bound down to the tighter natural-number floor bound needed for a cofinal witness is an unaddressed, nontrivial gap, not routine packaging.

</div>

<div class="rem">

*Remark 253* (HalfCarryCofinalTerminalOnlyStrip, widened to constant 6, still equivalent). , even after widening the strip constant from 4 to 6 ($`\mathrm{halfStripBound}'\,n = 2\,\mathrm{Nat.sqrt}\,n+6`$), is logically equivalent to $`1/2\in\mathrm{mersenneAchievementSet}`$ once three small, non-circular, mechanical lemmas are supplied (a general one-not-in-support corollary, the discrete envelope inequality $`\mathrm{Real.sqrt}\,n \le \mathrm{Nat.sqrt}\,n+1`$, and a routine prefix-to-`HalfWord` construction). None of the three gaps is equivalent to \#257 or \#249 individually: the equivalence sits one level up, at the socket itself. **This is a diagnostic, not a route**: it corrects TerminalOnlyCofinal.lean’s own "weaker producer" framing: the terminal-only cofinal strip carries no strategic advantage over proving achievement directly.

</div>

<div class="rem">

*Remark 254* (CofinalPositiveHalfGreedySkips is literally the problem). Repeated here for emphasis from the 257-cofinal-rows entry above: once the positivity conjunct of is recognised as unconditionally free (odd-numerator/even-subtrahend parity plus greedy nonnegativity, discharged at ), the socket collapses to $`(\mathrm{greedyMersenneSkippedSupport}(1/2))`$.Infinite, proved by A6 () to be *equivalent* to $`1/2\in
...
*Observation 256* (Prime-doubling: closed by explicit counterexample). **Route as conceived.** The universal recursion $`K(M+1) = 2K(M) - (\tau(M+1)-1)`$ (Part I, SE-1) doubles at every step and subtracts the divisor-excess $`\tau(M+1)-1`$. At a prime $`M+1=p`$, $`\tau(p)=2`$ so the subtracted term is $`1`$; the naive hope is that primality of an index forces the step to be "almost pure doubling," and that prime density (Chebyshev/PNT-scale) then bounds how long a run of near-pure-doubling steps can last, which would bound the danger run-length $`L_r`$ in SE-3’s equivalence (III).
...
*Observation 257* (Parity/perfect-squares: closed as a tautology of the recursion shape). **Route as conceived.** $`K(M)`$ is even exactly when $`M`$ is a perfect square (a true, provable fact). The hope was to use this parity law to forbid certain residues or run patterns in the digit stream of $`K`$, since parity constraints are a classical first attack on digit-run questions.
...
```
where we write $`C_s := 4\cdot\mathrm{rem}(s) - \mathrm{belowPulse}(s) - 4`$ for the integer coordinate that appears on the middle branch of the seam recursion (the same quantity that, along an all-right run, is the affine excess $`\mathrm{rem}(s)-2^s`$ one step later; Part I, RC-2).

<div id="prop:upper-unconditional" class="prop">

**Proposition 260** (The upper (carries) successor needs no exceptional-cell exclusion). *On the carries branch of the step (i.e. $`(\mathrm{seamAdjacentCut}\ s\ hs).\mathrm{successorCarries}`$ holds), the two-sided invariant propagates *unconditionally*, with zero exceptional cells and no hypothesis beyond `hcarry` itself:
``` math
\mathrm{successorCarries} \implies \mathrm{rem}(s+1) \le 2^{s+1}.
```
(from , closed by `omega`). In this precise sense (no cell exclusion is ever needed on this branch of the step) the upper successor contributes nothing to the exceptional-cell inventory: it is "impossible" for the upper branch to be a source of an exceptional cell in the induction step, because the branch is handled outright. `coord:seam-branch-classification` **Object or representation:** about a *representation* (the induction-step case split), not the orbit itself. <span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>*

</div>

<div id="thm:cd-neg3-impossible" class="thm">

**Theorem 261** ($`C_D=-3`$ is impossible at the final middle producer, $`D\ge 13`$). *Consider a hypothetical *final middle producer*: a middle transition at row $`D\ge 13`$ ($`\lnot\mathrm{carries}`$, and the middle-branch inequality $`4\mathrm{rem}(D)+\mathrm{gap}-\mathrm{belowPulse} < \mathrm{terminalWeight}`$ holds at $`D`$), followed by an all-right tail forever after ($`\forall s\ge D{+}1`$, $`\mathrm{seamGreedyWord}(s+1) =
...
The digit-carry route is not merely unclimbed; a family of general-purpose no-go and countermodel results independently constrains *what kind of proof* can ever close it. These constrain which proof architectures are structurally impossible, for any input. They say nothing about the wall’s numerics.
...
**Proposition 265** (Möbius-support countermodel: the natural negative-sign candidate overshoots). *The signed Lambert identity $`\sum_{d\ge1}\mu(d)/(2^d-1) = 1/2`$ is exact. Writing $`N:=\{d:\mu(d)=-1\}`$: $`\sum_{d\in N}1/(2^d-1) = 1/2 + \sum_{d\in P}1/(2^d-1)`$ where $`P:=\{d\ge2:
...
**Lemma 268** (Denominator complement survives scaling). *For $`x:\mathbb Q`$, $`c:\mathbb Z`$, $`H:\mathbb N`$: if $`H\mid x.\mathrm{den}`$ and $`(c\cdot x).\mathrm{den}\mid H`$ (i.e. multiplying by the integer $`c`$ shrinks the displayed denominator down *into* $`H`$), then the **complementary** denominator factor $`x.\mathrm{den}/H`$ divides $`c`$:
``` math
H\mid x.\mathrm{den} \ \land\ (c\cdot x).\mathrm{den}\mid H \implies x.\mathrm{den}/H \mid |c|.
```
. Equality form (height conservation): $`\exists\,t:\mathbb Z,\ H\cdot c\cdot x = t\cdot x.\mathrm{num}`$. The omitted denominator is transferred exactly to the coefficient $`t`$, never erased. .*

***Scope.** Fully generic $`\mathbb Q`$-arithmetic, no reference to either Erdős problem. It is the reusable "clearing a denominator by multiplying by a small integer $`c`$ cannot discard the complementary denominator factor; it only moves that factor into $`c`$" lemma. It gives a hard floor on how small $`c`$ can be if it is meant to kill a target denominator factor $`x.\mathrm{den}/H`$: any argument on either problem that hopes to "clear a denominator by a bounded multiplier" is bounded below by this lemma. `coord:rational-denominator/height` **Object or representation:** about a *representation*, pure denominator bookkeeping for $`\mathbb Q`$, with zero problem-specific content; the upstream primitive () is the domain-neutral extraction this lemma builds on. <span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>*

</div>

<div id="cor:mersenne-height" class="cor">

**Corollary 269** (Mersenne specialisation). *For positive $`x:\mathbb Q`$: if $`2^r\mid x.\mathrm{num}.\mathrm{natAbs}`$ and $`x < 2/(2^n-1)`$, then $`2^r\cdot(2^n-1) < 2\cdot x.\mathrm{den}`$: a numerator $`2`$-power lower bound plus a Mersenne-scale upper bound on $`x`$ together force a denominator lower bound, *without* introducing a global prefix LCM. (generic form ). A ready-made denominator-lower-bound tool for "synchronized tail" arguments on \#257’s Mersenne sums; composes directly with Lemma <a href="#lem:scalar-localization" data-reference-type="ref" data-reference="lem:scalar-localization">268</a> for a full numerator/denominator height-transport toolkit. `coord:rational-denominator/height, Mersenne instance` <span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>*
...
**Definition 270** (Upper-reset dyadic band escape).
``` math
...
```
. In words: at every upper-reset row $`d\ge13`$, the reset charge must avoid a linear-width forbidden band immediately below every dyadic threshold $`2^{d-j+1}`$, for every $`j\le d`$ simultaneously. Granted this socket, gives $`1/2\in\mathcal A`$ outright.

</div>

<div id="prop:critical-band-index" class="prop">

**Proposition 271** (Quantifier collapse: one critical index suffices, not $`d{+}1`$). *The abstract, purely combinatorial statement $`\mathsf{DyadicBandEscape}(d,E) \iff \exists j,\ \mathsf{CriticalDyadicBandIndex}(d,E,j)\land
E+2(d+j)\le 2^{d-j+1}`$ collapses the $`\forall j\in[0,d]`$ band-avoidance condition (formally $`d{+}1`$ separate inequalities) to checking exactly *one* nearest-boundary index $`j`$. Specialised to the concrete seam reset charge, $`\mathsf{SeamUpperResetCriticalBandEscape}`$ is proved logically *equivalent* to Definition <a href="#defn:band-escape" data-reference-type="ref" data-reference="defn:band-escape">270</a>’s socket. (seam specialisation ). Zero Mersenne/seam content in the core lemma, pure $`(d,E,j)`$ arithmetic over powers of 2. `coord:dyadic-boundary, generic` <span class="sans-serif">\[Lean\]</span><span class="sans-serif">scale:n/a</span>*

</div>

<div id="cert:band-through-31" class="obs">

*Observation 272* (Rows 13 through 31 are unconditionally certified). The linked certificate verifies Definition <a href="#defn:band-escape" data-reference-type="ref" data-reference="defn:band-escape">270</a> by kernel computation for every row $`13\le s\le31`$. For example, $`\mathrm{rem}(14)=392`$ and $`\mathrm{rem}(31)=4{,}187{,}487{,}147`$. No row in this interval fails the band-avoidance test. . <span class="sans-serif">\[Cert\]</span><span class="sans-serif">scale:bounded</span>

</div>

<div id="rem:band-caveat" class="rem">

*Remark 273* (What this does *not* show: read the quantifiers exactly). Definition <a href="#defn:band-escape" data-reference-type="ref" data-reference="defn:band-escape">270</a> is a $`\forall d\ge13`$ statement; Certificate <a href="#cert:band-through-31" data-reference-type="ref" data-reference="cert:band-through-31">272</a> is a finite, kernel-checked verification for $`13\le d\le31`$. **The finite certificate does not establish the universal socket.** Nothing in the tree proves $`\mathsf{SeamUpperResetDyadicBandEscape}`$ for all $`d\ge13`$. This is exactly the open producer named in Part I (SE-9) and Part III (RC-4/D1’s <span class="sans-serif">LargestSkipLateStepSocket</span>), stated here in its own coordinate.

The certified range through row 31 is real, certified evidence *at that finite range*, and it is a genuinely different fact from "the actual orbit avoids the unsafe band cofinally" or "at every row." The socket remains an unproved sufficient condition; the finite computation is not a proof of the cofinal or universal claim, and must never be quoted as one.

Concretely: the certificate says the band is avoided for $`1264`$ real resets through row $`2500`$ with zero classification anomalies (Part I, RC-7) and the sign law holds at all of them (RC-8). This is strong disconfirming-of-a-counterexample evidence, not a proof that the pattern continues past the certified range. <span class="sans-serif">\[Open\]</span><span class="sans-serif">scale:cofinal</span>
...
**Definition 274** (The short-window divisor phase). For $`M\ge 1`$ and $`L\ge 1`$ put
``` math
...
```
(), with the closed consumer , <span class="sans-serif">\[Lean\]</span>. The $`\forall j\le d`$ band check collapses to a single nearest-boundary index per row by , and rows $`13\le d\le 30`$ are kernel-verified by .
...
**Lemma 275** (Divisor-residue form of the short-window phase). *For all $`M,L\ge 1`$,
``` math
...
Lemma <a href="#lem:odometer" data-reference-type="ref" data-reference="lem:odometer">275</a> says the object is the evaluation of a *fixed* function on the profinite integers along the $`+1`$-orbit. Define
``` math
...
At $`H=\log M`$ the dual sum is of length $`M/\log M`$ and there is no saving at all: the method returns the trivial bound. This is not a deficiency of one paper; it is the reason short-interval divisor problems are stated with power-length windows. Technique family (a) is therefore out of range by an *exponential* amount in the window length, and not merely unproved here, and no quantitative improvement in Kloosterman bounds changes that, because the obstruction is the length of the dual sum, not the quality of its estimate.
...
The observed median of $`\mathrm{dist}`$ is $`0.24471`$ against $`0.25`$ for the uniform law, and $`P/t`$ hovers near $`2`$, the exact value for a uniform fractional part. Truncation is irrelevant at this range: the quantiles at $`L=16,24,32`$ agree to three significant figures.

So the fractional part of $`\Theta_L`$ behaves like Haar measure with a mildly heavy near-integer tail of constant $`c\approx 2.5`$. Feeding the required precision $`2^{-(r-5)/2}`$ into that profile,
``` math
...
The reciprocal-summable Lean theorem does not recover it because $`\sum_p1/p`$ diverges. The older pairwise-coprime theorem’s generic block-certificate engine would also demand, in the prime case,
``` math
...
```
where $`G:=\mathtt{greedyMersenneSupport}(1/2)`$. The forward directions of (2)–(5) are landed Lean (, , , , all <span class="sans-serif">\[Lean\]</span>). What is missing is the reverse directions, and they all run through one lemma.
...
**Lemma 276** (Perfect-square strip witness). *Let $`A\subseteq\mathbb{N}`$ with $`1\notin A`$ and $`\mathtt{erdosSupportSeries}\ 2\ A=1/2`$. Then for every $`k\ge1`$,
``` math
...
```
and the first term vanishes by hypothesis. Take $`N=k^{2}-1`$, so $`N+1=k^{2}`$. By (<span class="sans-serif">\[Lean\]</span>, unconditional) the tail at $`k^2`$ is at most $`2\sqrt{k^{2}}+4=2k+4`$, and by , $`\mathtt{halfStripBound}\ n
...
$`\mathtt{supportCoeff}\,A = \mathbf{1}_A * \mathbf{1}`$ is a Dirichlet convolution, so the dense branch asks: for an *arbitrary* $`0`$–$`1`$ arithmetic function, find a block of consecutive arguments on which the convolution with $`\mathbf{1}`$ is $`2`$-adically aligned. Möbius inversion $`\mathbf{1}_A = \mu * \mathtt{supportCoeff}\,A`$ is exact and costs nothing, which makes the $`2`$-adic structure of the incidence directly readable, and that is the technique family to use: $`2`$-adic Möbius/Dirichlet algebra is one possible source of compatible incidence prescriptions. Shared factors impose compatibility conditions on CRT data, which an application must verify. This is a requirement of that construction. The small-shift results in §<a href="#sec:257-weighted-returns" data-reference-type="ref" data-reference="sec:257-weighted-returns">1.4</a> show why neither divisor independence nor ordinary reciprocal summability should be imposed on every irrationality argument.
...
**Proposition 277** (Squarefree support: exact engine ceiling, not an open value). *Let $`A=\{n\ge2:n\text{ is squarefree}\}`$. Then
``` math
...
```
are linearly independent over $`\mathbb{Q}`$. After deleting the rational $`n=1`$ term, squarefree support is therefore irrational at every base $`2^j`$. Among the integer bases, the cited theorem leaves the non-powers of two uncovered. This is cited mathematics, not formalised here. Proposition <a href="#prop:squarefree" data-reference-type="ref" data-reference="prop:squarefree">277</a> is a failure of two proof schemas on a value already known to be irrational.
...
Proposition <a href="#prop:squarefree" data-reference-type="ref" data-reference="prop:squarefree">277</a> is a reason to separate the two halves rather than assimilate them. On the universal side the present certificate engine first meets a coordinate-dependent parity obstruction. The squarefree value is nevertheless already known to be irrational at power-of-two bases, and adjoining $`1`$ removes the opening-block obstruction without changing irrationality. Thus the no-go diagnoses this engine; it does not diagnose the value or the universal problem.
...
**Reader position.** 5 of 93; display band: front door. Formal editorial disposition: promote. These are separate classifications.
...
**Exact boundary.** Infinitude and summability of reciprocal conductor mass are explicit. No coprimality is assumed. Supports with divergent reciprocal mass require another argument; universal Erdős \#257 remains open.
...
**Reader position.** 10 of 93; display band: front door. Formal editorial disposition: split. These are separate classifications.
...
**Reader position.** 16 of 93; display band: major result. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 18 of 93; display band: major result. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 20 of 93; display band: major result. Formal editorial disposition: hold. These are separate classifications.
...
<a id="divisibility-weighted-support-irrationality"></a>

## Divisibility weighted support irrationality

**Reader position.** 32 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Finite divisibility-weighted mass implies hereditary all-base irrationality, including explicit reciprocal-divergent short-gap supports.
...
**Authority and reach.** Ordinary full proof; Lean checks the finite orbit and dyadic estimates, full infinite-support dyadic error summability and the radix close-return endgame.; reviewed ordinary theorem or obstruction; novelty unassessed.

**Exact boundary.** The complete divisibility-weighted theorem is proved in docs/research/erdos257-eight-return-proofs.md. The finite orbit and infinite-support dyadic error control are formalized, but the prime-part split and diagonal scale producer remain ordinary proof steps. No full-theorem Lean badge or universal \#257 solution follows from the checked ingredients.
...
- `ErdosProblems.Erdos257.cesaroMean_shiftedMersenneAtom_mul_le_gcdMean_add_error`

- `ErdosProblems.Erdos257.dyadic_observation_sum_le`

- `ErdosProblems.Erdos257.dyadic_supportObservationMass_sum_le`

- `ErdosProblems.Erdos257.summable_dyadic_supportObservationMass`

- `ErdosProblems.Erdos257.tendsto_dyadic_supportObservationMass_mean`

- `ErdosProblems.Erdos257.irrational_erdosSupportSeries_of_radix_closeReturn`

<a id="sub-logstar-reciprocal-mass-irrationality"></a>

## Sub logstar reciprocal mass irrationality

**Reader position.** 33 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Rational p/q forces liminf H_A(x)/ell(x) at least lambda_b(p/q)/2; sub-log-star mass implies hereditary all-base irrationality.

Rational p/q forces liminf H_A(x)/ell(x) at least lambda_b(p/q)/2; sub-log-star mass implies hereditary all-base irrationality.

**Authority and reach.** Ordinary full proof; finite portions independently checked by exact arithmetic. The new reduction module is not part of this paper’s published formal-source cut.; reviewed ordinary theorem or obstruction; novelty unassessed.

**Exact boundary.** Complete proofs: docs/research/erdos257-eight-return-proofs.md; composed arguments: docs/research/erdos257-eight-return-synthesis.md. No complete Lean formalization, external acceptance, rational-target membership or universal \#257 solution is asserted.
...
<a id="arbitrary-prime-power-support-irrationality"></a>

## Arbitrary prime power support irrationality

**Reader position.** 34 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Every infinite subset of the prime powers, with fixed dilations and finite modifications, has irrational sum at every integer base, using the reviewed analytic correlation input.
...
<a id="variable-exponent-fractional-cover-irrationality"></a>

## Variable exponent fractional cover irrationality

**Reader position.** 35 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** For finite positive divisor frames F_j and 0\<alpha_j\<=1, summability of C_j 2^(j alpha_j) B_j/(B_j-1)^2 with B_j=2^alpha_j gives hereditary all-base irrationality; fixed-alpha positive covers are included by regrouping.

For finite positive divisor frames F_j and 0\<alpha_j\<=1, summability of C_j 2^(j alpha_j) B_j/(B_j-1)^2 with B_j=2^alpha_j gives hereditary all-base irrationality; fixed-alpha positive covers are included by regrouping.
...
<a id="prime-cofactor-repair-obstruction"></a>

## Prime cofactor repair obstruction

**Reader position.** 38 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Actual modulus-420 tetraprime repair fails infinitely often; an exact 80-anchor prefix gives prime-cofactor jumps at least 7 for multiplier 120 and at least 2 for multiplier 420. General infinite admissible supports defeat every finite fixed multiplier menu.
...
<a id="prime-repair-finite-support-rigidity"></a>

## Prime repair finite support rigidity

**Reader position.** 39 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Bounded prime jumps characterize finite greedy support; eventual repair at every prime characterizes zero or a single Mersenne atom for targets below one.
...
**Reader position.** 51 of 93; display band: mechanism. Formal editorial disposition: split. These are separate classifications.
...
**Reader position.** 53 of 93; display band: mechanism. Formal editorial disposition: merge. These are separate classifications.
...
**Reader position.** 55 of 93; display band: mechanism. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 57 of 93; display band: mechanism. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 59 of 93; display band: mechanism. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 77 of 93; display band: frontier. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 78 of 93; display band: frontier. Formal editorial disposition: hold. These are separate classifications.
...
**Result-atom population.** 11 of 704 public coordinates. The public atom catalog groups them under this family.
...
**Reader position.** 79 of 93; display band: frontier. Formal editorial disposition: hold. These are separate classifications.

**Reader entry.** Every finite-core orthogonal-petal bouquet has irrational reciprocal-power value at every integer base at least two. Its defining summable petal reciprocals bound the actual support reciprocals, so no forced-slot selector is needed.
...
**Authority and reach.** Lean kernel; stronger Comparator interface configured, runtime receipt separate; unconditional all-base support-class corollary.

**Exact boundary.** The bouquet structure, positive finite cores, pairwise-coprime petals and summable petal reciprocals remain explicit. The former selector belongs to a stronger certificate construction and is unnecessary for this corollary. This family is contained in reciprocal-summable supports and supplies no reciprocal-divergent or universal endpoint.
...
- `Erdos249257.SupportSunflowerDichotomy.OrthogonalPetalBouquet.summable_reciprocalSupportTerm`
...
<a id="four-ninths-square-root-repair-windows"></a>

## Four ninths square root repair windows

**Reader position.** 80 of 93; display band: frontier. Formal editorial disposition: retain. These are separate classifications.

**Reader entry.** For every real x\>=0, let Q_N=floor(2^N x) minus the Lambert prefix numerator of the actual greedy support. Membership is equivalent to a nonincrease of Q in every \[K,K+2 floor(sqrt K)+12), to cofinal nonincreases, and to cofinal bounds by the next divisor load or by N+1. One strict-increase window excludes membership; 4/9 is a specialization.

For every real x\>=0, let Q_N=floor(2^N x) minus the Lambert prefix numerator of the actual greedy support. Membership is equivalent to a nonincrease of Q in every \[K,K+2 floor(sqrt K)+12), to cofinal nonincreases, and to cofinal bounds by the next divisor load or by N+1. One strict-increase window excludes membership; 4/9 is a specialization.

**Authority and reach.** Lean kernel; generic Comparator interface configured, runtime receipt separate; exact actual-real-target equivalences and finite nonmembership criterion.

**Exact boundary.** Rationality and periodic floor digits are not hypotheses. The theorem does not construct cofinal repairs or a strict window for 4/9,1/21 or1/2, and none of those open membership questions is decided. The stable historical family id is retained to preserve existing links.
...
- `ErdosProblems.Erdos257.mem_iff_greedyBinaryDefect_sqrt_windows`

- `ErdosProblems.Erdos257.mem_iff_greedyBinaryDefect_cofinal_repairs`

- `ErdosProblems.Erdos257.mem_iff_greedyBinaryDefect_cofinal_load_bound`

- `ErdosProblems.Erdos257.mem_iff_greedyBinaryDefect_cofinal_linear_bound`

- `ErdosProblems.Erdos257.not_mem_of_greedyBinaryDefect_strict_sqrt_window`

<a id="twenty-one-two-anchor-divergence-density"></a>

## Twenty one two anchor divergence density

**Reader position.** 81 of 93; display band: frontier. Formal editorial disposition: retain. These are separate classifications.

**Reader entry.** For the 1/21 greedy support, membership is equivalent to divergent reciprocal mass on the favorable selected rows. Two anchors give 60/105 favorable classes and conditional upper density \>= 2/7; twelve anchors give a larger restricted set with 1011/1260 favorable classes and conditional upper density \>= 337/840 \> 2/5. Each density refers to its own restricted set; no membership or lower density is established.
...
**Reader position.** 88 of 93; display band: technical support. Formal editorial disposition: hold. These are separate classifications.
...
**Reader position.** 89 of 93; display band: technical support. Formal editorial disposition: hold. These are separate classifications.
...
**Definition 279** (Half membership).
``` math
...
$`\mathrm{HALF}`$ is a *single instance* of the negation of $`\mathrm{U257}`$: any witness $`A`$ with $`0\notin A`$, $`\mathtt{erdosSupportSeries}\ 2\ A=1/2`$ is infinite (, <span class="sans-serif">\[Lean\]</span>, <span class="sans-serif">scale:uniform</span> — finite supports have odd-denominator value, so can never equal a dyadic rational), hence a genuine counterexample to $`\mathrm{U257}`$. So $`\mathrm{HALF}\Rightarrow\neg\mathrm{U257}`$, strictly — it does not decide $`\mathrm{U257}`$ if false, since $`\mathrm{U257}`$ could still fail at a different target $`t\in\mathbb{Q}`$. $`\mathrm{HALF}`$ is <span class="sans-serif">\[Open\]</span>. It is logically equivalent, by an unconditional Lean iff, to a purely combinatorial statement about one fixed orbit:
``` math
\mathrm{HALF}\ \Longleftrightarrow\ \left(\mathtt{greedyMersenneSkippedSupport}(1/2)\right).\mathrm{Infinite}
```
(, <span class="sans-serif">\[Lean\]</span>, <span class="sans-serif">scale:uniform</span>, coordinate `coord:mobius-mersenne`, target-generic in the reverse direction — the forward direction uses only that the target is rational, so the same theorem holds verbatim for every rational $`t\ge 0`$ in place of $`1/2`$).
...
**Proposition 280** (CofinalPositiveHalfGreedySkips is equivalent to HALF). *Define
``` math
...
<span class="sans-serif">\[Math, adversarially verified\]</span> (Survivor 7, composition-survivor pass). <span class="sans-serif">scale:cofinal</span>. `coord:mobius-mersenne, greedy orbit`. This closes off what the source bank advertised as "the shortest known path" to a disproof (): there is no shortcut through it, because it is not shorter.

<div id="prop:strip-equiv" class="prop">

**Proposition 281** (The terminal-only cofinal strip is equivalent to HALF). *Let $`\mathtt{HalfCarryCofinalTerminalOnlyStrip}`$ be the socket that a finite normalised word $`a`$ at cofinally many depths $`M`$ satisfies $`|\mathtt{integerHalfCarry}(\mathtt{wordSupport}\,a)(M-1)|
...
<span class="sans-serif">\[Math, adversarially verified, weakened\]</span> (Survivor 5). <span class="sans-serif">scale:cofinal</span>. `coord:support
rigidity, half-carry`. This corrects the module’s own "weaker producer" framing — the strip socket carries no strategic advantage over proving $`\mathrm{HALF}`$ outright.

<div class="obs">

*Observation 282*. Neither Prop. <a href="#prop:cpgs-equiv" data-reference-type="ref" data-reference="prop:cpgs-equiv">280</a> nor Prop. <a href="#prop:strip-equiv" data-reference-type="ref" data-reference="prop:strip-equiv">281</a> narrows the search space for $`\mathrm{HALF}`$. Their value is negative: they retire two named targets from the "worth chasing as an easier waystation" list. A third socket, the hypothesis $`\forall N,\ \mathtt{mobiusCenteredHalfCarry}\,A\,N\le 2\sqrt N+4`$ of , is likewise proved EQUIVALENT to $`\mathrm{HALF}`$ for the specific support $`A=G`$ (Survivor 4, <span class="sans-serif">\[Math, adversarially verified, weakened\]</span>) — not, as originally claimed, for every $`A`$ with $`1\notin A`$; the general-$`A`$ version is only a one-sided implication ($`\mathrm{hbound}\Rightarrow
...
```
(, <span class="sans-serif">\[Open\]</span>, <span class="sans-serif">scale:cofinal</span>). No compatibility is required between witness supports at different endpoints — the defining weakening versus a single coherent trajectory.
...
```
(). For $`4\le c`$, every $`d\in D`$ with $`2\le d<c`$, and $`\mathtt{localMersennePrefixValue}\,D<1/2`$:
``` math
\begin{align}
...
\end{align}
```
$`(\ast)`$ is proved unconditionally, uniform in $`c`$, no case split (, <span class="sans-serif">\[Lean\]</span>, <span class="sans-serif">scale:uniform</span>).

$`(\ast\ast)`$ is exactly one bit sharper and, via (an unconditional iff), is *equivalent* to an exact row already crossing the integer half-target at $`c`$. The true unproved gap between $`(\ast)`$ and $`(\ast\ast)`$ is additive, not multiplicative: the proof of $`(\ast)`$ derives $`\mathtt{localBinarySuffix}<2^{c-2}+|D|`$ and discards $`|D|\le c-2`$; so what is actually open is the linear-width anti-concentration
``` math
...
The generic (non-greedy) sharp-fill theorem itself is already unconditional and strictly more general than every consumer built on it: (<span class="sans-serif">\[Lean\]</span>) needs only $`(\ast\ast)`$ for *some* below-half core $`D`$, not a greedy prefix — this is the escape route named in Prop. <a href="#prop:cpgs-equiv" data-reference-type="ref" data-reference="prop:cpgs-equiv">280</a>’s remark: abandoning the canonical greedy orbit (which forces $`\mathrm{CPGS}`$-equivalence) for an arbitrary below-half core is a genuinely different, still-open target.
...
```
(), at essentially every step (<span class="sans-serif">\[Open\]</span>, <span class="sans-serif">scale:cofinal</span>). This is a strictly harder producer than (O3-supply) because it is pinned to the greedy prefix rather than an arbitrary core.
...
**Definition 284** (Reset $`\sqrt{\text{-escape}}`$ from row 10).
``` math
...
<span class="sans-serif">\[Math, fleet-audited\]</span>, coordinate `coord:deviation-affine / crossing-cell geometry`. The exponent $`(r+5)/2`$ is *derived*, not fitted: a violation forces an R-branch crossing cell $`(s,d)`$ with $`s\ge14`$, late row $`d\ge10`$ with run length $`L=(d-3)/2`$ or $`(d-4)/2\ge3`$; dividing the exact affine iterate $`w_{d+1+L}=4^Lw_{d+1}-C_L`$ by $`4^L`$ and absorbing the charge bound $`1\le C_L/4^L<(2s+4)/3`$ yields the exact exponent. This target is *equivalent*, by a Theorem-A composition already Lean-landed on the consumer end, to
``` math
\mathrm{SQRTESC}\ \Longrightarrow\ \mathrm{LargestSkipLateStepSocket}\ \Longrightarrow\ \mathrm{HALF}
```
(, <span class="sans-serif">\[Lean\]</span> for the second arrow, <span class="sans-serif">\[Math, fleet-audited\]</span> for the first). $`\mathrm{SQRTESC}`$ is equivalently stated as the R-run length bound
``` math
...
| CPGS (Prop. <a href="#prop:cpgs-equiv" data-reference-type="ref" data-reference="prop:cpgs-equiv">280</a>) | $`\Leftrightarrow`$ O2 | <span class="sans-serif">\[Open\]</span>, no shortcut |
| Terminal-only strip (Prop. <a href="#prop:strip-equiv" data-reference-type="ref" data-reference="prop:strip-equiv">281</a>) | $`\Leftrightarrow`$ O2 | <span class="sans-serif">\[Open\]</span>, no shortcut |
...
**O1 (universal).** The reciprocal-summable regime is settled, so a direct certificate route need only treat arbitrary infinite $`A`$ with divergent reciprocal mass. A producer for $`\mathrm{Cert}(A)`$ in that remaining regime is a statement about geometrically-weighted divisor sums achieving controlled residues modulo powers of two on prescribed dyadic blocks, uniformly over those supports. No result of this shape is identified in the literature cited here. The corpus therefore records a missing near-integer anti-concentration theorem for a divisor-weighted sum, without making a novelty or priority claim.
...
The linked Lean declarations, fixed toolchain, and library manifest are published in this repository. This manuscript provides navigation and exposition rather than proof authority.
