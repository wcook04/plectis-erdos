...
> **Contribution.** The paper gives an explicit basis and relation normal form for the dyadic totient kernel, exposes the squared-Lambert first-gcd-moment identity, proves level rank $`2^e+1`$, derives the all-base rank $`k^e+1`$ from Martin plus explicit reductions, and checks a finite denominator exclusion together with its first failing certificate scale.
>
