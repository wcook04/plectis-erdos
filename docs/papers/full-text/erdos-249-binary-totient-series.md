...
For every integer $`k\ge2`$ and every $`e\ge1`$, the sections of Euler’s totient through level $`e`$ have rational rank $`k^e+1`$. The zero-residue sections and the residues not divisible by $`k`$ form a basis. Explicit scalar reductions give unique integral coordinates and a basis of all integral relations. The arithmetic step is independence: divisibility makes an evaluation matrix diagonal modulo a prime, with nonzero diagonal entries. This CRT–Dirichlet argument proves the all-base theorem ordinarily in the note. Lean also checks canonical-family independence and the complete all-base rank unconditionally. Martin’s theorem supplies a broader external affine-independence result, while Coons proved non-$`k`$-regularity of $`\varphi`$; the exact ranks recover that theorem and quantify it level by level. A separate ordinary argument uses isolated blocks to prove that $`\sum_{n\ge1}(\varphi(n)\bmod m)2^{-n}`$ is irrational for every $`m\ge3`$, with a complete rationality classification for dyadic residue observables. For $`S=\sum_{n\ge1}\varphi(n)2^{-n}`$, one nonintegral tail shift forces full-depth certificates in every sufficiently late pair of multipliers. If $`S`$ were rational, one tempered integral carry would simultaneously have section rank at least $`2^e-1`$ at every depth and one common eventual period modulo a fixed multiplier. These necessary conditions do not decide $`S`$: its irrationality is equivalent to an explicit central-residue gap for every prospective denominator, whose arithmetic supply remains open.

<a id="sec:results"></a>

# A basis and all its relations

Set $`\varphi(0)=0`$. For $`k\ge2`$, the $`k`$-kernel consists of the sequences $`n\mapsto\varphi(k^jn+r)`$ with $`j\ge0`$ and $`0\le r<k^j`$. Coons proved that $`\varphi`$ is not $`k`$-regular for any $`k\ge2`$ \[coons, Thm. 3.2\], and Martin’s Theorem 1 implies that the functions $`n\mapsto\varphi(a_in+b_i)`$ are linearly independent along pairwise nonproportional affine forms with integer coefficients and positive slopes \[martin-phi-inequalities, Thm. 1\]. The following theorem gives the exact rank and a unique coordinate system at every finite level; Section <a href="#sec:rank" data-reference-type="ref" data-reference="sec:rank">1.1</a> proves its independence step directly.
...
*Proof.* Every finite subfamily lies in a sufficiently high truncation. The theorem gives its independence; the reductions give equality of the full spans. The level-zero clause is ordinary in this note; `PaperCompleteR7/ArithmeticAssemblies.lean` is not in this checkout. ◻
...
*Proof.* Choose a common period $`Q`$. On each progression $`n=a+Qt`$, the coefficients are constant and the cross determinants become $`Q(a_ib_j-a_jb_i)\ne0`$. Shift beyond the threshold and apply the affine independence just proved. The statement is ordinary in this note; `PaperCompleteR7/PeriodicAndPulse.lean` is not in this checkout. ◻
