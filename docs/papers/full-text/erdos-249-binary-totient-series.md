...
For every $`e\ge1`$, the dyadic sections of Euler’s totient through level $`e`$ have rational rank $`2^e+1`$. The two zero-residue sections and the odd-residue sections form a basis. Explicit scalar reductions give unique integral coordinates and a basis of all integral relations. The arithmetic step is independence: divisibility makes an evaluation matrix diagonal modulo a prime, with nonzero diagonal entries. The [dyadic rank $`2^e+1`$](https://github.com/wcook04/plectis-lean-erdos249-257/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientMahlerDefect.lean#L1084) holds for $`e\ge1`$. The all-base rank $`k^e+1`$ is the [rank transferred from linear independence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientKernelConditional.lean#L215), using Martin’s external independence; Coons proved non-$`k`$-regularity of $`\varphi`$. A separate ordinary argument uses isolated blocks to prove that $`\sum_{n\ge1}(\varphi(n)\bmod m)2^{-n}`$ is irrational for every $`m\ge3`$, with a complete rationality classification for dyadic residue observables. For $`S=\sum_{n\ge1}\varphi(n)2^{-n}`$, one nonintegral tail shift forces full-depth certificates in every sufficiently late pair of multipliers. The rank and bounded-residue results do not decide $`S`$: its irrationality is equivalent to an explicit central-residue gap for every prospective denominator, whose arithmetic supply remains open.
