...
For integers $`k\ge2`$ and $`e\ge1`$, we give a basis of the totient sections $`n\mapsto\varphi(k^jn+r)`$ through level $`e`$. Their rational rank is $`k^e+1`$. Explicit scalar reductions give unique integral coordinates and a basis of all integral relations. The independence argument uses CRT–Dirichlet separation, an earlier method of Yazdani credited to Shallit; Martin’s theorem already implies the requisite affine independence, and Coons previously proved non-$`k`$-regularity. The contribution here is the finite-level normal form, not those qualitative obstructions. We also prove irrationality of $`\sum_{n\ge1}(\varphi(n)\bmod m)2^{-n}`$ for every $`m\ge3`$ and classify rational dyadic residue observables. For $`S=\sum_{n\ge1}\varphi(n)2^{-n}`$, we prove a seed-to-ray amplification and record an equivalent central-residue condition. These results do not establish irrationality of $`S`$. Ordinary proofs are given in the note; formal counterparts and their precise scope are identified separately.

<a id="sec:results"></a>

# A basis and all its relations

Set $`\varphi(0)=0`$. For $`k\ge2`$, the $`k`$-kernel consists of the sequences $`n\mapsto\varphi(k^jn+r)`$ with $`j\ge0`$ and $`0\le r<k^j`$. Coons proved that $`\varphi`$ is not $`k`$-regular for any $`k\ge2`$ \[coons, Thm. 3.2\], and Martin’s Theorem 1 implies that the functions $`n\mapsto\varphi(a_in+b_i)`$ are linearly independent along pairwise nonproportional affine forms with integer coefficients and positive slopes \[martin-phi-inequalities, Thm. 1\]. The following theorem gives the exact rank, unique integral coordinates, and, in Corollary <a href="#cor:integral-normal-form" data-reference-type="ref" data-reference="cor:integral-normal-form">3</a>, all integral relations at every finite level. Section <a href="#sec:rank" data-reference-type="ref" data-reference="sec:rank">1.1</a> gives a direct proof of the independence step, with the antecedent of its separation method credited there. No priority claim is made for affine independence.

<div id="thm:kkernelrank" class="theorem">

**Theorem 1** (exact rank of every totient $`k`$-kernel truncation). *Let $`k\ge2`$ and $`e\ge1`$ be integers, write $`F^{(k)}_{j,r}(n)=\varphi(k^jn+r)`$, and put
``` math
...
1.  <span id="res:rank" label="res:rank"></span> <span class="smallcaps">\[unconditional progress\]</span> **Exact finite-level rank.** For integers $`k\ge2`$ and $`e\ge1`$, Theorem <a href="#thm:kkernelrank" data-reference-type="ref" data-reference="thm:kkernelrank">1</a> proves ordinarily that the totient $`k`$-kernel truncation has rank $`k^e+1`$. The dyadic case is also the [exact finite-truncation rank](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientMahlerDefect.lean#L1084); the reusable all-base Lean wrapper is [the same count from an independent family](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientKernelConditional.lean#L215), and retains linear independence as an explicit input. That input is discharged by the independent CRT–Dirichlet proof in [`AllBaseTotientKernel.lean`](https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1118); the [complete truncation rank](https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1239) therefore has no independence hypothesis. Martin supplies the broader external antecedent, rather than an axiom of this Lean proof.
...
The two ingredients are scalar reduction and arithmetic independence. The reductions explain which sections must be dependent. The difficulty is to exclude every additional relation. We do this one coefficient at a time: an evaluation will annihilate all the other columns modulo a prime while leaving the chosen column nonzero. Thus no simultaneous primality assertion for several affine forms is needed. The local formula for $`\varphi`$ gives the displayed reductions. For a prime $`p\mid k`$ that does not divide $`u`$, the argument $`k^{j-t}n+u`$ is a $`p`$-adic unit, so multiplication by $`k^t`$ contributes its power of $`p`$ and one factor $`1-1/p`$. When $`p\mid u`$, the argument already contains $`p`$, and only its power contributes. This gives $`C_k(t,u)`$.
...
*Proof.* Every finite subfamily lies in a sufficiently high truncation. The theorem gives its independence; the reductions give equality of the full spans. At level zero the only channel is $`\varphi(n)`$, which is nonzero since $`\varphi(1)=1`$, so its span has dimension one. This is an ordinary proof; it makes no claim about an additional formal declaration. ◻

</div>

<div id="cor:integral-normal-form" class="corollary">

**Corollary 3** (Integral normal form). *Let $`k\ge2`$ and $`e\ge1`$. The canonical family is a $`\mathbb{Z}`$-basis of the module generated by the sections through level $`e`$. For each omitted channel $`F_i`$, write its canonical reduction as $`F_i=a_iF_{j(i)}`$, where $`j(i)`$ is retained and $`a_i`$ is a nonnegative integer. In the free abelian group on all the channels, the vectors
``` math
...
The totient is not $`k`$-regular, in the sense of Allouche and Shallit \[allouche-shallit\], for any $`k\ge2`$: Coons proved that the $`\mathbb{Z}`$-module generated by its $`k`$-kernel is not finitely generated, by counting the poles of $`\zeta(s-1)/\zeta(s)`$ in the strip $`0\le\operatorname{Re}s\le1`$ \[coons, Cor. 3.1, Thm. 3.2\]. For integer-valued sequences this is equivalent to infinite dimension of the rational span: if the span has dimension $`d`$, some $`d`$ evaluations map it injectively to $`\mathbb{Q}^d`$ and the module into $`\mathbb{Z}^d`$ (compare the evaluation argument in \[coons-jsr, App. A\]). Theorem <a href="#thm:kkernelrank" data-reference-type="ref" data-reference="thm:kkernelrank">1</a> therefore recovers Coons’s theorem by a different route and gives the dimension at every finite level.

The generating-function context is stronger than non-regularity alone. For multiplicative coefficients in characteristic zero, Bell and Smertnig prove that $`k`$-Mahler implies $`k`$-regular \[bell-smertnig2026, Thm. 1.3\]; their totient example therefore rules out a $`k`$-Mahler equation for $`F_\varphi(z)=\sum_{n\ge1}\varphi(n)z^n`$ in every base $`k\ge2`$. Bell, Bruin and Coons give the corresponding algebraic and $`D`$-finite classification framework \[bell-bruin-coons, Thms. 1.5–1.6\]. These are restrictions on functions. Neither classification by itself determines the arithmetic nature of $`F_\varphi(1/2)`$.

For a $`k`$-regular sequence the difference between a basis of the kernel span and a redundant spanning family is material: matrices associated to a basis compute the growth exponent through their joint spectral radius, whereas a spanning family gives only an upper bound \[coons-jsr, Thm. 1, Prop. 4, Cor. 7\]. That theorem does not apply to $`\varphi`$, but the section maps $`g(n)\mapsto g(kn+i)`$ still have an explicit form here. Each sends $`F_{j,r}`$ to $`F_{j+1,k^ji+r}`$, so it carries $`V_{k,e}`$ into $`V_{k,e+1}`$, and no level is invariant under all of them because the dimensions increase. In the canonical bases, a retained section with $`j\ge1`$ goes to a retained section, while $`F_{0,0}`$ and $`F_{1,0}`$ go to $`F_{1,i}`$ and to $`F_{2,ki}`$, a positive integer multiple of $`F_{1,i}`$. Every element of $`\mathcal B_{k,e}`$ is therefore sent to a positive integer multiple of an element of $`\mathcal B_{k,e+1}`$.
...
**Corollary 4** (Freezing periodic coefficients). *If $`L_i`$ are pairwise nonproportional positive-slope affine forms and $`w_i`$ are rational-valued periodic sequences, then
``` math
...
*Proof.* Choose a common period $`Q`$. On each progression $`n=a+Qt`$, the coefficients are constant and the cross determinants become $`Q(a_ib_j-a_jb_i)\ne0`$. Shift beyond the threshold and apply the affine independence just proved. This is an ordinary proof of the periodic-coefficient statement; no additional formal declaration is asserted here. ◻

</div>

Applied on each progression, Martin’s Theorem 1 gives the same conclusion \[martin-phi-inequalities, Thm. 1\]. The same argument preserves the basis after deletion of a finite prefix. For the two zero channels, large $`n\equiv1\pmod k`$ and large powers of $`k`$ replace the two evaluations used above.

<a id="sec:family"></a>

# Bounded residues and rationality

There is an earlier coefficient-level obstruction: for each $`m\ge3`$, $`\varphi(n)\bmod m`$ is not automatic in any integer base $`k\ge2`$ \[yazdani2001, Cor. 4, p. 654\]; see also \[allouche-shallit-yassawi, Thm. 3 and Ex. 4\]. This does not prove irrationality of the dyadic sum. When $`m>2`$ the raw residues are not binary digits, and carrying changes the digit sequence. The residue theorem below instead forces neighbouring coefficients to vanish while retaining one central value, then uses a bounded-tail argument. Erick Wong posted a proof that $`\sum_{n\ge1}(\varphi(n)\bmod k)k^{-n}`$ is irrational for every integer $`k>2`$, with the base equal to the modulus, as an answer on Mathematics Stack Exchange on 29 March 2015 \[wong2015\]. Theorem <a href="#res:residueseries" data-reference-type="ref" data-reference="res:residueseries">5</a> fixes the base at $`2`$ for every modulus $`m\ge3`$ and classifies the dyadic residue observables.

<div id="res:residueseries" class="theorem">

**Theorem 5**. *For every $`m\ge3`$,
``` math
...
Subtract $`f(0)`$ from the observable and clear its denominators. Whenever $`f(s-1)\ne f(0)`$, the preceding construction gives arbitrarily long bounded isolated pulses, so Lemma <a href="#lem:bounded-pulse" data-reference-type="ref" data-reference="lem:bounded-pulse">6</a> applies. For the least-residue map choose $`s=-1`$: its central residue is $`m-2\ne0`$ when $`m\ge3`$. For $`m=2^k`$, every even residue $`r`$ has $`r+1`$ a unit, so a nonconstant restriction to the even residues supplies such a pulse. Conversely, $`\varphi(n)`$ is even for every $`n\ge3`$. Constancy on the even classes therefore makes all terms after $`n=2`$ constant, giving the stated rational value. This converse requires no recurrent-support theorem. The three clauses together are ordinary in this note (that `PaperCompleteR7` file is absent). ◻
...
\end{equation}
```
In particular a nonintegral tail difference is a precise arithmetic statement about $`S`$. The complementary rationality consequence uses the same carry for both kinds of rigidity: the [same-carry rank and modular-period theorem](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientTailCarryPeriod.lean#L224) gives rank at least $`2^e-1`$ at every depth and one common eventual period for all dyadic sections modulo a fixed positive multiplier. It supplies necessary structure under rationality, not a contradiction between the two properties.
...
The seed-to-ray step is [eventual two-syndetic amplification](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L364); it is equivalent to irrationality as [global full-depth equivalence](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L406).
...
*Equivalent formulation 8* (Canonical Mersenne gap). The series $`S`$ is irrational if and only if, for every $`c\ge0`$ and odd $`v\ge1`$, some positive multiple $`H`$ of $`\varphi(v)`$ satisfies
``` math
\begin{equation}
...
2.  <span id="res:actualsign" label="res:actualsign"></span> <span class="smallcaps">\[unconditional progress\]</span> **Positive LCM corridor and top-edge boundary.** For $`a\ge8`$ and $`J+(a+6)<2\cdot2^a`$, the true translated orbit difference $`\operatorname{totientTail}(2H_a+J)-\operatorname{totientTail}(H_a+J)`$ is strictly positive. If it is represented by an integer and the dyadic modulus has room for the directed endpoint strip, the carry survivor is strictly negative and the discrepancy residue is exactly the top-edge representative $`2^K-\mathrm{carry}_K`$, lying strictly between the strip’s lower edge and $`2^K`$. Thus the positive corridor eliminates the nonnegative true survivor but does not itself produce a central-band contradiction; an independent arithmetic exclusion of the top boundary is still required. *Checked:* [positive corridor](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientActualLcmOrbitSign.lean#L39), [negative survivor](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientActualLcmOrbitSign.lean#L172), [top-edge residue](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientActualLcmOrbitSign.lean#L211).

3.  <span id="res:factorideal" label="res:factorideal"></span> <span class="smallcaps">\[unconditional progress\]</span> **LCM factor-ideal shift-algebra no-go.** For every $`t\ge3`$, with $`H=\operatorname{lcm}(1,\ldots,t)`$, there is a nonzero synthetic dyadic coboundary whose forcing letters reproduce the actual totient differences at $`t-2`$ prescribed indices and lie in the ideal generated by $`\varphi(H)`$. Every finite integer shift polynomial preserves the exact coboundary cancellation, the $`H`$-factor ideal and all lower factor ideals, together with uniform state and letter bounds. A sparse-anchor variant also preserves every prescribed whole-ray anchor. Hence factor ideals, finite shift-algebra closure and these bounds cannot by themselves force a contradiction. The witness is synthetic: its letters are not claimed to be actual totient differences, nonlinear combinations are outside the theorem, and no cofinal family of certificates follows. *Checked:* [finite shift-algebra no-go](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/LcmFactorIdealPulseObstruction.lean#L798).

A rational control, proved in `ParityPerturbedRationalControl.lean`, has $`c(n)=\varphi(n)`$ for odd $`n`$, $`|c(n)-\varphi(n)|\le2`$ for even $`n`$, $`0\le c(n)\le n`$, and
``` math
...
```
The divisor-convolution identity $`\varphi=\mu*\mathrm{Id}`$ gives the displayed value of $`\Theta_2`$. Amiram Eldar posted the corresponding formula $`S=\tfrac12+\sum_{d\ge1}\mu(d)/(2^d-1)^2`$, together with its coprimality interpretation, to OEIS A256936 on 15 March 2026 \[eldar2026oeis\], and Steve Fan posted the formula on the problem’s forum thread on 16 May 2026 \[fan2026totient\]. The following exact extremum is stated in the long record as Theorem `thm:rankonefloor`; its proof is [the sharp-floor argument](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/RankOneSharpFloor.lean#L534) in the linked source.

<div id="res:rankonefloor" class="theorem">

**Theorem 9** (Positive rank-one floor). *For $`e\ge1`$ and $`Y\ge4`$, the denominator of $`Q(e,Y)`$ is positive, and the unique minimiser is $`(e,Y)=(1,5)`$. Every admissible quotient and every nonempty finite positive weighted average of such quotients satisfies
``` math
...
Allouche and Shallit introduced $`k`$-regular sequences \[allouche-shallit\]. Coons proved that $`\varphi`$ is not $`k`$-regular \[coons, Thm. 3.2\]; the exact ranks recover this, as explained after Corollary <a href="#cor:integral-normal-form" data-reference-type="ref" data-reference="cor:integral-normal-form">3</a>. Martin’s Theorem 1 makes the ratios of totients on any prescribed finite list of pairwise nonproportional affine forms with integer coefficients and positive slopes arbitrarily separated; this implies their linear independence \[martin-phi-inequalities, Thm. 1\]. Section <a href="#sec:rank" data-reference-type="ref" data-reference="sec:rank">1.1</a> gives a direct determinant proof tailored to the sections used here. The basis theorem gives the exact finite-level normal form. Section <a href="#sec:family" data-reference-type="ref" data-reference="sec:family">2</a> treats the residue series at base two, where Wong’s matched-base answer is credited, and classifies dyadic observables. Erdős and Graham record the irrationality of $`S`$ as probably hopeless to prove at present \[erdosgraham1980, p. 61\].

The [dyadic finite-level rank $`2^e+1`$](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientMahlerDefect.lean#L1084), residue-series irrationality, [full-depth amplification](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L364), canonical equivalence and the [positive rank-one floor](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/RankOneSharpFloor.lean#L624) are in the public snapshot. The unconditional all-base exact-rank theorem is also [kernel-checked](https://github.com/wcook04/plectis-erdos/blob/25ef6245d15a47548c6926369ae8f1a0f0a14a80/Erdos249257/AllBaseTotientKernel.lean#L1239): its CRT–Dirichlet independence proof does not assume Martin’s theorem. [Rank from an independent all-base family](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/TotientKernelConditional.lean#L215) remains the reusable conditional factorisation. The [canonical gap](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/ErdosProblems/Erdos249/CyclotomicAnchoredKill.lean#L3165) is the remaining arithmetic endpoint. The historical source links retain `99f4bf47422a`. The integral normal form has its ordinary proof above. The periodic transport lemma is checked in `PeriodicTotientIndependence.lean`; the eventual version follows by the displayed threshold shift. The rational control and the termwise-window and prime-dilation boundaries are proved in `ParityPerturbedRationalControl.lean` and `PrefixValuationAndControlRigidity.lean`, respectively. Finite evaluation ranks and the all-level carry lower bound remain distinct. The long record gives the Lean-checked Farey-window exclusion at window $`K=240`$: $`S\ne a/q`$ for every integer $`a`$ and every $`0<q\le Q_0`$, where $`Q_0\approx7.96\times10^{34}`$ is stated exactly there. It does not imply the universal gap.
...
**Problem 10** (first-harmonic anti-concentration on supplier fibres). For each $`h\ge1`$, do there exist $`s\ge1`$ and $`0<\eta<1`$ such that, for every $`X_0`$, there are $`X,L`$ with
``` math
...
The quantities are defined with the [budget condition](https://github.com/wcook04/plectis-erdos/blob/99f4bf47422abbd8757cbb22b50ba079d764d3a7/Erdos249257/FirstHarmonicPivot.lean#L543) in the long record. This note does not prove them. Balasubramanian, Giri and Srivastav prove asymptotics for shifted convolutions $`\sum_nF(n)G(n-h)`$ of divisor convolutions $`F=f*1`$, $`G=g*1`$ with decaying $`f,g`$, a class that contains $`\varphi(n)/n`$ \[bgs2016, Thm. 2.2\]; the first bound above needs cancellation against the dyadic phase, which that theorem does not supply.
