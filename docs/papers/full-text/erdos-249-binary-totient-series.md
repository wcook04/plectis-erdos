...
```
([exact frontier](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/TotientActualLcmOrbitNonintegrality.lean#L37)). A second exact frontier removes an apparently free search variable. Given a basepoint $`c`$ and a positive odd $`v`$, choose $`H>0`$ with $`\varphi(v)\mid H`$ and $`vM=2^H-1`$. Irrationality is equivalent to the existence of such $`H,M`$ for which the least nonnegative residue
``` math
\bigl(-\operatorname{totientBlock}(H,c)\bigr)\bmod M
```
lies strictly between the two endpoint intervals of width $`c+H+1`$ ([canonical Mersenne equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/CyclotomicAnchoredKill.lean#L2976)). Thus the basepoint is canonically $`N=c`$; no remote $`N`$ remains to be found. The residue evolves by an exact affine doubling recurrence. This is an exact reformulation, not a proof of the still-missing centered-gap supply.

A third exact frontier shows that locking certificate depth to its period costs nothing. One arbitrary-depth seed certificate on a $`d`$-ray forces full-depth certificates on an eventually $`2`$-syndetic set of multipliers, so the pointwise full-depth predicate, its cofinal version, the arbitrary-depth period-multiple supply, and irrationality of $`S`$ are equivalent ([eventual two-syndeticity](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L364); [pointwise equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L406); [cofinal equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L438)). This amplification does not produce the initial non-integral shift or seed; Problem #249 therefore remains open. A stronger quantitative consumer assumes that, for every $`a_0`$, there are $`a,q\in\mathbb{N}`$ such that
``` math
\begin{gathered}
...
```
For one window these budgets give the bound $`9X/10`$; their quantified cofinal supply implies irrationality ([budget predicate](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L543); [the $`9X/10`$ consequence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L549); [conditional endpoint consumer](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L577)). That decorrelation supply is unproved. The exact supplier-fibre bijection does not provide global prime isolation: at $`(X,L,s,m,N)=(16,20,1,2,18)`$ the pivot is $`38=2\cdot19`$, while $`19`$ also divides the distinct window argument $`N+1=19`$.

A distinct producer-side route studies the phase of the actual tail orbit rather than the LCM approximation. Its first harmonic obeys the exact squaring law [phase recurrence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean#L167), so iteration gives a power-of-two closed form [phase normal form](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean#L201). The strict prime-index gap condition is therefore equivalent to an initial-phase condition [initial-phase equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean#L278), and a dyadic-root initial phase is an exact obstruction [dyadic-root obstruction](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/TotientStrictPrimeEscape.lean#L322). Either a cofinal block-density hypothesis on nonpositive phases or cofinally many prime indices with a strict phase gap supplies a conditional irrationality consumer. Neither the phase-density premise, the cofinal prime-index supply, nor a prime-distribution bridge is proved here; this is a producer specification and classification, not an unconditional result for Problem #249.
...
A second unconditional result determines the positive rank-one Möbius–Mersenne cone exactly. Its quotient has the unique minimizer $`(e,Y)=(1,5)`$; every admissible quotient and every nonempty finite positive mixture exceeds $`\Theta_2=S-1/2`$ by more than $`21/320`$. Thus $`1/16`$ is a valid uniform unit-fraction floor, while $`1/15`$ already fails at the unique minimizer. Equivalently, a rational representation $`Q(e,Y)=p/q`$ satisfies $`|q\Theta_2-p|>21q/320`$. This rules out the complete positive rank-one and positive direct-sum mechanism, but not signed cancellation or genuinely coupled higher-rank approximants.
...
> **Contribution.** The paper first exposes the exact actual-LCM, canonical-Mersenne, and full-depth period-multiple frontiers, followed by two distinct quantitative consumers for Problem #249: separation of the actual power-two LCM orbit, and a four-term first-harmonic budget with a cofinal residual-decorrelation producer. It then gives the explicit odd-core basis, relation normal form, exact finite-level rank $`2^e+1`$, and the sharp positive rank-one/direct-sum floor with its unique minimizer, followed by a status-tagged inventory of mechanisms, no-go results, and finite evidence.
>
...
>
> **Executable review object.** A dedicated ActualLcmOrbit Comparator package carries the exact orbit identity and the sharp equivalence [between irrationality and cofinal actual-LCM non-integrality](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/TotientActualLcmOrbitNonintegrality.lean#L37). Separate packages carry the canonical-Mersenne and full-depth equivalences, the odd-core basis and finite-rank rows, and the sharp rank-one/direct-sum no-go family with its rational-linear-form endpoint. The stronger actual-LCM separation consumer, the first-harmonic conditional consumer, the top-edge obstruction, and the $`38=2\cdot 19`$ counterexample remain adjacent source evidence rather than declarations selected by those packages. Full-kernel infinite-dimensionality is prior-art context from Coons; the all-base independence input uses Martin externally; no novelty or priority claim is made for the conditional routes. The repository’s external-verification job compares these exact Lean propositions with separately declared challenge statements and an axiom budget, then asks Lean’s kernel to check the submitted proofs. The [formalisation manifest](https://github.com/wcook04/plectis-lean-erdos249-257/blob/main/formalization.yaml) and the commit-bound CI receipt record that check; they do not assess novelty, significance, or whether the original problem is solved.
...
```
([definition](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/TotientTailPeriodKiller.lean#L72)). It exists to make a statement about an infinite tail decidable by a finite computation, and it is faithful: some depth $`L`$ certifies $`(h,N)`$ if and only if $`R_{N+h}-R_N\notin\mathbb{Z}`$ ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/LcmConeFlatness.lean#L316)). For example,
``` math
D_{1,12,16}=-143140,\qquad
  \rho_{1,12,16}=53468,\qquad N+h+L+2=31,
```
so $`(1,12,16)`$ is a certificate. More generally, the test succeeds at $`(h,12,16)`$ for every $`h`$ with $`1\le h\le8`$ ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/TotientTailPeriodKiller.lean#L404)), so none of $`R_{13}-R_{12},\dots,R_{20}-R_{12}`$ is an integer ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/TotientTailPeriodKiller.lean#L407)).
...
1.  <span id="res:canonicalmersenne" label="res:canonicalmersenne"></span> <span class="smallcaps">\[formalised here\]</span> **Canonical Mersenne arithmetic frontier.** Let
    ...
    ```
    is equivalent, after quantifying over $`c,v`$, to irrationality of $`S`$. The basepoint is exactly $`c`$, the dyadic valuation of a prospective rational denominator; the older centered-residue supply searched for a remote $`N\ge\max(c,N_0)`$. The canonical statement implies that older supply by feeding $`\max(c,N_0)`$ into its $`c`$-slot. Moreover, whenever $`M\mid2^H-1`$, the finite residue obeys the exact recurrence
    ``` math
    \rho_{H,N+1,M}
     =\bigl(2\rho_{H,N,M}-(\varphi(N+H+1)-\varphi(N+1))\bigr)\bmod M.
    ```
    This removes analytic tails, carry trajectories, and remote-basepoint search from the equivalent residual, but it does not prove the required central gap. *Checked:* [residue recurrence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/CyclotomicAnchoredKill.lean#L2185), [dominance over the remote supply](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/CyclotomicAnchoredKill.lean#L2943), [exact equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/CyclotomicAnchoredKill.lean#L2976).
...
2.  <span id="res:fulldepth" label="res:fulldepth"></span> <span class="smallcaps">\[formalised here\]</span> **Full-depth ray amplification and exactness.** Fix $`d>0`$ and $`N`$. If $`(d,N,L)`$ is a finite tail-difference certificate at any depth $`L`$, then there is $`T>0`$ such that every pair $`\{t,t+1\}`$ with $`t\ge T`$ contains a multiplier $`m`$ for which $`(md,N,md)`$ is a certificate. Thus one seed forces an eventually $`2`$-syndetic family whose depth equals its period. Pointwise,
    ...
    ```
    Consequently the predicate demanding such a full-depth certificate for every $`d>0,N`$ is equivalent to irrationality of $`S`$. Its cofinal-in-$`N`$ version is equivalent both to the older arbitrary-depth period-multiple supply and to irrationality. The theorem proves that the depth lock is free; it does not produce the first non-integral shift or seed. *Checked:* [eventual two-syndeticity](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L364), [pointwise equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L388), [global equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L406), [cofinal depth deletion](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L421), [cofinal endpoint](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L438).
...
|:---|:---|:---|:---|
| $`\tau`$ | $`1.6066951\ldots=E`$ | $`g\equiv1`$ | The Erdős–Borwein constant; irrational \[erdos1948, theorem on p. 63\]; formalised here in the Lambert form $`\sum_{d\ge1}(2^d-1)^{-1}`$ ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/CertificateKernel.lean#L8335)) |
...
```
([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/CertificateKernel.lean#L18557)), the probability being the visible-coprime-pair series $`\sum_{\gcd(m,n)=1}2^{-(m+n)}`$. That probability is related to a distribution over the Stern–Brocot tree in a completely explicit way. For positive coprime $`(a,b)`$ put $`w_{a,b}=1/(2^{a+b}-1)`$. These reduced-slope masses sum to exactly $`1`$ ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/GcdMomentCalculus.lean#L349)), and at each node $`(a,b)`$ the cylinder mass $`1/((2^a-1)(2^b-1))`$ splits exactly into its stop mass and the masses of its two children ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/GcdMomentCalculus.lean#L474)). The coprimality probability is not that total mass; rather,
``` math
S-\tfrac12
...
```
([first difference](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FiniteEulerSieve.lean#L44)), and for every $`e\ge0`$,
``` math
...
<a id="exact-full-depth-reduction-and-a-stronger-sufficient-producer"></a>

## Exact full-depth reduction and a stronger sufficient producer

The older interface [ApFullDepthEscape](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/PeriodMultipleEscape.lean#L441) asks, for every $`d\ge1`$ and $`N`$, for a $`t\ge1`$ such that $`(td,N,td)`$ is a finite tail-difference certificate. It was originally recorded only as a sufficient condition because the certificate depth is forced to equal its period. The source-current amplifier removes that asymmetry completely. For fixed $`d,N`$, one certificate $`(d,N,L)`$ at any depth yields a threshold $`T>0`$ beyond which every adjacent pair of multipliers contains an $`m`$ with a full-depth certificate $`(md,N,md)`$ ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L364)). Together with finite-certificate faithfulness, this gives the pointwise equivalence
``` math
\exists t>0:\ (td,N,td)\text{ is certified}
 \quad\Longleftrightarrow\quad R_{N+d}-R_N\notin\mathbb{Z}
```
([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L388)). Therefore $`\operatorname{ApFullDepthEscape}`$ is equivalent to irrationality, not merely sufficient for it ([checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L406)). The cofinal full-depth supply is likewise equivalent to the arbitrary-depth period-multiple supply and hence to irrationality ([depth deletion](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L421); [endpoint equivalence](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/ErdosProblems/Erdos249/FullDepthRayAmplifier.lean#L438)). What remains open is the producer itself: the amplifier converts one non-integral shift, equivalently one seed, into a dense ray of full-depth certificates but does not furnish that seed.
...
X+t\le mp<2X+t
```
under the injective map $`p\mapsto mp-t`$ ([membership](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L299), [injectivity](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L367), [image equality](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L384)). This exact bijection does not imply that the supplier prime is isolated from every other argument in the window. At $`X=16`$, $`L=20`$, $`s=1`$, $`m=2`$, and $`N=18`$, the pivot is $`38=2\cdot19`$, while the same prime $`19`$ divides the distinct argument $`N+1=19`$ ([checked counterexample](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L394)). For a supplier write $`z_N`$ for its explicit totient pivot phase, $`w_N=\omega_N/z_N`$, and $`\bar z_m`$ for the mean of $`z_N`$ on the fibre with cofactor $`m`$. Define
``` math
\begin{aligned}
...
```
The decomposition is [checked](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L514), and the four budgets imply irrationality by [the checked consumer](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L577). More precisely, these four inequalities are the source predicate [the budget condition](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L543), and its checked consequence [the first-harmonic gap](https://github.com/wcook04/plectis-lean-erdos249-257/blob/234347aac64e8f186ca816281f68fa9ed1365b1b/Erdos249257/FirstHarmonicPivot.lean#L549) is the explicit estimate
``` math
...
```
The stronger unit-fraction floor $`1/15`$ fails already at the minimizer. Thus adding atoms or positive rank-one blocks cannot create a small linear form; the remaining route must use signed cancellation, off-diagonal coupling, or genuinely higher rank.
