...
Write $`\operatorname{def}(a,a')=a'-\operatorname{syl}(a)`$ for the deviation of the next term from the Sylvester successor, the [Sylvester defect](https://github.com/wcook04/plectis-erdos/blob/4ab50e144847a3393fb340341760e5d6907d702e/ErdosProblems/Erdos243/ReciprocalTailRigidity.lean#L53).
...
Formalised as the [local rigidity step](https://github.com/wcook04/plectis-erdos/blob/4ab50e144847a3393fb340341760e5d6907d702e/ErdosProblems/Erdos243/ReciprocalTailRigidity.lean#L1781). The hypothesis $`C_{n+1}\ne0`$ is not removable: at $`C_{n+1}=0`$ the identity gives no information about $`a'`$.
...
The criterion moves the obligation off the whole orbit. Only steps setting a new LCM numerator record contribute, a fixed LCM-scale baseline may be subtracted, and the remaining mass may be logarithmically discounted, so a counterexample has to sustain divergence along a sparse subsequence. The inherited part of the picture is one half-space. Proposition <a href="#res:classicalhalfspace" data-reference-type="ref" data-reference="res:classicalhalfspace">26</a> shows that the Erdős–Straus Theorem 3 quantity and Koizumi’s Corollary 20(1) quantity carry the sign of $`E_n`$ at every index, so each of those published criteria decides the eventual regime $`E_n\ge0`$, which Koizumi’s Proposition 19(2) closes. What is added here is a criterion valid in the complementary regime as well, stated under the hypotheses of Problem <a href="#res:problem" data-reference-type="ref" data-reference="res:problem">1</a> and no others. Two exclusions bound what these coordinates reach. Proposition <a href="#res:classicalhalfspace" data-reference-type="ref" data-reference="res:classicalhalfspace">26</a> exhibits cancellation-free blocks on which every classical hypothesis quantity fails by a fixed number of units, and Proposition <a href="#res:coprimalitycap" data-reference-type="ref" data-reference="res:coprimalitycap">27</a> shows that the data used by Theorem <a href="#res:barrier" data-reference-type="ref" data-reference="res:barrier">34</a> and the landing lemmas admit walks with rises of order $`\log u`$, so an argument using only those data excludes no negative part of order $`\log C_n`$. A producer that closes the gap has to enter the regime displayed in Section <a href="#sec:records" data-reference-type="ref" data-reference="sec:records">5</a>, where the negative part is at least $`(1-\delta)\log_2\log_2C_n`$ and is $`o(C_n)`$ at infinitely many indices, and inside that regime it has to use the congruence $`E_n\equiv D_n\pmod{C_n}`$ beyond coprimality, or supply a second mechanism that forces landings.
...
\end{equation}
```
This is the [repair-entropy conservation law](https://github.com/wcook04/plectis-erdos/blob/4ab50e144847a3393fb340341760e5d6907d702e/ErdosProblems/Erdos243/RepairEntropy.lean#L48).
...
K^L4^{|R|}<(K+1)^L.
```
The formal consequence is [the independent-repair cardinality bound](https://github.com/wcook04/plectis-erdos/blob/4ab50e144847a3393fb340341760e5d6907d702e/ErdosProblems/Erdos243/RepairEntropy.lean#L78). It rules out positive-density deletion of independent old moduli inside recoveries with a fixed relative-error budget.
...
```
The checked [tail-height estimate](https://github.com/wcook04/plectis-erdos/blob/4ab50e144847a3393fb340341760e5d6907d702e/ErdosProblems/Erdos243/ReciprocalTailRigidity.lean#L2164) and normalised vanishing make $`|E_n|`$ subexponential in $`n`$.
...
The [pinned formal-source revision](https://github.com/wcook04/plectis-erdos/tree/4ab50e144847a3393fb340341760e5d6907d702e) contains the Lean sources, the fixed toolchain, and the library manifest used in the verification. Proof authority remains with those sources; this manuscript provides navigation and exposition.
...
**Reader position.** 2 of 93; display band: front door. Formal editorial disposition: split. These are separate classifications.

**Reader entry.** Track one signed centered error: forcing it eventually to zero recovers the Sylvester recurrence.
...
**Result-atom population.** 32 of 704 public coordinates. The public atom catalog groups them under this family.
...
**Reader position.** 11 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** If the normalized negative centered mass is summable, the negative excursions must end and the Sylvester recurrence follows.
...
**Result-atom population.** 36 of 704 public coordinates. The public atom catalog groups them under this family.
...
**Reader position.** 15 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** A negative centered error cannot stay bounded forever under the stated normalized dynamics.
...
<a id="arithmetic-weighted-record-dichotomy"></a>

## Arithmetic weighted record dichotomy

**Reader position.** 21 of 93; display band: major result. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** For positive integer LCM states with exact lcm/gcd update and feedback rho U_next=U-V, V=L-(a-1)U, and only the lower centering bound -U\<=2V, bounded U is equivalent to finiteness of the record-step excess sum sum(-V-B)\_+ f(U), for each fixed integer B\>=0 and finite nonnegative nonincreasing weight with divergent integral. Unbounded height itself supplies arbitrarily many pairwise-coprime record digits; no fresh-index density or subexponential growth assumption is needed.

For positive integer LCM states with exact lcm/gcd update and feedback rho U_next=U-V, V=L-(a-1)U, and only the lower centering bound -U\<=2V, bounded U is equivalent to finiteness of the record-step excess sum sum(-V-B)\_+ f(U), for each fixed integer B\>=0 and finite nonnegative nonincreasing weight with divergent integral. Unbounded height itself supplies arbitrarily many pairwise-coprime record digits; no fresh-index density or subexponential growth assumption is needed.

**Authority and reach.** Ordinary proof with independent exact finite checks; ordinary arithmetic theorem and canonical-orbit criterion.

**Exact boundary.** The finite CRT crossing and global divergent-integral argument are stated in the public paper. The full dichotomy has no Lean or external challenge certificate. Turning the canonical weighted criterion into universal Sylvester termination still requires a genuine upper summability bound.
...
**Reader position.** 43 of 93; display band: mechanism. Formal editorial disposition: merge. These are separate classifications.

**Reader entry.** Fresh coprime moduli eventually defeat a reduced tail that rises too slowly.
...
**Reader position.** 70 of 93; display band: frontier. Formal editorial disposition: promote. These are separate classifications.

**Reader entry.** Periodic negative-error patterns cannot be the obstruction sought in Erdős 243.
