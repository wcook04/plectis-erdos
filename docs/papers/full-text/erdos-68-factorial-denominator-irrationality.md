...
Canonical factorial digits come from Cantor’s factorial expansion \[cantor1869\], in which a real number is rational exactly when its digits vanish from some index on. Galambos treats the rationality of Cantor series in \[galambos1976, Ch. II, §2.1, pp. 21–22\], and Koepf and Schmersau prove the irrationality direction for nonterminating factorial expansions whose digits are not eventually maximal \[koepf-schmersau, Example 3.2, p. 121\]. The following theorem is the analogue for the companion $`C=S-e+2`$, whose distinguished digit is $`m-2`$.
...
The following factorial-gap examples come from an exact modular scan; they show why the weights matter. At $`M=138`$, the maximal $`139`$-hits are $`69,122,137`$, with cofactor residues $`6,49,73`$, and
``` math
