...
The public Lean clone owns the mathematics and return path. The separate [public Plectis Python toolkit](https://github.com/wcook04/plectis) offers optional mechanism examples. The private workbench explains production, not proof authority, and contributors need no access to it. Any runner or no model at all can use the public evidence and review path. The [public website](https://wcook04.github.io/plectis/) is a rendered reading route. Its canonical generator and editorial configuration remain private; the Lean clone and optional toolkit are the inspectable sources.
...
Tao’s community wiki of AI contributions to the Erdős problems is the existing public register in this exact domain \[taowiki\]. It records which problems AI systems have touched and with what outcome. It carries no evidence classes, roles, or checkable claim boundaries, and it is edited by hand. The protocol here is meant to feed that register with returns that already carry those fields, and to leave a trail from each wiki line back to the exact commit, receipt, and surviving boundary.
