...
The regrouping is [finite smooth kernel sum grouped by height](https://github.com/wcook04/plectis-erdos/blob/f36a98bf3d3e6f65f1074e3b800e3293d5b8a51a/ErdosProblems/Erdos269/ThreePrimeRunningLcm.lean#L407). Fan and Lott build their van der Corput coefficients on sums of two squares by the same kind of regrouping: up to normalisation, the coefficient at $`d`$ sums a smooth weight over the representations $`d=v^2(x^2+y^2)`$, and the average over $`v`$ keeps the associated complete sums in a fixed sector \[fanlott2026, §3 and Corollary 4.4\].
