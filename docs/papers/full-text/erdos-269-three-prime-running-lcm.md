...
For three pairwise distinct primes, the reciprocal running-LCM kernel has nonsingular minors of every order, with one choice of indices valid in every third-coordinate layer. Dividing out row and column powers leaves a binary carry whose consecutive threshold columns differ on disjoint intervals. This gives both arbitrary-order non-separability and the exact rank of every finite sample. For two primes, the paper deduces transcendence of both running-LCM series from the Hecke–Mahler value theorem of Loxton and van der Poorten, in the form given by Bugeaud and Laurent. Steve Fan posted the same factorisation, reduction and conclusion first, on the problem’s forum thread (erdosproblems.com/269, 26 June 2026, post 7218); the two-prime result is not formalised in Lean. The repeated three-prime irrationality question remains open.
...
```
Prime-power divisibility gives $`\operatorname{L}=\operatorname{H}`$ (Proposition <a href="#res:lcm" data-reference-type="ref" data-reference="res:lcm">6</a>); Fan’s forum post states this identity for every finite prime set \[fan2026comment\].
...
The criterion below needs only a valid cap, and the bound just proved gives the cap $`90B(a+1)^2`$. The extended reasoning record proves the sharper bound $`X_a\le(n_a^2+8n_a+18)/9`$, where $`n_a`$ is the sum of the three height exponents at $`2^a`$.
