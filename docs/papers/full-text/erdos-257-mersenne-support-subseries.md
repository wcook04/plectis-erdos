...
```
is irrational for every infinite $`A\subseteq\mathbb{N}_{>0}`$. The problem remains open. We prove that, for every integer $`b\ge2`$, the sum $`\sum_{a\in A}(b^a-1)^{-1}`$ is irrational whenever $`A`$ is infinite and $`\sum_{a\in A}a^{-1}<\infty`$. No coprimality, periodicity, density, or powerful-support hypothesis is used. The proof transfers binary LCM–Cesàro close returns to every radix and ends with an exact integer-orbit contradiction. Thus the unresolved part of Problem #257 is the reciprocal-divergent case. Novelty and priority are unassessed, and this proof is not identified with Erdős’s omitted coprimality-free argument. The development also retains the classical full-support theorem and proves irrationality for every nonzero eventually periodic sequence of nonnegative rational weights.
...
**Arithmetic.** Every infinite reciprocal-summable support gives an irrational series at every integer base $`b\ge2`$; this contains the classical pairwise-coprime theorem as a special case. The full-support series and all nonzero eventually periodic nonnegative rational weight sequences are also irrational. Finite subsums have exact denominator order. **Geometry.** The achievement set is compact, perfect, nowhere dense, and has measure $`1`$, with an exact measure law after restricting exponents. **Open boundary.** The target values $`1/2`$ and $`1/21`$ remain undecided; any universal counterexample must have divergent reciprocal mass, and the geometric classification does not decide the arithmetic nature of individual subsums.
...
Section <a href="#sec:period" data-reference-type="ref" data-reference="sec:period">2</a> develops the arithmetic of Theorem <a href="#res:period" data-reference-type="ref" data-reference="res:period">3</a>. Section <a href="#sec:forced" data-reference-type="ref" data-reference="sec:forced">3</a> collects what rationality would force on an arbitrary infinite support, which is the part of this note that quantifies over every support rather than sampling. Section <a href="#sec:map" data-reference-type="ref" data-reference="sec:map">4</a> places Theorem <a href="#res:reciprocal-summable-support" data-reference-type="ref" data-reference="res:reciprocal-summable-support">2</a> beside representative supports already known to give irrational values, grouped by the argument that reaches them. Section <a href="#sec:squarefree" data-reference-type="ref" data-reference="sec:squarefree">5</a> treats the squarefree support, whose values are known at every power-of-two base and which two of the arguments used here provably cannot reach at any even base. Section <a href="#sec:geometry" data-reference-type="ref" data-reference="sec:geometry">6</a> gives the unrestricted and support-restricted topology and measure classifications, followed by the exact $`1/2`$ and $`1/21`$ membership criteria and unresolved branches. Section <a href="#sec:open" data-reference-type="ref" data-reference="sec:open">7</a> states what remains.
...
**Theorem 9** (squarefree divisor incidence). *For every $`n\ge1`$ the number of squarefree divisors of $`n`$ is $`2^{\omega(n)}`$, where $`\omega(n)`$ is the number of distinct prime factors, and hence
``` math
...
**Corollary 10** (joint power-of-two-base theorem). *For every $`h\ge1`$, the $`h+1`$ numbers
``` math
...
```
Thus the data $`N,K,L,C`$ may depend on $`q`$. The parity of Theorem <a href="#res:sfcount" data-reference-type="ref" data-reference="res:sfcount">9</a> refutes both alternatives for the squarefree incidence sequence at every even base.

<div id="res:blind" class="corollary">

**Corollary 11** (neither divisibility-first hypothesis has an instance). *For the squarefree support $`A_{\mathrm{sf}}`$ and every even base $`b\ge2`$, neither the digitwise nor the carry-aware block-certificate hypothesis holds. Both fail already at precision $`q=b^2`$.*
...
Corollary <a href="#res:blind" data-reference-type="ref" data-reference="res:blind">11</a> is a statement about two arguments failing on a value that Corollary <a href="#res:sfsettled" data-reference-type="ref" data-reference="res:sfsettled">10</a> shows to be irrational. The question it raises is therefore not whether the value can be reached, but what the failure is a property *of*. It is a property of where the support starts.
...
**Theorem 12** (geometry of $`\mathcal A`$). *$`\mathcal A`$ is compact, perfect, totally disconnected and nowhere dense, and has Lebesgue measure $`1`$.*
...
**Theorem 13** (volume dichotomy for every allowed support). *For every $`J\subseteq\mathbb{N}`$, exactly one of the following measure formulas applies:
``` math
\begin{array}{ll}
...
A rational point of $`\mathcal A`$ attained by an infinite support refutes Problem <a href="#res:problem" data-reference-type="ref" data-reference="res:problem">1</a>. The distinguished candidate is $`1/2`$, and Theorem <a href="#res:half" data-reference-type="ref" data-reference="res:half">14</a> gives a self-contained pair of exact alternatives.
...
**Theorem 15** (positive skips characterise the half-value). *The condition `CofinalPositiveHalfGreedySkips` holds if and only if $`1/2\in\mathcal A`$.*
...
**Theorem 17** (the sharp local two-adic obstruction). *Let $`p,D,q`$ be positive odd integers. If*
...
**Theorem 18** (orthogonal-petal sunflower criterion). *Let $`A\subseteq\mathbb{N}`$ support an `OrthogonalPetalBouquet` and satisfy `SunflowerForcedSlotTailSelection`. Then*
...
The hard mechanism is the persistent-reduced-modulus averaging argument: the selector supplies carries, and the support-series theorem turns that supply into irrationality. The exact formal consumers are [forced carry supply](https://github.com/wcook04/plectis-lean-erdos249-257/blob/3506d189a2660213ec0e53ee29360a464007b8a1/Erdos249257/SupportSunflowerDichotomy.lean#L531) and [irrationality endpoint](https://github.com/wcook04/plectis-lean-erdos249-257/blob/3506d189a2660213ec0e53ee29360a464007b8a1/Erdos249257/SupportSunflowerDichotomy.lean#L540). The source definition of the selector makes the missing analytic obligation explicit at [uniform tail selector](https://github.com/wcook04/plectis-lean-erdos249-257/blob/3506d189a2660213ec0e53ee29360a464007b8a1/Erdos249257/SupportSunflowerDichotomy.lean#L406).

<div id="res:compositedefect" class="theorem">

**Theorem 19** (the composite-dilation defect). *Let $`A\subseteq\mathbb{N}`$ and let $`a,x\in\mathbb{N}_{>0}`$ with $`a\in A`$. Define the foreign-divisor defect
``` math
...
Thus a composite bouquet cannot be analysed by copying the prime formula until this explicit foreign channel has been budgeted. This theorem provides that local budget only; it neither bounds defects along an infinite orbit nor supplies the uniform tail selector required by Theorem <a href="#res:sunflower" data-reference-type="ref" data-reference="res:sunflower">18</a>.
...
1.  **Problem <a href="#res:problem" data-reference-type="ref" data-reference="res:problem">1</a> itself**, now only for infinite $`A`$ with $`\sum_{a\in A}a^{-1}=\infty`$. Section <a href="#sec:forced" data-reference-type="ref" data-reference="sec:forced">3</a> constrains every hypothetical counterexample without excluding one. Every counterexample would have unbounded scaled-tail states, sublogarithmic divisor-coverage gaps and an admissible Boolean–Möbius scaled-tail sequence. No contradiction among the applicable constraints is known.

2.  **Membership of $`1/2`$ in $`\mathcal A`$.** Theorem <a href="#res:half" data-reference-type="ref" data-reference="res:half">14</a> makes this exactly equivalent to infinitely many greedy skips, and to a fatal gap on the other side; neither is proved. A proof of non-membership would take the form of one finite fatal gap.
...
For scope, all squarefree values at power-of-two bases, jointly in each finite family, are settled by Corollary 1.2 and Example 1.1 of  \[duverneytachiya, author-preprint p. 4\] (Corollary <a href="#res:sfsettled" data-reference-type="ref" data-reference="res:sfsettled">10</a>). Their cited corollary does not cover bases that are not powers of $`2`$, and this note makes no global open-status or priority claim for those values.

<a id="statements-and-declarations"></a>

# Statements and declarations

This manuscript is authored exposition, not proof authority. The linked Lean snapshot is authoritative only for its exact propositions; kernel checking establishes that a proposition was proved, not that it is interesting, novel, or sufficient. The displayed proof of Corollary <a href="#res:blind" data-reference-type="ref" data-reference="res:blind">11</a>, the derivation of Corollary <a href="#res:sfsettled" data-reference-type="ref" data-reference="res:sfsettled">10</a> from  \[duverneytachiya, Cor. 1.2 and Ex. 1.1, author-preprint p. 4\], and the general finite-change argument (from the two checked prefix lemmas) are expository arguments rather than named checked statements.
