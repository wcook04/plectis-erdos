...
A separate sufficient condition family is stated in terms of growth alone. Erdős proved irrationality of $`\sum_n1/a_n`$ for increasing integers with $`a_n\ge n^{1+\tau}`$ for some $`\tau>0`$ and every $`n`$, together with $`\limsup_na_n^{1/2^n}=\infty`$ \[erdos1975, Theorem 1\]. At $`a_n=2^{c_n}-1`$ the polynomial hypothesis is automatic, since $`c_n\ge n`$ gives $`a_n\ge2^n-1\ge n^{3/2}`$ for every $`n\ge1`$, and $`\log_2(2^{c_n}-1)`$ differs from $`c_n`$ by less than $`1`$, so the growth hypothesis reduces to $`\limsup_nc_n/2^n=\infty`$. The criterion therefore settles exactly the supports with $`\limsup_nc_n/2^n=\infty`$, at every integer base. That class and the reciprocal-summable class of Theorem <a href="#res:reciprocal-support" data-reference-type="ref" data-reference="res:reciprocal-support">1</a> are incomparable. The powerful integers are reciprocal summable and their increasing enumeration satisfies $`c_n\asymp n^2`$, so $`c_n/2^n\to0`$. In the other direction, take blocks of consecutive integers, the $`k`$th block being the $`4^{n_k}`$ integers from $`4^{n_k}`$ to $`2\cdot4^{n_k}-1`$ starting at index $`n_k`$, with $`n_{k+1}=n_k+4^{n_k}`$; then $`c_{n_k}/2^{n_k}=2^{n_k}\to\infty`$ while each block contributes reciprocal mass tending to $`\log2`$, so $`\sum_{a\in A}1/a`$ diverges. The extension of this lane to products of consecutive terms \[bkkkz2026\] settles Erdős Problem 1051 at the golden-ratio exponent and adds nothing beyond the $`d=1`$ case used above. Its rational-valued counterexamples are built by interval filling, which selects each term from a full discrete interval $`[\beta_n,\gamma_n]\cap\mathbb{N}`$ with $`\beta_n/\gamma_n\to0`$, so that consecutive choices differ by one and the attainable sums cover an interval. The strict tail inequality of Section <a href="#sec:geometry" data-reference-type="ref" data-reference="sec:geometry">7</a> makes the Mersenne achievement set totally disconnected, so no interval is ever attained and interval filling is unavailable here, on terms and on exponents alike. This retires a method; it does not decide the problem.
