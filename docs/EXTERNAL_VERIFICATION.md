...
**What this is.** Plectis is an AI-assisted research system. This public surface shows one checked frontier for each of eight Erdős problems, seven of them open. For each programme, read the question, the exact checked object, and the remaining open step before opening the technical registry.
...
**Question.** For a monic polynomial whose roots lie in the open unit disc, must two roots be joinable by a curve of length less than two inside the open unit lemniscate? Answer: no (ani, 7 September 2026, degree seven); the instance s = 10^-6 is formalised here.

**Checked frontier.** The concrete degree-seven polynomial f (ani's family at s = 10^-6, exact Gaussian-rational coefficients, rho = 1 - 10^-96) is monic with seven distinct zeros of modulus rho < 1, and every continuous path in {|f| < 1} joining two distinct zeros has total variation greater than two (Counterexample.Assembly.erdos1041_counterexample; axioms propext, Classical.choice, Quot.sound).

**Open boundary.** The asymptotic form of ani's theorem (for all sufficiently small s); only the instance s = 10^-6 is formalised.

**Read.** [Programme paper](../paper/1041/erdos-1041-lemniscate-newton-flow.pdf) · [Lean source](../lean/ErdosProblems/Erdos1041/Counterexample/Assembly.lean)
...
<code>ErdosProblems.Erdos1041.Counterexample.erdos1041_counterexample</code>
