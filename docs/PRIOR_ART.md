...
| G. Martin, [*Simultaneous inequalities among values of the Euler \(\varphi\)-function* (arXiv, 2006)](https://arxiv.org/abs/math/0603053) | **Direct antecedent for the independence step behind the totient \(k\)-kernel basis and exact finite-level rank.** *Source statement*: Theorem 1 — for positive affine forms \(a_in+b_i\) with \(a_ib_j\ne a_jb_i\) whenever \(i<j\), and for **any** constant \(C>0\), the set of \(n\) at which every consecutive ratio \(\varphi(a_in+b_i)/\varphi(a_{i+1}n+b_{i+1})\) exceeds \(C\) has positive lower density; the hypotheses are symmetric, so any prescribed ordering may be arranged. *Assumptions*: positive integer slopes, integer intercepts, pairwise non-proportionality. *Conclusion*: positive-lower-density simultaneous ratio separation. *Specialisation map*: take the forms to be the dyadic (or \(k\)-adic) sections \(k^jn+r\), whose pairwise cross-determinants \(k^{j_m}r_i-k^{j_i}r_m\) are nonzero exactly when the sections are distinct after reduction. *Proof mechanism*: given a vanishing rational combination with some \(c_s\ne0\), order \(L_s\) first and take \(C>\sum_{i\ne s}\lvert c_i\rvert/\lvert c_s\rvert\); that term then dominates the rest in absolute value, so no such relation exists. *Residual difference*: Martin does not mention \(k\)-kernels, \(k\)-regularity, or any rank formula. What remains local is the dilation identity, the resulting exact dimension \(k^e+1\), the two explicit bases, and the complete relation normal form. | The independence mechanism is Martin's and no priority is claimed for it. Recorded 11 August 2026, after an external review located this source; the earlier posture that no prior source supplied the mechanism is withdrawn. The remaining search assessment — that no source records the \(k\)-kernel corollary itself — is a dated search result, not a novelty theorem. Coons's non-\(k\)-regularity theorem already implies the \(\mathbb{Q}\)-span of the full kernel is infinite-dimensional, since for integer-valued families finite generation of the \(\mathbb{Z}\)-module and finite \(\mathbb{Q}\)-dimension are equivalent; the local addition is the exact growth, not infinite-dimensionality. |
