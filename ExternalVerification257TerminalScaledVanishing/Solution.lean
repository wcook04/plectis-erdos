...
noncomputable def supportCoeff (A : Set ℕ) (n : ℕ) : ℕ :=
  Erdos249257.supportCoeff A n

def affineBinaryOrbit (a : ℕ → ℤ) (u0 : ℤ) : ℕ → ℤ :=
  Erdos249257.affineBinaryOrbit a u0

noncomputable def integerHalfCarry (A : Set ℕ) : ℕ → ℤ :=
  Erdos249257.HalfCarryReachability.integerHalfCarry A

abbrev HalfWord (N : ℕ) := Fin (N + 1) → Bool

def wordSupport {N : ℕ} (a : HalfWord N) : Set ℕ :=
  Erdos249257.HalfCarryReachability.wordSupport a
