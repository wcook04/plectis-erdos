...
## Correction, 2026-09-06 (Erdős 249 corrective-integration pass)

This block corrects one statement above. Nothing earlier is edited or removed.

**The "usable target" in `Why every uniform cap had to die` is impossible, not
merely unproved.** That section states

> `‖v·2^c·S‖ ≥ c^{−A}` for some fixed `A > 1`, uniformly in odd `v`,

and says it "settles the supply". No such bound can hold. If `S` is irrational
then `2^c S` is irrational for every `c`, and for irrational `x` the set
`{v·x mod 1 : v odd}` is dense in `[0,1)`. So at every fixed `c` the infimum of
`‖v·2^c·S‖` over odd `v` is `0`, and no positive lower bound uniform in odd `v`
exists at that `c`, let alone one of the shape `c^{−A}`. Taking `v` large defeats
it before `c` is ever varied.

The sibling file already had this right. `research_packet.json`, field
`claim_ceiling`, reads: "A lower bound `‖v*2^c*S‖ >= c^-A` uniformly over all
positive odd `v` is not merely unproved but impossible for irrational `S`,
because the odd multiples of `2^c*S` are dense modulo one." The note and the
packet contradicted each other, and the packet is correct.

**What survives.** Result 3 above is unaffected: it is a measurement over a
finite range of `c` and a finite set of `v`, and its consequence
`S ≠ a/(v·2^c)` for `c ≤ 200000` and the eight tested odd `v` stands, with the
stated margin `2^{−19.11}` against a truncation error 180 binary orders below
it. The refutation of the uniform-cap programme survives too, for the same
reason: it is a statement about caps, not about a uniform-in-`v` lower bound.

What does not survive is the sentence naming a uniform-in-`v` measure bound as
the usable target. A per-`v` statement is the most that can be asked: for each
fixed odd `v`, a bound `‖v·2^c·S‖ ≥ c^{−A(v)}` is a classical irrationality
measure question about the explicit dyadic series `v·S`, and the supply
consequence `H ≤ (A+1)log₂c + log₂v + O(φ(v))` needs the constant to be
uniform in `v`, which is exactly the part that is unavailable. So the
translation to a measure question is real, and it does not reduce the supply
to a single classical statement.

**Downstream.** The 2026-09-06 deep inventory
(`docs/strategy/staging/rapid_breakout/audit/problem_249_deep_inventory_20260906.json`,
result id `supply_delay_diophantine_translation`) records this note's version
as a weight-5 result. That entry inherits the error. The corrective-integration
pass therefore declined to promote it to `docs/claims.json`, to either public
paper, or to the claim frontier, and recorded the reason in
