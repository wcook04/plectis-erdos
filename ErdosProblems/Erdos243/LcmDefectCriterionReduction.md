...
UPDATE 2026-09-07 (Type B r3): the implication `limsup Z_n < ∞ ⇒` eventual
Sylvester, under the original quadratic-limit and rationality hypotheses, is
the bounded-excess case of `weighted_lcm_record_excess` (live paper
`res:lcmbounded`). Theorems U1-2 and U1-3 below remain correct as a literature
map: at level `≤ 0` the LCM criterion is the product criterion after primitive
reduction, and on cancellation-free surviving blocks the LCM defect is
trivial. The r3 composition does not claim a new parent solution.
