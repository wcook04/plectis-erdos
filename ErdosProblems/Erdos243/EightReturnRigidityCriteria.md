...
feedback system. Type B r2 Proposition H (`1/σ`) is this statement; not a new
landing.
...
1. **`primitive_record_two_unit_rigidity` is a claim-frontier row id and not a
   Lean declaration.** Section 3 above names the real declarations correctly.
   The deep inventory of 2026-09-06 and the first result audit both cite
   `primitive_record_two_unit_rigidity` at `PrimitiveRecordBarrier.lean:435`; at
   that line the declaration is `recordRiseTwo_sylvesterNext_eventually`. The
   companion declarations are `odd_record_cut` (:209),
   `primitive_valuation_no_drop` (:59), `protectedPrimePower_persists` (:145),
   `exists_oddMultiple_trapHeight` (:291) and
   `numerator_bounded_of_oddPrimePower` (:332). Any surface citing the row id as
   a declaration is wrong.

2. **Section 12 records receipts for three of the six checkers it names.** The
   receipts paragraph covers sections 1, 3 and 4. `check_feedback_word_realizability.py`
   (section 5), `erdos243_legendre_locking_probe.py` (section 7) and
   `erdos243_good_prime_wall_probe.py` (section 8) are named as the verifiers
   but no run numbers are recorded for them in this note. The frontier rows
   carry `nine minimal finite-field certificates` for section 5 and
   `24 engineered landings, 0 counterexamples` for section 8; those numbers have
   no receipt paragraph here. Treat the section 5, 7 and 8 replay counts as
   recorded on the frontier rows and not in this note.

3. **What Theorem 13 and Corollary 14 supersede.** The relation paragraph in
   section 4 says Corollary 14 strengthens `slow_negative_part_rigidity` on two
   axes. The supersession is now recorded on disk in both directions, and it
   also covers `counterexample_frontier_profile`. Supersession retires the row
   and keeps the mathematics: `SlowNegativePartRigidity.md` Theorem 4 states its
   hypothesis directly on `E_n`, which Theorem 13 does not, and the two are
   readable side by side. See the correction block appended to that note on the
   same date.

4. **Negative-knowledge labels for section 7 and section 11.** The unconditional
   square transport with the Legendre defect charge is a
   `proved_class_obstruction` and not progress toward the endpoint: greedy
   Legendre-admissible walks have rises of order `u`, the landed-prime version is
   vacuous because a landed prime divides every later `E`, and the charge is
   identically zero on clean steps, which is where the surviving regime lives. It
   is also superseded in reach by `saturated_square_transport`. Result 10.2 is a
   `counterexample_to_proposal`: it refutes the implication section 3 named as
   the single missing ingredient, since clean rises cost nothing.

5. **Public placement.** Theorem 13 and Corollary 14 are now stated in the
   public note as Theorem `res:recorddichotomy` and Corollary
   `res:loglogboundary` in
   `R/paper/erdos-243-reciprocal-tail-rigidity.tex` section `sec:records`, with
   the receipt numbers `521026757646`, `1.8116` and `sigma^{-1} = 2` carried
   into the frontier row rather than into the paper.

Evidence class for this block: every quoted statement, declaration name and line
number was read this pass from the file it is attributed to. No build was run.
