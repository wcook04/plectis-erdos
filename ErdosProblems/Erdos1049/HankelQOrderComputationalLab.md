...
## Correction block, 2026-09-06 (appended; nothing above is rewritten)

Corrective-integration pass for #1049. The mathematics of this lab is unchanged. Three sentences
above carry a scope or an evidence status that the files named below do not support.

**(H1) "Hidden normalized-Hankel `q`-order is decisively eliminated as a source of the missing
cubic gain."** The precise label is `proved_class_obstruction`, and its scope is the ordinary
proof, not the kernel. The all-rank equality `ord_q V_N^* = N(N-1)(2N-1)/6` with leading
coefficient `(N!)^2 (N+1)!/2^N` is an ordinary mathematical argument. What is kernel-checked in
`AdelicHeightBridge.lean` is the all-depth associated-graded recurrence
(`hankelAssociatedCoeff_eq_reciprocal`, line 200), the initial monomial of transformed row `j = 1`
in every column with order exactly `l+1` and coefficient exactly `-6`
(`zudilinTransformedNormalizedMoment_one_initialMonomial`, line 1422), and the assembly of the two
closed forms (`zudilinSharpHankelOrderAndCoeff_algebraicAssembly`, line 1753). Read at both R HEAD
`8a28b2521e` and in this tree this pass; the two files are byte-identical.

**(H2) The row `j >= 2` boundary has moved, and the moved part lives in one module only.**
`ZudilinSharpHankelCoefficient.lean` in this tree proves the row `j = 2` initial monomial
`18 q^{2l+3}` (`zudilinRowInitialMonomial_two`, line 1357), the discharge of the row hypothesis for
`j <= 2` (`zudilinRowInitialMonomial_of_le_two`, line 1367), and the conditional sharp assembly
(`zudilinSharpHankelOrderAndCoeff_of_rowInitialMonomial`, line 1384). Rows `j >= 3` are open, so the
Lean identification of `V_N^*` itself with the assembled leading matrix stays conditional on that
row hypothesis. That module is present in this tree only: it is absent from the release tree
`public-source-redacted://release-final-20260905` and from the Palomar cut, it is
not imported by `ErdosProblems/Root.lean`, no declaration of it is named in
`ErdosProblems/AxiomAudit.lean`, and no external-verification package wraps it.

**(H3) "The focused `AdelicHeightBridge` build and final serialized replay pass."** That sentence
reproduces the entry's own metadata string, which
`state/formal_math/claim_frontier_v1.json` records for
`erdos1049.adelic_height_bridge` at the `source_replay` rung with state
`unverified_by_this_builder`, alongside the declared blockers "terminal positive and
deliberate-negative Comparator receipts pending", "public Lean source and portfolio-ledger
registration pending" and "public immutable source identity pending". No Comparator run exists for
any 1049 entry. The focused build and serialized replay are `reported_prior`; the terminal
Comparator is pending.

**(H4) Numbers re-verified this pass.** The table of orders `0, 1, 5, 14, 30, 55, 91` and leading
coefficients `1, 6, 108, 4320, 324000, 40824000, 8001504000` for `N = 1..7` was checked against the
two closed forms by hand: `7 * 6 * 13 / 6 = 91` and `(3!)^2 * 4! / 2^3 = 108`, and every other row
agrees. The receipt is
`state/formal_math/erdos257_period_noncollapse/erdos1049_hankel_qorder_receipt.json`, present on
disk this pass. The Vandermonde order `N(N-1)(N-2)/6` quoted above is the order of
`det(q^(jl))` alone and is correct as written; it is a different quantity from the determinant's
total order `sum_(j<N) j^2 = N(N-1)(2N-1)/6`, and the two are not to be conflated.
