...
* Convergence of `(K_n − W_n)/n²` to `C_0` is `o(1)`: the `34n` term in
  `M_n` contributes `O(1/n)`, while the `N log N` term of `Σφ` contributes
  `O((log n)/n)`.  The region theorem uses only this `o(1)` error.  At the
  razor-thin base 31/4 the
