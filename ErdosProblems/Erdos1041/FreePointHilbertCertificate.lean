...
`S_m ≤ m`, with equality only at the origin. Ordinary companion notes discuss
small-cardinality cases and critical-value reductions. The scalar certificates in
`FreePointFP4Complete.lean` do not by themselves formalise the complete FP4 theorem;
this module makes no claim that FP3, FP4, FP5, or the full torus reduction has been
proved in Lean. Its exact geometric and central conclusions are listed below.
...
## How the central series inputs are supplied

This module states `freePointSum_central_le_of_series` with explicit logarithmic
energy and row Cauchy--Schwarz hypotheses. The companion `LogKernelEnergy.lean`
proves the convergent power-sum energy identity, and `LogKernelVariance.lean`
proves the row bound by finite Cauchy--Schwarz followed by passage to the limit.
`LogKernelCentralCertificate.lean` packages these facts for the actual disk points.

`FreePointCentralCompletion.lean` applies these producers to this module's theorem:
for every positive `m`, the sole geometric condition `‖c_j‖ ≤ √(1 - exp(-2))`
gives `S_m ≤ m`. It needs no additional energy or variance assumption. This proof
does not construct the abstract infinite-dimensional feature map, which is not
needed for the finite-point conclusion. It also does not assert the equality case
or an inequality for arbitrary points in the full unit disk.

The diameter and geodesic theorems above remain independent results. The uniform
central radius is approximately `0.929873`; no priority claim or comparison with
unverified numerical thresholds is made here.
...
`Lⱼ = -log(1 - |cⱼ|²)`. These are series facts about the power sums `p_ν = ∑ₖ cₖ^ν`.
The companion `LogKernelCentralCertificate.lean` proves both for actual disk points,
and `FreePointCentralCompletion.lean` discharges them in the central-radius theorem.
This declaration retains its explicit inputs for reuse. -/
