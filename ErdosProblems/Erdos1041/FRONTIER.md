...
**`STRONGEST_RESULTS.json` exists.** The 2026-09-06 integration pass located it
in the release candidate at
`research_corpus/Erdos1041/STRONGEST_RESULTS.json`: 61828 bytes, 45 rows,
`status: open`, `source_checkpoint: 867638684907b4ebd4ab2ef7b5de22dafda71e36`,
and listed in `research_corpus/Erdos1041/CORPUS_MANIFEST.json` under
`strongest_result_map` with `result_count: 45` and sha256
`57e5d7cdfcb443a68039c0be4bfd9ea01278f50bc077396adc5cc7d3cefda3bf`.

This matters because a second surface says otherwise.
`docs/formal_math/palomar/longitudinal_truth_2026_09_01.md` line 95 records for
#1041 that "`STRONGEST_RESULTS.json` does not exist and never did;
`FRONTIER.md` warns readers off a phantom". Both halves are wrong. The file
exists in the release candidate, and the warning at the head of this note is a
staleness warning about activation lists, never a nonexistence warning. That
longitudinal-truth line should be corrected by its own owner; it is outside
this directory and is not edited here.

**The shipped map is stale in one named way.** Its row
`first_merge_critical_value_separation_certificate` still prints "Exact
convenient thresholds are `S=4` for `n>=3`, `S=3` for `n>=4`, and `S=2` for
`n>=7`". The sharp degree is `n >= 6`, proved by
`erdos1041.critical_value_separation_sharp_constant` and kernel-checked in the
threshold declaration `firstMerge_exact_convenient_thresholds`. The claim
frontier already carries `n>=7` in `stale_forms` and bands the old row `S`; the
public map has not been re-emitted since. Reconcile the map rather than
reconstructing it: 44 of its 45 rows are unaffected by this correction.

**Ladder ordering as of today.** The all-degree low-critical constant is
`13/25` (`AngularBudgetLowCriticalClosure.md`, Theorem A''), which dominates
`199/1000`, `9/25` and `2/5` on threshold while consuming rather than
replacing their mechanisms, and which is the better statement than the
fixed-degree arm from `n = 6` upward. See the correction appended to
`FixedDegreeClusterSeparationClosure.md` on the same date.
