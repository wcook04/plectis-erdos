#!/usr/bin/env python3
"""Exact five-sheet replay for the recentered positive row-23 carrier cell."""
...
SPEC = importlib.util.spec_from_file_location(
    "erdos1041_positive_row18_source_for_row23",
...
CENTER = ROW18.NEW_CENTER * Fr(207, 200) ** 5
RADIUS = Fr(1, 2550)
LEFT23 = Fr(
    936478669624778847504089720483029872571994670075770758193,
...
report(
    "PR23 exact recentered endpoints and R22 overlap",
    new_row[0].left == LEFT23
    and new_row[0].right == RIGHT23
    and old_row[0].right == R22
    and LEFT23 < R22 < RIGHT23,
    f"[{LEFT23},{RIGHT23}], overlap={R22-LEFT23}",
)
report(
    "PR23 five Krawczyk self-maps",
    all(
...
report(
    "PR23 regular oriented five-sheet row",
...
report(
    "PR23 upper disjoint exhaustive row",
...
report(
    "PR23 weak-sheet exact sign", upper < 0,
...
for sheet in range(1, 5):
    ...
    report(
        f"PR23 robust-sheet exact sign {sheet}", upper < 0,
...
for sheet, (old_cell, new_cell) in enumerate(zip(old_row, new_row)):
    ...
    report(
        f"PR23 exact branch overlap sheet {sheet}",
...
print("ALL ROW-23 CHECKS PASSED")
