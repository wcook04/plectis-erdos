...
Appended, not edited in place. The proof recorded in this section is sound; the
heading claims a wider death than the proof delivers.

**Defect (scope).** The heading reads "Consequence -- the producer hunt is
dead", and the body proves something narrower and exactly stated one paragraph
later: `hclose` is false for A = the positive integers, so no producer can
establish `hclose` there, while the value at that support is irrational by
Erdős 1948. That is a proved obstruction for one named criterion. It is not a
statement about producers in general, and it is not a statement about any other
criterion.

**Corrected statement.** `hclose` is sufficient and not necessary, and it is
false at A = the positive integers. Therefore no strengthening of the producer
can extend the `hclose` route past reciprocal-summable supports. Reaching the
reciprocal-divergent regime requires a different criterion.

**Label.** `proved_class_obstruction`, scoped to the criterion `hclose`. The
scope matters, because the corpus now has criteria that do reach into the
reciprocal-divergent regime by other means: the divisibility-weighted
criterion, the sub-log-star reciprocal-mass criterion, and the
variable-exponent positive fractional covers, all with ordinary proofs and no
formal check, in section 2 of
`paper/erdos-257-mersenne-support-subseries.tex`. Their existence is consistent
with this obstruction and shows why the general reading of the heading is wrong.

**Unchanged.** The sentence "that is not an artifact of how hard the producer
was to prove, it is the criterion's true reach" is correct as written, and it
is the sentence the heading should have used.
