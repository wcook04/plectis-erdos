...
The finite repair-window argument is separately implemented in
[BatchReturnSynthesis.lean](BatchReturnSynthesis.lean); its build receipt, rather
than this note, controls its formal status. Complete retained analytic arguments
are in [EightReturnAnalyticProofs.md](EightReturnAnalyticProofs.md).

The batch authority and per-return dispositions are recorded in
[eight_return_assimilation_20260905.json](eight_return_assimilation_20260905.json).
The original bytes remain in the private intake capsule. No agreement between
returns is used as proof. In particular, four returns repeat essentially one
prime-progression obstruction and do not constitute four independent results.
...
The verifier [selected_phase_density_audit.py](scripts/selected_phase_density_audit.py)
...
## Correction, 2026-09-06 (Erdős #257 corrective-integration pass)

Appended, not edited in place. The text above stands as written; this block
records where it is wrong and what the corrected statement is.

**Defect (numeric, and a scope error inside an equivalence).** The sentence at
line 157 reads "Membership of 1/21 is equivalent to upper density at least 2/5
for this new restricted set". Two things are wrong with it.

1. The certified threshold for the twelve-anchor set is 337/840, not 2/5. The
   preceding sentence gets this right: 1011 favorable classes modulo 1260 give
   conditional upper density at least 337/840. Numerically
   337/840 = 0.401190..., and 2/5 = 0.4, so 2/5 is the rounded-down reading of
   the certified bound and is strictly weaker.
2. Because the sentence is an equivalence, the weaker constant makes it false
   as stated. Membership implies upper density at least 337/840, which implies
   upper density at least 2/5; the converse implication from 2/5 back to
   membership is not available, since 2/5 does not imply 337/840. An
   equivalence has to carry the exact threshold.

**Corrected statement.** Membership of 1/21 in the Mersenne achievement set is
equivalent to cofinal divergence on the 1011 favorable phase classes modulo
1260, equivalently to upper density at least 337/840 for that restricted set,
equivalently to its infinitude or to divergent reciprocal mass of those rows.
The older 2/7 bound belongs to the two-anchor restricted set, which is a
different and smaller family; that sentence in the text above is correct.

**Label.** Both bounds are conditional on the divergence hypothesis. The right
negative-knowledge label for this row is conditional_reduction. It is not an
unconditional density theorem and it decides nothing about 1/21.

**Receipts.** The 337/840 figure and the 1011-class mask are in this file at
lines 148 to 158 and in the assimilation receipt named there;
`state/formal_math/claim_frontier_v1.json::claims["erdos257.twenty_one_two_anchor_divergence_density"].statement`
carries the same value, while that row's `title` printed the weaker 2/5 and is
corrected by
`docs/strategy/staging/rapid_breakout/audit/patches/frontier_257.json`.
`paper/erdos-257-mersenne-support-subseries.tex` in the release candidate reads
"conditional upper density at least 337/840>2/5", which is correct.

---

## Correction, 2026-09-06 (Type B revision assimilation)

Appended, not edited in place. The 2026-09-06 Type B revision asked to lead the
note with the weighted prime-part criterion. That request is rejected for the
live Palomar paper: the Lean-checked hereditary theorem is reciprocal
summability at every integer base, which is also the coprimality-free statement
Erdős already made. Weighted and variable-exponent criteria remain ordinary
enlargements, recorded in this file and in
[VariableExponentCoverSeparation.md](VariableExponentCoverSeparation.md).
Cover-independent means, the support \(A^\dagger\), and the pairwise-coprime
obstruction enlarge the variable-exponent remainder; they do not replace
