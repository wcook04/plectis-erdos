...
The September return assimilation changes the disposition of one example:
the reviewed all-base prime-power theorem, together with fixed dilation,
does prove irrationality for the support $2\mathbb P$. See
[the full analytic proof](EightReturnAnalyticProofs.md) and
[the current synthesis](EightReturnSynthesis.md#6-the-four-admitted-analytic-families-and-their-boundaries).
The algebraic and covariance counterexamples below remain valid: they refute
the naive transfer of the prime-local identity, not irrationality of that
particular dilated support. The analytic extension remains an ordinary proof.
