...
History above is left as written. One number and one label are corrected here.

**Number.** The line reading "the packet's certified `q > 10^{12039}`" is one
exponent too large. The receipt
`verification/erdos68-continued-fraction.json` in the release-final tree
(HEAD `8a28b2521e`) records
`denominator_lower_bound_power_of_two_exponent = 39990` and
`strict_decimal_power_lower_bound = 12038`, so the certified statement is
`q >= 2^39990 > 10^12038`. The claimed pair `2^39991 > 10^12039` is also false
on its own terms: `39991 * log10(2) = 12038.49`. Read the paragraph with
`10^12038`, and read the closing phrase as roughly 12,038 orders of magnitude
per layer. The same stale exponent appears at
`docs/formal_math/palomar/longitudinal_truth_2026_09_01.md:18`,
`docs/formal_math/palomar/prior_art_adjudication_2026_09_02.md:1638`, and in
`state/formal_math/claim_frontier_v1.json#erdos68.certified_cf_denominator_exclusion_10e12039`;
patches for those surfaces are at
`docs/strategy/staging/rapid_breakout/audit/patches/frontier_68.json`.

**Label.** "The route closes quantitatively" and "what kills this particular
assembly is an exponent race" are correct about this assembly and are wider
than the evidence when read as a statement about the coordinate. The precise
label for what is established here is `counterexample_to_proposal`: one named
Shidlovskii-type assembly is refuted by an exponent comparison. The function
class, the `E`-function coordinate, and the applicability of
Siegel--Shidlovskii are untouched, and the note already says so.
