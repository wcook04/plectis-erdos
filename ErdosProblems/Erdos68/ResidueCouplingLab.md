...
The sentence "the packet's own claim ceiling then closed that route as
asymptotically impossible" overstates the evidence class by one step, and the
phrase "their route's death" inherits the overstatement.

What is established: the Lean-checked ceiling
`factorialBlock_unique_owner_projection_lcm_ceiling`
(`ErdosProblems/Erdos68/EndpointWeightedPrivateSupport.lean:6524` at
release-final HEAD `8a28b2521e`) proves that any one-owner closing inequality
forces `(2p+1)L_p < 2p^2((2p-1)!)^2`. The comparison that makes this
impossible eventually uses `log L_p >> p^{4/3} log p`, which is an ordinary
derivation from Theorem 12 of Garaev, Luca and Shparlinski
(arXiv:math/0403422v1, p. 16). That derivation is not kernel-checked, and the
eventual source-to-ceiling comparison is not kernel-checked either. The claim
ceiling in `research_packet.json` already says so in its last clause.

Precise label: `proved_class_obstruction` against one-owner closing
certificates, with evidence class `ordinary_proof` for the asymptotic half and
`lean_kernel_checked` for the ceiling inequality. "Closed" and "death" should
be read as this obstruction and not as a kernel-decided exclusion. The
imported Luca--Shparlinski and Garaev--Garcia theorems survive independently
of the label, which is the point the section makes.
