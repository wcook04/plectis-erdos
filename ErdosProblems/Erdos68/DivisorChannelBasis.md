...
```
./repo-python formal_math/erdos257_period_noncollapse/ErdosProblems/Erdos68/scripts/check_divisor_channel_basis.py --quick
```
...
Round-6 companion (ordinary; Type B advisory): finite remote-support
optimisation \(\mu_{D,r}\) and the sharp example \(\mu_{2,4}=360\) live
in `FiniteSourceTailCertificate.md` (not inserted into the short-note
opening). Replay `scripts/check_finite_source.py`. Frozen R TeX was not
edited.
