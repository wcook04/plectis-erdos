...
```
./repo-python formal_math/erdos257_period_noncollapse/ErdosProblems/Erdos68/scripts/check_moment_saturation.py --quick
./repo-python formal_math/erdos257_period_noncollapse/ErdosProblems/Erdos68/scripts/check_moment_saturation.py --max-d 24 --limit 800
```
...
Round-6 finite-source companion (ordinary; Type B advisory): the live
short note now uses the quadratic theorem
\(g_D=\gcd(u_{D+1},\ldots,u_H)\) with \(H=D(2\ell-1)<2D^2\). The
envelope remains true and still supports the p-adic argument; it is no
longer the leading stopping proof. See `FiniteSourceTailCertificate.md`,
`scripts/check_finite_source.py`, and the uncompiled draft
`FiniteSourceTailCertificate.lean`. Frozen R long-record TeX was not
edited.
