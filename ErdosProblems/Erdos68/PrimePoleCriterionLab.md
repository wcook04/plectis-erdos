...
The first-layer residue identity is a specialisation of Louwsma–Martino,
Lemma 4.1 (arXiv:2309.07280v1, p. 10), which retains the full surviving
valuation. The factorial-gap examples and the Lean consumers remain distinct
from that general formula. A vanishing first-layer residue does not determine
the remaining exponent: the families `(4,12)` and `(4,20)` both have maximal
2-adic valuation two and a vanishing first-layer inverse sum, yet the
reciprocal sums are `1/3` and `3/10`.
