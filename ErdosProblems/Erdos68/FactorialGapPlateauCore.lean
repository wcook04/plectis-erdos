...
Lightweight core for the strict-successor recurrence, carry bounds, and exact
divisibility criterion.  Certificates live in `FactorialZeroPlateauCertificates`;
the supplement owns later results, and the facade re-exports all three modules.
-/
