...
If all-index escape from `W` is equivalent to irrationality, pointwise
`Z subset W` makes all-index escape from `Z` equivalent as well: missing `W`
implies missing `Z`, and rationality forces eventual membership in `Z`.
Pointwise narrower targets are not logically weaker cofinal statements.
They may be easier to prove by a particular argument. Restricting the lower
cylinder to primes is an additional arithmetic demand; it is only a sufficient
target in the current development. A generic counterexample (canonical digits
supported only at composite indices) is irrational and never makes the prime
lower-cylinder escape relative to the stated thresholds. That is not a
counterexample for the particular constant `S`.
