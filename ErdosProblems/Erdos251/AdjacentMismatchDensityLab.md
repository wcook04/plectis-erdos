...
satisfiable inside the computed range. **Finite observation only; cofinality
is not proved.**
...
The finite scan found many signed-window occurrences for the tested primes
and offsets. It establishes **no cofinality and no asymptotic lower bound**.
The producer is not decided as a theorem in the actual primes. The event
count remains an open analytic task. The polynomial countermodel still fails
to produce `Δ = ±2`; that is a statement about a different word.

A proof using the factorisation `P(A ∩ B) = P(A) P(B | A)` needs a lower
bound for the digit event *and* a lower bound for the window probability
conditional on that event, on the same scales. The identity is not a
decorrelation theorem and does not remove the joint-event obligation.
...
**Scope of "true in the actual primes".** The measurement covers 6,841,648
primes below `1.2 × 10^8` and offsets `h = 1..16`. It establishes occurrence at
every tested offset in that range, with density between `0.00418` and
`0.008248` and the last `h = 1` event at prime `119,995,753`. It is a finite
measurement over one range and it proves no cofinal statement. The band
rescaling by `log p` and its decelerating drift are evidence that the count
keeps growing; they are not a proof that it does. The phrase "true in the actual
primes" in "What this decides" should be read as "occurs at every tested offset
throughout the measured range", and the reading it excludes is the one it
actually excludes: the polynomial countermodel does not transfer to the primes.

**The named missing input is superseded.** "What this decides" names
Hardy–Littlewood `k`-tuple correlation of consecutive prime gaps at a fixed
offset as item (1) of the missing input. `FreePairStateCompressionLab.md` §1 and
§6 are later and weaker. Once the shift offset is free, the route needs, for
each modulus `t`, cofinally many pairs `M ≡ N (mod t)` with non-integral
certified tail difference, and the narrowest sufficient prime-theoretic input
recorded for the two-condition form is:

> two Polignac numbers differing by 2, both occurring infinitely often within a
> common index residue class mod `t`.

That is strictly less than `k`-tuple correlation at a fixed offset and is
adjacent to bounded-gap technology rather than to Hardy–Littlewood. Whether it
follows from known results is the open sub-question. Item (2), the
equidistribution statement for the dyadic tail, is unchanged, and the factorised
reading in "Two things fall out" is unchanged: the two factors can still be
attacked separately.

**The ±2 component may be redundant.** `FreePairStateCompressionLab.md` §2 gives
an ordinary proof that under the free-pair lattice the tail difference is an
integer whenever the modulus divides the offset, and an integer cannot lie in
`(1/2, 1)`, so the first component of the reduced event is already a
contradiction and the gap condition does no work. That derivation is ordinary,
unformalised and unchecked outside the 2026-09-06 audit read. It does not
retract anything measured here; it changes which component a proof has to
produce.

**Neither reading is a claim on #251.** The target irrationality is open, and
this file records a measurement about a producer, never a result about the
value of the series.
