...
For smooth numbers generated by three primes, the finite exponent model gives
exact strict counts and shell cardinalities.  At pure-power endpoints these
counts become explicit floor sums.  Iterating the associated carry recurrence
across a finite window then gives a residue obstruction: one escaping window
for each admissible reduced denominator rules out an everywhere-positive
bounded carry.

A cofinal escape condition is also recorded below, but it is stronger than
necessary; specializing its lower-bound parameter to zero supplies the single
window used in the contradiction.  The unresolved step for Erdős #269 is to
produce such escaping windows from the actual `{2,3,5}` shell multiplicities.
-/
...
for the strict inequality used by the floor-sum formula below. -/
...
/-- Restricted count at a pure `p`-power cutoff.  Its two-dimensional
floor-sum evaluation is established below. -/
...
This is an integer-only presentation of the restricted floor sum. -/
...
/-- Exact discrete derivative of the restricted floor sums:
`V_p(a+1) - V_p(a) = W_p(a+1)`, where `W_p(a+1)` is the number of admissible
`(q,r)` exponent pairs below `p^(a+1)`. -/
...
abstract fiber decomposition into the restricted floor sum. -/
...
/-- At a pure `p`-power cutoff, the exact restricted count is the
two-dimensional fiber sum. -/
...
/-- The identity `Ψ(p^a) = V_p(a)`: the strict three-prime smooth count at a
pure power equals the explicit restricted two-dimensional natural-log floor
sum. -/
...
two-dimensional fiber sums.  In adjacent jump notation this becomes
`m_N = V_q(b) - V_p(a)`. -/
...
multiplicity is exactly `V_q(b) - V_p(a)`. -/
...
digits are consecutive differences of a potential.  This is a recursive,
division-free weighted floor-sum formula for `R_(lo,h)`. -/
...
compression and is independent of the unresolved floor-sum
...
recurrence.  For Erdős #269, `smoothFactor` is the absorbed
...
/-- A cofinal form of local-window escape: for every admissible reduced
denominator and every requested starting point, some later finite window has
a residue too large for the prescribed short bound.  The carry contradiction
below needs only one such window for each denominator. -/
...
/-- A cofinal escape hypothesis rules out a positive reduced carry with the
matching short bound by supplying an escaping window after the lower bound
`0`.  The contradiction uses only that one window. -/
...
/-- The minimal local-window hypothesis for the carry contradiction: one
escaping window for each admissible reduced denominator.  No cofinality is
...
/-- A cofinal escape hypothesis implies the minimal one by taking the
requested lower bound to be zero. -/
