...
## Correction, 2026-09-06: the fiber irrationality in §5a is conditional

§5a writes, of the fiber values `T_{j,k} = Σ_{i≥0} 1/H(2^i·3^j·5^k)`:

> Each `T_{j,k}` is an all-ones Cantor series with bounded radices, hence
> **irrational** by the companion theorem (`PurePowerIrrationality.lean`).

Two corrections, in order of importance.

**1. The cited authority is conditional.** The companion theorem's proof
obtains its combinatorial input by asserting density of
`{(m·log_3 2, m·log_5 2)}` in the full two-torus, which needs ℚ-independence of
`1, log_3 2, log_5 2`. That independence is a four-exponentials instance and is
unproved; this repository records it as OPEN at `BlockKernelProgrammeLab.md`
§7.1. The dated correction block appended to `PurePowerIrrationalityNote.md` on
2026-09-06 states the theorem in its conditional form and assesses a proposed
repair by orbit closure that would restore the unconditional conclusion for the
channel `p = 2` of `{2,3,5}` without needing the independence. Until that repair
is written up, read §5a as: *each `T_{j,k}` is an all-ones Cantor series with
bounded radices, and is irrational under the hypothesis of the companion
theorem.*

**2. The cited file is the wrong authority.** `PurePowerIrrationality.lean`
proves no irrationality of any concrete series. Its own docstring says so: the
`clear` and `gaps` hypotheses of `irrational_of_clearing_and_small_gaps` are not
discharged there for `Σ₂` or for anything else, and no theorem in the file
establishes `Irrational Σ₂`. The authority for the fiber statement is
`PurePowerIrrationalityNote.md`, an ordinary proof with a kernel-checked
arithmetic core, and the citation should name the note.

**What is unaffected in §5a.** (F1), the fiber decomposition of `S + 1`; the
observation that every Cantor digit along a fiber equals `1`; (F2), the
factorised phase and the measured Lipschitz behaviour of `Φ`; (F3), the
covariance identity with maximum defect `7.8e−15`; and (F4). The only affected
sentence is the word "irrational" applied to `T_{j,k}`, and with it the summary
line that `S` is a lattice sum of *irrational* fiber values, which becomes a
lattice sum of fiber values that are irrational under the same hypothesis. The
`(0,0)` fiber value `1 + Σ₂ = 1.5931259677635827…` is unchanged as a number.
