...
```sh
./repo-python formal_math/probes/erdos269_fast_engine.py 3000
./repo-python formal_math/probes/erdos269_projective_cofinality.py --max-denominator 500 --max-start 3000
./repo-python formal_math/probes/erdos269_torus_fourier_representation.py --max-mode 320
./repo-python formal_math/probes/erdos269_torus_coding.py --max-a 300000 --verify-a 2600
./repo-python formal_math/probes/erdos269_escape_depth_law.py --max-start 20000 --max-q 120
./repo-python formal_math/probes/erdos269_stammering.py --max-a 400000 --max-q 150000
./repo-python formal_math/probes/erdos269_continued_fraction.py --start 1 --depth 15000
./repo-python formal_math/probes/erdos269_denominator_exclusion.py --start 1 --max-depth 420
```

---

## Correction, 2026-09-06: the stammering lane is deprioritised, not capped by Dirichlet

The section headed "Exact result: the stammering lane is closed" carries a
reason that does not hold. The measurement stands; the label and the mechanism
sentence change.

**The sentence being corrected.**

> The cap is structural rather than a search depth. Dirichlet in dimension two
> supplies simultaneous approximation only at exponent `1/2`, while the
> criterion needs exponent `1` for both slopes at the *same* denominator — a
> measure-zero condition on the slope pair.

Two faults.

**1. Dirichlet is an existence guarantee and caps nothing.** Dirichlet in
dimension two says that for *every* pair `(α, β)` there are infinitely many `q`
with `max(‖qα‖, ‖qβ‖) ≤ q^{-1/2}`. That is a lower bound on how well a pair can
be approximated, uniform over all pairs. It places no upper bound on how well
`(log_3 2, log_5 2)` in particular can be approximated. A pair with a larger
simultaneous exponent is not excluded by it. Reading "Dirichlet supplies only
exponent `1/2`" as "no pair exceeds exponent `1/2`" inverts the quantifier.

**2. The measure-zero remark is metric and says nothing about this pair.**
Exponent `1` at a common denominator is indeed a measure-zero condition on
`(α, β)`, by Khintchine–Groshev. Measure-zero sets contain points, and the
question is whether this specific pair is one of them. Nothing here decides it.

**A third fault, in the packet.** `research_packet.json` `negative_results[19]`
records the same retirement as: "because two rationally independent slopes cap
simultaneous approximation at Dirichlet exponent `1/2`". Beside the cap error,
the rational independence of `log_3 2` and `log_5 2` is itself unproved: it is
the four-exponentials instance recorded as OPEN at
`BlockKernelProgrammeLab.md` §7.1. The clause asserts an open statement as a
premise.

**Corrected label: `empirical_deprioritisation`.** The measurement is exact and
the conclusion it supports is a research-priority conclusion. Repetition
exponents at the record simultaneous-approximation denominators through
`q ≤ 150,000` decay to `1` like `1 + O(q^{-1/2})`, a full power short of what
the Adamczewski–Bugeaud criterion needs, and the failure margin is a power
rather than a constant, so extending the search is not the repair. That
deprioritises the lane and does not close it as a theorem.

**What the note already got right, and which now carries the whole weight.**
The "Claim boundary" paragraph directly below the corrected sentence: "This
retires the route on measured evidence plus the Dirichlet cap; it is not yet a
theorem. Turning it into one needs an effective lower bound
`max(‖q·log_3 2‖, ‖q·log_5 2‖) ≫ q^{-1+δ}`." Strike "plus the Dirichlet cap"
and that paragraph is exactly right, including its two negative results on the
available bounds: Laurent–Mignotte–Nesterenko gives
`|b₁log3 − b₂log5| > exp(−C (log B)²)`, weaker than any fixed power of `B`, and
Rhin's irrationality measure `≈ 5.117` for `log3/log2` gives only
`‖q·log_3 2‖ ≫ q^{-4.117}`. Naming that gap remains the useful output.

**Number correction in the same section.** The Diophantine-genericity
corroboration below quotes "empirical irrationality exponent `≈ 2`". The
authoritative receipt block
`state/formal_math/probes/erdos269_continued_fraction_receipt.json#result`
records `irrationality_exponent_proxy_max = 2.04373` and
`irrationality_exponent_proxy_tail_max = 1.00096`, alongside 13,109 certified
partial quotients, largest denominator 22,483 bits, largest partial quotient
129,114 and Lévy constant 1.18869. The `≈ 2` here is consistent with those and
needs no change. The figure `2.005` is **not** from that block: it comes from
`experiment_contract.result.summary` inside the same receipt file, which is a
stale block from an earlier and smaller run reporting 2,162 partial quotients
and a bound of `2^3737`. Do not quote the summary block.
