...
/-!
# Erdős #269 companion: irrationality of the pure prime-power running-LCM series
...
It is the sub-series carried by one prime channel, and unlike `S` it is provably
irrational.  The difference is exactly the obstruction that retired the torus
route for `S`:

* for `Σ₂` the normalized tail state is a function of the **radix word alone**
  (all Cantor digits are `1`), so states with the same radix prefix are close and
  states with different leading radices are separated;
* for `S` the digits are dyadic-shell lattice counts growing like `n²`, so the
  state is not a function of any compact coding at all.

This file formalises the arithmetic core of the argument, in a form that is
independent of the Sturmian combinatorics.
...
`p ≥ 3` are separated by at least `1/(3b)` (`stateGap_lower_bound`).

The Sturmian input, supplied outside this file, is that for every `L` the radix
word has a right-special factor of length `L` with two extensions differing by a
single non-dyadic channel.  Two occurrences `M ≠ M'` then satisfy
`y_M − y_M' = (y_{M+L} − y_{M'+L}) / P` with `P ≥ 2^L`, so the gap is nonzero
and tends to `0`.  `irrational_of_clearing_and_small_gaps` converts exactly that
into irrationality.

No claim about `S` itself is made here.
-/
